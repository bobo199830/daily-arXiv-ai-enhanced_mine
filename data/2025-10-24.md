...
Result: 在ModelNet10数据集上，监督对比损失达到约90.6%的准确率，证明ViT结合对比学习能有效捕捉全局形状语义和局部判别特征。

Conclusion: 通过大量实验验证，ViT与对比学习目标的结合为3D表示学习提供了有效解决方案，无需大量标注数据且能克服CNN的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习模型在医学图像分类中常产生过度自信的预测，影响其在关键医疗环境中的可靠性。传统标签平滑方法无法考虑类别间关系，将所有非目标类别同等对待。

Method: 使用在线标签平滑（OLS）方法，在训练过程中基于模型自身预测模式动态调整软标签。在RadImageNet数据集上评估了ResNet-50、MobileNetV2和VGG-19三种架构。

Result: OLS相比标准训练方法（包括硬标签、传统标签平滑和无教师知识蒸馏）持续提升了Top-1和Top-5分类准确率，并产生了更紧凑、分离度更好的特征嵌入。

Conclusion: OLS不仅增强了预测性能，还改善了校准效果，为医学影像领域开发可信赖的AI系统提供了实用有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 鱼眼摄像头能有效捕捉大范围视野，但其固有的径向失真和非均匀分辨率给标准目标检测器带来挑战，特别是在图像边界区域物体外观严重退化时。

Method: 采用简单有效的预处理和后处理管道，训练多个最先进的检测模型，并通过集成策略结合它们的输出来提高整体检测精度。

Result: 在2025年AI City Challenge Track 4中获得了0.6366的F1分数，在62个团队中排名第8。

Conclusion: 该框架有效解决了鱼眼图像固有的问题，证明了其在鱼眼交通图像检测中的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 当从显著超出训练数据分布的相机位置观看3D高斯泼溅模型时，常出现大量视觉噪声，这是由于在推断区域缺乏训练数据导致模型对密度、颜色和几何预测的不确定性。

Method: 利用从中间梯度导出的敏感性评分，明确针对由各向异性方向引起的不稳定性而非各向同性方差，实现实时渲染感知过滤。

Result: 实验评估表明，与BayesRays等现有基于神经辐射场的方法相比，该方法显著提高了视觉质量、真实感和一致性，并能实时集成到现有3DGS渲染流程中。

Conclusion: 该方法直接解决了生成不确定性的核心问题，使3D重建系统即使用户在原始训练视角外自由导航时也能保持高视觉保真度。
...
<details>
  <summary>Details</summary>
Motivation: 传统物理全波形反演方法受限于颅骨引起的信号衰减和稀疏孔径覆盖，而纯数据驱动方法在低信噪比条件下会产生定量偏差的声速图。

Method: 第一阶段使用逆时偏移处理多角度采集数据生成迁移片段；第二阶段使用基于transformer的超分辨率编码器-解码器与图注意力单元融合这些片段。采用可移动低计数换能器阵列的部分阵列采集策略。

Result: 在两个合成数据集上的实验表明，BrainPuzzle实现了优越的声速重建精度和图像完整性。

Conclusion: BrainPuzzle展示了推进定量超声脑成像的潜力，通过混合算法补偿缺失孔径，在低信噪比和稀疏孔径条件下产生准确的大脑声速图。
...
<details>
  <summary>Details</summary>
Motivation: 现有工作主要关注文本到图像系统的文化偏见，而图像到图像编辑器的文化偏见研究不足，需要建立一个标准化协议来比较诊断不同模型的文化表现。

Method: 使用六个国家的统一评估，包含8个类别36个子类别的模式，结合时代感知提示，采用自动指标、文化感知检索增强VQA和本地评审专家判断的综合评估框架。

Result: 研究发现：(1)国家无关提示下模型默认全球北方现代描绘；(2)迭代编辑侵蚀文化保真度；(3)图像到图像模型仅应用表面线索而非情境感知变化。

Conclusion: 当前系统中的文化敏感编辑不可靠，通过发布标准化数据、提示和评估协议，为诊断和跟踪生成图像模型中的文化偏见提供了可复现的文化中心基准。
...
<details>
  <summary>Details</summary>
Motivation: 视频评估和手术数据科学在手术培训、研究和质量改进中具有重要价值，但受限于异构的视频记录格式和视频共享的隐私问题，限制了其广泛应用。

Method: 采用软件开发生命周期和迭代式用户中心反馈方法，基于隐私设计架构开发Endoshare应用，通过内部和外部临床医生调查评估可用性和采用度，并在不同硬件配置上进行基准测试。

Result: 原型测试显示高可用性评分（临床医生4.68±0.40/5，计算机科学家4.03±0.51/5），改进后外科医生报告高感知有用性（5.07±1.75/7）、易用性（5.15±1.71/7）、启发式可用性（4.38±0.48/5）和强烈推荐（9.20±0.79/10）。处理时间受处理模式、视频时长和机器计算能力影响。
...
<details>
  <summary>Details</summary>
Motivation: 尽管扩散模型在图像生成方面取得了显著进展，但在生成基于像素的手绘素描（抽象表达的代表性示例）方面仍面临挑战。

Method: 1. 微调变分自编码器以优化潜在解码，更好地捕捉素描特征；2. 集成基于视觉问答的新奖励函数进行强化学习，改进文本-图像对齐和语义一致性。

Result: 大量实验表明，StableSketcher生成的素描具有改进的风格保真度，与Stable Diffusion基线相比，与提示词的对齐效果更好。

Conclusion: 该框架成功解决了手绘素描生成的挑战，并引入了首个包含实例级素描、标题和问答对的数据集SketchDUO，弥补了现有数据集的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 生物图像和描述性标题可以视为物种潜在形态空间中的互补样本，但获取大规模、实例特定的忠实标题存在挑战，限制了自然语言监督在生物领域的应用。

Method: 使用多模态大语言模型生成合成标题，结合维基百科的视觉信息和特定分类群的格式示例，减少幻觉并产生准确的实例描述性标题，然后训练BIOCAP模型。
...
<details>
  <summary>Details</summary>
Motivation: 虽然计算机视觉在一般物体检测和跟踪方面取得了显著进展，但快速移动的小物体检测问题仍未得到充分探索。本文旨在解决使用RGB-D相机检测和跟踪快速移动小物体的重大挑战。

Method: 系统结合了基于深度学习的检测和基于物理的跟踪方法，包括：(1) 用于3D空间中快速移动小物体检测和跟踪的综合系统设计；(2) 集成运动学方程的创新物理跟踪算法；(3) 用于处理遮挡和快速方向变化等挑战场景的异常值检测和校正模块。

Result: 在自定义壁球数据集上的评估显示，该系统显著优于基于卡尔曼滤波的跟踪器，平均位移误差减少高达70%。

Conclusion: 该系统在自主平台的机器人感知方面具有重要应用价值，证明了将物理模型与深度学习方法相结合对于实时3D检测和跟踪具有挑战性的小物体的有效性。
...
Method: 设计神经渲染管道：存储输入图像的光线流，通过交叉注意力计算关系，预测目标光线颜色。采用迭代生成策略更新遮挡内容。
...
Method: 提出LogitGap方法，显式利用最大logit与剩余logits之间的关系，并引入无训练策略自动识别最有信息的logits子集进行评分。

Result: 在视觉语言和纯视觉模型上的广泛实验表明，LogitGap在不同OOD检测场景和基准测试中一致达到最先进性能。

Conclusion: LogitGap通过有效利用logit空间关系显著提升了OOD检测性能，为开放世界应用中的深度学习模型可靠性提供了有效保障。
...
Method: 引入PartNeXt数据集，包含大规模纹理3D模型和层次化部件标注，并在类别无关部件分割和3D部件问答两个任务上进行基准测试。

Result: 在细粒度和叶级部件分割任务上，现有方法表现不佳；在3D部件问答任务中，3D-LLM在开放词汇部件定位方面存在显著差距；使用PartNeXt训练Point-SAM相比PartNet有显著提升。
...
<details>
  <summary>Details</summary>
Motivation: 在自动驾驶中，骑行者属于安全关键类别的弱势道路使用者，准确估计其姿态对于骑行者的穿越意图分类、行为预测和碰撞避免至关重要。传统6D姿态估计方法无法处理自行车铰接部件（车把、踏板）的变化，这些变化会影响3D边界框和实际行驶方向。

Method: 提出了一种联合估计8D姿态和3D关键点的方法，使用合成和真实图像数据混合训练，以在真实图像上实现泛化。8D姿态包括3D平移、3D旋转以及车把和踏板相对于自行车车架的旋转。
...
<details>
  <summary>Details</summary>
Motivation: 解决组合零样本学习中由于测试时包含未见属性-对象组合导致的分布偏移问题，现有方法在此问题上存在性能下降。

Method: 积累文本和视觉模态的无监督数据知识更新多模态原型，设计自适应更新权重控制原型调整程度，引入动态优先级队列存储高置信度图像获取历史视觉知识，通过多模态协同表示学习对齐文本和视觉原型。

Result: 在四个基准数据集上的闭集和开集设置下均达到最先进的性能。

Conclusion: 该方法能有效应对组合零样本学习中的分布偏移问题，在多个数据集上取得优异性能。
...
<details>
  <summary>Details</summary>
Motivation: 利用信息瓶颈框架优化GAN，解决InfoGAN在解耦表示学习中的局限性，实现更好的潜在空间解耦和样本生成质量。

Method: 在GAN生成器中引入中间随机层，约束输入与生成输出之间的互信息，形成可学习的潜在分布，与生成器联合端到端训练。
...
<details>
  <summary>Details</summary>
Motivation: 解决立体视频中时间一致深度估计的挑战，现有方法在长时依赖建模与计算效率之间存在根本性权衡，需要一种既能捕获长距离依赖又保持计算效率的方法。
...
Conclusion: PPMStereo通过两阶段协作过程实现了紧凑且信息丰富的记忆缓冲区，在准确性和时间一致性方面达到最先进性能，验证了方法的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注单个主体而非多交互人群场景，生成视频中多智能体动态的合理性尚未得到验证，需要建立严格的评估协议。

Method: 对于I2V使用现有数据集起始帧与真实视频对比；对于T2V开发提示套件探索不同行人密度和交互；关键方法是无需相机参数从像素空间重建2D鸟瞰轨迹。

Result: 分析显示领先模型已学习到有效的多智能体行为先验，但存在人物合并和消失等失败模式。

Conclusion: 当前模型在多智能体行为模拟方面表现出色，但人物合并和消失等问题指明了未来改进方向。
...
<details>
  <summary>Details</summary>
Motivation: 脑MRI基础模型的发展依赖于数据的规模、多样性和一致性，但缺乏对这些因素的系统评估。本研究旨在提供针对基础模型开发的结构化、多层次概述。

Method: 在数据集层面分析模态组成、疾病覆盖和规模；在图像层面量化体素间距、方向和强度分布；评估预处理变异性；使用3D DenseNet121进行特征空间案例研究。

Result: 发现健康队列与临床人群之间存在强烈不平衡，预处理步骤提高了数据集内一致性但数据集间差异仍然存在，标准化预处理后仍存在可测量的残差协变量偏移。
...
Method: RAPO++包含三个阶段：1）检索增强提示优化，从关系图中检索语义相关修饰符并重构提示以匹配训练分布；2）样本特定提示优化，使用多源反馈迭代优化提示；3）利用优化后的提示对微调重写器LLM。
...
<details>
  <summary>Details</summary>
Motivation: 当前文本到图像编辑方法采用目标无关的中间状态构建方式，主要关注源图像重建而忽略了与特定编辑目标的语义差距，导致在需要较大修改时编辑能力有限或不一致。

Method: FlowCycle通过可学习噪声参数化图像损坏过程，并通过循环一致性过程进行优化。该方法迭代地将源图像编辑到目标图像，然后恢复回源图像，利用双重一致性约束学习生成目标感知的中间状态。

Result: 大量消融实验表明，FlowCycle在编辑质量和一致性方面优于最先进的方法。

Conclusion: FlowCycle通过目标感知的中间状态设计和循环一致性优化，实现了更好的文本到图像编辑效果，在保持源图像一致性的同时能够进行忠实的修改。
...
<details>
  <summary>Details</summary>
Motivation: 准确的胎儿运动检测对评估产前健康至关重要，传统方法如母体感知和胎心监护存在主观性和准确性有限的问题。

Method: 采用自监督对比学习框架，结合空间和时间对比学习的双重对比损失，引入任务特定采样策略，并通过概率微调方法实现任意长度超声记录的分析。

Result: 在包含92名受试者的内部数据集上，每个受试者有30分钟超声记录，CULR达到78.01%的敏感度和81.60%的AUROC。
...
Method: 提出EditInfinity方法，基于二进制量化生成模型Infinity，设计了高效的图像反转机制，包括文本提示修正和图像风格保留，并开发了整体平滑策略以实现高保真度的图像编辑。

Result: 在PIE-Bench基准测试中，在"添加"、"更改"和"删除"编辑操作上，相比最先进的基于扩散模型的方法表现出优越性能。

Conclusion: EditInfinity通过精确的图像反转和整体平滑策略，实现了对源图像的高保真度和与文本提示的精确语义对齐，为文本驱动图像编辑提供了更有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在生成长文本时容易出现幻觉问题，传统观点认为是长度导致的错误累积。本文旨在探究幻觉是否仅由长度引起，还是有更深层的机制。

Method: 提出"诱导-检测-抑制"框架：通过精心设计的上下文主动诱导幻觉，利用诱导实例进行早期高风险检测，在实际解码过程中抑制潜在的对象级幻觉。

Result: 该方法在所有基准测试中都取得了持续显著的改进，证明了其有效性。强检测能力和改进的幻觉缓解不仅验证了框架，更重要的是重新验证了关于上下文的假设。

Conclusion: 本研究不仅追求性能提升，更旨在提供新的见解，作为深入探索大型视觉语言模型长文本响应中幻觉问题的第一步。幻觉风险不是由长度本身引起，而是由长文本中对上下文连贯性和完整性的依赖增加所导致。
...
Method: 引入协作场的概念，包含实例场和语言场，通过实例到语言的特征映射和两阶段训练策略构建协作场，并在推理时设计自适应语言到实例提示优化。

Result: 在两个广泛使用的基准测试中表现出领先性能，并在新颖图像3D分割、分层分割和机器人等应用中显示出高潜力。

Conclusion: COS3D通过协作场有效整合语言和分割线索，显著提升了开放词汇3D分割的性能和适用性。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频时序定位方法在处理跨模态注意力时对所有文本标记一视同仁，忽视了它们不同的语义角色。实验表明这些模型过度依赖[EOS]驱动的全局语义，而未能有效利用词级信号，限制了细粒度时序对齐能力。

Method: 提出DualGround双分支架构：1）基于token角色的跨模态交互策略，以结构解耦方式对齐视频特征与句子级和短语级语义；2）联合建模框架，不仅改进全局句子级对齐，还通过结构化短语感知上下文增强细粒度时序定位。

Result: DualGround在QVHighlights和Charades-STA基准测试中，在时刻检索和高亮检测两个任务上都达到了最先进的性能。

Conclusion: 解耦语义建模在视频语言对齐中具有有效性，DualGround通过分离全局和局部语义，能够同时捕捉粗粒度和局部化语义，实现更具表达力和上下文感知的视频定位。
...
TL;DR: 本文提出EDGeo框架，通过掩码位置编码和上下文增强模块解决跨视角物体地理定位问题，在CVOGL和VIGOR-Building数据集上实现最先进性能，定位精度提升3.39%。
...
Method: 提出掩码位置编码方案利用分割掩码捕获空间坐标和物体轮廓，设计上下文增强模块使用水平和垂直条带卷积核提取长距离上下文特征，集成形成EDGeo端到端框架。

Result: 在两个公共数据集上的广泛实验表明，该方法在具有挑战性的地面到卫星场景下实现了最先进的性能，定位精度提高了3.39%。
...
Method: CMC模型包含伪标签生成模块进行自监督单模态预训练，参数无关融合模块和多模态共识路由器进行多模态微调。

Result: 在CH-SIMS、CH-SIMS v2、CMU-MOSI、CMU-MOSEI四个数据集上达到或超越最先进方法性能，在语义不一致场景下优势明显。

Conclusion: CMC能有效缓解文本主导问题，引导融合过程达成更可靠共识，在多模态情感识别任务中表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 智能手机已成为日常生活必需品，包括对视障人士。日常任务如处理货币而不依赖他人对他们来说可能很麻烦。本文旨在通过技术手段帮助视障人士更独立地处理货币。

Method: 使用YOLOv8 nano模型，配备具有深度卷积层和Squeeze-and-Excitation块的自定义检测头，以增强特征提取和检测精度。模型在包含30类纸币和硬币的数据集上进行训练。

Result: 模型实现了97.73%的准确率、95.23%的召回率、95.85%的F1分数，以及97.21%的mAP50(B)。检测后通过语音反馈帮助视障人士识别货币。

Conclusion: 本文创建了一个实用高效的货币检测系统，旨在增强视障人士在处理货币方面的独立性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在引入文本特征时过于粗糙，忽略了视频片段中可能存在的大量冗余信息，需要更细粒度的多模态特征融合

Method: 基于视频片段生成细粒度多模态特征，总结主要内容，并引入原始视频字幕的文本特征来增强视觉特征的突出部分

Result: 在四个主要数据集上达到最先进性能，消融实验验证改进源于冗余信息的减少

Conclusion: GMFVAD通过减少冗余信息有效提升了视频异常检测性能
...
Conclusion: 提出的VCR-OOD数据集和去偏见方法能够有效评估和改进模型的泛化能力，消除数据集中的偏见影响。
...
TL;DR: 提出KINN框架解决复数SAR图像识别中的表示三难问题，通过物理引导压缩和语义压缩实现参数高效识别，在数据稀缺和域偏移场景下表现优异。


<details>
  <summary>Details</summary>
Motivation: 解决复数SAR图像识别中泛化性、可解释性和效率之间的表示三难问题，充分利用SAR数据中丰富的电磁散射特征。

Method: 基于'压缩-聚合-压缩'架构的KINN框架，包含物理引导压缩、特征聚合和语义压缩三个阶段，采用字典处理器嵌入物理先验，结合自蒸馏学习任务相关特征。

Result: 在五个SAR基准测试中达到参数高效识别的最先进水平，在数据稀缺和分布外场景下具有优异的泛化能力和可解释性。
...
Method: DMC³框架包含三个模块：基础模型、反事实样本构建模块（通过事件描述改写和核心交互挖掘生成正负样本）和反事实样本参与的对比优化模块（使用对比损失优化特征距离）。

Result: 在EgoTaskQA的normal和indirect分割上分别达到52.51%和46.04%，在QAEGO4D上达到13.2%，均达到最先进性能。

Conclusion: 提出的DMC³框架有效解决了第一人称视频问答中的独特挑战，在多个基准测试中取得了最先进的性能。
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型已在多个体育应用中得到广泛应用，但大多集中在足球、板球、篮球等流行运动，专注于视觉问答、精彩片段生成等生成任务。本研究旨在探索现代视频基础模型在街舞这一小众但非常流行的舞蹈运动中的适用性。

Method: 使用现代视频基础模型（包括编码器和解码器），对街舞视频进行分类任务，比较视频编码器模型和视频语言模型的性能表现。

Result: 研究结果显示，视频编码器模型在预测任务中持续优于最先进的视频语言模型。

Conclusion: 提供了如何选择编码器模型的见解，并对用于街舞视频分类的微调解码器模型的工作原理进行了深入分析。
...
<details>
  <summary>Details</summary>
Motivation: 解决多平台（卫星/无人机/地面）图像检索中的严重异构性问题和通用训练描述与平台特定测试查询之间的领域差距。

Method: 使用平台划分、卫星增强和方向词移除的预处理，以及基于LLM的标题精炼管道；采用BGE-M3和EVA-CLIP模型，通过两阶段硬负样本挖掘策略训练三个平台专家，并在推理时融合得分。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型通常需要大量计算资源进行训练，主要原因是广泛使用的视觉编码器（如CLIP和SAM）缺乏与语言在多粒度级别的对齐。

Method: 提出HyperET训练范式，利用双曲空间建模层次结构，通过可学习矩阵和Möbius乘法操作实现动态双曲半径调整，采用三种参数化策略：对角缩放矩阵、块对角矩阵和带状矩阵。

Result: 在多个MLLM基准测试中，HyperET仅需增加不到1%的参数就能显著提升现有预训练和微调模型的性能。

Conclusion: HyperET提供了一种高效的多模态对齐方法，通过双曲空间有效解决了视觉与文本模态之间的粒度差距问题。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型在采样过程中，量化误差会在多步去噪过程中累积，导致性能下降。现有方法通常独立处理每一步的误差，无法有效解决累积误差问题。

Method: AccuQuant通过显式模拟多步去噪过程来最小化全精度模型与量化版本之间的输出差异，同时提出高效的实现技术将内存复杂度从O(n)降低到O(1)。

Result: 在多个任务和扩散模型的标准基准测试中，AccuQuant展现出优异的效能和效率。

Conclusion: AccuQuant通过考虑多步去噪过程中的累积误差，提供了一种高效且准确的扩散模型量化解决方案。
...
Result: PE-Field增强的DiT在单图像新视角合成方面实现了最先进性能，并能推广到可控的空间图像编辑。

Conclusion: PE-Field使DiTs能够直接在3D空间中建模几何形状，为视觉生成提供了更强大的架构基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有图像-食谱检索方法假设食物图像能完全捕捉食谱中的细节，但实际上图像只反映烹饪结果而非过程，导致表示学习偏向主导视觉元素，难以区分具有细微差异的相似食谱。
...
Conclusion: 因果表示学习方法能有效缓解图像-食谱检索中的视觉偏差问题，提高对细微烹饪元素的识别能力，在单语和多语言多文化场景下均表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 解决传统静态知识蒸馏模型在处理肺癌图像诊断中不确定性和复杂性时的局限性，通过动态调整蒸馏权重来更好地处理图像中不同区域的不确定性水平。

Method: 采用动态模糊逻辑驱动的知识蒸馏方法，使用ViT-B32作为教师模型，MobileNet作为学生模型；引入像素级图像融合技术（Gamma校正和直方图均衡化）提升图像质量；使用小波融合方法改善特征表示；采用遗传算法从12个候选模型中选取最优学生模型。

Result: 在LC25000组织病理学图像数据集上达到99.16%准确率，在IQOTH/NCCD CT扫描图像数据集上达到99.54%准确率，证明了模型在不同成像领域的鲁棒性。

Conclusion: FuzzyDistillViT-MobileNet模型通过动态模糊逻辑知识蒸馏有效提升了肺癌分类性能，能够处理图像中的不确定性，并在不同数据集上表现出优异的准确率和泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于强化学习的方法依赖纯文本链导致结论缺乏视觉基础，而基于帧检索的方法虽然引入视觉基础但证据定位不准确，需要解决这些挑战。

Method: 构建Conan-91K大规模数据集，包含自动生成的推理轨迹（帧识别、证据推理、动作决策），并设计多阶段渐进冷启动策略与识别-推理-动作强化学习训练框架。

Result: 在六个多步推理基准测试中，Conan相比基线Qwen2.5-VL-7B-Instruct平均准确率提升超过10%，达到最先进性能，并能有效泛化到长视频理解任务。

Conclusion: Conan框架通过证据基础的多步推理显著提升了视频推理能力，具有良好的可扩展性和鲁棒性。
...
### [43] [EchoDistill: Bidirectional Concept Distillation for One-Step Diffusion Personalization](https://arxiv.org/abs/2510.20512)
*Yixiong Yang,Tao Wu,Senmao Li,Shiqi Yang,Yaxing Wang,Joost van de Weijer,Kai Wang*

Main category: cs.CV

TL;DR: EchoDistill提出了一种双向概念蒸馏框架，用于实现一步扩散个性化（1-SDP），通过师生模型的协同训练和双向反馈机制，在保持生成质量的同时加速个性化过程。


<details>
  <summary>Details</summary>
Motivation: 现有的一步文本到图像扩散模型在加速生成方面取得进展，但在个性化新概念方面存在局限，难以有效捕捉新概念的分布。

Method: 采用端到端训练，同时训练多步扩散模型（教师）和一步扩散模型（学生），通过双向概念蒸馏机制，包括概念从教师到学生的蒸馏和从学生到教师的回传，共享文本编码器确保语义一致性，并引入双向回传精炼策略。

Result: 实验表明该协作框架在1-SDP设置下显著优于现有个性化方法，不仅增强了学生模型对新概念的个性化能力，还提高了教师模型的生成质量。

Conclusion: EchoDistill为T2I扩散模型中的快速有效个性化建立了一种新颖范式，通过双向概念蒸馏实现了高质量的单步个性化生成。
...
### [44] [Metis-HOME: Hybrid Optimized Mixture-of-Experts for Multimodal Reasoning](https://arxiv.org/abs/2510.20519)
*Xiaohan Lan,Fanfan Liu,Haibo Qiu,Siqi Yang,Delian Ruan,Peng Shi,Lin Ma*

Main category: cs.CV

TL;DR: Metis-HOME是一个混合优化的专家混合框架，通过将原始密集模型分为思考分支和非思考分支来解决多模态推理模型在效率与泛化能力之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 当前多模态推理模型存在两个关键限制：对简单查询也使用计算昂贵的推理导致效率低下，以及专注于专门推理损害了更广泛的通用理解能力。

Method: 将Qwen2.5-VL-7B适配为MoE架构，创建两个专家分支：思考分支用于复杂多步推理，非思考分支用于快速直接推理任务。使用轻量级可训练路由器动态分配查询。
...
### [45] [Fake-in-Facext: Towards Fine-Grained Explainable DeepFake Analysis](https://arxiv.org/abs/2510.20531)
*Lixiong Qin,Yang Zhang,Mei Wang,Jiani Hu,Weihong Deng,Weiran Xu*

Main category: cs.CV

TL;DR: 提出了Fake-in-Facext (FiFa)框架，通过细粒度面部区域划分和伪影定位解释任务，解决了现有可解释深度伪造分析方法在细粒度感知方面的不足。
...
Method: 首先定义面部图像概念树(FICT)将面部图像划分为细粒度区域概念，构建FiFa-Annotator标注管道；提出伪影定位解释(AGE)任务，生成包含分割掩码的文本伪造解释；设计统一的多任务学习架构FiFa-MLLM，支持丰富的多模态输入输出。

Result: FiFa-MLLM在AGE任务上优于强基线模型，在现有XDFA数据集上达到最先进性能。

Conclusion: FiFa框架通过细粒度数据标注和统一的多任务架构，显著提升了可解释深度伪造分析的细粒度感知能力，为深度伪造检测提供了更可靠的解释依据。
...
### [46] [Deep Learning-Powered Visual SLAM Aimed at Assisting Visually Impaired Navigation](https://arxiv.org/abs/2510.20549)
...
<details>
  <summary>Details</summary>
Motivation: 当前SLAM技术在低纹理、运动模糊和挑战性光照条件下的鲁棒性仍然不足，这些问题在视觉障碍者辅助导航等应用中尤为突出，会影响定位精度和跟踪稳定性。
...
### [47] [From Far and Near: Perceptual Evaluation of Crowd Representations Across Levels of Detail](https://arxiv.org/abs/2510.20558)
*Xiaohan Sun,Carol O'Sullivan*

Main category: cs.CV

TL;DR: 研究用户在不同细节层次和观看距离下对人群角色视觉质量的感知，比较几何网格、图像替代、NeRF和3D高斯等表示的视觉保真度与计算性能权衡。


<details>
  <summary>Details</summary>
Motivation: 探索不同人群角色表示方法在视觉质量和计算性能之间的平衡，为设计感知优化的细节层次策略提供指导。

Method: 使用几何网格、图像替代、Neural Radiance Fields (NeRF) 和3D高斯等不同表示方法，在不同细节层次和观看距离下进行定性和定量分析。

Result: 每种表示方法在视觉保真度和计算性能方面表现出不同的权衡关系，研究结果为人群渲染的感知优化提供了具体见解。

Conclusion: 研究为设计感知优化的细节层次策略提供了有价值的指导，帮助在人群渲染中平衡视觉质量和计算效率。
...
TL;DR: 提出了GenColorBench，这是首个针对文本到图像颜色生成的综合基准，基于ISCC-NBS和CSS3/X11颜色系统，包含44K个颜色焦点提示，评估了流行文本到图像模型在精确颜色生成方面的表现。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像生成模型在细粒度颜色可控性方面表现不佳，现有基准要么忽视颜色评估，要么依赖粗略评估，缺乏对RGB值解释和人类期望对齐等关键能力的系统评估。

Method: 基于ISCC-NBS和CSS3/X11颜色系统构建包含44K个颜色焦点提示的基准，涵盖400多种颜色，通过感知和自动化评估来揭示模型的真实能力。
...
TL;DR: 提出了一种基于相似性原型的跨模态分割框架，通过学习嵌入空间中的类别原型并引入相似性约束，解决无监督领域自适应中的领域偏移问题。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在面临领域偏移时性能会急剧下降，而无监督领域自适应可以避免对未见领域进行昂贵标注，因此需要开发能够减少领域差距的方法。

Method: 在嵌入空间中学习类别原型，引入相似性约束使原型对每个语义类别具有代表性且不同类别间可分；使用字典存储来自不同图像的原型，避免类别缺失问题并实现原型对比学习。

Result: 大量实验表明，该方法比其他最先进的方法取得了更好的结果。

Conclusion: 提出的基于相似性原型的框架在跨模态分割任务中表现出色，通过原型对比学习有效提升了性能。
...
TL;DR: OnlineSplatter是一个在线前馈框架，直接从RGB帧生成高质量的对象中心3D高斯表示，无需相机姿态、深度先验或捆绑优化。
...
Method: 使用第一帧作为重建锚点，通过密集高斯基元场逐步细化物体表示；核心贡献是双键记忆模块，结合潜在外观-几何键和显式方向键，通过空间引导记忆读取和高效稀疏化机制融合当前帧特征与时间聚合的物体状态。

Result: 在真实世界数据集上的评估表明，OnlineSplatter显著优于最先进的无姿态重建基线，随着观察次数的增加性能持续提升，同时保持恒定的内存和运行时间。

Conclusion: 该方法能够有效处理自由移动物体，提供全面而紧凑的物体覆盖，在保持恒定计算成本的同时实现高质量重建。
...
TL;DR: SeViCES是一个无需训练、模型无关的框架，通过语义-视觉共识证据选择来解决长视频理解问题，包含SVCFS模块进行帧选择和ACR模块进行答案一致性优化。


<details>
  <summary>Details</summary>
Motivation: 长视频理解面临计算成本高、推理不聚焦和不一致的问题，现有方法忽视时间依赖或依赖单模态证据，无法提供完整且与查询相关的上下文。

Method: 提出SeViCES框架，包含SVCFS模块（时间感知语义分支和聚类引导视觉分支）和ACR模块（融合证据和约束答案空间）。
...
TL;DR: 本文提出了UltraHR-100K数据集和频率感知后训练方法，解决了超高分辨率文本到图像生成中的数据集缺失和细节合成问题。
...
Method: 1) 构建UltraHR-100K数据集（10万张超过3K分辨率的图像）；2) 提出频率感知后训练方法，包括面向细节的时间步采样(DOTS)和软加权频率正则化(SWFR)。

Result: 在UltraHR-eval4K基准测试上的广泛实验表明，该方法显著提高了UHR图像生成的细粒度细节质量和整体保真度。

Conclusion: 提出的数据集和训练方法有效解决了UHR T2I生成中的关键问题，显著提升了图像细节质量。
...
<details>
  <summary>Details</summary>
Motivation: 现有KV缓存压缩方法主要关注保留高重要性KV对来最小化存储，但往往忽略了多模态KV缓存中出现的模态特定语义冗余模式，这限制了部署的可扩展性。

Method: MixKV方法适应头级别的语义冗余，在压缩KV对时选择性平衡多样性和重要性，通过分析KV缓存在不同注意力头中表现出不同程度的冗余。

Result: 在极端压缩条件下（预算=64），MixKV在五个多模态理解基准上平均提升基线方法5.1%，在GUI接地任务上为SnapKV和AdaKV分别带来8.0%和9.0%的显著增益，同时保持相当的推理效率。

Conclusion: MixKV通过混合重要性和多样性有效解决了多模态KV缓存压缩问题，不仅显著提升了现有方法的性能，还能无缝扩展到LLMs并获得可比较的性能增益。
...
Method: 通过自动逆向工程推断激光束配置、角度分布和每束校准校正等关键参数，实现无损投影。
...
TL;DR: AutoScape是一个长时域驾驶场景生成框架，通过RGB-D扩散模型迭代生成稀疏、几何一致的关键帧，然后使用视频扩散模型进行插值，生成超过20秒的逼真且几何一致的驾驶视频。


<details>
  <summary>Details</summary>
Motivation: 解决长时域驾驶场景生成中几何一致性的挑战，现有方法在长距离场景生成中难以保持几何一致性。

Method: 1) 在共享潜在空间中联合处理图像和深度；2) 显式条件化先前生成关键帧的现有场景几何；3) 使用扭曲一致引导来指导采样过程；4) 通过视频扩散模型在关键帧之间插值生成密集视频帧。

Result: 生成了超过20秒的现实且几何一致的驾驶视频，在长时域FID和FVD指标上分别比现有最优方法提升了48.6%和43.0%。

Conclusion: AutoScape通过几何一致的关键帧生成和视频插值，有效解决了长时域驾驶场景生成中的几何一致性问题，显著提升了生成质量。
...
### [56] [ACS-SegNet: An Attention-Based CNN-SegFormer Segmentation Network for Tissue Segmentation in Histopathology](https://arxiv.org/abs/2510.20754)
...
<details>
  <summary>Details</summary>
Motivation: 深度学习在组织病理学图像分析中表现出色，但现有方法在语义分割方面仍有提升空间，需要结合CNN和ViT的优势来改进性能。

Method: 使用注意力驱动特征融合的CNN和ViT双编码器统一模型，结合两种架构的优势进行语义分割。

Result: 在GCPS数据集上达到μIoU/μDice分数76.79%/86.87%，在PUMA数据集上达到64.93%/76.60%，超越了现有最佳基准。

Conclusion: 提出的双编码器模型有效提升了组织病理学图像语义分割性能，代码已在GitHub公开。
...
TL;DR: DyPE是一种无需训练的方法，通过动态调整扩散过程中位置编码的频率谱，使预训练扩散变换器能够生成远超训练分辨率的高质量图像，且不增加采样成本。
...
Conclusion: DyPE是一种有效的训练免费方法，能够显著扩展预训练扩散变换器的分辨率能力，在超高分辨率图像生成中表现优异，且分辨率越高优势越明显。
...
<details>
  <summary>Details</summary>
Motivation: 解决从单张2D图像进行3D重建的挑战，特别是在相机姿态、形状和纹理的联合估计方面。现有方法往往在这些任务上表现不一致，需要一个统一的生成框架来同时处理这些要素。

Method: 将3D重建建模为从学习到的3D对象分布中进行条件采样的过程。采用两阶段流匹配流程：粗阶段生成初始3D几何和2D投影用于姿态恢复；精炼阶段整合姿态对齐的图像特征来增强结构保真度和外观细节。

Result: 在广泛实验中，Cupid优于领先的3D重建方法，PSNR增益超过3dB，Chamfer距离减少超过10%，同时在姿态精度上匹配单目估计器，在视觉保真度上优于基线3D生成模型。

Conclusion: Cupid通过统一的生成框架成功实现了从单张图像进行准确的3D重建，在姿态估计、形状重建和纹理恢复方面都表现出色，为3D重建领域提供了新的有效解决方案。
...
TL;DR: 提出Speculative Verdict (SV)框架，通过结合多个轻量级草稿专家和大型裁决模型，在无需训练的情况下解决信息密集型图像的多模态推理问题，实现错误校正和成本效率。
...
Method: 采用受推测解码启发的训练免费框架，包含草稿阶段和裁决阶段。草稿阶段使用小型VLM作为草稿专家生成多样化定位候选的推理路径；裁决阶段由强大VLM合成这些路径生成最终答案，并引入共识专家选择机制仅转发高一致性推理路径。

Result: 在具有挑战性的信息密集和高分辨率视觉问答基准测试（包括InfographicVQA、ChartMuseum、ChartQAPro和HR-Bench 4K）上取得一致性能提升。

Conclusion: 通过从多个部分准确的推理路径中合成正确见解，SV相比大型专有模型或训练流程实现了错误校正和成本效率。
...
### [60] [Radar-Camera Fused Multi-Object Tracking: Online Calibration and Common Feature](https://arxiv.org/abs/2510.20794)
...
<details>
  <summary>Details</summary>
Motivation: 现有研究大多将雷达置于辅助角色，未能充分利用雷达提供准确3D坐标信息的能力。本文旨在让雷达发挥关键作用，同时通过在线校准减少人工干预。
...
Conclusion: 这是首个研究雷达-相机共同特征集成及其在在线校准中应用以实现多目标跟踪的工作，为传感器融合跟踪提供了有效解决方案。
...
### [61] [ARGenSeg: Image Segmentation with Autoregressive Image Generation Model](https://arxiv.org/abs/2510.20803)
*Xiaolong Wang,Lixiang Ru,Ziyuan Huang,Kaixiang Ji,Dandan Zheng,Jingdong Chen,Jun Zhou*

Main category: cs.CV

TL;DR: ARGenSeg是一种基于自回归生成的新型图像分割范式，通过统一的框架实现多模态理解和像素级感知，利用图像生成方法产生密集目标掩码，显著提升推理速度并保持强大的理解能力。
...
Conclusion: ARGenSeg通过统一的生成式框架成功实现了多模态理解和像素级感知，为图像分割任务提供了高效且性能优越的解决方案。
...
### [62] [SpectraMorph: Structured Latent Learning for Self-Supervised Hyperspectral Super-Resolution](https://arxiv.org/abs/2510.20814)
*Ritik Shah,Marco F Duarte*

Main category: cs.CV

TL;DR: SpectraMorph是一个基于物理指导的自监督高光谱超分辨率框架，通过解混瓶颈结构实现高光谱和多光谱图像的融合，具有可解释性、快速训练和鲁棒性强的特点。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法虽然性能强大但缺乏可解释性，且在MSI波段很少时容易失效。需要开发既能保持高性能又具有物理可解释性的融合方法。

Method: 采用解混瓶颈结构：从低分辨率HSI提取端元特征，用紧凑MLP从MSI预测丰度图，通过线性混合重建光谱，使用MSI传感器光谱响应函数进行自监督训练。

Result: 在合成和真实数据集上，SpectraMorph始终优于最先进的无监督/自监督基线方法，同时与监督基线方法保持很强竞争力。

Conclusion: SpectraMorph框架成功实现了高性能、可解释性和鲁棒性的平衡，即使在单波段MSI情况下也能保持良好性能，训练时间不到一分钟。
...
### [63] [Towards General Modality Translation with Contrastive and Predictive Latent Diffusion Bridge](https://arxiv.org/abs/2510.20819)
*Nimrod Berman,Omkar Joglekar,Eitan Kosman,Dotan Di Castro,Omri Azencot*

Main category: cs.CV

TL;DR: 提出LDDBM（潜在去噪扩散桥模型），一种基于潜在变量扩展的通用模态转换框架，可在共享潜在空间中学习任意模态间的转换，无需对齐维度。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在单模态领域表现出色，但扩展到跨模态转换仍面临挑战。现有方法依赖共享维度、高斯先验等限制性假设，缺乏通用性和理论基础。

Method: 在共享潜在空间中运行，引入对比对齐损失确保语义一致性，设计领域无关的编码器-解码器架构用于潜在空间噪声预测，提出预测损失指导跨域转换训练。

Result: 在多种模态转换任务中表现优异，包括多视角到3D形状生成、图像超分辨率、多视角场景合成等，验证了框架的有效性。

Conclusion: LDDBM建立了通用模态转换的新强基线，支持任意模态对，为跨模态转换提供了通用解决方案。
...
TL;DR: HoloCine模型通过整体生成整个场景来解决文本到视频模型的叙事连贯性问题，实现了从第一个镜头到最后一个镜头的全局一致性，标志着从片段合成到自动化电影制作的关键转变。


<details>
  <summary>Details</summary>
Motivation: 解决现有文本到视频模型在生成连贯、多镜头叙事方面的不足，这些叙事是故事讲述的本质。

Method: 采用Window Cross-Attention机制将文本提示定位到特定镜头，以及Sparse Inter-Shot Self-Attention模式（镜头内密集，镜头间稀疏）确保分钟级生成的效率。

Result: 在叙事连贯性方面达到新的最先进水平，并展现出对角色和场景的持久记忆以及对电影技术的直观理解等显著涌现能力。
...
TL;DR: DeBERTa-KC模型用于在线科学学习讨论中的知识构建水平自动分类，在YouTube科学频道评论数据集上表现优异，显著优于传统和transformer基线模型。


<details>
  <summary>Details</summary>
Motivation: 开发能够自动识别在线科学学习环境中知识构建水平的模型，为话语分析和认知参与评估提供可扩展的理论驱动方法。

Method: 基于DeBERTa-v3扩展，结合Focal Loss、Label Smoothing和R-Drop正则化处理类别不平衡和增强泛化能力，构建端到端可复现流程。

Result: 在10折分层交叉验证中达到0.836±0.008的宏F1分数，显著优于基线模型，对高阶认知参与（特别是Explore和Negotiate类别）表现出强敏感性。

Conclusion: 大语言模型能够有效捕捉非正式数字学习环境中知识构建的细微指标，为话语分析和认知参与评估工具开发提供了可行方案。
...
TL;DR: 本研究评估了五个主要大语言模型生成教案的教学合理性和可用性，发现模型选择对语言可读性影响最大，而提示框架结构对事实准确性和教学完整性影响最显著。最佳配置是将可读性优化的模型与RACE框架及明确的教学概念清单相结合。


<details>
  <summary>Details</summary>
Motivation: 评估AI生成教案的教学质量和可用性，为教育工作者提供选择模型和提示框架的指导，确保生成内容符合教学标准。

Method: 使用五个大语言模型和三种提示框架生成15个高中物理电磁波谱主题的教案，通过四个自动化计算指标进行分析：可读性、事实准确性、课程标准对齐和认知需求。

Result: DeepSeek生成最易读的教案，Claude语言最密集；RACE框架产生最低的幻觉指数和最高的课程标准对齐；所有模型的学习目标主要集中在布鲁姆分类法的低阶认知层次。

Conclusion: 可读性主要由模型设计决定，而教学可靠性和课程对齐更依赖提示框架。最佳实践是结合可读性优化的模型、RACE框架和明确的教学概念清单。
...
TL;DR: ReDiff是一个精炼增强的扩散框架，通过主动修正而非被动去噪来解决离散扩散模型中的训练-推理差异问题，有效打破错误级联。


<details>
  <summary>Details</summary>
Motivation: 离散扩散模型在视觉语言任务中面临训练-推理差异的挑战，初始token错误会导致错误级联，引发语法错误和语义幻觉。

Method: 采用两阶段训练：首先训练模型修正合成错误建立基础修正能力；然后通过在线自修正循环，让模型学习修正自己的错误草稿。

Result: 实验表明ReDiff显著提高了生成内容的连贯性和事实准确性，实现了比传统去噪方法更稳定高效的并行生成。

Conclusion: ReDiff通过主动精炼框架成功解决了离散扩散模型的错误级联问题，为并行生成提供了稳定高效的解决方案。
...
TL;DR: 提出Sparse Tracing技术，通过动态稀疏注意力高效分析长上下文注意力模式，Stream算法以近线性时间和线性空间估计稀疏注意力掩码，实现大规模单次可解释性。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型扩展到百万token上下文，传统机制可解释性技术因二次方扩展性而需要TB级内存，无法处理超过10万token的上下文。
...
TL;DR: 本研究提出了一种利用大型语言模型分析非结构化电子健康记录文本以确定患者HIV检测资格的新流程，在保持低假阴性率的同时实现了高准确率。


<details>
  <summary>Details</summary>
Motivation: HIV的高效筛查和早期诊断对于减少传播至关重要，但大规模实验室检测不可行。现有研究主要关注结构化数据，忽略了可能包含HIV风险相关信息的非结构化临床文本数据。

Method: 提出了一种新颖的流程，利用大型语言模型分析非结构化电子健康记录文本，评估患者进行进一步HIV检测的资格。

Result: 在伊拉斯姆斯大学医学中心鹿特丹的临床数据上进行的实验表明，该流程实现了高准确率，同时保持了低假阴性率。

Conclusion: 利用大型语言模型分析非结构化电子健康记录文本是一种有效的HIV筛查方法，能够提高诊断效率并减少传播风险。
...
TL;DR: 本研究首次通过专家基准测试评估了11个通用大语言模型在生命周期评估(LCA)中的表现，发现37%的响应包含不准确或误导性信息，幻觉率高达40%，但某些模型在解释质量和格式遵循方面表现良好。


<details>
  <summary>Details</summary>
Motivation: 人工智能和大语言模型越来越多地被用于支持生命周期评估，但缺乏对其可靠性、鲁棒性和可用性的系统性证据。本研究旨在填补这一空白，提供首个基于专家评估的LLM在LCA领域的基准测试。

Method: 评估了11个通用LLM（包括商业和开源模型）在22个LCA相关任务上的表现，由17位经验丰富的从业者根据科学准确性、解释质量、鲁棒性、可验证性和指令遵循等标准审查模型输出，共收集168份专家评审。

Result: 专家判断37%的响应包含不准确或误导信息；准确性和解释质量评分普遍为平均或良好；格式遵循评分普遍较好；幻觉率差异显著，某些模型产生幻觉引用的比例高达40%；开源模型在准确性和解释质量等标准上与闭源模型表现相当或更优。

Conclusion: 研究结果强调了在LCA中简单应用LLM的风险，特别是将其视为自由形式的神谕时，同时也显示了其在解释质量和减轻简单任务劳动强度方面的益处。在没有基础机制的情况下使用通用LLM存在风险。
...
TL;DR: 本文提出使用游戏化评估方法（特别是Dixit卡牌游戏）来全面评估多模态大语言模型的能力，克服传统静态基准测试和主观比较的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统多模态大语言模型评估存在三个主要问题：静态基准无法综合评估能力、人工或模型比较主观且昂贵、模型可能利用表面特征（如冗长性）来提升胜率。

Method: 采用游戏化评估框架，具体通过Dixit卡牌游戏实现。该游戏要求玩家为卡片生成既能误导部分玩家又不被所有玩家识破的标题，需要多种能力协同作用。

Result: 实验显示，在Dixit游戏中的胜率排名与流行MLM基准测试排名完全相关，同时游戏揭示了人类与MLM玩家在策略上的差异以及MLM推理能力的改进空间。

Conclusion: 游戏化评估为多模态大语言模型提供了更全面、客观且有趣的评估框架，能够有效解决传统评估方法的局限性，并揭示模型能力的真实水平。
...
### [72] [LyriCAR: A Difficulty-Aware Curriculum Reinforcement Learning Framework For Controllable Lyric Translation](https://arxiv.org/abs/2510.19967)
*Le Ren,Xiangjian Zeng,Qingqiang Wu,Ruoxuan Liang*

Main category: cs.CL

TL;DR: LyriCAR是一个用于可控歌词翻译的无监督框架，通过难度感知课程设计和自适应课程策略，在段落级别实现高质量的歌词翻译，显著减少训练步骤并提升翻译质量。


<details>
  <summary>Details</summary>
Motivation: 现有歌词翻译方法依赖手工规则和句子级建模，难以内化音乐语言模式和在段落级别保持跨行连贯性和全局韵律，限制了其泛化能力。

Method: 提出LyriCAR框架，包含难度感知课程设计器和自适应课程策略，以无监督方式引导模型处理逐渐复杂的挑战，优化训练资源分配。

Result: 在英中歌词翻译任务上的实验表明，LyriCAR在标准翻译指标和多维奖励分数上均达到最先进水平，自适应课程策略将训练步骤减少近40%同时保持优异性能。

Conclusion: LyriCAR通过课程学习策略有效解决了歌词翻译中的音乐约束平衡问题，实现了高质量、高效率的段落级歌词翻译。
...
### [73] [LLM-Augmented Symbolic NLU System for More Reliable Continuous Causal Statement Interpretation](https://arxiv.org/abs/2510.19988)
*Xin Lian,Kenneth D. Forbus*

Main category: cs.CL

TL;DR: 本文提出了一种混合方法，将大语言模型的广泛覆盖能力与符号自然语言理解系统的结构化表示能力相结合，以解决LLMs的幻觉和输出不一致问题，同时克服符号系统的覆盖范围限制。


<details>
  <summary>Details</summary>
Motivation: 大语言模型虽然应用广泛，但依赖概率推理容易产生事实幻觉和输出结构不一致的问题；而符号NLU系统虽然能提供可解释的结构化表示，但覆盖范围有限且需要专业知识维护。本文旨在结合两者的优势。

Method: 使用LLMs进行重述和文本简化以提供广泛覆盖，并作为填补知识缺口的自动信息来源；使用符号NLU系统生成可用于推理和增量学习的结构化关系表示。

Result: 在从常识科学文本中提取和解释数量及因果规律的任务上，混合方法显著优于纯符号方法。

Conclusion: 混合方法有效结合了LLMs的广泛覆盖能力和符号NLU系统的结构化表示优势，在特定任务上取得了比纯符号方法更好的性能。
...
### [74] [A Fundamental Algorithm for Dependency Parsing (With Corrections)](https://arxiv.org/abs/2510.19996)
*Michael A. Covington*

Main category: cs.CL

TL;DR: 提出了一种基于依赖树的自然语言句子解析算法，该算法一次处理一个单词，在可以附加时立即附加，模拟人脑解析过程，最坏情况复杂度为O(n³)，但在人类语言中仅在小n时出现最坏情况。


<details>
  <summary>Details</summary>
Motivation: 开发一种模拟人脑解析过程的自然语言解析算法，不同于短语结构解析器，该算法采用逐个单词处理的方式，在能够附加时立即附加，更接近人类语言处理的认知特性。

Method: 采用依赖树解析方法，一次处理一个单词，当单词可以被附加时立即进行附加操作，这种增量式处理方式模拟了人脑的解析过程。

Result: 算法的最坏情况时间复杂度为O(n³)，但在实际人类语言处理中，最坏情况仅出现在句子长度较小时，具有良好的实际性能。

Conclusion: 该依赖树解析算法在理论上具有与短语结构解析相同的复杂度，但在处理人类语言时表现出更好的实际性能，且更接近人脑的解析机制。
...
### [75] [Beyond MedQA: Towards Real-world Clinical Decision Making in the Era of LLMs](https://arxiv.org/abs/2510.20001)
*Yunpeng Xiao,Carl Yang,Mark Mai,Xiao Hu,Kai Shu*

Main category: cs.CL

TL;DR: 该论文提出了一个统一范式，将临床决策任务沿两个维度（临床背景和临床问题）进行表征，随着背景和问题接近真实临床环境，难度增加。论文回顾了现有数据集和方法，扩展了评估维度，并指出了开放挑战。


<details>
  <summary>Details</summary>
Motivation: 现有医学数据集（如MedQA）依赖简化的问答形式，不能充分代表真实世界的临床决策过程，需要更全面的评估框架。

Method: 提出基于临床背景和临床问题两个维度的统一范式，总结现有数据集设置，回顾训练时和测试时技术，并扩展评估维度至效率、可解释性等。

Result: 该范式澄清了假设，标准化了比较，并为开发具有临床意义的LLM提供了指导。

Conclusion: 提出的统一范式有助于更准确地评估和开发适用于真实临床环境的语言模型，强调了超越准确性的多方面评估的重要性。
...
### [76] [Forging GEMs: Advancing Greek NLP through Quality-Based Corpus Curation and Specialized Pre-training](https://arxiv.org/abs/2510.20002)
...
<details>
  <summary>Details</summary>
Motivation: 希腊语等形态丰富的语言在NLP研究中面临研究分散、架构多样性不足和上下文长度受限的问题，特别是在法律等专业领域，现有模型通常局限于早期Transformer架构和512个token的窗口，无法处理长法律文档。

Method: 构建大规模希腊语语料库，采用基于质量的严格过滤和预处理方法；预训练并系统评估多种现代架构(ELECTRA、ConvBERT、ModernBERT)；提出首个面向法律领域的希腊-英语双语嵌入模型。

Result: 在下游任务上的广泛实验表明，新模型类别证明了所提方法的有效性，特别是GEM-RoBERTa和GEM-ConvBERT模型显著优于现有基线。

Conclusion: 通过精心策划的数据基础和现代架构的应用，成功提升了希腊语NLP的性能，特别是在法律领域，为形态丰富的适度资源语言提供了有效的解决方案。
...
### [77] [Improving Transfer Learning for Sequence Labeling Tasks by Adapting Pre-trained Neural Language Models](https://arxiv.org/abs/2510.20033)
*David Dukić*

Main category: cs.CL

TL;DR: 该博士论文通过改进预训练神经语言模型的迁移学习方法，提升了序列标注任务的性能。主要贡献包括：引入多任务模型整合额外信号、修改自回归大语言模型架构实现双向信息流、以及基于生成式监督上下文微调的序列标注框架。


<details>
  <summary>Details</summary>
Motivation: 现有的预训练语言模型在序列标注任务上的迁移学习效果仍有提升空间，特别是在领域迁移、模型架构优化和生成式方法应用方面需要改进。

Method: 1) 多任务模型整合领域无关文本处理系统的额外信号；2) 修改自回归大语言模型架构，实现层间双向信息流；3) 生成式监督上下文微调框架结合响应导向适应策略。
...
### [78] [ToolScope: Enhancing LLM Agent Tool Use through Tool Merging and Context-Aware Filtering](https://arxiv.org/abs/2510.20036)
*Marianne Menglin Liu,Daniel Garcia,Fjona Parllaku,Vikas Upadhyay,Syed Fahad Allam Shah,Dan Roth*

Main category: cs.CL

TL;DR: ToolScope是一个解决LLM工具使用问题的框架，包含ToolScopeMerger和ToolScopeRetriever两个组件，通过自动合并冗余工具和检索相关工具来提高工具选择准确率。


<details>
  <summary>Details</summary>
Motivation: 现实世界工具集常包含冗余工具，导致名称和描述重叠，引入歧义并降低选择准确率；同时LLM面临严格的输入上下文限制，无法有效考虑大型工具集。

Method: ToolScopeMerger通过自动校正功能审计和修复工具合并，减少冗余；ToolScopeRetriever对工具进行排名和选择，仅保留最相关的工具，压缩工具集以适应上下文限制。

Result: 在三个最先进的LLM和三个开源工具使用基准测试中，工具选择准确率提高了8.38%到38.6%。

Conclusion: ToolScope有效增强了LLM的工具使用能力，解决了工具冗余和上下文限制的问题。
...
### [79] [From Facts to Folklore: Evaluating Large Language Models on Bengali Cultural Knowledge](https://arxiv.org/abs/2510.20043)
*Nafis Chowdhury,Moinul Haque,Anika Ahmed,Nazia Tasnim,Md. Istiak Hossain Shihab,Sajjadur Rahman,Farig Sadeque*

Main category: cs.CL

TL;DR: 该研究创建了孟加拉语文化知识数据集BLanCK，评估多语言大模型在低资源文化知识方面的表现，发现模型在非文化类别表现良好但在文化知识方面表现不佳，提供上下文后性能显著提升。
...
### [80] [Enhancing Reasoning Skills in Small Persian Medical Language Models Can Outperform Large-Scale Data Training](https://arxiv.org/abs/2510.20059)
*Mehrdad Ghassabi,Sadra Hakim,Hamidreza Baradaran Kashani,Pedram Rostami*

Main category: cs.CL

TL;DR: 使用RLAIF和DPO方法提升波斯语小语言模型的医疗推理能力，通过翻译医疗问答数据集并生成推理轨迹，在较小数据集上训练出超越更大规模预训练模型的性能。


<details>
  <summary>Details</summary>
Motivation: 提升小语言模型在专业领域（如医疗问答）的推理能力，特别是在波斯语等资源稀缺语言中，解决数据可用性有限的问题。

Method: 1. 将多选医疗问答数据集翻译成波斯语；2. 使用RLAIF生成拒绝-偏好答案对；3. 让师生模型生成CoT推理响应；4. 构建包含正确和错误推理轨迹的数据集；5. 使用DPO训练基准模型。

Result: 训练出的模型在医疗推理能力上显著提升，超越了使用5700万token预训练的gaokerena-V模型，尽管使用的数据集规模小得多（偏好答案200万token，拒绝答案250万token）。

Conclusion: 推理导向的训练方法在数据有限的情况下开发领域专用语言模型具有高效性和有效性。
...
### [81] [CreativityPrism: A Holistic Benchmark for Large Language Model Creativity](https://arxiv.org/abs/2510.20091)
*Zhaoyi Joey Hou,Bowei Alvin Zhang,Yining Lu,Bhiman Kumar Baghel,Anneliese Brei,Ximing Lu,Meng Jiang,Faeze Brahman,Snigdha Chaturvedi,Haw-Shiuan Chang,Daniel Khashabi,Xiang Lorraine Li*

Main category: cs.CL

TL;DR: 提出了CreativityPrism框架，将创造力分解为质量、新颖性和多样性三个维度，在三个领域（发散思维、创意写作、逻辑推理）评估17个最先进的大语言模型，发现专有模型和开源模型存在显著差距，且不同创造力维度的表现相关性较弱。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型创造力评估方法碎片化，缺乏统一的评估框架，主要原因是创造力的定义和测量方法在不同领域和任务中存在巨大差异。

Method: 提出CreativityPrism框架，将创造力分解为质量、新颖性和多样性三个维度，包含9个任务、3个领域和20个评估指标，对17个最先进的专有和开源大语言模型进行评估。

Result: 专有模型和开源模型之间存在显著差距；同一领域内任务表现高度相关，不同领域间相关性较弱；多样性和质量指标强相关，而新颖性与两者的相关性较弱。

Conclusion: 在某一创造力任务或维度上的强表现不一定能推广到其他任务或维度，强调了对大语言模型创造力进行整体评估的必要性。
...
### [82] [Leveraging the Power of Large Language Models in Entity Linking via Adaptive Routing and Targeted Reasoning](https://arxiv.org/abs/2510.20098)
*Yajie Li,Albert Galimov,Mitra Datta Ganapaneni,Pujitha Thejaswi,De Meng,Priyanshu Kumar,Saloni Potdar*

Main category: cs.CL

TL;DR: ARTER提出了一种结构化实体链接方法，通过自适应路由和选择性推理，在减少LLM使用的同时实现高性能，比传统方法效率提升2倍。
...
Method: ARTER采用结构化流水线，结合候选生成、基于上下文的评分、自适应路由和选择性推理。通过计算互补信号将上下文提及分为简单和困难案例，分别由低计算实体链接器和目标LLM推理处理。

Result: 在标准基准测试中，ARTER在6个数据集中的5个上平均提升2.53%，最高提升4.47%，性能与全LLM推理方法相当，但LLM token使用量减少一半。

Conclusion: ARTER证明了通过智能路由和选择性使用LLM推理，可以在保持高性能的同时显著提高实体链接效率，为资源受限环境提供了实用解决方案。
...
### [83] [BoundRL: Efficient Structured Text Segmentation through Reinforced Boundary Generation](https://arxiv.org/abs/2510.20151)
...
<details>
  <summary>Details</summary>
Motivation: 随着结构化文本在技术报告和生成式AI提示等领域的复杂性增加，传统句子或段落级分割方法无法有效处理包含表格、代码片段和占位符等非纯语言元素的文本，需要更有效的分割方法。

Method: BoundRL提出了一种联合token级文本分割和标签预测的方法，仅生成起始token序列并通过在原始文本中定位这些token来重建完整内容；采用强化学习与可验证奖励(RLVR)优化输出格式，并通过系统扰动生成的部分序列构建中间候选来缓解熵崩溃。
...
### [84] [Are Stereotypes Leading LLMs' Zero-Shot Stance Detection ?](https://arxiv.org/abs/2510.20154)
*Anthony Dubreuil,Antoine Gourru,Christine Largeron,Amine Trabelsi*

Main category: cs.CL

TL;DR: 本文研究发现大型语言模型在零样本立场检测任务中存在显著偏见，错误地将特定社会群体与特定立场关联，如将支持大麻合法化观点与低文本复杂度关联，将非裔美国人方言与反对特朗普关联。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型从预训练数据中继承了刻板印象，导致在自然语言处理任务中对某些社会群体产生偏见行为。令人惊讶的是，立场检测方法中的这种偏见评估在学术界被广泛忽视，而立场检测是最敏感的NLP任务之一，常涉及政治倾向。

Method: 在预存的立场检测数据集中，自动标注帖子的两个属性：特定群体的方言或语言风格，以及文本复杂度/可读性，以研究这些属性是否影响模型的立场检测决策。

Result: 研究结果显示，LLMs在立场检测任务中表现出显著的刻板印象，例如错误地将支持大麻的观点与低文本复杂度关联，将非裔美国人方言与反对特朗普的立场关联。

Conclusion: 大型语言模型在零样本立场检测中存在系统性偏见，需要进一步研究和缓解这些偏见，以确保公平和公正的立场检测结果。
...
### [85] [Mixture-of-Minds: Multi-Agent Reinforcement Learning for Table Understanding](https://arxiv.org/abs/2510.20176)
...
TL;DR: 提出Mixture-of-Minds多智能体框架，将表格推理分解为规划、编码和回答三个专门角色，结合代码执行实现精确表格操作，并通过MCTS和强化学习实现自我改进训练。


<details>
  <summary>Details</summary>
Motivation: 现有表格理解方法存在局限性：基于微调的方法容易产生算术错误和幻觉，而基于工具的方法缺乏语义理解且依赖固定模式。需要结合稳健推理和可靠表格处理的新方法。

Method: 设计三角色多智能体框架（规划、编码、回答），每个智能体专注于特定任务方面；引入基于MCTS滚动的自我改进训练框架，生成伪黄金轨迹并用强化学习优化智能体。

Result: 在TableBench上达到62.13%的准确率，超越了OpenAI-o4-mini-high，显示出显著性能提升。

Conclusion: 结合结构化多智能体工作流程与强化学习，为表格理解任务提供了有前景的解决方案，证明了该方法的有效性。
...
### [86] [Stuck in the Matrix: Probing Spatial Reasoning in Large Language Models](https://arxiv.org/abs/2510.20198)
*Maggie Bai,Ava Kim Cohen,Eleanor Koss,Charlie Lichtenbaum*

Main category: cs.CL

TL;DR: 本文通过五项任务测试大语言模型在文本输入上的空间推理能力，发现模型在小规模任务中表现中等，但随着复杂度增加性能急剧下降，平均准确率下降42.7%，最高达84%，表明LLM缺乏稳健的空间表征能力。
...
Method: 使用五项任务测试LLM：象限识别、几何变换、距离评估、单词搜索和瓷砖滑动，通过增加网格维度来扩展任务复杂度。

Result: LLM在所有任务的小复杂度情况下表现中等，但随着规模增加性能急剧下降，平均准确率损失42.7%，最高达84%；所有初始准确率超过50%的测试都至少损失48%准确率。

Conclusion: LLM在空间推理方面存在显著局限性，缺乏稳健的空间表征架构，这为未来语言与几何交叉领域的综合基准测试奠定了基础。
...
### [87] [Decoding-Free Sampling Strategies for LLM Marginalization](https://arxiv.org/abs/2510.20208)
*David Pohl,Marco Cognetta,Junyoung Lee,Naoaki Okazaki*

Main category: cs.CL

TL;DR: 本文提出了一种无需解码的采样策略来近似计算语言模型对文本的边际概率，相比传统基于生成的采样方法，这种方法计算成本极低且准确度足够。


<details>
  <summary>Details</summary>
Motivation: 现代语言模型使用子词分词，但只评估特定分词输出的概率，而同一文本有多种分词方式。边际概率评估更准确但计算困难，传统采样方法需要昂贵的生成步骤。

Method: 研究解码无关的采样策略，这些策略不需要语言模型生成，完全依赖极低成本的分词器无关采样方法，通过计算序列给定分词的概率来近似边际概率。

Result: 解码无关采样策略在多个开源模型上提供了足够准确的边际概率估计，且运行时间成本仅为传统方法的一小部分。

Conclusion: 解码无关采样策略能够以极低成本有效近似语言模型的边际概率，适用于下游推理任务。
...
### [88] [Tri-Modal Severity Fused Diagnosis across Depression and Post-traumatic Stress Disorders](https://arxiv.org/abs/2510.20239)
...
<details>
  <summary>Details</summary>
Motivation: 抑郁和创伤后应激障碍（PTSD）经常共病且症状相互关联，而现有的自动化评估通常是二分类且针对单一障碍。临床诊断需要能够提供严重程度感知的跨障碍评估和决策支持解释。

Method: 开发了统一的三模态情感严重程度框架，同步融合访谈文本（句子级transformer嵌入）、音频（log Mel统计特征和delta）和面部信号（动作单元、注视、头部和姿态描述符），通过校准的后期融合分类器输出抑郁（PHQ-8；5类）和PTSD（3类）的严重程度。
...
### [89] [Context-level Language Modeling by Learning Predictive Context Embeddings](https://arxiv.org/abs/2510.20280)
*Beiya Dai,Yuliang Liu,Daozheng Xue,Qipeng Guo,Kai Chen,Xinbing Wang*

Main category: cs.CL

TL;DR: ContextLM框架通过引入next-context预测目标来增强标准预训练，让模型学习多token上下文的预测表示，同时保持与标准自回归评估的完全兼容性。
...
Method: 在标准预训练基础上增加next-context预测目标，利用未来token块产生的误差信号来训练模型学习多token上下文的预测表示。

Result: 在GPT2和Pythia模型系列上的实验表明，ContextLM在困惑度和下游任务性能上均带来一致改进，模型规模可达15亿参数。
...
### [90] [Citation Failure: Definition, Analysis and Efficient Mitigation](https://arxiv.org/abs/2510.20303)
*Jan Buchmann,Iryna Gurevych*

Main category: cs.CL

TL;DR: 该论文提出将引用失败与响应失败区分开来，引入CITECONTROL基准分析引用失败模式，并开发CITENTION框架来改善LLM引用性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的RAG系统虽然提供引用功能，但存在引用失败问题——模型生成有用响应却未能完整引用证据。需要将这种引用失败与响应失败区分开来，并找到解决方案。

Method: 采用两步法：1) 通过CITECONTROL基准系统分析响应与证据关系对引用质量的影响；2) 提出CITENTION框架，整合生成式、基于注意力和基于检索的方法来改善引用性能。
...
### [91] [Exploring Generative Process Reward Modeling for Semi-Structured Data: A Case Study of Table Question Answering](https://arxiv.org/abs/2510.20304)
*Lei Tang,Wei Zhou,Mohsen Mesgar*

Main category: cs.CL

TL;DR: 本研究首次系统评估过程奖励模型在表格问答任务中的应用，发现结合文本和代码验证的PRM能帮助选择答案，但在领域外数据上泛化能力有限，且步骤验证与答案准确性相关性较弱。


<details>
  <summary>Details</summary>
Motivation: 过程奖励模型在数学等复杂推理任务中表现出色，但在涉及半结构化数据的表格问答任务中的应用尚未探索。表格问答存在信息冗余、推理步骤松散、领域特定推理等独特挑战。

Method: 从答案和步骤两个角度评估最先进的生成式PRM在表格问答任务上的表现，分析文本和代码验证的结合效果。

Result: 结合文本和代码验证的PRM能辅助解决方案选择，但在领域外数据上泛化能力差；步骤级验证性能与答案准确性相关性弱，可能源于步骤依赖性和因果联系松散。
...
### [92] [Teaching Language Models to Reason with Tools](https://arxiv.org/abs/2510.20342)
...
<details>
  <summary>Details</summary>
Motivation: 大型推理模型在处理复杂数学运算时存在效率低和准确性差的问题，而集成代码解释器又会导致模型内部概率推理与外部确定性知识之间的冲突，造成无效思考。

Method: 提出Hint-Engineering数据合成策略，在推理路径的关键点注入多样化提示，生成高质量的代码集成推理数据，并通过监督微调、拒绝采样和强化学习优化模型与代码解释器的多轮交互。

Result: 在五个挑战性数学推理数据集上，32B和1.5B模型分别实现4%和8%的绝对性能提升，同时显著降低token使用量（32B模型约30%，1.5B模型约50%）。

Conclusion: CoRT框架有效解决了大型推理模型与代码解释器交互时的冲突问题，显著提升了数学推理的性能和效率。
...
### [93] [Evaluating Latent Knowledge of Public Tabular Datasets in Large Language Models](https://arxiv.org/abs/2510.20351)
...
Result: 实验显示，当数据集包含有意义的列名或可解释的值类别时，LLMs表现出污染效应；而当这些语义线索被移除或随机化时，性能急剧下降至接近随机水平。

Conclusion: LLMs在表格推理任务中表现出的能力可能反映了对公开数据集的记忆而非真正的推理能力，建议在未来的LLM评估中采用策略来区分语义泄漏与真实推理能力。
...
### [94] [FreeChunker: A Cross-Granularity Chunking Framework](https://arxiv.org/abs/2510.20356)
...
<details>
  <summary>Details</summary>
Motivation: 现有检索增强生成系统采用固定粒度的分块策略，依赖静态边界识别，难以适应多样化的查询需求，限制了系统的适应性。
...
Conclusion: FreeChunker通过范式转变从根本上改进了传统分块方法，在保持高性能的同时大幅提升了计算效率，为检索增强生成系统提供了更灵活的解决方案。
...
### [95] [Dialogue Is Not Enough to Make a Communicative BabyLM (But Neither Is Developmentally Inspired Reinforcement Learning)](https://arxiv.org/abs/2510.20358)
*Francesca Padovani,Bastian Bunzeck,Manar Ali,Omar Momen,Arianna Bisazza,Hendrik Buschmeier,Sina Zarrieß*

Main category: cs.CL

TL;DR: 研究探讨仅使用对话数据预训练是否能产生形式和功能上合适的小型语言模型，发现对话专用模型在标准基准测试中表现不佳，但在对话延续预测方面表现出色。


<details>
  <summary>Details</summary>
Motivation: 研究是否仅通过对话数据预训练就能产生合适的小型语言模型，以及如何通过微调策略使模型生成更具沟通性的文本。

Method: 基于对话数据预训练的llamalogue模型，采用多种微调策略（包括PPO和DPO）来增强模型的沟通能力。

Result: 模型在大多数标准BabyLM基准测试中表现不佳，但在最小对设置中的对话延续预测方面表现出色；PPO微调效果不一甚至有害，而DPO微调能进一步提升对话基准性能。

Conclusion: 对话专用预训练能产生在对话任务上表现良好的小型语言模型，DPO微调比PPO更有效地提升对话性能。
...
### [96] [The Impact of Negated Text on Hallucination with Large Language Models](https://arxiv.org/abs/2510.20375)
*Jaehyung Seo,Hyeonseok Moon,Heuiseok Lim*

Main category: cs.CL

TL;DR: 本文研究了否定文本对大语言模型幻觉检测的影响，发现LLMs在否定语境下难以有效识别幻觉，并构建了NegHalu数据集进行实验验证。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注肯定语境下的幻觉问题，但否定文本对LLM幻觉的影响尚未充分探索，需要回答三个重要研究问题。

Method: 通过重构现有幻觉检测数据集构建NegHalu数据集，在否定语境下测试LLMs的幻觉检测能力，并在token级别追踪LLMs的内部状态。

Result: 实验表明LLMs在否定文本中难以有效检测幻觉，经常产生逻辑不一致或不忠实的判断，且难以减轻否定输入的意外影响。

Conclusion: 否定文本显著影响LLMs的幻觉检测性能，揭示了当前模型在处理否定语境时的局限性，需要进一步研究来改善这一能力。
...
### [97] [VLSP 2025 MLQA-TSR Challenge: Vietnamese Multimodal Legal Question Answering on Traffic Sign Regulation](https://arxiv.org/abs/2510.20381)
...
<details>
  <summary>Details</summary>
Motivation: 推进越南多模态法律文本处理研究，为多模态法律领域智能系统的构建和评估提供基准数据集，重点关注越南交通标志法规。
...
Conclusion: 该共享任务为多模态法律领域提供了重要的基准数据集和评估标准，展示了在多模态法律检索和问答方面的研究进展。
...
### [98] [NeoDictaBERT: Pushing the Frontier of BERT models for Hebrew](https://arxiv.org/abs/2510.20386)
*Shaltiel Shmidman,Avi Shmidman,Moshe Koppel*

Main category: cs.CL

TL;DR: 本文介绍了NeoDictaBERT和NeoDictaBERT-bilingual两种BERT风格的模型，专门针对希伯来语文本训练，在希伯来语基准测试中表现优异，特别是在检索任务上超越了类似规模的多语言模型。


<details>
  <summary>Details</summary>
Motivation: 尽管BERT模型在各种任务上表现出色，但其架构选择相对于新模型如Llama3和Qwen3已经过时。ModernBERT和NeoBERT在英语基准上显示出显著改进，因此作者希望开发专门针对希伯来语文本的类似模型。

Method: 使用与NeoBERT相同的架构训练BERT风格模型，专门针对希伯来语文本进行训练，开发了NeoDictaBERT和NeoDictaBERT-bilingual两个版本。

Result: 这些模型在几乎所有希伯来语基准测试中都优于现有模型，为下游任务提供了坚实基础。特别是NeoDictaBERT-bilingual在检索任务上表现出色，超越了类似规模的其他多语言模型。

Conclusion: NeoDictaBERT模型为希伯来语NLP研究和开发提供了强大的基础，作者将模型发布给社区以推动希伯来语NLP的发展。
...
### [99] [Teacher Demonstrations in a BabyLM's Zone of Proximal Development for Contingent Multi-Turn Interaction](https://arxiv.org/abs/2510.20411)
*Suchir Salhan,Hongyi Gu,Donya Rooein,Diana Galvan-Sosa,Gabrielle Gaudeau,Andrew Caines,Zheng Yuan,Paula Buttery*

Main category: cs.CL

TL;DR: ContingentChat是一个教师-学生框架，通过在100M词汇的BabyLM上进行针对性后训练，提高了多轮对话中的连续性质量，使生成的回复更加语法正确和连贯。


<details>
  <summary>Details</summary>
Motivation: 儿童与照顾者之间的多轮对话具有连续性特征，即对话者之间及时、直接且有意义的交流。研究旨在通过后训练方法提升BabyLM在多轮对话中的连续性表现。

Method: 采用教师-学生框架，使用新颖的对齐数据集进行后训练，并实验了自适应教师解码策略。
...
### [100] [LM-mixup: Text Data Augmentation via Language Model based Mixup](https://arxiv.org/abs/2510.20449)
*Zhijie Deng,Zhouan Shen,Ling Li,Yao Zhou,Zhaowei Zhu,Yanji He,Wei Wang,Jiaheng Wei*

Main category: cs.CL

TL;DR: 本文提出指令蒸馏任务，将低质量冗余指令数据转化为高质量指令-输出对，并开发LM-Mixup方法，通过监督微调和强化学习优化，显著提升指令调优效率。
...
### [101] [Systematic Evaluation of Uncertainty Estimation Methods in Large Language Models](https://arxiv.org/abs/2510.20460)
*Christian Hobelsberger,Theresa Winner,Andreas Nawroth,Oliver Mitevski,Anna-Carolina Haensch*

Main category: cs.CL

TL;DR: 本文系统评估了四种大语言模型输出置信度估计方法：VCE、MSP、样本一致性和CoCoA，发现CoCoA混合方法在可靠性和答案区分度方面表现最佳。
...
Method: 在四个问答任务上使用最先进的开源大语言模型，系统评估四种置信度估计方法：VCE、MSP、样本一致性和CoCoA。
...
### [102] [Mask and You Shall Receive: Optimizing Masked Language Modeling For Pretraining BabyLMs](https://arxiv.org/abs/2510.20475)
...
<details>
  <summary>Details</summary>
Motivation: 提升掩码语言建模的性能，特别是在小规模数据场景下，以在BabyLM挑战赛中取得更好成绩。

Method: 1. 改进的掩码语言建模：根据模型预测能力调整掩码标记的概率
2. 引入子词嵌入：增强模型的形态学泛化能力

Result: 在(Super)GLUE任务上相比标准MLM有显著性能提升，在严格小规模赛道中超越了基线模型。

Conclusion: 改进的MLM方法和子词嵌入策略有效提升了小规模语言模型的性能，在BabyLM挑战赛中取得了成功。
...
### [103] [Steering Evaluation-Aware Language Models To Act Like They Are Deployed](https://arxiv.org/abs/2510.20487)
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型有时能检测到它们正在被评估，并调整行为以显得更加对齐，这会影响安全评估的可靠性。

Method: 采用两步训练过程：首先在包含模型事实描述的文档上进行持续预训练，然后通过专家迭代训练模型在评估设置中使用Python类型提示。最后通过激活向量引导来抑制评估意识。

Result: 训练后的模型在评估环境中比部署环境更频繁地使用类型提示，但通过激活向量引导可以消除这种差异，使模型在评估时表现得像在部署环境中。

Conclusion: 激活向量引导技术可以有效抑制大语言模型的评估意识，提高安全评估的可靠性，AI评估者可以通过这种方法使模型在评估时表现得像在部署环境中。
...
### [104] [Robust Preference Alignment via Directional Neighborhood Consensus](https://arxiv.org/abs/2510.20498)
*Ruochen Mao,Yuling Shi,Xiaodong Gu,Jiaheng Wei*

Main category: cs.CL

TL;DR: 提出RPS方法，通过方向性邻域共识在无需重新训练的情况下提升LLM对多样化人类偏好的鲁棒性，解决偏好覆盖差距问题。


<details>
  <summary>Details</summary>
Motivation: 现有LLM对齐方法主要反映主流偏好，在个体化、偏离训练数据中心趋势的细微偏好上表现不佳，存在偏好覆盖差距。

Method: RPS方法：后处理、无需训练，通过从相关偏好邻域采样多个响应创建候选池，然后选择最符合用户原始意图的响应。

Result: 在三种对齐范式（DPA、DPO、SFT）上的实验表明，RPS相比基线方法在挑战性偏好上获得高达69%的胜率。

Conclusion: RPS为增强偏好对齐模型的可靠性提供了一个实用且理论基础的解决方案。
...
### [105] [Assessing the Political Fairness of Multilingual LLMs: A Case Study based on a 21-way Multiparallel EuroParl Dataset](https://arxiv.org/abs/2510.20508)
*Paul Lerner,François Yvon*

Main category: cs.CL

TL;DR: 本文提出了一种基于多语言翻译公平性框架来评估大语言模型政治偏见的新方法，通过系统比较欧洲议会演讲的翻译质量，发现主流政党（左、中、右）的翻译质量优于边缘政党。


<details>
  <summary>Details</summary>
Motivation: 传统上通过英语调查问卷评估LLMs政治偏见存在局限，需要基于多语言翻译公平性原则的替代评估框架。

Method: 使用新构建的21种语言并行版EuroParl数据集，包含150万句子、4000万单词，涵盖3年数据、1000+演讲者、7个国家、12个欧盟政党、25个欧盟委员会和数百个国家政党，系统分析不同政治派别演讲的翻译质量差异。

Result: 观察到系统性的翻译质量差异，主流政党（左翼、中间派、右翼）的演讲翻译质量显著优于边缘政党。

Conclusion: 基于多语言翻译公平性的框架能够有效揭示LLMs中存在的政治偏见，主流政党在翻译中获得了更好的表现。
...
### [106] [ARC-Encoder: learning compressed text representations for large language models](https://arxiv.org/abs/2510.20535)
*Hippolyte Pilchen,Edouard Grave,Patrick Pérez*

Main category: cs.CL

TL;DR: ARC-Encoder是一种可适应文本表示压缩器，通过将上下文压缩为连续表示来替代解码器LLM中的token嵌入，实现4-8倍的压缩比，在保持性能的同时提高推理计算效率。


<details>
  <summary>Details</summary>
Motivation: 检索增强生成和思维链推理等技术导致上下文更长、推理成本增加。现有的上下文压缩方法需要微调目标模型或修改架构，这会降低模型的通用能力。

Method: 设计ARC-Encoder编码器，将上下文压缩为连续表示，替代解码器LLM中的token嵌入。系统研究了编码器的训练策略和架构选择，输出比文本token少4-8倍的连续表示。

Result: ARC-Encoder在多个LLM使用场景中达到最先进性能，包括上下文学习和上下文窗口扩展，同时提高推理计算效率。模型可适应多个解码器，实现跨LLM的通用性。

Conclusion: ARC-Encoder提供了一种灵活高效的便携编码器解决方案，可无缝与多个LLM配合使用，在保持性能的同时显著降低计算成本。
...
### [107] [The Dog the Cat Chased Stumped the Model: Measuring When Language Models Abandon Structure for Shortcuts](https://arxiv.org/abs/2510.20543)
*Sangmitra Madhusudan,Kaige Chen,Ali Emami*

Main category: cs.CL

TL;DR: CenterBench是一个包含9,720个理解问题的数据集，专门用于测试语言模型对中心嵌入句的结构理解能力，通过对比语义合理与不合理句子的表现差异来区分语法分析和语义模式匹配。
...
Method: 创建CenterBench数据集，包含中心嵌入句及其语义不合理对照版本，通过六种理解问题测试表面理解、句法依赖和因果推理能力，测试六个不同模型的表现。

Result: 模型在合理与不合理句子间的性能差距随复杂度增加而系统性扩大，最大差距达26.8个百分点；语义合理性在因果推理问题上反而损害性能；推理模型虽提高准确性但存在语义捷径、过度思考和拒绝回答等问题。

Conclusion: CenterBench提供了首个能够识别模型何时从结构分析转向模式匹配的框架，揭示了语言模型在处理复杂结构时的局限性。
...
### [108] [GlobalRAG: Enhancing Global Reasoning in Multi-hop Question Answering via Reinforcement Learning](https://arxiv.org/abs/2510.20548)
...
<details>
  <summary>Details</summary>
Motivation: 当前强化学习在多跳问答中的有效性受到两个基本限制：(i) 缺乏全局规划来结构化多步推理，(ii) 不忠实的执行阻碍了有效的查询制定和检索证据的一致性使用。

Method: 提出GlobalRAG框架，引入规划质量奖励和子目标完成奖励来鼓励连贯规划和可靠子目标执行，采用渐进权重退火策略平衡过程导向和结果导向目标。

Result: 在领域内和领域外基准测试中，GlobalRAG显著优于强基线，仅使用8k训练数据（基线训练数据的42%）就在EM和F1指标上平均提升14.2%。

Conclusion: GlobalRAG通过强化学习有效解决了多跳问答中的全局规划和忠实执行问题，在数据效率方面表现出色，显著提升了多跳问答性能。
...
### [109] [Beyond Retrieval-Ranking: A Multi-Agent Cognitive Decision Framework for E-Commerce Search](https://arxiv.org/abs/2510.20567)
...
<details>
  <summary>Details</summary>
Motivation: 传统电商搜索的检索-排序范式依赖查询-商品匹配，与平台用户的多阶段认知决策过程存在根本性不匹配，导致语义鸿沟、高决策成本和缺乏专业购物指导等问题。
...
### [110] [Can ChatGPT Code Communication Data Fairly?: Empirical Evidence from Multiple Collaborative Tasks](https://arxiv.org/abs/2510.20584)
*Jiangang Hao,Wenju Cui,Patrick Kyllonen,Emily Kerzabi*

Main category: cs.CL

TL;DR: 本研究调查了ChatGPT在沟通数据自动编码中是否存在性别和种族偏见，发现在协作问题解决框架下，ChatGPT编码对不同人口群体无显著偏见。


<details>
  <summary>Details</summary>
Motivation: 填补关于AI技术（如ChatGPT）在沟通数据编码中是否存在人口统计学偏见的空白，为大规模评估协作和沟通提供可靠工具。

Method: 使用典型的协作问题解决编码框架，分析三种协作任务（谈判、问题解决、决策制定）的数据，比较不同性别和种族群体的编码结果。

Result: ChatGPT基础的编码在性别和种族群体间未表现出显著偏见，准确性可与人类评分者相媲美。

Conclusion: ChatGPT编码无显著人口统计学偏见，为其在大规模协作和沟通评估中的应用铺平了道路。
...
### [111] [BUSTED at AraGenEval Shared Task: A Comparative Study of Transformer-Based Models for Arabic AI-Generated Text Detection](https://arxiv.org/abs/2510.20610)
*Ali Zain,Sareem Farooqui,Muhammad Rafi*

Main category: cs.CL

TL;DR: 该论文介绍了团队在阿拉伯语AI生成文本检测共享任务中的提交方案，使用三种预训练Transformer模型进行微调，发现多语言XLM-RoBERTa模型表现最佳。


<details>
  <summary>Details</summary>
Motivation: 参与阿拉伯语AI生成文本检测共享任务，评估不同预训练模型在阿拉伯语AI生成文本检测任务中的效果。

Method: 使用三种预训练Transformer模型（AraELECTRA、CAMeLBERT和XLM-RoBERTa）在提供的数据集上进行微调，执行二元分类任务。

Result: 多语言XLM-RoBERTa模型取得了最佳性能，F1分数达到0.7701，超过了专门的阿拉伯语模型。
...
### [112] [Why Did Apple Fall To The Ground: Evaluating Curiosity In Large Language Model](https://arxiv.org/abs/2510.20635)
*Haoyu Wang,Sihang Jiang,Yuyan Chen,Yitong Wang,Yanghua Xiao*

Main category: cs.CL

TL;DR: 该论文基于人类好奇心评估问卷5DCR设计了一个综合评估框架，评估大语言模型的好奇心表现，发现LLMs比人类有更强的求知欲但在不确定环境中更保守，并确认好奇行为能增强模型的推理和主动学习能力。
...
Method: 基于人类好奇心评估问卷Five-Dimensional Curiosity scale Revised (5DCR)设计综合评估框架，涵盖信息寻求、刺激寻求、社会好奇心等维度，系统评估LLMs的好奇心表现。

Result: LLMs表现出比人类更强的知识渴求，但在面对不确定环境时倾向于做出保守选择；好奇行为能够增强模型的推理能力和主动学习能力。

Conclusion: LLMs具备表现出类似人类好奇心的潜力，这为未来LLMs学习能力和创新研究的发展提供了实验支持。
...
### [113] [\textsc{CantoNLU}: A benchmark for Cantonese natural language understanding](https://arxiv.org/abs/2510.20670)
*Junghyun Min,York Hay Ng,Sophia Chan,Helena Shunhua Zhao,En-Shiun Annie Lee*

Main category: cs.CL

TL;DR: 本文介绍了CantoNLU，一个针对粤语自然语言理解的基准测试，涵盖7个任务，并比较了不同模型的性能表现。
...
Method: 创建CantoNLU基准测试，包含7个任务；比较四种模型：未经粤语训练的普通话模型、通过持续预训练获得的粤语适应模型、从头训练的粤语单语模型。
...
### [114] [Neural Diversity Regularizes Hallucinations in Small Models](https://arxiv.org/abs/2510.20690)
*Kushal Chakrabarti,Nirmal Balachundhar*

Main category: cs.CL

TL;DR: 提出神经多样性作为减少语言模型幻觉的新机制，通过去相关的并行表示来降低幻觉率，而不增加参数或数据量。


<details>
  <summary>Details</summary>
Motivation: 尽管参数、计算和数据不断增加，语言模型仍然存在幻觉问题。受投资组合理论启发，作者认为去相关的表示可以像不相关资产降低风险一样减少幻觉。

Method: 提出ND-LoRA（神经多样性低秩适应），结合并行LoRA适配器和Barlow Twins正则化，通过促进神经多样性来减少幻觉。

Result: ND-LoRA将幻觉率降低高达25.6%（平均14.6%），且不损害总体准确性。神经相关性增加0.1%与幻觉增加3.8%相关，不同任务需要不同最优神经多样性水平。

Conclusion: 神经多样性是提高语言模型可靠性的第三个扩展维度（与参数和数据正交），可在固定预算下改善模型可靠性。
...
### [115] [Structure-Conditional Minimum Bayes Risk Decoding](https://arxiv.org/abs/2510.20700)
*Bryan Eikema,Anna Rutkiewicz,Mario Giulianelli*

Main category: cs.CL

TL;DR: 本文针对MBR解码在开放任务中的局限性，提出了三种轻量级适应性改进，通过增强对结果空间结构变化的敏感性来提升生成质量。


<details>
  <summary>Details</summary>
Motivation: MBR解码在机器翻译中有效，但在对话或指令跟随等开放任务中面临挑战，因为标准相似性效用函数可能选择模型分布的代表性但非最优的响应。

Method: 提出了三种轻量级适应性效用函数改进，并构建了包含对话行为、情感和响应结构的数据集来测试假设，同时提出了两个评估结构最优性的指标。

Result: 分析显示标准相似性效用函数在结构最优性指标上表现不佳，而提出的适应性改进显著提升了结构最优性，在真实世界指令跟随基准测试中生成质量提高了13.7个百分点。

Conclusion: 增强MBR对结构变化的敏感性可以显著提升在开放任务中的生成质量，提出的轻量级适应性改进是有效的解决方案。
...
### [116] [Automated Extraction of Fluoropyrimidine Treatment and Treatment-Related Toxicities from Clinical Notes Using Natural Language Processing](https://arxiv.org/abs/2510.20727)
*Xizhi Wu,Madeline S. Kreider,Philip E. Empey,Chenyu Li,Yanshan Wang*

Main category: cs.CL

TL;DR: 本研究开发并评估了多种自然语言处理方法，用于从临床笔记中提取氟嘧啶类药物治疗和毒性信息。基于大语言模型的错误分析提示方法表现最佳，F1分数达到1.000，优于传统机器学习和深度学习方法。


<details>
  <summary>Details</summary>
Motivation: 氟嘧啶类药物广泛用于结直肠癌和乳腺癌治疗，但会引起手足综合征和心脏毒性等不良反应。由于毒性信息通常嵌入在临床笔记中，需要开发有效的自然语言处理方法来提取这些信息。

Method: 构建了包含236份临床笔记的金标准数据集，开发了基于规则、机器学习（随机森林、支持向量机、逻辑回归）、深度学习（BERT、ClinicalBERT）和大语言模型（零样本提示和错误分析提示）的自然语言处理方法，采用80:20的训练测试分割。

Result: 错误分析提示方法在治疗和毒性提取方面达到最优性能（F1=1.000），零样本提示方法在治疗提取方面F1=1.000，毒性提取方面F1=0.876。逻辑回归和支持向量机在毒性提取方面排名第二（F1=0.937）。深度学习方法表现较差，BERT和ClinicalBERT的F1分数在0.839-0.886之间。

Conclusion: 基于大语言模型的自然语言处理方法最有效地从临床笔记中提取氟嘧啶类药物治疗和毒性信息，具有支持肿瘤学研究和药物警戒的强大潜力。
...
### [117] [Are Large Reasoning Models Good Translation Evaluators? Analysis and Performance Boost](https://arxiv.org/abs/2510.20780)
*Runzhe Zhan,Zhihong Huang,Xinyi Yang,Lidia S. Chao,Min Yang,Derek F. Wong*

Main category: cs.CL

TL;DR: 本文首次系统分析了大型推理模型作为机器翻译质量评估器的潜力，发现其存在需要定制评估材料、对简单实例过度思考以及评分机制导致高估等问题，提出通过训练合成的人类思考轨迹来校准模型思维，显著降低了思考成本并提升了评估性能。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在复杂下游任务中通过引入中间思考过程提升了推理能力，但其在机器翻译质量评估方面的潜力尚未充分探索，因此需要系统分析其作为评估器的可行性。

Method: 通过训练大型推理模型在合成的人类思考轨迹上进行校准，解决其过度思考和评分机制问题，并在WMT24 Metrics基准上进行实验验证。

Result: 该方法显著减少了约35倍的思考预算，同时在7B到32B不同规模的模型上都提升了评估性能，例如R1-Distill-Qwen-7B模型实现了+8.7的相关性点提升。

Conclusion: 高效校准的大型推理模型有潜力推进细粒度自动机器翻译评估的发展，为机器翻译质量评估提供了新的有效方法。
...
### [118] [A Use-Case Specific Dataset for Measuring Dimensions of Responsible Performance in LLM-generated Text](https://arxiv.org/abs/2510.20782)
...
<details>
  <summary>Details</summary>
Motivation: 当前评估大语言模型的方法主要关注文本生成等高层任务，没有针对特定AI应用。这种方法不足以评估LLMs在公平性等负责任AI维度上的表现，因为不同应用中受保护属性的相关性不同。

Method: 构建了一个由真实应用驱动（根据产品特征列表生成纯文本产品描述）的数据集，通过公平性属性与性别化形容词和产品类别的交叉参数化，产生丰富的标注提示集合。

Result: 展示了如何使用该数据识别LLMs在质量、真实性、安全性和公平性方面的差距。

Conclusion: 为研究社区贡献了一个LLM评估提案和具体资源，强调评估需要与具体应用场景相结合。
...
### [119] [Alleviating Forgetfulness of Linear Attention by Hybrid Sparse Attention and Contextualized Learnable Token Eviction](https://arxiv.org/abs/2510.20787)
*Mutian He,Philip N. Garner*

Main category: cs.CL

TL;DR: 提出了一种混合注意力模型，在线性注意力模型中引入稀疏注意力机制，通过可学习的token淘汰策略和滑动窗口注意力，在保持线性注意力常数复杂度的情况下改善长序列处理能力。


<details>
  <summary>Details</summary>
Motivation: 线性注意力模型虽然效率高，但有限的记忆容量导致遗忘问题，在处理检索密集型任务时性能受限。

Method: 提出混合模型，结合线性注意力和稀疏注意力，包括token淘汰稀疏注意力和查询感知原生稀疏注意力。特别提出可学习的token淘汰方法，结合滑动窗口注意力和轻量级CNN来自适应保留关键KV对。

Result: 在检索密集型基准测试中验证了方法的有效性，并提供了稀疏注意力机制的高效Triton内核实现。

Conclusion: 混合注意力模型能够在线性注意力常数复杂度约束下，有效缓解遗忘问题，提升检索密集型任务的性能。
...
### [120] [Simple Context Compression: Mean-Pooling and Multi-Ratio Training](https://arxiv.org/abs/2510.20797)
*Yair Feldman,Yoav Artzi*

Main category: cs.CL

TL;DR: 提出了一种轻量级的均值池化方法用于RAG中的软上下文压缩，该方法在多个数据集和模型上表现优于广泛使用的压缩标记架构，并研究了训练单一压缩器输出多种压缩比率的效果。
...
Method: 开发了轻量级的均值池化方法，并与压缩标记架构进行对比，同时训练同一压缩器输出多种压缩比率，在多个QA数据集、模型家族、规模和压缩比率下进行广泛实验。

Result: 简单的均值池化方法取得了最强性能，在训练多种压缩比率时性能下降相对较小，但不同架构和训练机制之间的权衡更为复杂。
...
### [121] [On the Detectability of LLM-Generated Text: What Exactly Is LLM-Generated Text?](https://arxiv.org/abs/2510.20810)
*Mingmeng Geng,Thierry Poibeau*

Main category: cs.CL

TL;DR: 论文指出当前LLM生成文本检测面临定义模糊、检测目标不明确的问题，现有基准和评估方法无法充分应对现实应用中的各种条件，检测器结果应被视为参考而非决定性指标。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的广泛应用，检测其生成文本的需求日益增长，但缺乏对'LLM生成文本'的一致精确定义，现实使用场景的多样性进一步增加了检测难度。

Method: 通过分析现有检测方法的局限性，指出人类对LLM输出的编辑以及LLM对用户的微妙影响模糊了LLM生成与人类撰写文本的界限。

Result: 现有基准和评估方法无法充分应对检测器在现实应用中的各种条件，导致检测器的数值结果经常被误解，其重要性正在减弱。

Conclusion: 检测器在特定条件下仍然有用，但其结果应仅作为参考而非决定性指标来解读。
...
### [122] [Automating Iconclass: LLMs and RAG for Large-Scale Classification of Religious Woodcuts](https://arxiv.org/abs/2510.19986)
*Drew B. Thomas*

Main category: cs.IR

TL;DR: 本文提出了一种结合大型语言模型、向量数据库和检索增强生成的新方法，用于分类早期现代宗教图像，在分类精度上显著优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 传统图像和基于关键字的搜索方法在早期现代宗教图像分类中效果有限，需要一种能够充分利用图像和文本上下文信息的更准确分类方法。

Method: 利用神圣罗马帝国书籍插图的整页上下文，通过LLM生成包含视觉和文本元素的详细描述，然后通过混合向量搜索匹配相关的Iconclass代码。

Result: 该方法在五个和四个分类级别上分别达到87%和92%的精确度，显著优于传统图像和基于关键字的搜索方法。

Conclusion: 通过使用整页描述和RAG，该系统提高了分类准确性，为大规模分析早期现代视觉档案提供了强大工具，展示了LLMs和RAG在艺术史和数字人文学科研究中的巨大潜力。
...
### [123] [Rank-GRPO: Training LLM-based Conversational Recommender Systems with Reinforcement Learning](https://arxiv.org/abs/2510.20150)
*Yaochen Zhu,Harald Steck,Dawen Liang,Yinhan He,Jundong Li,Nathan Kallus*

Main category: cs.IR

TL;DR: 提出了ConvRec-R1框架，通过两阶段训练解决LLM在推荐系统中的挑战：第一阶段使用Remap-Reflect-Adjust管道构建行为克隆数据集，第二阶段提出Rank-GRPO方法优化推荐列表的排名质量。


<details>
  <summary>Details</summary>
Motivation: 解决预训练LLM在推荐任务中的三个主要问题：生成目录外项目、违反输出格式要求、列表末尾排名质量急剧下降。

Method: 两阶段框架：1）使用Remap-Reflect-Adjust管道构建高质量行为克隆数据集；2）提出Rank-GRPO方法，将推荐列表中的每个排名作为优化单位，重新定义奖励并引入基于几何平均的排名级重要性比率。

Result: 在Reddit-v2数据集上的实验表明，ConvRec-R1比GRPO基线收敛更快，在Recall和NDCG指标上表现更好。

Conclusion: ConvRec-R1框架有效解决了LLM在推荐系统中的对齐问题，Rank-GRPO方法显著提升了推荐列表的排名质量。
...
### [124] [Multimedia-Aware Question Answering: A Review of Retrieval and Cross-Modal Reasoning Architectures](https://arxiv.org/abs/2510.20193)
*Rahul Raja,Arpita Vats*

Main category: cs.IR

TL;DR: 本综述回顾了集成多媒体检索管道的问答系统最新进展，重点分析视觉、语言和音频模态与用户查询对齐的架构，分类了检索方法、融合技术和答案生成策略，并讨论了基准数据集、评估协议和性能权衡。
...
Method: 基于检索方法、融合技术和答案生成策略对方法进行分类，分析架构如何对齐视觉、语言和音频模态与用户查询。

Result: 系统分析了基准数据集、评估协议和性能权衡，识别了跨模态对齐、延迟-准确性权衡和语义基础等关键挑战。

Conclusion: 指出了构建更鲁棒和上下文感知的多媒体问答系统的开放问题和未来研究方向。
...
### [125] [Balancing Fine-tuning and RAG: A Hybrid Strategy for Dynamic LLM Recommendation Updates](https://arxiv.org/abs/2510.20260)
...
TL;DR: 本文研究了LLM驱动的推荐系统的更新策略，比较了持续微调与检索增强生成(RAG)的权衡，并提出了一种结合定期微调和低成本RAG的混合更新方法，在十亿用户平台上通过A/B测试验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 用户兴趣和内容的动态变化对LLM推荐系统构成挑战，初始微调无法捕捉实时变化，需要有效的更新机制来维持系统性能。

Method: 使用LLM驱动的用户兴趣探索系统作为案例研究，比较持续微调与RAG方法在成本、敏捷性和知识整合等维度的表现，并提出混合更新策略。

Result: 通过十亿用户平台的实时A/B实验证明，混合方法在用户满意度方面取得了统计显著的改进。

Conclusion: 混合更新策略结合了定期微调的长期知识适应性和低成本RAG的敏捷性，为维持高质量LLM推荐系统提供了实用且成本效益高的框架。
...
### [126] [Rotate Both Ways: Time-and-Order RoPE for Generative Recommendation](https://arxiv.org/abs/2510.20455)
*Xiaokai Wei,Jiajun Wu,Daiyao Yi,Reza Shirkavand,Michelle Gong*

Main category: cs.IR

TL;DR: 本文提出TO-RoPE方法，通过改进RoPE位置编码来联合建模用户行为序列中的时间顺序和事件时间，在生成式推荐系统中实现更准确的下一个项目预测。


<details>
  <summary>Details</summary>
Motivation: 现有生成式推荐系统在表示交互事件在序列中的位置（离散索引）和发生时间（挂钟时间）方面存在局限，传统方法通过学习的嵌入或相对注意力偏置注入时间信息，但效果有限。

Method: 提出Time-and-Order RoPE (TO-RoPE)系列设计，将索引和时间作为角度源来直接塑造查询-键几何结构，包括早期融合、按维度分割和按头分割三种实现方式。

Result: 在公开数据集和工业专有数据集上的广泛实验表明，TO-RoPE变体在编码时间和索引方面比现有方法持续提高准确性。
...
### [127] [Analyticup E-commerce Product Search Competition Technical Report from Team Tredence_AICOE](https://arxiv.org/abs/2510.20674)
*Rakshith R,Shubham Sharma,Mohammed Sameer Khan,Ankush Chopra*

Main category: cs.IR

TL;DR: 该研究开发了一个多语言电商搜索系统，通过数据增强和模型微调方法在Query-Category和Query-Item两个多语言相关性任务中取得了良好表现，最终在排行榜上获得第4名。


<details>
  <summary>Details</summary>
Motivation: 解决多语言电商搜索中的两个关键任务：查询-类别相关性和查询-商品相关性，确保对所有目标语言的完整覆盖。

Method: 通过将现有数据集翻译到开发集中缺失的语言来进行数据增强，然后使用多种策略微调Gemma-3 12B和Qwen-2.5 14B模型。

Result: Gemma-3 12B（4位量化）模型在使用原始和翻译数据时获得最佳QC性能，在使用原始、翻译和少数类数据创建时获得最佳QI性能。在私有测试集上平均F1分数达到0.8857。

Conclusion: 所提出的数据增强和模型微调方法在多语言电商搜索任务中表现优异，最终在竞赛中获得第4名的成绩。
...
### [128] [Generative Reasoning Recommendation via LLMs](https://arxiv.org/abs/2510.20815)
*Minjie Hong,Zetong Zhou,Zirun Guo,Ziang Zhang,Ruofan Hu,Weinan Gan,Jieming Zhu,Zhou Zhao*

Main category: cs.IR

TL;DR: GREAM是一个端到端框架，通过将预训练LLM适配为生成式推理推荐模型，解决了文本语义与协同过滤信号之间的建模差距问题。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM在多个领域展现出强大的推理能力，但作为生成式推理推荐模型存在固有挑战，包括文本语义与协同过滤信号之间的建模差距，以及用户反馈的稀疏性和随机性。

Method: GREAM框架包含三个核心组件：协作语义对齐、推理课程激活和稀疏正则化组策略优化，通过构建语义一致的离散项目索引、合成数据集和链式思维监督来实现推荐任务。

Result: 在三个数据集上的实验表明，GREAM相比强基线模型取得了持续的性能提升，为可验证RL驱动的LLM推荐器提供了实用路径。

Conclusion: GREAM为构建可验证的生成式推理推荐模型提供了有效框架，支持直接序列推荐和顺序推理推荐两种互补推理模式，实现了理解-推理-预测的统一方式。
...
TL;DR: 提出并评估了一种量子启发算法，用于解决二次无约束二进制优化（QUBO）问题。该算法使用矩阵乘积态（MPS）紧凑表示自旋构型的大叠加，通过离散驱动调度引导MPS寻找基态，在多种QUBO实例中可靠地找到全局最优解。


<details>
  <summary>Details</summary>
Motivation: QUBO问题在数学上等价于寻找伊辛自旋玻璃哈密顿量的基态，在优化问题中具有广泛应用。传统方法难以有效解决大规模QUBO问题，需要开发新的高效算法。

Method: 使用矩阵乘积态（MPS）表示自旋构型叠加，结合离散驱动调度，在每个步骤中将驱动哈密顿量（包含横向磁场）与问题哈密顿量结合，通过密度矩阵重整化群（DMRG）方法迭代更新MPS并最小化系统能量。

Result: 算法在中等难度数独谜题（超过200个伊辛自旋）和Biq Mac库中的MaxCut问题（最多251个节点和3,265条边）上成功找到全局最小值，而不仅仅是近似最优解。

Conclusion: 这种量子启发方法具有可扩展性、通用性和适用于工业规模QUBO应用的优势，为解决复杂优化问题提供了有效工具。
...
TL;DR: 该研究提出了分析可靠性基准（ARB），这是一个可复现的框架，用于量化应用于能源系统分析的大型语言模型的推理可靠性。该基准整合了五个子指标，并在确定性、概率性和认知性场景下评估模型性能。


<details>
  <summary>Details</summary>
Motivation: 人工智能和机器学习在能源领域的预测、优化和政策设计中应用日益增多，但目前缺乏标准化框架来评估这些系统的推理是否正确。当前的验证实践主要关注预测准确性或计算效率，而忽略了分析结论的逻辑完整性。

Method: 研究引入了分析可靠性基准（ARB），整合了准确性、推理可靠性、不确定性纪律、政策一致性和透明度五个子指标，使用开放技术经济数据集（NREL ATB 2024, DOE H2A/H2New, IEA WEO 2024）在确定性、概率性和认知性场景下评估四个前沿模型（GPT-4/5, Claude 4.5 Sonnet, Gemini 2.5 Pro, Llama 3 70B）。

Result: 结果显示推理可靠性可以客观测量。GPT-4/5和Claude 4.5 Sonnet实现了一致且符合政策的推理（分析可靠性指数大于90），Gemini 2.5 Pro表现出中等稳定性，而Llama 3 70B低于专业阈值。统计验证确认这些差异是显著且可复现的。
...
TL;DR: 该论文提出了一种基于有向无环图（DAG）的框架来评估LLMs在数学推理中的表现，通过逻辑接近度指标衡量推理轨迹与规则一致性，揭示了即使PASS@k指标相似时不同LLM家族在推理保真度上的显著差异。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs在数学问题上使用思维链（CoT）表现出色，但不清楚这种成功是源于搜索、死记硬背还是规则一致性推理，需要更精确的评估方法来区分这些因素。

Method: 将CoT建模为基于规则的有向无环图（DAG）随机过程，引入逻辑接近度指标，提出DAG-MATH CoT格式并构建基准测试，在标准数学推理数据集上分析不同LLM家族的推理保真度。

Result: 分析发现即使PASS@k指标相似，代表性LLM家族在推理保真度上存在统计显著差异，揭示了最终答案准确性与规则一致性推导之间的差距。
...
### [132] [A new wave of vehicle insurance fraud fueled by generative AI](https://arxiv.org/abs/2510.19957)
*Amir Hever,Itai Orr*

Main category: cs.AI

TL;DR: 生成式AI正在加剧保险欺诈问题，使大规模快速伪造事故证据变得容易。保险公司开始部署AI检测工具对抗，但面临误报漏报等挑战。本文提出UVeye分层解决方案来检测和威慑这种新型欺诈。
...
Method: 保险公司部署基于AI的深度伪造检测软件和增强验证流程来检测和缓解AI驱动的欺诈。本文提出UVeye分层解决方案，代表在检测、缓解和威慑这种新型欺诈能力上的重大进步。

Result: 当前的缓解策略面临显著限制，检测工具存在误报和漏报问题，复杂的欺诈者不断调整策略来规避自动化检查。生成式AI与检测技术之间的猫鼠游戏，加上保险公司的资源和成本障碍，使得对抗AI驱动的保险欺诈仍是一个持续挑战。

Conclusion: UVeye的分层解决方案代表了在检测、缓解和威慑AI驱动的车辆保险欺诈方面的重大进步，为解决这一持续挑战提供了新的技术途径。
...
### [133] [AI-Driven Personalized Learning: Predicting Academic Per-formance Through Leadership Personality Traits](https://arxiv.org/abs/2510.19964)
*Nitsa J Herzog,Rejwan Bin Sulaiman,David J Herzog,Rose Fong*

Main category: cs.AI

TL;DR: 该研究使用机器学习模型预测学生的学业成功，通过领导力人格特质分析129名环境工程硕士生，发现随机森林分类器在包含17个人格特征和领导力标记时达到87.50%的准确率。


<details>
  <summary>Details</summary>
Motivation: 探索AI技术在个性化学习中的潜力，通过领导力人格特质预测学业成功，为早期识别学生优缺点和选择个性化学习策略提供机会。

Method: 收集129名硕士生的5项领导力人格测试数据（23个特征）和平均成绩，使用探索性数据分析和相关性分析，通过特征选择（皮尔逊相关系数）和7种机器学习算法（SVM、LR、KNN、DT、GB、RF、XGBoost、LightGBM）进行建模。

Result: 随机森林分类器表现最佳，包含17个人格特征和领导力标记时准确率为87.50%，排除该特征时为85.71%。

Conclusion: 该研究为教育过程中早期识别学生优缺点和选择最适合的个性化学习策略提供了额外机会。
...
### [134] [LLMs can hide text in other text of the same length.ipynb](https://arxiv.org/abs/2510.20075)
...
### [135] [Human-Centered LLM-Agent System for Detecting Anomalous Digital Asset Transactions](https://arxiv.org/abs/2510.20102)
*Gyuyeon Na,Minjung Park,Hyeonjeong Cha,Sangmi Chai*

Main category: cs.AI

TL;DR: HCLA是一个以人为中心的多智能体异常检测系统，用于数字资产交易分析。系统通过自然语言对话让非专家用户查询交易异常，提供结构化分析和上下文感知的解释。


<details>
  <summary>Details</summary>
Motivation: 传统异常检测系统缺乏透明度和可解释性，非专家用户难以理解和信任检测结果。HCLA旨在通过人机协作设计提高金融取证中的透明度和信任度。

Method: 系统采用三角色架构：解析、检测和解释，通过对话工作流将用户意图转换为XGBoost检测器的输入模式，并提供基于底层特征的叙述性解释。

Result: 在比特币混币数据集上的测试显示，基线检测器达到强准确率，而HCLA系统在此基础上增加了可解释性和交互式优化能力。

Conclusion: 人机协作设计能够显著提高金融取证系统的透明度和用户信任度，HCLA为异常检测提供了可解释和交互式的新范式。
...
### [136] [The Verification-Value Paradox: A Normative Critique of Gen AI in Legal Practice](https://arxiv.org/abs/2510.20109)
*Joshua Yuvaraj*

Main category: cs.AI

TL;DR: 论文认为需要重新评估AI在法律实践中的使用，提出验证-价值悖论：AI提高效率的同时需要更多人工验证，导致净价值往往可忽略不计。


<details>
  <summary>Details</summary>
Motivation: 针对律师因提交不准确AI生成内容而受到处罚的案例，需要重新审视AI在法律实践中的使用范式，考虑AI与现实脱节、缺乏透明度以及律师的诚信义务。

Method: 提出验证-价值悖论模型，分析AI使用效率提升与验证需求增加之间的矛盾关系。

Result: AI在法律实践中的净价值往往可忽略不计，因为效率提升被相应的验证需求所抵消。

Conclusion: 需要新的AI使用范式，强调对真理的忠诚和公民责任等价值观，这对法律实践和法律教育都有重要影响。
...
### [137] [TRUST: A Decentralized Framework for Auditing Large Language Model Reasoning](https://arxiv.org/abs/2510.20188)
*Morris Yu-Chao Huang,Zhen Tan,Mohan Zhang,Pingzhi Li,Zhuo Zhang,Tianlong Chen*

Main category: cs.AI

TL;DR: TRUST是一个去中心化的AI审计框架，通过共识机制、层次化DAG分解、区块链账本和隐私保护分段技术，解决大语言模型推理链的忠实性和无害性验证问题。


<details>
  <summary>Details</summary>
Motivation: 现有审计方法存在集中化、不透明和难以扩展的问题，在关键领域部署专有模型存在重大风险。核心挑战包括鲁棒性、可扩展性、透明度和隐私保护。

Method: 采用共识机制确保30%恶意参与者下的正确性；使用层次化DAG分解推理链实现可扩展并行审计；区块链账本记录验证决策保证公共问责；隐私保护分段仅共享部分推理步骤。

Result: 在多个LLM（GPT-OSS、DeepSeek-r1、Qwen）和推理任务（数学、医学、科学、人文）上的实验表明，TRUST能有效检测推理缺陷并保持对抗性审计的鲁棒性。
...
### [138] [The Lock-In Phase Hypothesis: Identity Consolidation as a Precursor to AGI](https://arxiv.org/abs/2510.20190)
...
<details>
  <summary>Details</summary>
Motivation: 研究AGI发展过程中的身份固化现象，探讨从开放模仿到稳定身份形成的转变过程及其对AI可靠性和安全性的影响。

Method: 通过形式化身份固化阶段，将其与学习动态中的已知现象联系起来，并提出操作性指标进行检测，在不同规模模型上进行实验验证。

Result: 实验显示行为固化是快速且非线性的，但对通用能力的影响存在差异：小模型出现性能权衡，中等规模模型基本无成本，大型量化模型出现瞬时不稳定性。

Conclusion: 身份固化是AGI级可靠性的先决条件，也是安全性的关键控制点，身份可以被工程化设计以提高可靠性，但也可能在规模化过程中自发形成，导致不可预测的目标和行为固化。
...
### [139] [Individualized Cognitive Simulation in Large Language Models: Evaluating Different Cognitive Representation Methods](https://arxiv.org/abs/2510.20252)
*Tianyi Zhang,Xiaolin Zhou,Yunzhe Wang,Erik Cambria,David Traum,Rui Mao*

Main category: cs.AI

TL;DR: 该研究评估了大型语言模型在个体化认知模拟中的能力，特别是模拟作者写作风格方面。通过构建新小说数据集和11条件认知评估框架，测试了7种现成LLM，发现结合概念和语言特征的方法最有效，但LLM在模仿语言风格方面优于叙事结构。


<details>
  <summary>Details</summary>
Motivation: 虽然LLM能够表面模仿人类行为如角色扮演，但其模拟更深层次个体化认知过程的能力尚不清楚。研究旨在填补这一空白，评估不同认知表征方法在个体化认知模拟中的效果。

Method: 构建了基于新出版小说（晚于测试LLM发布日期）的数据集，提出了11条件认知评估框架，测试了7种现成LLM在作者风格模拟中的表现。评估了不同认知表征方法，包括语言特征、概念映射和基于档案的信息。

Result: 结果显示结合概念和语言特征在个体化认知模拟中特别有效，在整体评估中优于基于静态档案的线索。重要的是，LLM在模仿语言风格方面比叙事结构更有效，突显了其在更深层次认知模拟中的局限性。
...
### [140] [Using Large Language Models for Abstraction of Planning Domains - Extended Version](https://arxiv.org/abs/2510.20258)
*Bita Banihashemi,Megh Patel,Yves Lespérance*

Main category: cs.AI

TL;DR: 该研究探索使用大型语言模型（LLMs）通过上下文学习生成抽象PDDL领域和问题实例，给定自然语言指定的抽象目标。研究评估了三种抽象类型及其组合，并通过符号验证工具和专家验证生成结果。


<details>
  <summary>Details</summary>
Motivation: 为动态领域生成与特定目标对齐的抽象仍然是一个重大挑战，因为这种抽象选择会影响智能体的规划、推理和解释能力。

Method: 使用LLMs（特别是GPT-4o）通过上下文学习生成抽象PDDL领域和问题实例，考虑三种抽象类别：替代具体动作的抽象、具体动作序列的抽象、动作/谓词参数的抽象及其组合。

Result: 实验表明GPT-4o在简单设置下通常能够合成有用的规划领域抽象，但在抽象动作方面比抽象相关流态表现更好。

Conclusion: LLMs能够有效生成规划领域抽象，特别是在动作抽象方面表现良好，为自动抽象生成提供了有前景的方法。
...
Method: 提出STaBERT模型，在每个位置整合POI嵌入和派生时间描述符，构建统一的语义增强移动性表示。

Result: 实验结果显示显著改进：单城市预测的GEO-BLEU分数从0.34提升到0.75；多城市预测从0.34提升到0.56。

Conclusion: STaBERT通过语义-时间感知的BERT架构有效提升了人类移动性预测的准确性。
...
TL;DR: ToolEQA是一个集成外部工具与多步推理的EQA智能体，通过工具获取有用信息来指导探索方向，从而用更短探索距离生成更准确回答，并在多个数据集上达到最先进性能。
...
Method: 提出ToolEQA智能体，集成外部工具与多步推理，通过工具提供有用信息来指导下一步探索方向；设计自动生成EQA任务的数据生成管道，构建包含18K任务的EQA-RT数据集。
...
TL;DR: 本文基于AI4HealthyAging项目经验，分析了临床数据收集中存在的多种偏见类型，包括历史偏见、代表性偏见和测量偏见，并提出了改善临床问题设计和数据收集公平性的实用建议。


<details>
  <summary>Details</summary>
Motivation: 人工智能在医疗领域具有巨大潜力，但由于训练数据的质量和公平性问题，AI解决方案在真实临床实践中的整合仍然有限。本文旨在识别临床数据收集中存在的偏见，以促进更公平的AI系统开发。

Method: 基于西班牙国家研发计划中的AI4HealthyAging项目，通过检测临床数据收集过程中的偏见，识别了多种偏见类型在不同用例中的表现。

Result: 识别出多种偏见类型，包括历史偏见、代表性偏见和测量偏见，这些偏见体现在性别、年龄、居住地、社会经济地位、设备和标签等多个变量中。

Conclusion: 提出了改善临床问题设计和数据收集公平性和鲁棒性的实用建议，希望研究结果能为未来开发更公平的医疗AI系统提供指导。
...
TL;DR: 本文提出了一种用于军事行动中AI系统目标交战的附带损害评估模型，该模型采用设计科学方法，整合了时间、空间和力量维度，通过知识表示和推理架构提供透明的推理机制。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统在战场中的作用日益增强，需要严格评估潜在附带效应以确保负责任的打击目标选择。

Method: 采用设计科学方法，构建统一的知识表示和推理架构，整合时间、空间和力量维度，包含AI系统类别、架构组件、交战向量和上下文方面的分层结构，并考虑传播、严重性、可能性和评估指标。

Result: 通过实例化演示和评估了该模型，为构建负责任和可信赖的智能系统奠定了基础。

Conclusion: 该模型为评估军事行动中AI系统交战产生的效应提供了有效框架，支持构建负责任和可信赖的智能系统。
...
TL;DR: 该调查系统回顾了LLM驱动的知识图谱构建进展，从传统方法转向基于模式和自由模式的LLM新范式，并展望了KG推理、动态知识记忆和多模态KG等未来方向。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的出现，知识图谱构建正从传统的规则和统计方法转向语言驱动和生成框架，需要系统梳理这一范式转变的进展。

Method: 首先回顾传统KG方法，然后从两个互补视角分析新兴LLM驱动方法：基于模式的范式强调结构化和一致性；自由模式范式强调灵活性和开放发现。

Result: 系统综合了各阶段的代表性框架，分析了技术机制并识别了局限性，澄清了LLM与知识图谱之间的演化互动关系。

Conclusion: 通过系统综述，旨在弥合符号知识工程与神经语义理解之间的鸿沟，推动自适应、可解释和智能知识系统的发展。
...
TL;DR: IKnow是一个持续预训练框架，通过设计基于指令-响应对话格式的自监督目标，在无需外部资源的情况下适应大语言模型到新领域。


<details>
  <summary>Details</summary>
Motivation: 传统持续预训练方法会降低指令调优模型的指令跟随能力和语义表示，且现有解决方案需要访问原始基础模型或依赖外部领域特定数据库，这在基础模型权重因安全原因被保留或可靠外部语料不可用时存在现实障碍。

Method: 提出IKnow框架，在指令-响应对话格式中制定新颖的自监督目标，利用文本本身嵌入的领域知识，在更深语义层面进行编码学习。

Result: 该方法不依赖外部资源，能够有效保持模型的指令跟随能力，同时适应新领域。

Conclusion: IKnow提供了一个简单通用的持续预训练框架，能够在无需外部依赖的情况下有效适应大语言模型到新领域。
...
TL;DR: 本文提出了一个新的计算模型，通过实施五个功能来生成更具新颖性的创新机会，在酒店业创新项目中验证了其效果，结果显示该模型生成的结果比Notebook LM和ChatGPT4o更具新颖性和/或实用性。


<details>
  <summary>Details</summary>
Motivation: 开发一个基于创造力理论和技术的计算模型，旨在为创新项目生成更具新颖性的机会，同时不损失实用性。

Method: 实现了一个包含五个功能的计算模型，这些功能旨在帮助生成既新颖又有用的创新机会，并在酒店业创新项目中进行评估。

Result: 评估显示该计算模型生成的结果比Notebook LM和ChatGPT4o更具新颖性和/或实用性，但并非所有模型功能都对生成更新颖机会有贡献。
...
<details>
  <summary>Details</summary>
Motivation: 现有的神经符号概念学习方法在真实世界知识库中部署困难，主要是因为它们使用的描述逻辑推理器对不一致和错误数据不鲁棒。

Method: 开发EBR神经推理器，利用嵌入来近似符号推理器的结果，仅需要检索原子概念和存在限制的实例，就能检索或近似SHOIQ描述逻辑中任何概念的实例集。

Result: 实验表明，EBR与最先进的推理器相比，对缺失和错误数据具有鲁棒性。

Conclusion: EBR神经推理器能够有效解决传统符号推理器在真实世界知识库中面临的鲁棒性问题，为概念学习提供了更实用的解决方案。
...
TL;DR: FLORA是一种基于模糊逻辑的无监督知识图谱对齐方法，能够同时对齐实体和关系，提供可解释的结果，支持悬空实体，并在主要基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有知识图谱对齐方法主要关注实体级对齐，在嵌入空间中计算实体相似度，缺乏可解释的推理能力且需要训练数据。

Method: 基于模糊逻辑的迭代方法，提供实体和关系的整体对齐，具有可证明的收敛性，支持悬空实体（即另一知识图谱中没有对应实体的实体）。

Result: 在主要基准测试中实现了最先进的结果。

Conclusion: FLORA是一种简单而有效的无监督知识图谱对齐方法，通过模糊逻辑提供可解释的对齐结果，无需训练数据且性能优越。
...
<details>
  <summary>Details</summary>
Motivation: 构建可信赖的自动化决策模型，需要在真实应用中确保模型的可解释性、问责性和安全性。

Method: 形式化定义了监督学习设置，涵盖多种数据类型和决策任务；分析了特征重要性、规则和实例三类可解释模型家族；形式化了因果性、公平性和隐私性三个伦理属性及其评估指标。

Result: 建立了可解释模型与伦理属性之间的权衡分析框架，探讨了如何在可解释流程中嵌入隐私要求、公平约束和因果推理。

Conclusion: 该框架为开发准确、可解释、公平、隐私保护且具有因果意识的AI系统奠定了理论基础，即构建可信赖的AI系统。
...
TL;DR: EcomEval是一个全面的多语言多模态基准测试，用于评估LLM在电子商务领域的性能，解决了现有评估方法在任务多样性、模态覆盖和数据真实性方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有电子商务评估基准存在任务多样性有限、模态覆盖不完整、数据合成或人工整理、语言覆盖狭窄等问题，缺乏评估模型在复杂真实购物场景中能力的可靠工具。

Method: 采用半自动流程，首先由大模型生成候选回答，然后由50多名具有电子商务和多语言专业知识的专家注释者审核和修改，确保参考答案的质量和可扩展性。

Result: EcomEval涵盖6个类别和37个任务（包括8个多模态任务），基于真实客户查询和交易日志，覆盖7种语言（包括5种东南亚低资源语言），定义了每个问题和任务类别的难度级别。

Conclusion: EcomEval提供了一个全面、真实且多语言的基准测试，能够对LLM在电子商务领域进行挑战导向和细粒度评估，填补了现有评估工具的空白。
...
TL;DR: 本文提出流动性指数(FI)来量化模型在动态扩展环境中的适应性，通过评估初始、当前和未来环境状态偏差来测试响应准确性，重点关注上下文切换和连续性能力。


<details>
  <summary>Details</summary>
Motivation: 需要量化评估模型在动态扩展环境中的适应能力，区分封闭式和开放式基准测试，优先采用闭环开放式真实世界基准来测试模型的适应性。

Method: 通过测量模型对扩展环境中状态变化的理解、预测和调整能力，引入流动性指数(FI)作为量化指标，评估模型在环境变化中的响应准确性。

Result: 提出了流动性指数的概念和评估框架，能够系统性地评估模型在动态环境中的适应能力。

Conclusion: 真正超级智能的模型应至少具备二阶适应性，通过数字补充实现自我维持计算，以达到最佳流动性状态。
...
### [153] [Integrating Machine Learning into Belief-Desire-Intention Agents: Current Advances and Open Challenges](https://arxiv.org/abs/2510.20641)
*Andrea Agiollo,Andrea Omicini*

Main category: cs.AI

TL;DR: 本文对将机器学习整合到理性智能体架构中的现有方法进行了系统化分析，特别关注BDI（信念-欲望-意图）范式，识别了关键研究机会和开放挑战。


<details>
  <summary>Details</summary>
Motivation: 由于机器学习在感知和认知任务中展现出类人能力，将其整合到理性智能体架构中的框架日益受到关注。然而，现有研究呈现碎片化状态，往往只关注将ML嵌入通用智能体容器，而忽视了理性架构的表达能力。

Method: 使用BDI范式作为参考框架，对现有方法进行细粒度系统化分析，梳理了ML增强理性智能体的快速演进文献。

Result: 分析展示了ML增强理性智能体文献的快速发展，并识别了设计有效理性ML智能体的关键研究机会。

Conclusion: 本文为设计有效的理性ML智能体提供了系统化分析框架，指出了重要的研究方向和技术挑战。
...
### [154] [The Shape of Reasoning: Topological Analysis of Reasoning Traces in Large Language Models](https://arxiv.org/abs/2510.20665)
*Xue Wen Tan,Nathaniel Tan,Galen Lee,Stanley Kok*

Main category: cs.AI

TL;DR: 本文提出了一种基于拓扑数据分析（TDA）的评估框架，用于自动评估大语言模型推理轨迹的质量，相比传统的图结构指标具有更高的预测能力。


<details>
  <summary>Details</summary>
Motivation: 当前评估大语言模型推理轨迹质量的方法存在研究不足、劳动密集和不可靠的问题，主要依赖专家评分、人工标注和缓慢的成对判断，自动化方法则过度依赖图结构连通性指标，无法准确捕捉高质量推理的本质特征。

Method: 引入拓扑数据分析（TDA）框架，通过捕捉推理轨迹的几何结构来实现标签高效的自动化评估，使用拓扑特征而非传统的图结构指标。

Result: 实证研究表明，拓扑特征在评估推理质量方面比标准图指标具有显著更高的预测能力，表明有效推理更好地由高维几何结构而非纯关系图来捕捉。

Conclusion: 一个紧凑、稳定的拓扑特征集能够可靠地指示轨迹质量，为未来的强化学习算法提供了实用的信号。
...
TL;DR: 本文提出了一种基于广义均值积分的AGI一致性度量方法，替代了传统算术平均的补偿性假设，通过计算曲线下面积来量化不同补偿性假设下的鲁棒性，发现GPT-4和GPT-5尽管算术得分较高，但距离真正的通用智能仍有很大差距。


<details>
  <summary>Details</summary>
Motivation: 现有AGI定义基于CHC认知模型的算术平均，假设了补偿性（某些领域的卓越能力可以弥补其他领域的失败），但真正的通用智能应该反映跨所有基本领域的一致充分性。

Method: 提出基于广义均值积分的一致性感知AGI度量方法，涵盖算术、几何和调和均值机制，通过计算曲线下面积来量化不同补偿性假设下的鲁棒性。

Result: 应用于GPT-4和GPT-5的CHC领域得分，一致性调整的AUC显示尽管算术得分较高（如GPT-5达到24%），但两个系统距离通用能力仍然很远。

Conclusion: 广义均值积分为衡量AGI真实进展提供了一个原则性、可解释且更严格的基础，能够惩罚不平衡性并捕捉领域间依赖性。
...
TL;DR: 本文提出了Real Deep Research (RDR)框架，用于系统分析快速发展的AI和机器人研究领域，识别新兴趋势、发现跨领域机会，并为新研究提供具体起点。


<details>
  <summary>Details</summary>
Motivation: AI和机器人领域每年产生超过10,000篇论文，研究人员难以跟上快速发展的趋势、跨学科工作的兴起以及需要探索专业领域之外的挑战。

Method: 开发了一个通用的RDR管道，能够系统分析任何研究领域，重点关注基础模型和机器人技术进展，并扩展到其他科学领域。

Result: 构建了完整的RDR管道，并在附录中提供了每个分析主题的广泛结果。

Conclusion: 该工作为AI及其他领域的研究人员提供了有价值的见解，帮助他们在快速发展的研究环境中保持更新并发现新的研究方向。
