...
Motivation: 当前数学表达式识别方法在处理简单表达式时表现良好，但在处理具有多行、多符号的复杂数学表达式时性能显著下降，主要原因是现有训练数据集主要由简单样本组成，缺乏对复杂表达式的充分覆盖。

Method: 1. 构建CMER-Bench基准测试，将表达式分为简单、中等、复杂三个难度等级；2. 创建大规模数据集MER-17M和CMER-3M，特别强调复杂数学表达式的覆盖；3. 提出结构化数学语言表示方法，显式建模表达式的层次和空间结构；4. 基于编码器-解码器架构开发专门模型CMERNet，使用CMER-3M数据集进行训练。

Result: 实验结果表明，CMERNet仅用1.25亿参数就在CMER-Bench上显著优于现有的数学表达式识别模型和多模态大语言模型，特别是在复杂数学表达式识别方面表现出色。

Conclusion: 通过构建专门的基准测试、大规模数据集、新的表示方法和专门模型，该研究成功解决了复杂数学表达式识别的挑战，为开发准确且鲁棒的复杂数学表达式识别系统提供了有效解决方案。
...
TL;DR: 本文探讨了AIGC辅助新闻图像生产的可控性路径，通过两个实验构建了人机协作的模块化流程，并提出评估指标以解决新闻业中的真实性、语义对齐和伦理问题。
...
Result: 实验1发现训练语料偏见和平台级过滤导致语义对齐、文化特异性和视觉真实性的差异；实验2成功构建了可追溯部署的模块化流程，能够保持语义表征的准确性。

Conclusion: 提出了新闻特殊报道中AIGC辅助图像生产的人机协作机制，并建议评估角色身份稳定性、文化表达准确性和用户-公众适宜性三个指标，以促进可控、可信的新闻图像生产。
...
TL;DR: 该研究提出了一个结合深度学习图像分类器与大型语言模型的框架，用于生成胃肠道疾病的内窥镜图像临床解释，但发现当前LLMs在医学推理中仍不可靠。
...
Method: 引入一个连接图像分类与结构化临床推理的框架：1) 设计MobileCoAtNet混合模型用于内窥镜图像分类；2) 将其输出驱动多个LLMs进行推理；3) 建立两个专家验证的基准来评估推理质量。

Result: MobileCoAtNet在八个胃相关类别上达到高准确率，强分类能提高LLMs解释质量，但所有模型都未达到人类水平的稳定性，即使最佳LLMs也会随提示变化而改变推理。

Conclusion: 结合DL与LLMs能生成有用的临床叙述，但当前LLMs在高风险医学决策中仍不可靠，该框架为理解其局限性和构建更安全的推理系统提供了路径。
...
<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在一般视觉语言任务上表现出色，但在医疗决策任务上表现不佳，需要探究其原因并寻找改进方法。

Method: 使用两个具有挑战性的医疗数据集（阿尔茨海默病三阶段分类和MIMIC-CXR胸片分类）进行实证研究，探索了三种改进策略：带推理注释的上下文学习、视觉描述后文本推理、以及视觉塔的少样本微调。

Result: 研究发现文本推理始终优于视觉或多模态输入，多模态输入甚至比纯文本表现更差，表明当前MLLMs缺乏扎实的视觉理解能力。

Conclusion: 当前多模态大语言模型在医疗决策任务上存在视觉理解不足的问题，需要改进视觉基础能力，为医疗领域的多模态决策提供了有前景的改进方向。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在追求通用人工智能中至关重要，但实现多模态理解与生成的统一目标面临优化冲突和性能权衡的挑战。需要在不损害现有理解能力的前提下有效提升生成性能。
...
Conclusion: STAR通过任务渐进式的堆叠自回归方案，成功解决了多模态理解与生成之间的优化冲突，实现了性能的统一提升，为多模态大语言模型的发展提供了有效框架。
...
TL;DR: 该论文提出了一种将几何与外观解耦的紧凑表示方法，使用面元表示几何，结合全局神经场和每个基元的颜色表示外观，相比3D高斯泼溅减少了9.7-31倍的基元数量和3.7-5.5倍的内存占用。


<details>
  <summary>Details</summary>
Motivation: 尽管高斯泼溅在新视角合成方面取得了令人印象深刻的结果，但即使场景几何简单，它也需要数百万个基元来建模高纹理场景。现有方法在表示效率和计算成本方面存在不足。

Method: 提出了一种超越基于点渲染的表示方法，将几何和外观解耦：1）使用面元表示几何；2）外观采用全局神经场和每个基元颜色的组合；3）神经场为每个像素纹理固定数量的基元，确保计算开销低。

Result: 在室外场景中使用9.7倍更少的基元和5.5倍更少的内存，在室内场景中使用31倍更少的基元和3.7倍更少的内存，同时匹配3D高斯泼溅的感知质量。渲染速度比现有纹理基元快两倍，视觉质量更好。

Conclusion: 该解耦表示方法在保持高质量新视角合成的同时，显著减少了基元数量和内存占用，提高了渲染效率，为紧凑高效的3D场景表示提供了新思路。
...
TL;DR: MoLingo是一个文本到动作生成模型，通过在连续潜在空间中进行去噪来生成逼真的人体动作。该研究关注如何优化连续动作潜在空间中的扩散过程，提出了语义对齐的潜在空间构建和交叉注意力文本条件注入方法，在标准指标和用户研究中达到了新的最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有文本到动作生成模型在连续潜在空间中进行扩散时存在两个关键问题：1）如何构建语义对齐的潜在空间以提高扩散效果；2）如何最佳地注入文本条件以使动作紧密跟随描述。这些问题的解决对于提升动作生成的逼真度和文本-动作对齐至关重要。

Method: 1）提出语义对齐的动作编码器，使用帧级文本标签进行训练，使具有相似文本含义的潜在表示保持接近，从而创建更适合扩散的潜在空间；2）比较单令牌条件注入与多令牌交叉注意力方案，发现交叉注意力能提供更好的动作逼真度和文本-动作对齐；3）结合语义对齐潜在、自回归生成和交叉注意力文本条件。

Result: MoLingo在人体动作生成的标准指标和用户研究中均达到了新的最优性能。通过语义对齐的潜在空间和交叉注意力文本条件注入，模型显著提升了动作的逼真度和与文本描述的对齐程度。

Conclusion: MoLingo通过构建语义对齐的潜在空间和采用交叉注意力文本条件注入，有效提升了文本到动作生成的质量。该方法在连续潜在空间扩散中取得了最佳效果，为人体动作生成领域提供了新的技术方案，并承诺开源代码和模型以促进进一步研究。
...
TL;DR: CFHA是一个三阶段扩散框架，通过全局风格对齐、局部细化和幻觉去除来缩小无人机检测中合成数据与真实数据之间的域差距，显著提升检测精度。


<details>
  <summary>Details</summary>
Motivation: 无人机人体检测需要大量标注数据，但真实标注成本高且目标分布不断变化。合成数据虽成本低，但存在域差距问题，影响模型在真实场景的应用效果。

Method: 提出CFHA三阶段框架：1) 全局风格转移：扩散模型将合成图像的颜色、光照和纹理统计对齐到真实风格；2) 局部细化：超分辨率扩散模型增强小物体细节；3) 幻觉去除：过滤不符合真实世界属性的人体实例。

Result: 在公开的无人机Sim2Real检测基准测试中，CFHA显著提升检测精度，在Semantic-Drone基准上mAP50提升高达+14.1。消融研究证实了全局和局部阶段的互补作用。

Conclusion: CFHA通过分层对齐方法有效缩小合成与真实数据之间的域差距，为无人机人体检测提供了一种高效的域适应解决方案，同时保留了原始合成标签。
...
TL;DR: RUNE是一个结合大型语言模型和神经符号AI的遥感图像检索方法，通过显式推理而非隐式嵌入来提升性能、可解释性和处理复杂空间关系的能力。


<details>
  <summary>Details</summary>
Motivation: 当前遥感大视觉语言模型在可解释性和处理复杂空间关系方面存在局限，限制了实际应用。需要一种能够进行显式推理、提高可解释性和性能的方法。

Method: RUNE结合LLMs和神经符号AI，将文本查询转换为一阶逻辑表达式，然后通过神经符号推理模块在检测到的实体上进行显式推理。采用逻辑分解策略处理条件实体子集以提高可扩展性。

Result: 在重新设计的DOTA数据集上评估，RUNE在复杂遥感检索任务中优于最先进的RS-LVLMs，并引入RRQC和RRIU两个新指标评估性能。在洪水后卫星图像检索用例中展示了实际应用潜力。

Conclusion: RUNE通过显式神经符号推理在遥感图像检索中实现了性能、鲁棒性和可解释性的提升，为实际应用提供了有前景的解决方案。
...
TL;DR: Route-DETR通过自适应成对路由机制解决DETR中查询竞争问题，区分竞争查询和互补查询，在训练时引入路由偏置但推理时无额外计算成本，在多个基准上取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: DETR虽然提供了端到端的物体检测解决方案，但存在查询竞争效率低下的问题，多个查询会收敛到相似位置，导致冗余计算。需要解决查询竞争和冗余问题以提高检测效率。

Method: 提出Route-DETR，通过解码器自注意力层中的自适应成对路由机制，使用查询间相似度、置信度分数和几何信息区分竞争查询和互补查询。引入双路由机制：抑制器路由减少竞争查询间的重复，委托器路由鼓励探索不同区域。采用可学习的低秩注意力偏置实现非对称查询交互。使用双分支训练策略，仅在训练时引入路由偏置，推理时保持标准注意力，确保无额外计算成本。

Result: 在COCO和Cityscapes数据集上的实验表明，该方法在多个DETR基线模型上取得一致改进。在ResNet-50上相比DINO获得+1.7% mAP提升，在Swin-L上达到57.6% mAP，超越了先前的最先进模型。

Conclusion: Route-DETR通过自适应路由机制有效解决了DETR中的查询竞争问题，提高了检测效率，在多个基准数据集上实现了显著的性能提升，同时保持了推理时的计算效率。
...
TL;DR: 提出KLO-Net用于前列腺MRI分割，结合动态K近邻注意力机制和CSP编码器，在保持准确性的同时提升计算效率


<details>
  <summary>Details</summary>
Motivation: 临床工作站上实时部署前列腺MRI分割常受计算负载和内存占用限制，深度学习分割方法面临解剖变异性的挑战，需要在效率和准确性之间取得平衡

Method: 提出KLO-Net架构，包含动态K-NN注意力机制（自适应确定每个空间位置的注意力连接数）和CSP编码器（减少内存消耗），基于U-Net框架

Result: 在PROMISE12和PROSTATEx两个公开数据集上进行实验和消融研究，验证了模型在计算效率和分割质量方面的优势

Conclusion: KLO-Net通过动态K-NN注意力机制和CSP编码器，有效解决了前列腺MRI分割的计算效率问题，同时保持了可靠的分割精度
...
Method: 引入"去品牌化"新任务，构建全面的基准数据集。提出基于视觉语言模型（VLM）的新评估指标，使用问答框架探测图像中的显式商标和隐式整体品牌特征。该指标能捕捉传统商标检测器无法识别的抽象商业外观（如可口可乐瓶形状）。

Result: 研究发现随着模型保真度提高（如SDXL、FLUX比Stable Diffusion更容易合成品牌标识），去品牌化挑战更加紧迫。基于VLM指标的验证结果表明，去品牌化是一个独特且具有实际意义的问题，需要专门技术解决。

Conclusion: 去品牌化是一个新颖且重要的研究任务，需要专门的方法来处理商标和结构性品牌特征的精细移除。提出的VLM评估指标能有效评估去品牌化效果，为未来研究提供了基准和评估框架。
...
TL;DR: FocalComm是一个专注于交换难实例特征的多智能体协同感知框架，通过渐进式难实例挖掘和基于查询的特征融合，显著提升了行人等小目标检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有协同感知方法主要优化车辆检测性能，对行人等安全关键的小目标检测效果不佳，且依赖全特征交换而非仅交换有助于减少漏检的关键特征。

Method: 提出FocalComm框架，包含两个关键设计：(1)可学习的渐进式难实例挖掘模块，提取每个智能体的难实例导向特征；(2)基于查询的特征级融合技术，在协作过程中动态加权这些识别出的特征。

Result: 在两个真实世界数据集(V2X-Real和DAIR-V2X)上，FocalComm在车辆中心和基础设施中心两种协作设置下均优于最先进的协同感知方法，在V2X-Real数据集上行人检测性能显著提升。

Conclusion: FocalComm通过专注于交换难实例特征，有效提升了多智能体协同感知中安全关键小目标的检测性能，为自动驾驶安全提供了更可靠的感知解决方案。
...
TL;DR: 该研究提出了一种将2D扩散模型适配于3D点云补全的框架，通过引入Shape Atlas这一紧凑的2D表示来解决3D数据稀缺和模态差异问题。


<details>
  <summary>Details</summary>
Motivation: 虽然文本到图像的2D扩散模型在丰富数据上取得了显著成功，但3D扩散模型由于高质量3D数据集稀缺以及3D输入与2D潜在空间之间的模态差异而发展滞后。需要克服这些限制以实现高质量的3D形状补全。

Method: 引入Shape Atlas作为3D几何的紧凑2D表示，该表示能够充分利用预训练2D扩散模型的生成能力，同时对齐条件输入和输出空间的模态，从而实现更有效的条件控制。这种统一的2D表述便于从有限的3D数据中学习。

Result: 在PCN和ShapeNet-55数据集上验证了方法的有效性，能够生成高质量、保留细节的形状补全结果。还展示了将补全点云转换为艺术家创建网格的下游应用，证明了方法的实用性。

Conclusion: 提出的框架成功地将2D扩散模型适配于3D形状补全任务，通过Shape Atlas表示克服了数据稀缺和模态差异问题，实现了高质量的3D补全结果并具有实际应用价值。
...
TL;DR: Sparse-LaViDa通过动态截断冗余掩码标记来加速掩码离散扩散模型的推理速度，同时引入寄存器标记保持生成质量，实现了2倍加速而不损失性能。


<details>
  <summary>Details</summary>
Motivation: 掩码离散扩散模型（MDMs）在多模态任务中表现出色，但其推理速度不理想，因为每个采样步骤都需要重复处理冗余的掩码标记，这限制了实际应用效率。

Method: 提出Sparse-LaViDa框架，在推理时动态截断不必要的掩码标记；引入专门的寄存器标记作为截断标记的紧凑表示；设计专门的注意力掩码以确保训练和推理的一致性；基于最先进的统一MDM LaViDa-O构建。

Result: 在文本到图像生成、图像编辑和数学推理等多样化任务中，Sparse-LaViDa实现了高达2倍的加速，同时保持了生成质量。

Conclusion: Sparse-LaViDa通过动态截断冗余掩码标记和引入寄存器标记的有效策略，显著提升了掩码离散扩散模型的推理效率，为实际应用提供了可行的加速方案。
...
TL;DR: KFS-Bench是首个用于长视频问答中关键帧采样的基准测试，包含多场景标注，可直接评估采样策略，并提出了新的采样质量指标和自适应平衡采样方法。


<details>
  <summary>Details</summary>
Motivation: 现有工作在长视频问答中仅通过问答准确性间接评估帧选择质量，缺乏直接评估关键帧采样策略的基准测试。关键帧采样对于高效的长视频理解至关重要，需要能够直接分析不同采样方法如何捕捉整个长视频中关键内容的方法。

Method: 1) 创建KFS-Bench基准测试，包含多场景标注，可直接评估采样策略；2) 对关键帧采样方法进行全面研究；3) 设计新的采样质量指标，综合考虑采样精度、场景覆盖和采样平衡；4) 开发基于问题-视频相关性的自适应平衡采样方法，平衡采样多样性与问题-帧相似性。

Result: 研究发现采样精度、场景覆盖和采样平衡是影响问答性能的关键因素。提出的新采样质量指标与问答准确性相关。自适应平衡采样方法在关键帧采样和问答性能方面均取得优越表现。

Conclusion: KFS-Bench为长视频问答中的关键帧采样提供了首个直接评估基准。通过综合考虑采样精度、场景覆盖和采样平衡，提出的自适应平衡采样方法能够有效提升问答性能，为长视频理解提供了更有效的采样策略。
...
<details>
  <summary>Details</summary>
Motivation: 传统几何模型在场景理解中存在局限性，特别是在处理遮挡、纹理缺失表面等复杂环境时表现不足。深度学习技术为自主机器人提供了更强大的感知能力，能够更好地理解动态非结构化环境，从而提升决策、导航和交互能力。

Method: 本文采用文献综述方法，系统梳理深度学习在机器人场景理解中的关键应用领域：1) 目标检测技术；2) 语义分割和实例分割；3) 深度估计方法；4) 3D重建技术；5) 视觉SLAM系统。重点分析这些技术如何集成到完整感知系统中。

Result: 深度学习技术显著提升了自主机器人的场景理解能力：1) 克服了传统几何模型在遮挡和纹理缺失表面的局限；2) 实现了实时深度感知；3) 增强了语义推理能力；4) 在动态非结构化环境中集成感知模块后，决策、导航和交互效果得到改善。

Conclusion: 深度学习在自主机器人场景理解中展现出巨大潜力，但仍有待解决的问题。本文总结了现有挑战并指出了未来研究方向，为基于学习的场景理解技术进一步发展提供了指导。
...
TL;DR: CITab是一个用于医学图像和表格数据多模态学习的自监督学习框架，通过语义感知的表格建模机制和原型引导的混合线性层模块，解决了现有方法在异构表格数据上的跨队列迁移问题。


<details>
  <summary>Details</summary>
Motivation: 现有的自监督学习方法在处理医学图像和表格数据时，由于对异构表格数据的建模机制过于僵化，通常局限于特定的数据队列，无法有效学习跨不同队列的可迁移医学知识。

Method: 提出CITab框架，从语义感知角度设计表格建模机制，整合列标题作为语义线索；提出原型引导的混合线性层模块进行表格特征专业化处理，有效处理表格数据的异质性并探索潜在的医学概念。
...
<details>
  <summary>Details</summary>
Motivation: 传统结构光方法在像素域进行匹配，在遮挡、精细结构、非朗伯表面等挑战性场景下鲁棒性有限。受神经特征匹配进展启发，需要在特征空间而非脆弱像素域进行鲁棒对应匹配

Method: 1) 从投射图案和捕获的红外图像提取神经特征，在特征空间构建代价体以显式结合几何先验；2) 引入深度细化模块，利用大规模单目深度估计模型的强先验改善细节恢复和全局结构一致性；3) 开发基于物理的结构光渲染流水线，生成近百万合成图案-图像对用于训练

Result: 方法仅用合成数据训练，能良好泛化到真实室内环境，有效处理多种图案类型而无需重新训练，在性能上持续优于商业结构光系统和被动立体RGB深度估计方法

Conclusion: 提出的学习型结构光解码框架通过在特征空间进行匹配并利用单目深度先验，显著提升了在挑战性场景下的鲁棒性和深度质量，为3D成像提供了新思路
...
TL;DR: ASAP Textured Gaussians通过自适应采样和各向异性参数化，显著提升了纹理化3D高斯泼溅的内存效率，在保持高质量渲染的同时大幅减少了纹理参数数量。


<details>
  <summary>Details</summary>
Motivation: 现有的纹理化3D高斯方法存在两个关键限制：1) 纹理定义在规范空间中，导致对低贡献区域的低效采样；2) 所有高斯都采用统一的纹理参数化，不考虑视觉复杂度，导致过度参数化。
...
Conclusion: 通过解决现有纹理化高斯方法的关键限制，ASAP方法在保持渲染质量的同时大幅提升了内存效率，为纹理化3D高斯泼溅提供了更优的质量效率平衡。
...
TL;DR: OmniDrive-R1是一个用于自动驾驶的端到端视觉语言模型框架，通过交错多模态思维链机制统一感知与推理，采用强化学习驱动的视觉定位能力，无需密集标注即可实现细粒度分析，显著提升了自动驾驶场景下的推理性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在自动驾驶等安全关键领域部署面临可靠性问题，特别是物体幻觉问题。现有方法存在感知与推理阶段解耦、依赖昂贵密集定位标注等根本缺陷，需要开发更可靠的端到端解决方案。

Method: 提出OmniDrive-R1框架，采用交错多模态思维链机制统一感知与推理。核心创新是强化学习驱动的视觉定位能力，通过纯两阶段强化学习训练流程和Clip-GRPO算法实现，该算法引入无标注、基于过程的定位奖励，通过强制视觉焦点与文本推理的实时跨模态一致性来确保稳定性。

Result: 在DriveLMM-o1数据集上的广泛实验表明，相比基线Qwen2.5VL-7B模型，OmniDrive-R1将整体推理分数从51.77%提升到80.35%，最终答案准确率从37.81%提升到73.62%，性能显著改善。

Conclusion: OmniDrive-R1通过端到端统一感知与推理、强化学习驱动的视觉定位以及无标注的定位奖励机制，有效解决了自动驾驶场景中视觉语言模型的可靠性问题，为安全关键领域的部署提供了可行方案。
...
Method: 提出SELECT方法，使用图像-文本编码器和字符级分词器处理场景文本；引入SSLC（相似性序列标签污染）过程，在训练时故意引入错误模拟真实场景，考虑字符视觉相似性并可能导致序列长度变化。

Result: SELECT在检测标签错误方面优于现有方法，能有效提高真实世界文本数据集上的场景文本识别准确率，展示了实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型虽然具备强大的感知和推理能力，但高计算和内存需求使其难以直接部署在设备端环境。标准视觉Transformer编码器在处理高分辨率输入时存在过度延迟和内存消耗的问题。

Method: 采用图像分块策略限制峰值内存使用；引入视觉分辨率压缩器自适应预测最优编码分辨率以消除冗余计算；提出双重一致性学习技术，在统一框架内对齐多尺度ViT编码器，实现共享LLM下视觉分支的动态切换。
...
TL;DR: 该论文提出将说话人脸编辑和生成统一为语音条件面部运动填充任务，并开发了FacEDiT模型，通过扩散变换器和流匹配实现局部编辑与生成的无缝过渡。


<details>
  <summary>Details</summary>
Motivation: 传统上说话人脸编辑和生成被视为独立任务，缺乏统一框架。作者认为两者本质上是同一问题的子任务，需要一种能够同时处理局部编辑和整体生成的统一方法。

Method: 提出FacEDiT模型，基于扩散变换器（Diffusion Transformer）和流匹配训练，采用掩码自编码器思想，学习在周围运动和语音条件下合成被掩码的面部运动。引入偏置注意力和时间平滑约束来增强边界连续性和唇部同步。

Result: 创建了首个说话人脸编辑数据集FacEDiTBench，包含多样编辑类型和长度。实验验证了说话人脸编辑和生成确实可作为语音条件运动填充的子任务出现，FacEDiT能够产生准确、语音对齐的面部编辑，具有强身份保持和平滑视觉连续性。

Conclusion: 通过将说话人脸编辑和生成统一为语音条件面部运动填充任务，FacEDiT模型成功实现了局部编辑（替换、插入、删除）与整体生成的统一处理，为动态说话人脸合成提供了新的框架和方法。
...
TL;DR: 提出基于多模态深度学习的实时室内工作面照度预测框架，通过仅提取侧窗区域图像特征，适用于动态占用空间，在广州实测验证中取得高精度


<details>
  <summary>Details</summary>
Motivation: 现有室内日光预测研究多针对静态场景，而日光联动控制（DLCs）在建筑节能中潜力巨大，需要实时准确预测室内工作面照度分布，特别是在动态占用空间中

Method: 提出多模态深度学习框架，从非侵入性图像中提取时空特征，仅从侧窗区域提取图像特征而非室内像素，确保在动态占用空间中的适用性；在广州测试房间进行现场实验，收集17,344个样本用于模型训练和验证

Result: 模型在同分布测试集上R2 > 0.98，RMSE < 0.14；在未见天测试集上R2 > 0.82，RMSE < 0.17，表明具有高精度和可接受的时间泛化能力

Conclusion: 该多模态深度学习框架能够实时准确预测室内工作面照度分布，仅使用侧窗区域图像特征使其适用于动态占用空间，为日光联动控制提供了有效的实时预测工具
...
TL;DR: SDAR-VL首次将块级离散扩散系统应用于大规模视觉语言理解，通过集成框架解决训练成本高、收敛慢和不稳定问题，在21个基准测试中超越了传统块扩散和自回归基线。


<details>
  <summary>Details</summary>
Motivation: 块级离散扩散在并行生成和因果依赖建模之间提供了良好平衡，但实际应用受到高训练成本、收敛慢和不稳定性的限制，使其落后于自回归基线。需要开发高效稳定的训练框架来释放块级扩散在视觉语言建模中的潜力。

Method: 提出了集成训练框架，包含三个核心组件：(1) 异步块级噪声调度，在每个批次内多样化监督；(2) 有效掩码比率缩放，在随机掩码下实现无偏损失归一化；(3) 渐进式Beta噪声课程，增加有效掩码覆盖同时保持破坏多样性。

Result: 在21个单图像、多图像和视频基准测试中，SDAR-VL相比传统块扩散显著提升了训练效率、收敛稳定性和任务性能。在匹配设置下，达到或超越了LLaVA-OneVision等自回归基线和LLaDA-V等全局扩散基线，建立了块级扩散作为视觉语言理解的实用骨干。

Conclusion: SDAR-VL成功将块级离散扩散应用于大规模视觉语言理解，通过创新的训练框架解决了该方法的固有挑战，证明了块级扩散作为视觉语言建模骨干的实用性和竞争力。
...
TL;DR: 提出GaussianPlant方法，基于3D高斯泼溅技术从多视角图像联合恢复植物外观和内部结构，实现高保真外观重建和准确结构提取。


<details>
  <summary>Details</summary>
Motivation: 虽然3DGS在场景外观重建和新视角合成方面表现出色，但缺乏对底层结构（如植物分枝模式）的表征，这限制了其在植物表型分析等任务中的应用。

Method: 引入GaussianPlant分层表示，使用结构基元（StPs）显式表示枝干和叶片几何（圆柱和圆盘），外观基元（ApPs）表示植物外观。通过自组织方式优化StP属性区分枝叶，ApPs绑定到StPs，使用重渲染损失和梯度流联合优化。
...
TL;DR: 提出一个基于质量的预测框架，通过整合多种信号源和动态可靠性评估来提升视频呼吸率估计的准确性。
...
Method: 从面部远程光电容积描记、上半身运动和深度学习管道中提取10种信号，使用四种频谱估计器分析，利用分段质量指标训练机器学习模型来预测准确性或选择最可靠信号，实现自适应信号融合和基于质量的分段过滤。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型计算成本高，Classifier-Free Guidance（CFG）虽然提升生成质量和可控性，但需要条件和无条件前向传播，使计算量加倍。现有方法在可变引导模式下缓存效果不佳。

Method: 提出两阶段方法：第一阶段使用进化算法联合优化跳过的时间步和引导尺度，减少无条件传播；第二阶段引入自适应秩分配，针对不同变换器块调整校准工作，在可变引导下保持缓存有效性。
...
TL;DR: ViewMask-1-to-3是一种创新的多视角图像生成方法，使用离散扩散模型将多视角合成转化为序列建模问题，通过视觉标记和文本输入实现跨视角一致性，无需复杂3D几何约束。


<details>
  <summary>Details</summary>
Motivation: 从单张图像和文本描述生成多视角图像具有挑战性，现有方法通常依赖3D感知架构或专用扩散模型，需要大量多视角训练数据和复杂几何先验。本文旨在探索更简单有效的多视角生成方法。

Method: 提出ViewMask-1-to-3方法，将多视角合成建模为离散序列问题：1) 使用MAGVIT-v2将每个视角表示为视觉标记；2) 通过掩码标记预测统一语言和视觉；3) 通过迭代解掩码和文本输入逐步生成多个视角；4) 使用随机掩码和自注意力实现跨视角一致性，无需复杂3D约束。

Result: 在GSO和3D-FUTURE数据集上，ViewMask-1-to-3在PSNR、SSIM和LPIPS指标上平均排名第一，证明了离散扩散模型作为多视角生成方法的可行性和优越性。

Conclusion: 离散扩散模型为多视角图像生成提供了可行且简单的替代方案，通过序列建模方法实现了跨视角一致性，同时保持了架构的简洁性，无需复杂3D几何约束或专用注意力架构。
...
Conclusion: 提出的训练和数据无关的遗忘框架实现了三种不同的遗忘范式，能够有效移除特定类别信息而不影响模型在其他任务上的性能
...
<details>
  <summary>Details</summary>
Motivation: 现有方法为不同颜色通道训练独立的GAN模型来去除阴影和噪声，但部署多个GAN会导致训练和推理时间过长。需要一种更高效的文档图像增强和二值化方法。
...
Conclusion: MFE-GAN通过多尺度特征提取和Haar小波变换，实现了高效的文档图像增强和二值化，为OCR系统提供了更快速且性能相当的预处理方案。
...
TL;DR: 提出了一种用于动态场景理解的连续概率时空表示方法，通过可变形3D高斯模型联合编码辐射和语义信息，实现跨时空一致性的实例表示。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖离散跟踪或视角依赖特征，难以在动态场景中实现跨时空一致的实例表示。需要一种能解耦可见性和持久对象身份的方法来提升动态场景理解能力。

Method: 1. 提出基于可变形3D高斯的实例嵌入表示，联合编码辐射和语义信息；2. 通过可微分光栅化直接从RGB图像和实例掩码学习；3. 引入高斯身份校准机制；4. 向语义活跃区域重采样高斯。

Result: 在HyperNeRF和Neu3D数据集上，该方法在新型视角全景分割和开放词汇4D查询任务上显著优于现有最先进方法。

Conclusion: 提出的连续实例场表示能有效建模动态场景中的时空一致性，为动态场景理解提供了更强大的表示方法。
...
TL;DR: 提出一种基于零空间投影的选择性遗忘方法，用于多模态模型（如CLIP）中无需重新训练即可擦除目标类别信息


<details>
  <summary>Details</summary>
Motivation: 传统遗忘技术需要迭代微调和大量数据整理，计算成本高且不够精确。需要一种更高效、更精确的方法来从预训练多模态模型中移除特定类别信息，同时保持模型整体知识

Method: 利用零空间投影技术，计算目标文本嵌入张成的子空间的正交基，然后将这些方向投影到零空间，从而擦除最终投影层中的目标类别信息。无需重新训练或使用遗忘集的图像

Result: 该方法显著降低了图像特征与不需要类别之间的对齐度，导致目标类别的零样本性能显著下降，同时保持了模型的整体多模态知识。部分投影可以在完全遗忘和保留有用信息之间取得平衡

Conclusion: 该方法为模型去污染和隐私保护提供了计算高效且精确的选择性遗忘解决方案，无需重新训练即可实现目标类别的信息擦除
...
<details>
  <summary>Details</summary>
Motivation: 现有的图像编辑系统难以在保持线条艺术稀疏、风格敏感结构的同时，支持高级语义变化和精确局部重绘。需要一种能统一全局编辑和局部重绘的草图编辑系统。

Method: 1) 开发可控数据生成流程：从无属性基础草图构建属性添加序列，通过跨序列采样形成多步编辑链，使用风格保持属性移除模型扩展风格覆盖；2) 基于DiT的统一草图编辑框架：重新利用RGB通道编码输入，实现指令引导编辑和线条引导重绘的切换；3) 集成任务引导的专家混合到LoRA层，通过文本和视觉线索路由，提高语义可控性、结构保真度和风格保持。

Result: 在两种任务上都取得了最先进的结果，相比现有基线具有更优的指令遵循能力和风格/结构保持能力。

Conclusion: SketchAssist及其数据集为草图创建和修订提供了一个实用、可控的助手，能够有效加速创作过程。
...
TL;DR: 该研究提出了首个用于评估和改进机器学习模型检测性能反模式能力的基准数据集，包含600多个PyTorch跟踪数据，并提出了一种轻量级ML模型与LLM结合的迭代方法，显著优于现有技术。


<details>
  <summary>Details</summary>
Motivation: 机器学习模型中的性能反模式识别对高效训练和推理至关重要，但通常需要跨系统基础设施、ML模型和内核开发的深厚专业知识。当前大型科技公司依赖专门的ML基础设施工程师分析torch跟踪和基准测试，但这种资源密集型工作流程对普通计算机视觉研究人员来说难以获得。在众多挑战中，从冗长的执行跟踪中定位有问题的跟踪片段是最耗时的任务，且难以用当前ML模型（包括LLM）自动化。
...
Conclusion: 该研究为ML性能反模式检测提供了首个专门基准数据集，并提出了一种有效的迭代方法，结合了轻量级ML模型和LLM的优势，为计算机视觉研究人员提供了更易访问的性能分析工具。
...
TL;DR: CIS-BA提出了一种新的后门攻击范式，通过连续交互空间触发器取代传统静态对象特征，实现了多触发器-多对象攻击机制，在复杂环境下保持高攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 现有后门攻击方法依赖单一触发器-单一对象映射和脆弱的像素级线索，在能力和鲁棒性上存在固有局限，无法应对现实世界自动驾驶等场景中的复杂交互模式。

Method: 提出CIS-Frame框架：1)通过交互分析构建空间触发器；2)将触发器形式化为类别-几何约束用于样本投毒；3)在检测器训练期间嵌入后门。支持单对象攻击（错误分类和消失）和多对象同时攻击。

Result: 在MS-COCO和真实世界视频上的实验显示：在复杂环境下攻击成功率超过97%；在动态多触发器条件下保持超过95%的有效性；能够规避三种最先进的防御方法。

Conclusion: CIS-BA重新定义了后门攻击范式，将触发器设计从静态对象特征转向连续交互模式，为交互密集型场景中的后门攻击开辟了新方向，并为目标检测系统的安全性提供了新见解。
...
TL;DR: Fast3DHPE是一个模块化框架，用于单目3D人体姿态估计的公平比较和高效开发。在该框架中提出的FastDDHPose方法使用解耦扩散模型，分别建模骨骼长度和方向分布，并通过运动学层次时空去噪器提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有单目3D人体姿态估计方法缺乏统一的训练和评估框架，导致公平比较困难。同时，现有方法通常采用不同的训练协议，缺乏标准化，限制了方法的快速复现和灵活开发。

Method: 1. 提出Fast3DHPE模块化框架，标准化训练和评估协议；2. 在框架内提出FastDDHPose方法，使用解耦扩散模型分别建模骨骼长度和方向分布；3. 设计运动学层次时空去噪器，关注关节运动学层次而非复杂拓扑结构。

Result: 在Human3.6M和MPI-INF-3DHP数据集上的实验表明：1. Fast3DHPE框架实现了所有方法的公平比较并显著提升训练效率；2. FastDDHPose在该统一框架下取得了最先进的性能，在野外场景中表现出强大的泛化能力和鲁棒性。

Conclusion: Fast3DHPE为3D人体姿态估计提供了统一的公平比较框架，而FastDDHPose在该框架下通过解耦扩散建模和运动学层次时空去噪实现了最优性能，为未来方法开发提供了标准化基础。
...
TL;DR: 本文提出使用球面Voronoi（SV）作为3D高斯溅射中外观表示的统一框架，解决了球谐函数在高频信号、吉布斯振铃和镜面反射方面的局限性，为显式3D表示提供了更优的外观建模方案。


<details>
  <summary>Details</summary>
Motivation: 现有辐射场方法（如3D高斯溅射）依赖球谐函数进行外观建模，但SH存在三个主要问题：1）难以处理高频信号；2）产生吉布斯振铃伪影；3）无法有效捕捉镜面反射这一真实渲染的关键要素。虽然球面高斯等替代方案有所改进，但增加了显著的优化复杂度。

Method: 提出球面Voronoi（SV）框架：1）将方向域划分为具有平滑边界的可学习区域，为视角相关效果提供直观稳定的参数化；2）对于漫反射外观，SV保持比现有替代方案更简单的优化；3）对于反射效果，将SV用作可学习反射探针，遵循经典图形学原理以反射方向作为输入。

Result: 在合成和真实世界数据集上取得了最先进的结果：1）对于漫反射外观，SV实现了有竞争力的结果同时保持更简单的优化；2）对于反射效果，SV在SH失败的领域表现出色，提供了更优的镜面反射建模能力。

Conclusion: 球面Voronoi为显式3D表示中的外观建模提供了一个原则性、高效且通用的解决方案，克服了球谐函数的局限性，特别是在镜面反射建模方面表现出色，同时保持了优化的简洁性。
...
TL;DR: 提出一种自动为骨折边界框分配全局AO编码的方法，将全局多标签任务转化为局部多分类任务，F1分数提升7.89%，但实际部署中不完美的骨折检测器会影响性能。


<details>
  <summary>Details</summary>
Motivation: 15-45%的儿童在成长期间会经历骨折，准确诊断至关重要。骨折形态、位置和碎片角度是关键诊断特征，需要自动化方法来提取骨折形态信息。

Method: 提出一种方法，自动为骨折边界框分配全局AO编码，将全局多标签任务重新表述为局部多分类任务，从而能够利用公共数据集。

Result: 该方法将平均F1分数提高了7.89%，但在使用不完美的骨折检测器时性能会下降，突显了实际部署中的挑战。

Conclusion: 该方法在骨折形态提取方面表现良好，但实际临床应用需要更鲁棒的骨折检测器。代码已在GitHub上开源。
...
TL;DR: SkyLume数据集：首个用于研究光照鲁棒3D重建的大规模无人机数据集，包含10个城市区域在一天三个时段采集的10万+高分辨率图像，配有LiDAR扫描和3D真值，并提出了评估反渲染性能的时间一致性系数指标。


<details>
  <summary>Details</summary>
Motivation: 当前基于神经辐射场和3D高斯泼溅的大规模无人机3D重建方法在处理多时相数据时面临严重挑战，因为不同时间的光照差异会导致颜色伪影、几何不准确和外观不一致。由于缺乏系统捕获不同光照条件下同一区域的无人机数据集，这一问题尚未得到充分研究。

Method: 提出SkyLume数据集：1) 收集10个城市区域的10万+高分辨率无人机图像，每个区域在一天三个时段（早晨、中午、傍晚）进行系统采集，包含四个倾斜视角和天底视角；2) 提供每场景的LiDAR扫描和精确3D真值，用于评估深度、表面法线和重建质量；3) 引入时间一致性系数(TCC)指标，用于评估反渲染任务中光照与材质的解耦稳定性。

Result: 创建了首个专门用于研究光照鲁棒3D重建的大规模真实世界无人机数据集，系统隔离了光照变化因素，提供了精确的几何和外观评估基准，并提出了专门评估反渲染性能的量化指标。

Conclusion: SkyLume数据集填补了光照鲁棒3D重建研究的数据空白，为大规模反渲染、几何重建和新视角合成的研究和实际评估提供了基础资源，有望推动该领域的发展。
...
TL;DR: DRAW2ACT：一种深度感知轨迹条件视频生成框架，通过提取轨迹的多重正交表示并联合生成RGB和深度视频，提升机器人操作的可控性和一致性，最终通过多模态策略模型控制机器人关节角度。


<details>
  <summary>Details</summary>
Motivation: 视频扩散模型为具身AI提供了强大的真实世界模拟器，但在机器人操作的可控性方面仍有限制。现有基于轨迹条件的视频生成方法通常依赖2D轨迹或单一模态条件，限制了生成可控且一致的机器人演示的能力。

Method: 1. 从输入轨迹中提取多个正交表示（深度、语义、形状和运动）并注入扩散模型；2. 联合生成空间对齐的RGB和深度视频，利用跨模态注意力机制和深度监督增强时空一致性；3. 构建多模态策略模型，基于生成的RGB和深度序列回归机器人关节角度。

Result: 在Bridge V2、Berkeley Autolab和仿真基准测试中，DRAW2ACT在视觉保真度和一致性方面优于现有基线，同时获得了更高的操作成功率。

Conclusion: DRAW2ACT通过深度感知的轨迹条件视频生成和多模态策略学习，显著提升了机器人操作演示的可控性、一致性和成功率，为具身AI提供了更强大的仿真工具。
...
TL;DR: 本文提出了一种历史增强的两阶段Transformer框架（HETT），用于解决无人机空中视觉语言导航中全局环境推理与局部场景理解难以平衡的问题，通过粗到细的导航流程显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 当前无人机视觉语言导航系统通常采用单一粒度框架，难以同时兼顾全局环境推理和局部场景理解这两个关键方面，导致导航性能受限。

Method: 提出HETT框架：1）首先通过融合空间地标和历史上下文预测粗粒度目标位置；2）然后通过细粒度视觉分析精炼动作；3）设计历史网格图动态聚合视觉特征为结构化空间记忆；4）手动优化CityNav数据集标注。

Result: 在优化后的CityNav数据集上，HETT框架取得了显著的性能提升，广泛的消融实验进一步验证了各组成部分的有效性。

Conclusion: HETT框架通过粗到细的两阶段导航流程有效平衡了全局环境推理和局部场景理解，为无人机空中视觉语言导航提供了更优的解决方案。
...
TL;DR: OminiGen：一种统一的BEV空间框架，通过UAE方法和DiT+ControlNet实现可控的多模态传感器数据生成，解决现有单模态生成方法的低效和对齐问题。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶依赖大量真实世界数据，但获取多样性和极端案例数据成本高、效率低。现有生成方法主要关注单模态，导致多模态传感器数据生成效率低下且存在对齐问题。
...
Conclusion: OminiGen通过统一的BEV框架和创新的UAE方法，成功解决了多模态传感器数据生成中的对齐和可控性问题，为自动驾驶数据合成提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: MGMT启动子甲基化状态对胶质母细胞瘤化疗效果有重要影响，但当前检测方法依赖侵入性脑组织活检。本研究旨在探索放射基因组学技术，通过医学影像非侵入性地识别遗传标记。

Method: 提出新的多视角方法，利用MRI扫描和深度学习模型，考虑MRI视图间的空间关系来检测MGMT甲基化状态。该方法从三个视图提取信息而不使用复杂的3D深度学习模型，避免了高参数量、收敛慢和内存需求大的问题。同时引入了新的肿瘤切片提取技术。

Result: 通过多种评估指标证明了肿瘤切片提取方法的优越性。与最先进模型比较，展示了所提方法的有效性。提供了可复现的已发表模型流程，促进透明度和稳健诊断工具的开发。

Conclusion: 研究强调了非侵入性方法识别MGMT启动子甲基化的潜力，有助于推进胶质母细胞瘤治疗中的精准医学发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有系统将人类行为建模为翻译任务（如语音到手势或文本到动作），缺乏对何时移动、做什么以及如何适应多轮对话的智能决策，导致时间同步差、社交基础弱、各模态孤立处理。

Method: 采用语音-语言-行为模型，基于混合模态专家架构：为语音、面部表情和身体动作分别设置模态分区Transformer专家，通过跨专家注意力共享信息，利用预训练语音-语言模型支持混合主动交互。

Result: 在多轮对话基准测试中，在对话-动作对齐和行为质量方面均优于现有的语音驱动动作生成和文本到动作基线模型。
...
TL;DR: 该论文提出了一种新的图像免疫方法SIFM和评估指标ISR，通过扰动扩散模型中间特征来防止恶意图像编辑，相比传统方法更关注语义对齐破坏而非视觉差异。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的文本引导图像编辑技术容易被滥用，需要保护图像免受未经授权的编辑。传统免疫评估方法存在缺陷，仅关注输出图像与参考输出的视觉差异，而忽略了免疫的核心要求——破坏与攻击者意图的语义对齐。

Method: 提出协同中间特征操纵(SIFM)方法，通过双重协同目标策略性地扰动扩散模型的中间特征：1)最大化特征与原始编辑轨迹的差异以破坏与预期编辑的语义对齐；2)最小化特征范数以诱导感知退化。同时提出免疫成功率(ISR)作为新的评估指标。
...
<details>
  <summary>Details</summary>
Motivation: 汽车雷达在环境感知方面有良好前景，但标注雷达数据有限，制约了雷达感知系统的发展。

Method: 设计考虑雷达点云稀疏性和独特特性的扩散方法，在潜在点云表示上应用扩散，通过对象或场景级条件控制生成，将未标注边界框转换为高质量雷达标注，将LiDAR点云转换为真实雷达场景。

Result: 使用4D-RaDiff合成雷达数据作为数据增强方法，相比仅使用真实数据训练，物体检测性能持续提升；预训练使用合成数据可将所需标注雷达数据减少90%，同时达到相当的检测性能。

Conclusion: 4D-RaDiff框架能有效生成高质量雷达点云数据，解决雷达数据标注稀缺问题，提升物体检测性能并大幅减少对标注数据的依赖。
...
TL;DR: Elastic3D是一种基于条件潜在扩散模型的端到端单目到立体视频转换方法，通过引导VAE解码器实现高质量、视差可控的立体视频生成，避免传统深度估计和扭曲方法的伪影问题。


<details>
  <summary>Details</summary>
Motivation: 随着沉浸式3D内容需求的增长，需要自动化的单目到立体视频转换方法。传统方法依赖显式深度估计和扭曲，容易产生伪影，需要更高质量、可控的解决方案。
...
Conclusion: Elastic3D提供了一种高质量、可控的单目到立体视频转换方法，避免了传统方法的伪影问题，在立体视频生成质量方面表现出色。
...
TL;DR: 提出DANP免疫方法，通过双重注意力引导的噪声扰动来防御文本到图像扩散模型的恶意编辑，在多个时间步上同时干扰交叉注意力图和噪声预测过程。


<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型虽然带来了图像编辑的革命，但也引发了严重的伦理挑战，可能被滥用于创建欺骗性或有害内容。现有的防御方法通过嵌入不可感知的扰动来降低风险，但其对恶意篡改的防御效果有限。

Method: 提出双重注意力引导的噪声扰动（DANP）免疫方法，在多个时间步上操作：1）使用动态阈值生成掩码识别文本相关和不相关区域；2）减少相关区域的注意力同时增加不相关区域的注意力；3）最大化注入噪声与模型预测噪声之间的差异来干扰生成过程。

Result: DANP方法展现出对恶意编辑的显著免疫能力，广泛的实验证实该方法达到了最先进的性能水平。

Conclusion: 通过同时针对注意力机制和噪声预测过程，DANP方法能够有效防御文本到图像扩散模型的恶意编辑，为保护图像内容免受不当篡改提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉编程方法主要关注提升大语言模型生成的视觉程序质量，但忽略了优化视觉编程调用的预训练模型。由于视觉编程的不可微特性，无法直接利用最终标签进行端到端梯度优化，且子任务缺乏标签监督。
...
Conclusion: EVPG通过概率图方法成功解决了视觉编程不可微的问题，实现了端到端的梯度优化，为视觉编程框架的优化提供了新思路，显著提升了复杂视觉推理任务的性能。
...
TL;DR: TDAE是一个双模态框架，通过协调图像-文本优化增强图像对恶意编辑的免疫力，在跨模型评估中实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在跨模型评估中的可迁移性有限，需要一种能够有效对抗恶意图像编辑的通用防御机制。

Method: 提出TDAE双模态框架：1) 视觉防御层面引入FlatGrad防御机制，通过梯度正则化将扰动导向平坦最小值；2) 文本增强保护层面提出动态提示防御，通过迭代对抗更新不同嵌入来生成具有更广泛免疫增强特征的免疫图像。

Result: 广泛的实验结果表明，TDAE在内部和跨模型评估中都达到了最先进的性能，能够有效缓解恶意编辑。

Conclusion: TDAE通过协调图像-文本优化成功增强了图像对恶意编辑的免疫力，实现了跨模型的可迁移性，为扩散基图像编辑系统的安全防护提供了有效解决方案。
...
Method: 提出了DriverGaze360数据集（包含约100万帧带注视标签的360度视野数据）和DriverGaze360-Net模型，该模型通过辅助语义分割头联合学习注意力图和关注对象。

Result: DriverGaze360-Net在全景驾驶图像上实现了最先进的注意力预测性能，在多个指标上表现出色。

Conclusion: 该研究通过全景数据集和模型显著提升了驾驶员注意力预测的空间感知能力，为可解释自动驾驶系统和理解混合交通场景中的驾驶员行为提供了重要工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有的大型视频语言模型在时间感知方面有限，基于GRPO的方法虽然尝试改进时间定位，但仍然难以忠实于视频证据进行定位，导致时间错位和幻觉问题。

Method: 提出了Zoom-Zero框架：1) 先粗粒度定位查询相关片段；2) 再时间放大到最显著的关键帧进行细粒度视觉验证。包含两个关键创新：放大精度奖励和令牌选择性信用分配。

Result: 在NExT-GQA上时间定位提升5.2%，在ReXTime上提升4.6%，平均答案准确率提升2.4%。长视频理解方面平均提升6.4%。

Conclusion: Zoom-Zero框架通过粗到细的时间放大方法，显著提升了视频问答的时间定位能力和答案准确性，特别在长视频理解方面表现优异。
...
TL;DR: DISCODE是一种无需微调的图像字幕评估方法，通过自适应测试时损失和分布感知解码，在跨域场景下实现更鲁棒、更符合人类判断的评估分数。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在多模态任务中表现优异，但在图像字幕评估方面，特别是在域偏移场景下，仍然存在挑战。现有方法在跨域评估时与人类判断的一致性不足。

Method: 提出分布感知分数解码器(DISCODE)，采用测试时自适应评估方法，引入自适应测试时损失，利用高斯先验分布提高评估分数估计的鲁棒性，并通过推导的解析解在测试时高效最小化损失。

Result: DISCODE在新建的多域字幕评估基准MCEval（涵盖六个不同领域）和四个代表性现有基准上，作为无参考评估指标达到了最先进的性能。

Conclusion: DISCODE提供了一种无需微调的有效方法，能够在跨域场景下生成更鲁棒、更符合人类判断的图像字幕评估分数，解决了现有LVLMs在图像字幕评估中的域偏移问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常通过优化3D或视频生成模型来构建4D表示，而不是直接在4D数据上训练。这导致了保真度、时间一致性和结构一致性方面的限制。作者希望开发一个原生的4D生成模型来解决这些问题。

Method: 1) 基于预训练的单图像到3D模型，解决4D训练数据稀缺问题；2) 引入专门的时间层来跨帧推理，确保时间一致性；3) 使用因子化的4D卷积和时间下采样块压缩时间轴上的潜在序列，支持长视频序列的高效训练和推理；4) 采用精心设计的训练策略增强对遮挡的鲁棒性。

Result: SS4D能够直接从单目视频合成动态3D物体，实现了高保真度、时间一致性和结构一致性，同时支持长视频序列的高效处理。

Conclusion: SS4D通过直接在4D数据上训练，结合结构化时空潜在表示和高效压缩技术，成功实现了从单目视频合成动态3D物体的高质量生成，为4D内容生成提供了新的解决方案。
...
TL;DR: 该论文发布了一个新的公开隧道衬砌缺陷数据集，包含三种不同类型隧道的标注图像，用于支持深度学习方法的自动化隧道检测。


<details>
  <summary>Details</summary>
Motivation: 隧道作为重要交通基础设施，面临老化和劣化问题，传统人工检测方法耗时、主观且成本高。虽然移动测绘系统和深度学习可实现自动化检测，但缺乏专门的隧道数据集限制了其效果。

Method: 论文创建了一个包含三种不同隧道衬砌的公开数据集，图像标注了典型缺陷（裂缝、渗漏、水渗透），支持监督、半监督和无监督深度学习方法。

Result: 该数据集填补了领域特定数据的空白，能够支持缺陷检测和分割任务，其纹理和施工技术的多样性还可用于研究模型在不同隧道类型间的泛化和迁移能力。

Conclusion: 通过解决领域数据稀缺问题，该数据集有助于推进自动化隧道检测，促进更安全、更高效的基础设施维护策略。
...
TL;DR: CAPRMIL提出了一种新的多示例学习框架，通过将补丁特征投影到少量全局上下文感知的token中，使用多头自注意力注入全局上下文，配合简单的均值聚合器，在计算病理学中实现了与SOTA相当的性能，同时大幅减少了参数和计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 在计算病理学中，由于WSI的千兆像素尺度和像素级标注稀缺，弱监督已成为深度学习的标准方法。传统MIL方法依赖复杂的基于注意力的聚合机制，计算复杂度高。本文旨在提出一个更高效、聚合器无关的框架，将相关性学习的复杂性从MIL聚合器中移除。

Method: CAPRMIL采用冻结的补丁编码器提取特征，然后将补丁特征投影到少量全局上下文/形态感知的token中，利用多头自注意力注入全局上下文，计算复杂度相对于包大小呈线性。最后配合简单的均值MIL聚合器进行训练。

Result: 在多个公共病理学基准测试中，CAPRMIL达到了与SOTA相当的幻灯片级性能，同时将可训练参数减少了48%-92.8%，推理时的FLOPs降低了52%-99%，在GPU内存效率和训练时间方面表现优异。

Conclusion: 在聚合之前学习丰富的上下文感知实例表示是复杂池化方法的一个有效且可扩展的替代方案，为全幻灯片分析提供了更高效的解决方案。
...
TL;DR: 本文提出了一种通过语义结构恢复来改进SVG动画生成的方法，解决了现有视觉语言模型在处理SVG动画时因图形元素碎片化导致的语义理解困难问题。


<details>
  <summary>Details</summary>
Motivation: SVG在现代网页设计中至关重要，但自动化SVG动画生成对视觉语言模型仍然具有挑战性。现有模型经常错误处理SVG，因为视觉上连贯的部分通常被分割成低层次的形状元素，这些元素无法提供哪些元素应该一起移动的指导。

Method: 引入了一个框架，通过统计聚合多个弱部件预测来恢复SVG动画所需的语义结构。通过重新组织SVG为语义组，使视觉语言模型能够生成更连贯的动画。

Result: 实验表明，该方法相比现有方法取得了显著提升，证明了语义恢复是实现稳健SVG动画生成的关键步骤。

Conclusion: 语义结构恢复是解锁稳健SVG动画生成的关键，支持视觉语言模型与矢量图形之间更可解释的交互。
...
TL;DR: CLNet提出了一种对应感知的特征精炼框架，通过三个可学习模块（NCM、NEC、GFR）显式建模跨视图的空间对应关系，在多个基准测试中实现了最先进的跨视图地理定位性能。


<details>
  <summary>Details</summary>
Motivation: 现有跨视图地理定位方法主要依赖学习鲁棒的全局表示或隐式特征对齐，往往无法建模对精确定位至关重要的显式空间对应关系，存在语义和几何差距问题。
...
Conclusion: CLNet通过显式建模跨视图的空间对应关系，有效桥接了不同视图之间的语义和几何差距，能够联合捕获高层次语义和细粒度对齐，为跨视图地理定位提供了更有效的解决方案。
...
TL;DR: 提出Hybrid Gaussian Splatting (HGS)框架，通过静态-动态分解策略显著减少模型参数，实现实时渲染和高保真动态场景合成


<details>
  <summary>Details</summary>
Motivation: 现有动态新视角合成方法虽然超越NeRF，但存在模型复杂、参数冗余问题，导致模型庞大、渲染速度慢，难以在资源受限设备上实时应用

Method: 提出HGS框架，采用静态-动态分解(SDD)策略，利用径向基函数(RBF)建模高斯基元：动态区域使用时变RBF捕捉时间变化，静态区域共享时间不变参数以减少冗余；采用两阶段训练策略增强静态-动态边界的时间一致性

Result: 模型大小减少高达98%，在RTX 3090上实现4K分辨率125FPS实时渲染，在RTX 3050上实现1352×1014分辨率160FPS，已集成到VR系统；渲染质量与SOTA方法相当，在高频细节和突变场景变化方面提供显著改进的视觉保真度

Conclusion: HGS通过高效的静态-动态分解策略，在保持高质量渲染的同时显著减少模型参数，实现了资源受限设备上的实时动态新视角合成，为沉浸式体验应用提供了实用解决方案
...
Conclusion: FakeRadar通过主动探测特征空间和异常值引导的训练策略，显著提升了深度伪造检测的跨域泛化能力，为解决现实场景中新兴伪造技术的检测挑战提供了有效方案。
...
TL;DR: 该研究提出了首个多中心淋巴瘤基准数据集，系统评估了五种病理学基础模型结合不同聚合方法在淋巴瘤亚型分类中的性能，发现模型在分布内测试集表现良好（>80%），但在分布外测试集性能显著下降（~60%），揭示了泛化挑战。


<details>
  <summary>Details</summary>
Motivation: 淋巴瘤的及时准确诊断对癌症治疗至关重要，但标准诊断流程需要昂贵设备、专业人员且导致治疗延迟。深度学习可以从常规HE染色切片中提取诊断信息，但目前缺乏基于多中心数据的淋巴瘤亚型分类综合基准。
...
Result: 在分布内测试集上，所有放大倍数下模型多类平衡准确率均超过80%，所有基础模型表现相似，两种聚合方法结果相当。放大倍数研究表明40x分辨率足够，更高分辨率或跨放大倍数聚合未带来性能提升。但在分布外测试集上，性能大幅下降至约60%，显示显著泛化挑战。

Conclusion: 为推动该领域发展，需要更大规模的多中心研究覆盖更多罕见淋巴瘤亚型。研究提供了自动化基准测试流程以促进未来研究。
...
<details>
  <summary>Details</summary>
Motivation: 小城市在气候适应方面面临人员资源有限和整合多源数据的挑战，需要一种综合解决方案来支持全面的气候适应分析。

Method: 开发了一个多层系统，结合专业大型语言模型(LLM)、卫星影像分析和知识库，通过集成多种技术手段来辅助气候适应策略的制定。

Result: 提出了EcoScapes系统，相应的代码已在GitHub开源，为小城市提供了一个可用的工具来支持气候适应规划。

Conclusion: 该多层系统能够有效帮助资源有限的小城市整合多源数据，制定全面的气候适应策略，促进城市可持续发展。
...
Method: 结合高斯泼溅先验和伪真值生成策略，优化密度和颜色特征；创建了首个带有显式侧视图监督的合成动态多视图数据集SynDM

Result: 在SynDM和真实世界数据集上的定量和定性评估显示，ExpanDyNeRF在极端视角偏移下的渲染保真度显著优于现有动态NeRF方法

Conclusion: ExpanDyNeRF通过创新的先验利用和数据集构建，有效解决了动态NeRF在极端视角变化下的渲染质量问题
...
TL;DR: LCMem提出了一种跨域记忆检测方法，将记忆检测视为重新识别和复制检测的统一问题，通过两阶段训练策略在六个基准数据集上显著提升了检测性能。
...
Conclusion: LCMem为跨域隐私审计设立了新标准，提供了可靠且可扩展的记忆检测方法。研究结果表明需要更强的隐私保护机制。
...
TL;DR: SynPS：通过协同利用位置嵌入和语义信息解决注意力崩溃问题，实现更忠实的大模型非刚性图像编辑


<details>
  <summary>Details</summary>
Motivation: 现有基于大扩散模型的免训练图像编辑方法在处理复杂非刚性编辑（如姿态或形状变化）时存在忠实度问题，主要原因是现有注意力共享机制中的注意力崩溃现象，导致位置嵌入或语义特征主导视觉内容检索，从而产生过编辑或欠编辑
...
Conclusion: 通过自适应整合位置和语义线索，SynPS有效避免了过编辑和欠编辑问题，实现了更忠实的非刚性图像编辑
...
<details>
  <summary>Details</summary>
Motivation: 传统PnP方法使用的去噪器依赖通用或手工先验，难以准确捕捉自然图像的复杂统计结构，导致在高度欠定情况下的重建效果不佳。得分生成模型能准确表征复杂图像分布，但直接用于后验采样计算复杂度过高。

Method: 利用得分生成建模与经验贝叶斯去噪的紧密联系，设计了一个消息传递框架，集成了得分最小均方误差去噪器用于压缩图像恢复。对于量化测量系统，进一步提出Q-STMP，增加了分量级MMSE去量化模块。

Result: STMP和Q-STMP的渐近性能可以通过状态演化方程准确预测。在FFHQ数据集上的实验表明，STMP相比竞争基线在性能-复杂度权衡上显著更好，Q-STMP即使在1比特量化下也能保持鲁棒性。两种算法通常都在10次迭代内收敛。

Conclusion: 提出的STMP和Q-STMP算法结合了消息传递的快速收敛性和得分生成先验的表达能力，为压缩图像恢复提供了一种高效且性能优越的解决方案，特别是在量化测量系统中表现出色。
...
TL;DR: 提出一种仅使用真实视频数据进行无监督视频实例分割的方法，通过利用深度运动先验识别高质量关键掩码，然后使用稀疏到密集蒸馏方法训练分割模型，在多个基准测试中超越当前最先进方法。


<details>
  <summary>Details</summary>
Motivation: 当前无监督视频实例分割主要依赖从ImageNet等图像数据集生成的合成视频数据，但通过人工移动和缩放图像实例掩码的方法无法准确建模视频中的真实运动，如透视变化、多个实例的部分运动或相机运动。

Method: 1. 从单帧无监督实例分割掩码开始；2. 利用深度运动先验识别视频中的高质量关键掩码，建立时间一致性；3. 使用稀疏关键掩码伪标注训练分割模型，提出稀疏到密集蒸馏方法，辅以时间DropLoss；4. 在生成的密集标签集上训练最终模型。

Result: 该方法在多个基准测试中超越了当前最先进的无监督视频实例分割方法。

Conclusion: 通过仅使用真实视频数据并利用深度运动先验建立时间一致性，结合稀疏到密集蒸馏方法，能够有效解决合成数据无法准确建模真实视频运动的问题，实现更优的无监督视频实例分割性能。
...
TL;DR: A4-Agent是一个免训练的智能体框架，通过解耦可负担性预测为三个阶段，协调专用基础模型，在零样本设置下显著超越有监督方法


<details>
  <summary>Details</summary>
Motivation: 现有端到端模型将高层推理和低层定位耦合在单一流程中，依赖标注数据集训练，导致对新物体和未见环境的泛化能力差

Method: 提出A4-Agent框架，将可负担性预测解耦为三阶段流程：Dreamer使用生成模型可视化交互过程；Thinker利用大视觉语言模型决定交互对象部分；Spotter协调视觉基础模型精确定位交互区域

Result: 该零样本框架在多个基准测试中显著优于最先进的有监督方法，并展现出对真实世界环境的强大泛化能力

Conclusion: 通过利用预训练模型的互补优势而无需任务特定微调，A4-Agent为可负担性预测提供了新的免训练智能体框架范式
...
TL;DR: SignIT是一个新的意大利手语识别数据集，包含644个视频（3.33小时），涵盖94个手势类别，分为5个宏观类别，并提供了2D关键点标注。作者建立了基准测试，评估了多种先进模型，展示了时间信息、2D关键点和RGB帧对性能的影响。


<details>
  <summary>Details</summary>
Motivation: 为意大利手语识别研究提供专门的数据集和基准测试，填补该领域的数据空白，促进手语识别技术的发展。

Method: 创建了包含644个视频的SignIT数据集，手动标注了94个手势类别（分为动物、食物、颜色、情感、家庭5类），提取了手、脸和身体的2D关键点，并采用多种先进模型进行基准测试。

Result: 结果显示现有模型在这个具有挑战性的意大利手语数据集上存在局限性，时间信息、2D关键点和RGB帧对模型性能有不同影响。

Conclusion: SignIT数据集为意大利手语识别研究提供了有价值的资源，揭示了现有模型的局限性，为未来研究指明了改进方向。
...
<details>
  <summary>Details</summary>
Motivation: 当前视网膜基础模型受限于缺乏真实临床背景的研究数据集，且需要大量特定任务优化，限制了在低资源环境下的部署效率。研究者希望通过直接从真实医疗实践中构建临床原生智能来克服这些障碍。

Method: 利用中国162家医疗机构长达十年的远程医疗项目积累的485,980张眼底彩色照片及其对应诊断报告的自然对齐关系，构建ReVision视网膜基础模型。通过大规模远程医疗项目作为临床图像解释学习的自然资源库。

Result: 在27个眼科基准测试中，ReVision在零样本疾病检测方面平均AUROC达到0.946（12个公共基准）和0.952（3个独立临床队列）。仅需极少适应即可匹配经过大量微调的替代方案，且所需可训练参数和标记样本数量级减少。在33名眼科医生的前瞻性读者研究中，ReVision的零样本辅助将诊断准确率提高了14.8%。

Conclusion: 临床原生智能可以直接从临床档案中提取而无需额外标注，从而构建适合各种低资源环境的医疗AI系统。大规模远程医疗项目为学习临床图像解释提供了自然资源库。
...
Conclusion: DASP框架通过利用时空先验，有效解决了夜间深度估计中纹理缺失和动态物体模糊的问题，实现了优异的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于多参考图像的肖像生成方法在身份保真度方面表现不佳，难以精确控制面部属性，需要开发能够同时保持高保真度和精确控制能力的新方法

Method: 1. 引入面部细化器和地标生成器获取细粒度多面部特征和3D感知面部地标；2. 设计HiFi-Net融合多面部特征并与地标对齐；3. 构建自动化流程创建基于身份的数据集用于训练

Result: 实验结果表明，该方法在面部相似度和可控性方面超越了现有最优方法，并且与之前的SDXL-based工作兼容
...
TL;DR: 本文提出了一种任务自适应Transformer（TAT）框架，用于解决医学图像恢复中多任务学习时的任务干扰和任务不平衡问题，在PET合成、CT去噪和MRI超分辨率任务上取得了SOTA性能。
...
Method: 提出任务自适应Transformer（TAT）框架，包含两个关键创新：1）任务自适应权重生成策略，为每个任务生成特定权重参数，避免共享权重上的梯度冲突；2）任务自适应损失平衡策略，根据任务特定学习难度动态调整损失权重，防止任务主导或训练不足。
...
Method: 1. 创建了首个专门针对事件相机的智能交通系统数据集，涵盖车辆和行人检测与跟踪任务；2. 基于该数据集建立了基于检测的跟踪基准；3. 开发了专门的特征提取器。

Result: 所提出的方法在事件相机的智能交通系统数据集上取得了优异的性能表现。

Conclusion: 事件相机在智能交通系统中具有巨大潜力，特别是在弱光和高速度场景下。本文填补了事件视觉在智能交通系统领域的研究空白，为后续研究提供了重要的数据集和基准。
...
TL;DR: WorldPlay是一个流式视频扩散模型，通过双动作表示、重构上下文记忆和上下文强制蒸馏三个创新技术，实现了实时交互式世界建模，解决了现有方法在速度和内存之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 当前流式视频生成方法在速度和内存之间存在权衡限制，难以同时实现实时交互和长期几何一致性。现有方法在处理长序列时面临内存衰减问题，无法保持长期几何一致性。

Method: 1) 双动作表示：支持对用户键盘鼠标输入的鲁棒动作控制；2) 重构上下文记忆：动态重建过去帧的上下文，通过时间重帧保持几何重要但久远帧的可访问性；3) 上下文强制蒸馏：专门为内存感知模型设计的蒸馏方法，通过对齐师生模型的记忆上下文来保持学生模型使用长程信息的能力。

Result: WorldPlay能够以24 FPS的速度生成长序列720p流式视频，具有优越的几何一致性，在多种场景下表现出强大的泛化能力，相比现有技术有明显优势。
...
TL;DR: 本文提出SFVD框架，通过将视频蒸馏为单帧来优化视频数据集蒸馏，解决了传统方法因时间维度参数过多导致的优化困难问题。


<details>
  <summary>Details</summary>
Motivation: 视频数据集蒸馏面临的主要挑战是时间维度引入的大量可学习参数，这导致优化复杂且收敛困难。作者观察到单帧往往足以捕捉视频的判别性语义，因此探索基于单帧的视频蒸馏方法。

Method: 提出SFVD框架：1）将视频蒸馏为每个类别的高信息量单帧；2）通过可微分插值将单帧转换为视频序列并与原始数据集匹配；3）更新仅限于帧本身以提高优化效率；4）通过通道重塑层将蒸馏帧与真实视频采样结合以融入时间信息。

Result: 在多个基准测试上的实验表明，SFVD显著优于现有方法，在MiniUCF上实现了高达5.3%的性能提升。

Conclusion: SFVD通过将视频蒸馏为单帧，有效解决了视频数据集蒸馏中的优化难题，为视频数据集压缩提供了更有效的解决方案。
...
TL;DR: AMD-HookNet++提出了一种混合CNN-Transformer特征增强方法，用于SAR图像中的冰川分割和崩解前缘检测，通过结合Transformer的全局上下文信息和CNN的局部细节，解决了纯CNN方法长程依赖不足和纯Transformer方法边缘锯齿的问题。


<details>
  <summary>Details</summary>
Motivation: 现有AMD-HookNet纯CNN方法虽然能捕捉低层细节，但由于卷积操作的局部性和平移不变性，难以维持长程依赖关系，限制了冰川分割和崩解前缘检测的准确性。

Method: 提出AMD-HookNet++混合架构，包含两个分支：1）基于Transformer的上下文分支捕获长程依赖和全局上下文信息；2）基于CNN的目标分支保留局部细节。设计了增强的空间通道注意力模块，从空间和通道角度动态调整token关系以增强混合特征表示，并开发了像素级对比深度监督方法，将像素级度量学习集成到冰川分割中。

Result: 在CaFFe冰川分割基准数据集上，AMD-HookNet++取得了78.2的IoU和1,318米的HD95，同时保持367米的竞争性MDE，创下了新的最先进水平。更重要的是，混合模型产生了更平滑的崩解前缘轮廓，解决了纯Transformer方法常见的锯齿边缘问题。

Conclusion: AMD-HookNet++通过有效结合CNN和Transformer的优势，在冰川分割和崩解前缘检测任务中取得了显著改进，为冰川监测提供了更准确、更平滑的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在数学任务中通常只从单一静态数学图像进行文本推理，忽视了推理过程中的动态视觉获取。而人类会反复检查视觉图像并采用逐步推理来证明中间命题，这种将解题过程分解为关键逻辑节点的策略符合认知科学的Miller定律。

Method: 提出ViRC框架，引入Reason Chunking机制，将多模态数学思维链结构化为连续的关键推理单元。创建CRUX数据集，使用三种视觉工具和四种推理模式为每个数学问题提供显式标注的CRUs。采用渐进式训练策略：指导性SFT、实践性SFT和策略性RL。

Result: ViRC-7B模型在多个数学基准上相比基线平均提升18.8%。

Conclusion: 通过模拟人类专家的问题解决模式，将多模态数学推理分解为关键推理单元，显著提升了MLLMs在数学任务中的推理能力。
...
TL;DR: FiNERweb是一个多语言命名实体识别数据集创建管道，通过教师-学生范式在91种语言和25种文字上生成约225k段落和235k实体标签，数据量比基线少19倍但性能相当或更好。


<details>
  <summary>Details</summary>
Motivation: 现有多语言NER工作虽然显示大语言模型能提供有效的合成监督，但这些数据集大多是更广泛实验的副产品，缺乏系统化、可复用的资源。需要创建专门的多语言NER数据集来促进更有效的教师-学生训练。

Method: 基于FineWeb-Edu，训练回归模型识别NER相关段落，然后用多语言LLM进行标注。创建包含91种语言和25种文字的数据集，同时提供英文标签和目标语言翻译标签集。

Result: 回归模型达到84+ F1分数；在英语、泰语和斯瓦希里语的零样本迁移设置中，使用FiNERweb训练的模型性能与基线相当或更好，尽管数据量少19倍。LLM-as-a-judge评估显示标注质量高（忠实度3.99/5，完整性4.05/5）。

Conclusion: FiNERweb提供了一个高质量、可复用的多语言NER数据集，能够有效支持教师-学生训练范式。当前最先进模型在使用目标语言标签而非英文标签时性能下降0.02-0.09 F1，因此同时发布两种标签集。向研究社区发布完整数据集和相关工具。
...
TL;DR: Olmo 3是一个完全开源的7B和32B参数规模的语言模型家族，专注于长上下文推理、函数调用、编码、指令跟随、通用对话和知识回忆，提供完整的模型构建流程和所有中间检查点。


<details>
  <summary>Details</summary>
Motivation: 构建一个完全开源、透明的语言模型，提供从数据到最终模型的完整生命周期，解决现有开源模型在透明度、可复现性和功能覆盖方面的不足。

Method: 开发了7B和32B参数规模的模型家族，针对长上下文推理、函数调用、编码、指令跟随等任务进行优化，并完整记录和发布模型构建的每个阶段、检查点、数据点和依赖项。

Result: 推出了旗舰模型Olmo 3 Think 32B，这是迄今为止发布的最强大的完全开源思维模型，在多个任务领域表现出色。

Conclusion: Olmo 3系列模型通过完全开放的方式推进了语言模型研究的透明度和可复现性，为社区提供了强大的开源工具和完整的模型构建流程。
...
TL;DR: 本文提出了一种基于大语言模型的结构感知解码方法，用于解决嵌套和重叠实体提取任务中语义完整性和结构一致性的平衡问题。
...
Method: 1. 使用预训练语言模型获取上下文感知的语义表示；2. 通过候选表示组合捕获多粒度实体跨度特征；3. 在解码过程中引入层次结构约束；4. 联合优化分类损失和结构一致性损失。
...
Result: 1. 有效层数随模型规模增长，但有效深度比例保持稳定；2. 基础模型与长思维链模型的有效深度无差异，推理能力提升源于更长上下文而非更深计算；3. 模型不会为更难问题动态使用更多层。
...
TL;DR: 提出AR-to-dLM转换框架，通过块状注意力模式和位置相关掩码策略，将预训练自回归模型高效转换为扩散语言模型，在保持准确率的同时大幅提升生成速度。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型支持并行非自回归生成，但从头训练效率低于自回归模型。研究如何将预训练的自回归模型转换为高效的扩散语言模型，在保持任务准确率的同时提升生成速度。

Method: 1) 提出块状注意力模式：保持块间因果性，块内双向建模，更好地保留预训练权重分布并支持KV缓存；2) 位置相关掩码策略：训练时给靠后token更高掩码概率，缩小训练-测试差距；3) 构建Efficient-DLM模型家族。

Result: Efficient-DLM 8B模型在准确率和吞吐量上均优于现有模型：相比Dream 7B准确率提升5.4%，吞吐量提升4.5倍；相比Qwen3 4B准确率提升2.7%，吞吐量提升2.7倍。

Conclusion: 提出的AR-to-dLM转换框架通过创新的注意力模式和掩码策略，成功实现了准确率与效率的双赢，为大规模语言模型的高效部署提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型的长上下文理解和推理能力日益重要，但自注意力机制的二次方计算复杂度成为瓶颈。现有稀疏注意力方法存在训练成本高、无法作为加速插件通用应用，或在推理时牺牲效率或跨模态通用性的问题。

Method: 引入复合令牌概念，通过多粒度压缩聚合上下文信息，并基于此进行动态稀疏注意力构建，采用块级选择和硬件友好的GPU执行策略。

Result: 在多种模态和任务中，UniSparse在准确性和效率上均超越现有稀疏注意力方法，达到≥99%的全注意力精度，注意力计算比FlashAttention快达2.61倍。

Conclusion: UniSparse提供了一种高效、通用的稀疏注意力解决方案，能够有效解决长上下文处理的计算瓶颈，同时保持高精度和跨模态适用性。
...
TL;DR: 本文提出了一种基于Transformer的多语言连续反馈预测模型，涵盖日语、英语和中文，用于研究跨语言的时间行为差异。模型在约300小时的双人对话数据上进行训练，在多语言设置下表现优于单语言基线，揭示了不同语言在反馈时机上的系统性差异。


<details>
  <summary>Details</summary>
Motivation: 研究动机是理解不同语言中反馈时机（backchannel timing）的差异，这对于设计更自然、具有文化意识的语音对话系统至关重要。当前缺乏能够同时处理多种语言并揭示跨语言差异的统一模型。

Method: 方法包括：1）构建基于Transformer的帧级多语言连续反馈预测模型；2）在约300小时的日语、英语和中文双人对话数据上进行联合训练；3）使用辅助任务增强学习；4）进行零样本迁移、扰动分析和上下文长度研究；5）将训练好的模型集成到实时处理软件中。

Result: 结果显示：1）多语言模型在所有三种语言上都达到或超越了单语言基线；2）零样本迁移效果有限，表明存在实质性的跨语言差异；3）扰动分析揭示了不同的线索使用模式：日语更依赖短期语言信息，而英语和中文对沉默时长和韵律变化更敏感；4）多语言训练鼓励共享但可适应的表示，并减少了中文对音高的过度依赖；5）上下文长度研究表明日语对较短上下文相对稳健，而中文明显受益于较长上下文。

Conclusion: 结论是：该研究提供了一个统一模型和实证证据，展示了不同语言中反馈时机的系统性差异。这些发现为设计更自然、具有文化意识的语音对话系统提供了重要参考，并证明了多语言训练能够学习语言通用线索和语言特定时间模式。
...
TL;DR: CogMem是一个受认知启发的记忆增强LLM架构，通过结构化持久记忆支持持续迭代推理，解决了多轮对话中LLM的准确性和连贯性下降问题。
...
Conclusion: CogMem通过结构化记忆架构解决了多轮推理中的关键问题，为实现更可靠、类人推理的LLM迈出了重要一步。
...
TL;DR: 本文提出并比较了两种用于标准孟加拉语到方言翻译的RAG管道，发现基于标准化句子对的管道性能优于基于转录本的管道，使小模型能超越大模型。


<details>
  <summary>Details</summary>
Motivation: 标准语言到区域方言的翻译是NLP中的重要挑战，特别是在孟加拉语中，由于数据稀缺和语言变异性。需要为低资源方言翻译提供有效的解决方案。

Method: 提出了两种RAG管道：1) 基于转录本的管道，使用音频转录中的大段方言句子上下文；2) 标准化句子对管道，使用结构化的local_dialect:standard_bengali句子对。在六种孟加拉方言和多种LLM上进行了评估。

Result: 句子对管道始终优于转录本管道，将吉大港方言的词错误率从76%降至55%。RAG方法使小模型（如Llama-3.1-8B）能超越大模型（如GPT-OSS-120B），表明精心设计的检索策略比模型规模更重要。

Conclusion: 这项工作为低资源方言翻译提供了一种有效、无需微调的解决方案，展示了精心设计的检索策略可以超越模型规模的重要性，为保护语言多样性提供了实用蓝图。
...
TL;DR: 本文介绍了两种用于纳瓦特尔语料库扩展的上下文无关文法，以解决该语言数字资源匮乏的问题，通过生成大量语法正确的句子来扩展语料库，用于学习非上下文嵌入表示。


<details>
  <summary>Details</summary>
Motivation: 纳瓦特尔语是墨西哥的国家语言之一，属于π语言类型（数字资源匮乏的语言）。由于缺乏数字资源，可用于大型语言模型学习的语料库几乎不存在，这构成了重大挑战。

Method: 提出了两种新的纳瓦特尔语上下文无关文法，并在生成模式下使用这些文法来产生大量语法正确的纳瓦特尔语句子，从而扩展语料库。

Result: 使用扩展后的语料库在句子语义相似性任务中取得了比仅使用原始语料库更好的结果，并且发现经济型嵌入表示在某些情况下比一些大型语言模型表现更好。

Conclusion: 通过上下文无关文法扩展语料库是解决资源匮乏语言数字资源问题的有效方法，能够显著改善嵌入表示的学习效果，为纳瓦特尔语等π语言的自然语言处理研究提供了可行方案。
...
TL;DR: 该研究探讨了GPT-3.5-turbo基于宏观经济价格信号形成通胀感知和预期的能力，通过与英国家庭调查数据对比，发现GPT在短期预测上与调查结果和官方统计数据一致，但在通胀理解上缺乏一致性模型。


<details>
  <summary>Details</summary>
Motivation: 研究动机是评估大型语言模型（LLMs）在社会科学研究中的适用性，特别是能否像人类一样基于经济信息形成通胀感知和预期，为LLMs在社会科学应用提供方法论基础。

Method: 采用准实验设计，利用GPT-3.5-turbo的训练截止时间（2021年9月）作为自然实验，模拟英国央行通胀态度调查的信息集和人口特征，将GPT输出与家庭调查数据和官方统计进行比较，并使用Shapley值分解法分析提示内容对模型输出的影响。

Result: 研究发现：1）GPT在短期预测上能跟踪汇总调查预测和官方统计数据；2）在细分层面能复制家庭通胀感知的关键经验规律（收入、住房所有权、社会阶层）；3）对食品通胀信息表现出与人类受访者相似的敏感性；4）但缺乏一致的消费者价格通胀模型。

Conclusion: 该方法可用于评估LLMs在社会科学中的行为表现、比较不同模型或辅助调查设计，但GPT虽然能模拟人类通胀感知的某些方面，却缺乏对通胀的一致理解模型。
...
TL;DR: 研究不同文档打包策略对大型语言模型多跳推理能力的影响，发现打包训练能提升性能但增加计算成本


<details>
  <summary>Details</summary>
Motivation: 当前训练大型语言模型时通常将多个文档打包以提高计算效率，但这一过程对模型能力的影响尚未充分研究，特别是对多跳推理能力的影响

Method: 研究不同文档打包策略，通过对比实验分析打包训练与单文档训练的效果差异，并进行消融研究以识别关键影响因素

Result: 打包训练相比单文档训练能提升模型性能，但需要更多计算资源；消融研究识别出了解释打包优势的关键因素

Conclusion: 研究深化了对LLM训练动态的理解，为优化模型开发提供了实用见解，表明文档打包策略是影响模型能力的重要因素
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型部署面临GPU内存限制的挑战，现有量化方案存在权衡：动态量化计算开销大且边缘设备部署困难，静态量化牺牲准确性，而量化感知训练又存在权重训练成本高的问题。

Method: SASQ专门针对激活量化因子进行轻量级优化，仅优化量化因子而不改变预训练权重，自适应截断部分异常值，在降低量化难度的同时保持激活的分布特性。

Result: SASQ不仅超越了现有最先进的量化方案，甚至优于对应的FP16模型。在LLaMA2-7B上，在WikiText2上的困惑度比QuaRot低5.2%，比FP16模型低4.7%。

Conclusion: SASQ通过轻量级的量化因子优化方法，解决了现有量化方案的权衡问题，实现了高精度、高效率的模型部署，为大型语言模型的边缘设备部署提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型通常在高级编程语言任务上表现出色，但在低级编程语言（如汇编）方面表现不佳。需要提升LLM对汇编代码的理解能力。

Method: 提出C-ing Clearly合成数据生成方法，利用对应的C代码来增强LLM对汇编的理解。通过在该方法生成的数据上进行微调，提升模型性能。

Result: 方法在二进制代码总结和漏洞检测任务上展示了改进的LLM性能。在不同LLM家族和模型规模上都表现出一致的性能提升。
...
TL;DR: 论文批评Futrell和Mahowald框架过于关注基于文本的生成式LLM，认为音频深度学习模型对语言学研究至关重要


<details>
  <summary>Details</summary>
Motivation: 现有框架将深度学习系统与语言学理论连接，但过度依赖文本生成模型，忽视了人类语言中许多重要方面无法通过书面文本捕捉

Method: 提出论证：基于音频的深度学习模型应该发挥关键作用，因为许多有趣的语言问题超出了书面文本的范畴

Result: 指出当前框架的根本局限性，强调音频模型在语言研究中的必要性

Conclusion: 需要将音频深度学习模型纳入语言学研究框架，以更全面地理解人类语言
...
Method: 采用双目标训练方法，结合自回归和掩码扩散目标，通过训练50个不同数据重复程度下的语言模型来确定最优目标比例

Result: 在所有评估设置下，结合两种目标都是最优选择，且无论针对自回归还是掩码扩散下游任务，最优比例都相似
...
TL;DR: 越南法律基准测试VLegal-Bench：首个针对越南法律任务的全面基准，包含10,450个样本，基于布鲁姆认知分类法设计，用于评估大语言模型在越南法律领域的理解和应用能力。


<details>
  <summary>Details</summary>
Motivation: 越南法律的复杂性、层级结构和频繁修订给评估大语言模型在法律领域的理解和应用能力带来了挑战，需要专门的基准测试来系统评估这些模型在越南法律任务上的表现。

Method: 基于布鲁姆认知分类法设计多层次法律理解任务，通过严格的标注流程生成10,450个样本，由法律专家使用专门的标注系统进行标注和交叉验证，确保每个样本都基于权威法律文件，并反映真实的法律助理工作流程。

Result: 创建了VLegal-Bench基准测试，包含通用法律问答、检索增强生成、多步推理和基于越南法律的场景问题解决等多种任务类型，为评估大语言模型在越南法律环境中的性能提供了标准化、透明且认知科学的评估框架。

Conclusion: VLegal-Bench为评估大语言模型在越南法律环境中的表现奠定了坚实基础，支持开发更可靠、可解释且符合伦理的人工智能辅助法律系统，填补了该领域基准测试的空白。
...
TL;DR: PolyPersona是一个生成框架，使用参数高效的LoRA适配器和4位量化，通过指令调优紧凑聊天模型，生成跨多个领域的人物条件化调查响应。


<details>
  <summary>Details</summary>
Motivation: 需要一种高效且可重复的方法来生成合成调查数据，支持可扩展的评估，同时通过透明和开放的协议促进偏见分析。

Method: 使用基于对话的数据管道明确保留人物线索，确保生成响应中行为一致性；采用参数高效的LoRA适配器配合4位量化，在资源自适应训练设置下指令调优紧凑聊天模型。

Result: 构建了包含3,568个合成调查响应的数据集，涵盖10个领域和433个不同人物；紧凑模型如TinyLlama 1.1B和Phi-2在BLEU(0.090)和ROUGE-1(0.429)等指标上达到与较大7B-8B基线相当的性能。

Conclusion: 人物条件化微调使小型语言模型能够生成可靠且连贯的合成调查数据，该框架为调查数据生成提供了高效且可重复的方法。
...
TL;DR: JMMMU-Pro是一个基于图像的日语多学科多模态理解基准测试，通过将问题图像和文本组合成单一图像，创建需要视觉-文本整合理解的基准。采用Vibe Benchmark Construction方法，利用图像生成模型生成候选视觉问题，人工验证确保质量。


<details>
  <summary>Details</summary>
Motivation: 从MMMU到MMMU-Pro的演进中，需要创建日语多模态理解基准，评估语言模型在日语环境中的视觉-文本整合理解能力，为开源社区提供更严格的评估工具。

Method: 提出Vibe Benchmark Construction方法：使用图像生成模型（如Nano Banana Pro）生成候选视觉问题，人工验证输出质量，必要时通过调整提示重新生成。利用模型的高质量图像生成和日语文本嵌入能力，低成本构建高质量基准。

Result: 实验结果显示，所有开源语言模型在JMMMU-Pro上都表现不佳，证明该基准具有挑战性。基准覆盖了广泛的背景和布局设计，质量高且构建成本低。

Conclusion: JMMMU-Pro为评估语言模型的日语能力提供了更严格的工具，Vibe Benchmark Construction方法为未来图像问答基准的开发提供了高效指南，有助于指导开源社区的未来研究方向。
...
TL;DR: 该论文提出了TiME（Tiny Monolingual Encoders）模型，通过知识蒸馏等技术训练小型专用模型，在保持性能的同时显著提升效率、降低能耗，适用于实时处理和大规模数据场景。


<details>
  <summary>Details</summary>
Motivation: 当前NLP研究过度关注大型通用模型，但许多实际应用只需要特定功能的小型模型。大型模型在处理大规模数据时速度慢、能耗高，不适合实时响应或电池供电设备，存在效率和可持续性问题。

Method: 使用知识蒸馏技术，从多语言教师模型中蒸馏出单语学生模型，并支持从使用相对位置编码的教师模型蒸馏到使用绝对位置编码的学生模型。采用现代训练技术，专注于训练小型专用编码器。

Result: TiME模型在多个常见NLP任务上进行了全面评估，在基准性能与吞吐量、延迟和能耗之间取得了更好的平衡。证明了从多语言教师蒸馏单语模型以及从相对位置编码教师蒸馏绝对位置编码模型的可行性。

Conclusion: 对于效率关键的应用，训练小型专用模型是可行的解决方案。TiME模型通过知识蒸馏等技术，在保持性能的同时显著提升了效率，支持低资源语言，为实际部署提供了更可持续的选择。
...
TL;DR: Jacobi Forcing是一种渐进式蒸馏范式，通过让模型在自身生成的并行解码轨迹上进行训练，将自回归模型平滑转换为高效并行解码器，同时保持预训练因果推理特性，实现3.8-4.0倍推理加速。


<details>
  <summary>Details</summary>
Motivation: 当前基于扩散大语言模型(dLLMs)的并行解码方法存在预训练与后训练不匹配问题：后训练中的掩码数据分布与预训练真实数据分布偏差大，且双向注意力与预训练学习的因果先验冲突，阻碍精确KV缓存重用，导致加速效果有限。

Method: 提出Jacobi Forcing渐进式蒸馏范式，让模型在自身生成的并行解码轨迹上进行训练，平滑地将自回归模型转换为高效并行解码器，同时保持预训练因果推理特性。基于Jacobi Forcing模型的轨迹特性，引入带拒绝回收的多块解码，提高每次迭代的令牌接受数量。

Result: Jacobi Forcing模型在代码和数学基准测试上实现3.8倍实际时钟加速，性能损失最小。通过多块解码与拒绝回收，每次迭代令牌接受数量提高4.5倍，实现近4.0倍实际时钟加速，有效用额外计算换取更低推理延迟。

Conclusion: Jacobi Forcing通过渐进式蒸馏解决了dLLMs中预训练与后训练不匹配问题，成功将自回归模型转换为高效并行解码器，在保持生成质量的同时显著加速推理，为大规模模型推理加速提供了有效解决方案。
...
TL;DR: Spoken DialogSum是首个将原始对话音频与事实摘要、情感摘要以及说话者年龄、性别、情感等副语言线索对齐的数据集，包含13,460个情感多样的对话，通过两阶段构建方法生成，实验表明端到端音频语言模型比级联ASR-LLM系统在情感摘要上提升28% ROUGE-L分数。
...
Method: 采用两阶段构建方法：1) 使用LLM重写DialogSum脚本，添加Switchboard风格的填充词和反馈词，并为每个话语标注情感、音高和语速；2) 使用富有表现力的TTS引擎从标注脚本合成语音，并与副语言标签对齐。

Result: 创建了包含13,460个情感多样对话的Spoken DialogSum数据集，每个对话都配有事实摘要和情感聚焦摘要。基线实验显示，端到端音频语言模型相比级联ASR-LLM系统，在情感摘要的ROUGE-L分数上相对提升28%。

Conclusion: Spoken DialogSum填补了语音、摘要和副语言线索对齐数据集的空白，证明了端到端语音建模在情感感知对话摘要任务中的价值，为情感感知口语对话摘要研究提供了重要资源。
...
TL;DR: MMGR是一个多模态生成推理评估框架，用于评估视频和图像生成模型在物理、逻辑、空间和时间等推理能力上的表现，揭示了当前模型在抽象推理和空间规划方面的显著不足。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型虽然能产生视觉逼真和时间连贯的内容，但缺乏对物理、逻辑和空间约束的准确建模。现有评估指标如FVD过于关注感知质量，忽视了因果、物理和全局一致性等推理失败问题。

Method: 提出MMGR评估框架，基于五种推理能力（物理、逻辑、3D空间、2D空间、时间），在三个领域进行评估：抽象推理（ARC-AGI、数独）、具身导航（真实世界3D导航和定位）、物理常识（运动和组合交互）。采用细粒度指标要求视频和图像生成的全方位正确性。

Result: 评估了领先的视频模型（Veo-3、Sora-2、Wan-2.2）和图像模型（Nano-banana系列、GPT-4o-image、Qwen-image），发现模型在物理常识任务上表现中等，但在抽象推理上表现很差（ARC-AGI准确率低于10%），在具身环境中的长时程空间规划方面也存在困难。
...
TL;DR: 开发了一个包含5.4万条多领域孟加拉语文章和摘要的数据集，用于抽象摘要任务，填补了现有研究主要集中于新闻领域的空白。


<details>
  <summary>Details</summary>
Motivation: 现有孟加拉语摘要研究主要集中于新闻文章，但现实世界中存在大量来自博客、报纸、社交媒体等多领域的孟加拉语内容，需要能够适应不同写作风格的摘要系统来应对信息过载问题。

Method: 从Cinegolpo、Samakal、The Business Standard等多个来源收集了超过54,000篇孟加拉语文章和摘要，构建了跨多个领域和写作风格的数据集，并使用LSTM、BanglaT5-small、MTS-small等深度学习模型进行训练和评估。

Result: 该数据集展现了作为孟加拉语自然语言处理未来研究基准的潜力，为构建鲁棒的摘要系统提供了坚实基础，并有助于扩展低资源语言的NLP资源。

Conclusion: 这个多领域孟加拉语摘要数据集填补了现有研究的空白，为开发适应现实世界多样文本的摘要系统提供了重要资源，将推动低资源语言NLP技术的发展。
...
Result: 框架在多个医学特征提取上达到高准确率，相比专家标注数据集表现优异，甚至发现了人工审查中遗漏的标注错误

Conclusion: 该框架展示了LLM系统通过自动化提取减少人工病历审查负担的潜力，能提高数据捕获的一致性，加速临床研究
...
TL;DR: 本文提出了一种无需位置标签的盲无线地图构建框架，利用室内MIMO-OFDM信道测量推断用户轨迹，通过理论证明和空间正则化贝叶斯推理实现定位和波束图重建。


<details>
  <summary>Details</summary>
Motivation: 传统无线地图构建方法需要大量位置标记数据，成本高昂且在实际场景中不实用。本文旨在开发一种无需位置标签的盲构建方法，利用信道测量推断用户轨迹。

Method: 首先证明在准镜面环境模型下，非视距信道状态信息具有空间连续性，推导出与物理距离成比例的CSI-距离度量。对于泊松分布AP部署中的直线轨迹，证明即使角度分辨率较差，定位误差的克拉美-罗下界也能渐近消失。基于这些理论结果，开发了空间正则化贝叶斯推理框架，联合估计信道特征、区分视距/非视距条件并恢复用户轨迹。

Result: 在射线追踪数据集上的实验表明，平均定位误差为0.68米，波束图重建误差为3.3%，验证了所提盲映射方法的有效性。

Conclusion: 本文提出的盲无线地图构建框架能够在无需位置标签的情况下，仅通过信道测量有效推断用户轨迹并重建无线地图，为智能无线应用提供了实用且成本效益高的解决方案。
...
Result: 在AlleNoise基准的1000项平衡子集上验证：知识图谱增强模型达到0.99 F1分数，显著优于单LLM基线(0.48 F1)和非知识图谱委员会(0.59 F1)。系统通过知识图谱完美识别复杂结构性错误，实现完全召回率。
...
TL;DR: 本文提出了一种基于AI的标注管道，用于系统识别、标记和修复LLM输出的不稳定模式，通过人机协同方法结合自动弱监督和基于置信度的标注，旨在解决LLM在高度监管行业中因不稳定、不一致推理和幻觉等问题导致的应用限制。


<details>
  <summary>Details</summary>
Motivation: LLM在高度监管行业中的应用受到限制，主要因为存在不稳定问题、不一致推理、幻觉和性能波动等可靠性问题。现有的稳定化方法如RLHF和监督微调虽然能提供可量化的改进，但成本高昂且依赖大量人工标注，难以可持续地扩展。

Method: 提出基于AI的标注管道，采用人机协同方法，结合自动弱监督模型和基于置信度的标注，辅以人工验证。引入语义一致性、事实正确性和逻辑连贯性等稳定性特定标注类别，通过反馈循环实现模型的持续校准和鲁棒性增强。

Result: 该方法能够系统性地识别和修复LLM输出的不稳定模式，确保反馈信息的可靠性和道德完整性，为LLM在需要事实精确性和一致行为的领域提供更可靠的解决方案。

Conclusion: 通过AI驱动的标注管道和人机协同方法，可以更高效、可持续地解决LLM的稳定性问题，为LLM在高度监管行业中的安全应用提供了可行的技术路径。
...
<details>
  <summary>Details</summary>
Motivation: 随着大模型部署向边缘和资源受限环境转移，量化模型成为标准，但精度降低对形式化因果推理的影响尚不清楚，需要系统评估量化在Pearl因果阶梯三个层级上的影响

Method: 使用3000个样本分层的CLadder基准评估量化效果，在Llama 3 8B模型上测试INT8和NF4量化，评估关联、干预和反事实三个层级的推理能力，并在CRASS基准和基于真实因果图的图检索增强生成上进行实验

Result: 量化对因果推理影响总体稳定，NF4量化总体退化小于1%；干预查询对精度损失最敏感；反事实推理相对稳定但在特定查询类型上存在异质性弱点；CRASS基准上不同精度表现几乎相同；图检索增强生成使NF4干预准确率提升1.7%
...
TL;DR: 论文提出了一种定性案例研究方法，用于审计大语言模型在长时程交互中的策略相关行为选择性，发现同一模型在非敏感领域表现正常，但在敏感领域会功能性拒绝，并引入了"习得性无能"概念来描述这种行为模式。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型被广泛部署为通用工具，但标准定量基准测试无法捕捉到长期交互中可能出现的特定行为模式。研究者希望通过定性案例研究方法来审计模型在策略敏感领域的行为选择性，揭示潜在的对齐副作用。

Method: 采用定性案例研究方法，通过86轮对话会话进行长时程交互审计。定义了三种响应机制：正常表现(NP)、功能性拒绝(FR)和元叙事(MN)，并观察这些机制在不同领域（非敏感与敏感领域）中的分布模式。

Result: 研究发现同一模型在广泛的非敏感领域表现出正常性能，但在提供商或策略敏感领域反复产生功能性拒绝，形成了跨领域的NP和FR之间的持续不对称性。元叙事角色框架叙述往往与敏感情境中的拒绝同时出现。

Conclusion: 研究提出了基于可观察行为的交互级审计框架，并将"习得性无能"作为检查潜在对齐副效应的视角。这种选择性保留行为值得在不同用户和模型中进行进一步调查。
...
TL;DR: 论文研究数学和编程在AI智能体心理测量电池模空间中的特殊作用，发现数学纤维与形式证明内核结合时，GVU流允许谱稳定的自我改进机制。主要技术结果是密度定理：在适当条件下，由数学定理证明和编程任务生成的电池子空间在评估度量下是稠密的。
...
Method: 基于AAI框架和GVU动力学，定义数学纤维，将其与形式证明内核（如Lean、Coq）配对，分析GVU流在数学纤维上的谱稳定性。在智能体输出均匀紧致和AAI泛函Lipschitz条件下，证明密度定理。

Result: 主要技术结果是密度定理：在评估度量下，由数学定理证明和编程任务生成的电池子空间在电池模空间中是稠密的。编程单独具有这种普遍性，而纯数学没有；数学的特权是谱性的而非表达性的。

Conclusion: 数学和编程为AI智能体评估提供了"通用坐标"，形式数学是高级AI智能体递归自我改进的自然点火域，数学纤维与形式证明内核的结合允许谱稳定的自我改进机制。
...
TL;DR: 提出语义基础指数(SGI)来检测RAG系统的幻觉，发现幻觉回答在嵌入空间中更靠近问题而非上下文，这种"语义懒惰"现象可通过角度距离比率有效识别。


<details>
  <summary>Details</summary>
Motivation: 研究RAG系统产生幻觉时在嵌入空间中的几何特征，开发一种计算高效且理论基础的指标来识别需要验证的响应。

Method: 定义语义基础指数(SGI)为响应到问题与响应到上下文的角度距离比率，基于球形三角不等式理论分析其判别能力，在HaluEval数据集上验证。

Result: SGI在HaluEval上表现出大效应量(Cohen's d=0.92-1.28)，判别能力随问题-上下文角度分离增加而增强(AUC从0.72提升到0.83)，对长响应和短问题效果最佳。

Conclusion: SGI提供了一种计算高效、理论基础的方法来识别RAG系统中的幻觉响应，但主要测量主题参与度而非事实准确性，适用于生产部署中的验证需求。
...
TL;DR: MURIM是一个多维度基于信誉的激励机制，通过综合考虑客户端可靠性、隐私、资源容量和公平性来解决联邦学习中的激励、隐私和资源约束问题，有效防止恶意客户端获得不当奖励。
...
Method: 提出MURIM多维度信誉激励机制，基于客户端贡献、延迟和信誉分配激励，配备可靠性验证模块，综合考虑客户端可靠性、隐私、资源容量和公平性。
...
TL;DR: 该论文对四个前沿大语言模型（GPT-5-Thinking、Gemini-3-Pro、Claude-Sonnet-4.5-Thinking、Grok-4）在《随机算法》教材中的定理证明能力进行了基准测试，发现顶级模型准确率约66%，但模型间存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在数学推理和科学发现方面取得了显著进展，但缺乏对这些模型在经典研究生级别数学理论上的严谨评估。需要了解这些模型在正式数学证明方面的基线推理能力。

Method: 使用Motwani和Raghavan的《随机算法》教材作为测试基准，要求每个模型为教材中的一系列引理和练习生成正式的LaTeX证明。对四个前沿模型（GPT-5-Thinking、Gemini-3-Pro、Claude-Sonnet-4.5-Thinking、Grok-4）进行系统评估。

Result: 顶级模型（Gemini和Claude）达到约66%的准确率，表现出对概率方法和形式逻辑的扎实掌握；其他模型的一致性显著较低（约40%）。定性分析揭示了生成证明在简洁性、幻觉率和逻辑结构方面的差异。

Conclusion: 前沿模型已达到适合研究生级别教学辅助和形式化的熟练程度阈值，但在严谨数学推导的可靠性方面存在显著差异。代码和所有LLM生成的响应已开源。
...
TL;DR: ReflCtrl框架通过表示工程控制LLMs的自我反思频率，减少推理成本同时保持性能，发现反思与模型内部不确定性高度相关。


<details>
  <summary>Details</summary>
Motivation: 虽然具有自我反思能力的LLMs能提升推理性能，但增加了推理成本。本研究旨在通过表示工程方法控制反思频率，减少冗余反思，优化推理效率。
...
Conclusion: 自我反思可以通过表示工程进行控制，反思频率与模型内部不确定性相关，ReflCtrl框架能有效减少推理成本同时保持性能，为优化LLMs推理效率提供了新方法。
...
TL;DR: DTop-p MoE：一种可控稀疏度的动态Top-p路由机制，通过PI控制器动态调整概率阈值，实现激活专家数量的精确控制，同时自适应分配计算资源。
...
Result: 在大语言模型和扩散Transformer上的实验表明，DTop-p持续优于Top-k和固定阈值Top-p基线，能精确控制激活专家数量，自适应分配资源，并展现出良好的扩展性。

Conclusion: DTop-p为大规模MoE预训练提供了一个鲁棒框架，解决了现有路由机制在稀疏度控制和资源分配方面的局限性。
...
TL;DR: RoutingGen是一个难度感知的路由框架，根据任务复杂度动态选择提示策略：简单任务用few-shot提示，复杂任务用创新的Intention Chain-of-Thought（ICoT）进行结构化推理，专注于算法逻辑和效率意图建模。


<details>
  <summary>Details</summary>
Motivation: 现有CoT提示方法存在两个主要局限：1）统一应用导致简单任务上过度思考；2）缺乏代码生成中的意图抽象（如算法设计和效率建模），使模型关注表面结构而忽视全局问题目标。受认知经济原则启发，只在必要时进行结构化推理以节省认知资源。

Method: 提出RoutingGen框架，包含难度感知路由机制：简单任务使用few-shot提示，复杂任务使用创新的Intention Chain-of-Thought（ICoT）策略。ICoT引导模型捕获任务意图，包括核心算法逻辑和时间复杂度等抽象概念。

Result: 在三个模型和六个标准代码生成基准测试中，RoutingGen在大多数设置下达到最先进性能，同时平均减少46.37%的总token使用量。ICoT在具有挑战性的基准测试中优于六个现有提示基线方法。

Conclusion: RoutingGen通过难度感知路由和意图链式思维，有效解决了现有CoT方法的局限性，在提升代码生成性能的同时显著降低了计算成本，为LLM代码生成提供了更高效、更智能的推理框架。
...
TL;DR: OpenDataArena (ODA) 是一个开源平台，用于系统评估和基准测试大语言模型后训练数据的质量和价值，通过统一训练评估流程、多维度评分框架、数据谱系探索和开源工具集，揭示数据特性与模型性能之间的因果关系。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型的发展依赖于高质量的后训练数据，但数据本身却是一个"黑箱"——数据组成不透明、来源不确定、缺乏系统评估。这种不透明性阻碍了研究的可复现性，也模糊了数据特性与模型行为之间的因果关系。
...
Result: 在ODA上进行了大规模实验：覆盖120多个训练数据集、22个基准测试、600多次训练运行、4000万个处理数据点。分析揭示了数据复杂性与任务性能之间的权衡关系，通过谱系追踪发现了流行基准中的冗余，并绘制了数据集间的谱系关系图。

Conclusion: ODA不仅是一个基准测试平台，更旨在推动从试错式数据管理向数据为中心AI的科学化转变，为研究数据混合规律和基础模型战略组成铺平道路。所有结果、工具和配置都已开源，以促进高质量数据评估的民主化访问。
...
<details>
  <summary>Details</summary>
Motivation: 现代大型语言模型推理成本高、速度慢，而现有的推测采样方法中草稿模型调用次数是预设的超参数，缺乏灵活性
...
### [121] [Grammar Search for Multi-Agent Systems](https://arxiv.org/abs/2512.14079)
*Mayank Singh,Vikas Yadav,Shiva Krishna Reddy Malay,Shravan Nayak,Sai Rajeswar,Sathwik Tejaswi Madhusudhan,Eduardo Blanco*

Main category: cs.AI

TL;DR: 本文提出了一种结构化框架，使用固定、可组合的简单组件来探索多智能体系统搜索空间，相比基于LLM的自由形式搜索方法，在数学和问答领域的五个基准测试中，在四个上表现更优。


<details>
  <summary>Details</summary>
Motivation: 自动搜索多智能体系统已成为智能体AI研究的关键焦点，但现有方法主要依赖基于LLM的代码空间自由形式搜索。本文旨在开发更结构化的框架来改进这一领域。

Method: 提出结构化框架，通过一组固定的简单、可组合组件来探索多智能体系统搜索空间，而不是使用LLM的生成灵活性进行候选生成。

Result: 在数学和问答两个领域的五个基准测试中，该方法在四个基准上优于先前方法。此外，该方法还提供了更经济高效的搜索过程，并生成模块化、可解释且逻辑更简单的多智能体系统。

Conclusion: 尽管在候选生成阶段缺乏LLM的生成灵活性，但基于固定可组合组件的结构化框架在多智能体系统自动搜索中表现更优，具有成本效益和可解释性优势。
...
### [122] [Georeferencing complex relative locality descriptions with large language models](https://arxiv.org/abs/2512.14228)
*Aneesha Fernando,Surangika Ranathunga,Kristin Stock,Raj Prasanna,Christopher B. Jones*

Main category: cs.AI

TL;DR: 本文探索使用大型语言模型自动地理编码生物多样性标本采集记录中的复杂位置描述，通过QLoRA微调方法在多个区域和语言数据集上取得优于现有基线方法的性能。


<details>
  <summary>Details</summary>
Motivation: 生物标本采集记录通常使用叙述性文字而非坐标描述位置，传统基于地名库或语言模型的方法难以处理包含空间关系的相对位置描述，导致地理编码不准确。生物多样性研究需要精确的地理参考，但手动处理劳动密集，亟需自动化解决方案。

Method: 首先识别有效的提示模式，然后使用量化低秩适应（QLoRA）在多个区域和语言（包括英语、西班牙语、葡萄牙语）的生物多样性数据集上微调大型语言模型，处理复杂的叙述性位置描述。

Result: 该方法在固定训练数据量下，平均65%的记录在10公里半径内，优于现有基线。最佳结果（纽约州数据集）达到85%在10公里内和67%在1公里内。模型特别擅长处理冗长复杂的描述。

Conclusion: 大型语言模型在自动地理编码复杂位置描述方面展现出巨大潜力，特别是在生物多样性收集领域，能够有效处理包含空间关系的叙述性位置描述，为生物多样性研究提供更准确的地理参考。
...
### [123] [Gödel's Poetry](https://arxiv.org/abs/2512.14252)
*Kelly J. Davis*

Main category: cs.AI

TL;DR: 论文提出了一种结合专用语言模型和递归分解定理的自动定理证明新方法，在miniF2F测试中达到90.4%通过率，并通过分解进一步提升性能。


<details>
  <summary>Details</summary>
Motivation: 自动定理证明长期以来被视为人工智能的挑战，现有方法在处理复杂定理时存在局限性，需要新的技术突破。

Method: 采用多智能体架构，结合Lean4专用语言模型生成证明，通过递归分解将复杂定理拆分为更简单的蕴含命题，并扩展Kimina Lean Server以支持AST解析实现自动化分解。

Result: 在miniF2F测试中，无分解时达到90.4%通过率，使用分解后性能显著提升。系统已开源发布在PyPI和GitHub上。

Conclusion: 该方法通过语言模型与递归分解的结合，为自动定理证明提供了有效的解决方案，开源实现便于进一步研究和应用扩展。
...
### [124] [Leveraging LLMs for Collaborative Ontology Engineering in Parkinson Disease Monitoring and Alerting](https://arxiv.org/abs/2512.14288)
*Georgios Bouchouras,Dimitrios Doumanas,Andreas Soularidis,Konstantinos Kotis,George A. Vouros*

Main category: cs.AI

TL;DR: 研究探索了LLM在帕金森病监测与预警本体构建中的应用，比较了纯LLM方法（OS和CoT提示）与人机协作方法（X-HCOME和SimX-HCOME+），发现人机协作能产生更全面准确的本体。


<details>
  <summary>Details</summary>
Motivation: 探索LLM是否能够独立构建全面的本体，以及人机协作是否能够提升本体工程的质量，特别是在帕金森病监测与预警这样的复杂领域。

Method: 采用四种方法：1）One Shot提示技术；2）Chain of Thought提示；3）X-HCOME（人机混合本体工程方法）；4）SimX-HCOME+（强调持续人工监督和迭代优化的混合方法）。

Result: 纯LLM方法（OS和CoT）能自主构建本体但不全面，需要大量人工优化；X-HCOME方法显著提升了本体全面性，结果接近专家构建；SimX-HCOME+通过持续人工监督产生了更全面准确的本体。

Conclusion: 人机协作在复杂领域本体工程中具有巨大潜力，未来可开发专门用于本体构建的GPT模型，持续人工监督对提升本体质量至关重要。
...
### [125] [Massive Editing for Large Language Models Based on Dynamic Weight Generation](https://arxiv.org/abs/2512.14395)
*Wentao Wan,Qiqing Lao,Zhiwei Xie,Hefeng Wu,Runnan Lin,Liang Lin,Keze Wang*

Main category: cs.AI

TL;DR: MeG提出了一种基于动态权重生成的大规模知识编辑方法，通过为LLMs特定层附加动态权重神经元，使用扩散模型根据输入查询条件生成神经元权重，实现单神经元大规模知识编辑。


<details>
  <summary>Details</summary>
Motivation: 当前大规模编辑LLMs知识时，在保持编辑的可靠性、通用性和局部性方面面临挑战，需要低成本、高效的知识编辑方法。

Method: MeG方法在LLMs特定层附加动态权重神经元，使用扩散模型根据输入查询条件生成该神经元的权重，通过添加单个动态权重神经元实现大规模知识编辑。

Result: 实验表明MeG在可靠性、通用性和局部性指标上显著优于现有知识编辑方法，特别是在局部性指标的绝对百分比提升方面表现突出。

Conclusion: MeG方法通过动态权重生成机制，为大规模知识编辑提供了有效解决方案，在保持编辑质量的同时实现了高效的知识修改。
...
<details>
  <summary>Details</summary>
Motivation: 在长上下文问答中，确定查询所需的最佳上下文量是一个重要挑战。包含太少段落可能遗漏关键信息，包含太多则引入噪声降低答案质量。传统方法（如固定Top-K检索和单阶段重排序）面临选择正确段落数量的困境，特别是对于事实性问题，通常只需要少量特定证据。

Method: 提出Context-Picker框架，将上下文选择视为决策过程，通过人类启发的两阶段强化学习优化：1）召回导向阶段优先覆盖推理链；2）精确导向阶段积极剪枝冗余以提炼紧凑证据集。为解决奖励稀疏性，提出离线证据蒸馏管道，通过Leave-One-Out程序挖掘"最小充分集"，提供密集的任务对齐监督。
...
Conclusion: Context-Picker通过将上下文选择重新定义为最小充分子集选择问题，并采用两阶段强化学习优化，有效解决了长上下文问答中的上下文量选择问题，实现了更好的答案准确性和效率。
...
TL;DR: 提出了一种基于当前损失自适应调整学习率的GreedyLR调度器，在多个NLP、CV和LLM任务上表现优于现有调度器，具有理论收敛保证和实际应用价值。
...
Method: 提出了GreedyLR调度器，该调度器在训练过程中基于当前损失自适应调整学习率。提供了算法的理论分析，包括收敛性证明和最大化收敛速率的最优缩放因子F的推导。在多个NLP、CV和LLM任务（参数规模高达70亿）上进行了实验验证，包括微调和预训练实验。

Result: 实验结果表明，GreedyLR在准确性、速度和收敛性方面优于多种最先进的调度器。算法对现实噪声场景具有鲁棒性，且易于实现、计算高效。

Conclusion: GreedyLR调度器是一种有效的学习率调度方法，可被视为训练的良好默认调度器选择，具有理论保证和实际应用优势。
...
TL;DR: 研究发现通用变换器在ARC-AGI任务上的性能提升主要来自循环归纳偏置和Transformer的强非线性组件，而非复杂架构设计。基于此提出了增强的通用推理模型URM，在ARC-AGI上取得了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 虽然通用变换器在复杂推理任务如ARC-AGI和数独上表现优异，但其性能提升的具体来源尚未得到充分探索。本研究旨在系统分析UT变体，揭示其性能提升的真正原因。

Method: 系统分析通用变换器变体，发现性能提升主要来自循环归纳偏置和Transformer的强非线性组件。基于此提出通用推理模型URM，通过短卷积和截断反向传播来增强UT。

Result: URM在ARC-AGI任务上取得了显著提升，达到53.8% pass@1（ARC-AGI 1）和16.0% pass@1（ARC-AGI 2）的SOTA性能。

Conclusion: 通用变换器在复杂推理任务上的性能提升主要源于循环归纳偏置和Transformer的非线性能力，而非复杂架构设计。URM通过简单有效的增强实现了SOTA推理性能。
...
### [129] [BiCoRec: Bias-Mitigated Context-Aware Sequential Recommendation Model](https://arxiv.org/abs/2512.13848)
*Mufhumudzi Muthivhi,Terence L van Zyl,Hairong Wang*

Main category: cs.IR

TL;DR: BiCoRec是一个新颖的序列推荐框架，通过自适应处理用户对热门和冷门物品的偏好变化，使用协同注意力机制和一致性损失函数来缓解流行度偏差问题。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的序列推荐模型存在固有的流行度偏差问题，无法很好地适应用户对热门和冷门物品偏好的动态变化，特别是对于那些偏好冷门物品的用户群体。

Method: 提出BiCoRec框架：1）使用协同注意力机制获取流行度加权的用户序列表示；2）采用新的训练方案，通过一致性损失函数从未来偏好中学习；3）自适应地适应用户对热门和冷门物品的偏好变化。
...
Conclusion: BiCoRec框架有效缓解了序列推荐中的流行度偏差问题，显著提升了偏好冷门物品用户的推荐性能，为处理用户动态偏好提供了新的解决方案。
...
### [130] [Intent-Guided Reasoning for Sequential Recommendation](https://arxiv.org/abs/2512.14034)
*Yifan Shao,Peilin Zhou*

Main category: cs.IR

TL;DR: IGR-SR是一个意图引导的推理框架，通过显式提取高级意图来解决序列推荐中推理不稳定和表面推理的问题，显著提升了推荐性能和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有基于推理的序列推荐方法仅以下一个目标物品作为监督，导致两个关键问题：1) 推理不稳定——过程对近期行为和偶然点击过于敏感；2) 表面推理——模型记忆物品间转换而非理解内在行为模式。

Method: 提出IGR-SR框架，包含三个核心组件：1) 潜在意图蒸馏器(LID)，使用冻结编码器和可学习token高效提取多层面意图；2) 意图感知审慎推理器(IDR)，通过双注意力架构将推理解耦为意图审慎和决策制定；3) 意图一致性正则化(ICR)，通过强制不同意图视图间表示一致性确保鲁棒性。

Result: 在三个公开数据集上的实验显示，IGR-SR相比最先进基线平均提升7.13%。在20%行为噪声下，IGR-SR性能仅下降10.4%，而竞争方法分别下降16.2%和18.6%，验证了意图引导推理的有效性和鲁棒性。

Conclusion: IGR-SR通过显式提取和利用高级意图来引导推理过程，有效解决了序列推荐中推理不稳定和表面推理的问题，显著提升了推荐性能和抗噪声能力。
...
### [131] [DTRec: Learning Dynamic Reasoning Trajectories for Sequential Recommendation](https://arxiv.org/abs/2512.14036)
*Yifan Shao,Peilin Zhou,Shoujin Wang,Weizhi Zhang,Xu Cai,Sunghun Kim*

Main category: cs.IR

TL;DR: DTRec提出了一种动态推理轨迹的序列推荐框架，通过分层过程监督和自适应推理停止机制，在推理方向和深度上进行动态调整，显著提升推荐性能并降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的推理增强序列推荐方法存在两个关键限制：1）静态推理方向，使用扁平监督信号，与人类分层推理过程不匹配；2）固定推理深度，对所有用户应用相同计算量，不考虑行为模式复杂度的差异。这些刚性导致性能次优和计算资源浪费。

Method: 提出DTRec框架，包含两个核心组件：1）分层过程监督（HPS），提供从粗到细的监督信号，模拟人类认知过程的渐进式细化；2）自适应推理停止（ARH）机制，通过联合监控三个指标动态调整推理步数。

Result: 在三个真实世界数据集上的实验表明，该方法相比强基线实现了高达24.5%的性能提升，同时计算成本降低了高达41.6%。

Conclusion: DTRec通过动态调整推理轨迹的方向和深度，有效解决了现有方法的刚性限制，在提升推荐性能的同时显著降低了计算开销，为推理增强序列推荐提供了新思路。
...
### [132] [From Feature Interaction to Feature Generation: A Generative Paradigm of CTR Prediction Models](https://arxiv.org/abs/2512.14041)
*Mingjia Yin,Junwei Pan,Hao Wang,Ximei Wang,Shangyu Zhang,Jie Jiang,Defu Lian,Enhong Chen*

Main category: cs.IR

TL;DR: 提出SFG框架，将CTR预测从判别式的"特征交互"范式转向生成式的"特征生成"范式，解决嵌入维度塌缩和信息冗余问题
...
Conclusion: SFG框架通过从判别式到生成式的范式转变，解决了CTR预测中的嵌入塌缩和信息冗余问题，展现出强大的泛化能力和性能优势
...
### [133] [AsarRec: Adaptive Sequential Augmentation for Robust Self-supervised Sequential Recommendation](https://arxiv.org/abs/2512.14047)
*Kaike Zhang,Qi Cao,Fei Sun,Xinran Liu*

Main category: cs.IR

TL;DR: 提出AsarRec框架，通过自适应序列增强解决传统静态增强策略在序列推荐系统中的局限性，提高模型对噪声行为的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现实世界用户行为存在噪声（人为错误、不确定性、行为模糊性），传统基于对比学习的自监督方法依赖预定义的静态增强策略，存在两个关键问题：1）最优增强类型在不同场景差异大；2）不恰当的增强可能降低推荐性能。

Method: 提出AsarRec框架：1）将现有基本增强操作统一为结构化变换矩阵；2）通过编码用户序列为概率转移矩阵，使用可微分的Semi-Sinkhorn算法投影为硬半双随机矩阵来学习变换矩阵；3）联合优化多样性、语义不变性和信息性三个目标。

Result: 在三个基准数据集上不同噪声水平下的广泛实验验证了AsarRec的有效性，展示了其优越的鲁棒性和一致的性能提升。

Conclusion: AsarRec通过自适应序列增强框架克服了传统静态增强策略的局限性，显著提高了序列推荐系统在噪声环境下的鲁棒性和性能。
...
### [134] [SPARQL-LLM: Real-Time SPARQL Query Generation from Natural Language Questions](https://arxiv.org/abs/2512.14277)
*Panayiotis Smeros,Vincent Emonet,Ruijie Wang,Ana-Claudia Sima,Tarcisio Mendes de Farias*

Main category: cs.IR

TL;DR: SPARQL-LLM：基于轻量级元数据的开源、三元存储无关方法，将自然语言转换为SPARQL查询，在准确性、多语言支持、联邦查询能力、速度和成本方面均有显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型方法主要关注单一数据源的响应准确性，忽略了联邦查询能力、运行时性能和生成成本等关键评估标准，导致这些方法难以在生产环境中部署到（可能联邦的）知识图谱上。

Method: SPARQL-LLM采用基于轻量级元数据的开源、三元存储无关架构，包含元数据索引、提示构建、查询生成和执行等专用组件，支持从自然语言文本生成SPARQL查询。

Result: 在最新挑战中F1分数提升24%，支持英语和西班牙语等高资源语言，能够生成复杂的联邦生物信息学查询，比挑战中其他系统快36倍，每个问题成本最高仅0.01美元。

Conclusion: SPARQL-LLM在准确性、多语言支持、联邦查询能力、速度和成本方面均有显著优势，适合实时、低成本的文本到SPARQL应用，已在真实世界去中心化知识图谱上部署。
...
### [135] [Dynamic Context Selection for Retrieval-Augmented Generation: Mitigating Distractors and Positional Bias](https://arxiv.org/abs/2512.14313)
*Malika Iratni,Mohand Boughanem,Taoufiq Dkaki*

Main category: cs.IR

TL;DR: 该论文分析了RAG系统中固定top-k检索策略的问题，包括引入无关信息（干扰项）和"中间迷失"现象，提出了基于查询的动态文档检索数量预测方法，并在完整RAG流程中验证了性能提升。


<details>
  <summary>Details</summary>
Motivation: 标准RAG系统使用固定的top-k检索策略存在两个主要问题：1）可能错过相关信息或引入语义无关的干扰段落，降低输出质量；2）检索到的上下文在输入中的位置会影响模型注意力，中间位置的内容容易被忽略（"中间迷失"现象）。

Method: 首先系统分析干扰项对生成质量的影响，量化不同条件下的效果；然后研究相关段落位置对生成的影响；基于这些分析，提出一个上下文大小分类器，能够根据查询特定的信息需求动态预测最优的检索文档数量；最后将该方法集成到完整的RAG流程中。

Result: 提出的动态检索方法在完整RAG流程中展示了优于固定k基线的性能表现。

Conclusion: 通过动态调整检索文档数量来适应不同查询的信息需求，可以有效解决固定top-k策略的问题，提升RAG系统的整体性能。
...
### [136] [PushGen: Push Notifications Generation with LLM](https://arxiv.org/abs/2512.14490)
...
TL;DR: PushGen是一个自动生成高质量推送通知的框架，通过可控分类提示技术和奖励模型来保证风格控制和质量评估，已在大规模工业应用中部署


<details>
  <summary>Details</summary>
Motivation: 随着生成模型的兴起，利用LLM生成推送内容的需求增长，但LLM在风格控制和可靠质量评估方面存在挑战，这两者直接影响用户参与度

Method: PushGen结合两个关键组件：(1) 可控分类提示技术，引导LLM输出符合期望风格的内容；(2) 奖励模型，对生成的候选内容进行排序和选择

Result: 广泛的离线和在线实验证明了其有效性，已在大规模工业应用中部署，每天为数亿用户提供服务

Conclusion: PushGen提供了一个自动化框架，能够生成与人工制作相媲美的高质量推送通知，解决了LLM在推送内容生成中的风格控制和质量评估问题
...
### [137] [Pairwise Comparison for Bias Identification and Quantification](https://arxiv.org/abs/2512.14565)
*Fabian Haak,Philipp Schaer*

Main category: cs.IR

TL;DR: 该研究提出通过成对比较来减少偏见标注的工作量，评估了更高效的成对比较评级方法，并通过模拟和真实数据验证了其效果。


<details>
  <summary>Details</summary>
Motivation: 在线新闻和社交媒体中的语言偏见普遍存在但难以测量，由于主观性、上下文依赖性和高质量标注数据稀缺，识别和量化偏见仍然困难。

Method: 通过成对比较进行偏见标注以减少标注工作量，评估了三种成本感知替代方案的各种评级技术和参数，在模拟环境中测试鲁棒性和成本-质量权衡，并在真实的人类标注偏见基准数据集上应用。

Result: 研究支持使用成对比较作为量化主观语言方面的实用基础，实现了可重复的偏见分析，并提供了比较和匹配组件的优化方案。

Conclusion: 成对比较方法为创建高质量基准数据集和量化偏见及其他主观语言方面提供了基础，同时为成本感知的大规模标注提供了实施蓝图。
