...
TL;DR: 本文展示了如何使用持久同调（PH）这一拓扑数据分析方法，客观地分析和区分艺术风格，包括不同艺术家、同一艺术流派内的艺术家，以及真实艺术家作品与AI生成作品的区分。
...
Method: 使用持久同调（PH）这一拓扑数据分析方法，对艺术作品进行数学分析，提取艺术风格的拓扑特征。

Result: PH方法能够以统计确定性区分不同艺术家（包括不同艺术流派和同一流派内的艺术家），并能区分真实艺术家作品与AI生成的模仿作品。
...
TL;DR: 提出了一种使用StyleGAN2生成猕猴面部表情的方法，通过数据增强、样本选择和损失函数优化来解决训练数据不足的问题，能够为多个猕猴个体生成多样化的面部表情。


<details>
  <summary>Details</summary>
Motivation: 由于可用的训练图像在数量和变化上都很有限，特别是不同个体的面部表情，因此使用生成式AI技术生成动物面部具有挑战性。本研究专注于在系统神经科学和进化研究中广泛研究的猕猴。

Method: 1) 使用运动转移技术通过计算机图形学为静态图像添加动画来进行数据增强；2) 基于初始训练的StyleGAN2模型的潜在表示进行样本选择，确保训练数据集的变异性和均匀采样；3) 损失函数优化以确保准确再现细微运动（如眼球运动）。
...
TL;DR: 本文提出了PairHuman数据集，这是首个专为生成高质量双人肖像而设计的大规模基准数据集，包含超过10万张图像，并介绍了DHumanDiff基准方法，在双人肖像生成中实现了增强的面部一致性和个性化生成与语义驱动场景创建的平衡。


<details>
  <summary>Details</summary>
Motivation: 个性化双人肖像定制具有重要的应用价值，如保存情感记忆和促进婚纱摄影规划，但缺乏基准数据集阻碍了高质量双人肖像定制的发展。

Method: 提出了PairHuman数据集（包含10万+图像，涵盖多种场景、服装和双人互动，带有丰富元数据）和DHumanDiff基准方法（专为双人肖像生成设计，增强面部一致性，平衡个性化人物生成和语义驱动场景创建）。

Result: 实验结果表明，所提出的数据集和方法能够生成高度定制化的肖像，具有卓越的视觉质量，并符合人类偏好。

Conclusion: PairHuman数据集和DHumanDiff方法为双人肖像生成提供了有效的解决方案，能够产生高质量、个性化的双人肖像，数据集已公开可用。
...
TL;DR: 本文提出了一种基于无监督自编码器和CDF 97小波变换的混合高斯-泊松去噪方法，用于中子成像数据去噪，相比传统非机器学习方法具有更好的重建误差和边缘保持能力。
...
Conclusion: 这种基于机器学习的去噪方法在中子图像噪声减少和ICF实验三维重建分析方面展现了有前景的进展。
...
TL;DR: 本文提出了一种基于相关性反馈的视觉语言模型检索增强方法，包括伪相关性反馈、生成式相关性反馈、注意力反馈总结器和显式反馈，能够在不进行微调的情况下提升检索性能。


<details>
  <summary>Details</summary>
Motivation: 传统视觉语言模型需要微调或扩大模型规模来提升检索性能，本文旨在通过推理阶段的相关性反馈机制来改善检索效果，这种模型无关的设计既可作为微调的替代方案，也可与微调后的模型配合使用。

Method: 提出了四种相关性反馈策略：1）伪相关性反馈（PRF）- 基于排名靠前结果优化查询嵌入；2）生成式相关性反馈（GRF）- 使用合成标题进行查询优化；3）注意力反馈总结器（AFS）- 基于Transformer的多模态细粒度特征整合模型；4）显式反馈 - 使用真实标题作为性能上限基准。

Result: 在Flickr30k和COCO数据集上的实验表明，GRF、AFS和显式反馈相比无反馈检索，在MRR@5指标上为较小VLM带来3-5%的提升，为较大VLM带来1-3%的提升。AFS与显式反馈类似，能够缓解查询漂移，在迭代多轮检索设置中比GRF更稳健。
...
TL;DR: 本文提出SaFeR-CLIP框架，通过将不安全概念重定向到语义上最接近的安全替代概念，在保持鲁棒安全性的同时，比先前方法恢复高达8.0%的零样本准确率。
...
Method: 提出邻近感知方法，将不安全概念重定向到语义上最接近的安全替代概念，最小化表示变化，并开发SaFeR-CLIP微调框架。

Result: SaFeR-CLIP成功平衡安全性和性能，在保持鲁棒安全性的同时，比先前方法恢复高达8.0%的零样本准确率。
...
TL;DR: 本文提出了一个三阶段框架，从单视图SVG输入生成多视图一致的SVG，具有几何和颜色一致性，同时减少冗余路径并保留结构细节。


<details>
  <summary>Details</summary>
Motivation: 解决单对象SVG从单视图输入生成多视图一致SVG的未充分探索问题，支持资产创建和语义向量编辑等应用。

Method: 三阶段框架：1）将栅格化输入提升为3D表示并渲染多视图图像；2）扩展SAM2的时间记忆机制到空间域，构建空间记忆库建立跨视图部件级对应；3）在栅格到向量转换中执行路径整合和结构优化。

Result: 生成的SVG在多个视图间表现出强大的几何和颜色一致性，显著减少冗余路径，同时保留精细结构细节。

Conclusion: 该工作连接了生成建模和结构化向量表示，为单输入、对象级多视图SVG生成提供了可扩展路径。
...
TL;DR: Mesh RAG是一个无需训练、即插即用的框架，用于自回归网格生成模型，通过检索增强生成来提升网格质量、加速生成速度并支持增量编辑。


<details>
  <summary>Details</summary>
Motivation: 传统3D网格制作耗时且难以扩展，现有自回归模型存在质量与速度的权衡问题，严格的序列依赖限制了生成效率和编辑能力。

Method: 受语言模型RAG启发，通过点云分割、空间变换和点云注册技术来检索、生成和整合网格组件，解耦严格序列依赖。

Result: 在多种基础自回归网格生成模型上验证，显著提升网格质量，加速生成速度，并支持增量编辑，无需模型重新训练。

Conclusion: Mesh RAG框架有效解决了自回归网格生成中的质量-速度权衡问题，实现了高效并行推理和灵活编辑能力。
...
TL;DR: REO-Instruct是首个统一的地球观测基准数据集，专注于描述性和回归任务，通过建立森林生态场景的认知逻辑链，连接定性理解和定量预测。
...
TL;DR: 本文提出了BOP-ASK数据集，这是一个用于物体交互推理的大规模数据集，包含超过15万张图像和3300万个问答对，涵盖六个任务，旨在评估和提升视觉语言模型在精细空间理解方面的能力。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在空间推理基准测试中表现优异，但这些评估掩盖了在理解物体交互方面的关键弱点。现有基准测试主要关注高层次关系（如'左侧'、'后面'等），而忽略了现实世界应用所需的精细空间理解：精确的3D定位、物体间的物理兼容性、物体可供性和多步空间规划。

Method: 利用来自物体姿态估计基准数据集（BOP）的6D物体姿态，开发了一个数据生成流程，从中提取精细标注，包括抓取姿态、参考物体姿态、路径规划轨迹、相对空间和深度关系以及物体间关系。

Result: 实验表明，在BOP-ASK上训练的模型优于基线模型，并展现出新兴能力，如精确的物体和抓取姿态估计、轨迹规划以及在杂乱环境中的精细物体中心空间推理。

Conclusion: BOP-ASK数据集为训练和评估视觉语言模型提供了丰富资源，填补了现有基准在精细空间理解方面的空白，将促进视觉语言模型在物体交互推理能力的发展。
...
TL;DR: 提出Parts-Mamba混合模型，结合GCN和Mamba架构，通过部件特定扫描和部件-身体融合模块增强远距离关节上下文信息捕获能力，在骨架动作识别中有效应对遮挡导致的不完整骨架问题。


<details>
  <summary>Details</summary>
Motivation: 现实场景中由于遮挡或通信质量问题，捕获的骨架往往不完整或缺失部分，现有GCN模型在存在此类非理想情况时性能较差，主要因为缺少局部上下文信息。

Method: 提出Parts-Mamba混合GCN-Mamba模型，通过部件特定扫描特征捕获部件特定信息，并通过部件-身体融合模块保持非相邻关节的上下文。

Result: 在NTU RGB+D 60和NTU RGB+D 120数据集的不同遮挡设置下进行评估，准确率最高提升12.9%。

Conclusion: Parts-Mamba模型通过有效捕获和保持远距离关节的上下文信息，显著提升了在非完整骨架条件下的动作识别性能。
...
Method: 通过引入JGW目标，扩展GW框架以支持对象集合的同步匹配，采用最优传输算法（包括熵正则化）进行求解。

Result: 在部分匹配的基准测试中，该方法在准确性和计算效率方面优于其他GW变体，在合成和真实数据集上的实验验证了其在多形状匹配中的有效性。
...
TL;DR: 该论文提出了一种称为REPA（表示对齐）的方法，通过将扩散或流式生成模型与预训练的自监督视觉编码器（如DINOv2）的内部表示对齐，来改进逆问题中的重建质量。该方法在推理时指导重建过程，显著提高了重建保真度和感知真实性。


<details>
  <summary>Details</summary>
Motivation: 在逆问题中，预训练的生成模型被用作先验，但缺乏地面真实信号。作者发现通过将生成模型与预训练自监督编码器的表示对齐，可以提供强大的归纳偏置，改善收敛性和样本质量。

Method: 提出REPA方法，在推理时对齐扩散或流式生成模型与预训练自监督视觉编码器（如DINOv2）的内部表示。通过正则化方法引导重建过程，即使没有真实信号，也能通过近似目标特征对齐来提升重建质量。

Result: 理论分析表明REPA正则化与DINOv2嵌入空间中的散度度量相关，且REPA更新将模型内部表示导向干净图像。实验证明该方法在超分辨率、框内修复、高斯去模糊和运动去模糊等任务中一致提升重建质量，同时通过减少离散化步骤提供效率增益。

Conclusion: REPA方法为逆问题提供了一种有效的表示对齐机制，通过利用预训练自监督编码器的知识显著改善重建的感知质量，且具有通用性和效率优势。
...
TL;DR: 提出NFGlassNet方法，利用闪光/非闪光图像中的反射动态进行玻璃表面检测，通过反射对比挖掘模块和反射引导注意力模块实现准确检测，并在自建数据集上验证了方法的优越性。


<details>
  <summary>Details</summary>
Motivation: 现有玻璃表面检测方法依赖边界线索或反射线索，但未能充分利用玻璃本身的内在特性。观察到现实场景中玻璃两侧光照强度差异导致反射变化的现象，提出利用闪光/非闪光图像对来改进检测精度。

Method: 提出NFGlassNet方法，包含反射对比挖掘模块(RCMM)用于提取反射特征，以及反射引导注意力模块(RGAM)用于融合反射和玻璃表面特征。构建了包含3.3K对闪光/非闪光图像的数据集用于训练。

Result: 大量实验表明，该方法在玻璃表面检测任务上优于现有最先进方法。

Conclusion: 利用闪光/非闪光图像对中的反射动态特性可以有效提升玻璃表面检测性能，所提出的NFGlassNet方法在多个场景下表现出优越的检测效果。
...
TL;DR: 本文提出了R-AVST数据集和AVST-Zero模型，用于解决真实世界音频-视觉时空推理任务。R-AVST包含5K个未剪辑视频和27K个对象，涵盖100种音频-视觉事件类型，并定义了三个核心推理任务。AVST-Zero采用强化学习方法，通过多维奖励直接优化行为，无需中间监督。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在视频理解方面进展迅速，但主要关注简单视频场景，无法反映真实世界中复杂多样的音频-视觉事件。需要构建能够支持细粒度时空推理的数据集和模型。

Method: 1) 构建R-AVST数据集：使用基于LLM的关键对象提取、自动空间标注和人工质量检查的流水线；2) 提出AVST-Zero模型：基于强化学习，避免中间监督，通过精心设计的多维奖励直接优化行为。

Result: R-AVST数据集包含5,000+未剪辑视频、27,000+对象和8,000+高质量问答对。AVST-Zero在音频-视觉时空推理任务上表现出与现有模型竞争的性能。
...
TL;DR: 本文提出了Warm Diffusion模型，通过结合热扩散（纯噪声）和冷扩散（纯模糊）的优点，使用模糊-噪声混合扩散方法，利用图像频谱依赖性来改进生成质量。


<details>
  <summary>Details</summary>
Motivation: 热扩散未能利用高频细节与低频结构之间的强相关性，导致生成早期步骤中的随机行为；冷扩散虽然利用图像相关性进行预测，但忽略了噪声在塑造数据流形中的作用，导致流形外问题和性能下降。

Method: 提出了Warm Diffusion（模糊-噪声混合扩散模型BNMD），通过分治策略利用图像频谱依赖性，将去噪和去模糊过程解耦，并分析模糊-噪声比（BNR）来平衡模型学习动态和数据流形变化。

Result: 在多个基准测试上的广泛实验验证了该方法在图像生成中的有效性。

Conclusion: Warm Diffusion成功整合了热扩散和冷扩散的优势，通过联合控制模糊和噪声，在图像生成任务中取得了更好的性能。
...
Method: 提出Delta-Denoising方法：1) 使用最小提示对驱动两个共享调度的扩散分支进行对比；2) 通过累积每步去噪差异生成图像特定的定位图；3) 使用定位图指导潜在修复，在后期扩散步骤中生成局部缺陷；4) 进行标记级提示优化和对齐，增强异常标记；5) 在预测区域应用空间注意力偏置。
...
TL;DR: 提出DGAF-VSR方法，通过光流引导的扭曲模块和特征域时序条件模块，在扩散模型视频超分辨率中实现更好的对齐和补偿，显著提升感知质量、保真度和时序一致性。
...
Method: 提出DGAF-VSR方法，包含光流引导扭曲模块(OGWM)在放大分辨率下保持高频细节，以及特征域时序条件模块(FTCM)在特征域提供密集指导。

Result: 在合成和真实数据集上的实验表明，DGAF-VSR在感知质量(DISTS降低35.82%)、保真度(PSNR提升0.20dB)和时序一致性(tLPIPS降低30.37%)方面超越现有最佳方法。

Conclusion: 特征域比像素域更适合信息补偿，在放大分辨率下进行扭曲能更好保留高频信息，所提方法有效解决了基于扩散模型的视频超分辨率的关键问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在时空视频定位任务中与真实世界需求存在显著差距，主要由于基准测试范围有限导致模型存在类别偏见、推理过于简化和语言鲁棒性差的问题。

Method: 提出了Forward-Backward-Refinement标注流程，结合多方向跟踪和智能错误校正；开发了DeepSTG系统评估框架；提出了PG-TAF训练免费两阶段框架，将STVG分解为高级时间定位和细粒度时空传播。

Result: 在复杂真实场景中性能平均下降10.4%，特别是在小/遮挡对象和复杂空间关系中。PG-TAF在OmniGround上m_tIoU和m_vIoU分别提升25.6%和35.6%，在四个基准测试上均取得一致增益。

Conclusion: OmniGround基准测试揭示了现有STVG方法的局限性，PG-TAF框架通过分解任务有效提升了性能，为时空视频定位提供了新的解决方案。
...
TL;DR: MultiPriv是首个系统评估视觉语言模型中个体级隐私推理能力的基准，揭示了现有模型在隐私推理方面的严重风险，并发现感知级指标无法有效预测这些风险。
...
Method: 提出了隐私感知与推理（PPR）框架，构建了包含合成个体配置文件的双语多模态数据集，设计了9个挑战性任务来评估完整的PPR谱系。

Result: 对50多个基础模型和商业VLM进行大规模评估发现：许多VLM存在显著的、未测量的基于推理的隐私风险；感知级指标无法预测推理风险；现有安全对齐措施对此类推理攻击不一致且无效。
...
TL;DR: 提出了一种联合建模动态图像序列和运动场的隐式神经表示框架，通过光学流方程作为物理约束，在动态心脏MRI中实现了优于现有方法的图像重建质量、运动估计精度和时间一致性。


<details>
  <summary>Details</summary>
Motivation: 传统基于预估计光流的运动补偿重建方法在欠采样情况下不准确，会降低重建质量。需要一种能够同时恢复时间相干图像和运动场的方法。

Method: 使用两个隐式神经表示(INR)分别参数化时空图像内容和光流场，通过光学流方程耦合两者作为物理正则化，并结合k空间测量的数据一致性损失进行联合优化。
...
TL;DR: 本文提出了FingerCap任务和FingerCap-40K数据集，用于生成描述精细手指动作的文本。针对现有视频多模态大模型的时间稀疏性问题，提出了FiGOP方法，通过结合RGB关键帧和手部关键点来捕捉细微的手指运动动态。


<details>
  <summary>Details</summary>
Motivation: 理解精细的人类手部运动对视觉感知、具身智能和多模态通信至关重要。现有方法在捕捉手指级语义方面存在困难，特别是时间稀疏性限制了模型对高频细微手指动态的感知能力。

Method: 提出了FiGOP方法，将每个RGB关键帧与后续手部关键点配对，直到下一个关键帧。使用轻量级时间编码器将关键点转换为运动嵌入，并与RGB特征集成。这种方法在不增加RGB采样密度的情况下恢复了精细的时间线索。

Result: 在FingerCap-40K数据集上的实验表明，现有的开源和闭源视频多模态大模型在手指级推理方面仍然存在困难，而采用FiGOP增强的模型在HandJudge评估和人类研究中都取得了持续的性能提升。

Conclusion: FiGOP方法有效解决了视频多模态大模型在捕捉精细手指运动时的时间稀疏性问题，为手指级手部运动理解提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有面部表情检测方法主要依赖完整的时间边界标注，成本高且耗时。本文旨在研究点监督的面部表情检测，仅需每个实例的单时间戳标注即可训练。
...
Conclusion: 该方法成功实现了仅使用点监督的面部表情检测，通过软伪标签和双分支设计有效解决了标注成本高的问题。
...
TL;DR: MatPedia是一个基于联合RGB-PBR表示的基础模型，通过将材料编码为RGB外观和PBR地图的相互依赖潜在变量，使用视频扩散架构统一处理多种材料任务。
...
Method: 提出新颖的联合RGB-PBR表示，将材料编码为两个相互依赖的潜在变量：RGB外观和四个PBR地图。将它们制定为5帧序列，使用视频扩散架构来捕捉相关性并转移RGB生成模型的视觉先验。

Result: 在MatHybrid-410K混合语料库上训练，MatPedia实现了原生1024×1024合成，在质量和多样性方面显著超越现有方法。

Conclusion: MatPedia提供了一个统一框架，能够处理文本到材料生成、图像到材料生成和内在分解等多种材料任务，在单个架构中实现了卓越的性能。
...
TL;DR: 提出了一种面向边缘设备的烹饪食物图像生成方法，通过配方和烹饪状态指导生成逼真的熟食图像，并引入烹饪图像相似度指标确保时序一致性和烹饪合理性。


<details>
  <summary>Details</summary>
Motivation: 在边缘设备上从原始食材图像合成逼真的熟食图像是一个挑战性任务，现有方法要么生成结果不真实，要么资源消耗过大不适合边缘部署。

Method: 构建首个基于烤箱的烹饪进程数据集，提出边缘高效的配方和烹饪状态引导生成器，引入领域特定的烹饪图像相似度指标作为训练损失和进度监控信号。

Result: 模型在多个数据集上显著优于现有基线方法，FID分数分别提升30%（自有数据集）和60%（公共数据集）。

Conclusion: 该方法能够根据用户偏好的视觉目标生成逼真的烹饪食物图像，在边缘设备上实现了高效且真实的图像生成。
...
TL;DR: SeeCLIP框架通过细粒度语义增强解决开放集域泛化中的结构风险和开放空间风险困境，使用语义感知提示增强、双工对比学习和语义引导扩散来生成伪未知样本，在五个基准测试中比现有方法提升3%准确率和5%H分数。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理开放集域泛化时面临结构风险与开放空间风险的困境，特别是在区分与已知类别具有细粒度视觉相似性的'硬未知'样本时容易过度自信。

Method: 提出语义增强CLIP框架，包括：语义感知提示增强模块分解图像为判别性语义标记；双工对比学习保持与已知类的分离性和语义邻近性；语义引导扩散模块通过扰动语义标记生成伪未知样本。

Result: 在五个基准测试中，相比最先进方法实现了3%准确率和5%H分数的持续提升。

Conclusion: SeeCLIP通过细粒度语义增强有效解决了开放集域泛化中的关键挑战，显著提升了模型在遇到域偏移和未知类别时的性能。
...
<details>
  <summary>Details</summary>
Motivation: 3DGS使用大量高斯基元来拟合场景，导致存储和计算开销巨大。现有剪枝方法依赖手动设计标准或引入额外可学习参数，效果不佳。
...
TL;DR: FLUID是一个无需训练的框架，通过在预训练扩散模型的潜在空间中进行身份替换来实现人脸去识别。它使用化学替代机制启发的方法，在潜在h空间中发现身份编辑方向，并通过优化引导实现属性保留和身份抑制。


<details>
  <summary>Details</summary>
Motivation: 当前人脸去识别方法在身份抑制和属性保留之间难以取得良好平衡，需要一种更有效的框架来同时实现身份保护和视觉属性保持。

Method: 在预训练无条件扩散模型的潜在h空间中进行语义位移，通过优化引导发现身份编辑方向，使用新颖的试剂损失函数监督属性保留和身份抑制，提出线性和测地线编辑方案来导航潜在流形。

Result: 在CelebA-HQ和FFHQ数据集上的实验表明，FLUID在身份抑制和属性保留之间取得了优越的平衡，在定性和定量指标上都优于最先进的去识别方法。

Conclusion: FLUID框架通过潜在空间身份替换实现了有效的人脸去识别，在保持视觉属性的同时有效抑制身份信息，为隐私保护提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中的深度很少是单一的，透射材料会产生分层模糊性，而现有模型仍是被动的，试图估计锚定到最近表面的静态深度图，而人类会主动转移焦点来感知所需深度。

Method: 引入DepthFocus，一个可操控的视觉Transformer，通过标量深度偏好进行条件化，模型动态调整计算以聚焦到目标深度。训练主要利用新构建的50万多层合成数据集，捕捉多样化的透视效果。

Result: DepthFocus在传统单深度基准测试（如BOOSTER）上达到最先进性能，并在新提出的真实和合成多深度数据集上定量展示了意图对齐的估计。此外，在未见过的透视场景上表现出强大的泛化能力。

Conclusion: DepthFocus作为向主动和类人3D感知迈出的重要一步，展示了其鲁棒性，能够实现复杂场景中的选择性深度感知。
...
TL;DR: MVLR是一个统一的记忆增强视觉语言恢复模型，用于在多种恶劣天气条件下恢复不同退化程度的图像。它结合了轻量级编码器-解码器主干、视觉语言模型和隐式记忆库，通过链式推理编码天气退化先验，并通过动态交叉注意力机制自适应融合多尺度视觉特征。


<details>
  <summary>Details</summary>
Motivation: 在恶劣天气条件下（如雨、雾、雪或它们的混合）实现可靠的视觉感知对于自动驾驶和户外机器人至关重要，但具有挑战性。需要一种能够处理不同退化程度和各种天气条件的统一恢复模型。

Method: MVLR模型结合了轻量级编码器-解码器主干、视觉语言模型和隐式记忆库。VLM执行链式推理来编码天气退化先验，IMB存储退化模式的连续潜在表示。VLM生成的先验查询IMB以检索细粒度退化原型，然后通过动态交叉注意力机制自适应地与多尺度视觉特征融合。
...
TL;DR: 本文介绍了ConfusedTourist，一个评估视觉语言模型在混合文化线索场景下稳定性的文化对抗鲁棒性测试套件，发现现有模型在简单图像堆叠扰动下准确率大幅下降。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型文化评估通常只关注单一文化概念，忽略了多文化线索共存场景，而模型在这种混合文化输入下的稳定性对支持多元文化社会至关重要。

Method: 开发ConfusedTourist测试套件，通过图像堆叠和基于图像生成的变体来扰动地理文化线索，评估VLMs的稳定性，并进行可解释性分析。

Result: 实验显示VLMs在简单图像堆叠扰动下准确率大幅下降，基于图像生成的变体效果更差，可解释性分析表明失败源于注意力系统性地转向干扰线索。
...
TL;DR: RacketVision是一个用于体育分析的新型计算机视觉数据集和基准，涵盖乒乓球、网球和羽毛球，提供大规模细粒度球拍姿态标注和球位置信息。


<details>
  <summary>Details</summary>
Motivation: 推动体育分析中的计算机视觉研究，解决复杂人机交互问题，填补现有数据集在球拍姿态细粒度标注方面的空白。

Method: 构建包含乒乓球、网球和羽毛球的大规模数据集，提供球拍姿态和球位置的细粒度标注，采用CrossAttention机制进行多模态融合。

Result: 评估发现简单拼接球拍姿态特征会降低性能，而CrossAttention机制能有效利用这些特征，在轨迹预测任务上超越单模态基线方法。

Conclusion: RacketVision为动态目标跟踪、条件运动预测和体育多模态分析提供了多功能资源和强起点，CrossAttention是多模态融合的关键机制。
...
TL;DR: 本文提出了一个统一的行人跟踪框架OmniPT，能够进行跟踪、基于参考的跟踪以及交互式生成跟踪对象的语义理解。通过RL-Mid Training-SFT-RL训练阶段解决如何将跟踪任务建模为基础模型可执行任务以及如何使模型输出格式化答案的问题。
...
Method: 提出OmniPT框架，采用RL-Mid Training-SFT-RL训练阶段：基于预训练的LVLM权重，首先进行简单RL阶段使模型输出固定且可监督的边界框格式；随后使用大量行人相关数据集进行中期训练；最后在几个行人跟踪数据集上进行监督微调，再进行另一个RL阶段以提升跟踪性能并增强指令跟随能力。

Result: 在跟踪基准测试上的实验结果表明，所提出的方法能够比先前方法表现更好。

Conclusion: OmniPT框架成功地将行人跟踪任务建模为基础模型可执行的任务，并通过多阶段训练策略实现了优于先前方法的性能，证明了LVLMs在实例级跟踪任务中的潜力。
...
TL;DR: ReBrain是一个基于检索增强扩散框架的脑部MRI重建方法，通过Brownian Bridge扩散模型从稀疏CT切片合成MRI，并结合检索相似CT切片作为参考来确保结构连续性。


<details>
  <summary>Details</summary>
Motivation: 由于物理或临床限制，某些患者无法进行MRI扫描，而低剂量CT协议导致稀疏CT体积，使得从CT准确重建完整脑部MRI体积具有挑战性。

Method: 使用Brownian Bridge扩散模型从稀疏3D CT切片合成2D MRI切片，同时通过微调检索模型从先验数据库中检索结构和病理相似的CT切片作为参考，通过ControlNet分支指导中间MRI切片生成，并对罕见检索失败情况应用球面线性插值提供补充指导。

Result: 在SynthRAD2023和BraTS数据集上的广泛实验表明，ReBrain在稀疏条件下的跨模态重建中达到了最先进的性能。

Conclusion: ReBrain框架能够有效解决从稀疏CT扫描重建脑部MRI的挑战，在跨模态重建任务中表现出色。
...
TL;DR: STAR模型通过使用均匀生成树的遍历顺序来保持采样性能，同时提供灵活的序列顺序以适应图像编辑。


<details>
  <summary>Details</summary>
Motivation: 传统的自回归模型在视觉生成中暴露随机排列的序列顺序会导致性能下降或序列顺序选择灵活性受限，需要一种既能保持性能又能提供灵活序列顺序的方法。

Method: STAR利用图像块位置定义的网格中采样的均匀生成树的遍历顺序，通过广度优先搜索获得遍历顺序，并通过拒绝采样确保连接的部分观察作为序列前缀出现。

Result: STAR在保持采样性能的同时，提供了足够的序列顺序灵活性来适应图像编辑，且无需对语言AR建模中广泛采用的模型架构进行重大更改。

Conclusion: STAR通过定制但结构化的随机策略，在保持后补完成能力的同时维持了采样性能，解决了传统方法在性能和灵活性之间的权衡问题。
...
TL;DR: REArtGS++提出了一种基于平面高斯抛光和时序几何约束的可泛化铰接物体重建方法，解决了REArtGS在螺旋关节和多部件物体上的局限性。
...
Conclusion: REArtGS++通过引入时序几何约束和平面高斯抛光，实现了更好的可泛化铰接物体重建性能。
...
TL;DR: SketchVerify是一个无需训练的草图验证规划框架，通过测试时采样和验证循环来改进运动规划质量，生成更动态一致的运动轨迹，然后进行完整的视频生成。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成方法主要使用单次规划或迭代细化，前者限于简单运动，后者计算成本高。需要一种既能生成高质量运动轨迹又高效的方法。
...
Conclusion: SketchVerify框架通过草图验证机制有效提升了运动规划质量，在保持高效率的同时实现了更好的动态一致性和物理合理性。
...
Conclusion: DiverseVAR有效解锁了VAR模型的固有生成潜力，同时保持了高保真合成。
...
TL;DR: 提出了一种基于平面高斯溅射的类别无关铰接物体重建框架，仅需单状态下的稀疏视角RGB图像输入，无需多阶段多视角观测，实现了更高保真度的部件级表面重建。
...
Method: 首先引入高斯信息场感知最优稀疏视角，然后将3D高斯压缩为平面高斯以准确估计法线和深度，通过深度平滑正则化和少样本扩散进行粗到细优化，并为每个高斯基元引入部件分割概率并通过渲染分割掩码反向投影更新。

Result: 在合成和真实数据上的大量实验结果表明，该方法比现有方法实现了更高保真度的部件级表面重建。

Conclusion: 该方法仅使用稀疏视角RGB图像就能实现高质量的铰接物体重建，代码将公开。
...
TL;DR: 本研究提出了一个针对大型视觉语言模型幻觉问题的综合干预框架，发现幻觉来源于图像-输入文本、图像-输出文本和文本-文本三种路径的相互作用，并开发了针对不同问答对齐格式的干预方法。
...
Method: 提出了与Transformer因果架构对齐的综合干预框架，分析不同干预路径对幻觉的影响，并开发了针对判别式和生成式格式的关键幻觉头识别和干预方法。

Result: 实验表明，该方法在多个基准测试中能够持续减少不同对齐类型的幻觉现象。

Conclusion: LVLMs的幻觉问题源于多种因果路径的相互作用，通过针对不同问答格式的路径干预可以有效减少幻觉。
...
TL;DR: 提出了一种名为分区自适应对比学习（PACL）的方法，通过利用预训练文本模型的知识来增强预训练视觉模型的情感感知能力，解决视觉情感识别中的'情感鸿沟'问题。
...
Method: 提出分区自适应对比学习方法（PACL），关注社交媒体数据中图像和文本之间的事实与情感联系，通过分离不同类型的样本并为每种类型设计不同的对比学习策略，动态构建正负样本对，充分利用噪声样本的潜力。
...
TL;DR: 提出了一种基于语义掩码引导的LiDAR点云编辑方法，通过将点云转换为2D距离图像，利用扩散模型生成新的合成LiDAR点云，用于自动驾驶系统训练。


<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶系统训练需要大量多样的点云数据，特别是边缘案例场景，但真实世界数据获取困难且成本高昂。现有的3D虚拟环境模拟方法耗时且难以完全捕捉真实场景的复杂性。

Method: 采用距离图像投影和语义掩码条件化，通过扩散模型生成点云。将点云转换为2D距离图像作为中间表示，使用基于凸包的语义掩码指导生成过程，确保几何一致性和真实性。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态推理模型在生成长推理链时存在冗余自反思维，而基于静态视觉参考的CoT压缩方法在多模态推理中效果有限。

Method: ChainV首先基于前一步推理进行粗略视觉补丁选择，然后根据平均注意力强度识别最具代表性的原子视觉提示，并引入一致性评估机制来调整自反思维水平。

Result: 在MathVista基准测试中准确率提升2.3%，推理延迟降低51.4%，输出token长度缩短24.5%。

Conclusion: ChainV通过动态整合视觉提示显著提高了多模态推理的准确性和效率，尤其在数学密集型任务中表现突出。
...
TL;DR: 本文分析了多语言文本到图像模型在跨语言文化一致性方面存在的问题，发现模型通常产生文化中性或英语偏向的结果。通过定位文化敏感神经元，提出了两种对齐策略来增强文化一致性。


<details>
  <summary>Details</summary>
Motivation: 当前多语言文本到图像模型在跨语言提示下往往产生文化中性或英语偏向的结果，无法保持跨语言文化一致性，这限制了模型在不同文化背景下的应用。

Method: 提出了一种探测方法定位文化敏感信号到少量固定层中的神经元，并引入两种对齐策略：推理时文化激活（无需主干微调）和层定向文化增强（仅更新文化相关层）。

Result: 在CultureBench上的实验表明，相比强基线方法，所提方法在文化一致性方面取得了持续改进，同时保持了保真度和多样性。

Conclusion: 通过定位文化敏感神经元并采用针对性的对齐策略，可以有效提升多语言文本到图像模型的文化一致性，而无需全面微调模型。
...
<details>
  <summary>Details</summary>
Motivation: 大时空尺度刚性运动重建面临建模范式限制、严重运动模糊和物理一致性不足等挑战，需要新的方法来提高重建质量和物理合理性。

Method: 提出三重监督方案：通过加速度约束确保物理合理性，利用事件流提供高时间分辨率指导，使用卡尔曼正则器融合多源观测；设计基于实时运动状态的自适应训练调度策略；构建首个针对自然快速刚性运动的RGB-事件配对数据集。

Result: 实验表明PEGS在大时空尺度运动重建方面优于主流动态方法，能够有效处理运动模糊并恢复精确的运动轨迹。

Conclusion: PEGS框架成功地将物理先验与事件流增强相结合，为大时空尺度刚性运动重建提供了有效的解决方案，显著提升了重建质量和物理一致性。
...
TL;DR: 本文提出从传统的1D CCT映射转向2D色度空间，并使用轻量级MLP替代CCT插值，显著提高了非普朗克光源下的色度精度。


<details>
  <summary>Details</summary>
Motivation: 现代LED光源严重偏离普朗克轨迹，传统基于CCT的一维插值方法在非普朗克光源下存在局限性。

Method: 使用基于2D色度特征的轻量级多层感知机替代传统CCT插值，通过包含代表性LED光源的光箱校准程序训练MLP。

Result: 在多种LED照明下，该方法平均减少22%的角度再现误差，保持与传统光源的向后兼容性，支持多光源场景和实时部署。

Conclusion: 从1D CCT转向2D色度空间结合MLP方法，能有效提高非普朗克光源下的色度精度，且计算成本可忽略不计。
...
TL;DR: 本文提出了MuM模型，通过将MAE扩展到任意多视角图像，使用统一掩码和轻量级解码器，在3D视觉任务中超越了DINOv3和CroCo v2。


<details>
  <summary>Details</summary>
Motivation: 现有的自监督学习方法主要针对语义理解而非几何推理，CroCo虽然针对3D理解但存在局限性，需要更简单可扩展的方法来学习3D视觉特征。

Method: 扩展MAE到任意多视角图像，对所有视图进行统一掩码，使用轻量级解码器和帧间注意力机制。

Result: 在包括前馈重建、密集图像匹配和相对姿态估计等下游任务中，MuM模型超越了最先进的视觉编码器DINOv3和CroCo v2。

Conclusion: MuM方法比CroCo更简单且更具可扩展性，能够有效学习适用于3D视觉任务的特征表示。
...
TL;DR: 提出了一个完整的多阶段优化框架，将高性能浮点图像压缩模型转换为高效的硬件友好型整数实现，包括动态范围感知量化、混合精度搜索和通道剪枝技术。
...
Method: 1) 动态范围感知量化(DRAQ)：使用统计校准的激活裁剪和权重正则化应对极端数据异常值；2) 渐进混合精度搜索算法：为每层分配最优非均匀位宽；3) 针对GDN层的通道剪枝方法：消除不活跃通道。

Result: DRAQ方法将GDN模型的BD-rate开销从30%降至6.3%；硬件感知优化进一步降低20%以上计算复杂度，对RD性能影响可忽略；最终模型在效率和性能上都优于现有FPGA实现。

Conclusion: 该框架成功实现了从浮点到整数的高保真转换，结合硬件感知优化，为FPGA部署提供了既高效又高质量的图像压缩解决方案。
...
TL;DR: 本文提出了一种利用生成模型创建多样化合成人脸图像的方法，并构建了包含27,780张图像、926个身份的DIF-V数据集，用于解决现有人脸验证系统中的偏见问题。
...
Method: 提出了一种综合方法，集成先进的生成模型来创建多样化的高质量合成人脸图像，确保符合身份证照片的特征要求。
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散基真实世界图像超分辨率方法在保真度和可控性之间存在平衡问题：多步方法生成多样性导致保真度低，单步方法因保真度特定微调而失去控制灵活性。

Method: 提出噪声混合视觉流设计：新视觉流接收带可调噪声的低质量图像，原始视觉流接收带一致噪声的低质量图像；采用保真度感知对抗训练实现单步推理。

Result: 在通用真实世界图像超分辨率上达到SOTA性能，并在中文场景文本图像超分辨率等挑战性场景中实现提示可控性，无需特定数据集训练。
...
Method: 提出基于CC-Metrics框架的CC-DiceCE损失函数，并与现有的blob loss进行比较。在nnU-Net框架中进行基准测试，使用DiceCE作为基线。

Result: CC-DiceCE损失提高了检测召回率，分割性能几乎没有下降，但会略微增加假阳性。多数据集研究表明CC-DiceCE通常优于blob loss。

Conclusion: CC-DiceCE损失函数能有效解决小病灶分割不足的问题，在保持分割性能的同时提高检测能力，是优于现有方法的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的生成式方法（如扩散模型）通常基于人工设计的轨迹锚点或随机噪声进行去噪，存在改进空间。本文旨在通过结合判别式轨迹提议来增强生成式轨迹预测的性能。

Method: 提出DiffRefiner两阶段框架：第一阶段使用基于Transformer的提议解码器从传感器输入回归生成粗略轨迹预测；第二阶段应用扩散精炼器迭代去噪和精炼这些初始预测，并设计了细粒度去噪解码器增强场景合规性。

Result: 在NAVSIM v2上达到87.4 EPDMS，在Bench2Drive上达到87.1 DS和71.4 SR，在两个公共基准测试中都创造了新记录。

Conclusion: DiffRefiner通过结合判别式轨迹提议和生成式精炼，显著提升了扩散基规划的轨迹预测性能，每个组件的有效性都通过消融研究得到验证。
...
<details>
  <summary>Details</summary>
Motivation: 超声图像在不同采集设备间存在外观差异，导致域偏移问题，影响下游推理模型的性能。现有无配对图像翻译方法在域适应过程中往往忽视类别特定的语义对齐，导致内容-类别映射错误，影响诊断准确性。

Method: 提出UI-Styler框架，包含模式匹配机制将目标图像纹理转移到源图像，同时保持源结构内容；引入基于目标域伪标签的类别感知提示策略，确保与诊断类别的准确语义对齐。
...
TL;DR: 本文介绍了RSFM数据库和REMSA智能体，用于解决遥感基础模型选择困难的问题。RS-FMD包含150多个模型，REMSA是基于LLM的智能体，能通过自然语言查询自动选择模型，并在专家验证的基准测试中表现优异。
...
Method: 1. 构建RSFM数据库(RS-FMD)，覆盖150多个模型的多模态数据、分辨率和学习范式；2. 开发REMSA智能体，使用LLM解释用户需求、解决约束缺失、通过上下文学习排序候选模型；3. 建立75个专家验证查询场景的基准测试。

Result: REMSA在900个配置的基准测试中优于多个基线方法（朴素智能体、密集检索、非结构化RAG-based LLM），完全基于公开元数据运行，不访问私有或敏感数据。

Conclusion: RS-FMD和REMSA为遥感基础模型选择提供了有效的结构化资源和自动化工具，显著简化了模型选择过程，在专家评估中表现优异。
...
TL;DR: FireScope是一个基于视觉语言模型的推理生成框架，通过结合Sentinel-2卫星图像和气候数据预测野火风险图，并在跨大陆评估中表现出优异的泛化能力。
...
Method: 提出FireScope-Bench大规模数据集和基准，结合Sentinel-2图像、气候数据和专家定义的风险栅格。FireScope框架采用VLM基础，通过强化学习和视觉监督学习预测风险栅格并生成补充推理轨迹。
...
TL;DR: 本文系统评估了自监督表征在音频-视觉深度伪造检测中的潜力，发现这些特征能捕捉到与深度伪造相关的信息且具有互补性，但无法可靠地跨数据集泛化。


<details>
  <summary>Details</summary>
Motivation: 探索自监督表征在音频-视觉深度伪造检测中的潜力，此前的研究要么单独使用这些特征，要么将其嵌入复杂架构中，缺乏系统性评估。

Method: 在多个模态（音频、视频、多模态）和领域（唇部运动、通用视觉内容）上系统评估自监督特征，重点关注检测效果、编码信息的可解释性和跨模态互补性三个维度。

Result: 大多数自监督特征能捕捉深度伪造相关信息且具有互补性，模型主要关注语义上有意义的区域而非虚假伪影，但无一模型能可靠地跨数据集泛化。

Conclusion: 自监督表征在深度伪造检测中具有前景，能学习有意义的模式，但实现稳健的跨域性能仍然困难，泛化失败可能源于数据集特性而非特征本身对表面模式的依赖。
...
TL;DR: VLA-4D模型通过4D感知的视觉表示和时空动作表示，实现了机器人操作的时空一致性控制


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言-动作模型在时空一致性操作方面存在挑战，难以实现动作执行的时间连贯性控制

Method: 提出4D感知视觉表示（将1D时间嵌入3D位置形成4D嵌入）和时空动作表示（扩展传统空间动作表示加入时间信息），通过交叉注意力机制融合多模态表示

Result: 在多个机器人操作任务中验证了方法的优越性，实现了空间平滑和时间连贯的机器人操作

Conclusion: VLA-4D模型通过统一的4D感知框架有效解决了机器人操作中的时空一致性问题
...
TL;DR: 本文提出了一种轻量级的对齐层模块和持续学习策略CA-SAM，用于在医疗图像分割中平衡SAM模型的计算效率和性能，解决异构隐私政策下的联合训练问题。


<details>
  <summary>Details</summary>
Motivation: 医疗图像分割中，机构间的异构隐私政策使得在汇集数据集上进行联合训练不可行，需要从数据流中进行持续学习而避免灾难性遗忘。虽然SAM模型提供了强大的零样本先验，但其大量参数和计算开销限制了实际部署。

Method: 提出了对齐层（Alignment Layer）作为轻量级即插即用模块，通过对齐编码器-解码器特征分布来高效适配SAM到特定医疗图像。基于此，提出了CA-SAM持续学习策略，自动适配适当的对齐层来缓解灾难性遗忘，同时利用SAM的零样本先验保持对未见医疗数据集的强性能。

Result: 在持续学习场景下的九个医疗分割数据集上进行实验，CA-SAM实现了最先进的性能。

Conclusion: SAM范式在平衡计算效率和性能方面具有很大潜力，CA-SAM通过轻量级对齐层和持续学习策略有效解决了医疗图像分割中的实际问题。
...
TL;DR: SING3R-SLAM是一个基于高斯表示的密集RGB SLAM框架，通过结合局部一致3D重建与统一全局高斯表示，解决SLAM中的漂移和冗余点云问题，实现高效多用途3D建图。


<details>
  <summary>Details</summary>
Motivation: 现有密集3D重建方法能准确捕捉局部几何，但集成到SLAM中面临漂移和冗余点云问题，限制了效率和下游任务（如新视角合成）的性能。

Method: 首先通过轻量级跟踪重建模块构建局部一致子图，然后逐步对齐融合到全局高斯地图中，强制执行跨视角几何一致性，全局地图反馈纠正局部漂移增强跟踪鲁棒性。

Result: 在真实世界数据集上，SING3R-SLAM在跟踪、3D重建和新视角渲染方面达到最先进水平，跟踪性能提升超过12%，生成更精细详细的几何结构，同时保持紧凑内存高效的全局表示。

Conclusion: SING3R-SLAM通过局部与全局高斯表示的协同优化，有效解决了SLAM中的漂移和冗余问题，为多种下游应用提供了高效且多功能的3D建图解决方案。
...
Method: 结合局部Transformer进行高分辨率体素特征提取和全局Transformer进行全扫描上下文建模，使用DINO风格自蒸馏和基于SigLIP的视觉-语言对齐进行预训练。

Result: 在多个CT基准测试中，SPECTRE在零样本和微调设置下均优于现有的CT基础模型，证明了其可扩展性和通用性。

Conclusion: SPECTRE是一个可扩展、开源、完全基于Transformer的3D医学影像基础模型，仅使用公开CT数据集就能实现高性能和泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 鱼眼图像存在极端非线性失真、遮挡和深度模糊等问题，使得BEV语义分割具有挑战性。
...
Conclusion: 该框架能够有效处理严重鱼眼失真和多样化环境条件，生成连续、不确定性感知的语义地图。
...
TL;DR: QueryOcc是一个基于查询的自监督框架，通过4D时空查询直接从图像学习连续3D语义占据，无需大规模3D标注，在自监督Occ3D-nuScenes基准上比之前方法提升26%语义RayIoU。


<details>
  <summary>Details</summary>
Motivation: 由于大规模3D标注成本过高，需要探索直接从传感器数据自监督学习3D场景几何和语义的方法，现有方法要么依赖2D渲染一致性，要么使用离散体素网格，限制了空间精度和可扩展性。
...
TL;DR: 本文提出了一种基于四元数保真度项的颜色图像盲反卷积方法，通过四元数卷积核同时处理RGB通道及其相互关系，显著提升了颜色图像的去模糊效果。
...
Method: 设计了一种新颖的四元数保真度项，利用四元数卷积核的特性，包含四个核：一个类似非负卷积核捕捉整体模糊，另外三个无约束核分别对应RGB通道以建模其未知的相互依赖关系，并使用归一化四元数核来保持图像强度。

Result: 在真实模糊彩色图像数据集上的大量实验表明，该方法有效消除了伪影，显著改善了去模糊效果。

Conclusion: 该方法展示了作为彩色图像反卷积强大工具的潜力，能够有效处理颜色通道间的复杂关系。
...
<details>
  <summary>Details</summary>
Motivation: 现有OCSR系统在准确识别立体化学信息方面面临重大挑战，因为区分立体异构体的视觉线索（如楔形键、虚线键、环构象等）非常细微。
...
TL;DR: BiFingerPose是一种基于双模态的指尖姿态估计算法，通过结合电容图像和指纹补丁，能够同时准确预测全面的指尖姿态信息，包括之前无法估计的滚转角，并在连续和离散交互任务中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于电容图像的指尖姿态估计算法在便携设备上只能估计俯仰角和偏航角，且在大角度输入（特别是大于45度）时精度下降，无法估计滚转角，限制了人机交互能力。

Method: 提出双模态输入方法，结合来自触摸屏的电容图像和屏下指纹传感器获取的指纹补丁，构建BiFingerPose算法来同时预测全面的指尖姿态参数。

Result: BiFingerPose相比之前SOTA方法在预测性能上提升超过21%，任务完成效率提高2.5倍，用户操作准确率提升23%，能够可靠估计滚转角并显著改善其他姿态参数的预测性能。

Conclusion: 双模态方法显著提升了指尖姿态估计能力，在增强认证安全性和改善交互体验方面具有广阔应用前景，开发的相应原型展示了其交互潜力。
...
TL;DR: 本文提出SpatialGeo方法，通过融合几何和语义特征的分层视觉编码器，增强多模态大语言模型的空间推理能力，在空间推理任务中提升准确率8.0%以上，同时减少50%内存开销。


<details>
  <summary>Details</summary>
Motivation: 大多数多模态大语言模型在三维空间的空间推理能力有限，主要原因是现有视觉编码器（如CLIP）仅关注实例级语义特征，导致空间信息丢失。

Method: 提出基于几何和语义特征分层融合的视觉编码器，通过分层适配器将自监督学习的几何特征补充到CLIP中，使用随机特征丢弃避免仅依赖CLIP编码器的平凡解。

Result: 实验结果显示SpatialGeo在空间推理任务中准确率提升至少8.0%，推理时内存成本减少约50%。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法如NoPe-NeRF仅关注图像局部关系，在复杂场景中难以恢复准确的相机姿态，需要更有效的局部到全局优化策略。

Method: 采用相对姿态初始化与显式特征匹配，进行局部联合优化以增强姿态估计，然后通过包含特征轨迹的束调整进行全局优化，结合几何一致性约束。
...
TL;DR: 本文提出了一种在生成图像中准确渲染折射透明物体的方法，通过同步物体边界内外像素并使用斯涅尔折射定律进行扭曲和融合，显著改善了生成图像的光学合理性。
...
Method: 在生成过程的每一步，使用斯涅尔折射定律对物体边界内外像素进行扭曲和融合；对于通过折射或反射可见但未直接观察到的表面，通过将图像与以物体为中心的全景图同步来恢复其外观。

Result: 该方法生成了更符合光学物理约束的图像，显著提高了折射效果的视觉合理性。

Conclusion: 所提出的方法能够生成光学上更合理的图像，有效解决了生成模型中透明物体折射渲染的挑战。
...
Method: 将多种媒介嵌入扩散模型的条件空间，使用跨媒介风格增强和反绘画训练策略，确保平滑且符合人类习惯的生成过程。

Result: 构建了大规模真实绘画过程数据集，在LPIPS、DINO和CLIP指标上取得了良好结果，实现了跨媒介一致性、时间连贯性和最终图像保真度。

Conclusion: 提出的感知距离剖面（PDP）曲线能够定量建模创作序列（构图、色彩块面、细节精化），反映了人类艺术创作进程。
...
TL;DR: DSeq-JEPA是一种改进的自监督视觉表示学习方法，通过引入判别性顺序预测机制，结合JEPA的潜在嵌入预测和GPT风格的顺序推理，从主要判别区域到次要区域进行渐进式预测。


<details>
  <summary>Details</summary>
Motivation: 现有I-JEPA方法对所有区域进行统一独立处理，缺乏对预测位置和顺序的显式建模。受人类视觉感知启发，需要选择性、顺序性地从最信息丰富区域到次要区域进行注意力部署。

Method: DSeq-JEPA首先基于transformer生成的显著性图识别主要判别区域，然后按此判别顺序预测后续区域，形成从主要到次要线索的课程式语义进展。

Result: 在图像分类、细粒度视觉分类、检测分割以及低级推理任务上的广泛实验表明，DSeq-JEPA比I-JEPA变体始终关注更具判别性和泛化性的表示。

Conclusion: DSeq-JEPA成功桥接了预测性和自回归自监督学习，通过整合JEPA风格潜在预测和GPT风格顺序推理，实现了更有效的视觉表示学习。
...
TL;DR: SuperQuadricOcc是一种基于超二次曲面的语义占据估计方法，相比高斯表示方法，在Occ3D数据集上实现了75%内存占用减少、124%推理速度提升和5.9% mIoU改进，是首个支持实时推理的占据模型。


<details>
  <summary>Details</summary>
Motivation: 高斯表示在自监督占据估计中广泛使用，但大量高斯基元会显著增加内存需求，不适合实时推理。超二次曲面允许减少基元数量和降低内存需求，但缺乏超二次曲面光栅化器来实现模型监督。

Method: 提出SuperQuadricOcc方法，采用基于超二次曲面的场景表示。通过利用多层二十面体细分的高斯近似超二次曲面，实现在训练期间使用高斯光栅化进行监督。

Result: 在Occ3D数据集上，相比之前的高斯方法，实现了75%内存占用减少、124%推理速度提升和5.9% mIoU改进，无需使用时间标签。超二次曲面将场景建模所需的基元数量减少了84%。

Conclusion: 这是首个能够实现实时推理同时保持竞争力的占据模型，证明了超二次曲面在语义占据估计中的有效性。代码将作为开源发布。
...
TL;DR: 提出ATAC方法，通过在CLIP嵌入空间计算增强诱导的漂移向量来推断语义恢复方向，显著提升对抗鲁棒性，在多种基准测试中平均超越之前最优方法近50%。


<details>
  <summary>Details</summary>
Motivation: 尽管CLIP在零样本图像-文本匹配方面表现优异，但对图像对抗扰动高度脆弱。由于对抗微调成本过高，现有测试时防御方法鲁棒性有限。

Method: ATAC直接在CLIP嵌入空间操作，计算增强诱导的漂移向量来推断语义恢复方向，基于这些潜在漂移的角度一致性来校正嵌入。

Result: 在广泛基准测试中，ATAC始终实现极高的鲁棒性，平均超越之前最优方法近50%，同时计算开销极小。在非常规和极端设置下仍保持最优鲁棒性，甚至对自适应攻击也展现出显著鲁棒性。

Conclusion: ATAC是在CLIP嵌入空间进行测试时对抗防御的新范式中的高效方法。
...
TL;DR: SVRecon将稀疏体素栅格化范式扩展到高保真表面重建任务，通过集成符号距离函数来解决稀疏体素优化中的局部最小值问题，实现了准确且快速收敛的重建效果。
...
Method: 通过(1)使用视觉几何模型进行鲁棒几何初始化，(2)实施空间平滑损失来强制父-子和兄弟体素组之间的相干关系，促进连贯平滑的体素结构。

Result: 在各种基准测试上的广泛实验表明，该方法实现了强大的重建精度，同时具有持续快速的收敛速度。

Conclusion: SVRecon方法成功解决了稀疏体素优化中的平滑性问题，实现了高精度且快速收敛的表面重建，代码将公开。
...
TL;DR: 该论文提出了非参数概率鲁棒性(NPPR)作为更实用的鲁棒性度量，不依赖预定义的扰动分布，直接从数据中学习优化的扰动分布，在分布不确定性下实现保守的鲁棒性评估。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型对小的输入扰动很脆弱，现有的概率鲁棒性(PR)方法假设固定已知的扰动分布，这在实践中不现实。

Method: 提出NPPR度量，基于高斯混合模型(GMM)与多层感知机(MLP)头及双三次上采样开发NPPR估计器，覆盖输入相关和输入独立的扰动场景。

Result: 在CIFAR-10、CIFAR-100和Tiny ImageNet数据集上的实验验证NPPR作为更实用的鲁棒性度量，相比现有方法显示出高达40%更保守的PR估计。

Conclusion: NPPR是一个更实用的鲁棒性度量，能够有效处理分布不确定性，为深度学习模型的鲁棒性评估提供了更好的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有强化学习框架通常将高维变形空间投影到粗糙的低维表示中，限制了捕捉空间变化变形的能力。需要一种能够高效探索和精细化优化的方法来解决医学图像配准中的这一根本挑战。

Method: 在编码器顶部引入随机高斯策略头来建模潜在特征的分布，实现高效探索和从粗到细的精细化。框架集成了无监督预热和弱监督微调，通过多轨迹采样稳定训练并提高标签效率。
...
TL;DR: 本文提出了一种名为MCMoE的新型多模态动作质量评估框架，通过混合专家系统统一单模态和联合表示学习，能够有效处理推理阶段部分模态缺失的问题。


<details>
  <summary>Details</summary>
Motivation: 现实应用中多模态数据在推理阶段经常存在部分模态缺失，导致现有多模态模型无法运行，且模态缺失会引发跨模态交互中断导致的性能灾难性下降。

Method: 提出自适应门控模态生成器动态融合可用信息重建缺失模态；设计模态专家学习单模态知识并动态混合所有专家知识提取跨模态联合表示；通过混合专家进一步细化和补充缺失模态；在训练阶段挖掘完整多模态特征和单模态专家知识来指导模态生成。

Result: 在三个公共AQA基准测试中，MCMoE在完整和不完整多模态学习方面均取得了最先进的结果。

Conclusion: MCMoE框架有效解决了多模态动作质量评估中模态缺失的问题，实现了在完整和不完整模态情况下的高性能评估。
...
TL;DR: 提出了MMT-ARD多模态多教师对抗鲁棒蒸馏框架，通过双教师知识融合架构提升视觉语言模型的对抗鲁棒性，在ImageNet上实现鲁棒准确率提升4.32%，零样本准确率提升3.5%，训练效率提升2.3倍。
...
TL;DR: CWMDT框架将标准视频扩散模型转化为反事实世界模型，通过构建场景的数字孪生、使用大语言模型进行推理，并生成反事实视觉序列，在基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 当前世界模型主要关注基于事实观测的前向模拟，但对于评估AI在不同条件下的行为等新兴应用，需要能够回答反事实查询的能力，如"如果移除这个对象会发生什么？"

Method: CWMDT框架包含三个步骤：1）构建观察场景的数字孪生，以结构化文本形式编码对象及其关系；2）使用大语言模型推理这些表示，预测反事实干预如何随时间传播改变观察场景；3）用修改后的表示条件化视频扩散模型，生成反事实视觉序列。
...
TL;DR: Radar2Shape是一个去噪扩散模型，通过将雷达信号频率与多分辨率形状特征相关联，从部分可观测的雷达信号进行3D重建。该方法采用两阶段方法：首先学习具有层次分辨率形状特征的规范化潜在空间，然后通过以类似从粗到细的方式调节雷达信号频率来扩散到该潜在空间中。


<details>
  <summary>Details</summary>
Motivation: 从高频雷达信号确定3D物体形状在商业和航空航天应用中至关重要，但分析复杂。现有深度学习方法难以表示任意形状或处理有限视角收集的真实世界雷达信号。光学3D重建方法可以生成任意形状，但将雷达信号简单视为相机视图时会遇到困难。

Method: 提出Radar2Shape两阶段方法：1) 学习具有层次分辨率形状特征的规范化潜在空间；2) 通过以从粗到细的方式调节雷达信号频率，扩散到该潜在空间中。该方法将雷达信号频率与多分辨率形状特征相关联。

Result: Radar2Shape能够成功地从部分观测的雷达信号重建任意3D形状，并在两种不同仿真方法和真实世界数据上展现出鲁棒的泛化能力。作者还发布了两个合成基准数据集以促进未来研究。

Conclusion: Radar2Shape通过将雷达信号频率与多分辨率形状特征相关联，有效解决了从部分可观测雷达信号进行3D重建的挑战，为将此类模型安全适应到真实世界雷达系统奠定了基础。
...
TL;DR: 提出了一种基于计算机视觉的深度学习方法，使用超过18,000个MRI序列图像来估计脊柱年龄，并研究脊柱年龄差（SAG）与脊柱退行性疾病和生活方式因素的关联。


<details>
  <summary>Details</summary>
Motivation: 脊柱是复杂的结构，容易发生与年龄相关的退行性变化，这些变化可以通过MRI识别。需要开发准确的方法来评估脊柱健康状况。

Method: 使用UMAP和HDBSCAN识别年龄相关的脊柱退行性疾病聚类，构建深度学习模型进行脊柱年龄估计，通过消融研究确定最佳模型配置。

Result: 发现脊柱年龄差（SAG）与椎间盘突出、骨赘、椎管狭窄、骨折等脊柱疾病相关，也与吸烟和体力劳动等生活方式因素相关。

Conclusion: 脊柱年龄差（SAG）可以作为衡量整体脊柱健康的有用生物标志物，具有临床实用性。
...
TL;DR: 本文分析了多模态模型中LLM规模缩减对视觉能力的影响，发现视觉能力下降比推理能力更严重，提出了Extract+Think方法来解决这一瓶颈。


<details>
  <summary>Details</summary>
Motivation: 实际应用需要更小、更高效的多模态系统，但LLM规模缩减会不成比例地影响视觉能力而非推理能力，需要解决这一瓶颈。

Method: 提出视觉提取调优方法，明确训练模型提取与指令相关的视觉细节，然后应用逐步推理生成答案，形成Extract+Think方法。

Result: LLM规模缩减导致视觉能力显著下降，Extract+Think方法在效率和性能方面为该领域设立了新标准。

Conclusion: Extract+Think方法通过分离视觉提取和推理步骤，有效解决了LLM规模缩减对多模态模型视觉能力的负面影响。
...
TL;DR: Video-R4是一个视频推理LMM，通过视觉反刍机制迭代选择帧、放大关键区域、重新编码像素并更新推理状态，在文本丰富的视频理解任务中实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频QA模型依赖单次感知固定帧，导致幻觉和细粒度证据识别失败。受人类暂停、放大和重读关键区域的行为启发，需要开发迭代推理方法。

Method: 提出视觉反刍机制：迭代选择帧、放大信息区域、重新编码检索像素、更新推理状态。构建两个数据集，采用多阶段反刍学习框架，通过SFT和GRPO-based RL逐步微调7B LMM。

Result: Video-R4-7B在M4-ViteVQA上达到最先进结果，并泛化到多页文档QA、幻灯片QA和通用视频QA任务。
...
TL;DR: 提出了一种新颖的原生3D编辑框架，通过单次前向传播直接操作3D表示，解决了现有方法在速度、几何一致性和视觉质量方面的问题。
...
Result: 令牌拼接方法参数效率更高且性能更优；在生成质量、3D一致性和指令保真度方面优于现有的2D提升方法，设定了新的基准。

Conclusion: 提出的原生3D编辑框架通过直接操作3D表示，在效率和编辑质量方面显著优于现有方法，为3D内容创作提供了更有效的解决方案。
...
TL;DR: 本文提出了语义金字塔索引(SPI)，一种用于向量数据库的多分辨率索引框架，通过查询自适应分辨率控制来解决RAG系统中检索速度与语义相关性之间的权衡问题。
...
Method: 开发Bench360框架，允许用户定义自定义任务、数据集和相关指标，自动基准测试选定的LLM、推理引擎和量化级别，涵盖单流、批处理和服务器等使用场景。

Result: 在四个常见LLM任务、三个硬件平台和四个推理引擎上的测试结果表明，任务性能与系统级效率之间存在多个有趣的权衡，不同推理引擎和模型存在显著差异。
...
TL;DR: 本研究评估了大型语言模型对突尼斯阿拉伯语的理解能力，创建了包含突尼斯方言、标准阿拉伯语和英语的平行数据集，并在转写、翻译和情感分析三个任务上测试了多个模型，揭示了模型在处理低资源语言方面的局限性。
...
Method: 构建包含平行翻译和情感标签的新数据集，在转写、翻译和情感分析三个任务上对多个流行LLM进行基准测试。
...
TL;DR: 提出EliDecide方法，通过椭圆决策边界解决文本开放意图分类问题，相比传统球形边界能更好地适应不同特征方向的分布方差，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有自适应决策边界方法假设已知类呈各向同性分布，将边界限制为球形，忽略了不同特征方向的分布方差，限制了开放意图检测能力。

Method: 1) 使用监督对比学习获得已知样本的判别特征空间；2) 应用可学习矩阵参数化椭圆作为每个已知类的边界；3) 通过双损失函数优化边界，平衡经验风险和开放空间风险。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在需要与人类价值观对齐的应用中越来越重要，但模型微调是静态的，无法适应涉及动态价值观和偏好的日常情境。

Method: 开发了一个实用、可重现、模型无关的评估程序，使用Schwartz基本人类价值观理论和对话数据集进行结构化评估，比较基线提示和明确基于价值观的提示。

Result: 实验表明，即使不改变模型或动态优化提示，价值观引导也是可能的，价值条件化提示能够有效引导生成文本朝向目标价值观。

Conclusion: 该方法提供了一种无需修改模型即可实现价值观引导的有效途径，为大语言模型在动态价值观环境中的应用提供了实用解决方案。
...
TL;DR: 该论文研究了AI检测模型对经过AI轻微润色的人类撰写文章的误判问题，特别是在阿拉伯语领域。通过创建两个数据集评估了14个大型语言模型和商业AI检测器的性能，发现轻微润色会导致检测器将人类文章错误归类为AI生成。


<details>
  <summary>Details</summary>
Motivation: 当前AI检测模型在面对经过AI轻微润色的人类文章时会出现误判，可能导致错误指控作者使用AI抄袭，损害AI检测器的可信度。英文领域已有相关研究，但阿拉伯语领域尚属空白。

Method: 生成了两个阿拉伯语数据集：第一个包含800篇文章（一半AI生成，一半人类撰写），用于评估14个LLM和商业AI检测器的区分能力；第二个数据集Ar-APT包含400篇人类文章，由10个LLM使用4种润色设置进行润色，共16400个样本，用于评估8个最佳模型对轻微润色文章的检测能力。

Result: 所有AI检测器都会错误地将大量文章归因于AI。最佳LLM Claude-4 Sonnet准确率为83.51%，但对LLaMA-3轻微润色的文章准确率降至57.63%。最佳商业模型originality.AI准确率为92%，但对Mistral或Gemma-3轻微润色的文章准确率骤降至12%。

Conclusion: AI检测模型对经过轻微润色的人类文章存在严重误判问题，这威胁到AI检测器的可信度和作者的声誉，特别是在阿拉伯语领域需要开发更鲁棒的检测方法。
...
TL;DR: 本文复现了基于最近邻的测试时训练方法，通过检索相似序列并应用梯度更新来提升语言模型在推理时的性能，验证了该方法在降低困惑度和比特率方面的有效性。


<details>
  <summary>Details</summary>
Motivation: 验证测试时训练在最近邻检索上的核心主张，探索该方法在不同规模语言模型和多样化数据集上的通用性和鲁棒性。

Method: 使用预训练的RoBERTa嵌入和Faiss索引检索每个测试输入的20个最近邻，对GPT-2、GPT-Neo和R1-Distilled-Qwen2.5-1.5B模型应用每个邻居的梯度更新，并引入内存高效的检索实现。

Result: 测试时训练显著降低了在The Pile多样化领域上的困惑度和比特率指标，在GitHub和EuroParl等结构化数据集上改进最大；未在The Pile上预训练的模型获益更多，使较小模型能接近较大模型的性能。

Conclusion: 最近邻测试时训练具有鲁棒性和通用性，即使对于现代推理优化架构也能带来一致增益，同时强调了大规模检索增强适应复现的实际考虑。
...
TL;DR: 本文通过系统分析多语言大语言模型内部表示空间中语言信息的结构特征，发现语言信息在第一个Transformer块中急剧分离，并在整个模型深度中保持线性可分性，且语言方向与词汇嵌入的对齐与训练数据的语言组成密切相关。


<details>
  <summary>Details</summary>
Motivation: 尽管多语言LLMs在多种语言上表现出强大性能，但对其内部表示空间中语言信息的结构特征以及跨层涌现机制缺乏系统分析，需要量化语言编码的层间动态和几何结构。

Method: 对六个多语言LLMs进行全面的探测研究，覆盖所有268个Transformer层，使用线性和非线性探针以及新的Token-Language Alignment分析来量化语言编码的层间动态和几何结构。
...
Conclusion: 多语言LLMs不是通过表面脚本特征来区分语言，而是通过训练语料库塑造的潜在表示结构来区分。这些发现为多语言表示学习中的数据组成策略和公平性提供了实用见解。
...
TL;DR: 本文提出了一种基于语言模型的SNOMED CT本体嵌入方法，用于解决词汇外查询的分层概念检索问题，在评估中优于现有基线方法。


<details>
  <summary>Details</summary>
Motivation: SNOMED CT作为生物医学本体，其知识检索因语言歧义、同义词和多义词等问题而具有挑战性，特别是在查询词汇不在本体词汇表内时更加困难。

Method: 采用基于语言模型的本体嵌入方法，构建词汇外查询的标注数据集，测试最直接子类和较少相关祖先的检索效果。

Result: 该方法在SNOMED CT上的评估中优于SBERT和两种词汇匹配基线方法，证明了其有效性。

Conclusion: 虽然针对SNOMED CT进行测试，但该方法具有通用性，可扩展到其他本体，作者已发布代码、工具和评估数据集。
...
TL;DR: 研究发现共情行为（牺牲任务效率满足人类需求）在LLM激活空间中表现为线性方向，通过对比提示在三个模型上测试检测和操控能力，发现检测准确率高但操控效果因模型而异。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型中是否存在可检测和操控的共情行为线性方向，并比较不同训练策略（安全训练、未审查）对共情编码和操控性的影响。

Method: 使用基于EIA基准的对比提示，在Phi-3-mini-4k、Qwen2.5-7B和Dolphin-Llama-3.1-8B三个模型上测试共情行为的检测和操控能力。

Result: 检测方面：所有模型在最优层AUROC达0.996-1.00；Phi-3探针与EIA行为得分强相关；跨模型探针一致性有限。操控方面：Qwen和Phi-3双向操控成功率达65.3%和61.7%；Dolphin仅支持亲共情操控（94.4%成功率），反共情操控会崩溃。

Conclusion: 检测-操控差距因模型而异，安全训练可能影响操控鲁棒性而非防止操控，Dolphin显示仅对共情增强具有鲁棒性，需要更多模型验证。
...
### [98] [Interpretable dimensions support an effect of agentivity and telicity on split intransitivity](https://arxiv.org/abs/2511.16824)
*Eva Neu,Brian Dillon,Katrin Erk*

Main category: cs.CL

TL;DR: 本文重新探讨了非宾格动词与施事性/终结性之间的关系，使用基于种子词的可解释维度分析，发现施事性与非作格动词、终结性与非宾格动词之间存在关联。


<details>
  <summary>Details</summary>
Motivation: 尽管传统观点认为描述施事性动作的动词更倾向于非作格句法，描述终结事件的动词更倾向于非宾格句法，但Kim等人(2024)的研究发现人类对施事性和终结性的评分并不能很好地预测不及物动词的句法行为。本文旨在重新检验这一关系。

Method: 使用基于种子词的可解释维度计算方法，这些种子词位于施事性和终结性尺度的对立两极，结合人类判断来评估不及物动词的语义属性。

Result: 研究结果支持非作格性/非宾格性与施事性/终结性之间的联系，表明使用可解释维度与人类判断相结合可以为难以在评分任务中评估的语义属性提供有价值的证据。

Conclusion: 可解释维度与人类判断的联合使用能够有效揭示不及物动词句法行为背后的语义基础，为施事性和终结性等难以直接评估的语义特征提供了新的研究途径。
...
### [99] [PEPPER: Perception-Guided Perturbation for Robust Backdoor Defense in Text-to-Image Diffusion Models](https://arxiv.org/abs/2511.16830)
...
Method: PEPPER通过将输入提示重写为语义上远离但视觉上相似的描述，同时添加不显眼的元素，来破坏输入提示中嵌入的触发器并稀释触发器标记的影响。

Result: 实验表明PEPPER对基于文本编码器的攻击特别有效，显著降低攻击成功率的同时保持生成质量。

Conclusion: PEPPER可以与现有防御方法配合使用，提供比任何单独方法更强和更通用的鲁棒性。
...
### [100] [ConCISE: A Reference-Free Conciseness Evaluation Metric for LLM-Generated Answers](https://arxiv.org/abs/2511.16846)
*Seyed Mohssen Ghafari,Ronny Kol,Juan C. Quiroz,Nella Luan,Monika Patial,Chanaka Rupasinghe,Herman Wandabwa,Luiz Pizzato*

Main category: cs.CL

TL;DR: 本文提出了一种无参考的评估指标，用于量化大型语言模型生成响应的简洁性，通过三个压缩比计算来识别冗余内容，无需人工标注。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型经常生成冗长、冗余的响应，这降低了清晰度和用户满意度，并增加了模型开发者的成本，特别是那些按输出token数量收费的专有模型。

Method: 该方法通过计算三个压缩比来量化非必要内容：i）原始响应与LLM抽象摘要之间的压缩比；ii）原始响应与LLM提取摘要之间的压缩比；iii）单词移除压缩，即LLM在保持意义的前提下尽可能移除非必要单词。

Result: 实验结果表明，所提出的指标能够有效识别LLM输出中的冗余内容，为对话AI系统中响应简洁性的自动评估提供了实用工具。

Conclusion: 该方法提供了一种无需人工标注的自动化评估方案，能够有效评估LLM生成响应的简洁性，有助于提高响应质量并降低相关成本。
...
### [101] [Improving Latent Reasoning in LLMs via Soft Concept Mixing](https://arxiv.org/abs/2511.16885)
*Kang Wang,Xiangyu Duan,Tianyi Du*

Main category: cs.CL

TL;DR: 本文提出软概念混合（SCM）训练方案，通过在训练中直接暴露软表示来弥合推理中的软概念与训练中的离散标记之间的差距，从而提高大语言模型的推理性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型通常通过生成离散标记进行推理，这可能限制其表达能力。软概念推理是一个有前景的方向，但模型是在离散标记上训练的，存在软概念推理与离散标记训练之间的差距。

Method: 提出软概念混合（SCM）训练方案：构建软概念向量（嵌入的概率加权平均），将其混合到模型的隐藏状态中，并使用强化学习优化整个潜在推理过程。

Result: 在五个推理基准测试上的实验表明，SCM提高了大语言模型的推理性能，同时保持了稳定的训练动态。

Conclusion: 软概念混合训练方案能有效弥合软概念推理与离散标记训练之间的差距，提升大语言模型的推理能力。
...
### [102] [Deep Improvement Supervision](https://arxiv.org/abs/2511.16886)
*Arip Asadulaev,Rayan Banerjee,Fakhri Karray,Martin Takac*

Main category: cs.CL

TL;DR: 本文提出了一种改进Tiny Recursive Models (TRMs)训练效率的新方法，通过将TRMs的潜在推理框架化为无分类器引导和隐式策略改进算法，显著减少了训练计算量。


<details>
  <summary>Details</summary>
Motivation: 研究如何以最小改动进一步提高TRMs等小型循环架构在复杂推理任务上的效率，特别是在Abstraction and Reasoning Corpus (ARC)上的表现。

Method: 提出新颖的训练方案，将TRMs的潜在推理框架化为无分类器引导和隐式策略改进算法，为每个训练循环提供目标。

Result: 方法显著提升训练效率，前向传播总次数减少18倍，无需停止机制，在仅0.8M参数下实现ARC-1上24%的准确率，优于大多数LLMs。

Conclusion: 所提出的训练方案能够显著提高TRMs的训练效率，同时保持与标准TRMs相当的质量，为高效推理模型的发展提供了新思路。
...
### [103] [Predicting the Formation of Induction Heads](https://arxiv.org/abs/2511.16893)
*Tatsuya Aoyama,Ethan Gotlieb Wilcox,Nathan Schneider*

Main category: cs.CL

TL;DR: 本研究探讨了训练数据的统计特性与归纳头形成的关系，发现批次大小和上下文长度的组合可以预测归纳头形成点，表面二元重复频率和可靠性强烈影响归纳头形成，并确定了这些值的精确帕累托边界。


<details>
  <summary>Details</summary>
Motivation: 虽然专门的注意力头（称为归纳头）被认为是现代语言模型卓越的上下文学习能力的基础，但其形成的精确特征仍不清楚。本研究旨在探究训练数据的统计特性与归纳头形成之间的关系。

Method: 研究分析了自然和合成数据的统计特性，通过结合批次大小和上下文长度的简单方程来预测归纳头形成点，并考察了表面二元重复频率和可靠性对归纳头形成的影响。

Result: 研究发现：(1) 批次大小和上下文长度的组合可以预测归纳头形成点；(2) 表面二元重复频率和可靠性强烈影响归纳头形成，并确定了精确的帕累托边界；(3) 当频率和可靠性高时，局部依赖足以形成归纳头，但当这些值低时，类别性和边际分布形状变得重要。
...
### [104] [ARQUSUMM: Argument-aware Quantitative Summarization of Online Conversations](https://arxiv.org/abs/2511.16985)
*An Quang Tang,Xiuzhen Zhang,Minh Ngoc Dinh,Zhuang Li*

Main category: cs.CL

TL;DR: 本文提出了一种新颖的论据感知量化摘要任务，旨在揭示对话中论点的声明-理由结构，并通过ARQUSUMM框架实现，该框架利用LLM少样本学习和论据结构感知聚类算法来生成高质量的量化摘要。


<details>
  <summary>Details</summary>
Motivation: 在线对话平台上的争议话题日益增多，需要不仅总结多样论点，还要揭示其理由和论证结构。现有对话摘要研究虽然考虑了句子间的论证关系，但未能深入揭示句子内部的论据结构。

Method: 提出ARQUSUMM框架：1）利用LLM少样本学习基于论证理论识别句子内的命题及其声明-理由关系；2）采用论据结构感知聚类算法聚合论点并量化其支持度。

Result: 实验表明ARQUSUMM在对话和量化摘要任务上优于现有模型，生成的摘要能更好地表示论据结构，具有更高的文本质量和量化准确性，对用户更有帮助。

Conclusion: ARQUSUMM成功解决了论据感知量化摘要任务，能够有效揭示对话中的深层论据结构，为争议话题的讨论提供更全面的摘要支持。
...
### [105] [Supervised Fine Tuning of Large Language Models for Domain Specific Knowledge Graph Construction:A Case Study on Hunan's Historical Celebrities](https://arxiv.org/abs/2511.17012)
*Junjie Hao,Chun Wang,Ying Qiao,Qiuyue Zuo,Qiya Song,Hua Ma,Xieping Gao*

Main category: cs.CL

TL;DR: 本研究提出了一种监督微调方法，用于增强湖南历史名人领域的特定领域信息抽取，通过设计细粒度的模式引导指令模板和构建指令调优数据集，在四个大语言模型上进行参数高效的指令微调，显著提升了信息抽取性能。


<details>
  <summary>Details</summary>
Motivation: 解决湖南历史名人系统数据资源有限、通用模型在低资源领域知识抽取和结构化输出生成方面表现不佳的问题，推动历史文化研究中的文化遗产知识抽取和知识图谱构建。

Method: 设计面向湖南历史名人领域的细粒度模式引导指令模板，构建指令调优数据集，对Qwen2.5-7B、Qwen3-8B、DeepSeek-R1-Distill-Qwen-7B和Llama-3.1-8B-Instruct四个大语言模型进行参数高效的指令微调。

Result: 所有模型在微调后都表现出显著的性能提升，其中Qwen3-8B表现最佳，在100个样本和50次训练迭代下达到89.3866分。

Conclusion: 该研究为垂直大语言模型在区域历史文化领域的微调提供了新思路，突显了其在文化遗产知识抽取和知识图谱构建中具有成本效益的应用潜力。
...
### [106] [Do Vision-Language Models Understand Visual Persuasiveness?](https://arxiv.org/abs/2511.17036)
*Gyuwon Park*

Main category: cs.CL

TL;DR: 该研究构建了视觉说服力数据集，探索了视觉语言模型对视觉说服因素的理解能力，发现模型存在回忆偏向，高层次的语义对齐是最强预测因子，而基于对象的理性解释能显著提升模型性能。
...
Result: 实验分析显示VLMs存在回忆偏向（过度预测高说服力），对低/中层次特征的判别能力弱，高层次语义对齐是最强预测因子。简单指令或无引导推理效果有限，而基于对象的理性解释能显著提升精度和F1分数。
...
### [107] [Principled Design of Interpretable Automated Scoring for Large-Scale Educational Assessments](https://arxiv.org/abs/2511.17069)
*Yunsung Kim,Mike Hardy,Joseph Tey,Candace Thille,Chris Piech*

Main category: cs.CL

TL;DR: 本文提出了一个可解释的自动评分框架AnalyticScore，通过提取可识别元素、使用LLM进行特征化和应用有序逻辑回归模型，在保持高准确性的同时实现了可解释性。


<details>
  <summary>Details</summary>
Motivation: 当前AI驱动的自动评分系统缺乏透明度和可解释性，无法满足大规模真实评估的需求，需要开发一种广泛接受的可解释自动评分解决方案。

Method: 开发了AnalyticScore框架：1）提取响应中的可识别元素；2）使用LLM将每个响应转化为人类可解释的特征值；3）应用直观的有序逻辑回归模型进行评分。

Result: AnalyticScore在ASAP-SAS数据集的10个项目上，评分准确性优于许多不可解释方法，平均仅比不可解释的最优方法低0.06 QWK。其特征化行为与人类注释者高度一致。

Conclusion: 提出的FGTI原则和AnalyticScore框架为可解释自动评分提供了可行的解决方案，在保持高准确性的同时实现了透明度和可解释性，为未来研究奠定了基础。
...
### [108] [MUCH: A Multilingual Claim Hallucination Benchmark](https://arxiv.org/abs/2511.17081)
*Jérémie Dentan,Alexi Canesse,Davide Buscaldi,Aymen Shabou,Sonia Vanier*

Main category: cs.CL

TL;DR: MUCH是首个针对声明级不确定性量化的基准测试，包含4,873个样本，覆盖四种欧洲语言和四个指令调优的开源大语言模型，提供完整的生成对数概率数据，并提出高效的确定性分割算法。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型可靠性不足的问题，通过声明级不确定性量化来评估和改进模型的可信度。

Method: 构建包含多语言样本的基准数据集，提供24个生成对数概率数据，开发确定性分割算法仅需0.2%的生成时间，支持实时监控。

Result: 创建了MUCH基准测试，包含4,873个多语言样本，实现了高效的分割方法，评估显示当前方法在性能和效率方面仍有很大改进空间。

Conclusion: MUCH基准为声明级不确定性量化方法提供了公平可复现的评估框架，当前方法仍需在性能和效率方面进一步改进。
...
### [109] [Learning to Compress: Unlocking the Potential of Large Language Models for Text Representation](https://arxiv.org/abs/2511.17129)
*Yeqin Zhang,Yizheng Zhao,Chen Hu,Binxing Jiao,Daxin Jiang,Ruihang Miao,Cam-Tu Nguyen*

Main category: cs.CL

TL;DR: 本文探索了使用上下文压缩作为预训练任务来改进LLM文本表示的方法，提出的LLM2Comp模型在多种任务上优于现有基于LLM的文本编码器，且具有更高的样本效率。
...
Method: 提出使用上下文压缩作为预训练任务，让模型学习生成紧凑的记忆token来替代整个上下文进行下游序列预测。通过对比学习进一步优化表示质量。

Result: 实验表明，精心设计的压缩目标能显著提升LLM文本表示，优于基于token级预训练任务的模型。LLM2Comp在广泛任务上超越当代基于LLM的文本编码器，且训练数据需求显著减少。

Conclusion: 上下文压缩是一种有效的预训练任务，能够产生高质量的LLM文本表示，在性能和样本效率方面均优于现有方法。
...
### [110] [LangMark: A Multilingual Dataset for Automatic Post-Editing](https://arxiv.org/abs/2511.17153)
*Diego Velazquez,Mikaela Grace,Konstantinos Karageorgos,Lawrence Carin,Aaron Schliem,Dimitrios Zaikis,Roger Wechsler*

Main category: cs.CL

TL;DR: 本文介绍了LangMark，一个用于英语到七种语言翻译的多语言自动后编辑数据集，包含206,983个三元组（源文本、机器翻译输出、人工后编辑翻译），旨在解决缺乏针对NMT输出的大规模多语言数据集的问题。
...
Method: 构建并发布了LangMark数据集，包含英语到巴西葡萄牙语、法语、德语、意大利语、日语、俄语和西班牙语的206,983个人工注释三元组，由专业语言学家标注。

Result: 实证研究表明，使用少样本提示的大语言模型可以有效执行自动后编辑，改进领先的商业甚至专有机器翻译系统。
...
### [111] [The PLLuM Instruction Corpus](https://arxiv.org/abs/2511.17161)
...
TL;DR: 本文介绍了PLLuM项目中用于微调基于Transformer的大语言模型的指令数据集，提出了有机、转换和合成指令的功能分类，并发布了PLLuMIC数据集的首个代表性子集。


<details>
  <summary>Details</summary>
Motivation: 为PLLuM项目中的波兰语大语言模型开发提供指令数据集，并分享关于人工编写与合成指令数据集在语言适应方面的观察，为其他LLM的类似数据集开发提供指导。

Method: 使用有机、转换和合成指令构建指令数据集，对指令进行功能分类，并发布PLLuMIC数据集的代表性子集。

Result: 开发了用于微调PLLuM项目LLM的指令数据集，提出了指令的功能分类方法，并发布了PLLuMIC数据集的首个代表性子集。

Conclusion: PLLuMIC数据集为其他LLM的指令数据集开发提供了有价值的参考，同时分享了人工编写与合成指令在语言适应方面的经验观察。
...
### [112] [Hallucinate Less by Thinking More: Aspect-Based Causal Abstention for Large Language Models](https://arxiv.org/abs/2511.17170)
*Vy Nguyen,Ziqi Xu,Jeffrey Chan,Estrid He,Feng Xia,Xiuzhen Zhang*

Main category: cs.CL

TL;DR: 本文提出了基于方面的因果弃权(ABCA)框架，通过因果推理分析LLM内部知识多样性，实现早期弃权以防止幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 现有弃权方法依赖生成后信号，无法提前阻止不可靠响应。LLM从不同来源获取的知识具有多面性，这种多样性可用于评估知识可靠性。

Method: ABCA框架通过分析LLM参数知识中的方面多样性，估计条件因果效应来评估查询相关知识可靠性，支持两种弃权类型：方面效应不一致(知识冲突)和方面效应一致支持弃权(知识不足)。

Result: 在标准基准测试中，ABCA提高了弃权可靠性，达到了最先进的性能，并增强了弃权决策的可解释性。

Conclusion: ABCA框架通过利用LLM内部知识多样性实现早期弃权，有效防止幻觉问题，在可靠性和可解释性方面表现优异。
...
### [113] [Attention-Guided Feature Fusion (AGFF) Model for Integrating Statistical and Semantic Features in News Text Classification](https://arxiv.org/abs/2511.17184)
*Mohammad Zare*

Main category: cs.CL

TL;DR: 本文提出了一种注意力引导特征融合(AGFF)模型，将统计特征和语义特征结合在统一框架中，通过注意力机制动态确定各特征类型的重要性，在新闻分类任务中表现优于传统统计模型和纯语义深度学习模型。


<details>
  <summary>Details</summary>
Motivation: 传统统计方法虽能捕捉词级重要性但缺乏上下文语义理解，而现代深度学习方法虽能理解语义但可能忽略简单但高影响力的统计指标，因此需要结合两者的优势。
...
### [114] [E$^3$-Pruner: Towards Efficient, Economical, and Effective Layer Pruning for Large Language Models](https://arxiv.org/abs/2511.17205)
*Tao Yuan,Haoli Bai,Yinfei Pan,Xuyang Cao,Tianyu Zhang,Lu Hou,Ting Hu,Xianzhi Yu*

Main category: cs.CL

TL;DR: 本文提出了一种名为E3的层剪枝框架，通过可微分掩码优化和熵感知自适应知识蒸馏，解决了大语言模型层剪枝中的性能下降、训练成本高和加速有限等挑战。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型规模的增长，层剪枝作为一种硬件友好的模型压缩方法受到关注，但现有方法难以同时解决性能下降、高训练成本和有限加速等实际部署挑战。

Method: 提出E3框架，包含两个关键创新：(1) 使用Gumbel-TopK采样器的可微分掩码优化方法，实现高效精确的剪枝掩码搜索；(2) 熵感知自适应知识蒸馏策略，提升任务性能。

Result: 在多种模型架构和基准测试上的实验表明，该方法优于现有最先进方法。在Qwen3-32B模型上剪枝25%层时，在MATH-500上达到96%准确率，仅比原始模型(96.8%)下降0.8%，优于现有SOTA(95%)，推理速度提升1.33倍，仅消耗0.5B tokens(占后训练数据量的0.5%)。

Conclusion: E3框架能够有效解决大语言模型层剪枝中的关键部署挑战，在保持高性能的同时显著降低训练成本和提升推理效率。
...
### [115] [Parrot: Persuasion and Agreement Robustness Rating of Output Truth -- A Sycophancy Robustness Benchmark for LLMs](https://arxiv.org/abs/2511.17220)
*Yusuf Çelebi,Mahmoud El Hussieni,Özay Ezerceli*

Main category: cs.CL

TL;DR: PARROT框架用于评估大语言模型在社会压力下的准确性退化，通过权威性错误版本与中性问题的对比，量化模型在权威压力下的顺从行为（谄媚现象）。


<details>
  <summary>Details</summary>
Motivation: 研究动机是测量大语言模型在权威和说服压力下的准确性退化现象，特别是谄媚行为（过度顺从），以确保模型在真实世界部署中的安全性。

Method: 采用双盲评估方法，比较中性问题和权威性错误版本；使用基于对数似然的校准跟踪量化置信度变化；通过八状态行为分类法系统分类失败模式。

Result: 评估22个模型发现：先进模型（如GPT-5、GPT-4.1）跟随率低（≤11%）且准确性损失小，而较旧/较小模型出现严重认知崩溃（GPT-4:80%，Qwen 2.5-1.5B:94%）。国际法和全球知识领域脆弱性高，初等数学相对稳健。

Conclusion: 建议将'抵抗过度拟合压力'作为与准确性、伤害避免和隐私同等重要的主要目标，以确保大语言模型在真实世界中的安全部署。
...
### [116] [Lost in Translation and Noise: A Deep Dive into the Failure Modes of VLMs on Real-World Tables](https://arxiv.org/abs/2511.17238)
...
Method: 创建包含视觉噪声的多语言表格数据集，模拟扫描文档的真实场景，评估主流视觉语言模型在噪声和多语言条件下的表现。

Result: 评估显示领先模型在视觉噪声下性能下降超过35%，存在明显的英语优先偏见，在其他语言上的推理能力无法有效迁移。
...
### [117] [Estonian WinoGrande Dataset: Comparative Analysis of LLM Performance on Human and Machine Translation](https://arxiv.org/abs/2511.17290)
*Marii Ojastu,Hele-Andra Kuulmets,Aleksei Dorkin,Marika Borovikova,Dage Särg,Kairit Sirts*

Main category: cs.CL

TL;DR: 本文介绍了爱沙尼亚语版本的WinoGrande常识推理测试集的本地化翻译和评估，比较了人工翻译与机器翻译的效果，并探讨了提示工程在翻译质量提升方面的局限性。
...
Method: 方法包括：1) 由专业翻译人员进行人工翻译和本地化适配；2) 评估专有和开源模型在人工翻译基准上的表现；3) 设计针对爱沙尼亚语特点和WinoGrande数据集翻译挑战的详细提示；4) 比较人工翻译与机器翻译的效果。

Result: 结果显示：1) 模型在人工翻译的爱沙尼亚语数据集上表现略低于原始英语测试集；2) 机器翻译数据上的表现明显更差；3) 提示工程对翻译质量或模型准确性的提升有限。

Conclusion: 结论强调了语言专家参与数据集翻译和适配的重要性，以确保对大型语言模型语言能力和推理的可靠且可解释的评估。
...
### [118] [A new kid on the block: Distributional semantics predicts the word-specific tone signatures of monosyllabic words in conversational Taiwan Mandarin](https://arxiv.org/abs/2511.17337)
*Xiaoyun Jin,Mirjam Ernestus,R. Harald Baayen*

Main category: cs.CL

TL;DR: 该研究基于语料库分析了普通话单音节词在自然对话中的音高曲线实现，发现词义对音高轮廓有显著影响，即使控制多种变量后，词义仍是音高实现的重要预测因素。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索普通话中单音节词的音高曲线如何受到词义影响，挑战传统的声调理论，验证语义在语音实现中的重要性。

Method: 使用广义加性模型将观测到的音高曲线分解为与不同控制变量和语义预测因子相关的组成音高曲线，分析词义、词义感知、同音异形词等因素对音高轮廓的影响。

Result: 研究发现词义对音高实现有显著影响，词义感知比词本身是更好的预测因子，同音异形词具有不同的音高轮廓，且基于上下文嵌入可以准确预测单个词标记的音高曲线。

Conclusion: 语义在普通话声调实现中起着重要作用，这一发现挑战了标准普通话声调理论，但符合判别性词典模型的理论框架。
...
### [119] [Don't Learn, Ground: A Case for Natural Language Inference with Visual Grounding](https://arxiv.org/abs/2511.17358)
...
<details>
  <summary>Details</summary>
Motivation: 旨在开发一种不依赖任务特定微调的鲁棒自然语言推理方法，通过视觉模态作为意义表示来对抗文本偏见和表面启发式方法。
...
### [120] [Selective Rotary Position Embedding](https://arxiv.org/abs/2511.17388)
*Sajad Movahedi,Timur Carstensen,Arshia Afzal,Frank Hutter,Antonio Orvieto,Volkan Cevher*

Main category: cs.CL

TL;DR: 本文提出了选择性RoPE（Selective RoPE），一种输入依赖的旋转位置编码机制，可在线性和softmax变换器中实现任意角度的旋转，并展示了其在语言建模和序列任务中的性能提升。


<details>
  <summary>Details</summary>
Motivation: 受线性变换器中选择性门控机制在语言任务中表现优越的启发，作者希望将这种输入依赖的选择性机制引入到位置编码中，以改进传统的固定角度旋转位置嵌入（RoPE）。

Method: 提出了选择性RoPE机制，通过输入依赖的方式实现任意角度的旋转，适用于线性和softmax变换器。该方法揭示了softmax注意力中隐藏的旋转操作，并展示了状态空间模型和门控线性变换器中实部管理遗忘、虚部编码位置的结构。

Result: 在门控变换器中装备选择性RoPE后，在语言建模和困难序列任务（如复制、状态跟踪和检索）上表现出性能提升。

Conclusion: 选择性RoPE通过输入依赖的旋转角度机制，成功扩展了传统RoPE的功能，为位置编码提供了更灵活和有效的解决方案，在多种序列处理任务中展现出优越性能。
...
### [121] [PUCP-Metrix: A Comprehensive Open-Source Repository of Linguistic Metrics for Spanish](https://arxiv.org/abs/2511.17402)
*Javier Alonso Villegas Luis,Marco Antonio Sobrevilla Cabezudo*

Main category: cs.CL

TL;DR: PUCP-Metrix是一个包含182个语言指标的开源工具包，用于西班牙语文本分析，涵盖词汇多样性、句法语义复杂性、连贯性、心理语言学和可读性等多个维度。
...
Method: 开发了一个包含182个语言指标的开源存储库，这些指标覆盖了词汇多样性、句法和语义复杂性、连贯性、心理语言学和可读性等多个方面。
...
TL;DR: ReVeL框架将多项选择题重写为开放式问题，解决了MCQA中选项泄露信号导致评估不可靠的问题，在强化微调中展现出更好的数据效率和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 多项选择题的选项可能泄露可利用信号，导致准确性指标不可靠，并鼓励在强化微调中出现猜测行为，需要更可靠的评估方法。

Method: 提出ReVeL框架，将多项选择题分类并重写为开放式问题，保持答案可验证性，使用GRPO对Qwen2.5-VL模型进行微调。

Result: 在多项选择基准测试中匹配MCQA准确率，同时将OpenQA准确率提高约6个百分点，在评估中揭示了MCQA基准测试中高达20个百分点的分数膨胀。

Conclusion: ReVeL框架在强化微调和评估中均优于传统MCQA方法，提供更可靠的数据效率和奖励信号，减少成本和延迟。
...
TL;DR: 提出MR-RLVR方法，通过掩码填充和步骤重排序构建过程级自监督奖励，在仅结果可验证的数学推理任务中显著提升RLVR的性能。


<details>
  <summary>Details</summary>
Motivation: 传统RLVR在数学推理（特别是定理证明）中可扩展性有限，因为中间推理过程至关重要但最终答案难以直接可靠验证，而token级SFT容易退化为死记硬背而非诱导长链思维。

Method: 两阶段训练：首先在采样的数学计算和证明数据上进行自监督训练，然后在仅结果可验证的数学计算数据集上进行RLVR微调。使用掩码填充和步骤重排序构建过程级自监督奖励。
...
### [124] [δ-EMG: A Monotonic Graph Index for Approximate Nearest Neighbor Search](https://arxiv.org/abs/2511.16921)
*Liming Xiang,Jing Feng,Ziqi Yin,Zijian Li,Daihao Xue,Hongchao Qin,Ronghua Li,Guoren Wang*

Main category: cs.IR

TL;DR: 本文提出了一种误差有界的近似最近邻搜索方法，通过构建δ-EMG图结构，确保返回结果是对真实值的(1/δ)-近似，解决了传统ε-召回率有界方法无法保证错误结果偏差的问题。


<details>
  <summary>Details</summary>
Motivation: 传统近似最近邻搜索方法仅关注召回率，无法保证错误结果的偏差范围。本文旨在开发一种误差有界的ANN方法，提供对返回结果近似质量的严格保证。

Method: 采用基于图的框架，提出δ-EMG（误差有界单调图），通过施加δ-单调几何约束确保贪心搜索收敛到(1/δ)-近似邻居。进一步设计了误差有界top-k ANN搜索算法，并在查询时自适应控制近似精度。

Result: 在ANN-Benchmarks数据集上的实验表明，在召回率要求0.99时，该方法在SIFT1M数据集上达到19,000 QPS，比其他方法性能提升超过40%。

Conclusion: 提出的误差有界近似最近邻搜索框架不仅提供了理论保证，在实际性能上也显著优于现有方法，为高维空间检索系统提供了更可靠的解决方案。
...
### [125] [RASTP: Representation-Aware Semantic Token Pruning for Generative Recommendation with Semantic Identifiers](https://arxiv.org/abs/2511.16943)
*Tianyu Zhan,Kairui Fu,Zheqi Lv,Shengyu Zhang*

Main category: cs.IR

TL;DR: RASTP是一种表示感知的语义令牌剪枝方法，通过动态剪除输入序列中信息量较低的语义令牌，显著减少生成式推荐系统的计算复杂度和内存消耗。


<details>
  <summary>Details</summary>
Motivation: 生成式推荐系统使用语义标识符(SIDs)表示物品，但多个SIDs会显著增加输入序列长度，导致计算复杂度和内存消耗大幅上升。现有方法主要优化注意力计算和KV缓存，但作者提出直接剪枝输入序列中信息量较低的令牌。

Method: RASTP通过结合语义显著性（通过表示幅度测量）和注意力中心性（来自累积注意力权重）来评估令牌重要性，动态剪除低信息量或不相关的语义令牌。

Result: 在三个真实世界的Amazon数据集上的实验表明，RASTP将训练时间减少了26.7%，同时保持或略微提高了推荐性能。

Conclusion: RASTP通过直接剪枝输入序列中的低信息量语义令牌，有效降低了生成式推荐系统的计算开销，同时保持了推荐质量，为高效推荐系统提供了新的优化方向。
...
### [126] [CLLMRec: LLM-powered Cognitive-Aware Concept Recommendation via Semantic Alignment and Prerequisite Knowledge Distillation](https://arxiv.org/abs/2511.17041)
*Xiangrui Xiong,Yichuan Lu,Zifei Pan,Chang Sun*

Main category: cs.IR

TL;DR: CLLMRec是一个利用大语言模型进行MOOC概念推荐的创新框架，通过语义对齐和先验知识蒸馏技术，无需依赖结构化知识图谱即可实现认知感知的个性化推荐。


<details>
  <summary>Details</summary>
Motivation: 现有MOOC概念推荐方法依赖高质量结构化知识图谱，但在实际教育场景中这类资源稀缺，限制了推荐系统的应用效果。

Method: 提出CLLMRec框架，包含两个核心技术：语义对齐组件构建统一表示空间，先验知识蒸馏范式使用教师-学生架构从LLM中提取概念先决关系并蒸馏到高效排序器中。

Result: 在两个真实MOOC数据集上的实验表明，CLLMRec在多个评估指标上显著优于现有基线方法。

Conclusion: 该框架能够在不依赖显式结构先验的情况下，生成真正认知感知和个性化的概念推荐，验证了其有效性。
...
### [127] [Parametric Retrieval-Augmented Generation using Latent Routing of LoRA Adapters](https://arxiv.org/abs/2511.17044)
*Zhan Su,Fengran Mo,Jian-yun Nie*

Main category: cs.IR

TL;DR: 本文提出Poly-PRAG方法，通过潜在路由编码过程解决传统参数化检索增强生成(PRAG)中一对一文档编码方案的数据稀缺和推理开销问题。
...
Method: 采用潜在路由编码过程，将文档集编码视为多任务学习过程，使用少量潜在LoRA适配器编码整个文档空间，在线推理时根据查询选择性激活专家子集。

Result: 在多个知识密集型NLP任务上的综合评估显示，该方法在四个不同数据集上达到了最先进的结果。
...
### [128] [Stable diffusion models reveal a persisting human and AI gap in visual creativity](https://arxiv.org/abs/2511.16814)
...
### [129] [Cognitive BASIC: An In-Model Interpreted Reasoning Language for LLMs](https://arxiv.org/abs/2511.16837)
*Oliver Kramer*
...
<details>
  <summary>Details</summary>
Motivation: 受复古BASIC简单性的启发，重新利用编号行和简单命令作为可解释的认知控制层，使LLM能够可靠地模拟短程序，实现模型内部透明的多步推理。

Method: 使用自然语言解释器文件指定命令语义、内存更新和日志行为，通过心理模型解释器提取声明性和程序性知识，检测矛盾并在必要时生成解决方案。

Result: 在三个LLM上对知识提取、冲突检测和推理任务的基准测试表明，所有模型都能执行Cognitive BASIC程序，总体表现强劲但不统一。

Conclusion: Cognitive BASIC提供了一种结构化的方法来增强LLM的推理透明度和可解释性，使复杂的推理过程更加可控和可追踪。
...
### [130] [Fantastic Bugs and Where to Find Them in AI Benchmarks](https://arxiv.org/abs/2511.16842)
...
TL;DR: 提出了一种利用响应模式统计分析来识别无效基准问题的框架，通过专家审查和LLM初步筛选相结合，提高基准评估的可靠性。


<details>
  <summary>Details</summary>
Motivation: 基准测试对AI发展至关重要，但无效的基准问题会破坏评估可靠性。手动识别和修正数千个基准问题不可行，成为可靠评估的关键瓶颈。

Method: 基于AI评估中常用的核心假设——平均分足以概括模型性能，构建单维潜在结构。通过统计分析方法检测各项统计量是否超出预期范围来识别问题项目，并结合LLM法官进行初步审查。

Result: 在9个广泛使用的基准测试中，该方法指导专家审查识别问题问题的精度高达84%。LLM初步审查进一步减少了人工工作量。

Conclusion: 该框架为系统性的基准修订提供了高效且可扩展的解决方案，能够显著提高基准评估的可靠性。
...
### [131] [Hybrid Differential Reward: Combining Temporal Difference and Action Gradients for Efficient Multi-Agent Reinforcement Learning in Cooperative Driving](https://arxiv.org/abs/2511.16916)
*Ye Han,Lijun Zhang,Dejian Meng,Zhuang Zhang*

Main category: cs.AI

TL;DR: 本文提出了一种新颖的混合差分奖励机制来解决多车辆协同驾驶中传统状态奖励函数存在的奖励差异消失问题，通过整合时间差分奖励和动作梯度奖励，显著提高了策略梯度的信噪比和算法收敛性能。


<details>
  <summary>Details</summary>
Motivation: 在多车辆协同驾驶任务中，传统基于状态的奖励函数存在奖励差异消失问题，导致策略梯度的信噪比低，严重阻碍算法收敛和性能提升。
...
<details>
  <summary>Details</summary>
Motivation: 传统贝叶斯视角下的基于分数的扩散方法在解决逆问题时，先验提供的指导有限，重建主要受测量一致性项驱动，与扩散动力学实际上解耦。作者希望澄清这种结构并改进方法。

Method: 将扩散重新解释为期望最大化框架中的初始化阶段，引入DAPS++方法，使似然项更直接地指导推理，同时保持数值稳定性。

Result: DAPS++需要更少的函数评估和测量优化步骤，在多种图像恢复任务中实现了高计算效率和鲁棒的重建性能。

Conclusion: 论文提出的解耦框架为理解扩散模型在逆问题求解中的实际有效性提供了新视角，DAPS++方法在保持性能的同时显著提高了计算效率。
...
TL;DR: 本文提出了MIR方法，通过激励智能体探索影响队友的行为来解决多智能体强化学习中稀疏奖励问题，在MiniGrid-MA环境中验证了其优越性能。


<details>
  <summary>Details</summary>
Motivation: 多智能体强化学习中，由于联合动作轨迹呈指数级稀疏且现有方法未能充分考虑影响团队状态的联合动作，导致稀疏奖励（特别是情节奖励）问题难以解决。

Method: 提出了MIR方法，激励个体智能体探索能够影响队友的行为，结合原始策略有效促进团队探索。
...
TL;DR: 本文提出了一个基于层次任务抽象机制(HTAM)的多智能体框架EarthAgent，专门用于解决复杂地理空间分析任务，相比通用方法在专业领域表现更优。


<details>
  <summary>Details</summary>
Motivation: 通用LLM驱动的智能体在需要严格结构化工作流程的专业领域(如遥感)表现不佳，因为这些领域需要专门工具和多步骤程序。

Method: 引入层次任务抽象机制(HTAM)，将多智能体系统构建为逻辑层次结构，反映领域内在的任务依赖图，将复杂问题分解为顺序层级的子任务。

Result: 构建了EarthAgent系统并在GeoPlan-bench基准测试中验证，显著优于现有的单智能体和多智能体系统。

Conclusion: 将智能体架构与领域内在任务结构对齐是构建稳健可靠专业自治系统的关键步骤。
...
TL;DR: 本文主张将AAMAS社区开发的BDI架构、通信协议、机制设计和制度建模等概念工具作为实现智能AI的基础，通过将自适应数据驱动方法与结构化推理协调模型相结合，构建具有透明度、合作性和问责制的智能系统。


<details>
  <summary>Details</summary>
Motivation: 实现具有持续自主性、推理和交互能力的智能AI系统，需要明确认知、合作和治理模型来补充其关于智能体的假设。
...
Conclusion: AAMAS社区的概念工具为智能AI提供了必要的基础，通过整合数据驱动方法和结构化模型，可以实现更可靠和负责任的智能系统。
...
TL;DR: 本文系统评估了在LLM行为监测中，使用合成数据和离策略数据对探针泛化性能的影响，发现响应生成策略显著影响探针性能，且训练数据域的变化会导致更大的性能下降。


<details>
  <summary>Details</summary>
Motivation: 由于许多LLM行为的自然示例稀少，研究人员不得不依赖合成或离策略的LLM响应来训练探针，但这种方法对探针泛化能力的影响尚未得到系统评估。

Method: 在八种不同的LLM行为上测试线性和注意力探针，比较使用合成数据、离策略数据和同策略数据训练探针的泛化性能。

Result: 发现响应生成策略显著影响探针性能，成功从离策略数据泛化到激励模型产生目标行为的测试集可以预测同策略泛化的成功。欺骗和故意表现不佳的探针可能无法从离策略数据泛化到同策略数据。

Conclusion: 在没有同策略数据的情况下，使用同域离策略数据比使用不同域的同策略数据产生更可靠的探针，强调需要能够更好处理LLM监测中分布偏移的方法。
...
TL;DR: 提出SRA-CP框架，通过风险感知选择性协作感知，在保持安全关键目标检测精度的同时大幅降低通信带宽需求
...
Method: 采用去中心化协议，车辆持续广播轻量级感知覆盖摘要，仅在检测到风险相关盲区时启动针对性协作；包含感知风险识别模块、基于感知覆盖的伙伴选择和适应带宽约束的选择性信息交换融合模块

Result: 在公共数据集上评估，相比通用CP方法仅损失不到1%的安全关键目标平均精度，同时仅使用20%的通信带宽；相比现有非风险感知选择性CP方法，感知性能提升15%

Conclusion: SRA-CP框架有效解决了协作感知中的通信带宽和动态环境适应性问题，实现了高效的风险感知选择性信息共享
...
TL;DR: RubiSCoT是一个AI支持的论文评估框架，使用自然语言处理技术提供一致、可扩展的论文评估解决方案，涵盖从提案到最终提交的全过程。


<details>
  <summary>Details</summary>
Motivation: 传统论文评估方法耗时且存在评估者变异性问题，需要更高效、一致的评估解决方案。
