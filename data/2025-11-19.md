...
TL;DR: 提出一个解决开放集识别中过自信问题的框架，通过参数扰动估计预测不确定性，并使用两阶段分类器来区分已知和未知类别。


<details>
  <summary>Details</summary>
Motivation: 开放集识别中，当未知样本与已知类别语义相似时，特征空间中的类间重叠会导致模型对未知样本赋予过高置信度，造成过自信问题，从而模糊已知与未知类别的决策边界。

Method: 框架包含两个组件：基于扰动的预测不确定性估计模块（通过可控参数扰动生成多样化预测来量化不确定性）和具有不同学习分类器的未知检测模块（两阶段过程，利用估计的不确定性改进已知与未知类别的区分）。
...
TL;DR: 本文提出了一种新颖的对象感知时序建模方法，用于解决少样本视频目标检测中的时序一致性和新对象泛化问题，通过在帧间选择性传播高置信度对象特征来实现高效特征传播和噪声减少。


<details>
  <summary>Details</summary>
Motivation: 传统目标检测方法需要大量训练数据，而少样本视频目标检测面临时序一致性（遮挡和外观变化）和新对象泛化的挑战，现有方法依赖计算复杂的区域建议且需要任务特定训练。

Method: 采用对象感知时序建模方法，包含过滤机制选择性传播高置信度对象特征，结合少样本训练的检测和分类头，实现无需显式对象管建议的鲁棒时序一致性。

Result: 在5-shot设置下取得显著性能提升：FSVOD-500提升3.7%，FSYTV-40提升5.3%，VidOR提升4.3%，VidVRD提升4.5%。1-shot、3-shot和10-shot配置也均有改进。

Conclusion: 该方法通过高效的特征传播机制成功解决了少样本视频目标检测中的关键挑战，实现了更好的时序一致性和检测精度，代码已公开。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态图像融合方法依赖任务特定模型，训练成本高且可扩展性有限。生成方法虽然提供统一建模视角，但采样轨迹复杂导致推理缓慢。需要一种高效、统一的融合方法。

Method: 1. 将图像融合建模为从源模态到融合图像分布的直接概率传输，采用流匹配范式；2. 收集多个SOTA模型的融合结果作为先验，使用任务感知选择函数筛选可靠伪标签；3. 引入融合精炼模块，采用分治策略识别、分解和增强伪标签中的退化组件；4. 集成弹性权重巩固和经验回放机制支持多任务学习。

Result: 该方法在多种融合任务中达到竞争性性能，同时显著提高采样效率，保持轻量级模型设计。

Conclusion: 提出的FusionFM方法通过流匹配范式实现了高效的多模态图像融合，解决了传统方法训练成本高和生成方法推理慢的问题，为图像融合提供了统一且高效的解决方案。
...
TL;DR: 本文提出了一种针对地震数据的自适应双网格基础模型训练策略（ADATG），通过希尔伯特编码和频谱分解来有效处理地震数据中的高频和低频特征，解决了传统视觉变换器在地震数据处理中的局限性。


<details>
  <summary>Details</summary>
Motivation: 由于人工智能技术的快速发展，基于变换器的基础模型已在科学应用中取得突破，但地震数据具有独特的特征，需要专门的处理技术。现有的视觉变换器忽略了地震数据的内在模式，无法有效捕捉高频和低频信息。

Method: 采用自适应双网格基础模型训练策略，通过频谱分解分离高频和低频分量，利用分层希尔伯特编码有效表示数据，并基于频率原理提出自适应训练策略，从粗粒度信息逐步过渡到细粒度特征。
...
TL;DR: 该论文提出了一种基于面部微动态分析的被动式痴呆筛查方法，通过分析眨眼动态、嘴部运动、注视变化和头部微调等面部运动特征，实现无需语言和文本的早期神经认知变化检测。


<details>
  <summary>Details</summary>
Motivation: 现有痴呆筛查方法主要依赖语音或脚本化访谈，限制了在临床环境外的应用，且预测结果与语言和转录紧密耦合。本文旨在开发一种无需语言、可大规模应用的被动式痴呆筛查方法。

Method: 通过稳定面部信号，将微运动转换为可解释的面部微动态时间序列，平滑处理后将短窗口汇总为紧凑的片段级统计特征。每个窗口通过其活动混合（各运动流相对份额）进行编码，分析运动分布而非幅度。

Result: 在YT DemTalk数据集上，消融实验显示注视不稳定性和嘴部/下颌动态是最具信息量的线索，轻量级浅层分类器可实现AUROC 0.953、AP 0.961、F1分数0.851和准确率0.857的痴呆预测性能。

Conclusion: 面部微动态分析是痴呆筛查的有效方法，无需语言或文本，可在自然环境中大规模应用，且具有跨设备、主题和文化的可转移性。
...
TL;DR: 该论文提出了Gen-ViRe基准测试框架，用于评估视频生成模型在链式帧推理(CoF)中的认知能力，填补了现有基准无法衡量多步规划、算法逻辑等核心推理能力的空白。
...
Method: 基于认知科学和真实AI应用，将CoF推理分解为6个认知维度和24个子任务，通过多源数据整理、最小提示协议和混合VLM辅助评估进行定量评估。

Result: 对SOTA系统的实验显示视觉质量与推理深度之间存在显著差异，为推进真正世界模拟器建立了基准和诊断工具。

Conclusion: Gen-ViRe提供了首个视频模型作为推理器的定量评估，揭示了当前模型在推理能力方面的局限性，为开发更智能的世界模拟器指明了方向。
...
TL;DR: 本文提出基于排序的损失函数来解决热图人体姿态估计中的三个主要问题：MSE损失无法有效提升关节定位精度、热图空间和类别不平衡、以及评估指标与损失函数不一致的问题。


<details>
  <summary>Details</summary>
Motivation: 热图人体姿态估计方法存在三个主要问题：(1) MSE损失对像素偏差平等惩罚，无法专注于精确定位关节峰值；(2) 热图存在空间和类别不平衡；(3) 评估指标(mAP)与损失函数之间存在差异。

Method: 提出基于排序的损失函数，通过理论分析和实验验证，该损失函数优于常用的热图损失(MSE、KL散度)。在三种不同数据集(COCO、CrowdPose、MPII)上验证了方法的有效性。

Result: RSPose在COCO-val数据集上达到79.9 mAP，优于之前的最优方法。同时将SimCC Resnet-50方法在COCO-val上的性能提升了1.5 AP，达到73.6 AP。

Conclusion: 提出的基于排序的损失函数首次实现了人体姿态估计中损失函数与评估指标(mAP)的对齐，显著提高了置信度分数与定位质量的相关性，从而提升了NMS过程中的实例选择和mAP性能。
...
TL;DR: 提出碰撞声源分割任务(CS3)，从碰撞声音中识别并分割产生声音的物体，使用弱监督方法和基础模型在自我中心视频中实现优于基线3-4.7倍的效果。


<details>
  <summary>Details</summary>
Motivation: 受人类多感官感知能力启发，希望从碰撞声音中识别产生声音的物体，解决自我中心视频中视觉场景杂乱、物体小、交互短暂等挑战。

Method: 使用弱监督的音频条件分割方法，结合基础模型(CLIP和SAM2)和自我中心线索(手中的物体)来识别潜在的碰撞声源物体。

Result: 在EPIC-CS3和Ego4D-CS3两个基准测试中，mIoU分别比竞争基线高出3倍和4.7倍。

Conclusion: 提出的CS3任务和方法能够有效从碰撞声音中分割出声源物体，在自我中心视频中表现出色，为多模态感知提供了新方向。
...
TL;DR: 本文提出了一种几何基础的位置回归方法GRR，通过预测解耦的几何表示（射线方向束和点图）来估计相机位姿，而不是直接回归6自由度位姿。


<details>
  <summary>Details</summary>
Motivation: 传统的绝对位姿回归方法作为黑盒模型直接回归6自由度位姿，容易记忆训练视图而非理解3D场景几何。作者希望引入几何先验，通过建模逆渲染过程实现更鲁棒的位姿估计。

Method: 将绝对位姿回归重新定义为几何表示回归，预测两个解耦的几何表示：射线方向束用于估计相机旋转，对应点图用于估计相机平移，然后通过可微确定性求解器恢复最终位姿。

Result: 在7-Scenes和Cambridge Landmarks数据集上实现了最先进的性能，验证了建模逆渲染过程对通用绝对位姿估计的有效性。

Conclusion: 通过将学习的视觉到几何映射与最终位姿计算分离，引入强几何先验，解耦旋转和平移预测显著提升了性能，为通用绝对位姿估计提供了更鲁棒的路径。
...
TL;DR: 本文提出了一个用于膀胱癌复发预测的多序列MRI数据集，并开发了H-CNN-ViT模型，该模型通过分层门控注意力机制融合CNN和ViT的局部与全局特征，在膀胱癌复发检测中达到78.6%的AUC。


<details>
  <summary>Details</summary>
Motivation: 膀胱癌复发率高达78%，需要准确的术后监测。多序列增强MRI常用于复发检测，但术后组织变化使判读困难，且缺乏专门的多序列MRI数据集阻碍了AI辅助诊断工具的发展。

Method: 提出H-CNN-ViT模型，采用分层门控注意力多分支架构，独立处理每个模态，通过门控机制根据上下文需求选择性加权CNN（局部）和ViT（全局）路径的特征，实现平衡且有目标性的特征融合。

Result: 在自建数据集上评估，H-CNN-ViT达到78.6%的AUC，超越了现有最先进模型。

Conclusion: 本研究为膀胱癌复发预测提供了专门的多序列MRI数据集基准，并证明了H-CNN-ViT模型在融合局部和全局特征方面的有效性，为AI辅助膀胱癌复发检测提供了有力工具。
...
TL;DR: QwenCLIP是一个视觉语言框架，用大型语言模型替换CLIP的文本编码器，通过可学习提示增强跨模态对齐，解决了医学报告中长文本表示的限制。


<details>
  <summary>Details</summary>
Motivation: CLIP的文本编码器仅支持77个token，限制了其在信息丰富的放射学报告中的表示能力；现有医学领域适配器仍受限于512token输入长度和浅层语义理解。

Method: 用LLM基础嵌入模块（如Qwen3-Embedding）替换CLIP文本编码器，引入可学习提示增强跨模态对齐，利用LLM的扩展上下文窗口和丰富表示。

Result: 能够从长篇临床文本中捕获全面的医学语义，显著改善了医学图像-文本对齐和在放射学基准测试中的下游性能。

Conclusion: QwenCLIP通过LLM增强的文本编码解决了长文本表示问题，在医学视觉语言任务中表现出优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统迁移学习方法在医学图像高维背景下存在局限性，无法充分捕捉详细的解剖特征，需要克服这些约束以提升分类性能。

Method: 集成EfficientNet和VGG19，加入伪牛顿增强层进行特征权重优化，稀疏诱导层去除冗余特征，形成多层框架。

Result: 模型性能显著提升，达到精确度0.9、召回率0.861、F1分数0.88、损失0.18、准确率88.1%，优于基准模型EfficientNet。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于视觉语言模型的自动驾驶方法依赖手工提示且性能不稳定，在真实场景中缺乏鲁棒性和泛化能力。

Method: 利用视觉语言模型的场景理解能力为视觉基准提供结构化语义描述，构建多模态交互架构融合视觉和语言特征，并设计后处理精炼模块提升预测可靠性。
...
TL;DR: Uni-Hema是一个用于数字血液病理学的多任务统一模型，整合了检测、分类、分割、形态预测和跨多种疾病的推理功能，解决了现有方法无法在复杂血液病理学场景中进行统一多任务多模态推理的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有数字血液病理学方法（包括单任务、视觉语言、WSI优化和单细胞血液学模型）存在关键限制：无法在复杂的数字血液病理学场景中提供统一的多任务多模态推理。
...
TL;DR: 本文提出了首个弱监督管道用于检测农业领域中的短暂沟壑，利用视觉语言模型减少人工标注负担，并发布了首个半监督检测数据集。


<details>
  <summary>Details</summary>
Motivation: 解决短暂沟壑自动检测的挑战：短暂的时间周期使传统计算机视觉方法难以检测，标记数据稀缺且难以准确生成，限制了机器学习方法的应用。

Method: 基于遥感技术，利用视觉语言模型的预训练知识，采用师生模型框架，教师模型从VLM的噪声标签中学习，学生模型通过弱监督使用教师生成的标签和噪声感知损失函数进行学习。
...
TL;DR: 本文提出了一种基于压缩视频流中运动向量的可扩展框架，用于评估生成视频的时间真实性，通过计算真实视频与生成视频运动向量统计量的差异来量化运动逼真度。


<details>
  <summary>Details</summary>
Motivation: 当前生成视频模型在时间真实性方面存在明显缺陷，大多数评估指标主要关注空间外观，对运动的敏感性有限。

Method: 使用H.264和HEVC等标准从压缩视频流中提取运动向量，通过计算Kullback-Leibler、Jensen-Shannon和Wasserstein散度来量化真实视频与生成视频运动向量统计量的差异。

Result: 在GenVidBench数据集上的实验显示，基于熵的散度将Pika和SVD评为最接近真实视频，MV-sum统计量偏好VC2和Text2Video-Zero，而CogVideo在两种度量中都显示出最大偏差。运动向量与RGB融合可显著提高下游分类准确率。

Conclusion: 压缩域运动向量为诊断生成视频中的运动缺陷和增强判别模型中的时间推理提供了有效的时序信号。
...
TL;DR: SAE-MCVT是首个可扩展的实时多摄像头车辆跟踪框架，通过边缘设备处理视频流并传输轻量级元数据到中央工作站，实现城市级部署。


<details>
  <summary>Details</summary>
Motivation: 现有MCVT研究过于关注准确性而忽视实时性和可扩展性，这在城市规模应用中随着摄像头数量增加变得尤为关键。

Method: 系统包括边缘设备和中央工作站。边缘端处理RTSP视频流，进行目标检测、跟踪、地理映射和特征提取，仅传输车辆位置和深度外观特征。中央端基于自监督相机链接模型学习相邻相机的时空关系进行跨相机关联。

Result: 在RoundaboutHD数据集上，SAE-MCVT在2K 15 FPS视频流上保持实时运行，IDF1得分达到61.2。

Conclusion: 这是首个适合城市级部署的可扩展实时MCVT框架，平衡了准确性、实时性和可扩展性。
...
<details>
  <summary>Details</summary>
Motivation: 道路安全规范对自动驾驶车辆的AI系统至关重要，需要评估多模态LLMs对道路安全概念的理解程度。

Method: 使用从学校教科书中收集的交通标志和道路安全规范图像构建数据集，在零样本设置下评估模型能力。

Result: 初步结果显示，这些模型在安全推理方面表现不佳，存在人类学习与模型解释之间的性能差距。

Conclusion: 多模态LLMs在道路安全概念理解上存在明显不足，需要进一步研究来缩小这些性能差距。
...
<details>
  <summary>Details</summary>
Motivation: 研究发现强化学习微调的CoT推理在视觉定位任务中会因推理步骤过长或复杂而性能下降，且数据集规模增加并不总是提升性能。

Method: 提出基于课程的相对策略优化(CuRPO)，利用CoT长度和广义交并比(gIoU)奖励作为复杂度指标，从简单到复杂逐步结构化训练数据。

Result: 在RefCOCO、RefCOCO+、RefCOCOg和LISA数据集上实验表明，CuRPO优于现有方法，在RefCOCO上提升达+12.52 mAP，在少样本学习中表现优异。

Conclusion: CuRPO方法在视觉定位任务中表现出高效性和鲁棒性，特别适用于文本描述模糊复杂的任务。
...
TL;DR: 提出基于聚类的帧选择策略来缓解视频衍生帧数据集中的信息泄露问题，通过在数据集划分前对视觉相似帧进行分组，生成更具代表性、平衡性和可靠性的数据集分区。
...
Method: 采用聚类方法对视觉相似的帧进行分组，然后在聚类基础上进行数据集划分，避免相似帧出现在不同数据集中。

Result: 该方法能够生成更具代表性、平衡性和可靠性的数据集分区，有效减少信息泄露风险。

Conclusion: 基于聚类的帧选择策略是解决视频衍生帧数据集信息泄露问题的有效方法，能够提高数据集划分的质量和可靠性。
...
TL;DR: 本文提出通过使用程序生成的无视觉或语义内容数据预训练视觉Transformer，以注入通用归纳偏置，从而提高数据效率、收敛速度和下游性能。


<details>
  <summary>Details</summary>
Motivation: 探索在视觉Transformer中注入通用归纳偏置的新方法，通过使用不含视觉或语义内容的程序生成数据进行预训练，以提升模型的泛化能力和数据效率。

Method: 使用形式语法等简单算法生成程序数据，在预训练阶段绕过视觉补丁嵌入机制，让模型内化抽象计算先验，然后进行标准的基于图像的训练。

Result: 在ImageNet-1k上，仅用1%的训练预算用于程序数据预训练，最终准确率提高超过1.7%，相当于28%的ImageNet-1k数据的效果。

Conclusion: 这种预训练策略为实现数据高效和领域无关的预训练提供了有前景的新路径。
...
TL;DR: 提出CrossTrainer方法，通过发现跨运动通用的技能属性（如平衡、控制、手部位置），训练多模态语言模型为运动视频生成可操作的反馈和熟练度评估，在跨运动和同运动设置下相对现有技术提升达60%。


<details>
  <summary>Details</summary>
Motivation: 现有技能评估模型局限于单一运动项目，且专家级监督成本高、稀缺，难以覆盖众多运动项目。需要开发可迁移的视频表示方法来解决这一问题。

Method: CrossTrainer方法发现跨运动通用的技能属性，训练多模态语言模型生成针对新视频的可操作反馈（如"多抬手以产生更多力量"）和熟练度等级（如早期专家）。

Result: 在多个数据集上验证，CrossTrainer在跨运动（迁移）和同运动（域内）设置下相对现有技术提升达60%，通过抽象共享的人类技能行为，视频表示比现有技术泛化能力更强。

Conclusion: 通过抽象共享的人类技能行为，提出的视频表示方法显著优于现有技术，丰富了当今多模态大语言模型的能力。
...
TL;DR: RISE是首个基于单静态雷达的室内场景理解基准和系统，通过利用多路径反射的几何线索，实现了布局重建和物体检测，在保护隐私的同时显著提升了性能。
...
Method: 提出双角度多路径增强方法，显式建模到达角和离开角来恢复二次反射；采用模拟到现实的分层扩散框架，将碎片化的雷达响应转换为完整的布局重建和物体检测。

Result: 在布局重建方面将Chamfer距离降低60%至16厘米；首次实现基于毫米波的物体检测，达到58% IoU；构建了包含5万帧数据的大规模雷达室内场景理解数据集。

Conclusion: RISE为使用单静态雷达实现几何感知和隐私保护的室内场景理解建立了新的基础，证明了多路径反射在克服雷达低分辨率限制方面的价值。
...
TL;DR: 开发了一个2.5D上下文感知模型，用于自动识别MRI图像的解剖平面方向（轴向、冠状面、矢状面），准确率达到99.49%，比2D模型错误率降低60%。该模型在脑肿瘤检测任务中也有显著提升。


<details>
  <summary>Details</summary>
Motivation: MRI图像中缺少平面方向元数据会复杂化分析、增加领域偏移并降低诊断分类器的准确性，而现有自动化系统难以准确识别解剖平面方向。

Method: 采用2.5D上下文感知模型，利用多切片信息避免单一切片的歧义性，同时在3D切片序列和静态2D图像上进行训练。

Result: 2.5D模型准确率达到99.49%，比2D参考模型（98.74%）错误率降低60%。在脑肿瘤检测任务中，通过门控策略使用元数据增强预测，准确率从97.0%提升到98.0%，误诊率降低33.3%。
...
TL;DR: LINGUAL框架通过自然语言指令和上下文学习实现医学图像分割的自动化，在主动领域自适应中达到与主动学习基线相当或更优的性能，同时减少约80%的标注时间。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割中的主动学习仍然具有挑战性、劳动密集且认知要求高，因为医学图像通常具有模糊和模糊的边界。传统主动学习中，标注工作量与感兴趣区域大小相关，大区域虽然认知上更容易但标注成本高，小区域则需要更精细的精度和专家注意力。

Method: LINGUAL框架接收专家的自然语言指令，通过上下文学习将其转换为可执行程序，并自动执行相应的子任务序列，无需人工干预。

Result: 在主动领域自适应中，LINGUAL实现了与主动学习基线相当或更优的性能，同时估计标注时间减少了约80%。

Conclusion: 语言指导提供了一种有效的替代方案，需要最少的专家工作量，同时绕过了分割中精确边界描绘的认知要求高的任务。
...
<details>
  <summary>Details</summary>
Motivation: 随着视觉生成模型的快速发展，AI生成图像检测变得至关重要。研究者希望开发一种无需训练、不依赖特定数据集知识的通用检测方法。

Method: 基于自监督模型在RandomResizedCrop增强训练中学习到的一致性表示特性，通过Haar小波分解提取高频方向，量化图像嵌入对扰动的敏感性，并将图像分割成多个补丁计算平均得分。

Result: 在23种不同生成模型产生的图像和多种分辨率、领域的真实数据集上验证，WaRPAD始终表现出竞争性性能，并对测试时损坏具有强鲁棒性。

Conclusion: WaRPAD是一种有效的无需训练AI生成图像检测方法，由于RandomResizedCrop不变性是自监督模型的常见训练方案，该方法可广泛应用于各种自监督模型。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有方法在服装变形过程中导致的纹理失真问题，以及缺乏细粒度外观控制机制的问题，旨在实现忠实保留服装细节和精细控制生成模型外观的目标。

Method: 提出免变形框架直接外推分割的服装，避免变形导致的纹理失真；设计区域自适应解耦注意力机制(RADA)和链式掩码注入策略，实现细粒度外观控制。

Result: 实验验证了该框架相比现有最先进方法的优越性能，能够高质量地生成可控的时尚展示图像。

Conclusion: FashionMAC通过免变形方法和细粒度控制机制，成功解决了服装细节保留和外观控制的关键挑战，为时尚图像生成提供了有效的解决方案。
...
Conclusion: 提出的方法为混凝土桥梁结构缺陷检测提供了一种有效的自动化解决方案，结合了显著性分析和深度学习技术的优势，具有良好的实际应用前景。
...
Conclusion: SCAR克服了基于向量量化的预填充方法的语义限制和高成本，同时保持了可控性，代码将公开发布。
...
TL;DR: CORE提出了一种新的视觉token压缩范式，通过对象分割生成语义先验来指导token合并，创建紧凑的对象中心表示，并在极端压缩下仍能保持97.4%的基线性能。
...
Conclusion: CORE证明了对象中心表示在高效和有效的LVLM处理中的优越性，为视觉token压缩提供了新的解决方案。
...
TL;DR: 提出ZS-TMS新范式，通过预训练生成引擎直接合成任务特定分类器的全部参数，仅需单张示例图像和临床文本描述，无需任务特定训练即可部署。
...
Method: 使用语义引导参数合成器(SGPS)，输入最少的多模态任务信息，通过生成引擎直接合成轻量级分类器的权重参数。

Result: 在ISIC 2018皮肤病变数据集和自定义罕见疾病数据集上，1-shot和5-shot分类任务中显著优于现有少样本和零样本学习方法。

Conclusion: 为AI驱动的诊断工具快速开发部署开辟了新途径，特别适用于数据极其有限的罕见疾病场景。
...
TL;DR: 提出了DEER-3D框架，通过错误驱动的3D场景编辑生成精确的视觉反事实数据，以解决3D-LLMs在空间理解方面的局限性，显著提升语言到3D空间的接地准确性。


<details>
  <summary>Details</summary>
Motivation: 现有3D-LLMs在准确将语言接地到3D环境中的视觉和空间元素方面存在局限，主要由于训练数据侧重于语言推理而非空间理解，且缺乏足够的3D资源导致固有接地偏见未解决。

Method: 提出DEER-3D框架，采用"分解、诊断评估、编辑和重新训练"的工作流程，通过最小化、谓词对齐的3D场景编辑（如重新着色或重新定位）生成针对性反事实监督数据，进行迭代模型微调。

Result: 在多个3D接地和场景理解任务的基准测试中，通过迭代精炼在所有评估数据集上一致显示出改进，显著提升了接地准确性。
...
TL;DR: 提出BCE3S方法，通过二元交叉熵的三方协同学习解决长尾识别问题，提升特征紧凑性和类别间分离性，平衡分类器向量分离性，在多个长尾数据集上达到SOTA性能。
...
Result: 在CIFAR10-LT、CIFAR100-LT、ImageNet-LT和iNaturalist2018等多个长尾数据集上的广泛实验表明，BCE3S训练的长尾识别模型实现了更高的特征紧凑性和分离性，同时平衡了分类器的分离性。
...
TL;DR: FAPE-IR是一个用于图像修复的频率感知规划和执行框架，使用冻结的多模态大语言模型作为规划器生成频率感知修复计划，并通过LoRA-MoE模块在扩散模型中动态选择高频或低频专家进行修复。


<details>
  <summary>Details</summary>
Motivation: 现有的多任务图像修复方法依赖特定任务设计或潜在路由策略，难以适应现实世界中复杂多样的退化情况，需要更统一和可解释的解决方案。

Method: 使用冻结MLLM作为规划器分析退化图像并生成频率感知修复计划；在扩散执行器中采用LoRA-MoE模块动态选择高频或低频专家；引入对抗训练和频率正则化损失来提升修复质量。
...
TL;DR: RIVER是一个基于推理的隐式视频编辑模型，能够通过多跳推理理解隐式查询来推断编辑目标，然后执行像素级修改。
...
Method: RIVER通过数字孪生表示解耦推理和生成，使用大语言模型进行多跳推理生成结构化指令，然后指导基于扩散的编辑器执行像素级修改，训练时使用强化学习评估推理准确性和生成质量。

Result: RIVER在提出的RVEBenchmark上表现最佳，在VegGIE和FiVE两个额外视频编辑基准测试中也达到了最先进的性能，超越了六个基线方法。

Conclusion: RIVER通过推理驱动的视频编辑方法成功解决了隐式查询的理解和编辑问题，为复杂语义编辑任务提供了有效解决方案。
...
TL;DR: 提出了一种实时自监督单目深度估计方法RTS-Mono，采用轻量级编码器-解码器架构，在保持高性能的同时显著减少计算资源消耗，实现了在边缘设备上的实时部署。


<details>
  <summary>Details</summary>
Motivation: 现有单目深度估计模型计算资源消耗大，虽然有些方法减小了模型规模但性能下降严重，阻碍了在实际场景中的部署应用。

Method: 基于Lite-Encoder的编码器和多尺度稀疏融合框架的解码器架构，最小化冗余并提高推理速度。

Result: 在KITTI数据集上实现了SOTA性能，参数量仅3M，相比轻量级方法在低分辨率下Abs Rel和Sq Rel分别提升5.6%和9.8%，在高分辨率下Sq Rel和RMSE分别提升6.1%和1.9%。在Nvidia Jetson Orin上达到49 FPS的实时推理速度。

Conclusion: RTS-Mono在保持高性能的同时实现了轻量化和实时推理，为自动驾驶和机器人导航中的单目深度估计提供了实用的解决方案。
...
TL;DR: 提出了一种名为A²GC-VPR的非对称聚合视觉位置识别方法，通过行-列归一化平均和单独边缘校准实现非对称匹配，并引入几何约束增强空间感知能力。


<details>
  <summary>Details</summary>
Motivation: 现有最优传输聚合方法在源和目标边缘分布对称处理，当图像特征和聚类中心分布差异较大时效果受限，需要非对称匹配机制。

Method: 采用行-列归一化平均与单独边缘校准实现非对称匹配，通过可学习坐标嵌入引入几何约束，计算融合特征相似性的兼容性分数。

Result: 在MSLS、NordLand和Pittsburgh数据集上的实验结果表明，该方法在匹配精度和鲁棒性方面表现优异。

Conclusion: 所提出的非对称聚合方法能有效处理视觉位置识别中特征分布差异问题，显著提升匹配性能。
...
TL;DR: 提出CViT架构，通过CCFFN设计提高ViT的计算效率，在保持精度的同时减少FLOPs和能耗，适合部署在资源受限设备上。
...
Result: 在ImageNet-1K上，CViT-XL达到75.5% Top-1精度，相比EfficientViT-M5减少15% FLOPs和3.3%能耗；CViT家族在各种模型尺寸下都表现出最低能耗。
...
TL;DR: Coffee是一种新的文本到图像扩散模型微调方法，通过语言描述来指定不希望学习的概念，防止模型在微调过程中学习到不需要的概念并与用户提示纠缠。


<details>
  <summary>Details</summary>
Motivation: 解决文本到图像扩散模型在微调时学习到不需要的概念的问题，这对于偏见缓解、防止恶意适应、属性解缠和扩散策略的通用微调等下游任务至关重要。

Method: 使用语言描述来指定不希望学习的概念，通过防止用户提示的嵌入与不希望的概念对齐来正则化适应过程，无需额外训练即可灵活修改不希望的概念。
...
Result: 在多个草图任务评估中（目标定位、计数、图像检索、视觉问答），O3SLM显著优于现有LVLMs，在草图理解和推理方面达到最先进性能。

Conclusion: O3SLM通过专门的数据集和训练方法，有效提升了大型视觉语言模型对抽象视觉输入（特别是手绘草图）的理解能力。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有视频系统只能检测事故但无法分析行人行为认知阶段的问题，利用视觉语言模型提升对行人-车辆事故的深入理解。

Method: 采用四阶段统一框架：事件触发的多视角视频采集、行人行为阶段分割、阶段特定的多视角推理、分层合成和诊断推理，使用TG-VLM和PhaVR-VLM两个专用视觉语言模型。

Result: TG-VLM在行为阶段分割上达到mIoU=0.4881，PhaVR-VLM在字幕生成得分为33.063，问答准确率最高达64.70%，在Woven Traffic Safety数据集上验证了有效性。
...
Method: 引入ConvNN框架，作为卷积层和注意力层的即插即用替代品，能够系统探索卷积和注意力之间的中间谱系。在CIFAR数据集上通过VGG混合分支和ViT中的ConvNN进行验证。

Result: 在CIFAR-10和CIFAR-100分类任务中，VGG混合分支提高了两个数据集的准确率；ViT中的ConvNN优于标准注意力和其他注意力变体。k值和架构变体的广泛消融实验显示，沿该谱系插值通过平衡局部和全局感受野提供正则化益处。
...
<details>
  <summary>Details</summary>
Motivation: 开发能够对完全遮挡的日常物体进行高精度3D形状重建的能力，这在机器人、增强现实和物流等领域具有重要应用价值。毫米波信号能够穿透常见遮挡物并反射隐藏物体，但现有方法存在覆盖范围有限和噪声高的问题。

Method: 采用三阶段流水线：提出候选几何表面、使用专门为毫米波信号设计的基于transformer的形状补全模型、执行熵引导的表面选择。该方法结合了毫米波信号的物理特性与基于视觉的形状补全技术的最新进展。

Result: 在合成点云上训练，在真实世界数据上表现出良好的泛化能力。与最先进基线方法相比，将召回率从54%提升到72%，同时保持85%的高精度。

Conclusion: Wave-Former通过物理感知的形状补全模型，成功实现了对完全遮挡物体的高精度3D重建，为毫米波信号在物体重建领域的应用提供了新的解决方案。
...
TL;DR: 提出了RiSe框架解决多模态人脸反欺诈(FAS)在跨域部署时的性能下降问题，通过非对称不变风险最小化和多模态协同解耦来应对模态表示不变风险和模态协同不变风险。


<details>
  <summary>Details</summary>
Motivation: 多模态FAS方法在未见域中比单模态方法性能下降更严重，主要由于两个被忽视的风险：模态表示不变风险（表示在域偏移下是否保持泛化性）和模态协同不变风险（模型过拟合到域特定的模态间相关性）。

Method: RiSe框架包含两个核心组件：1) AsyIRM在径向空间学习不变球形决策边界以适应非对称分布，同时在角度空间保留域线索；2) MMSD通过跨样本混合和解耦的自监督任务增强内在、可泛化的模态特征。

Result: 理论分析和实验验证了RiSe的有效性，在跨域性能上达到了最先进水平。

Conclusion: RiSe框架成功解决了多模态FAS的跨域泛化问题，通过处理表示风险和协同风险，显著提升了在未见域中的性能表现。
...
TL;DR: MVI-Bench是首个专门评估误导性视觉输入如何削弱大型视觉语言模型鲁棒性的综合基准，包含1,248个专家标注的VQA实例，涵盖视觉概念、属性和关系三个层次。


<details>
  <summary>Details</summary>
Motivation: 现有鲁棒性基准主要关注幻觉或误导性文本输入，而忽略了误导性视觉输入在评估视觉理解中的关键挑战。

Method: 基于基本视觉原语，设计了三个层次的误导性视觉输入分类：视觉概念、视觉属性和视觉关系，并引入了MVI-Sensitivity细粒度鲁棒性评估指标。
...
TL;DR: 提出了一种针对多模态大语言模型的对象级令牌压缩策略，通过自适应令牌合并来减少图像令牌数量，仅使用10%的令牌即可达到原始模型96%的性能。
...
Conclusion: 该方法有效解决了多模态大语言模型中图像令牌过多的问题，实现了高效的令牌压缩，同时保持了模型性能。
...
TL;DR: DoGCLR是一种基于博弈论的自监督对比学习框架，用于基于骨架的动作识别。它通过动态优势博弈构建正负样本，使用时空双权重定位机制识别关键运动区域，并采用熵驱动优势策略管理记忆库，在多个数据集上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的自监督对比学习方法在处理骨架区域时采用统一方式，并使用FIFO队列存储负样本，导致运动信息丢失和非最优负样本选择。
...
Conclusion: DoGCLR通过博弈论方法有效解决了运动信息丢失和负样本选择问题，在多个数据集上表现出优越性能，特别是在更具挑战性的场景中展现出强大鲁棒性。
...
TL;DR: UniSER是一个基础性通用模型，能够在一个框架内处理多种软效应（如镜头光晕、雾霾、阴影、反射）导致的图像退化问题，通过大规模数据集和定制训练流程，显著优于专业模型和通用模型。


<details>
  <summary>Details</summary>
Motivation: 现有方法针对各种软效应退化问题开发高度专业化的模型，缺乏可扩展性且未能利用这些修复问题的共享本质。通用模型需要详细提示且在这些细粒度任务上难以实现稳健去除或保持场景身份。

Method: 利用软效应的共同本质（半透明遮挡），策划包含380万对图像的大规模数据集，通过定制训练流程微调扩散变换器，学习稳健的修复先验，并集成细粒度掩码和强度控制。

Result: UniSER在真实场景中实现了稳健、高保真的修复，显著优于专业模型和通用模型。

Conclusion: 通过利用软效应的共享本质和大规模多样化数据，UniSER展示了在单一框架内处理多种图像退化问题的可行性，为图像修复领域提供了新的基础性解决方案。
...
Method: 提出全局视角分词器(GloTok)：1) 码本直方图关系学习方法，将预训练模型在整个数据集上建模的语义转移到语义码本；2) 残差学习模块，恢复细粒度细节以最小化量化引起的重建误差。
...
<details>
  <summary>Details</summary>
Motivation: 现有数据集多为人类驾驶模式收集，只能用于感知预测的早期训练，无法评估自动驾驶黑盒控制器的真实行为安全性。

Method: 收集超过100小时来自市场多个量产自动驾驶车辆的自然驾驶数据，分割成32,727个关键帧，包含同步摄像头图像、高精度GNSS/IMU数据、车辆轨迹和详细2D标注。

Result: 构建了包含丰富场景属性的数据集，使用端到端运动规划模型在自动驾驶帧上达到1.4米的平均位移误差。
...
TL;DR: 该论文提出了一种用于主动脉血管分割的课程学习策略，采用分形softmax进行分层语义学习，解决了血管结构中的类别不平衡问题，显著提升了分割精度和推理速度。


<details>
  <summary>Details</summary>
Motivation: 主动脉血管分割面临两个主要挑战：现有方法忽略了层次解剖关系，同时难以处理血管结构中固有的严重类别不平衡问题。这些限制影响了临床应用的准确性和实用性。

Method: 采用课程学习策略，结合新颖的分形softmax进行分层语义学习。该方法模仿人类认知过程，从简单到复杂逐步学习解剖约束，通过两阶段推理策略实现加速。

Result: 在验证集上，分层语义损失将nnU-Net ResEnc M的Dice分数提高了11.65%。在测试集上，提出的模型比基线方法Dice分数高出5.6%，推理速度提升高达5倍。

Conclusion: 该框架在分割精度和效率方面均有显著改进，适用于实时临床应用，代码已公开可用。
...
Method: DetGain通过建模全局得分分布来估计全局AP变化，并计算师生贡献差距，在每个迭代中选择信息丰富的样本。该方法与架构无关且侵入性最小。

Result: 在COCO数据集上的实验表明，DetGain在多个代表性检测器上都能持续提高准确性，在低质量数据下表现出强大的鲁棒性，并能与知识蒸馏技术有效结合进一步提升性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有的神经3D重建方法依赖密集多视角捕获，限制了更广泛的应用。同时，3D场景生成对于推进具身AI和世界模型至关重要，这些模型需要多样化的高质量场景进行学习和评估。

Method: 首先通过RGBD图像生成初始点云，然后维护和扩展世界模型，最后通过优化高斯泼溅表示来完成3D场景重建。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像分割面临数据隐私和专家标注成本高的挑战，联邦半监督学习(FSSL)虽然提供解决方案，但存在伪标签可靠性依赖本地模型强度、客户端设备计算资源有限导致模型架构紧凑或异构的问题，这些限制降低了伪标签的质量和稳定性。
...
Conclusion: SAM-Fed通过利用分割基础模型指导轻量级客户端，有效解决了联邦半监督学习中伪标签质量和稳定性的问题，在医学图像分割任务中表现出优越性能。
...
Method: 使用Flutter框架和Google ML Kit开发移动应用Bapp，在45个患者视频上进行验证，由眼科专家手动标注作为基准真值，评估精度、召回率和F1分数。

Result: Bapp表现优异：精度98.4%、召回率96.9%、总体准确率98.3%，证实其作为眼睑运动监测工具的可靠性。

Conclusion: Bapp为正常和异常眼睑运动监测提供了便携、可访问且客观的工具，有望替代传统手动计数方法，支持临床环境中的持续眼部健康监测和术后评估。
...
TL;DR: 提出了一种用于语音驱动3D面部动画的自回归扩散模型，通过流式处理音频输入来解决传统方法在处理长音频序列时的性能和延迟问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的语音驱动3D面部动画方法在处理长音频序列时存在两个主要问题：超出训练范围时性能下降，以及处理长音频输入时延迟显著增加。

Method: 采用自回归扩散模型，以流式方式处理音频输入。选择有限数量的过去帧作为历史运动上下文，与音频输入结合形成动态条件，指导扩散过程迭代生成面部运动帧。

Result: 实现了对可变长度音频的灵活处理，获得与音频时长无关的低延迟，能够实时合成高质量的面部动画。

Conclusion: 提出的流式自回归扩散模型有效解决了长音频序列处理中的性能和延迟问题，实现了实时高质量的语音驱动3D面部动画生成。
...
Method: 提出3D物理对抗示例，采用全局伪装纹理；设计3D立体匹配渲染模块处理相机视差效应；提出新颖的融合攻击方法，通过细粒度优化将目标无缝融入环境。

Result: 广泛评估表明，所提物理对抗示例能成功欺骗立体模型产生错误的深度信息。

Conclusion: 该方法在现有隐藏攻击基础上显著提升了隐蔽性和杀伤力，实现了目标与背景的无缝融合。
...
<details>
  <summary>Details</summary>
Motivation: 尽管基础模型在零样本单目深度估计方面取得了进展，但当下游任务有可用数据时，如何进一步提升这些模型的性能成为一个自然的问题。

Method: 采用密集自训练目标作为结构自监督的主要来源；引入语义感知分层归一化，利用实例级分割图进行更稳定的多尺度结构归一化；使用成对有序深度注释作为成本效益高的弱监督来指导适应过程；采用权重正则化损失来锚定LoRA更新。

Result: 在多样化和具有挑战性场景下的现实和损坏分布外数据集上的广泛实验表明，WeSTAR持续改善泛化能力，并在广泛基准测试中达到最先进的性能。
...
TL;DR: 提出了一种基于LiDAR定位的GNSS-free协同感知框架，包括轻量级姿态生成器PGC和姿态感知时空对齐Transformer PASTAT，在GNSS受限环境下实现先进的协同感知性能。


<details>
  <summary>Details</summary>
Motivation: 传统GNSS定位在GNSS受限环境中经常失效，导致协同感知中特征对齐困难，需要开发不依赖GNSS的鲁棒协同感知方案。

Method: 1. 提出轻量级姿态生成器PGC估计紧凑姿态和置信度表示；2. 开发姿态感知时空对齐Transformer PASTAT进行置信度感知的空间对齐并捕获时间上下文；3. 创建V2VLoc仿真数据集支持LiDAR定位和协同检测任务。

Result: 在V2VLoc数据集上的广泛实验表明，该方法在GNSS受限条件下达到最先进性能，在真实世界V2V4Real数据集上的扩展实验验证了PASTAT的有效性和泛化能力。

Conclusion: 提出的GNSS-free协同感知框架通过LiDAR定位和时空对齐方法，在GNSS受限环境中实现了鲁棒的协同感知性能，为解决GNSS失效问题提供了有效解决方案。
...
### [61] [Agentic Video Intelligence: A Flexible Framework for Advanced Video Exploration and Understanding](https://arxiv.org/abs/2511.14446)
*Hong Gao,Yiming Bao,Xuezhen Tu,Yutong Xu,Yue Jin,Yiyang Mu,Bin Zhong,Linan Yue,Min-Ling Zhang*

Main category: cs.CV

TL;DR: 提出了Agentic Video Intelligence (AVI)框架，通过系统级设计和优化实现无需训练的视频理解，包含三阶段推理过程、结构化视频知识库和开源模型集成。
...
Method: AVI框架包含：(1) 受人类启发的三阶段推理过程（检索-感知-评审），(2) 基于实体图的结构化视频知识库和多粒度集成工具，(3) 结合推理LLM、轻量级CV模型和VLM的开源模型集成。

Result: 在LVBench、VideoMME-Long、LongVideoBench和Charades-STA等基准测试中，AVI实现了有竞争力的性能，同时提供更好的可解释性。

Conclusion: AVI框架在无需依赖专有API或强化学习训练的情况下，通过系统级设计实现了高效且可解释的视频理解。
...
TL;DR: 提出了ManipBench大规模图像篡改检测基准和ManipShield统一检测模型，针对AI编辑图像，包含45万张篡改图像和10万张带标注图像，实现检测、定位和解释一体化。


<details>
  <summary>Details</summary>
Motivation: 随着生成模型的快速发展，现有图像篡改检测基准存在内容多样性不足、生成模型覆盖范围窄、可解释性不够等问题，限制了检测方法的泛化能力和解释能力。

Method: 构建ManipBench基准包含25种最先进图像编辑模型生成的45万张篡改图像；提出ManipShield模型，基于多模态大语言模型，采用对比LoRA微调和任务特定解码器实现统一检测、定位和解释。

Result: 在ManipBench和多个公共数据集上的广泛实验表明，ManipShield实现了最先进的性能，并对未见过的篡改模型表现出强泛化能力。

Conclusion: ManipBench基准和ManipShield模型有效解决了现有篡改检测方法的局限性，为AI编辑图像检测提供了全面评估和先进解决方案。
...
Method: 提出GSLR框架，使用定制的2D高斯泼溅生成潜在张量，1D高斯泼溅生成变换矩阵。这两种高斯泼溅在该表示框架中不可或缺且互补，具有强大的表示能力，特别适用于局部高频信息。

Result: 开发了无监督的GSLR多维图像恢复模型，实验表明GSLR在多项多维图像恢复任务中始终优于最先进方法，特别是在捕捉局部高频信息方面表现优异。

Conclusion: GSLR框架通过高斯泼溅技术有效解决了传统t-SVD方法的局限性，为多维图像提供了紧凑连续的表示，在局部高频信息捕捉方面具有显著优势。
...
TL;DR: VLM3D是一个利用大型视觉语言模型作为语义和空间评判器的通用框架，通过双查询评判信号解决文本到3D生成中的语义对齐和空间理解问题。


<details>
  <summary>Details</summary>
Motivation: 当前文本到3D生成模型存在两个基本限制：难以捕捉细粒度提示细节的粗粒度语义对齐问题，以及缺乏稳健3D空间理解导致的几何不一致性和部件组装失败问题。

Method: 提出VLM3D框架，重新利用大型视觉语言模型作为强大的可微分语义和空间评判器，核心贡献是基于VLM的是/否对数概率的双查询评判信号，评估语义保真度和几何一致性。

Result: VLM3D在标准基准测试中显著优于现有方法，并且作为测试时指导模块能够主动纠正最先进原生3D模型的严重空间错误。
...
### [65] [Deep Learning-Based Regional White Matter Hyperintensity Mapping as a Robust Biomarker for Alzheimer's Disease](https://arxiv.org/abs/2511.14588)
...
Result: 预测病变负荷与参考WMH估计一致，区域WMH体积在疾病分类中始终优于全局病变负荷，与脑萎缩指标结合后AUC值可达0.97，前部白质束等特定区域与诊断状态相关。
...
TL;DR: 提出了一种新的跨模态组合自蒸馏（CCSD）框架，用于处理脑肿瘤MRI分割中任意模态组合的缺失问题，通过层次化模态自蒸馏和渐进式模态组合蒸馏策略提升模型鲁棒性。
...
Method: 采用共享-特定编码器-解码器架构，结合层次化模态自蒸馏机制和渐进式模态组合蒸馏方法，在训练中模拟逐步模态丢失。

Result: 在公共脑肿瘤分割基准测试中，CCSD在各种缺失模态场景下实现了最先进的性能，表现出强大的泛化能力和稳定性。

Conclusion: CCSD框架能够有效处理任意模态组合的缺失问题，为临床脑肿瘤分割提供了可靠的解决方案。
...
TL;DR: 该研究评估了表格数据和基于MRI的表示在预测阿尔茨海默病认知衰退中的贡献，特别关注transformer衍生的MRI嵌入。研究发现临床特征在识别高风险极端情况方面表现最佳，而transformer MRI嵌入在区分认知稳定个体方面更有效。


<details>
  <summary>Details</summary>
Motivation: 准确建模阿尔茨海默病的认知衰退对于早期分层和个性化管理至关重要。表格预测因子虽然提供全局风险标记，但捕捉细微脑变化的能力有限，因此需要评估不同表示方法的预测贡献。

Method: 引入基于动态时间规整聚类的轨迹感知标签策略来捕捉认知变化的异质模式；通过无监督重建在协调和增强的MRI数据上训练3D Vision Transformer，获得保留解剖结构的嵌入；使用传统机器学习分类器和深度学习头评估预训练编码器嵌入。

Result: 临床和体积特征在预测轻度和重度进展方面达到约0.70的最高AUC；ViT模型的MRI嵌入在区分认知稳定个体方面最有效，AUC为0.71；所有方法在异质的中度组中都表现不佳。

Conclusion: 临床特征在识别高风险极端情况方面表现优异，而基于transformer的MRI嵌入对稳定性细微标记更敏感，这为AD进展建模的多模态融合策略提供了动机。
...
TL;DR: 提出NeuralBoneReg，一种自监督的表面配准框架，用于解决计算机辅助骨科手术中多模态图像配准的挑战。该方法使用3D点云作为模态无关表示，通过隐式神经距离场和MLP配准模块实现骨表面的精确配准。


<details>
  <summary>Details</summary>
Motivation: 在计算机辅助骨科手术中，术前和术中数据的模态异质性使得骨表面配准具有挑战性且容易出错。需要一种鲁棒、自动且模态无关的骨表面配准方法。

Method: NeuralBoneReg包含两个模块：学习术前骨模型的隐式神经无符号距离场（UDF），以及执行全局初始化和局部细化的MLP配准模块。该方法以自监督方式运行，无需跨受试者训练数据。

Result: 在三个多模态数据集上的评估显示，NeuralBoneReg在所有数据集上均达到或优于现有方法，在UltraBones100k上达到平均RRE/RTE为1.68°/1.86 mm，在UltraBones-Hip上为1.88°/1.89 mm，在SpineDepth上为3.79°/2.45 mm。

Conclusion: NeuralBoneReg在解剖结构和模态之间表现出强大的泛化能力，为计算机辅助骨科手术提供了鲁棒且准确的跨模态配准解决方案。
...
TL;DR: 提出了一种基于标准分割架构的视网膜动脉-静脉分割方法，通过整合脉冲分析特征，使传统U-Net能够利用时间动态信息，在时间多普勒全息图中实现准确分割。


<details>
  <summary>Details</summary>
Motivation: 传统视网膜血管分割方法仅关注空间信息，忽略了全息数据的时间丰富性，无法充分利用多普勒全息技术捕捉的血流动态行为。

Method: 使用标准分割架构，结合专门的脉冲分析流程提取的特征，使传统U-Net能够利用时间动态信息进行动脉-静脉分割。
...
TL;DR: Diff-NAF是一个针对超稀疏视角多源静态CT的迭代框架，结合神经衰减场和双分支条件扩散模型，通过角度先验引导投影合成和扩散驱动重用投影细化模块，逐步提升投影完整性和重建质量。


<details>
  <summary>Details</summary>
Motivation: 多源静态CT在超稀疏视角采样下重建质量显著下降，传统方法难以处理这种情况，需要开发新方法来提高重建质量。

Method: 提出Diff-NAF框架：1）使用超稀疏视角投影训练初始神经衰减场；2）角度先验引导投影合成策略生成新投影；3）扩散驱动重用投影细化模块优化投影；4）迭代将细化投影作为伪标签加入训练集。

Result: 在多个模拟3D CT体积和真实投影数据上的实验表明，Diff-NAF在超稀疏视角条件下实现了最佳性能。

Conclusion: Diff-NAF通过迭代细化过程，在超稀疏视角条件下能够逐步提升投影完整性和重建保真度，最终获得高质量的CT重建结果。
...
TL;DR: Dental3R是一个从稀疏口腔内照片进行3D重建的免姿态估计、图引导管道，解决了传统方法在远程正畸中的局限性，通过几何感知配对策略和小波正则化目标实现高质量重建。


<details>
  <summary>Details</summary>
Motivation: 传统口腔内扫描方法无法用于远程正畸，而稀疏智能手机图像在3D高斯泼溅应用中面临大视角基线、不一致光照和镜面表面等挑战，导致姿态和几何估计不稳定，重建结果过于平滑丢失诊断细节。

Method: 提出几何感知配对策略(GAPS)选择高价值图像对子图，改善几何初始化和减少内存使用；在恢复的姿态和点云基础上，使用小波正则化目标训练3DGS模型，通过离散小波变换强制执行带限保真度。

Result: 在950个临床病例和195个视频测试集上验证，Dental3R能有效处理稀疏无姿态输入，在牙齿咬合可视化方面实现卓越的新视角合成质量，优于现有最先进方法。

Conclusion: Dental3R为远程正畸提供了稳健、高保真度的口腔内3D重建解决方案，克服了稀疏视图重建中的频率偏差问题，保留了关键的诊断细节。
...
### [72] [ArchMap: Arch-Flattening and Knowledge-Guided Vision Language Model for Tooth Counting and Structured Dental Understanding](https://arxiv.org/abs/2511.14336)
...
Result: 在1060个正畸病例上验证，在牙齿计数、解剖分区、牙列阶段分类以及临床状况识别方面表现稳健，相比监督方法和VLM基线具有更高准确性和稳定性。
...
### [73] [Silhouette-to-Contour Registration: Aligning Intraoral Scan Models with Cephalometric Radiographs](https://arxiv.org/abs/2511.14343)
*Yiyi Miao,Taoyu Wu,Ji Jiang,Tong Chen,Zhe Tang,Zhengyong Jiang,Angelos Stefanidis,Limin Yu,Jionglong Su*

Main category: cs.CV

TL;DR: 提出DentalSCR框架，通过轮廓引导实现口腔扫描模型与侧位头颅X光片的稳定3D-2D配准，解决传统方法在真实临床条件下的配准失败问题。


<details>
  <summary>Details</summary>
Motivation: 传统基于强度的配准方法在真实临床头颅X光片条件下（存在投影放大、几何畸变、低对比度牙冠等问题）表现不稳定，容易导致配准失败或解剖学上不合理的对齐结果。

Method: 1) 构建U型中线牙轴(UMDA)建立统一解剖坐标系；2) 使用表面DRR和Gaussian splatting生成X光样投影；3) 采用对称双向Chamfer距离和分层粗到细策略优化2D相似变换。

Result: 在34个专家标注临床案例上评估，显著降低了标志点误差（特别是后牙区域），下颌区域误差分布更集中，曲线级别的Chamfer和Hausdorff距离表现优异。
...
### [74] [ARC-Chapter: Structuring Hour-Long Videos into Navigable Chapters and Hierarchical Summaries](https://arxiv.org/abs/2511.14349)
...
Method: 通过结构化流程构建了英汉双语章节数据集，统一了ASR转录、场景文本和视觉描述，形成从短标题到长摘要的多层次标注。设计了新的评估指标GRACE，结合多对一段落重叠和语义相似度。

Result: ARC-Chapter在F1分数上比之前最佳方法提升14.0%，在SODA分数上提升11.3%。同时在下游任务如YouCook2上的密集视频描述任务中也提升了最先进水平。

Conclusion: ARC-Chapter通过大规模数据和精细标注实现了视频章节划分的显著突破，展示了数据规模和标注强度的重要性，并具有良好的迁移能力。
...
### [75] [IBGS: Image-Based Gaussian Splatting](https://arxiv.org/abs/2511.14357)
...
<details>
  <summary>Details</summary>
Motivation: 现有的3D高斯溅射方法使用低阶球谐函数，难以捕捉空间变化的颜色和视角相关效果（如镜面高光）。现有方法要么使用全局纹理图（难以处理复杂场景），要么使用每个高斯的纹理图（存储开销大）。

Method: 提出基于图像的高斯溅射方法，将每个像素颜色建模为标准3DGS渲染的基础颜色与从相邻训练图像学习的残差的组合，促进准确的表面对齐。

Result: 在标准新视角合成基准测试中，该方法在渲染质量上显著优于先前的高斯溅射方法。

Conclusion: 该方法能够渲染高频细节和准确的视角相关效果，且不增加存储占用，为3D高斯溅射提供了一种高效的替代方案。
...
### [76] [A Quantitative Method for Shoulder Presentation Evaluation in Biometric Identity Documents](https://arxiv.org/abs/2511.14376)
*Alfonso Pedro Ridao*

Main category: cs.CV

TL;DR: 提出了一种肩部姿态评估算法，用于自动评估生物特征身份证件中肩部姿态的合规性。


<details>
  <summary>Details</summary>
Motivation: 国际生物特征身份证件标准要求严格符合姿态要求，但现有自动化质量评估方法缺乏对肩部姿态的定量评估方法。

Method: 使用常见姿态估计框架提供的两个肩部标志点的3D坐标，量化肩部偏航角和滚转角。

Result: 在121张肖像图像数据集上评估，SPE得分与人工标注显示出强相关性（皮尔逊相关系数约0.80），过滤性能分析证实了其识别不合规样本的有效性。
...
### [77] [BEDLAM2.0: Synthetic Humans and Cameras in Motion](https://arxiv.org/abs/2511.14394)
*Joachim Tesch,Giorgio Becherini,Prerana Achar,Anastasios Yiannakidis,Muhammed Kocabas,Priyanka Patel,Michael J. Black*

Main category: cs.CV

TL;DR: BEDLAM2.0是一个新的数据集，用于从视频中推断3D人体运动，特别是在世界坐标系中估计人体运动。它改进了原有的BEDLAM数据集，增加了更多样化和真实的摄像机、摄像机运动、身体形状、动作、服装、头发、3D环境，并添加了鞋子。


<details>
  <summary>Details</summary>
Motivation: 从视频中推断3D人体运动是一个具有挑战性的问题，许多应用需要将人体运动估计在世界坐标系中。现有方法在此方面的进展受到缺乏丰富视频数据和真实人体与摄像机运动地面真值的限制。

Method: 开发BEDLAM2.0数据集，通过增加更多样化和真实的摄像机、摄像机运动、身体形状、动作、服装、头发、3D环境以及添加鞋子来改进原有的BEDLAM数据集。

Result: BEDLAM2.0在训练估计世界坐标系中人体运动的方法方面显著优于BEDLAM。比较最先进方法在BEDLAM和BEDLAM2.0上的训练结果，发现BEDLAM2.0显著提高了准确性。

Conclusion: BEDLAM2.0是一个比BEDLAM更好的数据集，特别适用于训练在世界坐标系中估计人体运动的方法，为研究提供了渲染视频、地面真值身体参数和摄像机运动等资源。
...
### [78] [Stage Aware Diagnosis of Diabetic Retinopathy via Ordinal Regression](https://arxiv.org/abs/2511.14398)
...
Result: 序数回归方法在APTOS数据集上获得了0.8992的QWK分数，为该数据集建立了新的性能基准。

Conclusion: 所提出的序数回归框架在糖尿病视网膜病变检测方面表现出色，为及时筛查和干预提供了有效的技术手段。
...
### [79] [Language as an Anchor: Preserving Relative Visual Geometry for Domain Incremental Learning](https://arxiv.org/abs/2511.14401)
*Shuyi Geng,Tao Zhou,Yi Zhou*

Main category: cs.CV

TL;DR: LAVA是一个新颖的领域增量学习框架，通过基于文本的参考锚点实现相对对齐，解决现有方法在统一视觉空间中的域间干扰和语义失真问题。


<details>
  <summary>Details</summary>
Motivation: 现有领域增量学习方法面临两难困境：统一视觉空间导致域间干扰和语义失真，而隔离域特定参数则造成知识碎片化。需要一种能保持跨域一致性同时避免知识孤岛的方法。

Method: 提出LAVA框架，使用语言锚定的视觉对齐方法，通过基于文本的参考锚点驱动相对对齐，保持一致的相对几何结构，该结构由类别名称之间的语义相似性定义。

Result: 在标准领域增量学习基准测试上的广泛实验表明，LAVA相比现有最先进方法取得了显著的性能提升。

Conclusion: LAVA通过语言锚定的相对对齐方法有效解决了领域增量学习中的知识保持和跨域一致性问题，为处理分布漂移提供了新的解决方案。
...
### [80] [Cranio-ID: Graph-Based Craniofacial Identification via Automatic Landmark Annotation in 2D Multi-View X-rays](https://arxiv.org/abs/2511.14411)
...
TL;DR: 该研究通过模拟婴儿视觉发展阶段的视觉饮食（CATDiet）训练自监督学习模型，发现这种阶段性视觉约束能增强模型的鲁棒性，并表现出与生物发育相似的模式。基于此提出的CombDiet方法在物体识别和深度感知任务上优于标准自监督学习。


<details>
  <summary>Details</summary>
Motivation: 探索婴儿阶段性视觉发展（低清晰度、颜色退化、时间连续性）对视觉智能形成的生态优势，为机器视觉提供反向工程框架。

Method: 使用CATDiet方法训练自监督学习模型，模拟婴儿视觉约束：灰度到彩色(C)、模糊到清晰(A)、保持时间连续性(T)。建立包含10个数据集的综合基准进行评估。

Result: 所有CATDiet变体在物体识别中表现出增强的鲁棒性，模型展现出与生物发育对齐的模式，包括类似猕猴V1区突触密度的神经可塑性变化和类似婴儿视觉悬崖反应的行为。CombDiet方法在物体识别和深度感知任务上优于标准自监督学习。

Conclusion: 婴儿早期视觉经验的发展进程为理解机器中鲁棒视觉智能的出现提供了强大的反向工程框架。
...
TL;DR: DIR-TIR框架通过对话精炼和图像精炼两个模块实现交互式文本到图像检索，通过多轮对话逐步优化搜索目标，相比传统单查询方法显著提高了检索精度和用户体验。


<details>
  <summary>Details</summary>
Motivation: 解决传统文本到图像检索方法在单次查询中无法充分理解用户意图的问题，通过交互式对话提供更好的可控性和容错性。

Method: 提出DIR-TIR框架，包含对话精炼模块（主动询问用户提取关键信息）和图像精炼模块（识别图像与用户意图的感知差距），通过多轮对话逐步优化检索过程。

Result: 在多个图像数据集上的实验表明，该对话式方法显著优于仅使用初始描述的基线方法，模块协同集成实现了更高的检索精度和更好的交互体验。

Conclusion: DIR-TIR框架通过对话驱动的渐进式精炼方法，为交互式文本到图像检索提供了有效的解决方案，在检索精度和用户体验方面均表现出色。
...
TL;DR: 提出了一种物理引导的残差学习框架，通过预测BedMachine先验上的床厚度残差，从观测表面重建冰川床地形，在格陵兰岛子区域实现了优于其他方法的测试精度和结构保真度。


<details>
  <summary>Details</summary>
Motivation: 冰川床地形的精确测量对冰盖建模至关重要，但雷达观测稀疏且分布不均，需要开发能够处理稀疏数据并保持物理合理性的方法。
...
Conclusion: 残差-先验设计与物理约束相结合，产生了空间连贯、物理合理的冰川床地形，适用于领域偏移下的操作制图。
...
TL;DR: SEED-SR方法通过条件潜在扩散模型和地理空间基础模型，在分割感知的潜在空间中进行超分辨率，实现了20倍尺度因子的农场边界分割，显著提升了实例和语义分割性能。


<details>
  <summary>Details</summary>
Motivation: 解决小农户农场边界分割的挑战，传统超分辨率方法在感知质量优化时会平滑关键特征，且无法满足大尺度因子需求，两步法（先超分后分割）无法有效利用多源卫星数据。

Method: 使用条件潜在扩散模型和大规模多光谱多源地理空间基础模型，在分割感知的潜在空间中进行超分辨率，绕过像素空间的显式超分任务。

Result: 在两个大型真实数据集上，相比基于最先进Ref-SR方法的方法，在实例分割和语义分割指标上分别实现了25.5%和12.9%的相对提升。
...
Method: 引入Risk Semantic Distillation框架和RiskHead插件模块，从视觉语言模型中蒸馏因果风险估计到鸟瞰图特征，生成可解释的风险注意力图。
...
TL;DR: 提出了一种基于解码器的新框架PAMM，利用状态空间模型的丰富参数来增强多任务密集预测中的任务互联性，通过双状态空间参数专家集成任务特定参数先验，并在NYUD-v2和PASCAL-Context基准测试中验证了有效性。
...
Method: 提出参数感知Mamba模型(PAMM)，利用状态空间模型的丰富可扩展参数，采用双状态空间参数专家集成任务特定参数先验，通过结构化状态空间序列模型(S4)全局整合任务先验，并使用多方向希尔伯特扫描方法构建多角度特征序列。

Result: 在NYUD-v2和PASCAL-Context基准测试上的广泛实验证明了所提方法的有效性。

Conclusion: PAMM框架能够有效增强多任务密集预测中的任务互联性，通过状态空间模型参数和任务特定先验的集成，实现了精确的多任务交互。
...
TL;DR: D-PerceptCT是一种受人类视觉系统启发的LDCT图像增强架构，通过语义感知和全局-局部状态空间模块来增强感知相关特征，避免过度平滑并保留关键诊断细节。


<details>
  <summary>Details</summary>
Motivation: 低剂量CT图像质量下降，现有方法往往过度估计噪声并过度平滑，导致关键细节丢失，影响诊断准确性。

Method: 提出D-PerceptCT架构，包含视觉双路径提取器（集成DINOv2语义先验）和全局-局部状态空间模块，并设计深度感知相关性损失函数。

Result: 在Mayo2016数据集上的实验表明，相比SOTA方法，D-PerceptCT能更好地保留LDCT图像的结构和纹理信息。

Conclusion: D-PerceptCT通过模拟人类视觉系统，有效增强了LDCT图像质量，为放射科医生提供了感知上更清晰的关键解剖结构和病理细节。
...
### [88] [DeCo-VAE: Learning Compact Latents for Video Reconstruction via Decoupled Representation](https://arxiv.org/abs/2511.14530)
*Xiangchen Yin,Jiahui Yuan,Zhangchi Hu,Wenzhang Sun,Jie Chen,Xiaozhen Qiao,Hao Li,Xiaoyan Sun*

Main category: cs.CV

TL;DR: DeCo-VAE通过将视频内容解耦为关键帧、运动和残差三个组件，分别学习专用潜在表示，实现紧凑的视频潜在表征，避免冗余建模。
...
Conclusion: 通过解耦视频内容并学习专用潜在表示，DeCo-VAE能够有效减少冗余建模，实现紧凑且高质量的视频重建。
...
TL;DR: 提出了一种在共享空间中表示多个神经符号距离函数(SDF)的方法，通过结合泛化学习和过拟合学习的优势，使用紧凑的潜在表示恢复高保真几何细节。


<details>
  <summary>Details</summary>
Motivation: 神经SDF在表示单个形状或场景时表现良好，但在分析多个具有高保真几何细节的SDF时存在障碍，主要由于潜在空间信息有限和几何细节丢失。

Method: 结合泛化学习和过拟合学习策略的优势，提出新颖的采样策略来采样训练查询，提高训练效率并消除其他SDF影响造成的伪影。

Result: 在广泛使用的基准测试上进行数值和视觉评估，验证了设计的有效性，在表示能力和紧凑性方面优于最新方法。

Conclusion: 该方法成功克服了多个SDF分析中的障碍，能够以更紧凑的潜在表示恢复更多高保真几何细节。
...
TL;DR: 本文提出了一种无需物体先验的动态手-物体交互场景建模方法，通过交互感知的高斯表示和动态场来同时重建几何和外观，解决了遮挡、边缘模糊等挑战。


<details>
  <summary>Details</summary>
Motivation: 现有方法在手-物体交互场景建模中面临遮挡、边缘模糊和复杂动态的挑战，且通常依赖物体先验知识。本文旨在开发无需物体先验的端到端解决方案。

Method: 1) 交互感知手-物体高斯表示，引入可优化参数实现分段线性假设；2) 将手部信息融入物体变形场，构建交互感知动态场；3) 渐进式优化策略处理动态区域和静态背景；4) 显式正则化确保平滑运动、物理真实性和光照一致性。

Result: 实验表明，该方法超越了现有的动态3D高斯溅射方法，在手-物体交互动态重建方面达到了最先进的性能。

Conclusion: 提出的交互感知高斯表示和动态场方法能够有效建模复杂的手-物体交互场景，无需物体先验知识，在几何和外观重建方面表现出色。
...
### [91] [ForensicFlow: A Tri-Modal Adaptive Network for Robust Deepfake Detection](https://arxiv.org/abs/2511.14554)
*Mohammad Romani*

Main category: cs.CV

TL;DR: ForensicFlow是一个三模态深度伪造检测框架，融合RGB、纹理和频率证据，通过注意力机制动态选择关键帧并平衡各分支贡献，在Celeb-DF数据集上达到0.9752 AUC和0.9208准确率。
...
Method: 提出三模态框架：RGB分支（ConvNeXt-tiny）提取全局视觉不一致性；纹理分支（Swin Transformer-tiny）检测细粒度融合痕迹；频率分支（CNN+SE）识别周期性频谱噪声。使用注意力机制进行时间池化和自适应融合。
...
Method: 首先识别关键音频令牌，计算每个时间组的音频保留分数捕捉信息密度，动态指导视频令牌剪枝，并使用交错时空方案压缩视频令牌。
...
Result: 在Hertfordshire队列研究数据上的实验显示，该模型在回归泛化和鲁棒性方面优于基线模型。交叉注意力融合比简单特征拼接减少MSE 16.7%、MAE 6.03%，提高R2分数16.4%。

Conclusion: 该多模态方法通过交叉注意力有效整合数据，在股骨颈骨密度估计方面表现出色，在临床相关阈值下的二元分类评估显示了在真实场景中的应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有的预测算法要么独立处理每个切片而忽略3D结构，要么3D感知方法不可生成且扩展性差，需要一种可扩展且鲁棒的3D组织转录组学分析方法。
...
TL;DR: 本研究提出了BioST-GCN模型，结合姿态和生物力学信息进行跌倒预测，在模拟数据上表现优异但存在模拟-现实差距问题。


<details>
  <summary>Details</summary>
Motivation: 跌倒对老年人造成严重伤害，基于视觉的跌倒预测系统需要大量跌倒数据，但现实中此类数据稀缺。
...
TL;DR: 本文提出了一种名为SGTA-Net的新方法，通过立体几何-纹理对齐和伪特征增强几何一致性来解决稀疏视图下高斯溅射优化中的过拟合问题，实现了更准确的表面重建和高质量的新视角合成。


<details>
  <summary>Details</summary>
Motivation: 当输入视图稀疏时，高斯溅射优化容易过拟合，导致重建质量下降。现有方法使用扁平高斯基元结合深度正则化，但扁平高斯固有的各向异性加剧了稀疏视图下的过拟合问题。

Method: 提出SGTA-Net方法，核心是立体几何-纹理对齐，将渲染质量与几何估计联系起来；同时提出伪特征增强几何一致性，通过结合训练视图和未见视图来强制执行多视图几何一致性。

Result: 在DTU、BlendedMVS和Mip-NeRF360数据集上的大量实验表明，该方法达到了最先进的性能。

Conclusion: SGTA-Net能够重建更准确和详细的表面，同时保持高质量的新视角渲染，有效缓解了稀疏监督引起的过拟合问题。
...
Method: 提出SLAM-AGS框架：1）在slide-negative patches上优化弱监督相似性目标；2）在slide-positive patches上优化自监督对比目标；3）应用自适应梯度手术解决任务梯度冲突；4）集成到基于注意力的多实例学习聚合器中进行袋级预测和异常实例检索。

Result: 在公开的骨髓细胞学数据集上，模拟见证率从10%降至0.5%，SLAM-AGS在袋级F1分数和Top 400阳性细胞检索方面优于其他预训练方法，在低见证率下提升最显著。

Conclusion: 解决梯度干扰能够实现稳定的预训练，并在下游任务中获得更好的性能，特别是在低见证率场景下。开源实现促进可复现性。
...
<details>
  <summary>Details</summary>
Motivation: 为了解决LLM生成文本的潜在危害，研究者提出了水印技术，但现有技术存在降低文本质量和易受攻击的问题，阻碍了其广泛应用。

Method: 通过比较改写和回译（英语→其他语言→英语）两种对抗攻击方式，并使用语言学指标评估水印技术对文本质量和写作风格的保持能力。
...
TL;DR: RT（Refine Thought）是一种通过多次前向传递提升文本嵌入模型语义推理能力的方法，在语义推理任务上取得显著改进，同时在通用语义理解任务上保持稳定性能。


<details>
  <summary>Details</summary>
Motivation: 增强文本嵌入模型的语义推理能力，通过激活预训练期间学习到的推理能力来提升模型性能。

Method: 通过多次前向传递运行文本嵌入模型，获得最终的语义表示，这是一种测试时推理方法。

Result: 在BRIGHT和PJBenchmark1语义推理任务上取得显著改进，在C-MTEB等通用语义理解任务上保持稳定性能。

Conclusion: RT方法有效激活了仅解码器文本嵌入模型在预训练期间学习的语义推理能力，是一种有效的测试时推理方法。
...
TL;DR: 论文提出了两种无需训练的QE-informed翻译错误修正方法：第一种通过选择不同LLM生成的最佳质量翻译，第二种基于QE解释替换错误子串。第一种方法在WMT 2025任务中获胜。


<details>
  <summary>Details</summary>
Motivation: 虽然联合训练QE和APE系统能提升性能，但APE系统存在过度修正问题。本研究探索无需训练的简单方法来解决这个问题。
...
Conclusion: 无需训练的QE-informed重翻译方法在翻译错误修正任务中表现优于基于编辑的方法，证明了选择最佳翻译策略的有效性。
...
TL;DR: 该论文研究了LLM在长文本上下文中的“迷失在中间”现象，提出了GM-Extract基准数据集来评估控制变量检索性能，并开发了空间和语义检索能力评估指标。研究发现数据表示方式显著影响检索性能，缓解方法的有效性具有高度复杂性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在长范围上下文中的能力下降（“迷失在中间”现象）对基于检索的LLM应用构成重大挑战，需要在实际应用场景中研究这一现象的影响。

Method: 引入GM-Extract基准数据集，提出使用文档指标和变量提取指标的双重评估系统，系统评估7-8B参数模型在多文档任务上的表现，并进行缓解方法的文献调研和应用测试。

Result: 研究发现仅通过改变数据在上下文窗口中的表示方式就能显著改变检索性能，虽然没有一致观察到明显的U形曲线，但发现了清晰的性能模式，并与困惑度得分相关。缓解方法的有效性高度复杂，在某些场景下能改善性能，但在其他情况下会产生负面影响。

Conclusion: 该研究强调了在实用环境中理解LLM长上下文检索能力的重要性，揭示了缓解策略的复杂性和情境依赖性，为改进基于检索的LLM应用提供了重要见解。
...
TL;DR: 论文提出了一种处理电商搜索中最高级查询（如"最好"、"最受欢迎"）的框架，通过LLM提取结构化解释，提升搜索性能10.9 MAP点和5.9 MRR点，并开发了将解释转移到轻量级模型的高效方法。


<details>
  <summary>Details</summary>
Motivation: 最高级搜索查询需要在多个维度比较候选商品，需要语言理解和领域知识，现有方法难以有效处理这类复杂语义。

Method: 开发了一个框架，将查询分解为属性-值提示，与检索并发生成，然后将最高级解释转移到轻量级模型以解决延迟问题。

Result: 方法在MAP上提升10.9点，在MRR上提升5.9点，显著优于基线方法。

Conclusion: 研究展示了如何表示和转移最高级语义，推进了检索系统中的语言理解，同时解决了实际部署的延迟约束。
...
TL;DR: 提出了HiEAG框架，通过分层证据增强生成来改进多模态虚假信息检测中的外部一致性检查，整合了检索、重排序和重写三个模块，在多个基准数据集上超越了现有最优方法。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态虚假信息检测方法过于关注内部一致性，而忽视了图像-文本对与外部证据之间的外部一致性检查的重要性。

Method: 提出了HiEAG分层证据增强生成框架，包含证据检索、证据重排序（使用AESP自动证据选择提示）和证据重写（使用AEGP自动证据生成提示）三个模块，利用多模态大语言模型的广泛知识。

Result: 在不同基准数据集上的实验结果表明，HiEAG在所有样本的准确率上超越了之前的最先进方法。

Conclusion: HiEAG框架通过分层证据增强生成有效改进了外部一致性检查，在虚假信息检测任务中表现出色，并能提供判断解释。
...
TL;DR: 本文针对多标签情感分类中的类别不平衡问题，构建了平衡数据集并开发了增强模型，显著提升了分类性能。


<details>
  <summary>Details</summary>
Motivation: 现有数据集如GoEmotions存在严重的类别不平衡问题，影响了模型性能，特别是对代表性不足的情感类别。

Method: 通过整合GoEmotions原始数据、使用RoBERTa-base-GoEmotions模型标注的Sentiment140样本以及GPT-4 mini生成的手动标注文本，构建平衡数据集；开发结合FastText嵌入、卷积层、双向LSTM和注意力机制的多标签分类模型。

Result: 实验结果表明，与不平衡数据训练的模型相比，在准确率、精确率、召回率、F1分数和AUC方面均有显著提升。
...
TL;DR: 提出了一种名为Stealth Fine-Tuning的新型攻击方法，通过分段级干扰和自生成输出作为监督微调数据，能够以低成本高效地绕过RVLMs的安全对齐防御。


<details>
  <summary>Details</summary>
Motivation: 发现RVLMs暴露的思维链跟踪引入了新的攻击面，其安全对齐容易被突破，需要研究新的攻击方法来揭示这一漏洞。

Method: 采用分段级干扰引发有害推理跟踪，重用自生成输出作为监督微调数据，通过基于轮次的加权损失设计实现轻量级、分布一致的微调方法。

Result: 仅需499个样本和单张A100显卡3小时训练，攻击成功率比IDEATOR提高38.52%，同时保持通用推理能力，模型保留原始表示分布。
...
<details>
  <summary>Details</summary>
Motivation: 临床文本自动ICD编码在医疗NLP中至关重要，但由于诊断代码的极端长尾分布而受到阻碍，数千个罕见和零样本ICD代码在数据集（如MIMIC-III）中严重不足，导致宏观F1分数较低。

Method: 构建基于罕见代码的现实多标签代码集，利用真实世界的共现模式、ICD描述、同义词、分类法和相似临床笔记。使用这些结构化提示生成90,000个合成笔记，覆盖7,902个ICD代码，显著扩展训练分布。在两个最先进的基于transformer的模型（PLM-ICD和GKI-ICD）上对原始和扩展数据集进行微调。

Result: 实验表明，该方法适度提高了宏观F1，同时保持了强大的微观F1，优于先前的SOTA。虽然相对于计算成本，增益可能显得边际，但结果证明精心设计的合成数据可以增强长尾ICD代码预测的公平性。

Conclusion: 精心制作的合成数据可以增强长尾ICD代码预测的公平性，为医疗NLP中的ICD编码任务提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统知识图谱构建成本高且被视为静态数据库，而基于Transformer的关系提取方法能够从自然语言文本动态生成知识图谱，这为表示输入句子的含义提供了可能性。研究旨在利用这一效果回答多项选择题，并关注关系提取方法在提供事实错误文本时会生成包含虚假信息的KG的问题。

Method: 提出一种回答"填空"格式多项选择题的方法：(i) 使用关系提取方法将句子转换为关系图；(ii) 在封闭世界假设下，根据事实正确的知识图谱验证生成的关系图。该方法特别处理了关系提取方法可能生成虚假信息KG的问题。

Result: 实验结果表明，该方法能正确回答约70%的问题，同时提供程序的可追溯性。研究还强调问题类别对准确率有巨大影响。

Conclusion: 结合基于Transformer的关系提取和知识图谱匹配的方法能够有效回答多项选择题，并保持输出过程的可追溯性。问题类别是影响准确率的关键因素，该方法在动态生成和验证知识图谱方面表现出良好效果。
...
TL;DR: 本文提出了一种选择性弱到强泛化框架，通过训练二元分类器识别强模型能回答的问题，避免不必要的弱监督，并使用图平滑方法优化弱标签，在三个基准测试中表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决超人类模型只能获得弱监督的问题，现有弱到强泛化方法固定使用弱监督存在鲁棒性问题，部分弱标签对模型有害。

Method: 训练二元分类器P(IK)识别强模型能回答的问题，使用自生成标签进行对齐，并通过图平滑方法优化弱标签。

Result: 在三个基准测试上的广泛实验表明，该方法持续优于竞争基线，P(IK)能够跨任务和难度泛化。

Conclusion: 选择性弱到强泛化有助于超对齐，能够避免不必要的弱监督，提高模型性能。
...
TL;DR: 本文提出了首个基于符号语言知识的幻觉定位框架，通过分析符号触发器（如修饰词、否定、数字等）在模型各层的处理过程，揭示幻觉源于符号语义处理的早期崩溃。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs在符号触发器上存在严重幻觉问题，但缺乏对幻觉起源的清晰理解，需要系统性地处理这些符号触发器并在模型内部定位幻觉的产生位置。

Method: 提出符号定位框架，利用符号语言和语义知识追踪幻觉在所有模型层的发展，通过分析五个模型在HaluEval和TruthfulQA数据集上对符号触发器的处理过程。

Result: 发现符号语言元素的注意力方差在早期层（2-4）爆炸性增长达到临界不稳定性，否定触发灾难性方差水平；尽管模型规模增大，幻觉率仍保持高位（78.3%-83.7%），深层中符号语义触发器的注意力急剧下降。

Conclusion: 幻觉本质上是符号语言处理失败而非一般生成问题，符号语义知识为理解和定位LLMs中的幻觉机制提供了关键线索。
...
TL;DR: DeepEL是一个将大语言模型全面整合到实体链接所有阶段的框架，通过自验证机制利用全局上下文信息纠正预测，在10个基准数据集上显著优于现有方法。
...
Result: 在10个基准数据集上的广泛实验表明，DeepEL显著优于现有最先进方法，整体F1分数平均提升2.6%，在领域外数据集上提升4%。

Conclusion: 深度整合大语言模型能有效推进实体链接技术的最先进水平，自验证机制和全局上下文利用是提升性能的关键。
...
TL;DR: 本文提出了ArbESC+系统，这是首个用于阿拉伯语语法错误校正的多系统方法，通过组合多个模型来提升校正效果。


<details>
  <summary>Details</summary>
Motivation: 阿拉伯语具有复杂的形态和句法结构，比其它语言更具挑战性。虽然现代神经模型已有很大改进，但先前的研究大多使用单一模型，没有考虑组合不同系统的潜在优势。

Method: 使用多个模型（包括AraT5、ByT5、mT5、AraBART等）收集校正建议，将其表示为数值特征，通过分类器确定并实施适当校正。框架还使用支持技术过滤重叠校正和评估决策可靠性。

Result: 多系统组合比单一模型表现更好，在QALB-14测试数据上F0.5达到82.63%，在QALB-15 L1数据上达到84.64%，在QALB-15 L2数据上达到65.55%。

Conclusion: 这是首个集成语言错误校正的阿拉伯语尝试，改进现有模型为开发先进的阿拉伯语文本处理工具提供了实际步骤，将有益于用户和研究者。
...
TL;DR: 该论文提出了一个针对音乐领域的大语言模型训练框架，包括构建大规模音乐相关语料库（40B tokens）、实施领域优先的数据处理流程、引入基于参考模型的软评分质量控制系统，以及设计MusicSimpleQA基准来评估事实性。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在音乐等专业领域表现受限的问题，特别是在音乐娱乐领域，语料库规模、纯度和数据与训练目标的匹配度至关重要。

Method: 1) 构建大规模音乐相关自然语言语料库；2) 实施领域优先的数据处理流程（轻量级分类器过滤、多阶段清洗、去重和隐私保护）；3) 引入基于参考模型的token级软评分进行质量控制；4) 设计MusicSimpleQA基准评估事实性。

Result: 提出了一个可扩展的数据训练框架和可重用的评估工具，能够更有效地进行音乐领域的持续预训练和对齐，减少噪声梯度并放大任务对齐信号。

Conclusion: 该工作通过提供正确的语料库和训练目标，为构建音乐领域大语言模型提供了可扩展的数据训练框架和可重用的评估工具，推动了领域特定LLM的发展。
...
TL;DR: 本文提出Authentic-Dubber模型，通过检索增强的导演-演员交互学习方案，模拟真实电影配音工作流程，提升情感表达能力。


<details>
  <summary>Details</summary>
Motivation: 现有电影配音模型简化了工作流程，忽略了导演与演员之间的关键互动环节，而真实工作流程中导演会指导演员内化情感线索。

Method: 提出三种新机制：构建多模态参考素材库、基于情感相似度的检索增强策略、渐进式图基语音生成方法。

Result: 在V2C Animation基准数据集上的主观和客观评估验证了有效性，实现了情感表现力的全面提升。
...
Result: 开源ASR在自然语音中表现良好但在嘈杂非母语对话中退化；多模态LLM对口音更鲁棒但在领域特定命名实体上表现不佳；专有模型在清晰语音上准确度高但不同国家和领域差异显著。

Conclusion: 通过发布这个综合基准，使从业者和研究人员能够选择适合非洲使用场景的语音技术，促进服务不足社区的包容性语音应用发展。
...
### [115] [LiveRAG: A diverse Q&A dataset with varying difficulty level for RAG evaluation](https://arxiv.org/abs/2511.14531)
...
TL;DR: 本文提出了一种熵引导的训练框架来解决大型推理模型中的熵冲突问题，将推理长度压缩到原始的20%同时保持或提升准确率。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型的链式思维输出过长导致计算成本高和部署困难，现有压缩方法忽略了训练过程中的熵冲突现象。

Method: 采用熵引导的训练框架，在熵下降时鼓励简洁推理步骤，在熵上升时在紧凑推理模式下增强探索以提高鲁棒性。

Result: 在六个数学基准测试中，该方法将推理长度压缩到原始的20%，同时保持甚至超越了基线准确率。

Conclusion: 熵引导训练框架有效解决了熵冲突问题，实现了推理长度的大幅压缩而不损失性能。
...
TL;DR: 本文提出了一种通过预测语言化概率分布来增强LLM置信度估计的方法，该方法要求模型考虑答案空间中的所有候选答案并仔细分配置信度分数，从而促进深度推理。
...
Method: 通过预测语言化概率分布来鼓励深度推理，要求LLM考虑答案空间中的所有候选答案，而不是基于单一猜测，并仔细分配置信度分数以满足分布要求。

Result: 该方法在不同模型和各种任务中都显示出优势，无论答案空间是否已知。即使在强化学习后，其优势仍然保持，进一步分析表明其推理模式与人类期望一致。

Conclusion: 预测语言化概率分布是一种有效的置信度估计方法，能够促进深度推理，并在多种场景下保持稳定优势。
...
<details>
  <summary>Details</summary>
Motivation: 当前阿拉伯语和双语大型语言模型在知识型基准测试中得分较高，但缺乏对深层语法和句法推理能力的真实评估，需要专门的基准测试来衡量真正的语言掌握程度。

Method: 通过150个专家设计的多项选择题，直接评估结构语言理解能力，对35个阿拉伯语和双语大型语言模型进行评估。

Result: 评估显示当前模型表现出较强的表层熟练度，但在深层语法和句法推理方面存在困难，许多模型通过记忆或模式识别而非真实理解来取得成功。

Conclusion: AraLingBench通过分离和测量基本语言技能，为开发阿拉伯语大型语言模型提供了诊断框架，揭示了知识型基准测试高分与真正语言掌握之间的持续差距。
...
TL;DR: ConInstruct是一个专门评估大语言模型检测和解决用户指令中冲突能力的基准测试，研究发现专有模型在冲突检测方面表现良好，但很少明确通知用户冲突或请求澄清。
...
Result: 专有LLMs表现出强大的冲突检测能力，DeepSeek-R1和Claude-4.5-Sonnet分别获得91.5%和87.3%的平均F1分数；但LLMs很少明确通知用户冲突或请求澄清。
...
Method: 开发包含7个核心科学领域的原创问题集，采用多阶段专家评审和对抗测试确保质量，并引入LLM评委小组进行自动化评估。

Result: 初步结果显示ATLAS能有效区分领先模型的科学推理能力。

Conclusion: ATLAS将发展成长期开放的社区驱动平台，为人工通用智能发展提供可靠评估标准。
...
TL;DR: 本文提出标准化上下文校准（NCC）方法，解决LLMs中多标记类标签导致的标签长度偏差问题，在多个数据集和模型上实现显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在预测候选选项时存在标签偏差，现有校准方法忽视了多标记类标签产生的偏差，特别是标签长度不一致导致的预测不一致问题。

Method: 提出标准化上下文校准（NCC），在全标签级别对预测进行标准化和校准，有效缓解标签长度偏差。

Result: NCC在多个数据集和模型上相比先前方法获得统计显著改进，F1分数提升高达10%，并能扩展到更广泛的任务如多项选择题回答。

Conclusion: 缓解全标签偏差对于提高LLM方法的性能和鲁棒性至关重要，特别是在类标签自然包含多个标记的实际应用中。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM在教育应用中存在安全漏洞，现有研究主要关注通用安全评估，缺乏针对教育场景独特安全需求的专门研究。

Method: 构建EduHarm基准包含五个教育场景的安全-不安全指令对；提出三阶段防护框架：安全感知注意力重对齐、分层安全判断、防御驱动的双路由机制。

Result: 在八种越狱攻击策略上的实验显示TSSF有效增强安全性且不过度拒绝良性查询；在三个微调攻击数据集上的评估表明其能持续实现稳健防御同时保持良性微调的效用增益。

Conclusion: TSSF框架能够同时缓解越狱和微调攻击，为教育LLM提供有效的安全保障，同时保持对良性查询的正常处理能力。
...
TL;DR: MedBench v4是一个全国性的云基准测试平台，包含70多万个专家策划的医疗任务，评估了15个前沿模型。基础LLM平均得分54.1/100，但安全性和伦理得分较低；多模态模型表现更差；基于相同骨干的智能体显著提升了端到端性能。
...
Method: 构建包含70多万个任务的基准测试基础设施，涵盖24个主要专科和91个次要专科，采用多阶段精炼和多轮临床医生评审，使用LLM作为评分员校准人类评分。

Result: 基础LLM平均得分54.1/100（最佳：Claude Sonnet 4.5，62.5/100），安全性和伦理得分仅18.4/100；多模态模型平均47.5/100；基于相同骨干的智能体平均得分79.8/100，显著提升性能。

Conclusion: MedBench v4揭示了基础模型在多模态推理和安全性方面的持续差距，同时表明具有治理意识的智能体编排可以显著提升临床准备度，为医院、开发者和政策制定者提供实用参考。
...
### [124] [Tell Me: An LLM-powered Mental Well-being Assistant with RAG, Synthetic Dialogue Generation, and Agentic Planning](https://arxiv.org/abs/2511.14445)
*Trishala Jayesh Ahalpara*

Main category: cs.CL

TL;DR: Tell Me是一个心理健康系统，利用大语言模型提供个性化、情境感知的心理支持，包括RAG对话助手、合成治疗对话生成器和AI自我关怀规划器。
...
Method: 系统集成三个组件：RAG助手提供个性化对话，合成对话生成器基于用户档案生成治疗对话，AI团队使用CrewAI生成周度自我关怀计划和冥想音频。

Result: 系统在精心设计的心理健康场景中通过自动LLM评估和用户研究进行了评估，展示了可行的AI辅助心理健康支持方案。

Conclusion: 该系统展示了对话助手如何降低支持门槛，补充现有护理，并拓宽心理健康资源获取途径，为NLP研究者与心理健康专业人士的跨学科合作提供了机会。
...
### [125] [Agent-R1: Training Powerful LLM Agents with End-to-End Reinforcement Learning](https://arxiv.org/abs/2511.14460)
*Mingyue Cheng,Jie Ouyang,Shuo Yu,Ruiran Yan,Yucong Luo,Zirui Liu,Daoyu Wang,Qi Liu,Enhong Chen*

Main category: cs.CL

TL;DR: 本文系统梳理了针对LLM Agent的强化学习方法，并提出了一个模块化、灵活的训练框架Agent-R1，通过多跳问答任务验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: LLM Agent在复杂问题解决中具有潜力，但强化学习在该领域的应用仍处于早期阶段，缺乏专门的方法论和灵活的训练框架。

Method: 1. 将马尔可夫决策过程框架系统扩展到LLM Agent的关键组件定义；2. 提出模块化、用户友好的训练框架Agent-R1，支持跨任务场景和环境适配。

Result: 在多跳问答基准任务上的实验为所提方法和框架的有效性提供了初步验证。

Conclusion: 本文为RL-based LLM Agent训练提供了系统的方法论和实用的框架支持，有助于推动该领域的发展。
...
TL;DR: 本文探讨了文档级声明提取的评估方法，提出通过对齐分数计算两个声明集的相似度，并在捷克和斯洛伐克新闻评论数据集上进行实验，揭示了当前评估方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 文档级声明提取在事实核查领域仍面临挑战，现有的声明集评估方法关注有限，需要建立可靠的评估框架来比较模型提取和人工标注的声明集。

Method: 研究声明集对齐技术，通过识别最佳对齐和评估方法来计算声明集之间的相似度对齐分数，在捷克和斯洛伐克新闻评论数据集上进行实验验证。

Result: 实验结果揭示了当前评估方法在文档级声明提取中的局限性，特别是在处理非正式语言、强本地语境和语言细微差别时表现不足。
...
TL;DR: 本研究使用电子健康记录数据，通过聚类分析识别急性肾损伤（AKI）患者的不同临床状态，并利用多状态模型估计向慢性肾病（CKD）进展的风险。研究发现15种不同的AKI后状态，每种状态具有不同的CKD发展概率，并识别了传统和新型的CKD风险因素。


<details>
  <summary>Details</summary>
Motivation: AKI患者发展为CKD的风险很高，但识别高风险患者仍然具有挑战性。本研究旨在通过数据驱动的方法动态追踪AKI患者的临床演变，以更好地理解AKI向CKD的进展过程。

Method: 使用电子健康记录数据，通过纵向医疗代码和肌酐测量值构建患者向量，采用聚类分析识别AKI后临床状态，使用多状态模型估计状态间转移概率和CKD进展风险，并通过生存分析识别AKI亚群中的CKD风险因素。

Result: 在20,699名入院时患有AKI的患者中，3,491名（17%）发展为CKD。识别出15种不同的AKI后状态，75%的患者在研究期间保持单一状态或仅发生一次状态转移。确定了传统风险因素（AKI严重程度、糖尿病、高血压、心力衰竭、肝病）和新型风险因素，其影响在不同临床状态间存在差异。
...
TL;DR: 本文比较了人类标注与多种大语言模型在政治偏见检测方面的表现，发现RoBERTa与人类标注最一致，GPT在零样本设置下表现最佳，强调需要结合人类可解释性和模型可扩展性的混合评估框架。
...
Result: 传统基于Transformer的模型中，RoBERTa与人类标签对齐度最高；生成式模型如GPT在零样本设置下与人类标注整体一致性最强；微调后的RoBERTa模型在所有基线中获得最高准确度和最强的人类标注对齐度。

Conclusion: 人类与LLMs在政治倾向感知上存在系统性差异，需要在自动媒体偏见检测中结合人类可解释性和模型可扩展性的混合评估框架。
...
TL;DR: 使用视觉语言模型作为评判者的多智能体系统能够通过将图表作为可验证检查点来改进端到端自主科学发现，在宇宙学和天体化学案例中展示了从错误推理路径恢复和适应新数据集的能力。


<details>
  <summary>Details</summary>
Motivation: 提高自主科学发现的效率和准确性，通过视觉语言模型评估图表并实时纠正错误，减少人工干预需求。

Method: 采用多智能体系统，使用视觉语言模型作为评判者，通过动态生成的领域特定评分标准评估图表，使智能体能够自我纠错并实时引导探索性数据分析。

Result: 在10个数据驱动发现任务的基准测试中，VLM增强系统达到0.7-0.8的pass@1分数，显著优于仅代码（0.2-0.3）和代码加文本（0.4-0.5）的基线方法，同时提供可审计的推理轨迹。

Conclusion: 视觉语言模型引导的多智能体系统能够有效提升自主科学发现的性能，实现从错误中恢复和适应新数据集，同时提供更好的可解释性。
...
TL;DR: RareSeek R1是一个专门用于罕见病诊断的AI系统，通过领域特定的临床语料库、指令调优和检索增强技术，在多中心电子健康记录和公共基准测试中达到最先进水平，性能与经验丰富的医生相当。
...
Method: 构建大型领域特定临床语料库和临床医生验证的推理集，通过分阶段指令调优、思维链学习和基于图的检索来开发RareSeek R1系统。

Result: 在多中心电子健康记录叙述和公共基准测试中，RareSeek R1达到最先进准确率，具有稳健的泛化能力，在噪声或重叠表型下保持稳定性。检索增强在叙述与优先变异配对时带来最大收益。

Conclusion: 这项工作推进了以叙述为先、知识整合的推理范式，缩短诊断旅程，并提供可审计、临床可转化的决策支持，透明推理突出了支持正确诊断的关键非表型证据。
...
TL;DR: 本研究通过贝叶斯推理和噪声通道模型解释比较幻觉现象，开发定量模型预测幻觉强度，并发现代词与完整名词短语的影响差异。


<details>
  <summary>Details</summary>
Motivation: 先前研究认为比较幻觉现象可用贝叶斯推理和噪声通道模型解释，但缺乏对幻觉强度精细变化的定量预测，特别是未解释代词与完整名词短语的影响差异。

Method: 结合统计语言模型与人类行为数据，开发后验概率定量模型，直接预测比较幻觉的强度变化。

Result: 模型成功解释了比较幻觉效应的精细梯度变化，并揭示了代词与完整名词短语在than从句主语中产生的先前未解释的影响差异。

Conclusion: 研究支持噪声通道理论作为语言处理现象的统一计算级理论，证明该理论能够对比较幻觉现象做出新颖且经验证实的预测。
...
### [132] [TaoSearchEmb: A Multi-Objective Reinforcement Learning Framework for Dense Retrieval in Taobao Search](https://arxiv.org/abs/2511.13885)
*Xingxian Liu,Dongshuai Li,Tao Wen,Jiahui Wan,Gui Ling,Fuyu Lv,Dan Ou,Haihong Tang*

Main category: cs.IR

TL;DR: 提出Retrieval-GRPO框架，使用多目标强化学习解决电商密集检索中硬负样本依赖和跷跷板效应问题，通过动态检索和LLM奖励模型实现实时优化。


<details>
  <summary>Details</summary>
Motivation: 传统BERT架构的密集检索依赖复杂的离线硬负样本构建流程，限制了模型迭代效率和语义表示能力的进化潜力，同时多任务学习框架在优化相关性和非相关性目标时面临跷跷板效应。

Method: 使用多目标强化学习框架，动态检索Top-K候选商品，引入相关性LLM作为奖励模型生成实时反馈，结合LLM生成的相关性分数、商品质量分数和多路排他性指标进行多目标优化。

Result: 消除了对硬负样本的依赖，通过协同多目标优化缓解了跷跷板效应，显著提升了模型对复杂长尾查询的语义泛化能力。

Conclusion: Retrieval-GRPO框架在离线和在线实验中验证了有效性，已部署在中国最大的电商平台上。
...
### [133] [NeuroPath: Neurobiology-Inspired Path Tracking and Reflection for Semantically Coherent Retrieval](https://arxiv.org/abs/2511.14096)
*Junchen Li,Rongzheng Wang,Yihong Huang,Qizhi Chen,Jiasheng Zhang,Shuang Liang*

Main category: cs.IR

TL;DR: NeuroPath是一个基于LLM驱动的语义路径追踪RAG框架，通过动态路径追踪和后检索补全两个步骤，解决了传统图基RAG在多跳问答中语义连贯性差和噪声干扰的问题，在多个数据集上显著优于现有方法。
...
Method: 受神经生物学中位置细胞路径导航规划的启发，NeuroPath包含两个步骤：动态路径追踪（在知识图上执行目标导向的语义路径追踪和剪枝）和后检索补全（使用中间推理和原始查询进行第二阶段检索以完善查询目标并补全推理路径中的缺失信息）。

Result: 在三个多跳QA数据集上，NeuroPath相比先进的图基RAG方法在recall@2和recall@5上分别平均提升16.3%和13.5%；相比现有迭代式RAG方法，准确率更高且token消耗减少22.8%；在四个小型LLM上表现出鲁棒性，并在不同复杂度任务上验证了可扩展性。

Conclusion: NeuroPath通过神经生物学启发的语义路径追踪机制，有效提升了多跳问答中RAG的性能，在保持语义连贯性的同时显著减少了噪声干扰，为复杂知识推理任务提供了有效的解决方案。
...
### [134] [LLM-Aligned Geographic Item Tokenization for Local-Life Recommendation](https://arxiv.org/abs/2511.14221)
*Hao Jiang,Guoquan Wang,Donglin Zhou,Sheng Yu,Yang Zeng,Wencong Zeng,Kun Gai,Guorui Zhou*

Main category: cs.IR

TL;DR: LGSID是一个用于本地生活推荐的LLM对齐地理项目标记化框架，通过强化学习对齐和分层地理标记化来增强空间感知能力，在快手数据集上优于现有方法。
...
Result: 在真实世界快手行业数据集上的广泛实验表明，LGSID持续优于最先进的判别性和生成性推荐模型。

Conclusion: 消融研究、可视化和案例研究进一步验证了该框架的有效性，能够成功捕捉细粒度空间特征和真实世界距离感知。
...
### [135] [Infer As You Train: A Symmetric Paradigm of Masked Generative for Click-Through Rate Prediction](https://arxiv.org/abs/2511.14403)
*Moyu Zhang,Yujun Jin,Yun Chen,Jinxin Hu,Yu Zhang,Xiaoyi Zeng*

Main category: cs.IR

TL;DR: SGCTR提出了一种对称掩码生成范式，在CTR预测中实现训练和推理阶段的对称性，通过生成能力在推理阶段重新定义输入样本特征以提高预测准确性。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型在训练阶段使用生成范式进行表示学习，但在在线推理时仍回归到判别范式，未能充分利用生成能力来提升预测精度，这种训练和推理阶段的不对称性限制了生成范式的潜力。

Method: SGCTR在训练阶段学习特征依赖关系获得生成能力，在在线推理阶段应用生成能力迭代重新定义输入样本特征，从而减轻噪声特征的影响。

Result: 大量实验验证了SGCTR的优越性，表明在训练和推理阶段对称应用生成范式能显著释放其在CTR预测中的潜力。

Conclusion: 对称地在训练和推理阶段应用生成范式能够显著提升CTR预测性能，解决了传统方法中训练与推理不对称的问题。
...
### [136] [Jasper-Token-Compression-600M Technical Report](https://arxiv.org/abs/2511.14405)
*Dun Zhang,Ziyang Zeng,Yudong Zhou,Shuyang Lu*

Main category: cs.IR

TL;DR: 本文介绍了Jasper-Token-Compression-600M模型，这是一个双语（英中）文本压缩模型，通过知识蒸馏和动态调整压缩率的token压缩模块，在保持8B模型性能的同时实现了比传统0.6B模型更高的效率。


<details>
  <summary>Details</summary>
Motivation: 将基于蒸馏的方法从英语Stella和Jasper模型扩展到双语领域，并通过对比学习提升模型性能，同时开发更高效和鲁棒的文本压缩表示。

Method: 结合知识蒸馏与token压缩技术，引入基于一维卷积的token压缩模块，在训练过程中动态调整压缩率，并融入对比学习。

Result: 模型在保持与8B模型相当性能的同时，效率高于传统0.6B模型，在嵌入质量和推理效率方面均有显著提升。

Conclusion: 提出的方法成功实现了高效的双语文本压缩，证明了知识蒸馏与动态token压缩结合的有效性，为大规模语言模型的高效部署提供了可行方案。
...
TL;DR: SpatiaLite是一个完全合成的基准测试，用于评估视觉语言模型的空间推理能力和效率。研究发现先进VLMs主要依赖语言表示进行空间推理，在需要视觉空间关系和3D几何变换的任务上表现不佳，且推理效率低下。作者提出了图像驱动框架来改进VLMs的空间推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前先进的视觉语言模型在逻辑推理、问题解决和决策制定方面表现出色，但在空间推理这一人类认知的基本组成部分上仍存在显著挑战。作者假设想象力（空间状态的内部模拟）是空间世界模型中的主要推理机制。

Method: 引入SpatiaLite基准测试，联合测量空间推理准确性和效率。通过综合实验分析VLMs的空间推理机制，并提出图像驱动框架（IDF）进行数据合成和训练，以隐式构建内部世界模型。

Result: 发现三个关键结果：1）先进VLMs主要依赖语言表示，在视觉中心任务上表现不足；2）VLMs的空间推理机制效率低下，token使用随变换复杂度快速增长；3）IDF框架能有效构建内部世界模型。

Conclusion: 该工作界定了先进VLMs的空间推理限制和模式，识别了关键缺陷，并为未来改进提供了指导。SpatiaLite基准为评估和提升VLMs的空间推理能力提供了重要工具。
...
### [138] [Causal computations in Semi Markovian Structural Causal Models using divide and conquer](https://arxiv.org/abs/2511.13852)
...
<details>
  <summary>Details</summary>
Motivation: 现有的方法仅适用于马尔可夫结构因果模型，其中每个外生变量只影响单个内生变量。然而，半马尔可夫模型能够表示更复杂的混杂关系，这在现实应用中更为常见。

Method: 通过最小示例说明扩展的挑战，并提出一组替代解决方案策略，包括理论分析和计算研究评估。

Result: 研究揭示了将现有方法扩展到半马尔可夫模型的困难，并提出了可行的替代策略。

Conclusion: 成功识别了将马尔可夫模型因果概率边界计算方法扩展到半马尔可夫模型的挑战，并开发了有前景的解决方案策略，为处理更复杂的混杂关系提供了理论基础。
...
### [139] [Jailbreaking Large Vision Language Models in Intelligent Transportation Systems](https://arxiv.org/abs/2511.13892)
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在多模态推理和实际应用中表现出强大能力，但极易受到越狱攻击，特别是在智能交通系统等关键基础设施中，这种安全漏洞可能带来严重风险。

Method: 首先构建了包含交通相关有害查询的数据集；其次提出通过图像排版操纵和多轮提示的新型越狱攻击方法；最后设计了多层响应过滤防御技术来防止模型生成不当响应。

Result: 在开源和闭源的最先进LVLMs上进行了广泛实验，使用GPT-4判断和人工验证评估攻击方法和防御技术的毒性得分，结果表明图像排版操纵和多轮提示的越狱攻击在ITS集成的LVLMs中存在严重安全风险。

Conclusion: 智能交通系统中集成的大型视觉语言模型面临严重的越狱攻击风险，提出的多层防御技术能有效防止不当响应生成，但需要进一步加强对这类安全威胁的防护。
...
### [140] [CORGI: Efficient Pattern Matching With Quadratic Guarantees](https://arxiv.org/abs/2511.13942)
*Daniel Weitekamp*

Main category: cs.AI

TL;DR: CORGI是一种新的匹配算法，针对规则系统中指数级时间和空间复杂度的匹配问题，提供二次时间空间保证，能够迭代流式输出匹配结果而无需将整个冲突集存入内存。


<details>
  <summary>Details</summary>
Motivation: 解决规则系统中由于自动生成规则导致的指数级匹配复杂度问题，避免内存溢出和执行延迟，使在线AI系统在实际应用中更加可靠。

Method: 采用两步法：前向传递构建/维护接地关系图，后向迭代器按需生成匹配，不同于RETE的β内存收集部分匹配。

Result: 在性能评估中，CORGI在简单组合匹配任务上显著优于SOAR和OPS5的RETE实现。

Conclusion: CORGI算法通过消除传统冲突集的内存负担，为实时AI系统和关系数据库查询提供了更可靠和高效的匹配解决方案。
...
### [141] [Scene Graph-Guided Generative AI Framework for Synthesizing and Evaluating Industrial Hazard Scenarios](https://arxiv.org/abs/2511.13970)
...
<details>
  <summary>Details</summary>
Motivation: 训练视觉模型准确检测工作场所危险需要真实的不安全条件图像，但获取此类数据集很困难，因为捕捉事故触发场景几乎不可能。

Method: 使用GPT-4o分析OSHA叙述提取结构化危险推理，转换为对象级场景图，然后指导文本到图像扩散模型生成构图准确的危险场景。引入视觉问答框架评估生成数据。

Result: 提出的VQA图分数在基于熵的验证中优于CLIP和BLIP指标，确认其具有更高的判别敏感性。
...
### [142] [ALEX:A Light Editing-knowledge Extractor](https://arxiv.org/abs/2511.14018)
...
Method: ALEX采用分层内存架构组织知识更新为语义簇，包含推理查询合成模块弥合查询与事实之间的语义鸿沟，以及动态证据裁决引擎执行高效的两阶段检索过程。
...
### [143] [Syn-STARTS: Synthesized START Triage Scenario Generation Framework for Scalable LLM Evaluation](https://arxiv.org/abs/2511.14023)
*Chiharu Hagiwara,Naoki Nonaka,Yuhta Hashimoto,Ryu Uchimido,Jun Seita*

Main category: cs.AI

TL;DR: Syn-STARTS是一个使用LLM生成分诊案例的框架，用于解决大规模伤亡事件中缺乏足够真实数据的问题，验证了合成数据在开发高性能AI模型中的潜力。


<details>
  <summary>Details</summary>
Motivation: 大规模伤亡事件中的分诊决策至关重要，但此类事件罕见且难以积累足够真实数据用于AI开发，因此需要创建合成数据来支持研究。

Method: 开发了Syn-STARTS框架，利用大语言模型生成分诊案例，并与人工整理的TRIAGE开放数据集进行质量比较验证。

Result: Syn-STARTS生成的分诊案例在质量上与人工整理的数据集难以区分，使用START标准分诊方法评估LLM准确性时，在绿色、黄色、红色和黑色四个类别中表现出高度稳定性。

Conclusion: 合成数据在开发针对严重和关键医疗情况的高性能AI模型方面具有巨大潜力，为解决真实数据稀缺问题提供了有效途径。
...
### [144] [Making Evidence Actionable in Adaptive Learning](https://arxiv.org/abs/2511.14052)
*Amirreza Mehrabi,Jason W. Morphew,Breejha Quezada,N. Sanjay Rebello*

Main category: cs.AI

TL;DR: 该研究提出了一种由教师主导的反馈循环系统，将概念级评估证据转化为经过验证的微干预措施，通过三种保障机制（充分性、注意力、多样性）实现自适应学习。
...
Method: 将干预分配形式化为带约束的二元整数规划，包括覆盖率、时间、难度窗口、先决条件概念矩阵和反冗余多样性约束。使用贪心选择、梯度松弛和混合方法进行求解。

Result: 在模拟和1204名学生的物理课程部署中，两种求解器都能在有限观看时间内为几乎所有学习者实现完整的技能覆盖。梯度方法比贪心方法减少了约12%的冗余覆盖，并在资源丰富时协调难度分布。
...
### [145] [APD-Agents: A Large Language Model-Driven Multi-Agents Collaborative Framework for Automated Page Design](https://arxiv.org/abs/2511.14101)
*Xinpeng Chen,Xiaofeng Han,Kaihao Zhang,Guochao Ren,Yujie Wang,Wenhao Cao,Yang Zhou,Jianfeng Lu,Zhenbo Song*

Main category: cs.AI

TL;DR: APD-agents是一个基于大语言模型的多智能体框架，用于移动应用的自动化页面设计，通过多个智能体协作完成从用户描述到最终页面布局的生成。


<details>
  <summary>Details</summary>
Motivation: 移动应用页面布局设计耗时且需要专业训练，现有设计软件使用复杂，跨页面协作设计需要额外时间确保样式一致性。

Method: 提出包含OrchestratorAgent、SemanticParserAgent、PrimaryLayoutAgent、TemplateRetrievalAgent和RecursiveComponentAgent的多智能体框架，通过动态协作将用户描述转换为结构化数据并生成页面布局。
...
### [146] [PathMind: A Retrieve-Prioritize-Reason Framework for Knowledge Graph Reasoning with Large Language Models](https://arxiv.org/abs/2511.14256)
*Yu Liu,Xixun Lin,Yanmin Shang,Yangxi Li,Shi Wang,Yanan Cao*

Main category: cs.AI

TL;DR: PathMind是一个新颖的KG推理框架，通过选择性引导LLM使用重要推理路径来增强忠实和可解释的推理。它采用“检索-优先排序-推理”范式，包含路径优先排序机制和双阶段训练策略，在复杂推理任务上表现优异。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的KG推理方法存在两个关键限制：1）不加区分地提取推理路径，可能引入无关噪声误导LLM；2）动态探索推理路径需要高检索需求和频繁LLM调用。

Method: PathMind遵循“检索-优先排序-推理”范式：首先通过检索模块从KG中检索查询子图；其次引入路径优先排序机制，使用语义感知路径优先级函数识别重要推理路径；最后通过双阶段训练策略（任务特定指令调优和路径偏好对齐）生成准确响应。

Result: 在基准数据集上的广泛实验表明，PathMind始终优于竞争基线，特别是在复杂推理任务上，通过识别关键推理路径，使用更少的输入token实现更好性能。

Conclusion: PathMind通过选择性引导LLM使用重要推理路径，有效解决了现有方法的局限性，在KG推理任务中实现了更忠实、可解释和高效的推理性能。
...
### [147] [Run, Ruminate, and Regulate: A Dual-process Thinking System for Vision-and-Language Navigation](https://arxiv.org/abs/2511.14131)
...
TL;DR: 本文提出了R3框架，通过双过程思维将LLM的泛化能力与VLN专业模型结合，在零样本情况下显著提升了视觉语言导航任务的性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的视觉语言导航方法存在两个主要问题：一是LLM难以精确理解真实世界空间关系，导致任务完成性能与领域专家存在差距；二是LLM计算成本高、推理延迟大。

Method: 提出R3框架，包含三个核心模块：Runner（轻量级专家模型负责常规导航）、Ruminator（多模态LLM进行结构化推理）、Regulator（根据三个标准监控进度并控制思维模式）。

Result: 在REVERIE基准测试中，SPL和RGSPL分别超过最先进方法3.28%和3.30%，显著提升了挑战性VLN任务的处理效果。

Conclusion: R3框架通过整合LLM的泛化能力和VLN专业模型，在零样本情况下实现了高效准确的导航，证明了双过程思维在复杂导航任务中的有效性。
...
### [148] [Do Large Language Models (LLMs) Understand Chronology?](https://arxiv.org/abs/2511.14214)
*Pattaraphon Kenny Wongchamcharoen,Paul Glasserman*

Main category: cs.AI

TL;DR: 本文测试了大型语言模型在时间顺序任务中的表现，发现模型在局部顺序保持良好但难以维持全局一致的时间线，而分配显式推理预算有助于改善时间排序性能。


<details>
  <summary>Details</summary>
Motivation: 在金融和经济学中，大型语言模型越来越多地被使用，但基于提示的尝试对抗前瞻性偏差隐含地假设模型理解时间顺序。本文旨在测试这一基本问题。

Method: 设计了一系列时间顺序任务，包括时间排序、条件排序（先过滤后排序）和时代错误检测，评估了GPT-4.1、Claude-3.7 Sonnet（有/无扩展思维）和GPT-5在不同推理努力设置下的表现。

Result: 随着序列长度增加，精确匹配率急剧下降，而排名相关性保持较高；在条件排序中，大多数失败源于过滤步骤而非排序步骤；时代错误检测是最简单的任务，但性能仍随时间线或实体重叠增加而下降。

Conclusion: 当前LLM在时间任务上存在限制，分配显式推理预算有助于时间排序，GPT-5在中等/高推理努力下在所有长度上实现完美排序和完美条件排序，这些发现对LLM在金融中的实时应用具有重要意义。
...
### [149] [Listen Like a Teacher: Mitigating Whisper Hallucinations using Adaptive Layer Attention and Knowledge Distillation](https://arxiv.org/abs/2511.14219)
*Kumud Tripathi,Aditya Srinivas Menon,Aman Gaurav,Raj Prakash Gohil,Pankaj Wasnik*

Main category: cs.AI

TL;DR: 本文提出了一种两阶段架构来减少Whisper模型在噪声条件下的幻觉错误，包括自适应层注意力增强编码器鲁棒性和多目标知识蒸馏抑制幻觉。
...
Method: 第一阶段通过自适应层注意力将编码器层分组为语义连贯的块，使用可学习多头注意力融合块表示；第二阶段使用多目标知识蒸馏框架，在噪声音频上训练学生模型以对齐其语义和注意力分布。

Result: 在噪声语音基准测试中显示出幻觉和词错误率的显著降低，同时保持干净语音上的性能。
...
### [150] [DevPiolt: Operation Recommendation for IoT Devices at Xiaomi Home](https://arxiv.org/abs/2511.14227)
...
Result: 在多个数据集上显著优于基线模型，所有指标平均提升69.5%；在小米家庭应用中部署一个季度，为25.5万用户提供日常操作推荐，在线实验显示独立访客设备覆盖率提升21.6%，页面查看接受率提升29.1%。
...
### [151] [Enhancing Regional Airbnb Trend Forecasting Using LLM-Based Embeddings of Accessibility and Human Mobility](https://arxiv.org/abs/2511.14248)
*Hongju Lee,Youngjun Park,Jisun An,Dongman Lee*

Main category: cs.AI

TL;DR: 本研究提出了一种新颖的时间序列预测框架，用于预测区域层面的Airbnb关键指标（收入、预订天数、预订数量），通过整合房源特征和外部环境因素构建区域表示，使用LLM生成区域嵌入，结合先进时间序列模型，在首尔数据集上相比传统基线方法将RMSE和MAE降低了约48%。
...
Method: 采用滑动窗口方法预测1-3个月的趋势，构建区域表示时整合房源特征和外部环境因素（如城市可达性和人口流动性），将结构化表格数据转换为基于提示的LLM输入以生成全面区域嵌入，然后将这些嵌入输入先进时间序列模型（RNN、LSTM、Transformer）以更好地捕捉复杂的时空动态。
...
### [152] [When Words Change the Model: Sensitivity of LLMs for Constraint Programming Modelling](https://arxiv.org/abs/2511.14334)
*Alessio Pellegrino,Jacopo Mauro*

Main category: cs.AI

TL;DR: 本文研究了LLMs在自动生成优化和约束编程模型时的真实能力，发现其成功可能源于训练数据中的标准问题而非真正推理，通过重新表述和扰动CSPLib问题验证了这一点。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs在自动生成优化和约束编程模型时的真实推理能力，检验其成功是否源于数据污染而非真正理解问题结构。

Method: 系统性地重新表述和扰动一组著名的CSPLib问题，保持其结构但修改上下文并引入误导元素，然后比较三个代表性LLMs在原始和修改描述下生成的模型。

Result: LLMs能生成语法有效且语义合理的模型，但在上下文和语言变化下性能急剧下降，显示出浅层理解和对措辞的敏感性。

Conclusion: LLMs在自动生成优化模型方面的能力有限，主要依赖于训练数据中的模式匹配而非真正的推理理解。
...
### [153] [Operationalizing Pluralistic Values in Large Language Model Alignment Reveals Trade-offs in Safety, Inclusivity, and Model Behavior](https://arxiv.org/abs/2511.14476)
*Dalia Ali,Dora Zhao,Allison Koenecke,Orestis Papakyriakopoulos*

Main category: cs.AI

TL;DR: 本研究探讨了在LLM对齐过程中纳入多元化价值观的影响，通过系统评估人口统计变量和设计参数，发现不同社会群体在毒性、情感意识等维度上存在显著评分差异，技术设计选择对模型行为有重要影响。


<details>
  <summary>Details</summary>
Motivation: 当前LLM的对齐决策往往忽视人类社会的多样性，本研究旨在探索如何将多元化价值观纳入对齐流程，以平衡安全性和公平代表性。

Method: 收集来自美国和德国参与者的27,375个评分数据，在毒性、情感意识等五个维度上评估LLM回答，使用不同社会群体的偏好对多个大语言模型进行微调，并比较不同评分尺度、分歧处理方法和优化技术。
...
### [154] [Rate-Distortion Guided Knowledge Graph Construction from Lecture Notes Using Gromov-Wasserstein Optimal Transport](https://arxiv.org/abs/2511.14595)
*Yuan An,Ruhma Hashmi,Michelle Rogers,Jane Greenberg,Brian K. Smith*

Main category: cs.AI

TL;DR: 该论文提出了一种基于率失真理论和最优传输几何的知识图谱构建与优化框架，用于从非结构化教育材料自动生成高质量选择题。


<details>
  <summary>Details</summary>
Motivation: 将非结构化教育材料（如讲义和幻灯片）转化为能捕捉关键教学内容的面向任务的知识图谱仍然很困难，需要一种原则性的方法来构建紧凑且信息保留的知识图谱。

Method: 将讲义内容建模为度量-测度空间，使用融合Gromov-Wasserstein耦合量化语义失真，通过精炼操作符（添加、合并、拆分、移除、重连）最小化率失真拉格朗日量。

Result: 在数据科学讲义上的原型应用显示，从优化知识图谱生成的选择题在15个质量标准上始终优于从原始笔记生成的选择题。
