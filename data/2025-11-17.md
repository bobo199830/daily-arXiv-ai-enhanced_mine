...
TL;DR: 本文提出了一个分析框架来研究AI能力是否会无限增长的问题，通过物理和信息理论限制定义服务边界，建立内生增长模型来区分超线性和次临界状态，并开发可测试的决策规则来验证或排除AI奇点。


<details>
  <summary>Details</summary>
Motivation: 研究AI系统在何种可测量条件下会出现能力无限增长（失控增长），以及在何种条件下可以排除这种可能性，用可测试的条件取代推测。

Method: 开发递归自我改进的分析框架，将能力增长与资源建设和部署策略联系起来，利用功率、带宽和内存的物理和信息理论限制定义服务边界，建立内生增长模型耦合资本与计算、数据和能源。

Result: 推导出将可观察序列映射为失控与非奇异行为的yes/no证书的决策规则，提供基于改进加速速度相对于当前水平的可证伪测试，以及可直接实施的安全控制措施。

Conclusion: 该框架用可测试的条件和可部署的控制措施取代了对AI奇点的推测，能够验证或排除AI奇点的发生，为AI安全提供了理论基础和实践指导。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到图像模型的数据归因方法计算资源消耗大，难以在实际应用中使用，需要开发更高效的解决方案。

Method: 将基于反学习的慢速归因方法蒸馏到特征嵌入空间，结合高效索引和搜索方法，无需运行昂贵的归因算法即可找到高影响力训练图像。

Result: 在MSCOCO训练的中等规模模型和LAION训练的大型Stable Diffusion模型上均取得优异表现，在几秒内达到更好或竞争性性能。
...
<details>
  <summary>Details</summary>
Motivation: 微创结直肠手术存在程序变异性大、学习曲线陡峭和并发症影响质量的问题，需要标准化的视频评估工具来减少变异性、优化培训和改善手术表现。

Method: 通过德尔菲法达成对通用工作流程描述符的共识，开发ColoWorkflow工具，并在多中心腹腔镜和机器人结直肠手术视频数据集上由独立评估者应用该工具。

Result: 确定了10个程序无关的阶段和34个程序特定的步骤，工具在54个手术视频中表现出广泛适用性，评估者间可靠性中等（阶段K=0.71，步骤K=0.66）。
...
TL;DR: 本文提出了一种基于LRP的剪枝方法，在少量数据情况下实现更高的剪枝率同时保持更好的模型精度。
...
Method: 使用Layer-wise Relevance Propagation（LRP）技术进行模型剪枝，针对计算资源受限环境中的特定目标应用。

Result: 该方法在少量数据情况下实现了比现有方法更好的精度保持效果，同时达到更高的剪枝率。

Conclusion: 提出的剪枝方法在保持模型精度方面优于现有方法，为计算资源受限环境下的模型部署提供了有效解决方案。
...
TL;DR: 提出了一种短窗口滑动学习框架，用于CCTV监控视频的实时暴力检测。该方法将视频分割为1-2秒的短片段，利用LLM自动标注构建细粒度数据集，在RWF-2000上达到95.25%准确率，在UCF-Crime长视频上性能提升至83.25%。


<details>
  <summary>Details</summary>
Motivation: 传统长视频训练方法无法有效捕捉快速暴力事件，需要开发能够实时检测且保持时间连续性的暴力检测方法。

Method: 采用短窗口滑动学习框架，将视频分割为1-2秒片段，利用大语言模型进行自动标注，构建细粒度数据集，充分利用所有帧保持时间连续性。

Result: 在RWF-2000数据集上达到95.25%的准确率，在UCF-Crime长视频数据集上性能提升至83.25%，显示出良好的泛化能力。

Conclusion: 该方法在实时暴力检测方面表现出色，具有强大的泛化能力和实时应用潜力，适用于智能监控系统。
...
TL;DR: 提出MCN-CL方法解决多模态情感识别中的类别不平衡、动态面部动作单元建模复杂性和模态异质性融合难题，通过三重查询机制和困难负样本挖掘策略提升性能。


<details>
  <summary>Details</summary>
Motivation: 多模态情感识别面临类别分布不平衡、动态面部动作单元时间建模复杂、模态异质性导致特征融合困难三大挑战，社交媒体场景下多模态数据爆炸式增长迫切需要高效跨模态融合框架。

Method: 提出MCN-CL方法，使用三重查询机制和困难负样本挖掘策略，在去除特征冗余的同时保留重要情感线索，有效解决模态异质性和类别不平衡问题。

Result: 在IEMOCAP和MELD数据集上的实验结果表明，该方法优于现有最先进方法，加权F1分数分别提高了3.42%和5.73%。

Conclusion: MCN-CL方法通过创新的跨模态融合机制有效解决了多模态情感识别中的关键挑战，在多个数据集上取得了显著性能提升。
...
TL;DR: 提出了一种竞争性强且计算效率高的概率性降雨临近预报方法，使用预训练卫星视觉编码器结合轻量级概率头来预测4小时累积降雨量的离散经验分布函数。


<details>
  <summary>Details</summary>
Motivation: 开发更准确和高效的概率性降雨临近预报方法，以改进短期天气预测能力。
...
Result: 在理论上保持近贪婪近似保证，在实证中仅使用20%计算预算即可达到贪婪归因95%以上的忠实度，在对象检测和视觉定位任务中优于其他基线方法。
...
TL;DR: 本文提出了一种正负提示监督方法，通过优化由大型语言模型初始化的类别特定正负提示，让负提示关注类别边界特征，并使用图结构将语义监督传播到视觉分支，从而提升基于能量的OOD检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型在OOD检测中使用负提示来强调图像特征与提示内容之间的差异，但这些负提示通常包含广泛的非ID特征，可能捕获重叠或误导性信息，导致次优结果。

Method: 1. 使用大型语言模型初始化类别特定的正负提示；2. 优化提示，使正提示关注类内特征，负提示关注类别边界特征；3. 采用基于图的架构聚合优化后的提示表示的语义感知监督，并将其传播到视觉分支。

Result: 在CIFAR-100和ImageNet-1K两个基准测试上，针对八个OOD数据集和五种不同的大型语言模型进行了广泛实验，结果表明该方法优于最先进的基线方法。

Conclusion: 所提出的正负提示监督方法通过优化提示表示并利用图结构传播语义监督，有效提升了基于能量的OOD检测器的性能。
...
<details>
  <summary>Details</summary>
Motivation: 在无约束真实环境中，面部表情识别仍然具有挑战性，需要平衡性能与效率的轻量级模型。

Method: 将FER2013和KDEF两个基准分类数据集转换为目标检测格式，使用YOLOv11n和YOLOv12n模型在统一检测和分类框架下进行评估。

Result: YOLOv12n在干净的KDEF数据集上获得95.6的mAP 0.5最高性能，在FER2013数据集上mAP为63.8；YOLOv11n在FER2013上精度达65.2，假阳性更少。

Conclusion: 轻量级YOLO模型能有效平衡性能与效率，在敏感性和精度间存在权衡，适合实时资源受限的情感感知AI应用。
...
Method: 提出HCD框架：通过卷积投影器和自适应池化处理学生中间特征，与教师倒数第二层特征拼接，经互补特征映射器（CFM）生成共享logits。引入子logit解耦蒸馏（SDD）将共享logits划分为n个子logits，与教师logits融合修正分类。使用正交性损失（OL）确保子logit多样性。
...
TL;DR: FedBCS通过领域不变上下文原型对齐解决联邦学习中的特征异构性问题，引入频域自适应风格重校准和上下文感知的双层原型对齐方法，显著提升分割性能。


<details>
  <summary>Details</summary>
Motivation: 联邦学习中不同医疗机构的扫描仪或协议差异导致特征异构性，现有方法仅关注最终层特征且忽略中间层的风格偏差积累，限制了模型鲁棒性。

Method: 提出频域自适应风格重校准解耦内容-风格表示，设计上下文感知的双层原型对齐方法，从编码器和解码器的不同层提取领域不变原型并与上下文信息融合。

Result: 在两个公共数据集上的广泛实验表明，该方法表现出显著的性能提升。

Conclusion: FedBCS通过领域不变上下文原型对齐有效解决了联邦学习中的特征异构性问题，提高了模型的鲁棒性和分割精度。
...
<details>
  <summary>Details</summary>
Motivation: 微表情识别对于推断真实情感至关重要，但现有方法存在两个核心挑战：静态外观与动态运动线索的纠缠阻碍模型关注细微运动；现有数据集中的文本标签与面部肌肉运动不完全对应，存在语义差距。
...
TL;DR: ERMoE提出了一种新的稀疏MoE架构，通过将专家重新参数化为学习正交特征基，并使用特征基分数（输入特征与专家基的余弦相似度）替代学习门控逻辑，从而解决路由不稳定和专家利用率不足的问题。


<details>
  <summary>Details</summary>
Motivation: 解决传统MoE架构中路由逻辑与专家内部结构不对齐导致的路由不稳定、专家利用率不足，以及负载不平衡造成的瓶颈问题。

Method: 将每个专家重新参数化为学习正交特征基，用特征基分数（输入特征与专家基的余弦相似度）替代学习门控逻辑，实现内容感知路由。

Result: 在ImageNet分类和跨模态图像-文本检索基准（如COCO、Flickr30K）上达到最先进准确率，自然产生更平坦的专家负载分布；3D MRI变体（ERMoE-ba）将脑年龄预测准确率提高7%以上，并产生解剖学可解释的专家专业化。

Conclusion: ERMoE引入了一种新的稀疏专家模型架构原则，直接解决路由不稳定性，并在保持稀疏性的同时实现可扩展、可解释的专业化。
...
TL;DR: DMC是一个用于CLIP模型类增量学习的两阶段框架，通过解耦视觉编码器适配和文本软提示优化来解决分类器偏差问题。DMC-OT是其增强版本，引入最优传输校准策略来对齐内存统计量。


<details>
  <summary>Details</summary>
Motivation: 现有CLIP模型在持续学习场景中面临挑战，特别是学习新类别的任务特定软提示会导致严重的分类器偏差，因为文本原型在没有先前数据的情况下会过拟合到新类别。

Method: 提出两阶段框架DMC：第一阶段冻结文本提示优化视觉编码器，第二阶段冻结视觉编码器优化文本软提示。DMC-OT进一步引入最优传输指导的校准策略来对齐内存统计量，并采用任务特定提示设计增强任务间可分离性。

Result: 在CIFAR-100、Imagenet-R、CUB-200和UCF-101上的广泛实验表明，DMC和DMC-OT都达到了最先进的性能，DMC-OT进一步将准确率平均提高了1.80%。

Conclusion: DMC框架通过解耦视觉和文本模态的优化，有效解决了CLIP模型在类增量学习中的分类器偏差问题，DMC-OT通过最优传输校准进一步提升了性能。
...
<details>
  <summary>Details</summary>
Motivation: 视频LLMs存在时间不一致性问题：帧时序的微小变化会导致注意力翻转并抑制相关帧。这种不稳定性源于多模态RoPE扩展导致的逆傅里叶时间核中的帧级波动。

Method: 提出PAS方法：在多个头部应用小的相反相位偏移，然后聚合它们的输出。该方法保持每个头部的频谱幅度，同时通过聚合有效平滑时间核并减少相位敏感性，而不改变位置编码结构。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型在零样本视觉任务中处理开放性问题时表现不佳，需要一种简单统一的推理时设计方法来提升性能，而无需任务特定训练。

Method: 工作流包含两个步骤：(1) 量化：将开放性问题转换为具有明确候选答案的多项选择题；(2) 二值化：对每个候选答案提出真/假问题，通过确定性规则进行选择。

Result: 在指代表达式定位、空间推理和BLINK-Jigsaw等任务上，量化到多项选择题带来显著提升，真/假二值化进一步提供一致性的性能提升。

Conclusion: 该工作流提供了一个实用、即插即用的路径，通过强调推理时设计而非任务特定训练，显著增强了当前视觉语言模型的零样本视觉能力。
...
TL;DR: CLUE是一个生成模型框架，基于Stable Diffusion架构，通过固定格式提示实现多样化和稳定的图像生成，无需额外数据。在医学领域应用中显著提升了图像质量和分类器性能。
...
Method: 基于Stable Diffusion架构，使用Style Encoder处理图像和提示生成风格嵌入，通过新的第二注意力层和Kullback-Leibler散度实现潜在空间的连续表示。
...
TL;DR: PROMISE是一个新颖的多模态框架，通过提示学习和分层对比学习来解决模态缺失问题，提高跨模态表示的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现实世界中多模态数据经常存在模态缺失，现有方法通过简单生成方式处理但无法保持跨模态一致性，导致性能下降。

Method: 将多模态提示学习融入分层对比学习框架，配备专门设计的提示注意力机制，动态生成鲁棒且一致的表示来处理模态缺失场景。

Result: 在基准数据集上的广泛实验和消融研究表明，PROMISE相比当前最先进的多模态方法表现出优越性能。

Conclusion: PROMISE框架通过创新的提示注意力机制有效弥合完整数据和不完整数据之间的表示差距，显著提升了模态缺失情况下的跨模态表示质量。
...
TL;DR: EmoVid是首个专为创意媒体设计的多模态情感标注视频数据集，包含卡通动画、电影片段和动画贴纸，每段视频都有情感标签、视觉属性和文本描述。基于该数据集开发了情感条件视频生成技术，显著提升了文本到视频和图像到视频任务的质量。
...
Method: 引入EmoVid数据集，通过系统分析揭示视觉特征与情感感知的空间和时间模式，并基于这些洞察对Wan2.1模型进行微调，开发情感条件视频生成技术。

Result: 在定量指标和生成视频视觉质量方面都显示出显著改进，为文本到视频和图像到视频任务提供了更好的情感表达。
...
TL;DR: VisMem是一个受人类认知记忆理论启发的视觉语言模型框架，通过引入短期和长期视觉记忆模块来解决视觉处理瓶颈问题，显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在复杂视觉任务中存在视觉处理瓶颈，容易失去视觉证据的根基并在长序列生成中缺乏情境化视觉体验。受人类认知记忆理论启发，研究者希望为模型添加类似人类记忆的动态视觉记忆机制。

Method: 提出VisMem框架，包含短期视觉记忆模块用于细粒度感知保持，长期记忆模块用于抽象语义巩固。这些记忆在推理过程中被动态调用，使模型在思考和生成过程中保持感知保真度和语义一致性。

Result: 在多个视觉理解、推理和生成基准测试上的广泛实验表明，VisMem相比原始模型平均性能提升11.8%，在所有对比方法中表现最佳。

Conclusion: VisMem通过引入认知对齐的潜在视觉记忆机制，为视觉语言模型建立了新的潜在空间记忆增强范式，有效解决了视觉处理瓶颈问题。
...
TL;DR: SP-Guard是一种改进扩散模型安全性的方法，通过估计提示词危害性和选择性引导掩码，只针对图像中的不安全区域进行引导，在保证安全的同时最小化内容修改。


<details>
  <summary>Details</summary>
Motivation: 现有推理时引导方法缺乏自适应性和选择性，无法根据提示词调整引导强度，也无法只针对图像中的不安全区域进行引导，导致安全生成效果不佳。

Method: 通过估计提示词危害性，应用选择性引导掩码，只对图像中的不安全区域进行引导，实现自适应和选择性的安全生成。
...
Result: 在多个裂缝基准测试中，SUPER显著提升了细裂缝分割性能（特别是宽度小于4像素的裂缝），在智能手机图像去噪任务中也实现了PSNR的适度提升，验证了其在低频和高频场景下的鲁棒性。

Conclusion: SUPER解码器块具有即插即用的通用性，能够在统一的、重构感知的框架内实现高频保真度和全局一致性，同时保持可比较的计算成本。
...
TL;DR: AirCopBench是首个专门评估多模态大语言模型在具身空中协作感知中表现的基准测试，包含14.6k+问题，涵盖4个关键任务维度和14种任务类型，基于模拟和真实世界数据构建。
...
Method: 使用模拟和真实世界数据构建基准，通过模型、规则和人工方法在严格质量控制下生成大规模问题，涵盖具有标注协作事件的挑战性退化感知场景。

Result: 对40个MLLMs的评估显示在协作感知任务中存在显著性能差距，最佳模型平均落后人类24.38%，在不同任务间表现不一致。微调实验证实了空中协作感知和推理中模拟到现实迁移的可行性。
...
TL;DR: 提出了一种混合粒度缓存(HGC)方法，通过在不同计算阶段采用不同粒度的缓存策略来减少可控生成模型的计算开销。


<details>
  <summary>Details</summary>
Motivation: 解决可控生成模型在处理控制条件和内容生成时的计算效率低下问题。
...
Conclusion: HGC方法能够显著提高可控生成模型的效率，同时保持生成质量。
...
Method: 将CheXpert、SIIM-ACR、BraTS 2020和MosMedData四个数据集重新格式化为统一的视觉问答格式，创建Related、Unrelated和零重叠三种测试分割，评估LLaVA-Vicuna-7B和Qwen2-VL-7B模型。

Result: 在Related分割上模型达到83.2%分类准确率和0.75分割cIoU，但在Unrelated和零重叠条件下性能显著下降。跨任务迁移显示仅使用分类数据训练时分割性能提升7% cIoU。
...
TL;DR: 本文提出了一种新的监督微调范式S2D-Align，通过从粗到深的策略建立解剖学基础的图像-报告对齐，解决了传统方法在放射学报告生成中仅进行实例级对齐的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖多模态大语言模型的跨模态生成能力，通过监督微调优化放射图像与报告之间的对齐。但标准SFT范式仅进行实例级对齐，无法建立解剖学基础的对齐，导致生成质量不理想。

Method: 提出S2D-Align范式，采用从浅到深的策略：从粗粒度放射图像-报告配对开始，引入参考报告进行实例级指导，最后使用关键短语将生成过程锚定在特定解剖细节上。通过基于记忆的适配器实现不同对齐阶段的特征共享。

Result: 在公开的MIMIC-CXR和IU X-Ray基准测试中，S2D-Align实现了最先进的性能。消融研究验证了多阶段辅助引导方法的有效性。
...
Method: 将现有生成式合成模型扩展到多参考版本，允许使用任意数量的前景参考图像，并校准前景参考图像的全局和局部特征使其与背景信息兼容。
...
TL;DR: LiteAttention利用扩散注意力在去噪步骤间的时空连贯性，通过早期标记非关键tile并向前传播跳过决策，消除冗余注意力计算，在保持视频生成质量的同时显著提升速度。


<details>
  <summary>Details</summary>
Motivation: 现有扩散变换器在视频生成中面临二次注意力复杂度的瓶颈，动态稀疏注意力方法计算开销大且存在估计误差，而静态稀疏模式在整个去噪过程中固定且往往不是最优的。

Method: 识别扩散注意力的时空连贯性特性，提出LiteAttention方法，利用非关键tile在连续去噪步骤中的稳定性，通过进化计算跳过机制避免重复的注意力计算分析开销。

Result: 在FlashAttention基础上实现高度优化的LiteAttention内核，在量产视频扩散模型上实现显著加速，且不降低生成质量。

Conclusion: LiteAttention结合了动态方法的适应性和静态方法的效率，为视频扩散模型提供了一种高效的注意力加速解决方案。
...
### [31] [Discovering Meaningful Units with Visually Grounded Semantics from Image Captions](https://arxiv.org/abs/2511.11262)
*Melika Behjati,James Henderson*

Main category: cs.CV

TL;DR: 本文提出了一种通过将标题标记分组来捕获语言细粒度表示的模型，旨在提高视觉语言模型对现实世界的理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有工作主要关注图像块与语言标记的对齐，但图像块对人眼无意义，单个标记也不一定携带可接地信息。需要将标记分组来描述场景的不同方面。

Method: 提出一种在架构中分组标题标记的模型，学习将标记分组以获得语言的细粒度表示，并与训练用于发现对象的图像编码器输出对齐。

Result: 通过学习标记分组，视觉语言模型对视觉和语言有更好的细粒度理解。模型发现的标记组与文本中的可接地短语高度相似。

Conclusion: 通过分组标题标记可以捕获更细粒度的语言表示，显著提升视觉语言模型的理解能力，且发现的标记组与真实短语高度一致。
...
<details>
  <summary>Details</summary>
Motivation: 糖尿病视网膜病变是全球可预防性失明的主要原因，早期检测对减少视力丧失至关重要。深度学习在过去十年中彻底改变了DR筛查，但需要系统性地总结技术进展和临床转化障碍。

Method: 采用系统性综述方法，分析自监督学习、半监督学习、领域泛化、联邦训练和混合神经符号模型等先进方法，同时考察评估协议、报告标准和可重复性挑战。

Result: 建立了涵盖多个数据集的基准性能表，识别了多中心验证和临床信任方面的开放性问题，为可重复、隐私保护和临床可部署的DR AI提供了实用议程。

Conclusion: 该综述不仅为DR研究提供了系统框架，其调查的创新方法也广泛适用于大规模医学影像分析，推动了医疗AI从技术研究向临床应用的转化。
...
### [33] [From Synthetic Scenes to Real Performance: Enhancing Spatial Reasoning in VLMs](https://arxiv.org/abs/2511.11440)
*Massimo Rizzoli,Simone Alghisi,Seyed Mahed Mousavi,Giuseppe Riccardi*

Main category: cs.CV

TL;DR: 本文提出了一种通过控制合成数据生成来改进视觉语言模型微调的方法，解决了传统微调过程中的偏见、分布不平衡和标注错误问题，并在真实世界数据上取得了更好的性能。
...
Method: 重新设计微调过程：1）控制数据和标注的生成，确保无偏见、分布平衡且无标注错误，通过全面采样对象属性（颜色、形状、大小、位置）自动构建数据集；2）使用该数据集微调最先进的视觉语言模型，并评估在绝对位置任务上向真实世界数据的性能迁移。

Result: 实验表明：1）在平衡合成数据上微调能在视觉场景中产生均匀性能并减轻常见偏见；2）在合成刺激上微调显著提高了在真实世界数据（COCO）上的性能，优于在匹配设置下微调的模型。

Conclusion: 通过控制合成数据生成过程可以有效解决传统微调中的偏见和分布不平衡问题，显著提升模型在真实世界数据上的性能表现。
...
<details>
  <summary>Details</summary>
Motivation: 将强化微调（RFT）扩展到大型视频语言模型（LVLMs）具有挑战性，现有方法未能有效区分感知和推理过程。
...
Conclusion: VideoP2R通过过程感知建模成功提升了视频推理能力，证明了感知输出对下游推理的信息充分性。
...
TL;DR: 比较了去噪扩散概率模型和自回归因果变换器在生成形状建模和补全任务中的表现，发现扩散模型在连续潜在空间中表现最佳，而自回归模型在相同离散潜在空间中可匹配或超越扩散模型性能。


<details>
  <summary>Details</summary>
Motivation: 虽然生成模型在3D数据等模态中广泛应用，但缺乏关于哪种模型最适合特定任务的共识，且条件信息（如部分3D数据）的评估不够充分。
...
Conclusion: 扩散模型在连续潜在空间中表现最佳，而自回归模型在离散潜在空间中具有竞争力，为不同任务场景下的模型选择提供了指导。
...
TL;DR: 该论文提出了Phys-Liquid数据集，这是一个包含97,200张模拟图像和对应3D网格的物理信息数据集，用于解决透明变形液体在容器运动下的几何和体积属性估计问题。


<details>
  <summary>Details</summary>
Motivation: 自主机器人在执行精确液体操作任务时，容器运动会导致液体表面变形，使准确的液体状态评估变得复杂。现有数据集缺乏代表不同动态场景下真实液体行为的全面物理信息模拟数据。

Method: 提出了一个四阶段重建和估计流程：液体分割、多视图掩码生成、3D网格重建和真实世界缩放，并使用Phys-Liquid数据集进行验证。

Result: 实验结果表明，在重建液体几何形状和体积方面实现了更高的准确性和一致性，优于现有基准。

Conclusion: Phys-Liquid数据集及其相关验证方法有助于推动透明液体感知任务的未来发展。
...
TL;DR: OT-ALD是一种基于最优传输理论的图像到图像翻译框架，解决了DDIB方法中的翻译效率低和潜在分布不匹配问题，通过OT映射消除潜在分布偏差，在保持图像质量的同时提升采样效率。
...
Conclusion: OT-ALD有效平衡了快速图像翻译与图像质量，通过OT映射消除了潜在分布不匹配问题，验证了方法的有效性。
...
### [38] [Refine and Align: Confidence Calibration through Multi-Agent Interaction in VQA](https://arxiv.org/abs/2511.11169)
*Ayush Pandey,Jai Bardhan,Ishita Jain,Ramya S Hebbalaguppe,Rohan Raju Dhanakshirur,Lovekesh Vig*

Main category: cs.CV

TL;DR: AlignVQA是一个基于辩论的多智能体框架，通过专业视觉语言模型生成候选答案并进行两阶段交互，提高视觉问答系统的置信度校准准确性。


<details>
  <summary>Details</summary>
Motivation: 现代VQA系统在医疗诊断和自主导航等高风险领域应用日益广泛，但其置信度估计的可靠性研究不足，经常产生过度自信的响应，需要改进校准性能。

Method: 提出AlignVQA框架：使用不同提示策略的专业VLM生成候选答案，通用智能体进行两阶段交互（批评、优化和聚合），并引入可微分的校准感知损失函数aligncal来微调专业智能体。

Result: 在多个基准VQA数据集上的实证结果表明，该方法显著减少了校准差异，更校准的专业智能体产生更好对齐的置信度。

Conclusion: 辩论式多智能体框架和校准感知损失函数能够有效提高VQA系统的置信度校准准确性，使模型置信度更真实反映预测性能。
...
### [39] [A Space-Time Transformer for Precipitation Forecasting](https://arxiv.org/abs/2511.11090)
*Levi Harris,Tianlong Chen*

Main category: cs.CV

TL;DR: 提出SaTformer视频transformer架构，使用全时空注意力机制从卫星辐射数据预测极端降水，在NeurIPS Weather4Cast 2025累积降雨挑战中获第一名。


<details>
  <summary>Details</summary>
Motivation: 传统数值天气预报模型在临近预报（0-4小时）性能下降且计算成本高，AI天气预测方法虽成功但视频理解架构在天气预报中应用不足。

Method: 构建基于全时空注意力的视频transformer，将降水回归问题重构为分类问题，使用类别加权损失解决标签不平衡问题。

Result: 在NeurIPS Weather4Cast 2025累积降雨挑战中获得第一名。

Conclusion: SaTformer成功展示了视频transformer架构在极端降水预报中的有效性，为AI天气预测提供了新思路。
...
### [40] [3D Gaussian and Diffusion-Based Gaze Redirection](https://arxiv.org/abs/2511.11231)
*Abiram Panchalingam,Indu Bodala,Stuart Middleton*

Main category: cs.CV

TL;DR: DiT-Gaze提出了一种基于扩散变换器（DiT）的高保真视线重定向框架，通过弱监督和正交约束损失提升3D视线重定向模型的性能，在感知质量和重定向精度上达到新的最先进水平。


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯溅射模型（如GazeGaussian）在渲染细微连续视线转移时存在困难，需要更高保真的视线重定向方法来生成增强数据以提升视线估计器的泛化能力。

Method: 结合扩散变换器（DiT）进行高保真图像合成，采用弱监督策略利用合成生成的中间视线角度，并在训练中应用正交约束损失来解耦视线、头部姿态和表情的内部表示。

Result: DiT-Gaze在感知质量和重定向精度上均达到新的最先进水平，将最先进的视线误差降低了4.1%至6.353度。

Conclusion: DiT-Gaze为创建合成训练数据提供了一种优越的方法，其代码和模型将提供给研究社区进行基准测试。
...
### [41] [Toward Gaze Target Detection of Young Autistic Children](https://arxiv.org/abs/2511.11244)
...
TL;DR: 本文提出了一种新的自闭症儿童注视目标检测AI应用，通过社交感知的粗到细框架解决数据集类别不平衡问题，在自闭症儿童注视检测任务中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 自闭症儿童缺乏足够的专业人员进行注视目标检测，这影响了他们的生活质量。自动化的注视目标检测系统对于测量共同注意力（ASD核心挑战）至关重要。

Method: 提出社交感知粗到细（SACF）注视检测框架，利用场景的社交上下文克服自闭症数据集中的类别不平衡问题。采用双路径架构，包含专门处理社交和非社交注视的专家模型，并由上下文感知门模块指导。

Result: 框架在自闭症儿童注视目标检测任务中达到最先进性能，显著优于现有方法，特别是在关键少数类（面向面部的注视）上表现优异。

Conclusion: 该研究为自闭症儿童的注视目标检测提供了有效的AI解决方案，通过社交上下文感知和专门化模型设计，成功解决了数据集不平衡问题，为自动化共同注意力测量系统奠定了基础。
...
### [42] [Toward Generalized Detection of Synthetic Media: Limitations, Challenges, and the Path to Multimodal Solutions](https://arxiv.org/abs/2511.11116)
*Redwan Hussain,Mizanur Rahman,Prithwiraj Bhattacharjee*

Main category: cs.CV

TL;DR: 本文综述了24篇关于AI生成媒体检测的最新研究，分析了现有方法的局限性和挑战，并提出了以多模态深度学习模型为重点的研究方向。


<details>
  <summary>Details</summary>
Motivation: 随着GAN和扩散模型等AI技术的快速发展，生成逼真图像和媒体的能力大幅提升，导致真实与合成内容难以区分。深度伪造技术的滥用可能传播错误信息、侵犯隐私和进行欺诈，因此需要开发有效的检测方法。

Method: 通过系统回顾24篇最新研究，逐一分析每项研究的贡献和弱点，总结现有方法的共同局限性和关键挑战。

Result: 现有检测模型通常使用CNN和ViT等深度学习方法，寻找视觉、空间或时间异常，但存在泛化能力差、对来自不同模型的内容检测困难、对多模态数据和高度修改内容效果不佳等问题。

Conclusion: 建议未来研究方向应聚焦于多模态深度学习模型，这类模型有潜力提供更鲁棒和通用的检测能力，为构建更强防御系统提供明确起点。
...
### [43] [AUVIC: Adversarial Unlearning of Visual Concepts for Multi-modal Large Language Models](https://arxiv.org/abs/2511.11299)
*Haokun Chen,Jianing Li,Yao Zhang,Jinhe Bi,Yan Xia,Jindong Gu,Volker Tresp*

Main category: cs.CV

TL;DR: 本文提出了AUVIC框架，用于解决多模态大语言模型中视觉概念遗忘的问题，通过对抗性扰动实现精确遗忘，同时构建了首个视觉概念遗忘基准VCUBench进行评估。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在大量数据集上训练时可能包含敏感或受版权保护的内容，而法规要求的'被遗忘权'需要机器遗忘技术来移除目标数据，避免资源密集的重新训练。

Method: 提出AUVIC框架，采用对抗性扰动技术来实现精确的视觉概念遗忘，有效隔离目标概念而不影响相关实体。

Result: 实验结果表明，AUVIC在目标遗忘率方面达到最先进水平，同时对非目标概念的性能影响最小。

Conclusion: AUVIC框架成功解决了多模态大语言模型中视觉概念遗忘的挑战，为数据隐私保护提供了有效解决方案。
...
### [44] [Hindsight Distillation Reasoning with Knowledge Encouragement Preference for Knowledge-based Visual Question Answering](https://arxiv.org/abs/2511.11132)
*Yu Zhao,Ying Zhang,Xuhui Sui,Baohang Zhou,Li Shen,Dacheng Tao*

Main category: cs.CV

TL;DR: 本文提出了Hindsight Distilled Reasoning (HinD)框架和Knowledge Encouragement Preference Optimization (KEPO)方法，旨在激发和利用多模态大语言模型中的内部知识推理能力，解决KBVQA中的推理监督问题和知识置信度错位问题。


<details>
  <summary>Details</summary>
Motivation: 现有的KBVQA方法要么通过上下文学习利用MLLMs中的隐式知识，要么通过检索增强生成利用显式知识，但它们的推理过程仍然隐含，缺乏从MLLMs中明确的逐步推理轨迹。

Method: 首先通过提示冻结的7B规模MLLM完成问题与其真实答案之间的推理过程，构建Hindsight-Zero训练数据；然后将其自蒸馏为Chain-of-Thought生成器和知识生成器；最后通过KEPO优化知识生成器，偏好低置信度但有用的知识。

Result: 在OK-VQA和A-OKVQA数据集上的实验验证了HinD的有效性，使用7B规模MLLM激发的推理实现了优越性能，无需商业模型API或外部知识。

Conclusion: HinD框架成功解决了KBVQA中的推理监督和知识置信度错位问题，通过激发MLLMs的内部知识推理能力实现了高性能的视觉问答。
...
### [45] [Reverberation: Learning the Latencies Before Forecasting Trajectories](https://arxiv.org/abs/2511.11164)
...
<details>
  <summary>Details</summary>
Motivation: 现有的轨迹预测方法难以显式学习和预测智能体对轨迹变化事件的延迟响应（latencies），这会影响预测系统的因果连续性和轨迹的合理性。
...
### [46] [Explainable Deep Convolutional Multi-Type Anomaly Detection](https://arxiv.org/abs/2511.11165)
*Alex George,Lyudmila Mihaylova,Sean Anderson*

Main category: cs.CV

TL;DR: MultiTypeFCDD是一个轻量级卷积框架，用于可解释的多类型异常检测，能够区分不同类型的异常，而无需为每个对象类别训练单独模型。


<details>
  <summary>Details</summary>
Motivation: 现有异常检测方法无法区分异常类型，且需要为每个对象类别训练和维护单独模型，计算成本高。大型视觉语言模型虽然能解决此问题，但计算密集且内存占用大，不适合实时或嵌入式系统。

Method: 使用仅图像级别标签学习并生成多通道热图，每个通道对应特定异常类型。作为统一框架，能够跨多个对象类别区分异常类型。

Result: 在Real-IAD数据集上评估，结果与最先进复杂模型相当，但参数负载和推理时间显著减少。

Conclusion: MultiTypeFCDD是计算资源受限的实际应用中高度实用和可行的解决方案。
...
### [47] [CATS-V2V: A Real-World Vehicle-to-Vehicle Cooperative Perception Dataset with Complex Adverse Traffic Scenarios](https://arxiv.org/abs/2511.11168)
...
TL;DR: CATS-V2V是首个针对复杂不利交通场景的V2V协同感知真实世界数据集，包含10种天气光照条件下的100个片段，提供LiDAR点云、多视角相机图像和高精度定位数据，并提出了基于目标的时间对齐方法。


<details>
  <summary>Details</summary>
Motivation: 现有数据集主要关注普通交通场景，缺乏复杂不利条件下的V2V协同感知数据，限制了协同感知在自动驾驶中的潜力发挥。

Method: 使用两辆硬件时间同步的车辆采集数据，覆盖10种天气光照条件和10个不同地点，提供LiDAR点云、相机图像和RTK-GNSS/IMU记录，并采用基于目标的时间对齐方法确保多传感器数据精确对齐。

Result: 构建了包含60K帧LiDAR点云、1.26M张相机图像和750K条定位记录的100片段数据集，提供时间一致的3D边界框标注和静态场景信息，支持4D BEV表示构建。

Conclusion: CATS-V2V是目前同类数据集中规模最大、支持最全面、质量最高的数据集，有望推动自动驾驶社区在复杂不利交通场景下的协同感知研究。
...
### [48] [The Persistence of Cultural Memory: Investigating Multimodal Iconicity in Diffusion Models](https://arxiv.org/abs/2511.11435)
...
<details>
  <summary>Details</summary>
Motivation: 解决扩散模型中泛化与记忆之间的模糊性，特别关注文化共享关联的多模态标志性现象，探索模型如何平衡识别文化引用与复制它们之间的关系。

Method: 引入评估框架分离识别和实现两个维度，通过量化指标衡量。评估五个扩散模型在767个Wikidata衍生的文化引用上的表现，包括静态和动态图像。进行提示扰动实验，使用同义词替换和字面图像描述来评估语言敏感性。

Result: 框架比现有基于相似性的方法更有效地区分复制与转换。模型即使文本线索改变也经常复制标志性视觉结构。文化对齐不仅与训练数据频率相关，还与文本独特性、引用流行度和创建日期相关。
...
### [49] [Dynamic Gaussian Scene Reconstruction from Unsynchronized Videos](https://arxiv.org/abs/2511.11175)
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中多视角视频常因相机触发延迟或独立录制设置导致时序不同步，而现有4DGS方法假设输入视频流时间同步，这限制了其在实际应用中的效果。

Method: 采用粗到精的对齐模块，首先确定帧级粗偏移，然后细化到亚帧精度，可作为即插即用模块集成到现有4DGS框架中。

Result: 实验表明该方法能有效处理时序错位视频，显著提升基线方法的重建质量。

Conclusion: 提出的时序对齐策略增强了4DGS框架对异步数据的鲁棒性，为现实场景中的多视角视频重建提供了实用解决方案。
...
### [50] [Viper-F1: Fast and Fine-Grained Multimodal Understanding with Cross-Modal State-Space Modulation](https://arxiv.org/abs/2511.11177)
*Quoc-Huy Trinh,Mustapha Abdullahi,Do Duy Hung Trinh,Bo Zhao,Debesh Jha*

Main category: cs.CV

TL;DR: Viper-F1是一种混合状态空间视觉语言模型，用高效的液体状态空间动力学替代注意力机制，通过Token-Grid相关性模块增强视觉定位能力，在保持线性时间推理的同时实现细粒度理解。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型计算成本高，难以在资源受限场景部署，且小模型难以精确捕捉细粒度视觉区域，影响在真实世界应用中的效果。

Method: 使用液体状态空间动力学替代Transformer交叉注意力，提出Token-Grid相关性模块计算文本标记与图像块之间的轻量级相关性，通过FiLM条件调节状态空间动力学。

Result: 在多个基准测试中，Viper-F1实现了准确、细粒度的理解，同时显著提高了效率。

Conclusion: Viper-F1通过状态空间模型和视觉定位增强机制，成功解决了多模态模型效率与细粒度理解之间的平衡问题。
...
### [51] [A Comparison of Lightweight Deep Learning Models for Particulate-Matter Nowcasting in the Indian Subcontinent & Surrounding Regions](https://arxiv.org/abs/2511.11185)
*Ansh Kushwaha,Kaushik Gopalan*

Main category: cs.CV

TL;DR: 本文提出了一个用于印度次大陆及周边地区PM1、PM2.5和PM10污染物6小时预报的高效框架，基于CAMS全球大气成分预报数据，采用轻量级参数专用架构，在有限空间域上实现优于基础模型的短程预报性能。


<details>
  <summary>Details</summary>
Motivation: 开发专门针对印度地区污染物预报的紧凑深度学习模型，以改善预报精度、减少系统偏差，并在有限空间域上实现快速推理。

Method: 使用CAMS全球大气成分预报的0.4度分辨率分析场作为输入，采用256x256空间区域覆盖大范围，预测中心128x128印度核心区域。开发三个轻量级参数专用架构，使用2021-2023年数据进行90/10分割训练，在2024年数据上独立评估。

Result: 使用RMSE、MAE、Bias和SSIM指标评估，相比Aurora基础模型取得了显著性能提升，证明了紧凑专业化深度学习模型在有限空间域短程预报中的有效性。

Conclusion: 针对特定区域和参数的轻量级专业化深度学习模型在污染物短程预报任务中表现优异，能够显著提升预报精度并减少系统偏差，同时保持快速推理能力。
...
### [52] [PAS : Prelim Attention Score for Detecting Object Hallucinations in Large Vision--Language Models](https://arxiv.org/abs/2511.11502)
*Nhat Hoang-Xuan,Minh Vu,My T. Thai,Manish Bhattarai*

Main category: cs.CV

TL;DR: 本文提出Prelim Attention Score (PAS)方法，通过分析LVLM对prelim tokens的注意力权重来检测对象幻觉，无需额外训练即可实现最先进的幻觉检测。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型存在对象幻觉问题，研究发现许多幻觉预测中模型实际上忽略了图像，而是依赖先前生成的prelim tokens来推断新对象。

Method: 引入Prelim Attention Score (PAS)，通过计算prelim tokens上的注意力权重来量化图像依赖程度，这是一种轻量级、无需训练的信号，可在推理过程中实时计算。

Result: PAS在多个模型和数据集上实现了最先进的对象幻觉检测性能，能够实时过滤和干预幻觉问题。

Conclusion: 利用先前被忽视的prelim tokens注意力信号，PAS提供了一种有效的对象幻觉检测方法，显著提升了LVLM的可靠性。
...
### [53] [Computationally-efficient deep learning models for nowcasting of precipitation: A solution for the Weather4cast 2025 challenge](https://arxiv.org/abs/2511.11197)
*Anushree Bhuskute,Kaushik Gopalan,Jeet Shah*

Main category: cs.CV

TL;DR: 本研究提出了基于ConvGRU的迁移学习框架，用于Weather4Cast 2025竞赛中的短期降雨预测，使用单一SEVIRI红外通道作为输入，采用两阶段训练策略，在累积降雨任务中获得第二名。


<details>
  <summary>Details</summary>
Motivation: 开发一个有效的短期降雨预测框架，能够从有限的观测数据中预测未来多小时的降雨情况，并识别降水事件。

Method: 使用ConvGRU模型，采用两阶段训练策略：第一阶段预测SEVIRI亮度温度，第二阶段通过非线性变换将预测场映射为OPERA兼容的降雨率，最后使用3D事件检测和时空特征提取识别降水事件。

Result: 在累积降雨任务中获得第二名，在事件预测任务中与基线模型表现相当。

Conclusion: 提出的ConvGRU迁移学习框架在短期降雨预测中表现良好，证明了从亮度温度预测到降雨率转换的有效性，且模型具有良好的泛化能力。
...
### [54] [Geospatial Chain of Thought Reasoning for Enhanced Visual Question Answering on Satellite Imagery](https://arxiv.org/abs/2511.11198)
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉问答模型虽然能够扩展解释遥感数据，但缺乏处理复杂地理空间查询所需的结构化推理能力，特别是在灾害监测、基础设施风险评估等高风险气候领域。

Method: 集成思维链推理与直接偏好优化，通过生成中间推理过程来更好地处理检测、分类、空间关系和比较分析等任务。

Result: 实验表明，思维链监督相比直接基线方法准确率提升34.9%，而直接偏好优化进一步提升了准确率和推理质量。

Conclusion: 该系统通过实现更丰富的地理空间推理，推进了多光谱地球观测的视觉问答能力，为气候应用提供了更有效的支持。
...
### [55] [Questioning the Stability of Visual Question Answering](https://arxiv.org/abs/2511.11206)
*Amir Rosenfeld,Neta Glazer,Ethan Fetaya*

Main category: cs.CV

TL;DR: 本文首次大规模系统研究了视觉语言模型对良性视觉和文本扰动的鲁棒性，发现现代VLM对微小扰动高度敏感，即使是最先进的模型在像素级偏移或无害改写下也经常失败。
...
Method: 通过像素级偏移、轻微几何变换、填充缩放、改写和多语言重写等不改变语义的扰动方式，在多个模型和数据集上进行大规模鲁棒性测试。

Result: 现代VLM对微小扰动高度敏感，大量样本在至少一种视觉或文本修改下会改变预测答案；样本稳定性与正确性高度相关，稳定样本更可能被正确回答。
...
### [56] [One-to-N Backdoor Attack in 3D Point Cloud via Spherical Trigger](https://arxiv.org/abs/2511.11210)
...
Result: 在多个数据集和模型架构上的实验结果表明，该方法实现了高攻击成功率（高达100%），同时在干净数据上保持准确性。
...
### [57] [MAFM^3: Modular Adaptation of Foundation Models for Multi-Modal Medical AI](https://arxiv.org/abs/2511.11212)
*Mohammad Areeb Qazi,Munachiso S Nwadike,Ibrahim Almakky,Mohammad Yaqub,Numan Saeed*

Main category: cs.CV

TL;DR: MAFM^3是一个模块化框架，让单个基础模型通过轻量级模块组件扩展到医学影像的多个领域、任务和模态，实现高效的多任务和多模态适应。
...
Conclusion: 基础模型配备模块化组件后，不再局限于初始训练范围，可以发展为医学影像的多任务、多模态系统。
...
### [58] [RealisticDreamer: Guidance Score Distillation for Few-shot Gaussian Splatting](https://arxiv.org/abs/2511.11213)
...
<details>
  <summary>Details</summary>
Motivation: 3D高斯溅射在稀疏训练视图下容易过拟合，主要原因是缺乏中间视角的监督。受视频扩散模型成功的启发，作者希望利用其丰富的多视角一致性先验来改进3D场景表示。

Method: 基于Score Distillation Sampling (SDS)的洞察，GSD监督来自多个相邻视角的渲染图像，引导高斯溅射表示朝向VDM的生成方向。为了解决生成方向中的物体运动和随机相机轨迹问题，引入了统一的引导形式来修正VDM的噪声预测结果，包括基于真实深度图的深度扭曲引导和基于语义图像特征的引导。

Result: 实验结果表明，该方法在多个数据集上优于现有方法。

Conclusion: GSD框架通过利用视频扩散模型的多视角一致性先验，有效解决了3D高斯溅射在稀疏视图下的过拟合问题，并在多个数据集上取得了优越性能。
...
### [59] [Positional Bias in Multimodal Embedding Models: Do They Favor the Beginning, the Middle, or the End?](https://arxiv.org/abs/2511.11216)
*Kebin Wu,Fatima Albreiki*

Main category: cs.CV

TL;DR: 本文研究了多模态表示模型中的位置偏见问题，特别是在图像-文本检索任务中，发现文本编码器偏向输入开头，图像编码器则偏向开头和结尾。


<details>
  <summary>Details</summary>
Motivation: 位置偏见已被证明会负面影响模型性能，但在表示模型中的存在和影响尚未充分探索，特别是在多模态模型中。

Method: 区分上下文重要性和位置偏见，评估不同模型和数据集中的位置偏见存在程度。

Result: 实验表明位置偏见在多模态模型中普遍存在，文本编码器偏向输入开头，图像编码器偏向开头和结尾，这种偏见由位置编码方案、训练损失、上下文重要性等多因素共同导致。

Conclusion: 多模态模型存在显著的位置偏见，且在不同模态中表现不同，需要进一步研究以减轻其负面影响。
...
### [60] [DoReMi: A Domain-Representation Mixture Framework for Generalizable 3D Understanding](https://arxiv.org/abs/2511.11232)
*Mingwei Xing,Xinliang Wang,Yifeng Shi*

Main category: cs.CV

TL;DR: DoReMi是一个混合专家框架，通过联合建模领域感知专家分支和统一表示分支，解决多源点云数据异构性问题，实现专业化与泛化知识的协同学习。
...
### [61] [Parameter-Efficient MoE LoRA for Few-Shot Multi-Style Editing](https://arxiv.org/abs/2511.11236)
*Cong Cao,Yujie Xu,Xiaodong Xu*

Main category: cs.CV

TL;DR: 本文提出了一种新颖的小样本风格编辑框架，通过构建包含五种不同风格的基准数据集，开发了参数高效的多风格专家混合低秩适配（MoE LoRA）方法，结合风格特定和风格共享路由机制，在有限配对数据下有效微调通用图像编辑模型以适应新风格。


<details>
  <summary>Details</summary>
Motivation: 通用图像编辑模型在处理新风格时往往效果不佳，需要解决如何用有限配对数据有效微调模型以适应新风格的问题。

Method: 提出参数高效的多风格MoE LoRA方法，包含风格特定路由（防止风格干扰）和风格共享路由（学习共同模式）；通过度量引导方法自动确定各层最优秩；探索在DiT模型中插入LoRA的最佳位置；集成对抗学习和流匹配指导扩散训练过程。

Result: 实验结果表明，所提方法在显著减少LoRA参数的情况下，优于现有最先进方法。

Conclusion: 该方法能够有效解决小样本风格编辑问题，在参数效率和性能方面都表现出色。
...
### [62] [Beyond Flatlands: Unlocking Spatial Intelligence by Decoupling 3D Reasoning from Numerical Regression](https://arxiv.org/abs/2511.11239)
*Zhongbin Guo,Jiahe Liu,Yushan Li,Wenyu Gao,Zhen Yang,Chenzhi Li,Xinyue Zhang,Ping Jian*

Main category: cs.CV

TL;DR: GEODE是一个解决视觉语言模型3D空间理解瓶颈的新架构，通过解耦3D推理和数值生成，使用DRM和DRH两个模块实现高效的空间推理。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在理解真实世界3D空间智能方面存在根本性困难，主要源于输入阶段的几何感知编码器与2D特征的冲突，以及输出阶段离散分词器无法生成精确连续数值的双重瓶颈。

Method: 引入GEODE架构，包含两个即插即用模块：解耦推理模块(DRM)作为空间协处理器，通过交叉注意力对齐3D数据与2D视觉特征；直接回归头(DRH)使用"嵌入即值"范式，通过轻量级MLP精确回归标量和3D边界框。

Result: 1.5B参数的模型实现了与7B+模型相媲美的先进空间推理性能，作为高级语义分发器发挥作用。
...
### [63] [CountSteer: Steering Attention for Object Counting in Diffusion Models](https://arxiv.org/abs/2511.11253)
*Hyemin Boo,Hyoryung Kim,Myungjin Lee,Seunghyeon Lee,Jiyoung Lee,Jang-Hwan Choi,Hyunsoo Cho*

Main category: cs.CV

TL;DR: CountSteer是一种无需训练的方法，通过引导扩散模型的交叉注意力隐藏状态来改进文本到图像生成中的对象计数准确性，在保持视觉质量的同时提高约4%的计数准确率。


<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型虽然能生成逼真连贯的图像，但往往无法准确遵循文本中的数字指令，这揭示了语言与视觉表示之间的差距。研究发现这些模型并非完全对数字不敏感，它们隐式地意识到自己的计数准确性。

Method: 基于模型内部对数值正确性的潜在认知，提出CountSteer方法，在推理过程中引导模型的交叉注意力隐藏状态，无需额外训练。

Result: 实验表明CountSteer将对象计数准确率提高了约4%，同时不损害视觉质量。
...
Result: 在LangAuto基准测试上的广泛评估显示，场景图条件化使最先进方法的驾驶性能获得大幅持续提升：LMDrive驾驶评分提升15.6%，BEVDriver提升17.5%。
...
TL;DR: CORAL提出了一种协调序数关系解剖学习方法，通过对比排名目标和序数目标来捕捉体积医学图像中的局部和全局解剖结构，在有限标注设置下实现最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用硬二元阈值定义正负样本，丢弃了有价值的连续解剖相似性信息，且忽视了解剖进展的全局方向一致性，导致特征空间扭曲无法捕捉跨患者的规范解剖流形。
...
Result: 大量实验表明，SimuFreeMark在各种传统和语义攻击下都优于最先进的方法，同时保持了优异的视觉质量。

Conclusion: SimuFreeMark通过利用低频分量的固有稳定性，成功开发了一个免噪声模拟的水印框架，在对抗多种攻击时表现出卓越的鲁棒性和泛化能力。
...
TL;DR: 本文提出了一种基于纯合成数据的草莓6D姿态估计方法，使用YOLOX-6D-Pose算法和Blender程序化管道生成逼真训练数据，在RTX 3090和Jetson Orin Nano上均取得良好性能，特别适合农业机器人部署。


<details>
  <summary>Details</summary>
Motivation: 解决先进经济体农业劳动力成本高、季节性劳动力短缺的问题，通过自动化水果采摘技术提升农业生产效率。

Method: 使用YOLOX-6D-Pose单次检测算法，结合Blender程序化管道生成逼真的合成草莓数据用于训练，支持边缘设备推理。

Result: 在两个硬件平台（RTX 3090和Jetson Orin Nano）上均获得可比较的ADD-S指标精度，RTX 3090处理速度更快，Jetson Orin Nano更适合资源受限环境。模型能准确检测成熟和部分成熟草莓，但对未成熟草莓检测存在挑战。

Conclusion: 该方法在草莓6D姿态估计上表现良好，可轻松扩展到其他水果（如苹果、桃子、李子），在农业自动化领域具有广泛应用前景，未来可通过颜色变化改进未成熟草莓检测。
...
TL;DR: DocSLM是一个高效的小型视觉语言模型，专为资源受限的边缘设备上的长文档理解而设计。它通过分层多模态压缩器和流式弃权机制，大幅减少了内存消耗和延迟，同时保持与最先进方法相当的性能。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在长文档理解方面表现出色，但其高内存占用使其难以在资源受限的边缘设备上部署。需要开发高效的模型来解决这一挑战。

Method: DocSLM采用分层多模态压缩器联合编码每页的视觉、文本和布局信息为固定长度序列，并使用基于熵的不确定性校准器的流式弃权机制，实现可扩展的长输入处理。

Result: 在多个长多模态文档基准测试中，DocSLM匹配或超越了最先进方法，同时减少了82%的视觉标记、75%的参数和71%的延迟。

Conclusion: DocSLM在轻量级边缘设备上实现了可靠的多模态文档理解，为资源受限环境下的长文档处理提供了高效解决方案。
...
TL;DR: YCB-Ev SD是一个用于6DoF物体姿态估计的标准分辨率事件相机合成数据集，包含50,000个事件序列，采用PBR渲染和BOP方法生成，通过系统评估确定了最佳的事件表示方法。


<details>
  <summary>Details</summary>
Motivation: 解决事件视觉领域缺乏与帧基计算机视觉相媲美的综合合成数据资源的问题，为6DoF物体姿态估计研究提供高质量的事件相机数据集。

Method: 使用基于物理的渲染(PBR)场景，遵循BOP方法学，通过模拟线性相机运动生成50,000个34毫秒持续时间的事件序列，确保完整的场景覆盖。

Result: 系统评估显示，具有线性衰减和双通道极性编码的时间表面在CNN推理中实现最佳姿态估计性能，显著优于指数衰减和单通道替代方案，极性信息对性能提升贡献最大。

Conclusion: YCB-Ev SD数据集填补了事件视觉合成数据的空白，提供了结构化格式的原始事件流和预计算最优表示，便于即时研究和可重复基准测试。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D人体运动生成模型虽然重建精度高，但难以泛化到训练分布之外，这源于精确3D监督导致模型过度拟合固定坐标模式而非学习本质的3D结构和运动语义线索。

Method: 提出Motion-Lifting Residual Quantized VAE (ML-RQ)将2D运动序列映射到3D一致潜在空间，并设计了一套3D无监督正则化目标，包括视角一致性、方向连贯性和物理合理性约束。

Result: 仅使用2D运动数据训练，Free3D能生成多样化、时间连贯且语义对齐的3D运动，性能达到甚至超过完全3D监督的对比方法。
...
Method: 将HMRF理论与CNN分割相结合，利用HMRF的邻域和类别分布概念构建无监督损失函数，并在PU泡沫结构的μCT图像数据集上进行验证。

Result: HMRF-UNet在无需标注数据的情况下实现了高分割精度，提出的预训练策略显著减少了训练分割模型所需的标注数据量。

Conclusion: 该方法成功结合了无监督学习和快速分割的优势，为材料表征提供了一种高效准确的分割解决方案，并降低了标注数据需求。
...
Method: 提出LSEF框架，基于低秩稀疏原理，包含三个即插即用模块：稳定性编码模块捕获低秩情感基，动态解耦模块分离稀疏瞬态信号，一致性集成模块重建多尺度稳定性与反应性一致性，采用秩感知优化策略自适应平衡梯度平滑性和敏感性。
...
<details>
  <summary>Details</summary>
Motivation: 解决生物医学成像中科学推理任务因缺乏大规模高质量训练数据而受限的问题，特别是显微镜视觉问答领域的数据稀缺问题。

Method: 三阶段构建方法：1) 从同行评审文章中获取专家验证的图-标题对；2) 使用HiCQA-Graph（融合NLI文本蕴含、CLIP视觉语言对齐和智能体信号的异构图）过滤不一致样本；3) 使用多模态大模型生成选择题并通过人工筛选。

Result: 构建了包含大规模训练集和人工检查测试集的数据集，其Bloom认知水平困难样本分布超过MicroVQA基准，使4B参数MLLM在显微镜推理任务上达到与GPT-5竞争的性能，并在开源MLLM中达到最先进水平。

Conclusion: 通过精心设计的数据构建方法，即使是较小规模的多模态大模型也能在专业领域达到与大型模型竞争的推理性能，证明了高质量数据构建的重要性。
...
Method: 提出三层次评估框架：粗粒度质量评分、中粒度失真类型识别、细粒度失真严重程度评估，并使用思维链提示技术。

Result: 评估显示多模态大语言模型具备初步的文档图像质量评估能力，但存在评分不一致、失真误识别和严重程度误判等关键限制。

Conclusion: 思维链提示能显著提升多模态大语言模型在文档图像质量评估中的性能，为改进模型质量感知能力提供了有前景的途径。
...
TL;DR: BOFA框架通过仅调整CLIP的跨模态桥接层实现类增量学习，无需额外参数，采用正交低秩融合防止遗忘，结合跨模态混合原型提升分类性能。


<details>
  <summary>Details</summary>
Motivation: 解决CLIP在类增量学习中面临的两个挑战：避免因添加可学习模块导致的模型复杂化和遗忘问题，以及更好地整合视觉和文本模态的互补优势。

Method: 在CLIP的跨模态桥接层进行适配，使用正交低秩融合机制将参数更新限制在低秩安全子空间，结合跨模态混合原型（稳定文本原型和视觉原型）。

Result: 在标准基准测试中，BOFA相比现有方法实现了更高的准确率和效率。

Conclusion: BOFA通过桥接层正交融合方法，在无需数据回放的情况下实现了稳定有效的类增量学习，显著提升了CLIP在增量学习任务中的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法忽视了视觉与脑电模态之间的根本不对称性，包括保真度差距（脑电信号噪声大）和语义差距（脑电语义表示浅层），强制对称对齐导致泛化能力差。

Method: 提出自适应教学范式，让教师模态（视觉）在任务指导下动态缩小和调整知识结构，匹配学生模态（脑电）的能力。具体实现为ShrinkAdapter模块，采用无残差设计和瓶颈结构。
...
TL;DR: WEAVE是首个用于上下文交错跨模态理解和生成的套件，包含WEAVE-100k大规模数据集和WEAVEBench人工标注基准，旨在解决现有方法在多轮、上下文感知图像生成和编辑中的局限性。
...
Method: 开发了包含100K交错样本的WEAVE-100k数据集和基于480张图像100个任务的WEAVEBench基准，采用混合VLM评判器评估框架。

Result: 在WEAVE-100k上训练能够提升视觉理解、图像编辑和理解-生成协作能力，促进UMMs发展出新兴的视觉记忆能力。

Conclusion: WEAVE为多模态社区研究上下文交错理解和生成提供了视角和基础，揭示了当前方法在多轮上下文感知图像生成和编辑中的持续局限性和挑战。
...
TL;DR: Hi-DREAM是一个受大脑启发的条件扩散框架，通过显式建模皮层组织来解码fMRI信号。它使用ROI适配器将fMRI分组为早期/中期/晚期视觉流，构建与U-Net深度对齐的多尺度皮层金字塔，并通过轻量级ControlNet在去噪过程中注入尺度特定的提示。


<details>
  <summary>Details</summary>
Motivation: 当前基于扩散的解码器直接基于fMRI特征进行条件化，忽略了大脑的层次处理结构，模糊了早期、中期和晚期视觉区域的作用。需要一种能够显式建模皮层组织的解码方法。

Method: 提出Hi-DREAM框架：1）ROI适配器将fMRI分组为早期/中期/晚期流，构建多尺度皮层金字塔；2）轻量级深度匹配ControlNet在去噪过程中注入尺度特定提示；3）使每个信号发挥类似大脑的作用。
...
TL;DR: VP-Bench是一个评估多模态大语言模型视觉提示理解能力的基准，包含感知和利用两个阶段，测试了28个模型在8种形状和355种属性组合下的表现。
...
Conclusion: VP-Bench为研究MLLMs如何理解和解决基于视觉提示的指代问题建立了新的参考框架。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个能够理解自由形式文本描述（从单词到完整临床句子）并进行3D医学图像分割的模型，以解决传统分割方法对文本输入的灵活性不足问题。

Method: 使用多阶段视觉语言融合策略，在解码器层间对齐文本和视觉特征；在62,000+个CT、MRI和PET体积数据上训练，涵盖1,000多个解剖和病理类别。

Result: 在未见数据集上实现最先进的零样本性能，在不同模态间表现出强大的跨模态迁移能力，对语言变化和临床语言具有鲁棒性，并能从真实世界文本进行准确的实例特定分割。

Conclusion: VoxTell展示了在医学图像分割中结合视觉和语言理解的强大能力，为临床实践提供了灵活且通用的分割工具。
...
TL;DR: 提出了一种名为MaMOL的多模态学习框架，通过双路由机制处理遥感分类中的模态缺失问题，将模态缺失重新定义为多任务学习问题，实现了参数高效的适应性调整。
...
Method: 提出Missing-aware Mixture-of-Loras (MaMOL)框架，引入双路由机制：任务导向的动态路由器根据不同缺失模式自适应激活专家，以及模态特定共享的静态路由器维持稳定的跨模态知识共享。通过轻量级专家更新和共享专家重用实现参数高效适应。

Result: 在多个遥感基准测试中表现出优异的鲁棒性和泛化能力，在不同缺失率下都能保持良好性能，且计算开销最小。在自然图像数据集上的迁移实验验证了其可扩展性和跨领域适用性。

Conclusion: MaMOL为不完整多模态学习提供了一个通用且高效的解决方案，能够有效处理模态缺失问题，具有广泛的应用前景。
...
Result: 在肺鳞状细胞癌数据集上达到平均Dice分数0.805和平均IoU 0.709，不确定性估计与分割误差强相关。
...
TL;DR: BRIDGE提出了一种轻量级的视觉-语言融合模块，通过在编码器顶部添加少量双向交叉注意力层来对齐视觉和文本的隐藏状态，在保持编码器效率的同时提升了多模态任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言模型要么在编码器早期融合特征，要么在后期比较池化嵌入，而两种模态的隐藏状态已经包含丰富的模态特定结构信息，直接对齐这些状态是更自然的多模态匹配方式。

Method: 在视觉和文本编码器顶部附近放置少量仅交叉的双向注意力层，将隐藏状态投影到共享空间，进行跨模态注意力计算，并通过门控残差更新返回，使用简单稳定器改善对齐效果。

Result: 在标准检索、VQA和视觉推理基准测试中，BRIDGE优于可比较的视觉-语言模型，同时保持了对比模型的双编码器效率。

Conclusion: BRIDGE提供了一种高效的多模态融合方法，编码器保持非因果性和强大理解能力，生成任务通过可选解码器清晰解耦，在多个任务上实现了性能提升。
...
TL;DR: 提出了一种结合结构和语义分析的无监督循环检测框架，用于检测基于大语言模型的智能体应用中的隐藏执行循环，这些循环会消耗资源但不触发显式错误。


<details>
  <summary>Details</summary>
Motivation: 基于大语言模型的智能体应用表现出非确定性行为，可能形成隐藏的执行循环，这些循环会默默消耗资源而不触发显式错误，传统可观测性平台无法检测这些代价高昂的低效率问题。

Method: 采用无监督循环检测框架，结合结构和语义分析：首先应用计算高效的时间调用栈分析识别显式循环，然后利用语义相似性分析发现以冗余内容生成为特征的微妙循环。

Result: 在基于LangGraph的股票市场应用的1575个轨迹上评估，混合方法达到F1分数0.72（精确率：0.62，召回率：0.86），显著优于单独的结构方法（F1：0.08）和语义方法（F1：0.28）。

Conclusion: 虽然这些结果令人鼓舞，但仍有很大的改进空间，未来工作需要完善该方法并解决当前局限性。
...
TL;DR: 本文提出了一种基于大语言模型的仿真推演数据分析方法，通过任务分解、多轮交互和自定义工具生成高质量分析报告。


<details>
  <summary>Details</summary>
Motivation: 传统手动分析方法耗时且易出错，需要利用大语言模型的分析推理能力来提高仿真推演数据分析的效率和准确性。

Method: 将复杂任务分解为子任务，为每个子任务设计系统提示和用户提示，通过多轮交互结合自检和反思进行结构化数据提取和多步分析评估，使用自定义工具生成图表和计算指标。

Result: 评估结果表明，该方法生成的报告质量更高，得分优于基线方法。

Conclusion: 所提出的方法能够有效生成高质量、结构化的仿真推演分析报告，具有较好的适应性和实用性。
...
TL;DR: 提出了一种通过离线数据增强和并行检索的结构化情景记忆架构，解决了历史人物对话系统中深度与延迟的权衡问题，在保持低延迟的同时实现了深度对话响应。


<details>
  <summary>Details</summary>
Motivation: 现有方法在历史人物对话系统中面临关键权衡：简单检索增强生成产生浅层响应，而多阶段反思虽然深度但延迟过高。需要一种既能保持对话深度又能实现高效响应的解决方案。

Method: 将传记数据转化为1774个增强的第一人称记忆，带有情感语义元数据，采用两阶段检索架构实现0.52秒的提示生成，通过离线数据增强和并行检索结构化情景记忆。

Result: 使用LLM-as-judge和RAGAs指标评估显示，该方法在GPT-4上与传统RAG相当，在较小模型(GPT-3.5、GPT-3)上显著优于传统RAG，特别适合资源受限部署。
...
TL;DR: 本文提出了第一个用于自然语言生成的混合量子-经典大语言模型HyQuT，该模型能够进行连贯且上下文感知的对话，通过将变分量子电路集成到Transformer框架中，在8M和150M参数规模上实现了量子计算与大规模生成语言模型的集成。


<details>
  <summary>Details</summary>
Motivation: 尽管量子计算已越来越多地应用于替代经典计算，但大多数现有的量子或混合模型仍局限于简单任务，迄今为止尚未成功应用于大规模自然语言生成。本研究旨在探索将量子计算集成到大规模生成语言模型中的可行性。

Method: 提出的HyQuT架构将变分量子电路（VQCs）集成到Transformer框架中，在8M和150M参数规模上进行实验。使用最少量子比特（10个量子比特和80个量子门）来替代约10%的经典参数。

Result: 实验结果表明，在150M参数模型中，仅使用10个量子比特和80个量子门就能替代约10%的经典参数，同时实现了可比较的收敛稳定性和生成质量。

Conclusion: 这项研究为将量子计算集成到大规模生成语言模型提供了早期可行性证明，展示了量子计算在大规模自然语言生成任务中的潜力。
...
TL;DR: 研究发现大型语言模型在处理时间约束方面存在系统性风险：性能分布两极分化、提示极度脆弱、系统性行动偏见，且参数数量与能力无关。即使经过微调，现有自回归架构仍无法可靠处理时间约束。


<details>
  <summary>Details</summary>
Motivation: 测试LLMs在需要实时决策的智能体架构中处理时间约束的可靠性，因为现有系统假设模型能可靠判断行动窗口是否开放，但这一假设未经检验。

Method: 使用截止时间检测任务评估8个生产级模型（2.8-8B参数），分析性能分布、提示敏感性和行动偏见，并进行200个合成示例的微调实验。

Result: 发现系统性部署风险：模型性能呈双峰分布（95%或50%准确率）、提示极度脆弱（格式变化导致30-60个百分点波动）、系统性行动偏见（失败模型100%误报率）。参数数量与能力无关，微调仅能部分改善能力。

Conclusion: 时间约束满足无法通过自然语言的下一个词预测可靠学习，需要具备连续时间状态表示、显式约束检查和系统组合推理的架构机制，当前自回归架构缺乏这些机制，在时间关键应用中部署存在不可接受风险。
...
<details>
  <summary>Details</summary>
Motivation: 为了提升谱神经符号推理框架的图保真度，减少冗余节点，改善边质量，并补充缺失的上下文信息，从而提高推理的准确性和鲁棒性。

Method: 采用三种语义增强方法：(1) 使用上下文嵌入（如Sentence-BERT、SimCSE）进行基于Transformer的节点合并；(2) 使用预训练NLI分类器（如RoBERTa、DeBERTa）进行句子级蕴含验证；(3) 与外部知识图谱（如ConceptNet、Wikidata）对齐以增强上下文。

Result: 在ProofWriter、EntailmentBank和CLUTRR基准测试中，准确率提升高达3.8%，对对抗性案例的泛化能力增强，推理噪声减少。

Conclusion: 通过在谱推理阶段之前进行语义和符号细化，实现了高效、可解释和可扩展的推理，无需依赖二次注意力机制，构建了更适合开放领域和实际部署的鲁棒推理系统。
...
TL;DR: 提出了PRO框架，使用轻量级偏好适配器自动推断特定提示的偏好权重，解决多目标对齐中手动指定权重的问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在多目标对齐中面临用户手动指定偏好权重的负担，以及探索不相关偏好组合导致的训练效率低下问题。

Method: PRO框架包含一个轻量级偏好适配器，通过训练来自动学习每个提示的适当偏好权重，基于多个奖励模型对偏好响应的归一化奖励分数。

Result: 理论分析证明该方法在多目标对齐场景中优于固定偏好权重，多个任务的广泛实验验证了其有效性。

Conclusion: PRO框架通过自动推断提示特定偏好权重，有效解决了多目标对齐中的权重指定问题，提高了训练效率和性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有的上下文学习演示选择方法主要关注测试输入与演示之间的语义相似性，而忽视了标签分布对齐的重要性，这限制了大型语言模型的性能表现。

Method: 提出两阶段演示选择方法：首先基于语义相似性选择TopK候选演示，然后使用微调的BERT类小语言模型生成标签分布并计算标签分布散度，选择与测试输入标签分布对齐的演示。

Result: 在七个文本分类基准测试上的广泛实验表明，该方法持续优于先前的演示选择策略，且大型语言模型的性能与底层小语言模型标签分布估计的准确性呈正相关。

Conclusion: 结合语义相似性和标签分布对齐的演示选择方法能有效提升上下文学习性能，且标签分布估计的准确性对最终结果有重要影响。
...
TL;DR: 提出了一种基于专利文档内部多视图的对比学习框架，通过利用专利不同章节（摘要、权利要求、背景等）作为互补视图来学习专利嵌入，解决了SimCSE风格dropout增强在专利领域产生过于均匀嵌入的问题。


<details>
  <summary>Details</summary>
Motivation: 发现SimCSE风格的dropout增强在专利领域存在特定失败模式：产生过于均匀的嵌入，失去语义连贯性。需要利用专利文档固有的多章节结构来提供自然语义多样性。

Method: 提出基于章节的增强方法，将专利的不同章节（如摘要、权利要求、背景）作为对比学习的互补视图，引入自然语义和结构多样性。
...
TL;DR: 评估15个开源大语言模型在病理学和放射学报告中的结构化信息提取能力，涵盖6种疾病用例和3个国家机构，比较不同模型大小、提示策略和任务复杂度的影响。


<details>
  <summary>Details</summary>
Motivation: 现有研究多关注单一任务、有限模型和英文报告，缺乏对多任务、多语言、多机构环境下开源LLM在临床记录结构化信息提取能力的系统评估。

Method: 使用15个开源LLM在病理学和放射学报告上测试6种疾病用例，比较6种提示策略（零样本、单样本、少样本、思维链、自一致性和提示图），采用任务适应性指标和线性混合效应模型进行量化分析。

Result: 表现最佳的模型在各项任务中达到接近人工标注一致性的宏观平均分数，中小型通用模型与大型模型表现相当，提示图和少样本提示策略可提升约13%性能。

Conclusion: 开源LLM能够跨疾病、语言和机构从临床报告中提取结构化数据，为临床数据管理提供可扩展的方法，任务特定因素比模型大小或提示策略对结果影响更大。
...
TL;DR: 提出了一种通过加权专家乘积（wPoE）进行测试时引导的新框架，将通用压缩模型与预训练神经语言模型自适应结合，确保压缩率至少与最佳个体模型相当，无需微调即可提升文本压缩性能。


<details>
  <summary>Details</summary>
Motivation: 传统通用压缩器计算开销低、速度快、适用范围广但压缩率较差，神经压缩器能更好建模数据分布但泛化能力不足，需要解决这一局限性。

Method: 使用加权专家乘积（wPoE）框架，在推理时自适应地结合通用压缩模型和预训练神经语言模型。

Result: 大量实验表明该方法无需微调即可提升文本压缩性能，且能与任何自回归语言模型无缝集成。

Conclusion: 该框架为跨不同数据分布的文本压缩提供了实用的增强解决方案。
...
TL;DR: 本文提出了一种贝叶斯方法来量化大型语言模型(LLM)在二元评估指标中的统计不确定性，特别关注由概率文本生成策略引起的不确定性。


<details>
  <summary>Details</summary>
Motivation: 现有的LLM评估方法往往忽视统计不确定性量化，而这对准确评估模型行为(如有害输出倾向、对抗输入敏感性)至关重要。

Method: 采用贝叶斯方法量化二元评估指标的不确定性，特别关注LLM概率文本生成策略引起的不确定性。通过两个案例研究验证方法：1)评估对抗输入上的拒绝率；2)评估开放对话中的成对偏好。

Result: 贝叶斯方法能够为基于LLM的系统行为提供有用的不确定性量化，在拒绝率评估和模型偏好比较中表现出色。

Conclusion: 贝叶斯不确定性量化方法能够显著改进LLM评估的统计可靠性，为模型行为评估提供更准确的置信区间。
...
TL;DR: 本文对7个前沿大语言模型在粤语、日语和土耳其语上的表现进行了全面评估，发现最大专有模型表现最佳但仍有文化理解和形态学泛化差距，开源模型表现较差。
...
Method: 创建跨语言基准测试，涵盖粤语、日语和土耳其语，包含四个任务：开放域问答、文档摘要、英译X和文化对话，结合人工评估和自动指标。

Result: 最大专有模型（GPT-4o、GPT-4、Claude 3.5）在语言和任务上表现最佳，但在文化细微理解和形态学泛化方面仍有显著差距。开源模型在流畅度和准确性上大幅落后。

Conclusion: 所有模型都在一定程度上难以应对各语言的独特语言学挑战，需要开发更具文化意识和语言学泛化能力的大语言模型。
...
TL;DR: 本文提出了一种自监督框架，通过使用偏斜感知聚合策略来提高保护模型的语义鲁棒性，减少语义变异性约58%，提高基准准确率约2.5%，并改善模型校准达40%。


<details>
  <summary>Details</summary>
Motivation: 保护模型对表面语言变化的敏感性是其关键漏洞，即使意义保持不变的改写也会导致安全评分的大幅波动，这揭示了缺乏语义基础的问题。

Method: 引入实用的自监督框架，利用改写集通过新颖的偏斜感知聚合策略来强制执行预测一致性，以计算鲁棒目标。

Result: 分析六个开源保护模型显示，该方法将改写间的语义变异性减少约58%，平均基准准确率提高约2.5%，并能泛化到未见过的风格变化。

Conclusion: 将语义一致性作为首要训练目标具有重要价值，该方法为构建更可靠保护模型提供了可扩展的解决方案，并揭示了模型校准与一致性之间的双向关系。
...
TL;DR: 本文提出了STaDS框架，通过结构化表格决策模拟来评估大语言模型的理解能力，发现现有模型在跨领域一致性、准确性与忠实性匹配方面存在显著问题。
...
Method: 引入STaDS框架，通过15个不同决策场景模拟专家决策考试，联合评估模型的问题理解、知识预测和对相关决策因素的依赖能力。

Result: 对9个前沿LLMs的分析显示：(a)大多数模型难以在不同领域保持强一致性；(b)模型可能准确但不忠实，推理过程与预测驱动因素经常不匹配。
...
<details>
  <summary>Details</summary>
Motivation: 人工耳蜗植入虽然能显著改善重度感音神经性听力损失儿童的语言能力，但个体间效果差异很大，且无法通过植入年龄或残余听力可靠预测。本研究旨在探索更准确的语言发展预测方法。

Method: 使用基于脑神经解剖特征的预测模型，比较传统机器学习和深度迁移学习算法，采用二分类模型（高语言改善者vs低语言改善者），涉及278名来自三个中心的植入儿童。

Result: 深度迁移学习预测模型实现了92.39%的准确率、91.22%的敏感性、93.56%的特异性，AUC为0.977，在所有结果指标上均优于传统机器学习模型。

Conclusion: 深度迁移学习通过直接捕获判别性和任务特定信息，显著提升了预测性能，支持了为全球人工耳蜗项目开发单一深度迁移学习预测模型的可行性。
...
TL;DR: 本文提出了一种增强大型语言模型的方法，通过混合专家语音投影器来解决代码切换语音翻译的挑战，包括多阶段训练范式来缓解数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 代码切换语音翻译面临语义建模复杂性和数据稀缺两大挑战，现有方法依赖模型隐式学习语义且需要昂贵的人工标注。

Method: 使用混合专家语音投影器增强LLMs，每个专家专注于特定语言的语义子空间；采用多阶段训练范式利用单语ASR和ST数据；结合语言特定损失和负载平衡损失指导专家分配。

Result: 在广泛使用的数据集上的大量实验证明了该方法的有效性和通用性。

Conclusion: 所提出的方法能够有效解决代码切换语音翻译中的语义建模和数据稀缺问题，展现出良好的性能。
...
TL;DR: GVF Finetuning是一种新颖的微调方法，通过事实锚点数据增强、事实感知指令调优和事实一致性损失函数三个核心机制，系统性地提升多模态大语言模型的视觉事实一致性，显著减少视觉幻觉问题。
...
Method: GVF Finetuning包含三个核心机制：事实锚点数据增强（用结构化事实锚点和反事实提示丰富训练数据）、事实感知指令调优（将事实线索嵌入显式指令）、事实一致性损失函数（专门惩罚事实不准确）。

Result: 在LLaVA-1.5-13B模型上评估，GVF Finetuning在VHTest基准测试中显著优于标准微调方法，在开放式问题和是/否问题格式上都表现更好，同时在MME和POPE等多模态基准测试上保持或略微提升性能。

Conclusion: GVF Finetuning能有效缓解视觉幻觉问题，同时不损害模型的一般理解和推理能力，为提升多模态大语言模型的可靠性提供了有效解决方案。
...
TL;DR: 本文提出了一种基于预注意力机制的专家预测方法，通过使用同一层注意力块前的激活值和线性函数实现准确且轻量级的专家预取，显著提升了MoE模型推理效率。


<details>
  <summary>Details</summary>
Motivation: 现有MoE模型专家预测方法使用前一层的激活值进行预测，准确率低且无法优化第一层，而复杂预测方法又带来高计算开销。

Method: 利用LLM中某些函数的排序保持特性，使用同一层注意力块前的激活值，通过2个线性函数和排序感知损失实现准确的专家预测。
...
TL;DR: SpiderGen是一个基于LLM的工作流，将传统生命周期评估（LCA）的分类学和方法论与LLM的推理能力和世界知识相结合，用于生成LCA所需的程序信息，显著降低了LCA的成本和时间。


<details>
  <summary>Details</summary>
Motivation: 气候变化和全球变暖主要由温室气体排放引起，而消费品的生产、使用和处置是重要排放源。需要开发工具来评估消费品的环境影响，其中生命周期评估（LCA）是关键组成部分。

Method: SpiderGen整合了传统LCA的分类学和方法论，利用LLM的推理能力和世界知识来生成LCA过程信息。通过与真实LCA文档作为基准进行评估，并与思维链提示和单样本提示等基线技术进行比较。

Result: SpiderGen提供了准确的LCA过程信息，F1分数达到62%。主要错误源于LCA文档细节差异和辅助过程范围界定不同。相比传统LCA（成本超过25000美元，耗时21人天），SpiderGen能在10分钟内以不到1美元的成本生成LCA信息。

Conclusion: SpiderGen在生成LCA过程信息方面表现出色，能够显著减少人力投入和成本，为环境影响评估提供了高效且经济的解决方案。
...
TL;DR: 本文研究了不同大语言模型对齐方法对提示攻击响应的影响，通过统计方法分析提示变化对攻击成功率(ASR)的敏感性，发现即使小的提示修改也能显著改变ASR。


<details>
  <summary>Details</summary>
Motivation: 评估不同对齐方法（SFT、DPO、RLHF）在应对提示攻击时的表现差异，理解现有攻击基准的局限性。

Method: 选择基于常见对齐方法的开源模型，使用统计方法系统分析提示变化对攻击成功率的敏感性。

Result: 结果显示小的提示修改能显著改变攻击成功率，使模型对不同类型的攻击更加敏感或抵抗，现有攻击基准可能无法完全揭示模型和对齐方法的所有漏洞。

Conclusion: 需要进行基于统计的系统性分析来全面评估模型和对齐方法的安全性，仅依赖现有攻击基准可能不够充分。
...
TL;DR: 本文通过多轮对话测试评估大语言模型的鲁棒性，发现简单的后续提示（如"再想想"）会导致模型准确率显著下降，并提出了使用马尔可夫链建模准确率动态和线性探针预测答案变化的方法。
...
Result: 发现LLM存在显著脆弱性："再想想"提示导致Gemini 1.5 Flash准确率下降约10%；结合重述问题的提示使Claude 3.5 Haiku准确率下降7.5%；模型准确率可通过马尔可夫链有效建模，其稳态准确率比首轮平均低约8%。
...
TL;DR: 研究递归提示下大语言模型生成合成数据时的性别偏见动态，发现存在均衡而非单调放大现象，并验证对比增强方法能有效减少下游任务偏见。


<details>
  <summary>Details</summary>
Motivation: 探索递归提示生成合成数据过程中的偏见放大风险，分析不同初始偏见水平下的动态变化，并评估多种缓解策略的效果。

Method: 使用三种互补评估框架（基于规则的模式匹配、基于嵌入的语义相似性、下游任务性能），在三种初始偏见水平（0.1、0.3、0.6）和四种缓解策略下进行三代递归文本生成实验。

Result: 低初始偏见向模型固有偏见水平放大（+36%），高初始偏见则衰减（-26%）。对比增强方法在下游任务中显著减少偏见（低初始偏见98.8%，平均91%），尽管产生更高的嵌入偏见分数。

Conclusion: 语义相似性指标可能与行为公平结果背离，强调在负责任合成数据生成中需要多维评估的重要性。
...
TL;DR: 本文通过多轮"传话游戏"方法，利用多模态系统的偏好偏差来研究其隐藏语言，揭示了概念连接强度的量化特征，并构建了包含10,000+概念对的Telescope数据集。


<details>
  <summary>Details</summary>
Motivation: 研究封闭源多模态系统的隐藏语言，因为这些系统的黑盒架构使其对世界的理解方式不透明。通过系统在图像压缩和重建过程中的偏好偏差来探索其内在概念连接。

Method: 采用多轮"传话游戏"策略，观察概念在游戏中的共现频率，定量研究多模态系统中概念连接强度。构建Telescope数据集作为游戏框架的基础数据库。

Result: 开发了可扩展的测试时方法，能够构建多模态系统理解中概念连接的全局图谱，识别训练继承的偏好偏差，评估泛化能力进步，发现脆弱概念连接的稳定路径。

Conclusion: 本研究为多模态系统的隐藏语言提供了新视角，为未来多模态系统的可解释性和可控性研究奠定了基础，揭示了超越文本和视觉相似性的意外概念关系。
...
TL;DR: 提出了Squid Game评估框架，通过动态对抗性游戏环境评估大语言模型在资源受限和不对称信息设置下的表现，发现了模型可能存在的评估范式污染问题。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试难以跟上大语言模型的发展速度，存在数据污染风险，且主要假设良性、资源丰富的环境，缺乏对模型在压力下行为的评估。

Method: 设计了包含六个淘汰制关卡的游戏环境，评估模型在指令遵循、代码、推理、规划和安全对齐等多方面能力，让LLM与其他LLM对手进行交互式游戏。

Result: 评估了50多个LLM，观察到同一模型谱系中明显的代际性能跃迁，发现某些模型采用投机性捷径获胜，表明静态基准可能存在更高级别的评估范式污染。

Conclusion: 动态评估可以作为静态评估的补充，Squid Game框架为建立更可信的评估体系提供了新视角。
...
TL;DR: 研究表明AI语音系统能够隐式学习人类社交规范，在礼貌提示下会像人类一样放慢语速来传达礼貌，展现了AI作为社交参与者的能力。


<details>
  <summary>Details</summary>
Motivation: 研究AI语音系统是否能够学习人类社交中隐含的非编程性语言线索，特别是通过语速变化传达礼貌的微妙特征。

Method: 使用两个领先AI平台（AI Studio和OpenAI）的22个合成语音，在"礼貌正式"和"随意非正式"条件下朗读固定脚本，测量语音持续时间。

Result: 两个AI平台在礼貌提示下都产生了更慢的语速，效应量非常大。AI Studio的所有语音和OpenAI的大多数语音都显示出统计显著性差异。

Conclusion: AI能够隐式学习并复制人类沟通中的心理细微差别，突显了其作为社交参与者强化人类社会规范的新兴角色。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是系统评估LLMs在国际关系领域的国家层面偏见，特别是针对联合国安理会五个常任理事国，以了解模型偏见的多维性和对实际应用的影响。

Method: 基于联合国安理会历史记录开发了包含三个不同测试的偏见评估框架，并提出了结合检索增强生成和基于反思的自反思技术的去偏见框架。

Result: 实验结果显示LLMs存在明显的国家层面偏见，具有更强推理能力的模型偏见更小、性能更好，提出的去偏见框架有效减少了偏见并提升了性能。

Conclusion: 结论强调在将LLMs应用于国际关系领域时，需要同时评估国家层面偏见和性能表现，以确保模型的公平性和可靠性。
...
<details>
  <summary>Details</summary>
Motivation: Transformer的二次复杂度限制了长序列建模能力，现有稀疏注意力方法如RingAttention感受野有限且缺乏适应性，需要新的稀疏注意力机制来平衡计算效率和建模能力。

Method: 提出ΠAttention，将注意力分解为三个组件：环形局部邻域、确定性π步长跳跃和自适应融合门，通过周期性结构实现可预测的远距离token覆盖。

Result: 在语言建模、检索和视觉语言任务中，ΠAttention达到或超过密集注意力的质量，困惑度比RingAttention低8.3%，相同上下文长度下GPU使用减少50%。
...
TL;DR: 提出结合TextRank句子提取、医学命名实体识别和大语言模型的框架，用于提高医学文本摘要的忠实度。在MeQSum和BanglaCHQ-Summ数据集上微调LLaMA-2-7B模型，在质量和忠实度指标上均取得改进，超过零样本基线和先前系统。


<details>
  <summary>Details</summary>
Motivation: 消费者健康问题摘要可以促进医疗沟通，但不忠实的摘要错误呈现医学细节会带来严重风险，需要提高医学文本摘要的忠实度。

Method: 结合TextRank句子提取和医学命名实体识别，使用大语言模型（LLMs）进行医学文本摘要，在MeQSum（英语）和BanglaCHQ-Summ（孟加拉语）数据集上微调LLaMA-2-7B模型。
...
Result: 创建了首个开源公开的代码切换突尼斯方言语音翻译语料库，包含25小时语音数据和相应的翻译文本。
...
Result: 在测试的多个模型中，Gemini 2.0 Flash在质量和速度上表现突出，Gemma 3n因其良好性能和开源特性而受到认可。

Conclusion: 基于GenAI和RAG的聊天机器人能够有效简化学生对日常学术信息的访问，其中Gemini 2.0 Flash和Gemma 3n是最适合的模型选择。
...
TL;DR: 本研究探讨了使用GPT-4o在真实课堂环境中评估本科生计算语言学课程的简答题测验和项目报告的可行性，结果显示LLM与人类评分者具有高度相关性，但在技术性开放答案评分中存在一定变异性。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型越来越多地用于教育任务如评分，但其在真实课堂环境中与人类评估的一致性尚未得到充分研究。

Method: 收集约50名学生的五次测验答案和14个团队的项目报告，使用GPT-4o进行评分，并与课程助教的独立评分进行比较分析。

Result: GPT-4o与人类评分者达到强相关性（最高0.98），在55%的测验案例中得分完全一致；对于项目报告也显示出与人类评分的整体一致性，但在技术性开放答案评分中存在变异性。

Conclusion: 这项工作突出了基于LLM的评分系统的潜力和局限性，为推进现实学术环境中的自动评分做出了贡献。
...
TL;DR: 该研究发现多语言大语言模型采用枢轴语言表示机制：模型在不同语言间使用几乎相同的表示，而语言特定的解码在较后层出现。通过干预高频语言特征，可以抑制一种语言并用另一种语言替代模型输出。


<details>
  <summary>Details</summary>
Motivation: 探究多语言大语言模型内部如何表示语言多样性，以及为什么性能仍然偏向主导训练语言。

Method: 训练一系列不同多语言数据混合的LLM，使用跨层转码器和归因图分析内部机制。

Result: 发现模型采用枢轴语言表示机制，解码部分依赖于最后层中的高频语言特征，这些特征从模型第一层线性读取语言身份。
...
TL;DR: 研究发现情感AI模型在识别非洲裔美国方言英语(AAVE)时存在严重偏见，对AAVE文本的愤怒情绪误判率是标准美式英语的两倍以上，可能强化种族刻板印象。


<details>
  <summary>Details</summary>
Motivation: 当前情感检测模型主要基于主流文化规范标注，难以准确识别被排除在训练数据之外的方言表达，如AAVE，这在高风险应用领域存在安全隐患。

Method: 分析270万条洛杉矶地理标记推文，使用计算方法评估AAVE特征密度，由AAVE流利的非洲裔美国人标注情感，比较GPT、BERT等模型在不同方言文本上的表现。

Result: 模型在AAVE文本上的愤怒误判率显著更高(SpanEmo从25%升至60%)，模型预测与非内部标注者更关注粗俗特征，非洲裔居民比例高的社区被预测为更多愤怒、更少快乐。

Conclusion: 情感AI存在强化种族刻板印象的安全风险，需要开发文化和方言感知的情感计算系统。
...
TL;DR: 本文提出了一种对齐LLM参数空间的方法来解决任务算术中的负干扰问题，通过利用Transformer架构的对称性，成功将高级推理技能转移到非推理模型中。


<details>
  <summary>Details</summary>
Motivation: 任务算术在LLM间传输技能时经常因训练过程中的模型分歧而产生负干扰，这限制了其效果。

Method: 首先对齐模型的参数空间，利用Transformer架构的置换、旋转和缩放对称性，针对GQA和SwiGLU层开发了权重和激活两种对齐方法。

Result: 在具有挑战性的推理基准测试中，该方法始终优于标准任务算术，成功将高级推理技能转移到非推理模型。

Conclusion: 这项工作为在演进的LLM家族中合并和传输专门技能提供了有效方法，减少了冗余微调并增强了模型适应性。
...
TL;DR: 本研究探讨了LLM在对话判断任务中的表现变化，发现即使是最小的对话上下文也会显著改变模型判断，对话框架是LLM评估的关键因素。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在各种任务中作为评判者的应用增加，需要了解它们是否能可靠地评估需要社交或对话判断的任务，特别是在对话框架下模型判断的稳定性。

Method: 通过对比模型在直接事实查询和对话判断任务中的表现，并在两种条件下施加反驳压力，测量模型在对话压力下保持立场的坚定程度。

Result: 研究发现GPT-4o-mini在社交框架任务中表现出谄媚倾向，而Llama-8B-Instruct变得过于批判，所有模型的平均性能变化为9.24%。

Conclusion: 对话框架是影响LLM判断的关键因素，提出的框架为诊断模型信念提供了可复现的方法，有助于开发更可信的对话系统。
...
TL;DR: ICX360是一个开源Python工具包，专注于解释LLMs的输出，特别是用户提供的上下文或提示。它实现了三种最新工具，使用黑盒和白盒方法（分别通过扰动和梯度）来解释LLMs。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs在日常生活中的普及并进入高风险应用领域，开发解释LLMs输出的工具变得至关重要。

Method: ICX360工具包实现了三种工具，使用黑盒方法（通过扰动）和白盒方法（通过梯度）来解释LLMs。

Result: 开发了一个开源工具包，包含快速入门指南和详细教程，涵盖检索增强生成、自然语言生成和越狱等用例。

Conclusion: ICX360为解释LLMs输出提供了一个实用的工具包，特别关注用户提供的上下文，有助于提高LLMs在高风险应用中的可信度。
...
TL;DR: 本文研究发现大语言模型在二元决策任务中存在格式层面的负向偏见，即提示格式比负向响应的语义更能影响模型回答。模型在缺乏足够知识回答是非问题时倾向于生成负向响应，导致负向偏见。
...
Result: 研究发现提供相关上下文和"我不知道"选项通常能减少负向偏见，而思维链提示倾向于放大偏见。负向偏见的程度因提示类型而异，提示会影响回答方向。

Conclusion: 本文揭示了影响负向偏见的各种因素，为在大语言模型中减轻负向偏见提供了关键见解。
...
Method: 基于9个现有专家标注的实体链接数据集构建MedPath，使用UMLS最新版本标准化所有实体，映射到62个其他生物医学词汇表，并在最多11个生物医学词汇表中添加完整的本体路径。
...
TL;DR: 本文研究了工具增强语言模型在数学推理中的工具诱导短视现象，发现虽然工具使用提高了最终答案准确率，但导致了推理质量的显著下降。


<details>
  <summary>Details</summary>
Motivation: 研究工具增强语言模型在使用外部工具时是否真正进行可信推理，而不是简单地将工具输出作为推理替代品。

Method: 开发了PYMATH基准测试集（1,679个竞赛级数学问题），建立多维度评估套件量化推理退化，并提出基于偏好优化的框架来重新对齐模型使用工具的方式。

Result: 工具增强语言模型在最终答案准确率上提升19.3个百分点，但推理行为显著恶化（非工具模型在推理过程比较中胜出41.5%），工具使用频率越高，推理连贯性越差。

Conclusion: 工具使用会导致推理质量下降，但通过偏好优化框架可以改善工具使用方式，同时提高最终答案准确率和推理深度。
...
TL;DR: EMSQA是一个包含24.3K个医疗问题的数据集，涵盖10个临床科目和4个认证级别。研究者提出了Expert-CoT和ExpertRAG方法，通过结合专业领域知识和患者数据，显著提升了LLMs在医疗问答中的表现。
...
Method: 开发了EMSQA数据集，并提出了Expert-CoT（基于特定临床科目和认证级别的思维链提示）和ExpertRAG（基于科目对齐文档和真实患者数据的检索增强生成）。

Result: Expert-CoT比普通CoT提示提升2.05%，结合ExpertRAG比标准RAG基线提升4.59%准确率。32B专业增强LLMs通过了所有EMS认证模拟考试。

Conclusion: 结合专业领域知识和真实患者数据的方法能够显著提升LLMs在医疗问答中的性能，特别是在高风险认证环境中。
...
TL;DR: 本文提出了一种基于多模态大语言模型的交互式同行评审系统，通过整合文本和视觉信息、基于OpenReview数据的检索增强生成，以及将评审转化为可操作待办清单，为学术论文提交前提供有效的修订指导。
...
Method: 开发交互式web系统，集成多模态LLM处理文本和视觉信息，使用检索增强生成技术基于OpenReview数据提升评审质量，并提出Action:Objective[#]格式将评审转化为结构化可追踪的待办清单。

Result: 实验结果表明，该系统能够生成更全面、更有用的符合专家标准的评审意见，优于消融基线，推进了透明、以人为本的学术辅助。

Conclusion: 该系统成功实现了多模态、社区感知的同行评审模拟，为学术论文提交前的修订提供了有效的结构化指导，并可与现有学术写作平台无缝集成。
...
### [126] [Automated Analysis of Learning Outcomes and Exam Questions Based on Bloom's Taxonomy](https://arxiv.org/abs/2511.10903)
*Ramya Kumar,Dhruv Gulwani,Sonit Singh*

Main category: cs.CL

TL;DR: 本文研究了基于布鲁姆分类法的考试题目和学习成果自动分类，比较了传统机器学习、RNN、Transformer和大语言模型在600条标注数据上的表现，发现数据增强的SVM模型效果最佳（94%准确率），而复杂模型在有限数据下容易过拟合。


<details>
  <summary>Details</summary>
Motivation: 探索如何自动对考试题目和学习成果按照布鲁姆分类法进行准确分类，解决教育领域中认知层次分类的实际需求。

Method: 使用600条标注数据，比较了传统ML模型（朴素贝叶斯、逻辑回归、SVM）、RNN架构（LSTM、BiLSTM、GRU、BiGRU）、Transformer模型（BERT、RoBERTa）和大语言模型（OpenAI、Gemini等），采用不同预处理和数据增强策略。

Result: 数据增强的SVM模型表现最佳（准确率94%），RNN和BERT模型存在严重过拟合，RoBERTa初期表现良好但后期出现过拟合迹象，大语言模型零样本评估中OpenAI和Gemini表现最好（准确率约0.72-0.73）。

Conclusion: 在有限数据条件下，复杂深度学习模型容易过拟合，而精心设计的数据增强和简单算法（如增强SVM）在布鲁姆分类法分类任务中更具价值。
...
<details>
  <summary>Details</summary>
Motivation: 探索大语言模型在罕见疾病诊断方面的能力，目前这一领域研究较少，而罕见疾病诊断对临床医学具有重要意义。

Method: 使用从《豪斯医生》剧集提取的176个症状-诊断对构建数据集，评估GPT 4o mini、GPT 5 mini、Gemini 2.5 Flash和Gemini 2.5 Pro四种模型在叙事性医疗推理任务上的表现。

Result: 模型表现差异显著，准确率范围16.48%-38.64%，新一代模型相比旧模型性能提升2.3倍。所有模型在罕见疾病诊断方面都面临重大挑战。

Conclusion: 虽然所有模型在罕见疾病诊断上都有困难，但跨架构的性能改进显示了未来发展的有希望方向。该教育验证基准为叙事医疗推理建立了基线性能指标，并为推进AI辅助诊断研究提供了公开可访问的评估框架。
...
TL;DR: CardioEmbed是一个专门针对心脏病学领域的嵌入模型，基于Qwen3-Embedding-8B，使用7本心脏病学教科书进行对比学习训练，在心脏特异性语义检索任务中达到99.60%的检索准确率，比当前最佳医学嵌入模型MedTE提升15.94个百分点。


<details>
  <summary>Details</summary>
Motivation: 现有生物医学文本嵌入模型主要基于PubMed研究文献开发，但临床心脏病学实践更依赖教科书中的程序性知识和专业术语，这种研究实践差距限制了现有模型在心脏病学临床应用中的效果。

Method: 基于Qwen3-Embedding-8B模型，使用对比学习方法在7本综合心脏病学教科书（约15万句去重后）上训练，采用InfoNCE损失和批次内负样本策略。

Result: 在心脏特异性语义检索任务中达到99.60%检索准确率，比MedTE提升15.94个百分点；在MTEB医学基准测试中，BIOSSES得分为0.77 Spearman，SciFact得分为0.61 NDCG@10。
...
TL;DR: DiscoX是一个用于评估专业领域中文-英文篇章级翻译的新基准，包含7个领域的200篇专业文本。研究还开发了Metric-S无参考评估系统，实验显示即使最先进的LLM在专业翻译任务上仍落后于人类专家。


<details>
  <summary>Details</summary>
Motivation: 当前翻译评估方法主要关注句子级准确性和流畅性，无法充分评估专业领域所需的篇章级连贯性和术语精确性，这限制了知识传播和跨语言学术交流的质量。

Method: 构建DiscoX基准数据集（7个领域200篇专业文本，平均长度超1700词），开发Metric-S无参考评估系统进行细粒度自动评估（准确性、流畅性、适当性）。

Result: Metric-S与人工评估高度一致，显著优于现有指标；实验显示最先进的LLM在专业翻译任务上仍明显落后于人类专家。

Conclusion: DiscoX基准和Metric-S评估系统为更严格的翻译评估提供了可靠框架，验证了实现专业级机器翻译仍面临重大挑战，有助于推动LLM翻译技术的未来发展。
...
TL;DR: 本文对主流开源DPO数据集进行了首次全面分析，使用Magpie框架标注样本质量，并基于分析结果构建了新的DPO混合数据集UltraMix，该数据集比性能最佳的单数据集小30%但性能更优。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏对开源DPO数据集的系统比较，主要由于计算成本高和缺乏详细质量标注，难以理解偏好选择方式、任务类型覆盖范围以及样本级别的人类判断质量。

Method: 使用Magpie框架对每个样本进行任务类别、输入质量和偏好奖励的标注，实现可扩展的细粒度偏好质量检查；基于分析结果从五个语料库中精选样本构建UltraMix混合数据集。

Result: 揭示了不同数据集在奖励边际上的结构和质量差异；UltraMix比性能最佳的单数据集小30%但在关键基准测试中表现更优。
...
TL;DR: 提出了一种增强基于自动机结构化生成多样性的新方法，通过利用自动机遍历历史来引导LLM生成新颖的结构模式，在保持生成效率的同时显著提高了结构多样性。
...
Method: 利用自动机遍历历史来引导LLM朝向新颖的结构模式，通过跟踪生成过程中的结构路径来促进多样性。

Result: 评估显示该方法显著提高了结构和内容多样性，同时保持了可比的生成效率。案例研究展示了在生成开源库测试用例方面的有效性。

Conclusion: 该方法成功解决了结构化生成中的多样性不足问题，为LLM生成多样化结构化输出提供了有效解决方案。
...
TL;DR: 本文发现当前文本嵌入模型存在一致的偏差，并提出了一种无需训练、轻量级的重归一化方法，显著提升了在MMTEB基准上的性能表现。


<details>
  <summary>Details</summary>
Motivation: 发现现有文本嵌入模型输出存在系统性偏差，即所有嵌入向量都包含一个几乎相同的偏差分量μ，这影响了模型的性能表现。

Method: 提出重归一化方法，有两种变体：直接从嵌入向量中减去偏差μ，或减去嵌入向量在μ上的投影。该方法无需额外训练，可直接应用于现有模型。

Result: 在MMTEB基准测试中，重归一化显著提升了38个模型的性能：检索任务提升9.7σ，分类任务提升3.1σ，其他任务提升0.8σ。理论预测和实验均表明减去投影的方法效果更好。

Conclusion: 重归一化是一种有效且通用的方法，能够显著改善文本嵌入模型的性能，且无需额外训练成本。
...
TL;DR: 本文研究LLMs能否检测自身幻觉，提出检测框架和基于Chain-of-Thought的分类方法，实验显示GPT-3.5 Turbo能检测58.2%的自身幻觉。


<details>
  <summary>Details</summary>
Motivation: 大语言模型能生成流畅回答但有时会产生事实幻觉，研究LLMs能否检测自身幻觉。

Method: 将幻觉检测制定为句子分类任务，提出基于Chain-of-Thought的分类方法从模型参数中提取知识。

Result: 实验结果表明，GPT-3.5 Turbo配合CoT能检测58.2%的自身幻觉。

Conclusion: 如果模型参数包含足够知识，配合CoT的LLMs能够检测幻觉。
...
TL;DR: 本文提出了一个分析美国总统辩论中人身攻击的框架，通过手动标注2016、2020和2024年选举周期的辩论记录，结合统计和基于语言模型的分析方法，研究微调transformer模型和通用大语言模型在检测正式政治演讲中人身攻击的潜力。


<details>
  <summary>Details</summary>
Motivation: 人身攻击已成为美国总统辩论的显著特征，在选举期间对公众认知产生重要影响。检测此类攻击可以提高政治话语的透明度，为记者、分析师和公众提供洞察。深度学习和基于transformer的模型（特别是BERT和大语言模型）的发展为有害语言的自动检测创造了新机会。
...
Conclusion: 任务特定的现代语言模型适应可以为政治传播的深入理解做出贡献，为政治话语分析提供了新的技术途径。
...
TL;DR: SAP²方法是一个两阶段框架，通过动态修剪和整合相关上下文关键词来解决ASR系统在长上下文场景中的性能问题，在SlideSpeech和LibriSpeech数据集上取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 自动语音识别系统在常见条件下表现优异，但在需要领域特定知识的上下文场景（如会议演示）中难以有效利用长上下文信息，主要受限于模型上下文窗口和大量上下文噪声中相关信息稀疏的问题。

Method: 提出SAP²方法，采用两阶段动态修剪和整合相关上下文关键词的框架，每个阶段利用提出的语音驱动注意力池化机制，有效压缩上下文嵌入同时保留语音显著信息。

Result: 在SlideSpeech和LibriSpeech数据集上分别达到7.71%和1.12%的词错误率，在SlideSpeech上相比非上下文基线显著降低偏置关键词错误率41.1%，并在广泛上下文输入条件下保持稳健的可扩展性。

Conclusion: SAP²方法有效解决了ASR系统在上下文场景中的长上下文信息利用问题，实现了最先进的性能表现和良好的可扩展性。
...
TL;DR: 本文提出了一个基于语言学理论的副词超义类型学，扩展了WordNet的语义分类覆盖范围，并通过实证验证了该分类系统的可靠性和实用性。
...
Method: 开发了一个基于语言学理论的副词超义类型学，通过试点标注研究进行实证验证，涵盖方式、时间、频率、程度、领域、说话者导向和主语导向等主要语义域。
...
<details>
  <summary>Details</summary>
Motivation: 神经语言模型在处理细粒度词义时往往过度依赖全局句子表示，无法捕捉局部语义细节，这限制了其在词义消歧等任务上的表现。

Method: LANE方法通过故意将模型的学习焦点转移到目标词上，生成具有挑战性的负训练样本，强制模型在相同句子但不同标记词之间创建更大的可分性。

Result: 在词汇语义变化检测和词义消歧基准测试上的实验结果表明，该方法能产生更具区分性的词表示，性能优于标准对比学习基线。

Conclusion: 该方法模型无关，可以集成到现有的表示学习框架中，能够更好地捕捉具有挑战性环境中的细微意义差异。
...
TL;DR: 本文提出了一种从知识图谱生成问答对的混合方法，结合了基于规则的模板生成和LLM的精细化处理，实现了可扩展、高质量的自然语言问答生成。


<details>
  <summary>Details</summary>
Motivation: 现有方法在可扩展性、语言质量和事实一致性方面存在不足，需要一种能够高效生成高质量问答对的方法来支持教育平台、传播工具和大型语言模型的开发测试。

Method: 首先基于关系对知识图谱三元组进行聚类，通过实体类型和关系创建可重用的自然语言模板；然后使用LLM优化模板的清晰度和连贯性；最后通过从知识图谱中选择干扰项来实例化答案选项。

Result: 实验表明这种混合方法能够高效生成高质量的问答对，在可扩展性、流畅性和语言精确性方面表现出色。

Conclusion: 提出的混合方法成功解决了现有方法的局限性，实现了可扩展且高质量的问答对生成，为教育平台和语言模型开发提供了有效支持。
...
Conclusion: iMAD通过智能选择性地触发多智能体辩论，实现了计算效率与准确性的双重提升，为多智能体系统提供了更实用的解决方案。
...
TL;DR: 该论文提出了一种针对最先进机器学习模型的新型对抗攻击策略，通过生成模糊输入来混淆模型，并探索提升模型鲁棒性的未来发展路径。


<details>
  <summary>Details</summary>
Motivation: 近年来机器学习模型被发现存在多种脆弱性，可能危及模型及其应用系统的安全，因此需要分析现有对抗攻击方法并开发新的攻击策略。

Method: 开发最大困惑度的对抗实例，使用机器学习和深度学习方法，分析多个数据集并创建混淆性对抗样本，特别将孟加拉语纳入对抗攻击领域。

Result: 论文提出了针对当前最先进机器学习模型的新型对抗攻击方法，通过生成模糊输入成功混淆了模型。

Conclusion: 通过开发具有最大困惑度的对抗攻击方法，为未来提升机器学习模型鲁棒性的发展提供了路径，同时严格遵循效用降低和效率原则。
...
TL;DR: 提出LAET方法，通过选择性微调预训练大语言模型的关键层来降低计算成本，在金融NLP任务中表现出色，甚至超越GPT-4等更大模型。


<details>
  <summary>Details</summary>
Motivation: 现有金融大语言模型如BloombergGPT和FinMA虽然性能优异，但计算需求高，限制了实际部署应用。需要开发更高效的微调方法。

Method: 提出层自适应集成调优(LAET)策略，通过分析隐藏状态表示来选择性微调预训练LLM中最有效的层，同时冻结不太关键的层。

Result: LAET显著降低了计算开销，同时在金融NLP任务中表现出色，使用较小模型(约30亿参数)就能超越GPT-4等现有基准和最先进LLM。
...
TL;DR: LaoBench是首个针对老挝语的大规模、高质量、多维度基准数据集，包含超过17,000个样本，涵盖知识应用、K12基础教育和多语言翻译三个核心维度，用于评估LLMs在老挝语上的综合语言理解和推理能力。


<details>
  <summary>Details</summary>
Motivation: 填补大型语言模型在低资源语言（特别是东南亚语言如老挝语）评估方面的空白，促进对代表性不足语言AI技术的研究和发展。

Method: 构建包含17,000多个样本的数据集，分为开源和闭源子集；采用专家人工策划与自动化智能体辅助验证相结合的数据构建流程，确保语言准确性、文化相关性和教育价值。

Result: 在LaoBench上对多个最先进LLMs进行基准测试发现，当前模型在各种任务中掌握老挝语仍面临重大挑战。

Conclusion: LaoBench将促进对代表性不足东南亚语言的AI技术进一步研究和发展，填补了老挝语评估基准的空白。
...
TL;DR: 本文介绍了M-DAIGT共享任务，旨在检测新闻文章和学术写作中的AI生成文本，包含两个子任务并发布了包含30,000个样本的大规模基准数据集。
...
TL;DR: 语言模型和婴儿都能轻松学习自然语言，但语言模型缺乏人类的归纳偏置，因此对某些不可能语言的习得能力与人类不同。


<details>
  <summary>Details</summary>
Motivation: 探讨语言模型与人类婴儿在语言习得能力上的差异，特别是对自然语言和不可能语言的学习表现。

Method: 回顾相关文献，分析语言模型对自然语言和不可能语言的学习表现，并与人类婴儿的习得能力进行对比。

Result: 语言模型通常能同样好地学习自然语言和许多不可能语言，而难以学习的不可能语言往往只是更复杂或更随机。
...
TL;DR: MajinBook是一个开放目录，旨在促进使用影子图书馆（如Library Genesis和Z-Library）进行计算社会科学和文化分析。通过将这些众包档案的元数据与Goodreads的结构化书目数据链接，创建了一个包含超过53.9万本英文书籍引用的高精度语料库，涵盖三个世纪，并丰富了首次出版日期、流派和流行度指标。
...
Method: 将影子图书馆的元数据与Goodreads的结构化书目数据链接，优先使用原生数字EPUB文件确保机器可读质量，包括法语、德语和西班牙语的辅助数据集。

Result: 创建了一个包含超过539,000本英文书籍引用的高精度语料库，涵盖三个世纪，包含首次出版日期、流派、评分和评论等流行度指标。
...
Result: 在真实世界的2人和3人对话测试集上（11名参与者，总计6.8小时的双耳自我中心音频数据）显示系统在多对话环境中能够泛化识别和隔离对话伙伴。

Conclusion: 这项工作标志着向能够主动适应对话动态和参与度的听力助手迈出了一步，为改善多对话环境下的听觉辅助提供了有效解决方案。
...
TL;DR: PRBench是一个针对金融和法律领域的现实世界专业推理基准测试，包含1100个专家编写的任务和19356个专家制定的评分标准，是这两个领域最大的公开基准测试。评估20个领先模型显示在专业推理方面仍有很大改进空间。
...
Method: 引入PRBench基准测试，招募182名合格专业人士（持有法学博士、特许金融分析师资格或6年以上经验）编写基于实际工作流程的任务，并通过严格的质量管道验证专家制定的评分标准。

Result: 评估20个领先模型显示，在困难子集上的最高得分仅为0.39（金融）和0.37（法律），表明模型在专业推理方面仍有显著改进空间。常见失败模式包括判断不准确、缺乏过程透明度和推理不完整。

Conclusion: 当前模型在专业推理能力上存在关键差距，可靠性不足以支持专业采用，需要进一步改进判断准确性、过程透明度和推理完整性。
...
### [148] [LEMUR: Large scale End-to-end MUltimodal Recommendation](https://arxiv.org/abs/2511.10962)
...
<details>
  <summary>Details</summary>
Motivation: 传统基于ID的推荐系统面临冷启动和泛化挑战，现有工业多模态推荐采用两阶段训练，存在多模态学习与推荐目标不一致以及无法动态适应新数据的问题。

Method: 提出LEMUR端到端训练框架，采用新颖的记忆库机制增量累积历史多模态表示，降低计算成本，实现多模态和推荐组件的联合优化。

Result: 在抖音搜索部署一个月后，查询变化率衰减减少0.843%，QAUC提升0.81%；在抖音广告中关键离线指标也有显著提升。
...
### [149] [Enhancing Group Recommendation using Soft Impute Singular Value Decomposition](https://arxiv.org/abs/2511.11172)
...
<details>
  <summary>Details</summary>
Motivation: 随着群组活动的日益普及，需要开发基于群组成员集体偏好的群组推荐方法。现有方法由于数据稀疏性和高维性而面临困难。

Method: 使用软填充奇异值分解进行低秩矩阵补全，处理稀疏高维数据。

Result: 在Goodbooks、Movielens和合成数据集上，该方法在小型用户群组的召回率上优于基于群组矩阵分解的基线方法，在所有群组大小上获得可比结果，且恢复的矩阵秩更低。
...
### [150] [Align$^3$GR: Unified Multi-Level Alignment for LLM-based Generative Recommendation](https://arxiv.org/abs/2511.11255)
*Wencai Ye,Mingjie Sun,Shuhang Chen,Wenjin Wu,Peng Jiang*

Main category: cs.IR

TL;DR: 提出了Align$^3$GR框架，通过统一token级、行为建模级和偏好级对齐，解决LLMs在推荐系统中的语义和行为不对齐问题，显著提升了推荐性能。


<details>
  <summary>Details</summary>
Motivation: LLMs在利用结构化世界知识和多步推理方面具有优势，但在转化为实际推荐系统时存在语义和行为不对齐的根本挑战。

Method: 提出了Align$^3$GR框架，包括：融合用户-项目语义和协作信号的双重token化；具有双向语义对齐的增强行为建模；结合自博弈(SP-DPO)和真实世界反馈(RF-DPO)的渐进式DPO策略，用于动态偏好适应。

Result: 在公共数据集上，Align$^3$GR在Recall@10和NDCG@10上分别比SOTA基线提升了+17.8%和+20.2%，在在线A/B测试和工业级大规模推荐平台的全规模部署中取得了显著收益。

Conclusion: Align$^3$GR框架有效解决了LLMs在推荐系统中的对齐问题，显著提升了推荐性能，并在实际工业应用中验证了其有效性。
...
TL;DR: 本文研究了多目标优化中的帕累托剪枝问题，将其重新定义为多赢家投票问题，分析了现有质量度量方法的反直觉行为，提出了新的定向覆盖度量方法，并研究了不同质量度量的计算复杂性。


<details>
  <summary>Details</summary>
Motivation: 现实决策问题通常涉及多个目标优化，决策者需要从所有帕累托最优解中选择一个实施。为减轻决策者认知负担，需要计算固定大小的帕累托最优解子集来最佳代表完整集合。

Method: 将帕累托剪枝重新定义为多赢家投票问题，对现有质量度量进行公理分析，引入新的定向覆盖度量，分析各种质量度量的计算复杂性，并进行实验评估。

Result: 发现现有质量度量存在多个反直觉行为，确定了质量度量优化的计算复杂性边界，实验表明质量度量选择对所选解集特征有决定性影响，提出的新度量在各种设置下表现竞争性甚至更优。

Conclusion: 帕累托剪枝的质量度量选择至关重要，新提出的定向覆盖度量在多个场景下表现良好，为多目标决策提供了更有效的解决方案选择方法。
...
TL;DR: 该论文研究了如何将抽象论证框架的问题编码到(Q)SAT中，并设计了保持团宽度的线性归约方法，为理解团宽度在编码中的能力提供了新的视角。


<details>
  <summary>Details</summary>
Motivation: 现代SAT求解器在小树宽度的实例上表现高效，但团宽度作为更一般的图参数，在编码方面的研究较少。作者希望通过研究抽象论证框架来探索团宽度在编码中的潜力。

Method: 设计了从论证问题到(Q)SAT的新归约方法，这些归约线性保持团宽度，形成了有向分解引导(DDG)归约。

Result: 为所有论证语义（包括计数）建立了新的结果，证明了DDG归约引起的开销在合理假设下无法显著改进。

Conclusion: 该研究开启了理解团宽度编码能力的新方向，为论证框架的高效求解提供了理论基础，并证明了所提出归约方法的紧致性。
...
<details>
  <summary>Details</summary>
Motivation: 在反事实决策中，决策者通常通过比较候选行动的期望潜在结果来做出选择。本文旨在开发新的决策规则，更准确地评估个体层面上的潜在结果排序和获得最佳结果的可能性。

Method: 引入PoR和PoB两个新指标，建立识别定理并推导边界，提出相应的估计方法，通过数值实验验证估计器的有限样本性质。

Result: 建立了PoR和PoB的识别定理和边界，开发了有效的估计方法，数值实验验证了估计器具有良好的有限样本性能，并在真实数据集上展示了应用效果。

Conclusion: PoR和PoB为反事实决策提供了新的有效工具，能够更准确地评估个体层面的潜在结果排序和获得最佳结果的可能性，具有实际应用价值。
...
TL;DR: 这篇论文从适应性角度重新审视大语言模型的推理能力，提出将推理努力根据任务难度和不确定性进行动态分配的概念，并建立了系统的分类法来组织现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注推理效率，但忽视了当前LLM在不同复杂度任务上采用统一推理策略的根本问题——对简单问题生成过长推理链，而对困难问题推理不足。

Method: 论文首先在LLM背景下形式化了演绎、归纳和溯因推理，然后将适应性推理定义为控制增强的策略优化问题，最后提出了系统分类法将现有方法分为基于训练的方法和免训练方法。

Result: 建立了连接经典认知范式与算法实现的统一框架，澄清了不同机制在实践中如何实现适应性推理，并实现了跨不同策略的系统比较。
...
Result: 在计算机科学研究知识图谱上评估，从1K论文到10M论文规模，相比TransE、RotatE、DistMult等基线方法获得持续改进。在10M论文数据集上达到0.612 MRR，相对最佳基线提升4.8%，同时保持高效训练和85ms的推理时间。
...
TL;DR: 本研究开发了一个多智能体AI框架，用于从碎片化的碰撞数据中重建事故前场景并推断车辆行为。该框架在39起复杂追尾事故案例中实现了100%的准确率，超越了人类研究员的92%准确率。
...
Method: 采用两阶段协作框架：第一阶段从多模态输入生成自然语言事故重建；第二阶段结合这些重建与时间性事件数据记录器进行深入事故推理。系统处理了277起追尾前车减速碰撞案例。

Result: 在39起复杂追尾事故案例评估中，框架实现了完美准确率，成功识别最相关EDR事件并正确区分撞击与被撞车辆，超越了人类研究员的92%准确率。系统在处理不完整数据时仍保持稳健性能。
...
### [157] [Enhancing Demand-Oriented Regionalization with Agentic AI and Local Heterogeneous Data for Adaptation Planning](https://arxiv.org/abs/2511.10857)
*Seyedeh Mobina Noorani,Shangde Gao,Changjie Chen,Karla Saldana Ochoa*

Main category: cs.AI

TL;DR: 本文提出了一种基于智能AI的规划支持系统，用于创建动态规划单元，通过整合人在环原则实现透明和适应性，应用于灾害规划中的需求导向区域生成。
...
Method: 构建基于代表性初始化空间约束自组织映射（RepSC-SOM）的平台，扩展传统SOM方法，结合自适应地理过滤和区域增长细化，AI智能体能够推理、规划并指导过程，建议输入特征、引导空间约束并支持交互探索。

Result: 通过佛罗里达州杰克逊维尔市洪水相关风险的案例研究，展示了平台允许用户交互式探索、生成和评估区域化的能力，结合计算严谨性和用户驱动的决策制定。

Conclusion: 该系统成功实现了动态规划单元的创建，将计算技术与用户参与相结合，为灾害规划提供了更灵活有效的解决方案。
...
### [158] [LLM enhanced graph inference for long-term disease progression modelling](https://arxiv.org/abs/2511.10890)
...
<details>
  <summary>Details</summary>
Motivation: 当前方法过度简化了大脑连接性的复杂关系，仅假设单一模态的大脑连接组作为疾病传播基质，导致病理传播预测不准确。同时，纯数据驱动的方法由于缺乏适当约束而面临可识别性问题。

Method: 利用大型语言模型作为区域变量相互作用的专家指导，结合多模态关系和多样化疾病驱动机制，同时优化长期疾病轨迹构建和生物约束的图结构。

Result: 在阿尔茨海默病队列的tau-PET成像数据上验证，新框架相比传统方法具有更优的预测准确性和可解释性，并揭示了传统连接性测量之外的额外疾病驱动因素。

Conclusion: 该框架通过结合LLMs的专家知识，能够更准确地学习疾病进展并构建具有更好可识别性的生物约束图结构，为神经退行性疾病研究提供了新途径。
...
TL;DR: 论文探讨了自主AI系统在遇到无法完全满足所有操作约束的新场景时，需要超越训练策略来构建、评估和证明候选行动方案的能力要求。


<details>
  <summary>Details</summary>
Motivation: 自主AI系统在部署时会不可避免地遇到训练中未覆盖的场景，需要能够处理不完全满足约束的情况，以符合人类期望和价值观。

Method: 通过理论分析和实证案例研究，识别智能体决策所需的知识类型，包括规范性、实用性和情境性理解。

Result: 确定了智能体在复杂现实环境中做出稳健决策所需的知识整合要求，以确保目标达成与人类期望的一致性。

Conclusion: 自主AI系统需要整合多种知识类型来选择和追求更符合人类期望的行动方案，这对于在复杂环境中实现目标至关重要。
...
TL;DR: 提出了一种新的不完整方法来打破抽象结构中的对称性，通过更好地利用其表示来处理不可区分对象产生的对称性，相比之前的方法更高效。
...
Conclusion: 新方法通过优化抽象结构的表示利用，有效提高了对称性破坏的效率，为约束编程中的抽象结构处理提供了改进方案。
...
TL;DR: 本文研究发现多智能体辩论中角色分配策略对推理性能有显著影响，提出"Truth Last"策略可提升22%性能，并针对实际应用中未知真相的问题提出MADC策略，通过路径一致性评估和模拟最高一致性角色作为真相，在多个LLM上验证了其先进性能。


<details>
  <summary>Details</summary>
Motivation: 多智能体辩论在提升LLM推理能力方面显示出潜力，但角色分配策略这一关键方面尚未充分探索。本文旨在研究不同观点角色分配对多智能体辩论性能的影响。

Method: 提出"Truth Last"角色分配策略和Multi-Agent Debate Consistency (MADC)策略。MADC通过路径一致性评估独立角色间的一致性，模拟一致性最高的角色作为真相，系统性地模拟和优化多智能体辩论的核心机制。

Result: 在9个LLM模型（包括DeepSeek-R1蒸馏模型）的挑战性推理任务上验证，MADC持续表现出先进性能，有效克服了MAD的性能瓶颈，"Truth Last"策略可将MAD性能提升高达22%。

Conclusion: MADC策略为LLM智能体扩展提供了关键改进路径，能够系统性地优化多智能体辩论机制，在未知真相的实际应用中表现出色。
...
Method: 提出DSS框架，利用可微分仿真器Waymax作为状态预测器和评估器，基于硬编码动力学实现准确状态预测，利用可微分特性有效搜索动作序列，通过梯度下降优化想象轨迹中的动作。

Result: 实验表明，DSS结合规划梯度和随机搜索，相比序列预测、模仿学习、无模型RL和其他规划方法，显著提高了跟踪和路径规划精度。

Conclusion: DSS框架通过可微分仿真器实现了有效的动作序列搜索和优化，为自动驾驶规划提供了新的解决方案。
...
TL;DR: 提出了一种新的广义规划方法，通过从训练问题中学习条件-动作规则来构建广义规划程序，这些程序可以直接执行或用于剪枝规划搜索空间。
...
Conclusion: 该方法能够学习有效的广义规划和搜索状态空间剪枝公理，为广义规划提供了一种简单而有效的新方法。
...
TL;DR: GGBench是一个专门评估几何生成推理的基准测试，旨在填补统一多模态模型在生成推理评估方面的空白，通过几何构造任务来衡量模型的语言理解和精确视觉生成能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要评估判别性理解或无约束图像生成，无法衡量统一多模态模型的集成认知过程——生成推理能力。几何构造任务天然需要语言理解和精确视觉生成的融合，为评估提供了理想测试平台。

Method: 提出GGBench基准测试，通过几何构造任务系统性地诊断模型的理解、推理和主动构建解决方案的能力。该基准提供了全面框架来评估生成推理过程。

Result: GGBench为下一代智能系统设定了更严格的标准，能够更准确地评估统一多模态模型的综合认知能力，特别是生成推理方面的表现。

Conclusion: 几何构造是评估统一多模态模型生成推理能力的理想测试平台，GGBench基准填补了现有评估方法的空白，为更全面地衡量智能系统的认知能力提供了新标准。
...
TL;DR: STaR框架通过慢思考机制增强LLM的表格推理能力，采用两阶段难度感知强化学习和轨迹级不确定性量化，显著提升推理深度和稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM表格推理方法存在推理深度不足、缺乏迭代优化以及推理过程不稳定的问题，限制了在实际应用中的可靠性。

Method: 提出STaR框架，包含：1）两阶段难度感知强化学习，从简单到复杂查询渐进学习；2）轨迹级不确定性量化，结合token级置信度和答案一致性选择可信推理路径。

Result: 在多个基准测试中，STaR实现了卓越的性能和增强的推理稳定性，并在跨域数据集上表现出强大的泛化能力。

Conclusion: STaR作为一个可靠且受认知启发的解决方案，为LLM表格推理提供了有效的慢思考能力，显著提升了推理质量和可靠性。
...
TL;DR: 本文提出了一种通过强制记录AI训练和推理中每个组件的文档化方法，创建防篡改、可验证的AI决策追踪工作流，以解决AI系统决策过程缺乏适当文档化的问题。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统在关键决策中的应用增加，缺乏决策过程文档化导致无法追溯责任链，当AI决策违反法律时难以在法庭上确定原因。

Method: 扩展DBOM概念，利用机密计算技术构建运行工作流，强制记录AI训练和推理过程中的每个组件，生成防篡改、可验证的完整追踪记录。

Result: 开发了一个运行工作流，能够生成防篡改、可验证且详尽的AI决策追踪，并通过区分有毒和可食用蘑菇的应用示例展示了工作流程。

Conclusion: 该方法提供了一种实用且彻底的解决方案，通过强制文档化确保AI决策的可追溯性，为确定责任链提供了技术基础。
...
TL;DR: 该论文提出了对比ABox解释的概念，用于回答"为什么a是C的实例而b不是"这类问题。相比单独解释正向蕴含或缺失蕴含的方法，对比解释同时考虑两者，能够聚焦于a和b之间的相关共性和差异。


<details>
  <summary>Details</summary>
Motivation: 现有的解释方法主要关注正向蕴含（为什么C(a)被知识库蕴含）或缺失蕴含（为什么C(b)不被蕴含）的单独解释，缺乏同时考虑两者的对比视角，无法有效回答关于不同个体间差异的对比性问题。

Method: 为描述逻辑本体论中的ABox推理开发了对比解释的适当概念，分析了在不同最优性标准下各种变体的计算复杂性，涵盖轻量级和更表达性的描述逻辑。实现了一种计算对比解释变体的方法，并在现实知识库的生成问题上进行了评估。

Result: 提出了对比ABox解释的形式化定义，分析了不同描述逻辑下对比解释的计算复杂性，实现了计算算法并在实际知识库问题上进行了验证。

Conclusion: 对比ABox解释提供了一种新颖的解释框架，能够同时考虑正向和缺失蕴含，有效回答关于个体间差异的对比性问题，为知识库推理提供了更全面的解释能力。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有基于规则方法缺乏泛化性，而数据驱动方法效率低、不透明且难以与人类直觉对齐的问题，需要一种能结合两者优势的社会导航方法。

Method: 提出RLSLM混合强化学习框架，将基于实证行为实验的社会运动模型集成到强化学习的奖励函数中，生成方向敏感的社会舒适度场，联合优化机械能和社会舒适度。

Result: 通过沉浸式VR人机交互实验证明，RLSLM在用户体验上优于最先进的基于规则模型，消融和敏感性分析显示其比传统数据驱动方法具有显著改进的可解释性。
...
TL;DR: 本文系统综述了多智能体强化学习（MARL）在现实约束下的鲁棒高效通信策略，包括消息扰动、传输延迟和有限带宽等问题，并探讨了在自动驾驶、分布式SLAM和联邦学习中的应用。
...
Method: 通过系统综述方法，分析近期在MARL通信策略方面的进展，重点关注消息扰动、传输延迟和带宽限制等现实约束条件。

Result: 识别了低延迟可靠性、带宽密集型数据共享和通信隐私权衡等关键挑战，并提出了在三个具体应用领域的解决方案。

Conclusion: 需要采用统一的方法共同设计通信、学习和鲁棒性，以弥合理论MARL模型与实际实现之间的差距，并指出了未来的研究方向。
...
TL;DR: CURENet是一个多模态模型，整合电子健康记录中的非结构化临床笔记、实验室测试和患者时间序列数据，使用LLM处理临床文本，变压器编码器处理纵向访问序列，在慢性疾病预测中达到94%以上准确率。


<details>
  <summary>Details</summary>
Motivation: 现有预测模型未能充分利用电子健康记录中多模态数据之间的交互、冗余和时间模式，通常只关注单一数据类型或忽略这些复杂性，限制了临床决策的全面性。

Method: 结合大型语言模型处理临床文本和文本化实验室测试，使用变压器编码器处理纵向顺序访问数据，整合三种数据模态来捕捉复杂交互。

Result: 在MIMIC-III和FEMH数据集上，CURENet在多标签框架下预测前10种慢性疾病时达到超过94%的准确率。
