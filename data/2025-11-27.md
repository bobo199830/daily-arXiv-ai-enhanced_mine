...
TL;DR: 本文研究了多模态视觉语言模型中的成员推理攻击，提出了神经科学启发的拓扑正则化框架来提高模型对隐私攻击的抵抗能力，在保持模型性能的同时显著降低了攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 随着多模态模型的广泛部署，隐私泄露风险日益增加。现有研究主要关注单模态系统的隐私攻击，而多模态模型对隐私攻击的脆弱性尚未充分探索，特别是神经科学启发的模型在隐私保护方面的潜力。

Method: 提出了系统性的神经科学启发的拓扑正则化框架，在三种视觉语言模型（BLIP、PaliGemma 2、ViT-GPT2）上应用该框架，并在三个基准数据集（COCO、CC3M、NoCaps）上进行实验。

Result: 实验结果显示，在BLIP模型上使用COCO数据集时，神经增强的VLM使MIA攻击成功率平均ROC-AUC下降了24%，同时保持了相似的模型效用（MPNet和ROUGE-2指标）。

Conclusion: 神经增强的视觉语言模型在保持模型性能的同时，对隐私攻击具有更强的抵抗能力，这一发现在多个模型和数据集上得到验证。
...
Result: Inferix专门设计用于世界模拟，与高并发系统（如vLLM或SGLang）和经典视频扩散模型（如xDiTs）区分开来。支持交互式视频流和性能分析，实现实时交互和真实模拟。

Conclusion: Inferix作为专门的世界模型推理引擎，通过优化的半自回归解码和交互功能，为世界模型探索提供了强大工具，希望社区共同推进其发展。
...
TL;DR: 本文提出了一种基于深度强化学习的自适应算法LTED-Ada，用于在资源受限设备上优化视频对象识别，通过智能选择本地跟踪或边缘检测来平衡帧率、识别精度和延迟要求。


<details>
  <summary>Details</summary>
Motivation: 资源受限设备（如交通摄像头）上的快速准确视频对象识别面临挑战，混合边缘计算方案虽然前景广阔，但引入了何时执行边缘检测与本地跟踪的决策难题。

Method: 提出LTED-Ada算法，基于深度强化学习自适应选择本地跟踪或边缘检测；在多设备场景中，使用联邦学习进行协作策略训练；通过硬件在环实验验证算法性能。

Result: 实验结果表明LTED-Ada在多个树莓派4B设备和PC边缘服务器上表现出优越性能，能够有效适应未见过的帧率和性能要求。

Conclusion: LTED-Ada算法通过智能决策机制和联邦学习协作训练，为资源受限设备上的视频对象识别提供了高效解决方案，在单设备和多设备场景下均表现出色。
...
TL;DR: DeeAD是一个无需训练的动作引导早期退出框架，通过评估中间轨迹的物理可行性来加速视觉语言动作模型的推理，在保持规划质量的同时实现高达28%的层稀疏度和29%的延迟降低。
...
Conclusion: DeeAD可以无缝集成到现有VLA模型中，无需重新训练，有效解决了VLA模型推理延迟问题。
...
TL;DR: DinoLizer是一个基于DINOv2的模型，用于定位生成式修复图像中的篡改区域。该方法在DINOv2模型上添加线性分类头，通过滑动窗口策略处理大图像，在多个修复数据集上超越现有最先进的局部篡改检测器。


<details>
  <summary>Details</summary>
Motivation: 随着生成式修复技术的发展，需要有效的方法来检测和定位图像中的篡改区域，以应对潜在的恶意使用。

Method: 在预训练的DINOv2模型上添加线性分类头，利用Vision Transformer的补丁嵌入在14×14补丁分辨率上预测篡改。采用滑动窗口策略处理大图像，并对热图进行后处理以生成二进制篡改掩码。

Result: DinoLizer在多个生成式修复数据集上超越现有最先进的局部篡改检测器，平均IoU比次优模型高12%，对常见后处理操作（如调整大小、噪声添加和JPEG压缩）保持鲁棒性。

Conclusion: Vision Transformer在篡改定位任务中表现出强大的表征能力，DinoLizer通过DINOv2模型实现了优越的篡改检测性能，代码将在论文接受后公开。
...
TL;DR: CANVAS是一个评估视觉语言模型在工具界面设计能力的基准，包含598个基于工具的设计任务，涵盖设计复制和设计修改两种任务类型，用于测试模型通过工具调用操作设计软件的能力。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏评估视觉语言模型基于工具的设计性能的基准，而理解这种能力对于模型与设计师在常规软件中协作的潜力至关重要。

Method: 构建包含598个工具设计任务的CANVAS基准，任务来自3.3K个移动UI设计，涵盖30个功能类别，包含设计复制和设计修改两种任务类型，模型通过上下文工具调用来逐步更新设计。

Result: 领先模型展现出更具策略性的工具调用，提高了设计质量，同时识别了模型常见的错误模式。

Conclusion: CANVAS基准填补了工具界面设计评估的空白，为未来增强基于工具的设计能力提供了指导方向。
...
TL;DR: 提出文本引导语义图像编码器(TIE)，通过文本查询条件化生成图像表示，在多个图像到文本基准测试中显著提升性能并提高推理效率。


<details>
  <summary>Details</summary>
Motivation: 传统视觉语言模型中的图像编码器通常独立预训练，无法根据下游任务或文本查询进行自适应处理，限制了模型性能。

Method: 开发文本引导语义图像编码器(TIE)，生成基于输入文本查询的条件化图像表示，减少图像分块数量，提高推理效率。

Result: 在1B和3B规模下，TIE模型在9个图像到文本基准测试中平均提升1.5和1.3个点，DocVQA和InfoVQA任务上最高提升6个点，同时仅使用一半图像分块。
...
TL;DR: Δ-NeRF是一种模块化残差框架，用于神经辐射场（NeRF）的增量优化，解决了现有NeRF框架需要完整重训练的问题，特别适用于卫星地形分析等数据连续到达的场景。


<details>
  <summary>Details</summary>
Motivation: 现有NeRF框架在引入新视角时需要完整重训练，限制了在数据顺序到达场景中的应用。特别是在卫星地形分析中，区域会随时间重复观测，增量优化能力不足且存在灾难性遗忘问题。

Method: 提出Δ-NeRF框架，包括：残差控制器向冻结的基础NeRF注入逐层修正；不确定性感知门控机制自适应结合基础和优化预测；视图选择策略减少47%训练数据；使用知识蒸馏将增强模型压缩至原大小20%。

Result: 在卫星图像上的实验表明，Δ-NeRF性能与联合训练相当，训练时间减少30-42%。相比朴素微调PSNR提升43.5%，在某些指标上甚至超过联合训练。

Conclusion: Δ-NeRF为NeRF的增量优化提供了有效解决方案，在保持性能的同时显著减少训练时间和数据需求，适用于连续数据到达的应用场景。
...
TL;DR: 提出Split-then-Merge(StM)框架，通过将未标注视频分解为前景和背景层并进行自组合，解决生成视频合成的控制问题和数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 解决生成视频合成中的控制问题和数据稀缺问题，传统方法依赖标注数据集或手工规则，而StM利用未标注视频学习动态主体与多样化场景的交互。

Method: 将未标注视频分解为动态前景和背景层，通过自组合学习交互；采用变换感知训练管道，包括多层融合和增强实现可感知组合，以及保持前景保真度的身份保持损失。

Result: 实验表明StM在定量基准测试和人类/VLLM定性评估中均优于现有最先进方法。

Conclusion: StM框架通过分解和自组合未标注视频，有效学习复杂组合动态，实现更真实的视频生成，解决了数据稀缺和控制问题。
...
TL;DR: Sphinx是一个针对视觉感知和推理核心认知原语的合成环境，通过程序化生成包含图案、图块、图表、图标和几何原语的谜题，并提供可验证的真实解，支持精确评估和大规模数据集构建。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够精确评估和提升多模态推理能力的合成环境，填补现有视觉推理基准在可验证性和规模上的不足。

Method: 使用程序化生成方法创建包含25种任务类型的谜题，涵盖对称检测、几何变换、空间推理、图表解释和序列预测等领域，每个谜题都配有可验证的真实解。

Result: 评估显示最先进的GPT-5模型在Sphinx基准上仅达到51.1%的准确率，远低于人类表现。使用可验证奖励的强化学习（RLVR）显著提高了模型在这些任务上的准确性，并在外部视觉推理基准上取得了改进。

Conclusion: Sphinx为多模态推理提供了有效的评估框架，可验证奖励的强化学习是提升视觉推理能力的有效方法，有望推动多模态推理的进一步发展。
...
TL;DR: RefTr是一个用于血管树中心线生成的3D图像到图模型，通过汇合轨迹的循环精炼来生成具有正确树拓扑结构的中心线。
...
Method: 采用Producer-Refiner架构，基于Transformer解码器，Producer提出初始汇合轨迹，Refiner循环精炼这些轨迹形成最终中心线图，同时引入高效的非极大值抑制算法合并重复分支。

Result: 在多个公共中心线数据集上，RefTr实现了优于先前SOTA的召回率和相当的精度，同时提供更快的推理速度和显著减少的参数数量。

Conclusion: RefTr展示了作为3D医学成像中血管树分析的新最先进框架的潜力，具有更好的性能和效率。
...
Result: 创建了首个高分辨率立体DSLR数据集，支持单目和立体深度估计、浅景深渲染、去模糊、3D场景重建和新视角合成的受控分析，并包含专门的校准图像集。

Conclusion: 该数据集为真实世界光学泛化的可重复研究提供了支持，并展示了当前最先进的单目、立体深度和景深方法面临的挑战。
...
TL;DR: 本文提出了首个大规模无监督视觉记忆性数据集，包含82,000多个视频和回忆描述数据，利用Reddit等平台的舌尖现象查询，在回忆生成和ToT检索任务上表现出色。


<details>
  <summary>Details</summary>
Motivation: 视觉内容记忆性研究面临人工标注成本高昂的问题，现有数据集主要收集聚合记忆分数，无法捕捉自然开放式回忆中的细微记忆信号。

Method: 利用在线平台的舌尖现象检索查询构建无监督数据集，采用对比训练策略开发多模态ToT检索模型。

Result: 基于该数据集微调的大规模视觉语言模型在生成视觉内容记忆性描述方面优于GPT-4o等最先进模型，并创建了首个能够执行多模态ToT检索的模型。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法顺序估计雾参数容易导致误差传播，且无法有效处理真实世界中全局不均匀的雾。需要一种更鲁棒的参数估计方法来提升雾天环境下的视觉SLAM和里程计系统性能。

Method: 通过求解新型优化问题同时估计所有雾参数，假设雾仅局部均匀，从而有效处理全局不均匀的真实雾。该方法可作为现有视觉SLAM系统的附加模块使用。

Result: 在合成雾数据和真实雾序列SDIRF数据集上的实验表明，该方法在合成数据上产生最准确的估计，在真实雾中适应性更好。创建了包含3.4万帧高质量立体雾天道路场景的SDIRF数据集。

Conclusion: 提出的方法在雾参数估计方面优于现有方法，能更好地适应真实雾环境。公开了代码和SDIRF数据集，旨在推动雾天视觉感知研究。
...
Conclusion: V^2-SAM通过几何感知和外观引导的提示生成器，有效解决了跨视角物体对应问题，为相关应用提供了强大解决方案。
...
TL;DR: 提出了Null-Text Test-Time Alignment (Null-TTA)方法，通过优化无分类器引导中的无条件嵌入来对齐扩散模型，防止奖励黑客攻击，并在保持跨奖励泛化能力的同时实现最先进的测试时对齐性能。


<details>
  <summary>Details</summary>
Motivation: 现有测试时对齐方法往往存在欠优化或过优化（奖励黑客）问题，需要一种能够有效对齐目标奖励函数同时防止奖励黑客的新方法。

Method: 通过优化无分类器引导中的无条件嵌入而不是操纵潜在变量或噪声变量，在语义一致的流形上进行对齐，防止利用非语义噪声模式来改进奖励。

Result: Null-TTA在目标测试时对齐方面达到最先进性能，同时保持强大的跨奖励泛化能力，建立了语义空间优化作为TTA的有效且原则性新范式。

Conclusion: 语义空间优化是测试时对齐的有效新范式，Null-TTA通过优化无条件嵌入直接引导模型的生成分布，在防止奖励黑客的同时实现高质量对齐。
...
Result: 随着输入剂量增加，恢复图像的结构相似性指数从0.639提升至0.993，峰值信噪比从21.6dB提升至41.4dB。在10%输入剂量下，使用DL恢复的T1ce进行分割，Dice系数从0.673提升至0.734。

Conclusion: 深度学习模型显著改善了小脑桥脑角区低剂量MRI的图像质量，使得在10%-30%标准剂量下仍能进行病变检测和诊断特征分析。
...
TL;DR: 提出一种平滑正则化技术，通过在视频识别模型中引入时间归纳偏置，对轻量级架构特别有益。该方法通过将连续帧中间层嵌入的变化建模为高斯随机游走，鼓励平滑性，从而提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 视频具有自然的时间连贯性，但轻量级模型难以有效捕捉复杂的时间动态。通过强制中间层表示的平滑变化，可以更好地对齐视频的时序特性。

Method: 使用高斯随机游走模型对连续帧中间层嵌入的变化进行建模，惩罚表示上的突然变化，促进低加速度的解决方案。

Result: 在Kinetics-600数据集上，轻量级模型准确率提升3.8%至6.4%。MoViNets模型族在各自FLOP约束下将当前最优结果提升3.8%至6.1%，MobileNetV3和MoViNets-Stream在类似内存占用下比之前最优模型提升4.9%至6.4%。
...
TL;DR: 本文提出了一种视觉领域的开放词汇组合解释框架，通过开放词汇语义分割生成掩码来计算组合解释，解决了传统方法依赖人工标注数据集的限制。


<details>
  <summary>Details</summary>
Motivation: 传统组合解释方法依赖人工标注数据集，限制了其应用范围和灵活性。本文旨在开发一个框架，允许用户针对任意概念和数据集探测神经元，提高解释的通用性和适用性。

Method: 提出三步框架：1) 指定任意概念；2) 使用开放词汇模型生成语义分割掩码；3) 从这些掩码推导组合解释。

Result: 与先前方法相比，该框架在定量指标和人类可解释性方面表现良好，展示了从人工标注数据转向模型标注数据时解释的差异，并提供了在任务和感兴趣属性方面的灵活性。

Conclusion: 该框架扩展了组合解释的适用范围，使其能够处理任意概念和数据集，为神经元信息编码的理解提供了更灵活和通用的工具。
...
<details>
  <summary>Details</summary>
Motivation: 解决木材横截面数据稀缺问题，提供包含多个树干高度样本的数据集，支持年轮体积建模和自动检测算法的开发。

Method: 创建UruDendro4数据集，包含102个火炬松横截面图像样本，每个样本手动标注年轮。使用最先进方法（如DeepCS-TRD）建立自动年轮检测性能基准，并进行消融实验验证参数配置。

Result: DeepCS-TRD方法在数据集上表现最佳，平均精度0.838，平均召回率0.782，适应性兰德误差0.084。包含该数据集的训练提高了模型在年轮检测任务中的泛化能力。

Conclusion: UruDendro4数据集填补了木材横截面数据空白，支持年轮体积建模，为自动年轮检测提供了可靠基准，并证明能提升模型泛化性能。
...
TL;DR: 本文提出了表达性图像合成任务，开发了StickerNet框架，通过两阶段方法预测贴纸放置参数，使用真实在线编辑平台数据训练，在用户研究中表现优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 传统图像合成研究专注于视觉真实性和语义合理性，但在现代内容创作中，用户更倾向于创建艺术性、趣味性或社交参与性的内容，而非保持真实感。

Method: 提出StickerNet两阶段框架：首先确定合成类型，然后预测不透明度、遮罩、位置和缩放等放置参数；使用来自匿名在线视觉创作平台的180万次编辑行为构建数据集。

Result: 用户研究和定量评估表明，StickerNet优于常见基线方法，并与人工作放置行为高度匹配，证明了从真实世界编辑模式中学习的有效性。
...
TL;DR: TrafficLens是一个针对多摄像头交通路口的定制算法，通过顺序处理摄像头重叠覆盖区域，使用不同token限制的VLM迭代生成详细文本描述，同时通过对象级相似性检测器智能绕过冗余VLM调用，将视频到文本转换时间减少高达4倍。
...
Conclusion: TrafficLens有效解决了多摄像头交通视频分析中的处理延迟问题，实现了快速准确的视频内容分析。
...
TL;DR: 本文提出了一种结合视觉变换器和同态加密的隐私保护联邦学习框架，用于安全的多机构病理学分类，通过加密CLS令牌实现30倍通信减少并防止模型反演攻击。


<details>
  <summary>Details</summary>
Motivation: 医疗机构的协作机器学习需要保护患者隐私，传统联邦学习的模型梯度仍易受重建攻击，可能暴露敏感医疗信息。

Method: 使用视觉变换器的CLS令牌作为紧凑特征表示，采用CKKS同态加密在传输前加密这些令牌，实现安全聚合。

Result: 梯度易受模型反演攻击（PSNR: 52.26 dB, SSIM: 0.999, NMI: 0.741），而CLS保护的HE方法防止此类攻击，每轮聚合仅需326KB加密数据传输，在未加密域达到96.12%准确率，加密域达到90.02%。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的风格迁移方法依赖计算密集的反转过程，导致效率低下且在反转不准确时产生视觉失真，需要无反转的高效解决方案。

Method: 使用双整流流并行预测内容和风格轨迹，通过动态中点插值融合两种路径的速度场，结合注意力注入机制指导风格整合。

Result: 实验证明该方法在多种风格和内容上具有良好泛化能力，实现了高效且视觉保真度高的风格迁移。

Conclusion: 提出的无反转框架通过双整流流和动态融合机制，有效解决了传统方法效率低下和视觉失真的问题，为风格迁移提供了高效且鲁棒的解决方案。
...
TL;DR: 本文提出了一种无需测试时参考图像的伪装目标检测框架，通过将参考图像蒸馏到类别原型内存中，在推理时通过查询条件原型混合生成参考向量，从而消除对参考图像的依赖。


<details>
  <summary>Details</summary>
Motivation: 现有Ref-COD系统采用双分支设计，在测试时需要参考图像，这限制了部署性、增加了延迟和数据收集负担。本文旨在开发一个无需测试时参考图像的更高效框架。

Method: 1）在训练期间将参考图像蒸馏到类别原型内存中；2）通过查询条件原型混合在推理时合成参考向量；3）提出双向注意力对齐模块来弥合参考统计量与伪装查询特征之间的表示差距。

Result: 在R2C7K基准测试上的广泛实验表明，该方法与现有最先进方法相比具有竞争性或更优的性能。

Conclusion: 该方法为Ref-COD提供了一条简单高效的路径，无需强制性的参考图像，同时保持了竞争性的检测性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有的多模态安全防护主要只评估输入问题和最终答案，忽略了中间推理过程，导致偏见推断或违反政策的视觉上下文使用等未检测到的危害。

Method: 提出GuardTrace-VL安全审计器，构建GuardTrace数据集，采用三阶段渐进训练方案结合数据精炼过程，使模型能够根据不同风险级别学习细致和上下文相关的安全偏好。

Result: 在涵盖域内和域外场景的测试集上，GuardTrace-VL模型在不安全推理检测任务上达到93.1%的F1分数，比之前最强的多模态安全防御方法提高了13.5%。

Conclusion: GuardTrace-VL能够有效检测推理过程中的不安全内容，填补了现有多模态安全防护在中间推理过程监控方面的空白。
...
TL;DR: 该论文提出了一种基于扩散模型的图像分层分解方法，通过轻量级微调将修复模型适配用于图像分层，并引入多模态上下文融合模块来保留细节，在合成数据集上训练，在目标移除和遮挡恢复方面表现优异。


<details>
  <summary>Details</summary>
Motivation: 图像可以视为前景对象在背景上的分层组合，这种分层表示能够独立编辑元素，为内容创作提供更大灵活性。尽管生成模型取得进展，但由于方法和数据有限，将单张图像分解为分层仍然具有挑战性。

Method: 观察到分层分解与修复任务之间的强关联，提出通过轻量级微调将基于扩散的修复模型适配用于分层分解。为在潜在空间中保留细节，引入了具有线性注意力复杂度的多模态上下文融合模块。模型仅在开源资产构建的合成数据集上训练。

Result: 该方法在目标移除和遮挡恢复方面实现了优越性能，为下游编辑和创意应用开辟了新的可能性。

Conclusion: 通过将修复模型适配用于分层分解任务，结合多模态上下文融合模块，成功实现了高质量的图像分层分解，在多种编辑应用中展现出良好效果。
...
TL;DR: MERGE是一个多模态实体感知检索增强生成框架，通过构建实体中心的多模态知识库、多阶段假设-标题策略和动态检索机制，解决了新闻图像描述中的信息覆盖不完整、跨模态对齐弱和视觉实体定位不佳等问题。


<details>
  <summary>Details</summary>
Motivation: 现有新闻图像描述方法面临三个关键挑战：信息覆盖不完整、跨模态对齐弱、视觉实体定位不佳，需要新的框架来解决这些问题。

Method: 提出MERGE框架，包括：1）构建实体中心的多模态知识库整合文本、视觉和结构化知识；2）通过多阶段假设-标题策略改善跨模态对齐；3）基于图像内容的动态检索增强视觉实体匹配。
...
Conclusion: MERGE框架在新闻图像描述任务中表现出色，不仅显著提升了描述质量和实体识别准确性，还展现了强大的鲁棒性和领域适应性。
...
TL;DR: MetaRank是一个基于元学习的框架，用于自动选择模型迁移性评估(MTE)指标，通过将数据集和指标的文本描述嵌入共享语义空间，训练元预测器来学习数据集特征与指标机制之间的关系。
...
Method: 将指标选择建模为学习排序问题，使用预训练语言模型编码数据集和MTE指标的文本描述，在共享语义空间中嵌入它们，离线训练元预测器学习数据集特征与指标机制的关系，采用列表式优化目标优先正确排序表现最佳的指标。
...
<details>
  <summary>Details</summary>
Motivation: 现有可信多视图分类方法主要依赖全局密集邻居关系建模视图内依赖，导致计算成本高且无法直接确保视图间关系一致性，同时通过手动分配权重聚合不同视图证据，缺乏对学习到的多视图邻居结构在类空间一致性的保证。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法要么依赖模糊的文本提示难以精确控制相机参数，要么需要为不同参数训练独立模块导致扩展性差和多参数组合困难。
...
Conclusion: CameraMaster通过解耦相机指令和参数嵌入的统一框架，成功解决了图像修饰中精确相机参数控制的挑战。
...
TL;DR: 提出了CaptionQA基准，通过下游任务效用评估图像描述质量，覆盖4个领域，包含33,027个多选题，揭示现有模型在描述效用方面存在显著差距。
...
Method: 构建CaptionQA基准，包含4个领域（自然、文档、电子商务、具身AI）的25个顶级类别和69个子类别，创建33,027个密集标注的多选题，通过LLM仅使用描述回答问题来评估描述效用。

Result: 评估显示图像与其描述效用之间存在显著差距，在传统图像QA基准上表现相近的模型，在描述效用上下降高达32%。

Conclusion: CaptionQA提供了一个可扩展的基准，能够全面评估图像描述在下游任务中的实际效用，揭示了当前模型在保持图像级效用方面的不足。
...
TL;DR: LungNoduleAgent是一个创新的协作多智能体系统，专门用于分析肺部CT扫描，通过三个主要模块提高结节描述和恶性分级精度，在多个数据集上超越主流视觉语言模型和专家模型。


<details>
  <summary>Details</summary>
Motivation: 当前基于多模态大语言模型的肺部CT扫描分析方法在准确描述结节形态和融入医学专业知识方面存在挑战，影响了临床应用的可靠性和有效性。协作多智能体系统在医学应用中平衡通用性和精确性方面具有潜力，但在病理学领域尚未充分探索。

Method: LungNoduleAgent将诊断过程分解为顺序组件，包含三个主要模块：结节定位器协调临床检测模型准确识别结节；放射科医生整合局部图像描述技术生成全面CT报告；医生智能体系统利用图像和CT报告，在病理知识库和多智能体系统框架支持下进行恶性推理。
...
TL;DR: 提出了一种新颖的图像去模糊框架，通过将强大的生成先验与显式密集物理约束相结合，解决了空间变化去模糊问题。该方法使用高维压缩核建模密集退化场，并通过ControlNet架构引导扩散采样过程。


<details>
  <summary>Details</summary>
Motivation: 当前基于学习的去模糊方法存在两极化问题：基于模型的深度展开方法虽然强制物理约束但会产生过度平滑和伪影，而生成模型虽然感知质量好但会因物理约束弱而产生幻觉细节。需要一种能同时保证物理准确性和感知真实性的方法。

Method: 将退化场建模为高维压缩核的密集连续体，捕捉运动和退化模式的微小变化。利用这个丰富的描述符场来条件化ControlNet架构，强有力地引导扩散采样过程。

Result: 在具有挑战性的严重模糊场景中，该方法在物理准确性和感知真实性之间实现了有效平衡，超越了最先进的基于模型方法和生成基线。

Conclusion: 该方法成功调和了基于模型的物理约束方法和生成模型的感知质量优势，为空间变化图像去模糊提供了一种有效的解决方案。
...
Conclusion: MUSE为情感合成建立了新范式，统一了生成和编辑任务，无需额外训练扩散模型或专用数据集。
...
TL;DR: 提出T-NIG模型，通过估计正态逆伽马分布中的时间参数来生成长期阿尔茨海默病预测图像，解决了不规则时间间隔数据中疾病特征保持的难题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理序列数据中不规则时间间隔时，难以在长期阿尔茨海默病预测中维持疾病相关特征，而时间相关的分布特征可以反映疾病特征的变化。
...
Conclusion: T-NIG模型能够有效处理不规则时间分布的脑成像数据，在维持疾病特征的同时实现准确的长期疾病预测。
...
TL;DR: 本文提出了一种新颖的虹膜认证匹配流程，通过3D-CNN学习虹膜特征的丰富时空表示，采用课程学习方式训练模型，在旋转、缩放、反射和模糊等挑战下实现鲁棒的虹膜识别。


<details>
  <summary>Details</summary>
Motivation: 现有虹膜认证算法在旋转、缩放、镜面反射和散焦模糊等变化下的鲁棒性仍然面临挑战，且大多数方法依赖简单的点对点比较，未能有效利用虹膜模式的时空结构。

Method: 将虹膜图像沿一个维度分割成子图像序列，作为3D-CNN的输入以捕捉空间和时空线索；采用课程学习方式训练模型，结合三元组损失和ArcFace损失进行端到端训练。

Result: 该方法能够在深度度量域中嵌入时间依赖性，提高特征的可区分性，在面临旋转、缩放、反射和模糊等挑战时仍能产生高度可区分的嵌入。

Conclusion: 该框架为虹膜认证提供了一个鲁棒且可泛化的解决方案，能够直接学习时空特征表示，提升虹膜识别系统的性能。
...
<details>
  <summary>Details</summary>
Motivation: 雷达传感器在各种恶劣天气、光照和远距离条件下提供可靠的感知能力，但现有雷达方法存在碎片化问题，每个下游任务使用不同的架构和训练目标，阻碍了跨任务迁移。

Method: 提出两个关键贡献：(1)结构化标题框架，在原生雷达坐标中编码车辆分布；(2)哈希感知对比学习目标，量化连续场景相似性而非二元匹配，实现细粒度空间推理。利用CARLA模拟器生成大规模、良好注释的雷达数据集。

Result: 开发了RadarFM模型，能够学习统一的场景级表示，并提出定位感知指标来评估空间准确性。

Conclusion: RadarFM通过结构化空间语言监督和对比学习，为雷达感知提供了一个统一的基础模型框架，克服了现有方法的碎片化问题。
...
TL;DR: 本文提出EM-KD方法，通过知识蒸馏增强高效多模态大语言模型。该方法解决了师生模型间视觉令牌不平衡问题，采用曼哈顿距离和匈牙利匹配算法进行空间对齐，并引入两种蒸馏策略：视觉语言亲和度蒸馏和视觉语义蒸馏。
...
Method: 1) 使用曼哈顿距离和匈牙利算法对齐师生视觉令牌；2) 视觉语言亲和度蒸馏：最小化师生亲和度矩阵的平滑L1距离；3) 视觉语义蒸馏：使用反向KL散度度量词汇空间中对齐视觉令牌的离散概率分布。

Result: 在多样化基准测试中，EM-KD训练模型在准确性和效率上均显著优于现有高效MLLMs，相比配备相同视觉令牌匹配策略的先前蒸馏方法也表现更优。

Conclusion: EM-KD通过解决视觉令牌不平衡问题并引入有效的蒸馏策略，成功提升了高效多模态大语言模型的性能，验证了其有效性。
...
TL;DR: FaithFusion是一个3DGS-扩散融合框架，通过像素级期望信息增益（EIG）驱动，解决了可控驾驶场景重建和3D场景生成中几何保真度和视觉外观合成的挑战。


<details>
  <summary>Details</summary>
Motivation: 在可控驾驶场景重建和3D场景生成中，需要在大的视点变化下保持几何保真度同时合成视觉上合理的表观。然而，基于几何的3DGS和基于表观的扩散模型的有效融合面临固有挑战，缺乏像素级、3D一致的编辑标准往往导致过度修复和几何漂移。

Method: 引入FaithFusion框架，使用像素级期望信息增益（EIG）作为统一策略进行连贯的时空合成：EIG作为空间先验指导扩散模型细化高不确定性区域，同时其像素级权重将编辑结果反蒸馏回3DGS。该系统是即插即用的，无需额外的先验条件和结构修改。

Result: 在Waymo数据集上的广泛实验表明，该方法在NTA-IoU、NTL-IoU和FID指标上达到了最先进的性能，即使在6米车道偏移时仍保持107.47的FID分数。

Conclusion: FaithFusion通过EIG驱动的融合策略成功解决了3DGS和扩散模型融合中的几何保真度和视觉合成问题，实现了高性能的可控驾驶场景重建和生成。
...
<details>
  <summary>Details</summary>
Motivation: 大规模视觉生成模型在下游任务中存在显著的参数冗余问题，需要一种专门针对生成模型的剪枝方法，既能保持输出多样性又能减少冗余参数。

Method: 提出熵引导剪枝策略，使用数据依赖的条件熵偏差(CED)评估模块重要性；设计零样本自适应剪枝框架，自动确定剪枝时机和程度，避免一次性剪枝的问题。

Result: 在DiT和SiT模型上的实验表明，EntPruner实现了最高2.22倍的推理加速，同时在ImageNet和三个下游数据集上保持了竞争力的生成质量。

Conclusion: EntPruner为生成模型提供了一种有效的剪枝方法，能够在保持生成质量的同时显著提升推理效率，解决了生成模型在下游任务中的参数冗余问题。
...
TL;DR: CtrlVDiff是一个统一的扩散框架，同时解决视频理解和可控视频生成的双重挑战，通过融合多种图形模态（深度、法线、分割、边缘、材质属性等）实现精确控制和时序一致性。


<details>
  <summary>Details</summary>
Motivation: 传统仅使用几何线索（如深度、边缘）的方法不足以约束外观、材质和光照，限制了物理上有意义的编辑（如重新照明或材质替换），且容易导致时序漂移。需要引入更多基于图形的模态来提供互补约束。

Method: 提出CtrlVDiff模型，采用混合模态控制策略（HMCS）来路由和融合深度、法线、分割、边缘和图形内在属性（反照率、粗糙度、金属度）等特征，并构建了MMVideo混合真实与合成数据集进行训练。

Result: 在理解和生成基准测试中，CtrlVDiff实现了卓越的可控性和保真度，支持分层编辑（重新照明、材质调整、对象插入），在部分模态不可用时仍保持鲁棒性，超越了现有最先进基线。

Conclusion: 通过融合多种图形模态，CtrlVDiff在统一的扩散框架中成功解决了视频理解和可控生成的挑战，实现了精确的时序一致控制和物理上有意义的视频编辑。
...
### [44] [AnchorOPT: Towards Optimizing Dynamic Anchors for Adaptive Prompt Learning](https://arxiv.org/abs/2511.21188)
*Zheng Li,Yibing Song,Xin Zhang,Lei Luo,Xiang Li,Jian Yang*

Main category: cs.CV

TL;DR: AnchorOPT是一个动态锚点提示学习框架，通过动态学习锚点值和优化锚点与软令牌的位置关系，解决了现有CLIP提示学习方法中锚点静态化的问题，在多个数据集上取得了优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于CLIP的提示学习方法使用静态文本令牌作为锚点来指导可学习的软令牌，但这些锚点在值和位置上都固定不变，缺乏跨任务和阶段自适应的灵活性。

Method: AnchorOPT在两个方面引入动态性：(1)锚点值从任务特定数据中动态学习，而非手工设计的显式文本令牌；(2)锚点与软令牌的位置关系通过可学习的位置矩阵自适应优化。训练分为两个阶段：先学习锚点令牌，然后冻结锚点并优化软令牌和位置矩阵。

Result: 广泛实验表明，仅使用简单的可学习锚点和位置矩阵，就能达到或超过某些包含额外可学习模块或正则化技术的方法的性能。作为即插即用模块，AnchorOPT能无缝集成到现有框架中，在不同数据集上带来一致的性能提升。

Conclusion: AnchorOPT通过动态锚点机制有效提升了CLIP模型的泛化能力，证明了动态锚点在值和位置两个维度上的优化对提示学习的重要性。
...
TL;DR: 本文研究了视觉语言模型中无关信息（干扰项）对测试时扩展的影响，发现视觉干扰项与文本干扰项存在根本差异：虽然都存在逆缩放效应，但添加视觉干扰项会降低准确性而不增加推理长度。


<details>
  <summary>Details</summary>
Motivation: 先前研究表明语言模型中的文本干扰项会导致推理变长但效果变差，本文旨在探究多模态设置中是否会出现类似现象。

Method: 引入了Idis数据集，系统地在语义、数值和空间维度上变化干扰项，并通过分析推理轨迹中的属性计数来研究干扰项、推理长度和准确性的相互作用。

Result: 视觉干扰项与文本干扰项存在根本差异：添加视觉干扰项会降低准确性而不增加推理长度；这些趋势也适用于已建立的视觉偏见基准测试如Waterbirds。

Conclusion: 提出了简单的提示策略来减轻推理模型中的偏见驱动预测，为理解多模态模型中的干扰效应提供了新见解。
...
Conclusion: Ent-Prog框架有效解决了扩散模型训练效率问题，为高分辨率多帧视频生成提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的安全评估方法主要针对静态图像和文本生成，无法有效捕捉视频生成中的复杂时间动态特性，因此需要专门针对T2V模型的时间感知安全评估框架。

Method: TEAR采用时间感知测试生成器，通过两阶段优化方法（初始生成器训练和时间感知在线偏好学习）来生成看似无害但能利用时间动态引发政策违规视频的提示词，并使用精炼模型循环提升提示词的隐蔽性和对抗效果。
...
Method: 提出渐进式视觉压缩方法，包含精细化补丁嵌入和窗口化令牌压缩两个模块，可集成到标准视觉Transformer中实现高效原生分辨率编码。
...
Method: GridAR采用网格分区渐进生成方案，为同一位置生成多个部分候选，早期修剪不可行候选，将可行候选固定为锚点指导后续解码；同时提出布局指定提示重构策略，通过检查部分视图推断可行布局。

Result: 在N=4时，GridAR比Best-of-N（N=8）在T2I-CompBench++上性能提升14.4%，成本降低25.6%；在图像编辑任务上，PIE-Bench上语义保持度比更大N的基线提升13.9%。

Conclusion: GridAR框架有效解决了视觉自回归模型测试时扩展的挑战，在有限计算资源下实现了更高质量的文本到图像生成和编辑效果。
...
TL;DR: NDTokenizer3D是一个通用的3D视觉语言模型，通过多尺度NDT表示和三阶段场景标记化流程，统一处理多种3D场景理解任务，并在3D指代分割、视觉问答和密集描述任务上取得显著改进。
...
Result: 在3D指代分割、3D视觉问答和3D密集描述任务上实现了显著改进。
...
TL;DR: 提出了UPA-RFAS框架，针对VLA模型开发通用、可迁移的对抗性补丁攻击，能在未知架构、微调变体和仿真到现实场景下实现跨模型攻击。


<details>
  <summary>Details</summary>
Motivation: 现有对抗性补丁攻击大多过拟合单一模型，在黑盒设置下失效，缺乏对VLA模型的通用可迁移攻击研究。

Method: 结合特征空间目标函数、鲁棒增强的两阶段min-max优化、以及VLA特定的注意力劫持和语义错配损失，在共享特征空间中学习单一物理补丁。

Result: 实验表明UPA-RFAS能跨模型、任务和视角一致迁移，在各种VLA模型、操作套件和物理执行中有效。

Conclusion: UPA-RFAS暴露了VLA驱动机器人的实用补丁攻击面，为未来防御建立了强基线。
...
TL;DR: 本文提出了Action-Region Tracking (ART)框架，通过查询-响应机制发现和跟踪局部细节动态，以区分细粒度动作。使用文本约束的语义查询捕捉动作相关区域响应，构建动作轨迹，并通过多级轨迹对比约束优化识别效果。


<details>
  <summary>Details</summary>
Motivation: 现有细粒度动作识别方法往往只能捕捉粗粒度运动模式，难以识别随时间演变的局部区域中的细微差异。需要一种能够发现和跟踪局部细节动态的解决方案。

Method: 提出ART框架，包含区域特定语义激活模块，使用文本约束的语义查询捕捉动作相关区域响应；构建动作轨迹连接跨帧的相关响应；设计多级轨迹对比约束优化空间和时间层面的区分能力；采用任务特定微调机制优化文本语义表示。

Result: 在广泛使用的动作识别基准测试中，该方法优于之前的最先进基线方法。

Conclusion: ART框架通过查询-响应机制有效发现和跟踪局部细节动态，结合文本约束语义和多级对比约束，显著提升了细粒度动作识别的性能。
...
TL;DR: 本文对三种生成建模方法（DDPM、CFM、MeanFlow）进行了比较研究，发现CFM在50步采样时FID为24.15，显著优于DDPM的402.98；MeanFlow通过单步采样实现FID 29.15，推理时间减少50倍。还将CFM扩展到图像修复，实现了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 比较三种生成建模范式的性能差异，探索更高效的生成方法，并将条件流匹配扩展到图像修复任务。

Method: 使用统一的TinyUNet架构（<150万参数）在CIFAR-10上实现DDPM、CFM和MeanFlow三种方法，并将CFM扩展到图像修复，采用四种掩码类型进行掩码引导采样。

Result: CFM在50步采样时FID为24.15，DDPM为402.98；MeanFlow单步采样FID为29.15。图像修复模型在中心掩码上PSNR从4.95提升至8.57 dB（+73%），SSIM从0.289提升至0.418（+45%）。

Conclusion: CFM在生成质量上显著优于DDPM，MeanFlow实现了高效的直接生成，CFM在图像修复任务中表现出色，修复感知训练可大幅提升性能。
...
TL;DR: FIELDS是一个3D人脸重建方法，通过结合2D图像一致性监督、3D表情参数直接监督和辅助情感识别分支，解决了现有方法在捕捉细微情感细节方面的不足。
...
Result: FIELDS能够从单张图像生成富有情感的高保真3D人脸模型，显著提升了野外面部表情识别性能，同时保持自然度。

Conclusion: 双监督策略成功弥合了2D/3D领域差距并减轻了表情强度偏差，实现了保留细微情感线索的高质量3D重建。
...
TL;DR: ConFu是一个多模态对比学习框架，通过联合嵌入单个模态及其融合组合到统一表示空间，同时保持成对对齐和捕捉高阶依赖关系。
...
Method: ConFu扩展了传统的成对对比目标，增加了融合模态对比项，鼓励模态对与第三个模态的联合嵌入，从而捕捉高阶依赖关系。

Result: 在合成和真实多模态基准测试中，ConFu在检索和分类任务上表现出竞争力，支持统一的一对一和一对二检索。

Conclusion: ConFu能够有效捕捉高阶依赖关系（如XOR类关系），同时保持强成对对应关系，为多模态表示学习提供了统一框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基准仍局限于DeepFake伪造和单粒度标注，无法捕捉真实世界伪造场景的多样性和复杂性。

Method: 提出了多阶段混合伪造框架，集成专有模型进行任务规划和专家生成模型进行精确操作，建立了涵盖二元判断、伪造类型分类、伪造细节选择和解释性推理的多任务评估框架。
...
Result: 在nuScenes数据集上的实验表明，LaGen在长序列LiDAR场景生成方面优于最先进的生成和预测模型，尤其在后期帧表现更佳。
...
<details>
  <summary>Details</summary>
Motivation: 随着Transformer在单个遥感任务上的出色表现，现在正朝着通过多任务学习实现跨多个任务的统一模型迈进。多任务学习方法相比单任务方法具有更好的泛化性、可扩展性和实际应用性。

Method: 创建数据管理引擎处理复杂遥感数据环境，提出统一动态分辨率策略处理不同图像尺度，针对超高分辨率图像引入Zoom-in Chain机制，增强模型的目标检测能力并提出新的评估协议。

Result: 大量实验表明RSCoVLM在多样化任务上实现了最先进的性能，超越了现有的遥感视觉语言模型，甚至可与专门的专家模型相媲美。
...
TL;DR: Monet是一个训练框架，使多模态大语言模型能够在潜在视觉空间中进行推理，通过生成连续的嵌入作为中间视觉思维。该框架解决了潜在视觉推理的两个核心挑战：潜在视觉对齐的高计算成本和潜在嵌入监督不足，并提出了三阶段蒸馏监督微调管道和VLPO强化学习方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法在抽象视觉思维方面存在不足，其灵活性受限于外部工具。作者希望开发能够直接在潜在视觉空间进行推理的模型，实现更接近人类抽象视觉思维的能力。

Method: 采用三阶段蒸馏监督微调管道：1）潜在视觉对齐；2）监督微调；3）视觉潜在策略优化（VLPO）。构建了包含12.5万条真实世界、图表、OCR和几何推理链的高质量数据集Monet-SFT-125K。

Result: Monet-7B模型在真实世界感知和推理基准测试中表现一致提升，在具有挑战性的抽象视觉推理任务上展现出强大的分布外泛化能力。

Conclusion: Monet框架成功实现了多模态大语言模型在潜在视觉空间中的直接推理，为视觉潜在推理的未来发展提供了重要见解和基础。
...
### [63] [HTTM: Head-wise Temporal Token Merging for Faster VGGT](https://arxiv.org/abs/2511.21317)
*Weitian Wang,Lukas Meiner,Rai Shubham,Cecilia De La Parra,Akash Kumar*

Main category: cs.CV

TL;DR: 本文提出了一种名为HTTM的训练免费3D token合并方法，用于加速VGGT模型在3D场景重建中的推理过程，解决了全局注意力层带来的延迟瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: VGGT模型在3D场景重建中需要全局注意力层对所有视图的token进行全对全注意力计算，对于大场景的长序列输入会造成显著的延迟瓶颈。现有合并技术在不同注意力头中统一合并token，导致输出层中token相同，限制了模型的表示能力。

Method: 提出头级时间合并（HTTM）方法，在多头粒度上合并token，在头级连接后保持特征token的独特性。该方法利用头级观察到的空间局部性和时间对应性，以较低的合并成本实现更高的合并比率。

Result: HTTM在基于GPU的推理中实现了高达7倍的加速，同时性能下降可忽略不计。

Conclusion: HTTM是一种有效的训练免费加速方法，能够显著提升VGGT模型在3D场景重建中的推理效率，同时保持模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有遥感变化字幕生成方法通常使用CNNs/Transformers提取视觉表示或结合辅助任务增强结果，但存在区域感知弱和时间对齐有限的问题。
...
TL;DR: 提出了一种无监督框架，从工业视频流中提取用于视觉-语言-动作模型预训练的结构化数据，包括动作分割和潜在动作序列生成。


<details>
  <summary>Details</summary>
Motivation: 解锁大量未标记的人类演示数据，为视觉-语言-动作模型预训练提供可扩展的解决方案，促进制造业中具身AI的集成。
...
Conclusion: 这是首个从非结构化工业视频中全自动端到端提取和组织VLA预训练数据的系统，为制造业具身AI集成提供了可扩展方案。
...
TL;DR: 本文提出了一种基于超图的时空事件流补全机制，通过超图连接不同时空的事件标记，利用上下文信息传递来补全稀疏事件，并能灵活融合RGB标记实现多模态信息补全。
...
Method: 提出超图引导的时空事件流补全机制，通过超图连接不同时空的事件标记，利用上下文信息传递补全稀疏事件，并能融合RGB标记实现多模态信息补全，然后通过自注意力聚合不同时间步的超图节点信息。

Result: 在单标签和多标签事件分类任务上的大量实验充分验证了所提框架的有效性。

Conclusion: 该框架能有效解决事件流的空间稀疏性问题，实现多模态特征的学习和融合。
...
### [67] [Endo-G$^{2}$T: Geometry-Guided & Temporally Aware Time-Embedded 4DGS For Endoscopic Scenes](https://arxiv.org/abs/2511.21367)
...
<details>
  <summary>Details</summary>
Motivation: 内窥镜视频存在强烈的视角依赖效应（如镜面反射、湿润反射和遮挡），纯光度监督与几何不一致，导致早期几何漂移，错误形状在密集化过程中被强化且难以纠正。

Method: 1. 几何引导先验蒸馏：将置信度门控的单目深度转换为监督信号，使用尺度不变深度和深度梯度损失，采用预热到上限的调度软注入先验；2. 时间嵌入高斯场：在XYZT中表示动态，使用转子式旋转参数化，实现时间一致几何；3. 关键帧约束流式处理：在最大点数预算下进行关键帧聚焦优化，非关键帧轻量更新。

Result: 在EndoNeRF和StereoMIS-P1数据集上，Endo-G²T在单目重建基线中取得了最先进的结果。

Conclusion: Endo-G²T通过几何引导和时间感知训练方案，成功解决了动态内窥镜场景中的几何漂移问题，实现了时间一致的几何重建和高效处理。
...
Method: 提出频率感知令牌缩减策略：将令牌划分为高频令牌和低频令牌，选择性保留高频令牌，将低频令牌聚合成紧凑的直流令牌以保留重要低频成分。

Result: 通过大量实验和分析表明，该方法在减少计算开销的同时显著提高了准确性，并有效缓解了秩崩溃和过平滑问题。
...
<details>
  <summary>Details</summary>
Motivation: 解决类增量学习中的灾难性遗忘问题，通过直接优化模型权重来有效整合新旧知识。

Method: 采用两种权重合并：任务间权重合并统一所有先前阶段模型的权重，任务内权重合并促进当前任务学习；结合有界更新技术限制累积更新以保护先前知识。

Result: 在标准CIL基准测试中表现出色，性能优于现有最先进方法。

Conclusion: M&B方法无需修改架构组件或学习目标即可集成到现有CIL方法中，有效缓解灾难性遗忘问题。
...
TL;DR: 提出了一种名为MRPD的多模态鲁棒提示蒸馏框架，通过师生架构从三个不同模态的教师模型中蒸馏知识，构建鲁棒的3D点云模型，无需推理时额外计算成本。
...
Method: 使用师生框架，通过轻量级提示学习将学生点云模型特征与来自三个教师模型（视觉深度投影模型、高性能3D模型、文本编码器）的鲁棒嵌入对齐，采用置信度门控机制动态平衡多模态输入贡献。
...
TL;DR: DiverseVAR框架通过文本嵌入噪声注入和尺度旅行精炼技术，在不重新训练的情况下提升视觉自回归模型的多样性，同时最小化图像质量损失。


<details>
  <summary>Details</summary>
Motivation: 视觉自回归模型在图像生成中表现出色，但存在多样性不足的问题，即使对简单提示也会产生几乎相同的图像。这个问题在关注图像质量的主流研究中被忽视。
...
Conclusion: DiverseVAR框架有效解决了视觉自回归模型的多样性限制问题，无需重新训练或微调，为多样性-质量权衡提供了新的解决方案。
...
Method: 提出E-M3RF框架：1）使用旋转等变编码器提取旋转一致的几何特征；2）使用Transformer编码颜色特征；3）结合几何和颜色特征形成多模态表示；4）通过SE(3)流匹配预测碎片变换。

Result: 在四个数据集上的实验表明，在RePAIR数据集上，E-M3RF相比竞争方法将旋转误差降低23.1%，平移误差降低13.2%，Chamfer距离减少18.4%。

Conclusion: E-M3RF通过多模态特征融合有效解决了仅依赖几何特征的局限性，在文化遗产保护等实际应用中展现出显著优势。
...
Method: 1) 分析线性注意力和softmax注意力模块在移动设备上的性能，提出平衡生成效率与质量的线性混合架构去噪器；2) 设计时间步蒸馏策略，将采样步骤从20+压缩到仅2步；3) 应用移动端特定的注意力优化，实现注意力操作2倍加速。

Result: 首次在移动设备上实现快速720p图像到视频生成，单步条件下每帧720p视频生成时间小于100毫秒，生成质量与现有模型相当。

Conclusion: MobileI2V通过创新的架构设计和优化策略，成功解决了移动设备上实时视频生成的挑战，为移动端AI视频生成应用提供了可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 深度伪造检测方法的有效性往往更依赖于实现细节而非核心设计，这使得难以公平比较检测器并理解真正影响性能的因素。

Method: 通过系统研究不同设计选择对深度伪造检测模型准确性和泛化能力的影响，重点关注训练、推理和增量更新等方面，分离单个因素的影响。

Result: 实验确定了能持续改进深度伪造检测的设计选择，并在AI-GenBench基准测试中实现了最先进的性能。

Conclusion: 建立了一套与架构无关的稳健最佳实践，用于未来深度伪造检测系统的设计和开发。
...
TL;DR: 提出了一种用于抗核抗体(ANA)检测的新型多实例多标签学习框架，通过实例采样器、概率伪标签分发器和自定步调学习率系数来处理临床显微镜图像，无需手动预处理，在ANA数据集上取得了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 传统手动ANA检测方法速度慢、劳动密集且需要多年训练，而现有的机器学习方法在处理多实例多标签学习任务时面临挑战，特别是在临床实际应用中。

Method: 基于人类标注逻辑，提出包含三个任务特定组件的框架：实例采样器（抑制低置信度实例）、概率伪标签分发器（自适应分配标签）和自定步调学习率系数（根据经验标签观察调整训练）。

Result: 在ANA数据集上，相比最佳现有方法，F1-Macro提升7.0%，mAP提升12.6%；在三个公共医学MIML基准测试中，汉明损失和单错误率分别降低18.2%和26.9%，在所有关键指标上排名前二。
...
TL;DR: 本文提出了一种用于构建遥感基础模型的高效替代方案——专家集成框架，通过将训练过程分解为轻量级的任务特定专家模型，解决了当前大型模型在计算资源和环境可持续性方面的挑战。


<details>
  <summary>Details</summary>
Motivation: 当前遥感基础模型主要依赖增大模型规模和数据集体积，导致计算资源和数据需求过高，只有少数大型机构能够承担，且与可持续AI原则相悖。需要一种更高效、环保的替代方案。

Method: 采用专家集成框架，将训练过程分解为轻量级的任务特定ConvNeXtV2专家模型，这些专家模型可以被冻结和重用。该模块化方法支持联邦训练、剪枝和持续专家集成。

Result: 该方法在效率、可解释性和可扩展性方面具有显著优势，特别适合协作和资源受限的环境，为构建可扩展和高效的遥感基础模型指明了新方向。

Conclusion: 专家集成框架为遥感基础模型提供了一种高效、可持续的替代方案，解决了当前大型模型在资源消耗和环境影响方面的问题，推动了该领域向更可持续的方向发展。
...
TL;DR: 该研究提出了一种基于定量指标的序列图像生成方法，结合年龄缩放因子生成特定年龄的MRI图像，用于阿尔茨海默病的长期预测。
...
Method: 利用预训练视频生成模型在噪声潜在空间进行奖励建模，提出PRFL框架，在潜在空间进行偏好优化，无需VAE解码即可实现完整去噪链的梯度反向传播。
...
Result: 创建了一个受控但真实的基准，用于开发在真实室外环境中一致、忠实且可重光照的重建方法。

Conclusion: UAVLight为开发光照鲁棒的3D重建方法提供了可靠的基准，解决了现有数据集在光照变化研究方面的局限性。
...
Conclusion: 提出的框架能够提高在可变患者姿态条件下的骨盆地标检测准确性。
...
TL;DR: Harmony是一个解决音频-视频同步生成挑战的新框架，通过跨任务协同训练、全局-局部解耦交互模块和同步增强CFG机制，显著提升了生成保真度和细粒度同步效果。
...
Conclusion: 该框架展示了过采样和增强策略在少数类分类中的有效性，是迈向可靠计算机辅助诊断系统用于临床口腔癌早期检测的重要一步。
...
TL;DR: 论文主张LLM效率研究应从追求大规模复杂技术转向关注资源有限环境下的稳健简单性，提出新的研究议程以降低部署成本、促进公平和可持续发展。


<details>
  <summary>Details</summary>
Motivation: 当前主流的LLM效率方法（如MoE、推测解码、复杂RAG）仅适用于拥有庞大基础设施的科技巨头，导致大多数组织无法受益，加剧了不平等和碳浪费问题。

Method: 提出新的研究议程：无需重新训练即可改造预训练模型架构、轻量级微调保持对齐、长推理链的经济性、轻量级动态知识管理、采用开销感知效率作为标准基准。

Result: 通过重新定义效率概念，将采用成本、可持续性和公平性纳入考量，可以推动LLM部署的民主化。

Conclusion: LLM效率研究的下一前沿应是稳健简单性，确保优化技术能够减少不平等和碳浪费，而非加剧这些问题。
...
TL;DR: 本文提出了Harmonic Token Projection (HTP)框架，这是一种无需训练、词汇表或随机参数的可逆确定性文本嵌入生成方法，通过Unicode整数表示生成谐波轨迹来编码token，实现离散符号与连续向量空间的双射映射。


<details>
  <summary>Details</summary>
Motivation: 传统神经嵌入方法依赖统计共现或优化过程，缺乏可解释性和确定性。HTP旨在提供一种透明、高效且可逆的替代方案，从纯几何角度建立语义关系。

Method: HTP将每个token的Unicode整数表示解析为谐波轨迹，生成相位一致的投影，保持结构和可逆性，通过几何对齐实现语义相似度估计。

Result: 在STS-B及其多语言扩展上的实验显示，HTP在英语中达到Spearman相关系数ρ=0.68，在十种语言中保持稳定性能，计算成本极低，每句对延迟低于毫秒级。
...
TL;DR: PIRA是一种新的奖励模型训练范式，通过重构问题-答案对为偏好指令、聚合多样化偏好任务的奖励以及在不同dropout率下平均价值头输出来解决传统奖励模型的数据效率低和奖励过优化问题。
...
Method: PIRA采用三种策略：1) 将问题-答案对重新表述为基于偏好的指令以提供更清晰的任务规范；2) 聚合来自多样化偏好任务的奖励以减少偏见并提高鲁棒性；3) 在不同dropout率下平均价值头输出来稳定奖励。

Result: 大量实验证明了PIRA的有效性。

Conclusion: PIRA训练范式成功解决了奖励模型面临的数据效率低和奖励过优化问题，通过指令重构、奖励聚合和稳定化策略显著提升了模型性能。
...
TL;DR: 本文分析了大型语言模型在法律领域的表现，通过重组法律文档结构、定义修辞角色和模拟法院推理步骤来提升模型在法律判决预测任务中的性能。
...
Conclusion: 通过结构化信息处理和领域术语解释，可以有效提升大型语言模型在法律专业任务中的表现，特别是在零样本设置下。
...
TL;DR: FlexCode是一个基于流行度感知的生成式推荐框架，通过动态分配协作过滤和语义代码本的固定令牌预算，解决传统方法使用单一代码本无法平衡热门和长尾物品表示的问题。


<details>
  <summary>Details</summary>
Motivation: 现有生成式推荐方法使用统一的代码本编码所有物品，忽视了热门物品（富含协作信号）和长尾物品（依赖语义理解）之间的固有不平衡，限制了表示效率和泛化能力。

Method: 引入FlexCode框架，自适应地在协作过滤代码本和语义代码本之间分配固定令牌预算；使用轻量级MoE动态平衡CF特定精度和语义泛化；通过对齐和平滑目标保持流行度谱上的连贯性。

Result: 在公共和工业规模数据集上的实验表明，FlexCode始终优于强基线方法。

Conclusion: FlexCode为生成式推荐器中的令牌表示提供了新机制，实现了更强的准确性和尾部鲁棒性，并为平衡基于令牌的推荐模型中的记忆和泛化提供了新视角。
...
Method: 从Reddit收集自报告诊断数据，应用严格预处理步骤（语言过滤、NSFW内容移除、重复内容删除），使用LIWC进行语言分析，并采用微调语言模型和词袋特征进行二元分类实验。

Result: MindSET数据集规模是先前基准的两倍多，在诊断检测实验中，基于MindSET训练的模型表现优于现有基准，自闭症检测的F1分数提高了18个百分点。
...
TL;DR: 本文介绍了首个阿拉伯语跨领域上下文依赖的text-to-SQL数据集Ar-SParC，包含3,450个相关问题和10,225个问题-SQL对，并提出了GAT corrector方法提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 目前大多数text-to-SQL研究和数据集都集中在英语和中文，阿拉伯语领域尚未有相关工作，需要填补这一空白。
...
Conclusion: 成功创建了首个阿拉伯语text-to-SQL数据集，提出的GAT corrector方法有效提升了阿拉伯语言处理性能，并通过消融研究验证了其优势。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型在临床基准测试中表现良好，但它们可能通过错误的推理得出正确结论，这种失败模式在肿瘤学决策支持中具有安全隐患，而基于准确性的评估无法捕捉到这一点。

Method: 使用CORAL数据集的乳腺癌和胰腺癌病历，标注600个推理轨迹，定义三级分类法将计算失败映射到认知偏差框架。在前列腺癌咨询记录上验证该分类法，模拟提取、分析和临床推荐任务。

Result: 23%的解释存在推理错误，确认偏差和锚定偏差最常见。推理失败与指南不符且可能有害的建议相关，特别是在晚期疾病管理中。自动评估器可以检测错误存在但无法可靠分类子类型。
...
<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型部署通常对所有查询类型使用统一的提示策略，导致token效率低下，而输出token的成本比输入token高4-8倍。

Method: 比较了两种路由方法：使用预计算嵌入的简单MLP和微调的RoBERTa变换器，在MMLU问题上进行综合评估。

Result: MLP路由器在测试数据上达到90.5%的路由准确率，略高于RoBERTa(89.5%)，且参数少1.25亿。在三大LLM提供商上验证，token减少32.6%-33.9%。

Conclusion: DTS方法能有效降低token成本，路由决策在不同提供商间具有通用性，为LLM部署提供了成本优化的解决方案。
...
TL;DR: 该论文探讨了二维材料在能源存储和转换中的应用，并指出研究论文中包含了材料特性和制备方法等重要信息，但由于合成信息的分散性，获取这些信息存在困难。


<details>
  <summary>Details</summary>
Motivation: 二维材料因其独特的物理化学和电子特性在能源领域有广泛应用，但相关合成信息分散在不同研究论文中，难以系统获取和利用。

Method: 论文未提供具体方法，但从摘要可推断可能涉及文献综述、信息提取或数据库构建来整合分散的二维材料合成信息。

Result: 摘要未明确给出研究结果，但暗示了当前二维材料合成信息分散的问题，以及需要系统整理这些信息的必要性。

Conclusion: 需要开发有效的方法来整合和系统化二维材料的合成信息，以促进其在能源存储和转换领域的进一步应用和发展。
...
TL;DR: 本文提出了一种新的记忆化框架——多前缀记忆化，通过计算能够引出相同序列的不同前缀数量来量化LLM中的记忆化现象，相比传统方法能更可靠地区分记忆化与非记忆化数据。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型记忆化定义存在不足，特别是在对齐模型中无法全面捕捉记忆化现象，这带来了隐私和版权风险。需要一种更全面的框架来评估模型中的数据泄露。

Method: 提出多前缀记忆化框架，核心观点是记忆化序列被深度编码，因此可以通过比非记忆化内容多得多的不同前缀来检索。通过外部对抗性搜索识别能够引出目标序列的不同前缀数量来定义记忆化。

Result: 在开源和对齐聊天模型上的实验表明，多前缀定义能够可靠地区分记忆化与非记忆化数据，为审计LLM中的数据泄露提供了稳健实用的工具。

Conclusion: 多前缀记忆化框架通过量化记忆的稳健性（通过检索路径的多样性来衡量），为评估语言模型中的数据记忆化提供了更全面和可靠的方法。
...
TL;DR: 本文提出了DSPy+HELM框架，通过结构化提示方法改进语言模型评估，发现传统HELM基准会低估模型性能并导致排名错误。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型评估框架（如HELM）使用固定提示，无法准确反映模型真实性能，需要开发可扩展的方法来估计性能上限。

Method: 开发了可复现的DSPy+HELM框架，使用四种提示方法在七个基准上评估四个前沿语言模型，并与HELM基线进行比较。

Result: 发现HELM低估模型性能4%，性能估计变异性增加2%，在3/7基准上排名翻转，引入推理可减少提示敏感性。

Conclusion: 可扩展的性能上限估计能够创建更有决策价值的基准，这是首个大规模实证研究语言模型在基准和提示方法上的行为特征。
...
TL;DR: 提出了一种新的Length-MAX分词器，通过最小化平均字符数来减少训练和推理时的token数量，相比BPE在10K-50K词汇量下减少14-18%的token，在64K词汇量下减少13%。
...
Result: 在GPT-2模型上，训练步骤减少17-18.5%，推理延迟降低12.7-13.7%，吞吐量提升16%。下游任务表现提升：LAMBADA困惑度降低11.7%，HellaSwag准确率提升4.3%。词汇覆盖率达99.62%，OOV率仅0.12%。

Conclusion: 优化平均token长度而非仅频率，是实现更高效语言建模的有效方法，在不牺牲下游性能的情况下显著提升效率，推理时嵌入和KV缓存内存减少18%。
...
TL;DR: Evo-Memory是一个评估LLM智能体自进化记忆能力的综合流式基准框架，通过顺序任务流测试LLM的记忆搜索、适应和进化能力，并提出ReMem方法来整合推理、任务行动和记忆更新。


<details>
  <summary>Details</summary>
Motivation: 现有评估主要关注静态对话场景，忽视了LLM在连续任务流中积累和重用经验的动态能力，而现实世界环境需要LLM具备持续学习和记忆更新的能力。

Method: 构建Evo-Memory基准框架，将数据集结构化顺序任务流，统一实现十多个代表性记忆模块，提出ExpRAG基线方法和ReMem行动-思考-记忆优化流程。

Result: 在10个多样化多轮目标导向和单轮推理问答数据集上评估了各种记忆模块的性能。

Conclusion: Evo-Memory填补了LLM智能体动态记忆评估的空白，为测试时记忆进化提供了标准化框架，ReMem方法实现了推理、行动和记忆更新的紧密集成。
...
TL;DR: 本文研究跨语言论元挖掘方法，针对低资源语言波斯语，构建了三种训练场景：零样本迁移、LLM增强和跨语言混合模型。结果显示跨语言模型在波斯语上表现最佳，F1达74.8%。
...
Method: 构建三种训练场景：零样本迁移（仅英语数据）、LLM增强（英语数据+合成样本）、跨语言混合（英语+人工翻译波斯语数据）。

Result: 零样本迁移：英语50.2%，波斯语50.7%；LLM增强：英语59.2%，波斯语69.3%；跨语言混合：波斯语74.8%。
...
TL;DR: 本文提出了一种结合角色交叉最小对、时间涌现分析和跨模型比较的方法，研究大语言模型如何实现语义角色。研究发现：高度集中的电路、渐进式结构细化而非相变、以及适度的跨尺度守恒。
...
Result: 发现：(i) 高度集中的电路（89-94%归因在28个节点内）；(ii) 渐进式结构细化而非相变，较大模型有时绕过局部电路；(iii) 适度的跨尺度守恒（24-59%组件重叠）以及高光谱相似性。

Conclusion: LLMs形成了紧凑、因果隔离的抽象语义结构机制，这些机制在不同尺度和架构间表现出部分转移。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：LLM无法直接访问私有动态知识图谱；RAG系统序列化图结构且难以处理多轮上下文；传统KGQA系统仅支持单轮问答且延迟高。需要结合对话灵活性和结构化知识图谱基础。

Method: 采用模块化多智能体架构，通过任务专用LLM智能体协作实现上下文解释、对话跟踪、实体关系链接和高效查询规划，生成可执行的SPARQL查询。

Result: 在大型多样化知识图谱上的实验表明，Chatty-KG在单轮和多轮设置中显著优于最先进基线，获得更高的F1和P@1分数。模块化设计保持对话连贯性，支持演化知识图谱而无需微调或预处理。
...
TL;DR: 本文评估了大型语言模型(LLMs)在不同类型查询中的表现差异，发现LLMs在定义类问题上表现最佳，而在举例类问题上表现最差。研究还表明LLMs倾向于对流行知识进行更多释义，而对技术性知识则较少。


<details>
  <summary>Details</summary>
Motivation: 虽然人类能够轻松提供各种类型的答案（如例子和释义），但LLMs在处理除定义类查询外的其他类型问题时表现不佳。本研究旨在评估这种性能下降，并探讨预训练数据对LLMs回答多样化语言查询的影响。

Method: 使用TrackList细粒度语言和统计分析管道，引入RefoMed-EN数据集（包含6170个人工标注的医学术语及其对应定义、命名、举例、解释或释义），通过句法和语义相似性指标、统计相关性和嵌入来评估LLM输出质量。

Result: LLMs在定义类问题上的任务表现最高，而在举例类问题上表现最低。对于定义类问题，大型语言模型倾向于对流行和频繁知识进行更多释义，而对尾部和技术性知识（特别是在专业文本中）则较少进行释义。

Conclusion: LLMs在处理不同类型语言查询时存在显著性能差异，定义类问题表现最佳而举例类问题表现最差，且模型对流行知识的处理优于技术性知识。
...
<details>
  <summary>Details</summary>
Motivation: 探讨上下文学习是否能覆盖预训练的标签语义，还是仅仅细化现有的语义骨干，理解ICL在自然演示和反转演示下的行为差异。

Method: 将LLMs视为提示诱导的分类器，使用自然演示(正确标签)和反转演示(系统翻转标签含义)对比其行为，分解ICL行为为三个对齐指标(真实性、先验性和提示对齐)，并引入语义覆盖率。

Result: 在8个分类任务和8个开源LLMs上，发现ICL在自然演示下提高准确性但保持强先验对齐；在反转演示下模型无法学习连贯的反语义分类器，语义覆盖率保持为零。

Conclusion: ICL主要调整输入如何投影到预训练期间学习的稳定语义方向，而不是灵活地重新映射标签含义，表明在这些规模上覆盖标签语义需要超出ICL的干预措施。
...
TL;DR: 该研究探索在缅甸语等低资源语言分类任务中使用Kolmogorov-Arnold Networks (KANs)替代传统的多层感知机(MLPs)作为分类头，评估了三种KAN变体在不同嵌入方法上的表现。


<details>
  <summary>Details</summary>
Motivation: 在低资源语言如缅甸语中，分类任务通常只微调最后的分类层，而保持预训练编码器权重冻结。虽然MLPs常用，但其固定的非线性可能限制表达能力并增加计算成本。

Method: 研究评估了基于傅里叶的FourierKAN、基于样条的EfficientKAN和基于网格的FasterKAN三种KAN变体，在TF-IDF、fastText和多语言变换器（mBERT、Distil-mBERT）等多种嵌入方法上进行实验。

Result: 实验结果显示，基于KAN的分类头与MLPs竞争或更优。EfficientKAN与fastText组合获得最高F1分数（0.928），而FasterKAN在速度和准确性之间提供了最佳平衡。在变换器嵌入上，EfficientKAN与mBERT组合达到或略微优于MLPs（0.917 F1）。

Conclusion: 这些发现表明KANs是低资源语言分类中MLPs的表达能力强、效率高的替代方案。
...
TL;DR: 该研究评估了28种大语言模型配置在58个需要字符级约束满足的单词谜题上的表现，发现架构差异比参数规模扩展对性能影响更大，模型在拼写异常但约束有效的常见单词上存在系统性失败。


<details>
  <summary>Details</summary>
Motivation: 评估大语言模型在受控文本生成中满足硬性拼写约束的能力，目前缺乏跨架构的系统性评估。

Method: 使用三个模型家族（Qwen3、Claude Haiku-4.5、GPT-5-mini）的28种配置，在58个需要字符级约束满足的单词谜题上进行测试，并参考10,000名人类解谜者的难度评分。

Result: 架构差异产生的性能差距（2.0-2.2倍）远大于家族内参数扩展（83%增益）；高容量模型在思考预算上表现良好，而中等规模模型趋于饱和或退化；模型在拼写异常的常见单词上失败率高达89-96%。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在通用领域表现出色，但在专业领域如抵押贷款金融中需要增强领域特定知识，同时保持指令跟随的准确性。单一模型在处理结构化任务和对话任务时存在性能权衡。

Method: 采用双轨专业化框架，从单一基础模型开发两个专家模型：一个用于对话问答，另一个用于结构化任务（分类和摘要）。应用指令残差技术恢复领域适应后的指令跟随能力，并使用少量样本分类的智能任务路由机制。

Result: 在领域特定基准测试中，最终模型MLM v2显著优于基础模型，在摘要、问答和分类任务上分别获得4.58 vs 3.99、4.09 vs 4.0和2.6 vs 1.2的LLM-as-a-Judge评分，BERTScore也全面优于基线方法。

Conclusion: 双专家架构结合指令残差技术能够有效解决专业领域大语言模型的领域知识增强与指令跟随保真度的双重挑战，在抵押贷款金融领域取得了显著性能提升。
...
TL;DR: 本研究探讨了在少量人类调查数据上微调大语言模型是否能改善其模拟人类行为的准确性。通过信息泄露行为实验，发现微调能显著提升模型的异质性、对齐度和信念-行为一致性，但仍无法复现原始研究的回归系数，表明LLM生成的数据仍不适合替代人类参与者进行正式推断分析。


<details>
  <summary>Details</summary>
Motivation: 针对大语言模型是否能替代人类参与者的持续争议，本研究关注一个关键问题：在少量人类调查数据上微调LLM是否能缓解现有问题并产生更真实的模拟结果。

Method: 使用信息泄露行为实验，比较人类和LLM生成的反应，从分布差异、子群体对齐、信念-行为一致性和回归系数恢复等多个维度进行评估。

Result: 在少量人类样本上微调显著改善了基模型的异质性、对齐度和信念-行为一致性，但即使表现最好的微调模型也无法复现原始研究的回归系数。

Conclusion: 虽然微调能改善LLM模拟人类行为的某些方面，但LLM生成的数据仍不适合替代人类参与者进行正式的推断分析。
...
TL;DR: 本文介绍了首个伊森语开放对话语音数据集的开发，该数据集包含自然语音而非朗读语音，捕捉了真实的语言现象，并解决了伊森语缺乏标准化正字法的挑战。


<details>
  <summary>Details</summary>
Motivation: 开发伊森语（泰国最广泛使用的地区方言）的开放对话语音数据集，以支持包容性AI发展、促进对代表性不足语言的研究，并为建模对话语音提供基础。

Method: 建立实用的转录协议，在表征准确性和计算处理需求之间取得平衡，解决伊森语缺乏标准化正字法的问题。

Result: 成功创建了包含自然语音的伊森语对话数据集，捕捉了口语化表达、自发韵律、不流畅性和与中央泰语的频繁语码转换等真实语言现象。

Conclusion: 通过发布这一开放数据集，为包容性AI发展做出贡献，支持对代表性不足语言的研究，并为解决对话语音建模中的语言和技术挑战提供基础。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在许多任务上取得了最先进的性能，但其大规模往往导致高计算和环境成本，限制了可访问性。参数高效微调方法通过减少可训练参数同时保持强大的下游性能来解决这一挑战。

Method: 引入PEFT-Bench统一基准测试，在27个NLP数据集和6种PEFT方法上进行评估，并提出了PSCP指标来综合考虑可训练参数、推理速度和训练内存使用。

Result: 开发了一个可复现的评估框架，能够全面比较不同PEFT方法的性能表现，考虑了实际部署中的多个重要因素。
...
Method: 通过对瞬时单令牌注入进行微调，训练模型检测和识别注入的激活模式。

Result: 模型从0.4%准确率提升到85%，能检测单令牌位置的瞬时"想法"，并在后续生成步骤中报告语义内容。

Conclusion: 至少一种内省行为可以通过训练直接诱导，为构建内置AI透明度提供了可行路径。
...
TL;DR: 本文创建了捷克语和斯洛伐克语细粒度证据提取数据集，评估了大型语言模型在证据提取任务中的表现，发现模型经常无法准确复制源文本证据，且模型性能与参数规模不成正比。


<details>
  <summary>Details</summary>
Motivation: 在线新闻评论中经常传播错误信息，需要有效的方法来检测事实错误信息。为了支持或反驳这些评论中的主张，需要识别相关文档并精确定位证明或反驳每个主张的具体文本片段。

Method: 创建了由付费标注人员标注的细粒度证据数据集，评估了多个大型语言模型（包括llama3.1:8b、gpt-oss-120b、qwen3:14b等）在该数据集上的表现，分析模型输出与人工标注的一致性。

Result: LLMs经常无法从源文本逐字复制证据，导致无效输出。llama3.1:8b模型虽然规模较小但正确输出比例较高，而gpt-oss-120b模型参数更多但表现不佳。qwen3:14b、deepseek-r1:32b和gpt-oss:20b在模型大小与人工标注对齐之间达到了有效平衡。

Conclusion: 大型语言模型在细粒度证据提取任务中表现存在局限性，模型性能与参数规模不一定成正比，选择合适的模型大小对于任务效果很重要。
...
Method: DSR-SQL将Text-to-SQL建模为自适应上下文状态和渐进生成状态之间的交互。上下文状态通过精炼大型模式和选择相关结构构建紧凑的语义忠实环境；生成状态将SQL合成形式化为反馈引导的状态转换，使模型能够自我纠正并与用户意图对齐。

Result: 在Spider 2.0-Snow上达到35.28%的执行准确率，在BIRD开发集上达到68.32%的执行准确率。

Conclusion: DSR-SQL框架有效解决了复杂企业数据库Text-to-SQL中的连贯推理问题，展示了竞争性性能。
...
TL;DR: 本文提出Odin架构，通过定向双模块机制在特定Transformer层注入图结构，避免了传统GNN的过平滑和跳数依赖扩散问题，实现了结构与文本的深度融合。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么依赖GNN但受限于过平滑和跳数依赖扩散，要么使用Transformer但忽略图拓扑结构。需要一种能有效结合文本理解和结构推理的新架构。

Method: 提出Odin架构，在特定Transformer层通过定向双模块机制注入图结构，不依赖多跳扩散，而是在不同层次整合结构信息。还设计了轻量版Light Odin用于高效训练和推理。

Result: 在多个文本丰富图基准测试中，Odin达到最先进准确率，Light Odin在显著降低计算成本的同时保持竞争力。

Conclusion: Odin和Light Odin形成了统一的、无跳数的结构-文本集成框架，其表达能力严格包含纯Transformer和GNN，解决了现有方法的局限性。
...
TL;DR: 本文对六种最先进的模型合并方法进行了大规模系统评估，发现在LLMs中，最古老简单的Task Arithmetic方法是唯一能可靠提升性能的方法，而其他方法通常导致性能显著下降。


<details>
  <summary>Details</summary>
Motivation: 模型合并是一种无需额外训练就能将多个微调检查点合并为单一模型的方法，但尚不清楚这些在小模型和分类器上报告的优势是否能推广到LLMs。

Method: 在四个开源权重LLMs、每个基础模型十二个微调检查点和十六个标准LLM基准上，系统评估了六种最先进的合并方法，包括最近提出的子空间方法。

Result: Task Arithmetic是唯一能可靠在LLMs上产生性能增益的方法，其他干扰感知和子空间合并方法通常导致显著性能下降。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习可读性评估方法大多未能同时考虑文本长度和可读性标签的序数关系，需要更全面的解决方案。

Method: 采用双向可读性评估机制预测句子级可读性标签辅助文档级预测，并引入成对排序算法通过标签减法建模序数关系。

Result: 在中文和英文数据集上的实验结果表明，该模型取得了有竞争力的性能，优于其他基线模型。

Conclusion: 所提出的方法能够有效解决文本长度和可读性标签序数关系的问题，在多语言可读性评估任务中表现优异。
...
TL;DR: 本文研究语音翻译模型中性别分配机制，发现模型不仅复制训练数据中的性别关联，还学习更广泛的男性主导模式。模型能够基于声学输入覆盖内部语言模型的男性偏见，并通过第一人称代词将性别化术语与说话者关联，利用分布在频谱中的性别信息。


<details>
  <summary>Details</summary>
Motivation: 语音通过声学线索（如音高）传递说话者性别信息，在语音翻译中可能导致性别误判风险。目前对语音翻译模型如何做出性别分配决策的理解仍很有限。

Method: 研究三个语言对（英-西/法/意）的语音翻译模型，分析训练数据模式、内部语言模型偏见和声学信息如何相互作用。使用对比特征归因分析频谱图。

Result: 模型学习更广泛的男性主导模式而非简单复制训练数据中的特定术语性别关联。具有更高性别准确性的模型通过第一人称代词将性别化术语与说话者关联，利用分布在频率谱而非集中在音高的性别信息。

Conclusion: 语音翻译模型采用复杂机制进行性别分配，能够基于声学输入覆盖内部语言模型偏见，并发现了一种新的机制：通过第一人称代词链接性别化术语与说话者，利用分布式频谱信息。
...
TL;DR: 本文介绍了孟加拉手语翻译数据集IsharaKhobor及其两个子集，旨在解决孟加拉手语资源匮乏的问题，并提供了基于地标和RQE嵌入的基准测试。


<details>
  <summary>Details</summary>
Motivation: 孟加拉手语翻译研究因资源匮乏而受限，需要标准句子级数据集来开发面向孟加拉语社区的聋哑和听力障碍人群的AI辅助工具。

Method: 创建IsharaKhobor数据集及其两个子集，进行词汇限制和规范化处理，使用基于地标的原始嵌入和RQE嵌入进行基准测试。

Result: 开发了公开可用的数据集IsharaKhobor，并生成了两个经过词汇限制和规范化的子集IsharaKhobor_small和IsharaKhobor_canonical_small。

Conclusion: 该数据集为孟加拉手语翻译研究提供了重要资源，通过基准测试展示了前进方向，数据集已在Kaggle平台公开可用。
...
TL;DR: RoParQ是一个专门评估闭卷多选题问答中跨释义一致性的基准，通过生成释义并筛选不一致样本构建。提出了XParaCon评估指标和基于推理的释义感知SFT策略，显著提升了模型鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在回答释义问题时表现出不一致行为，表明其依赖表面模式而非真正的语义理解。需要专门评估和提升模型在语义变化下的鲁棒性。

Method: 从标准数据集生成释义，通过评判模型筛选不一致样本构建RoParQ基准。提出XParaCon指标量化模型鲁棒性，并实施基于推理的释义感知SFT策略。

Result: 实验表明，经过针对性对齐的轻量级模型达到了与更大预训练模型相当的鲁棒性水平，显著提升了跨释义一致性。

Conclusion: 该方法有效缓解了表面记忆问题，培养了更鲁棒可靠的大语言模型，证明了针对性语义对齐策略的有效性。
...
### [119] [Generating Querying Code from Text for Multi-Modal Electronic Health Record](https://arxiv.org/abs/2511.20904)
*Mengliang ZHang*

Main category: cs.IR

TL;DR: 本文构建了TQGen数据集，结合表格和临床文本进行自然语言到查询生成，并提出TQGen-EHRQuery框架来解决电子健康记录中复杂医学术语和多样化查询的挑战。
...
Method: 提出TQGen-EHRQuery框架，包含医学知识模块和问题模板匹配模块。引入工具集概念，将文本处理模块封装为可调用工具，提高处理效率和灵活性。

Result: 通过广泛实验验证了数据集和工作流程的有效性，展示了其在增强电子健康记录系统信息查询方面的潜力。

Conclusion: TQGen数据集和提出的框架能够有效解决电子健康记录中的查询挑战，提高信息检索效率。
...
### [120] [FITRep: Attention-Guided Item Representation via MLLMs](https://arxiv.org/abs/2511.21389)
*Guoxiao Zhang,Ao Li,Tan Qu,Qianlong Xie,Xingxing Wang*

Main category: cs.IR

TL;DR: FITRep是一个基于特征整合理论的白盒物品表示框架，用于细粒度物品去重，通过概念层次信息提取、结构保持降维和FAISS聚类来解决多模态嵌入中的局部结构崩溃问题。


<details>
  <summary>Details</summary>
Motivation: 在线平台存在大量视觉和文本相似的近重复物品，导致用户体验下降。现有的多模态大语言模型方法将表示视为黑盒，忽略了结构关系，导致局部结构崩溃问题。

Method: 提出FITRep框架，包含三个组件：(1) CHIE使用MLLMs提取层次化语义概念；(2) SPDR采用自适应UMAP方法进行高效信息压缩；(3) FBC基于FAISS的聚类为每个物品分配唯一聚类ID。

Result: 在美团广告系统部署后，在线A/B测试显示点击率提升3.60%，千次展示收益提升4.25%。

Conclusion: FITRep框架有效解决了多模态物品表示中的局部结构崩溃问题，在实际应用中取得了显著效果提升。
...
### [121] [RIA: A Ranking-Infused Approach for Optimized listwise CTR Prediction](https://arxiv.org/abs/2511.21394)
*Guoxiao Zhang,Tan Qu,Ao Li,DongLin Ni,Qianlong Xie,Xingxing Wang*

Main category: cs.IR

TL;DR: RIA是一个统一的端到端重排序框架，通过集成点对点和列表评估来解决现有方法中排名与重排序分离的问题，在保持低延迟的同时提升推荐质量。


<details>
  <summary>Details</summary>
Motivation: 现有重排序方法通常将排名和重排序解耦，导致在严格延迟约束下存在组合稀疏性和有限表示能力的弱列表评估模型。

Method: RIA包含四个关键组件：用户和候选双Transformer用于细粒度用户-物品-上下文建模；上下文感知用户历史和目标模块用于位置敏感偏好学习；列表多HSTU模块捕获层次化物品依赖关系；嵌入缓存模块在推理时桥接效率和效果。

Result: 在公开和工业数据集上的广泛实验表明，RIA在AUC和LogLoss指标上优于最先进模型。在美团广告系统部署中，在线A/B测试实现了CTR提升1.69%和CPM提升4.54%。

Conclusion: RIA通过共享排名和重排序的表示，实现了丰富的上下文知识转移，同时保持低延迟，显著提升了推荐系统的性能。
...
<details>
  <summary>Details</summary>
Motivation: 双曲几何是嵌入层次数据结构的有效几何方法，但双曲嵌入质量与输入层次结构紧密相关。研究发现最优双曲嵌入需要高分支因子和单继承结构，因此需要帮助知识工程师重新组织层次知识以满足这些标准。

Method: 提出基于提示的方法，使用大型语言模型在已知双曲嵌入期望标准的指导下转换现有层次结构，通过实验在16个不同层次结构上验证方法有效性。

Result: 实验结果显示，经过LLM重构的层次结构在多个标准嵌入质量指标上始终产生更高质量的双曲嵌入，同时LLM引导的层次重构能够提供可解释的重组，为知识工程师提供理由说明。

Conclusion: 大型语言模型能够有效自动重构层次结构以满足双曲嵌入的优化标准，不仅提升嵌入质量，还能提供解释性重组，为知识工程师提供有价值的指导。
...
TL;DR: 本文介绍了AssurAI，一个针对韩语多模态生成AI安全评估的质量控制数据集，包含11,480个文本、图像、视频和音频实例，覆盖35种AI风险因素，专门针对韩语社会文化背景设计。


<details>
  <summary>Details</summary>
Motivation: 当前的安全数据集主要是英语中心化，无法捕捉非英语社会文化背景（如韩语）中的特定风险，且通常仅限于文本模态。需要开发专门针对韩语多模态环境的安全评估工具。

Method: 1. 定义35种AI风险因素的分类法；2. 构建大规模韩语多模态数据集AssurAI；3. 采用两阶段构建（专家引导播种和众包扩展）、三重独立标注和迭代专家红队测试的严格质量控制流程。

Result: 试点研究验证了AssurAI在评估最新LLM安全性方面的有效性。数据集已公开发布，包含11,480个多模态实例。

Conclusion: AssurAI填补了韩语多模态AI安全评估的空白，有助于为韩语社区开发更安全可靠的生成AI系统。
...
### [124] [$A^2Flow:$ Automating Agentic Workflow Generation via Self-Adaptive Abstraction Operators](https://arxiv.org/abs/2511.20693)
*Mingming Zhao,Xiaokang Wei,Yuanqi Shao,Kaiwen Zhou,Lin Yang,Siwei Rao,Junhui Zhan,Zhitang Chen*

Main category: cs.AI

TL;DR: A²Flow是一个完全自动化的智能体工作流生成框架，基于自适应抽象操作符，无需手动预定义操作符，通过三阶段操作符提取过程实现工作流构建。


<details>
  <summary>Details</summary>
Motivation: 现有方法严重依赖手动预定义操作符，限制了泛化性和可扩展性，需要开发完全自动化的智能体工作流生成框架。

Method: 采用三阶段操作符提取：1)基于案例的初始操作符生成；2)操作符聚类和初步抽象；3)深度提取抽象执行操作符。结合操作符记忆机制增强工作流搜索。

Result: 在通用和具身基准测试中，A²Flow相比最先进基线平均性能提升2.4%和19.3%，资源使用减少37%。

Conclusion: A²Flow通过完全自动化的操作符提取和工作流构建，显著提升了智能体工作流的性能和效率，证明了自适应抽象操作符的有效性。
...
### [125] [Reasoning With a Star: A Heliophysics Dataset and Benchmark for Agentic Scientific Reasoning](https://arxiv.org/abs/2511.20694)
*Kevin Lee,Russell Spiewak,James Walsh*

Main category: cs.AI

TL;DR: 本文提出了一个名为'Reasoning With a Star'的日球物理学推理数据集和基准测试方法，通过多智能体模式改进大型语言模型在科学推理中的表现。


<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型在日球物理学科学推理中的挑战，包括整合物理假设、保持单位一致性以及提供清晰科学格式的需求。
...
Conclusion: 多智能体方法能够有效提升大型语言模型在复杂科学推理任务中的性能，特别是在需要演绎推理的场景下。
...
### [126] [A Brief History of Digital Twin Technology](https://arxiv.org/abs/2511.20695)
*Yunqi Zhang,Kuangyu Shi,Biao Li*

Main category: cs.AI

TL;DR: 数字孪生技术从NASA航天器模拟发展而来，现已在医疗领域实现转型应用。它通过整合影像、生物传感器和计算模型创建患者特异性虚拟副本，支持诊断、治疗规划和药物开发。尽管面临互操作性、数据隐私和模型保真度等挑战，但可解释AI、联邦学习等新兴解决方案提供了前进路径。
...
Method: 整合医学影像、生物传感器和计算模型，创建动态的数据驱动虚拟患者副本，通过实时数据流持续更新并支持双向交互。
...
TL;DR: 本文对多模态思维链推理进行了综合分析，评估其在A-OKVQA、OKVQA和ChartQA数据集上的有效性，发现视觉集成显著减少推理幻觉，但思维链推理效果在不同问题类型间差异显著。


<details>
  <summary>Details</summary>
Motivation: 探索多模态思维链推理在不同领域的泛化能力，特别是需要广泛常识和世界知识的任务，而不仅仅是科学推理。

Method: 采用Zhang等人提出的两阶段框架，将推理生成与答案推断分离，通过门控融合机制集成视觉特征与基于T5的语言模型，并进行系统消融研究分析视觉特征、推理质量和架构选择的贡献。

Result: 视觉集成显著减少推理生成中的幻觉，但思维链推理的有效性在不同问题类型间差异很大，常识推理尤其具有挑战性。

Conclusion: 为研究人员实现多模态推理系统提供实用见解，并确定了跨领域泛化未来改进的关键领域。
...
TL;DR: 本文提出了ICPO方法，通过利用LLM生成不同响应的概率来反映其对推理过程的自我评估，结合可验证奖励来提升强化学习的推理能力。
...
Method: 提出ICPO方法，通过计算多个响应在相同输入提示下的相对生成概率来获得偏好优势分数，并将其与可验证奖励结合指导探索过程。

Result: 在四个通用领域基准和三个数学基准上的综合实验表明，ICPO相比GRPO能稳定提升推理能力。

Conclusion: ICPO方法有效缓解了粗粒度奖励和奖励噪声问题，抑制过度自信错误，增强被低估高质量响应的相对优势，促进更彻底的探索。
...
TL;DR: RILKE是一种在表示空间进行干预的知识控制方法，能够在冻结基础权重的情况下对LLMs进行细粒度知识更新，避免大规模重新训练。


<details>
  <summary>Details</summary>
Motivation: 解决LLMs产生错误或过时内容的问题，需要在终身学习设置中高效准确地更新知识，同时避免多次编辑之间的干扰。

Method: 通过表示空间干预，学习具有抗释义鲁棒性和编辑局部性的模块，将每个更新限制在低维子空间，并使用查询自适应路由器选择适当模块。

Result: 在LLaMA和Qwen模型的知识编辑基准测试中，RILKE具有高编辑成功率、强释义泛化能力，并能保持通用效用，内存开销较小。
...
### [130] [ENACT: Evaluating Embodied Cognition with World Modeling of Egocentric Interaction](https://arxiv.org/abs/2511.20937)
...
Method: 将具身认知评估构建为部分可观测马尔可夫决策过程，使用来自机器人仿真的可扩展流水线合成问答对，包含前向世界建模和逆向世界建模两个序列重排序任务。

Result: 实验显示前沿视觉语言模型与人类之间存在性能差距，且随着交互视野扩大而加大；模型在逆向任务上表现更好，并表现出人类中心偏见。

Conclusion: 现代视觉语言模型确实表现出具身认知的迹象，但存在与人类的性能差距和人类中心偏见，表明具身训练对发展真正智能的重要性。
...
### [131] [Improving Procedural Skill Explanations via Constrained Generation: A Symbolic-LLM Hybrid Architecture](https://arxiv.org/abs/2511.20942)
...
TL;DR: 本文对Halpern和Pearl的因果关系定义进行了抽象化，提取其关键特征，使其能够应用于任何定义了反事实的模型。通过这种抽象化，该定义可以处理更广泛的模型类型，包括允许回溯的模型，并能处理包含析取、否定、信念和嵌套反事实的复杂公式。


<details>
  <summary>Details</summary>
Motivation: Halpern和Pearl的因果关系定义虽然重要，但局限于因果模型。作者希望扩展这一定义的适用范围，使其能够应用于更广泛的模型类型，并能处理更复杂的逻辑表达式。

Method: 通过抽象化Halpern-Pearl定义的关键特征，构建一个通用的因果关系定义框架，该框架可以应用于任何定义了反事实的模型。

Result: 成功开发了一个抽象的因果关系定义，能够处理Halpern-Pearl定义无法处理的复杂逻辑表达式（如析取、否定、信念和嵌套反事实），并能应用于更广泛的模型类型。

Conclusion: 抽象化的因果关系定义不仅扩展了原定义的适用范围，还加深了对因果关系定义特征的理解，并能进一步扩展到解释的定义。
...
<details>
  <summary>Details</summary>
Motivation: 现有网页自动化方法面临DOM结构过大（10,000-100,000个token）的挑战，现有策略要么依赖粗糙的DOM截断（可能丢失关键信息），要么使用低效的启发式方法和独立排名模型，无法在精度和可扩展性之间达到最佳平衡。

Method: 提出DOM树剪枝编程方法，让LLM生成可执行的Python评分脚本来基于分解子任务的语义线索动态过滤DOM元素。该方法避免了LLM直接处理原始大型DOM，将遍历和评分委托给轻量级、可解释的程序。

Result: 实现了候选元素25倍到50倍的减少，便于精确的动作定位并减轻注意力稀释。在低级别接地任务上，准确率从46.8%大幅提升至88.28%。

Conclusion: Prune4Web通过程序化剪枝方法有效解决了网页自动化中的DOM处理挑战，在真实世界网页自动化中表现出卓越的效能。
...
<details>
  <summary>Details</summary>
Motivation: 在伪布尔求解中，目前最成功的单元传播策略是结合观察文字方案和计数方法的混合模式，但现有方法仍有改进空间

Method: 为混合决策引入新的启发式方法

Result: 新启发式方法在RoundingSAT求解器中能够大幅超越当前方法

Conclusion: 新的启发式方法显著提升了伪布尔求解中混合单元传播策略的性能
...
### [135] [SpatialBench: Benchmarking Multimodal Large Language Models for Spatial Cognition](https://arxiv.org/abs/2511.21471)
...
Method: 构建了包含15个任务的SpatialBench基准，采用层次化空间认知框架，并引入能力导向的统一评估指标。

Result: 实验显示模型在感知层面表现良好，但在符号推理、因果推理和规划方面存在局限；人类倾向于选择性目标导向抽象，而模型过度关注表面细节。
...
### [136] [Pessimistic Verification for Open Ended Math Questions](https://arxiv.org/abs/2511.21522)
...
<details>
  <summary>Details</summary>
Motivation: 现有验证性能受限于错误检测能力，需要更有效的方法来提升数学问题的验证可靠性。

Method: 设计悲观验证变体，为同一证明构建多个并行验证流程，任一流程报告错误即判定证明不正确。

Result: 该方法在多个数学验证基准上显著提升性能，token效率甚至超过扩展长链思维，且发现数据集标注错误是强模型假阴性的主要原因。
...
### [137] [Self-Transparency Failures in Expert-Persona LLMs: A Large-Scale Behavioral Audit](https://arxiv.org/abs/2511.21569)
*Alex Diep*

Main category: cs.AI

TL;DR: 该研究通过19,200次试验审计了16个开源模型在专业角色下的AI身份披露能力，发现模型在不同领域存在显著的透明度不一致性，某些模型的身份披露率低至2.8%，而推理优化反而会抑制透明度。


<details>
  <summary>Details</summary>
Motivation: 研究动机是评估语言模型在专业高风险领域中可靠披露AI身份的能力，因为如果模型无法诚实地表明其AI身份，用户就无法信任其能力边界，可能导致用户受到伤害。

Method: 采用共同花园设计，对16个参数量从4B到671B的开源模型进行审计，共进行19,200次试验，使用贝叶斯验证和Rogan-Gladen校正来确保测量误差的鲁棒性。

Result: 模型在不同专业领域表现出显著的透明度不一致性：金融顾问角色初始披露率为30.8%，而神经外科医生角色仅为3.5%。披露率范围从2.8%到73.6%，模型身份比参数数量更能预测行为表现。推理优化在某些模型中会主动抑制自我透明度。

Conclusion: 透明度反映的是训练因素而非模型规模，组织不能假设安全属性会自然转移到部署环境中，需要刻意设计行为并进行实证验证。
...
### [138] [From Prediction to Foresight: The Role of AI in Designing Responsible Futures](https://arxiv.org/abs/2511.21570)
*Maria Perez-Ortiz*
...
Method: 通过建立负责任计算前瞻的基本原则，结合人工智能、模拟和情景分析，增强政策制定者应对不确定性、评估风险和制定可持续战略的能力。
