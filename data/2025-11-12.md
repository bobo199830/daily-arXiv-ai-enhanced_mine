...
TL;DR: 提出了一种名为双矩方法(MoDM)的新数据融合框架，通过结合均匀和非均匀两种不同取向分布下的投影图像二阶矩，能够从二阶统计量中准确重建分子结构。


<details>
  <summary>Details</summary>
Motivation: 利用不同实验条件下收集的多个数据集，通过数据融合提升冷冻电镜中分子结构重建的质量，证明数据集多样性在计算成像任务中的重要性。

Method: 开发了基于凸松弛的算法，仅使用二阶统计量(二阶矩)来重建分子结构，该方法从均匀和非均匀两种不同取向分布下的投影图像中提取信息。

Result: 证明这些矩在一般情况下能够唯一确定底层结构(除全局旋转和反射外)，算法实现了准确的重建效果。
...
<details>
  <summary>Details</summary>
Motivation: 传统图像传感器动态范围有限，模数相机通过折叠入射辐照度来解决此问题，但需要专门的解包算法。现有HDR方法不适用于模数恢复，且深度学习技术在该领域进展缓慢。

Method: 提出选择性时空视觉Transformer（SSViT），采用令牌选择策略提高效率并聚焦关键区域，利用Transformer捕获全局依赖和时空关系来解决折叠视频帧。

Result: 实验证实SSViT从8位折叠视频产生高质量重建，在模数视频恢复中达到最先进性能。

Conclusion: SSViT是首个用于模数视频重建的深度学习框架，通过新颖的Transformer架构和令牌选择策略，成功解决了模数恢复问题并取得优异性能。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然成功，但已知存在产生不连贯或不现实样本的幻觉问题。先前工作将此归因于模式插值和分数平滑现象，但缺乏在采样过程中防止其生成的方法。

Method: 在推理过程中对分数函数进行后验调整，利用拉普拉斯算子来减少模式插值幻觉。为高维情况推导了使用Hutchinson迹估计器的有限差分变体的高效拉普拉斯近似方法。

Result: 该方法在玩具1D/2D分布和高维图像数据集上显著减少了幻觉样本的比例。

Conclusion: 提出的拉普拉斯校正能有效减少扩散模型中的模式插值幻觉，并探索了拉普拉斯与分数不确定性之间的关系。
...
TL;DR: ASAG是一种新的扩散模型引导方法，通过最优传输理论和Sinkhorn算法重新解释注意力机制，在自注意力层中注入对抗性成本来降低查询和键之间的像素相似性，从而改善样本质量。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散模型引导方法如CFG依赖于启发式扰动函数，缺乏理论基础且需要手动设计失真。需要一种更原则性的方法来改进注意力机制。

Method: 提出ASAG方法，从最优传输角度重新解释注意力分数，通过Sinkhorn算法故意破坏传输成本，在自注意力层中注入对抗性成本来减少查询和键之间的像素相似性。

Result: ASAG在文本到图像扩散中表现出持续改进，增强了IP-Adapter和ControlNet等下游应用的可控性和保真度。

Conclusion: ASAG是一种轻量级、即插即用的方法，无需模型重新训练即可提高可靠性，为扩散模型提供了一种原则性的注意力引导机制。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有面部替换方法在实时性能和视觉质量方面的限制，推动该技术在娱乐、教育、通信等领域的实际应用部署。

Method: 开发LiveNeRF框架，专注于实现实时面部替换，同时保持视觉质量。

Result: 成功实现33FPS的实时性能，具有优越的视觉质量，能够实际应用于直播、视频会议和交互媒体。

Conclusion: 面部替换技术具有广泛的应用前景，但需要负责任地部署，包括用户同意验证和检测系统集成，以确保积极的社会影响并降低风险。
...
TL;DR: TrackStudio是一个无需标记的运动追踪工具包，结合现有开源工具提供模块化、基于GUI的完整流程，无需编程技能即可实现自动2D/3D追踪、校准、特征提取等功能。


<details>
  <summary>Details</summary>
Motivation: 现有运动追踪工具需要专业技术知识，缺乏为非专家用户提供的易用、集成化解决方案。

Method: 将成熟的开源工具整合到单一模块化GUI流程中，提供自动2D/3D追踪、校准、预处理、特征提取和可视化功能。

Result: 在76名参与者的测试中，平均帧间相关性超过0.98，手部追踪的平均三角测量误差低于13.6mm，表现稳定一致。
...
Method: 提出统一方法，结合显著性感知和粒度感知建模以及区域级不确定性建模。利用模态特定偏差识别显著特征，不依赖脆弱的跨模态注意力；将区域特征表示为高斯分布的混合以捕捉细粒度不确定性。

Result: 在Flickr30K和MS-COCO上的广泛实验表明，该方法在各种骨干架构上实现了最先进的性能，显著增强了细粒度图像-文本对齐的鲁棒性和可解释性。

Conclusion: 该方法通过显著性感知和不确定性建模有效解决了细粒度图像-文本对齐的关键挑战，为多模态学习应用提供了更强大的基础。
...
TL;DR: UltraGS是一个针对超声成像优化的高斯泼溅框架，通过深度感知高斯泼溅策略和超声特定的轻量级渲染函数SH-DARS，实现了高质量的超声图像新视角合成，在PSNR、SSIM和MSE指标上达到最先进水平，并能实时合成（64.69 fps）。


<details>
  <summary>Details</summary>
Motivation: 超声成像作为无创临床诊断的基石，其有限视野限制了新视角合成。为解决这一问题，需要开发专门针对超声成像特性的新视角合成方法。

Method: 1) 深度感知高斯泼溅策略：为每个高斯分配可学习的视野，实现精确深度预测和结构表示；2) SH-DARS轻量级渲染函数：结合低阶球谐函数和超声特定波物理（深度衰减、反射、散射），准确建模组织强度；3) 贡献临床超声检查数据集。

Result: 在三个数据集上的广泛实验表明，UltraGS在PSNR（最高29.55）、SSIM（最高0.89）和MSE（低至0.002）方面达到最先进水平，同时实现64.69 fps的实时合成。

Conclusion: UltraGS通过专门针对超声成像特性的优化设计，成功解决了超声图像新视角合成的挑战，在保持高质量的同时实现了实时性能，为临床诊断提供了有力工具。
...
Conclusion: VectorSynth能够实现像素级精确的卫星图像合成，支持交互式工作流，为快速假设模拟、空间编辑和地图信息内容生成提供了有效工具。
...
### [10] [Compression then Matching: An Efficient Pre-training Paradigm for Multimodal Embedding](https://arxiv.org/abs/2511.08480)
*Da Li,Yuxiao Luo,Keping Bi,Jiafeng Guo,Wei Yuan,Biao Yang,Yan Wang,Fan Yang,Tingting Gao,Guorui Zhou*

Main category: cs.CV

TL;DR: CoMa是一种压缩预训练方法，作为对比学习的预热阶段，可将视觉语言模型转化为具有竞争力的嵌入模型，在MMEB基准上实现了新的最先进结果。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型通过获取可转移的语义嵌入来增强多模态表示学习，但有效的嵌入需要同时保留输入的完整语义内容并突出下游任务的判别性特征。作者认为这两个目标可以解耦：对输入的全面理解有助于嵌入模型通过对比学习在下游任务中获得更好性能。

Method: 提出CoMa压缩预训练阶段作为对比学习的预热阶段，仅使用少量预训练数据将视觉语言模型转化为竞争性嵌入模型。

Result: 实验表明，CoMa在MMEB基准上实现了与同类规模视觉语言模型相比的最新最先进结果，在效率和效果上都实现了优化。

Conclusion: CoMa方法通过解耦语义保留和判别性特征学习目标，仅需少量预训练数据即可将视觉语言模型有效转化为高质量的嵌入模型，在效率和性能上均表现出色。
...
TL;DR: 该论文提出了PGCD和DAPD方法来解决类增量医学图像分割中的知识保留问题，通过原型引导的校准蒸馏和双对齐原型蒸馏来增强旧类知识的保留效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在两个问题：一是采用一刀切策略，对所有空间区域和特征通道同等对待，阻碍了准确旧知识的保留；二是仅关注旧类全局原型对齐而忽视新数据中的局部表示，导致知识退化。

Method: 提出PGCD方法利用原型-特征相似度来校准不同空间区域的类特定蒸馏强度，有效强化可靠的旧知识并抑制误导信息；同时提出DAPD方法将当前模型提取的旧类局部原型与全局原型和局部原型进行对齐。

Result: 在两个广泛使用的多器官分割基准上的综合评估表明，该方法优于最先进的方法，突显了其鲁棒性和泛化能力。

Conclusion: 该方法通过原型引导的校准蒸馏和双对齐原型蒸馏，有效解决了类增量医学图像分割中的知识保留问题，在保持旧类知识的同时学习新类，取得了优越的性能表现。
...
TL;DR: 本文揭示了扩散模型中初始噪声的可分析语义模式，提出了基于信息论的语义擦除-注入两步法来调制噪声，实现训练无关的生成优化。


<details>
  <summary>Details</summary>
Motivation: 传统观点认为扩散模型的初始噪声是随机元素，仅贡献生成多样性。本文发现噪声中蕴含可分析的语义模式，这为优化生成过程提供了新视角。

Method: 提出训练无关的通用两步法：1) 基于信息论的语义擦除，去除噪声中不需要的语义；2) 利用扩散模型生成过程与语义注入的等价性，向清理后的噪声注入语义。

Result: 实验证明该方法在基于DiT和UNet架构的各种文本驱动内容生成模型中均有效，提供了生成一致性的通用工具。

Conclusion: 噪声不仅包含丰富语义信息，还能通过简单操作进行语义调制，这为扩散模型生成优化提供了新颖且有效的途径。
...
<details>
  <summary>Details</summary>
Motivation: 现实场景中多标签数据集普遍存在标签噪声，严重影响检索性能，且现有方法忽略多标签数据的部分语义重叠问题。
...
Conclusion: SCBCH框架能有效处理标签噪声和多标签语义重叠问题，提高跨模态检索的鲁棒性和泛化能力。
...
TL;DR: DCDNet通过特征解耦和动态融合解决跨域小样本分割中的特征纠缠问题，在四个数据集上实现了最先进的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有的DPO方法在扩散模型中存在两个主要限制：1）整个模型微调导致高计算成本；2）对参考模型质量敏感，引入不稳定性和偏差。

Method: 使用轻量级可训练的偏好分类器直接建模样本间的相对偏好，将偏好学习与生成模型解耦，无需整个模型微调和参考模型依赖。

Result: PC-Diffusion在偏好一致性方面与DPO相当，同时显著降低训练成本，实现高效稳定的偏好引导生成。

Conclusion: PC-Diffusion通过偏好分类器框架有效解决了DPO方法的局限性，提供了理论保证和实证优势。
...
Method: 提出动态实例和轮廓一致性对比学习(DI3CL)预训练框架，包含动态实例模块增强全局上下文感知，轮廓一致性模块引导模型关注SAR土地覆盖对象的几何轮廓。构建了包含460,532张SAR图像的大规模SARSense数据集。

Result: 在SAR土地覆盖制图、水体检测和道路提取等多种任务上的实验表明，DI3CL方法优于现有方法。

Conclusion: DI3CL为SAR土地覆盖分类提供了一个强大的基础模型，能够加速各种下游模型的开发和部署，具有优异的泛化能力。
...
TL;DR: 提出Q-Scorer框架解决多模态大语言模型在图像质量评估任务中离散token输出与连续质量分数不匹配的问题，通过轻量级回归模块和IQA专用分数token提升性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在图像质量评估任务中存在离散token输出与连续质量分数要求不匹配的问题，导致性能受限，且传统转换方法存在转换误差和语义混淆问题。

Method: 提出Q-Scorer框架，包含轻量级回归模块和IQA专用分数token，集成到MLLM流程中，通过理论分析指导设计。

Result: 在多个IQA基准测试中达到最先进性能，在混合数据集上泛化良好，与其他方法结合时性能进一步提升。

Conclusion: Q-Scorer有效解决了MLLM在IQA任务中的离散-连续不匹配问题，显著提升了评估性能并保持了模型原有能力。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有无需训练方法在实际部署中准确性和效率不足的问题，利用预训练LLMs的推理能力开发更灵活、泛化性强的3D场景理解方法。

Method: 引入层次化平面增强场景图支持开放词汇，采用主导平面结构作为空间锚点；设计任务自适应子图提取方法动态过滤查询无关信息。

Result: 在Space3D-Bench上相比ConceptGraphs实现28.7%的EM@1提升和78.2%的速度提升；在ScanQA上达到与训练方法相当的性能，真实世界实验证实其鲁棒性和泛化能力。

Conclusion: Sparse3DPR通过结合预训练LLMs推理能力和创新的场景图设计，为无需训练的3D场景理解提供了高效准确的解决方案，具有实际部署价值。
...
TL;DR: 合成相关扩散成像（CDI^s）作为标准扩散成像协议的增强方法，可显著提升前列腺癌病灶分割性能，在94%的评估配置中可靠增强或保持分割性能，最高实现72.5%的相对改进。


<details>
  <summary>Details</summary>
Motivation: 当前深度学习在前列腺癌病灶分割方面性能有限（Dice分数≤0.32），需要探索新的成像技术来提升分割准确性。

Method: 使用200名患者的共配准CDI^s、DWI和ADC序列，在六种最先进的分割架构上进行全面评估。

Result: CDI^s集成在94%的配置中可靠增强或保持分割性能，个别架构实现高达72.5%的统计显著相对改进。CDI^s + DWI是最安全的增强路径，在50%的架构中实现显著改进且无性能下降。
...
<details>
  <summary>Details</summary>
Motivation: 当前3D场景中的人体运动合成方法主要关注场景结构而忽略语义理解，限制了运动合成的质量和准确性。

Method: 提出统一的场景语义占用(SSO)表示，采用双向三平面分解获得紧凑SSO，通过CLIP编码将场景语义映射到统一特征空间，结合运动方向指令进行帧级场景查询控制。

Result: 在ShapeNet家具、PROX和Replica数据集上的大量实验表明，该方法在杂乱场景中具有领先性能，验证了其有效性和泛化能力。
...
TL;DR: MonoCLUE通过结合局部聚类和广义场景记忆的视觉特征来增强单目3D检测，在遮挡和有限可见性条件下实现鲁棒性能，在KITTI基准测试中达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 单目3D物体检测存在深度模糊和视野受限的问题，导致几何线索不足以及在遮挡或截断场景中精度降低。现有方法虽然加入了深度信息，但忽略了视觉线索对鲁棒识别的重要性。

Method: 1. 对视觉特征进行K-means聚类以捕获不同的物体级外观部分（如引擎盖、车顶），改善部分可见物体的检测；2. 构建广义场景记忆，通过跨图像聚合聚类特征提供一致的表示；3. 将局部聚类特征和广义场景记忆集成到物体查询中，引导注意力到信息丰富的区域。

Result: 该方法在KITTI基准测试中实现了最先进的性能，特别是在遮挡和有限可见性条件下表现出色。

Conclusion: MonoCLUE通过统一的局部聚类和广义场景记忆策略，能够在遮挡和有限可见性条件下实现鲁棒的单目3D检测。
...
<details>
  <summary>Details</summary>
Motivation: 受大型语言模型跨领域泛化能力的启发，作者旨在解决扩散模型在视觉任务中受限于"单任务-单模型"范式的问题，提高模型在多任务场景中的通用性和可扩展性。

Method: 将过程制定为从图像块标记到任务特定表示的通用流匹配问题，利用强大的自监督基础模型作为锚点，引入多尺度循环任务嵌入机制，学习通用速度场来桥接异构任务间的差距。

Result: 在分类、检测、分割、深度估计和图像-文本检索等任务上的广泛实验表明，该模型在零样本和微调设置下均实现了有竞争力的性能，超越了先前的一般模型和多个专业模型。
...
TL;DR: 提出了一种分层自回归的草图生成方法，可以在生成过程中灵活控制笔画级特征，支持在任意时刻进行笔画编辑。


<details>
  <summary>Details</summary>
Motivation: 现有方法需要在生成前一次性提供所有笔画条件，无法在生成过程中进行动态调整。为了更灵活地控制草图绘制过程，需要支持实时笔画级操控。

Method: 采用三层分层自回归生成过程：1)预测笔画嵌入表示要绘制的笔画；2)在画布上锚定笔画位置；3)将嵌入转换为绘制动作序列。整个过程自回归进行，考虑已生成笔画及其位置来预测当前笔画。

Result: 实现了在草图生成过程中任意时刻对笔画级特征的灵活操控，通过调整暴露的可编辑笔画嵌入来动态控制绘制过程。

Conclusion: 提出的分层自回归方法为草图生成提供了更灵活的笔画级控制能力，支持生成过程中的实时编辑和调整。
...
TL;DR: 本文对基于幂律变换的文本极性检测方法进行了理论分析，解释了为什么暗文本在亮背景上时类间方差会增大，而亮文本在暗背景上时类间方差会减小。


<details>
  <summary>Details</summary>
Motivation: 文本极性检测是图像二值化的重要预处理任务，现有基于幂律变换的直观方法虽然有效但缺乏理论支撑，本文旨在提供该现象的理论分析。

Method: 对幂律变换后的图像进行理论分析，研究文本和背景作为两个类别时，类间方差随文本极性的变化规律。

Result: 理论分析证实了经验观察：暗文本在亮背景上时类间方差增大，亮文本在暗背景上时类间方差减小。

Conclusion: 本文为基于幂律变换的文本极性检测方法提供了理论依据，增强了该方法的可靠性和理解深度。
...
TL;DR: 该论文提出了首个大规模细粒度水下分割数据集AquaOV255和首个水下开放词汇分割基准UOVSBench，并开发了无需训练的地球到海洋迁移框架Earth2Ocean，将陆地视觉语言模型直接应用于水下分割任务。


<details>
  <summary>Details</summary>
Motivation: 现有分割数据集和模型主要局限于陆地场景，缺乏针对水下环境的大规模细粒度分割能力，这限制了海洋生物多样性监测和生态评估的准确性。

Method: 提出了Earth2Ocean框架，包含两个核心组件：几何引导视觉掩码生成器（GMG）通过自相似几何先验优化视觉特征以感知局部结构；类别-视觉语义对齐（CSA）模块通过多模态大语言模型推理和场景感知模板构建来增强文本嵌入。

Result: 在UOVSBench基准上的广泛实验表明，Earth2Ocean实现了显著的性能提升，同时保持高效的推理速度。

Conclusion: 该工作填补了水下开放词汇分割的空白，提供了一种无需额外水下训练的有效迁移方法，为海洋生态监测提供了重要工具。
...
TL;DR: 提出HD²-SSC框架，通过高维语义解耦模块扩展2D图像特征，以及高密度占用精炼模块提升语义密度，解决相机3D语义场景补全中的维度差距和密度差距问题。


<details>
  <summary>Details</summary>
Motivation: 现有SSC方法存在输入输出维度差距和标注-现实密度差距，导致2D平面视图难以准确预测真实世界的3D立体密集占用。

Method: 设计高维语义解耦模块扩展2D特征，解耦粗粒度像素语义；采用高密度占用精炼模块的'检测-精炼'架构，利用上下文几何和语义结构完成缺失体素和修正错误体素。
...
TL;DR: 本文提出了一种基于计算机视觉的新型图像路径规划算法，并与A*和PRM算法进行了比较分析。该方法使用无人机生成的地形视差图，结合边缘、线条、角点检测和立体深度重建技术来定义轨迹候选路径点。


<details>
  <summary>Details</summary>
Motivation: 地形深度对路径安全有重要影响，但二维图像无法区分陨石坑和山丘等特征，因此需要开发能够利用深度信息的路径规划方法。

Method: 使用无人机生成地形视差图，应用边缘、线条、角点检测和立体深度重建技术处理图像，通过ArUco标记姿态估计和圆检测技术自动识别起点和终点。

Result: 在V-REP仿真程序和实验室物理设置中进行测试，与A*和PRM算法比较，结果令人满意，证明了所提算法的有效性。

Conclusion: 提出的基于计算机视觉的路径规划算法在考虑地形深度信息方面表现出色，为复杂地形下的路径规划提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决医学影像中深度学习模型的隐私问题和数据异质性挑战，联邦学习虽提供可行方案，但资源成本高且数据异构性影响性能，特别是在使用视觉语言模型时。

Method: 使用掩码特征适应模块作为通信模块减少通信负载，冻结CLIP编码器降低计算开销，设计掩码MLP作为本地分类器，采用自适应KL散度蒸馏实现模块间相互学习，结合模型压缩传输参数。

Result: 在四个公开医学数据集上验证，性能显著提升（如ISIC2019上比次优基线高8%），资源成本合理（比FedAVG快120倍）。

Conclusion: FedMedCLIP在医学图像分类中实现了可行性能与合理资源成本的平衡，为隐私保护的联邦学习提供了有效解决方案。
...
TL;DR: DiffRegCD是一个统一的密集配准和变化检测框架，通过将对应关系估计重新表述为高斯平滑分类任务，利用预训练扩散模型的多尺度特征，在多种数据集上优于现有基线方法。
...
Method: 将对应关系估计重新表述为高斯平滑分类任务实现亚像素精度；利用预训练去噪扩散模型的冻结多尺度特征；通过受控仿射扰动生成配准和变化检测的配对真值数据。

Result: 在航空（LEVIR-CD、DSIFN-CD、WHU-CD、SYSU-CD）和地面（VL-CMU-CD）数据集上的广泛实验表明，DiffRegCD始终优于近期基线方法，在广泛的时间和几何变化下保持可靠。

Conclusion: 扩散特征和基于分类的对应关系估计为统一变化检测提供了强大基础，DiffRegCD在多种数据集上表现出色且鲁棒性强。
...
TL;DR: 本文提出ISExplore策略，通过自动选择信息丰富的5秒参考视频片段来加速说话人脸生成，相比传统方法处理数分钟视频，能提升5倍以上的数据处理和训练速度，同时保持高质量输出。


<details>
  <summary>Details</summary>
Motivation: 现有基于NeRF或3DGS的说话人脸生成方法需要处理数分钟参考视频，计算负担重且耗时数小时，限制了实际应用价值。研究发现视频信息质量比长度更重要，短而信息丰富的片段能达到甚至优于完整视频的效果。
...
Conclusion: 视频信息质量比长度更重要，ISExplore策略通过智能选择信息丰富的短片段，显著提升了说话人脸生成的效率和实用性。
...
TL;DR: 提出了一种基于多模态原型的多实例学习方法，通过平衡信息压缩方案促进双向交互，利用冻结LLM生成任务特异性病理实体描述作为文本原型，视觉分支学习实例级原型以减少对冗余数据的依赖，使用基于相似度度量的立体最优传输算法进行融合。


<details>
  <summary>Details</summary>
Motivation: 解决计算病理学中全切片图像计算成本高的问题，以及病理任务通常只提供袋级标签而LLM生成的实例级描述存在偏差的挑战，需要构建任务特异性病理实体原型来学习可泛化特征并增强模型可解释性。

Method: 多模态原型多实例学习，利用冻结LLM生成任务特异性病理实体描述作为文本原型，视觉分支学习实例级原型，采用立体最优传输算法进行跨模态融合。

Result: 在三个不同癌症数据集上进行少样本分类和可解释性实验，结果表明所提方法具有优越的泛化能力。

Conclusion: 提出的多模态原型多实例学习方法通过双向交互和平衡信息压缩，有效提升了病理图像分析的泛化性能和可解释性。
...
TL;DR: 提出了一个纯Mamba架构的行人重识别框架ReIDMamba，通过多粒度特征提取器和排序感知三元组正则化，有效解决了Transformer在ReID中的可扩展性问题，在五个基准数据集上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 解决Transformer在行人重识别中的可扩展性问题（二次内存和计算复杂度），同时利用Mamba架构提取鲁棒的判别性特征。

Method: 设计了基于Mamba的强基线，引入多个类别token；开发了多粒度特征提取器模块（多分支架构和类别token融合）和排序感知三元组正则化（减少多分支特征冗余）。

Result: ReIDMamba参数仅为TransReID的三分之一，GPU内存使用更低，推理吞吐量更快，在五个行人重识别基准数据集上达到最先进性能。

Conclusion: 这是首个将纯Mamba驱动方法集成到行人重识别研究中的工作，证明了Mamba架构在ReID任务中的有效性和优越性。
...
TL;DR: MMAssist是一种基于多模态辅助的3D无监督域自适应方法，通过图像和文本特征作为桥梁来对齐源域和目标域的3D特征，提升LiDAR点云3D目标检测性能。


<details>
  <summary>Details</summary>
Motivation: 虽然点云和图像通常同时采集，但在3D无监督域自适应中图像数据的利用被忽视。本文旨在探索多模态数据在3D UDA中的潜力。

Method: 使用预训练视觉骨干提取2D边界框图像特征，采用大型视觉语言模型生成文本描述并提取文本特征，在源域和目标域训练中通过特征对齐和融合实现3D特征增强，同时结合2D检测器生成伪标签。
...
<details>
  <summary>Details</summary>
Motivation: 当前通用放射学报告生成模型在描述罕见但临床重要的病理（如骨折）方面存在不足，需要专门针对骨折病理开发更准确的模型

Method: 使用MAIRA-2和CheXagent的编码器训练骨折特异性视觉语言模型

Result: 专门模型在生成准确骨折描述方面相比通用模型有显著改进，分析了模型输出在骨折类型、位置和年龄方面的表现差异

Conclusion: 开发并公开发布了表现最佳的骨折报告模型，为罕见病理的准确报告研究提供了基础
...
TL;DR: 提出语义引导框架CSF-Net解决大掩码图像修复问题，通过预训练的Amodal Completion模型生成结构感知候选作为语义先验，使用基于transformer的融合框架将候选与上下文特征融合，生成语义引导图像来提升修复质量。


<details>
  <summary>Details</summary>
Motivation: 解决大掩码图像修复中视觉内容缺失和上下文线索有限的问题，需要补偿有限的上下文信息来提升修复质量。

Method: 利用预训练的Amodal Completion模型生成结构感知候选作为语义先验，提出Context-Semantic Fusion Network（CSF-Net）基于transformer的融合框架，将候选与上下文特征融合生成语义引导图像。

Result: 在Places365和COCOA数据集上的广泛实验表明，CSF-Net有效减少物体幻觉，同时增强视觉真实性和语义对齐，能无缝集成到现有修复模型中且无需架构更改。

Conclusion: CSF-Net通过语义引导有效提升大掩码图像修复的质量，促进结构准确性和语义一致性，在各种掩码条件下都能持续提升性能。
...
TL;DR: 本文提出了一种基于YOLOv8n的优化低功耗边缘AI系统，用于在STM32U575ZI微控制器上实现杂草检测，通过模型压缩技术实现51.8mJ/推理的低能耗实时检测。


<details>
  <summary>Details</summary>
Motivation: 传统化学除草方法存在环境污染和抗药性杂草问题，而现有精准除草方案依赖高功耗计算平台，难以在资源受限的农业环境中部署。

Method: 采用YOLOv8n目标检测器，结合结构化剪枝、整数量化和输入图像分辨率缩放等压缩技术，在STM32U575ZI微控制器上部署。

Result: 在包含74种植物的CropAndWeed数据集上训练评估，实现了检测精度与效率的平衡，单次推理能耗仅51.8mJ。

Conclusion: 该系统支持实时原位杂草检测，为电力受限的农业环境提供了可扩展部署方案，是精准农业的可行解决方案。
...
TL;DR: SharpV是一种高效的自适应视觉令牌和KV缓存剪枝方法，通过动态调整剪枝比和自校准机制，解决了VideoLLMs中的二次计算复杂度和KV缓存扩展问题。
...
Method: SharpV采用自适应剪枝机制，基于时空信息动态调整剪枝比例，并通过自校准方式剪枝退化的视觉特征。这是一个无需访问注意力分数的两阶段剪枝框架。
...
TL;DR: EAGLE框架通过外观和几何感知记忆机制，在自我中心视觉中实现统一的2D-3D视觉查询定位，显著提升检索精度和3D空间投影能力。


<details>
  <summary>Details</summary>
Motivation: 解决自我中心视觉查询定位中的挑战，包括相机运动、视角变化和外观变化，需要更鲁棒的方法来统一处理2D和3D任务。

Method: 提出EAGLE框架，整合外观感知元学习记忆(AMM)指导的分割和几何感知定位记忆(GLM)驱动的跟踪，通过结构化记忆库存储高置信度检索样本，并结合视觉几何接地Transformer(VGGT)实现2D-3D任务统一。

Result: 在Ego4D-VQ基准测试中达到最先进的性能，实现了精确的轮廓描绘和鲁棒的空间区分能力。

Conclusion: EAGLE通过记忆整合机制有效支持目标外观变化的长期和短期建模，实现了自我中心视觉中2D-3D视觉查询定位的统一和优化。
...
TL;DR: AdvRoad是一种生成道路风格对抗性海报的方法，通过在道路上放置自然外观的对抗性图案，使3D物体检测器感知到不存在的物体，同时避免引起人类注意。


<details>
  <summary>Details</summary>
Motivation: 当前基于视觉的3D物体检测系统容易受到对抗性攻击，但现有方法生成的对抗性海报外观不自然且容易被发现和防御，因此需要开发更隐蔽的攻击方法。

Method: 采用两阶段方法：道路风格对抗性生成和场景关联适应，确保对抗性海报具有自然外观的同时最大化攻击效果。

Result: 实验表明AdvRoad在不同检测器、场景和欺骗位置上都表现出良好的泛化能力，物理攻击进一步证明了其在真实环境中的实际威胁。

Conclusion: AdvRoad展示了在自动驾驶场景中生成自然外观对抗性海报的可行性，揭示了视觉3D检测系统面临的安全风险。
...
TL;DR: 本文提出Cascade HQP-DETR方法，通过高质量合成数据生成、基于高质量提议的查询编码和级联去噪算法，解决了合成数据训练对象检测模型时的收敛慢、过拟合和噪声敏感问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于合成图像的监督对象检测方法面临三个主要问题：合成数据集质量差、DETR检测器收敛慢且容易过拟合到合成模式、均匀去噪压力导致模型过拟合伪标签噪声。

Method: 1) 使用LLaMA-3、Flux和Grounding DINO构建高质量数据管道生成FluxVOC和FluxCOCO数据集；2) 提出高质量提议引导的查询编码，利用SAM生成的提议和RoI池化特征初始化对象查询；3) 设计级联去噪算法，通过逐层增加IoU阈值动态调整训练权重。

Result: 仅使用FluxVOC训练12个epoch，在PASCAL VOC 2007上达到61.04% mAP@0.5的SOTA性能，超越了强基线方法，其竞争性的真实数据性能证实了架构的通用适用性。

Conclusion: Cascade HQP-DETR通过高质量合成数据生成、图像特定先验引导的查询初始化和动态去噪策略，有效解决了合成数据训练中的关键挑战，实现了优异的跨域泛化性能。
...
TL;DR: 本文提出了3DGS-QA，首个针对3D高斯泼溅渲染技术的主观质量评估数据集，并开发了基于3D高斯原语的无参考质量预测模型。


<details>
  <summary>Details</summary>
Motivation: 随着3D高斯泼溅技术的快速发展，虽然算法性能和视觉保真度得到重视，但其渲染内容在不同重建条件下的感知质量尚未得到系统研究。实际应用中，视点稀疏性、有限训练迭代、点云降采样、噪声和颜色失真等因素会显著降低视觉质量，但它们的感知影响缺乏系统研究。

Method: 构建了包含15种对象类型、225个退化重建的3DGS-QA数据集，并提出了直接在原生3D高斯原语上操作的无参考质量预测模型。该模型从高斯表示中提取空间和光度线索，以结构感知方式估计感知质量。

Result: 实验结果表明，该方法在3DGS内容评估中始终实现卓越性能，证明了其鲁棒性和有效性。同时基准测试了传统和基于学习的质量评估方法。

Conclusion: 3DGS-QA数据集和提出的质量预测模型填补了3D高斯泼溅技术质量评估的研究空白，为未来3DGS质量评估研究提供了基础。数据集和代码已公开可用。
...
TL;DR: GECO2是一个端到端的少样本计数和检测方法，通过新的密集查询表示在不同尺度上逐步聚合特定示例的特征信息，解决了多样化尺寸物体和密集小物体检测的问题，在计数和检测准确率上比现有最优方法提升10%，同时运行速度快3倍且GPU内存占用更小。


<details>
  <summary>Details</summary>
Motivation: 现有的少样本计数器在处理包含多样化尺寸物体和密集小物体区域的图像时存在困难，因为它们通常采用临时解决方案（如上采样和分块处理）来应对计算和内存需求。

Method: 提出了一种新的密集查询表示方法，在不同尺度上逐步聚合特定示例的特征信息，生成高分辨率密集查询，从而能够检测大物体和小物体。

Result: GECO2在计数和检测准确率上比现有最优方法提升10%，同时运行速度快3倍，GPU内存占用更小。

Conclusion: GECO2通过端到端的密集查询表示方法有效解决了物体尺度问题，在少样本计数和检测任务中实现了显著的性能提升和效率改进。
...
<details>
  <summary>Details</summary>
Motivation: 传统雷达心跳感知方法因噪声而性能下降，基于学习的方法需要昂贵的标记信号进行监督训练。为克服这些限制，需要开发无监督方法。

Method: 使用雷达距离矩阵中的心跳范围和噪声范围分别构建正负样本，提出噪声对比三元组损失，仅利用正样本、负样本和传统雷达方法生成的伪标签信号，避免依赖地面真实信号。设计了具有自适应噪声感知标签选择的伪标签增强方法。

Result: 在Equipleth数据集和自收集雷达数据集上的广泛实验表明，该无监督方法达到了与最先进有监督方法相当的性能。

Conclusion: Radar-APLANC框架成功实现了无需地面真实生理信号的无监督雷达心跳感知，在噪声鲁棒性方面表现优异，为雷达生理监测提供了新的解决方案。
...
TL;DR: 本文研究发现Stable Diffusion模型的语义理解能力主要来自CLIP文本编码器，而非反向扩散过程。通过回归层预测语义属性，发现CLIP在语义表示方面表现最佳，而扩散过程主要充当视觉解码器角色。


<details>
  <summary>Details</summary>
Motivation: 研究潜在扩散模型（如Stable Diffusion）在文本到图像生成过程中是否真正理解图像语义，以及这种语义理解来自模型的哪个部分。

Method: 使用简单的回归层对Stable Diffusion进行探测，预测物体的语义属性，并将预测结果与人类标注进行比较。特别关注CLIP文本编码器和反向扩散过程在语义表示中的作用。

Result: 发现语义理解能力主要归因于CLIP文本编码器，而非反向扩散过程。不同语义属性的解码准确度差异显著，且在反向扩散过程中属性区分度逐渐降低。
...
TL;DR: 提出了一个名为Beyond the Pixels的分层评估框架，用于评估生成模型中的身份保持能力。该框架通过结构化推理将身份评估分解为特征级转换，减少幻觉并提高一致性。


<details>
  <summary>Details</summary>
Motivation: 现有评估指标依赖全局嵌入或粗略的VLM提示，无法捕捉细粒度身份变化且诊断洞察有限。需要更精确的方法来评估生成模型的身份保持能力。

Method: 分层分解主体为(类型,风格)->属性->特征决策树，并提示具体转换而非抽象相似度分数。这种方法将VLM分析基于可验证的视觉证据。

Result: 在四个最先进的生成模型上验证了该框架，显示与人类判断在测量身份一致性方面有很强的一致性。同时引入了包含1,078个图像-提示对的新基准，涵盖多样主体类型和转换轴。

Conclusion: Beyond the Pixels框架提供了一种更可靠和诊断性的方法来评估生成模型的身份保持能力，特别是在处理细粒度身份变化方面表现出色。
...
TL;DR: 提出了StableMorph方法，使用基于扩散的图像合成技术生成高质量、无伪影的融合人脸图像，解决了现有方法生成图像模糊、有伪影的问题，为形态攻击检测系统提供了更真实的测试数据。


<details>
  <summary>Details</summary>
Motivation: 现有融合图像生成方法产生的图像质量差、容易检测，无法代表最危险的攻击场景，需要高质量的融合图像来开发和评估有效的形态攻击检测系统。

Method: 使用现代基于扩散的图像合成技术生成融合人脸图像，能够生成全头部图像，避免常见视觉缺陷，并提供对视觉属性的精确控制。

Result: StableMorph生成的图像质量可与真实人脸图像相媲美甚至更优，同时保持欺骗人脸识别系统的强大能力，对现有检测解决方案构成更大挑战。
...
TL;DR: 本文介绍了一个专注于尼龙面罩（NFMs）的新型演示攻击数据集，使用iPhone 11 Pro收集了3,760个真实样本和51,281个NFM攻击样本，评估了五种最先进的PAD方法在未见攻击条件下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 人脸识别系统在广泛应用中仍易受演示攻击（PAs）影响，尼龙面罩因其弹性结构和逼真外观能够紧密模拟受害者面部几何形状，构成严重威胁。

Method: 创建专注于尼龙面罩的新型数据集，使用iPhone 11 Pro收集真实样本和NFM攻击样本，涵盖人类和人体模型的四种不同演示场景，并基准测试五种最先进的PAD方法。

Result: 结果显示不同方法之间存在显著的性能差异，突显了NFMs带来的挑战，并强调了开发能够有效泛化到新兴欺骗威胁的PAD技术的重要性。

Conclusion: 尼龙面罩对现有PAD方法构成严重挑战，需要开发更具泛化能力的技术来应对此类新兴欺骗威胁。
...
<details>
  <summary>Details</summary>
Motivation: 污水处理厂泡沫形成会降低处理效率并增加成本，但传统机器学习方法需要大量标注数据且存在数据隐私问题，不同污水处理厂之间数据共享困难。

Method: 采用联邦学习范式结合SAM2图像分割模型，通过Flower框架在分布式客户端进行模型微调，中央雾服务器聚合模型权重而不访问私有数据。

Result: 该框架加速了训练收敛，提高了分割性能，即使在有限本地数据集下也能良好工作，并在真实污水处理厂图像、合成泡沫数据集和公开数据集上进行了验证。

Conclusion: 该研究为污水处理厂泡沫自动跟踪提供了实用、可扩展且注重隐私的解决方案，展示了将大规模基础模型集成到联邦学习系统中解决现实工业挑战的巨大潜力。
...
Method: 构建PEOD数据集，包含130+时空对齐序列和340k手动边界框，其中57%数据在低光、过曝和高速运动条件下采集。在三种输入配置(事件、RGB、事件-RGB融合)上对14种方法进行基准测试。
...
<details>
  <summary>Details</summary>
Motivation: 多模态学习面临手动标注成本高昂的问题，而无监督领域自适应在单模态设置中已被广泛研究，但在多模态设置中探索较少。本文旨在解决异构多模态领域自适应中不同模态领域偏移变化的主要挑战。

Method: 首先引入信息瓶颈方法独立学习每个模态的表示，然后在表示空间中使用相关对齐匹配源域和目标域。将问题表述为多目标任务，寻求帕累托最优解，通过简化问题为二次规划问题并进一步近似得到闭式解，实现高效的模态平衡多模态领域自适应算法。

Result: 广泛的实证结果表明所提方法的有效性，Boomda在性能上优于竞争方案。

Conclusion: 提出的Boomda方法通过平衡多目标优化有效解决了多模态领域自适应问题，为处理不同模态的领域偏移提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 新视角合成图像的感知质量评估面临挑战，因为缺乏像素对齐的真实参考。全参考方法在不对齐时失效，而无参考方法存在泛化问题。
...
Conclusion: 提出的非对齐参考图像质量评估框架能有效解决新视角合成图像的质量评估问题，具有良好的泛化能力和与人类感知的一致性。
...
TL;DR: 提出LandSegmenter框架，通过弱监督数据、多模态适配器和置信度融合策略，解决遥感领域LULC映射中数据标注成本高和模型泛化性差的问题。
...
Method: 构建LAS弱标注数据集，集成遥感专用适配器进行跨模态特征提取，采用文本编码器增强语义感知，并通过类置信度引导融合策略提升零样本性能。

Result: 在6个精确标注的LULC数据集上验证，零样本和迁移学习实验显示LandSegmenter达到竞争性或更优性能，特别是在零样本设置下。

Conclusion: 该框架有效证明了弱监督在构建任务特定基础模型中的实用性，为遥感LULC映射提供了可扩展的解决方案。
...
Conclusion: TbVAD通过完全在文本域内进行异常检测和解释，实现了语言驱动的可解释视频异常检测框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散方法如IC-Light在生成阴影时仍无法同时实现高外观真实性和几何精度，特别是在复杂人体姿态下。

Method: 使用关键点线性模型(KPLM)建模人体关节，通过阴影三角形算法(STA)计算阴影角度、长度和空间位置。

Result: 在阴影真实度基准测试中达到最先进性能，特别是在复杂人体姿态下，并能有效泛化到多方向重光照场景。

Conclusion: 该方法通过物理合理的阴影投影和几何计算，显著提升了合成图像中阴影的真实感和几何准确性。
...
TL;DR: 提出分布式零样本学习框架DistZSL，通过跨节点属性正则化和全局属性-视觉共识来解决分布式数据异构性问题，提高对未见类别的学习效果。


<details>
  <summary>Details</summary>
Motivation: 现有零样本学习方法通常假设数据集中存储，但在实际应用中数据往往分布在多个节点上且存在异构性，需要开发能够充分利用分布式数据的有效学习框架。

Method: 提出两个关键组件：1）跨节点属性正则化器，强制不同节点间属性特征距离相似；2）全局属性-视觉共识，使不同节点的属性与视觉特征分布的双向映射保持一致。

Result: 大量实验表明，DistZSL在从分布式数据学习方面达到了最先进的性能。

Conclusion: DistZSL框架通过解决分布式数据异构性问题，能够有效学习未见类别的模型，在分布式零样本学习任务中表现出色。
...
Method: 集成预训练视觉语言模型和潜在扩散模型，使用VLM自动提取正常图像描述作为扩散模型的训练条件，学习鲁棒的正常图像特征表示。

Result: 在Real-IAD数据集上像素级PRO指标提升高达25点，在COCO-AD数据集上提升8点，优于现有最先进的基于扩散的方法。

Conclusion: VLMDiff框架通过结合VLM和LDM，无需人工标注或额外训练即可实现有效的多类别视觉异常检测，显著提升了性能表现。
...
TL;DR: WarpGAN提出了一种新的3D GAN反演方法，通过扭曲和修复策略将图像修复技术融入3D GAN反演，以解决现有方法在遮挡区域生成质量差的问题。
...
### [62] [UI2Code$^\text{N}$: A Visual Language Model for Test-Time Scalable Interactive UI-to-Code Generation](https://arxiv.org/abs/2511.08195)
*Zhen Yang,Wenyi Hong,Mingde Xu,Xinyue Fan,Weihan Wang,Jiele Cheng,Xiaotao Gu,Jie Tang*

Main category: cs.CV

TL;DR: UI2Code$^\text{N}$是一个通过分阶段预训练、微调和强化学习训练的视觉语言模型，采用交互式UI到代码范式，统一了UI到代码生成、UI编辑和UI优化三个关键能力，在UI到代码和UI优化基准测试中达到了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 解决当前UI编程自动化方法中的两个关键限制：多模态编码能力发展不足，以及单轮范式很少利用迭代视觉反馈。

Method: 采用交互式UI到代码范式，通过分阶段预训练、微调和强化学习训练视觉语言模型，统一UI到代码生成、UI编辑和UI优化能力，并探索测试时扩展用于交互式生成。

Result: 在UI到代码和UI优化基准测试中，UI2Code$^\text{N}$在开源模型中建立了新的最先进水平，性能与领先的闭源模型（如Claude-4-Sonnet和GPT-5）相当。

Conclusion: 交互式UI到代码范式更好地反映了真实世界的工作流程，提高了可实现的性能上限，UI2Code$^\text{N}$展示了在多模态编码方面的基础性改进。
...
TL;DR: 2Dto3D-SR是一个实时单视角3D超分辨率框架，无需高分辨率RGB引导，通过将3D数据编码为结构化2D表示，直接应用现有2D图像超分辨率架构。
...
Conclusion: 该几何引导管道为现实场景提供了一个简单而可行的实用解决方案，特别是在高分辨率RGB数据不可访问的情况下。
...
TL;DR: 提出了一种基于语义和关系改进多模态对齐与融合的方法，通过提取多级语义特征、分组相关语义以及使用可继承的交叉注意力来优化视觉语言对齐。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言微调方法通常忽略了文本上下文中强调的图像内部语义关系信息，导致多模态对齐性能不佳。

Method: 首先从不同视觉编码器提取多级语义特征以捕捉更多视觉关系线索；然后学习将视觉特征投影到相关语义组；最后使用可继承的交叉注意力融合视觉和文本特征，通过丢弃低相关性的视觉语言特征对来全局去除冗余视觉关系。

Result: 在8个基础模型和两个下游任务（视觉问答和图像描述）上的评估表明，该方法优于所有现有方法。

Conclusion: 该方法通过同时考虑语义和关系，有效提升了多模态对齐和融合的性能。
...
TL;DR: NERVE是一种无需训练的开集词汇语义分割方法，通过整合全局和局部信息，利用稳定扩散模型的自注意力层邻域结构，并引入随机游走进行亲和度精炼，无需传统后处理技术。


<details>
  <summary>Details</summary>
Motivation: 现有无需训练的开集词汇语义分割方法存在计算成本高、注意力图融合效果差、依赖固定大小高斯核等问题，需要更有效的方法来准确分割任意形状的物体。

Method: 提出NERVE方法，集成全局和细粒度局部信息，利用稳定扩散模型自注意力层的邻域结构，使用基于熵的不确定性选择最相关的注意力图，并采用随机游走进行空间扩散过程。

Result: 在7个流行的语义分割基准测试中实现了最先进的零样本分割性能，无需使用CRF或PAMR等传统后处理技术。

Conclusion: NERVE为开集词汇语义分割提供了一个有效的无需训练方法，能够准确分割任意形状的物体，在多个基准测试中表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本驱动多对象图像编辑方法主要遵循定位-编辑范式，关注独立的对象定位和编辑，但忽略了关键的对象间交互。被忽视的注意力纠缠在对象冲突区域阻碍了解耦的多对象编辑，导致对象间编辑泄漏或对象内编辑约束。

Method: 提出'分解-编辑-融合'框架：1) 冲突感知层分解模块，使用注意力感知IoU方案和时间相关区域移除来增强冲突感知和抑制；2) 对象层编辑模块，建立协调的层内文本引导和跨层几何映射，实现解耦的语义和结构修改；3) 透明度引导层融合模块，通过精确的透明度引导学习促进结构连贯的对象间层融合。

Result: 大量实验验证了LayerEdit相对于现有方法的优越性，在复杂多对象场景中展现出前所未有的对象内可控性和对象间连贯性。

Conclusion: LayerEdit通过精确的对象层分解和连贯融合，成功解决了多对象编辑中的冲突问题，实现了高质量的解耦编辑效果。
...
Method: 使用扩散模型，在去噪过程中结合VAE编码的空间特征（来自航拍RGB图像和估计高度图）和CLIP语义嵌入，确保生成结果在几何上受场景3D结构约束且在语义上与内容一致。

Result: 在CVUSA、CVACT和Auto Arborist三个数据集上评估，SSIM平均提升7.3%，能稳健处理宽窄视野，展现强泛化能力。

Conclusion: Top2Ground方法在从航拍图像生成地面视图方面表现出色，无需中间表示，具有良好的几何约束和语义一致性。
...
TL;DR: ImageBindDC是一个新颖的多模态数据压缩框架，通过在ImageBind统一特征空间中使用特征函数损失，实现三个层次的分布一致性：单模态对齐、跨模态对齐和联合模态对齐，显著提升了多模态数据压缩效果。


<details>
  <summary>Details</summary>
Motivation: 传统数据压缩技术在单模态场景中成功，但在多模态场景中难以保持模态间复杂依赖关系，因此需要开发能够有效保留多模态依赖的数据压缩方法。

Method: 在ImageBind统一特征空间中操作，使用特征函数损失在傅里叶域实现精确统计对齐，通过无限矩匹配确保三个层次的分布一致性：单模态对齐、跨模态对齐和联合模态对齐。

Result: 在NYU-v2数据集上，仅使用每类5个压缩数据点训练的模型就能达到与完整数据集训练相当的无损性能，相比之前最佳方法绝对提升8.2%，压缩时间减少4倍以上。

Conclusion: ImageBindDC通过统一特征空间和特征函数损失，在多模态数据压缩中实现了突破性性能，为高效多模态模型训练提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 在极端条件下（如光线不足、剧烈相机运动），现有语义分割方法因RGB信息丢失而性能下降。事件模态具有高速和高动态特性，但与RGB存在异构性，导致特征不匹配和优化困难。

Method: 提出边缘感知语义一致性框架：1) 边缘感知潜在重编码，通过重编码分布将事件-RGB特征对齐到统一语义空间，利用预建边缘字典作为线索；2) 重编码巩固和不确定性优化，利用重编码边缘特征和不确定性指标解决异构融合问题。

Result: 在两个合成和一个真实世界的事件-RGB语义分割数据集上验证，在DERS-XS数据集上比现有最佳方法提升2.55% mIoU，在空间遮挡下具有优越的鲁棒性。

Conclusion: 该方法通过边缘线索有效解决了事件和RGB模态的异构融合问题，在极端条件下实现了更鲁棒的语义分割性能。
...
TL;DR: SWAN是一个开源的网页应用，通过滑动手势实现直观的图像块分类，解决了大规模病理图像标注的瓶颈问题。在试点研究中，SWAN与传统文件夹分类方法相比，实现了快速标注且保持了一致的标注质量。


<details>
  <summary>Details</summary>
Motivation: 大规模病理图像数据集的标注是开发稳健深度学习模型的主要瓶颈。传统的基于文件夹的标注工作流程通常缓慢、疲劳且难以扩展。

Method: 开发了SWAN（SWipeable ANnotations），一个开源、MIT许可的网页应用，支持桌面和移动平台，通过滑动手势实现图像块分类，提供实时元数据捕获和灵活的滑动-类别映射。

Result: 在4名病理学家标注600个有丝分裂图像块的试点研究中，SWAN的成对一致性百分比为86.52%-93.68%（Cohen's Kappa = 0.61-0.80），与传统文件夹方法的86.98%-91.32%（Cohen's Kappa = 0.63-0.75）相当，参与者评价工具高度可用。
...
TL;DR: 提出了一种机制感知的无监督通用图像融合方法(MAUGIF)，基于双交叉图像自编码器，根据融合任务的固有机制进行分类，并在解码阶段根据融合机制调整解码器架构。


<details>
  <summary>Details</summary>
Motivation: 现有融合方法要么高度任务特定，要么采用统一策略忽略不同任务的独特融合机制，需要一种能适应不同融合机制的统一框架。

Method: 基于双交叉图像自编码器，将源图像映射到共享潜在空间，捕获共同内容并隔离模态特定细节；解码器作为特征注入器，根据融合机制选择性重新整合各模态特征。

Result: 在多种融合任务上进行了广泛实验，验证了方法的有效性和泛化能力。

Conclusion: MAUGIF方法能够有效处理不同融合机制的任务，在性能和可解释性方面都有提升。
...
TL;DR: SkelSplat是一个基于可微分高斯渲染的多视角3D人体姿态估计框架，无需3D真值监督即可实现跨视角的无缝融合，在遮挡场景下表现出鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有的多视角方法依赖大规模标注数据集进行训练，在测试场景变化时泛化能力差，需要一种不依赖3D真值监督的通用解决方案。

Method: 将人体姿态建模为3D高斯骨架（每个关节一个高斯），通过可微分渲染进行优化，并提出了一种新颖的独热编码方案以实现关节的独立优化。

Result: 在Human3.6M和CMU数据集上优于不依赖3D真值的方法，相比基于学习的方法将跨数据集误差降低了47.8%，在遮挡场景下无需特定场景微调即表现出鲁棒性。

Conclusion: SkelSplat提供了一种无需3D真值监督的多视角3D人体姿态估计方法，具有良好的泛化能力和对遮挡的鲁棒性。
...
TL;DR: 本文提出了一种减少AI系统连续版本间不一致性的方法，通过保持原始模型的边界同时学习改进模型，结合边界校准和双重焦点蒸馏损失来降低负翻转率。


<details>
  <summary>Details</summary>
Motivation: 随着训练类别数量的增加，AI系统更新时会出现负翻转问题（新模型错误分类之前正确分类的样本），这在新类别引入时尤为明显，因为新类别会减少原始类别的边界并引入冲突模式。

Method: 提出保持原始模型边界的方法，通过边界校准项鼓励原始类别和新类别之间更大的相对边界，并结合双重焦点蒸馏损失从新旧数据中学习适当的决策边界。

Result: 在图像分类基准测试上的广泛实验表明，该方法在保持高整体准确率的同时，持续降低了负翻转率。

Conclusion: 该方法有效解决了AI系统更新时的负翻转问题，通过边界保持和蒸馏学习实现了版本间的一致性改进。
...
TL;DR: 本研究比较了不同对齐策略在乳腺X光片纵向深度学习风险建模中的效果，发现基于图像的配准方法在所有指标上均优于特征级对齐和隐式对齐方法，能提供更准确的时间一致性预测和更平滑的解剖学合理形变场。


<details>
  <summary>Details</summary>
Motivation: 乳腺X光筛查对早期乳腺癌检测至关重要，通过深度学习风险模型可以个性化筛查间隔。虽然现有方法越来越多地纳入纵向信息，但准确的空间对齐仍是关键挑战，错位会掩盖有意义的组织变化并降低模型性能。

Method: 研究评估了多种对齐策略：基于图像的配准、特征级对齐（有/无正则化）和隐式对齐方法。使用两个大规模乳腺X光数据集，评估每种方法在预测准确性、精确度、召回率和形变场质量等关键指标上的表现。

Result: 基于图像的配准在所有指标上始终优于特征级和隐式对齐方法，能实现更准确的时间一致性预测并生成平滑、解剖学合理的形变场。在特征空间中应用基于图像的形变场可获得最佳风险预测性能。

Conclusion: 研究强调了基于图像的形变场在纵向风险建模中空间对齐的重要性，提供了改进的预测准确性和鲁棒性。这种方法有强大潜力增强个性化筛查，并为高风险个体实现更早期干预。
...
TL;DR: DiveSeg是一个基于DINO预训练模型的水下实例分割框架，通过AquaStyle Aligner嵌入水下颜色风格特征和ObjectPrior Prompter提供对象级先验，在UIIS和USIS10K数据集上实现了最先进性能。
...
Method: 提出DiveSeg框架，包含两个关键组件：AquaStyle Aligner（在水下颜色风格特征嵌入到DINO微调过程中）和ObjectPrior Prompter（通过二值分割提示提供对象级先验）。
...
Method: 提出SASG-DA方法：1）语义表示引导机制确保生成忠实性；2）高斯建模语义采样策略实现灵活多样生成；3）稀疏感知语义采样策略增强目标多样性。

Result: 在Ninapro DB2、DB4和DB7基准数据集上的实验表明，SASG-DA显著优于现有数据增强方法，有效改善了识别性能和泛化能力。

Conclusion: 该方法通过提供忠实且多样化的样本，有效缓解了过拟合问题，提升了sEMG手势识别的性能。
...
TL;DR: 本文提出了一种分层向量量化变分自编码器方法，用于学习运动到干净图像特征的可分离嵌入，实现无需特定运动类型训练的通用MRI运动伪影校正。


<details>
  <summary>Details</summary>
Motivation: 生理运动会影响磁共振成像的诊断质量，现有基于机器学习的运动校正方法往往针对特定应用和数据集，难以泛化到不同的运动类型和身体区域。

Method: 使用分层向量量化变分自编码器学习运动到干净图像特征的可分离嵌入，部署码本捕获多分辨率下的有限运动模式集合，实现从粗到细的校正，并训练自回归模型学习无运动图像的先验分布。

Result: 在模拟全身运动伪影上验证了该方法，观察到在不同运动严重程度下都能实现鲁棒的校正，模型有效分离了模拟运动有效扫描的物理运动特征。

Conclusion: 该方法提高了基于机器学习的MRI运动校正的泛化能力，运动特征分离的方法在跨解剖区域和运动类型的应用中具有潜力。
...
<details>
  <summary>Details</summary>
Motivation: RoPE在NLP中成功被认为源于位置等变性，但作者质疑这种严格等变性是否真的对性能至关重要，特别是在扩展到多维数据时。

Method: 提出Spherical RoPE方法，使用非交换生成器替代传统RoPE的交换生成器，不要求严格的位置等变性。

Result: 实验发现Spherical RoPE在视觉任务中具有等同或更好的学习行为，表明相对位置嵌入可能不如普遍认为的重要。

Conclusion: 相对位置编码在计算机视觉中可能不是必需的，这一发现将促进未来开发更快、泛化更好的视觉位置编码方法。
...
TL;DR: 本文提出了基于文本的空中-地面人员检索任务，构建了TAG-PEDES数据集和TAG-CLIP检索框架，通过专家混合模块和视角解耦策略解决跨视角异构性问题。
...
Method: 提出TAG-CLIP框架，包含分层路由的专家混合模块学习视角特定和视角无关特征，以及视角解耦策略分离视角特定特征以实现更好的跨模态对齐。

Result: 在提出的TAG-PEDES数据集和现有T-PR基准上评估了TAG-CLIP的有效性，数据集和代码已开源。

Conclusion: TAG-PR任务具有重要实践意义，提出的方法和数据集为解决跨视角异构检索问题提供了有效解决方案。
...
TL;DR: RAPTR是一个基于雷达的室内3D人体姿态估计方法，在弱监督下仅使用3D边界框和2D关键点标签，通过两阶段姿态解码器架构实现优于现有方法的性能。
...
Method: 提出RAPTR框架，采用两阶段姿态解码器：姿态解码器使用3D模板损失估计初始3D姿态；关节解码器使用2D关键点标签和3D重力损失细化姿态。采用伪3D可变形注意力增强多视图雷达特征。

Result: 在两个室内雷达数据集上评估，RAPTR显著优于现有方法，在HIBER数据集上减少关节位置误差34.3%，在MMVR数据集上减少76.9%。

Conclusion: RAPTR证明了在弱监督下仅使用3D边界框和2D关键点标签即可实现高质量的室内3D人体姿态估计，为雷达姿态估计提供了一种更可扩展的解决方案。
...
TL;DR: Anatomy-VLM是一个细粒度的视觉语言模型，通过多尺度信息整合来改进医学影像诊断，模仿临床医生分析解剖结构的工作流程，在疾病预测和解剖解释方面表现出色。


<details>
  <summary>Details</summary>
Motivation: 当前主流视觉语言模型将医学影像视为整体，忽略了疾病诊断所需的关键细粒度图像细节，而临床医生通过先验医学知识识别重要解剖结构区域来进行诊断。

Method: 设计模型编码器定位医学图像中的关键解剖特征，用结构化知识丰富这些区域进行上下文感知解释，并对齐多尺度医学信息生成临床可解释的疾病预测。

Result: Anatomy-VLM在分布内和分布外数据集上都取得优异性能，在下游图像分割任务中验证了其细粒度对齐能捕获解剖和病理相关知识，支持零样本解剖级解释。

Conclusion: Anatomy-VLM通过模仿临床医生工作流程，整合多尺度信息，实现了专家级的临床解释能力，在医学影像诊断中表现出强大的性能。
...
TL;DR: CleverBirds是一个用于细粒度鸟类物种识别的大规模知识追踪基准数据集，收集了超过40,000名参与者对10,000多种鸟类的1,700万道选择题回答，平均每位参与者回答400个问题，旨在支持视觉知识追踪方法的发展。
...
Method: 通过公民科学平台eBird收集数据，构建了包含40,000多名参与者、1,700万道选择题、涵盖10,000多种鸟类物种的大规模知识追踪基准数据集。

Result: 追踪学习者的知识具有挑战性，特别是在不同参与者子组和问题类型之间，不同形式的上下文信息提供不同程度的预测效益。

Conclusion: CleverBirds是同类基准中规模最大的之一，提供了更多可学习概念，有望为研究视觉专业知识在不同时间和个体间的发展开辟新途径。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界应用需要结合多种视频能力的复杂迭代工作流，而现有专业AI模型只能处理孤立任务，无法满足需求。

Method: 采用Plan-and-Act双智能体架构：规划智能体解析用户意图并分解为结构化步骤，执行智能体通过模块化MCP工具服务器执行；使用分层多级内存系统维持长期推理和上下文连续性。
...
Result: 成功构建了3D4D框架，能够将静态内容转换为4D场景，支持自适应、用户驱动的4D环境探索。

Conclusion: 3D4D框架为复杂4D环境的可视化提供了一种有效的交互式解决方案，项目代码已开源。
...
<details>
  <summary>Details</summary>
Motivation: 真实场景中获取精确相机位姿具有挑战性，现有NeRF方法严重依赖准确外参估计，且其隐式体表示与广泛采用的多边形网格差异大，在标准3D软件中渲染和操作效率低

Method: 联合优化相机位姿同时学习隐式场景表示，捕获精细几何细节和真实感外观，直接重建可编辑3D网格

Result: 在标准基准测试中，该方法在相机位姿不确定情况下实现了准确鲁棒的3D重建

Conclusion: 该方法弥合了神经隐式表示与实际机器人应用之间的差距，使重建网格能够高效用于下游任务
...
TL;DR: 提出基于注意力机制的Vision Transformer架构，用于用户设备定位，在室内外场景中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习定位方法存在两个主要问题：对输入数据给予同等注意力权重；不适用于非序列数据（如瞬时CSI）。

Method: 使用基于注意力机制的Vision Transformer架构，专注于从CSI矩阵中提取角度延迟剖面(ADP)信息。

Result: 在DeepMIMO数据集上：室内RMSE为0.55m，室外为13.59m；在ViWi数据集上：室外遮挡场景RMSE为3.45m。相比现有方法性能提升约38%。

Conclusion: 所提出的注意力机制Vision Transformer架构在用户设备定位任务中表现出色，特别是在处理非序列CSI数据时具有明显优势。
...
TL;DR: SENCA-st是一种新颖的多模态架构，通过共享编码器和邻域交叉注意力机制，有效整合组织病理学图像和空间转录组学数据，在保留两种模态特征的同时，特别关注结构相似但功能不同的区域。


<details>
  <summary>Details</summary>
Motivation: 当前的组织病理学-空间转录组学区域分割方法存在两个极端：要么过度依赖空间转录组学数据而忽视组织病理学特征，要么使用普通对比学习导致组织病理学图像过于突出而丢失功能信息。这两种方法都会导致模型要么陷入空间转录组学的噪声中，要么过度平滑而丢失关键信息。

Method: 提出SENCA-st（共享编码器与邻域交叉注意力）架构，该架构通过共享编码器保留两种模态的特征，并使用交叉注意力机制特别强调在组织病理学中结构相似但在空间转录组学中功能不同的区域。

Result: 该模型在检测肿瘤异质性和肿瘤微环境区域方面表现出卓越性能，超越了现有最先进方法，这在临床应用中至关重要。

Conclusion: SENCA-st架构成功解决了多模态数据整合中的关键挑战，为空间转录组学和组织病理学图像的有效融合提供了创新解决方案，在肿瘤异质性检测方面具有重要临床价值。
...
### [89] [GRIP: In-Parameter Graph Reasoning through Fine-Tuning Large Language Models](https://arxiv.org/abs/2511.07457)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在处理序列文本数据方面表现出色，但难以有效处理结构化数据如图谱。现有方法要么产生大量token开销，要么需要大规模后训练和复杂对齐过程，但效果仍不理想。

Method: 提出GRIP框架，通过精心设计的微调任务让LLMs内化图中的关系信息，使用轻量级LoRA参数存储知识，实现无需原始图的推理。

Result: 在多个基准测试上的广泛实验验证了该方法的有效性和效率。

Conclusion: GRIP框架成功解决了LLMs处理图结构数据的挑战，通过参数内知识注入实现了高效的知识图谱内化。
...
TL;DR: DuTerm是一个用于术语约束机器翻译的两阶段架构，结合了术语感知的NMT模型和基于提示的LLM后编辑，在多个语言对上实现了比严格约束执行更高质量的翻译。
...
TL;DR: Motif-2-12.7B是一个新的开放权重基础模型，通过架构创新和系统级优化推动大语言模型的效率前沿。该模型在Motif-2.6B基础上集成了分组差分注意力机制，在55万亿token上进行预训练，并在分布式环境中实现显著吞吐量和内存效率提升。
...
Method: 采用分组差分注意力机制改进表示效率；使用课程驱动数据调度器在55万亿token上进行预训练；集成MuonClip优化器和自定义高性能内核；采用三阶段监督微调管道。

Result: 在多样化基准测试中表现出竞争力，证明经过深思熟虑的架构扩展和优化训练设计可以与更大模型的性能相媲美。

Conclusion: 通过架构扩展和优化训练设计的结合，可以在不增加模型规模的情况下实现与大模型相竞争的性能，为高效大语言模型的发展提供了新思路。
...
TL;DR: 本研究提出了LAHIS方法，通过单次前向和后向传播识别多语言能力中注意力头的重要性，揭示了语言特定和语言通用头的存在，并提出轻量级适应方法改善多语言性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的多语言能力日益增强，但多头自注意力机制在多语言处理中的作用尚未充分探索，需要理解其内部机制以提升多语言性能。

Method: 提出LAHIS方法，通过单次前向和后向传播计算注意力头在多语言能力中的重要性分数，并在多个模型上进行验证。

Result: 发现存在语言特定和语言通用注意力头，语言特定头能够实现跨语言注意力转移，缓解目标语言生成问题。提出的轻量级适应方法仅需20个可调参数即可提升XQuAD准确率。

Conclusion: 该工作从多头自注意力角度增强了大型语言模型的可解释性和多语言能力，为理解多语言处理机制提供了新视角。
...
TL;DR: 本文研究了如何优化基于大语言模型的文档重排序方法，通过多种优化技术将延迟从61.36秒降低到0.37秒，性能损失很小，使LLM重排序更适合实时应用。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的成对重排序方法（PRP）虽然有效，但计算复杂度高、延迟大，难以在实时应用中部署。

Method: 采用多种优化方法：使用更小的模型、限制重排序文档集、降低精度、通过单向顺序推断减少位置偏差、限制输出标记等。
...
Method: 评估三个荷兰语特定LLMs与LIWC和Pattern工具在佛兰芒语情感价预测任务中的表现，使用约25000条自发文本响应数据。
...
TL;DR: 本文重新评估了使用现成的自然语言推理（NLI）评分结合简单词汇匹配标志的轻量级方法，发现在长问答任务中能达到GPT-4o的准确率（89.9%），同时参数需求大幅减少。


<details>
  <summary>Details</summary>
Motivation: 评估大型语言模型（LLM）答案具有挑战性：词汇指标无法捕捉语义细微差别，而'LLM作为评判者'评分计算成本高昂。本文旨在重新评估轻量级替代方案。

Method: 使用现成的自然语言推理（NLI）评分，并增加简单的词汇匹配标志。同时引入DIVER-QA基准，这是一个包含3000个样本的人类标注数据集，涵盖五个问答数据集和五个候选LLM。

Result: NLI评分方法在长问答任务中达到89.9%的准确率，与GPT-4o相当，但所需参数数量显著减少。
...
TL;DR: 本文系统评估了六种广泛使用的无参考事实性指标在长文档摘要中的可靠性，发现这些原本为短文本摘要设计的指标在长文档场景下表现不稳定，对语义等效摘要产生不一致评分，且在信息密集声明中可靠性下降。


<details>
  <summary>Details</summary>
Motivation: 评估抽象文本摘要的事实一致性仍然是一个重大挑战，特别是在长文档场景中，传统指标难以处理输入长度限制和长距离依赖关系。

Method: 通过七种事实性保持扰动（包括释义、简化、同义词替换、逻辑等价否定、词汇缩减、压缩和源文本插入）来探测指标的鲁棒性，并分析其对检索上下文和声明信息密度的敏感性。

Result: 在三个长文档基准数据集上的结果显示，现有短文本指标对语义等效摘要产生不一致评分，对信息密集声明的可靠性下降，扩展检索上下文在某些领域能提高稳定性但无指标能在长上下文条件下持续保持事实对齐。
...
Result: CAPO在奖励准确率上比现有基线方法提升至少16%，并通过扩大首选和非首选响应之间的差距来改善跨语言对齐。
...
TL;DR: 该论文提出"关键虚构"方法，利用LLM的幻觉来填补因社会政治不平等造成的档案空白，为历史中的"隐藏人物"重建证据约束的叙事。


<details>
  <summary>Details</summary>
Motivation: LLM的幻觉虽然存在问题，但经过精心约束后可以具有社会价值，特别是用于填补因社会政治不平等导致的档案空白。

Method: 使用开放式叙事填空任务，让LLM生成角色时间线中被掩盖的事件，评估经过数据污染审计的完全开放模型和未审计的基线模型在不同提示下的表现。

Result: 验证了LLM具备执行关键虚构的基础叙事理解能力，表明受控且明确指定的幻觉可以支持LLM在知识生产中的应用，同时保持历史准确性和忠实度。

Conclusion: 精心约束的LLM幻觉可以成为有用的知识生产工具，填补历史档案中的空白，而不会损害历史准确性。
...
TL;DR: 该研究探讨了语境可预测性如何影响在线语言产生中的词汇形式和选择，特别关注了基于未来语境的后向可预测性效应，并提出了新的信息论可预测性度量方法。


<details>
  <summary>Details</summary>
Motivation: 虽然基于过去语境的前向可预测性效应在语言产生和理解中已得到较好理解，但基于未来语境的后向可预测性效应仍不清楚，可能与未来规划相关。

Method: 在两个自然语音语料库研究中，使用改进的度量和更强大的语言模型，提出了整合未来和过去语境可预测性的新信息论度量方法，分别研究词长效应和替换错误。

Result: 研究发现提出的后向可预测性替代方法在两个研究中产生定性相似的效果，通过对替换错误的细粒度分析揭示了说话者在词汇规划过程中如何优先处理形式、意义和基于语境的信息。

Conclusion: 这些发现阐明了过去和未来语境在说话者编码和选择词汇中的功能作用，为语境可预测性效应与句子规划机制之间提供了桥梁。
...
### [100] [TurkEmbed: Turkish Embedding Model on NLI & STS Tasks](https://arxiv.org/abs/2511.08376)
...
<details>
  <summary>Details</summary>
Motivation: 现有土耳其语嵌入模型通常依赖机器翻译数据集，可能限制其准确性和语义理解能力，需要开发更精确的本地化模型。
...
TL;DR: 本文介绍了CUFEInse v1.0保险领域专业评估基准的构建方法、多维评估体系和设计理念，对11个主流大语言模型进行全面评估，发现通用模型在精算能力和合规适应性方面存在瓶颈，高质量领域专用训练在保险垂直场景中具有优势但存在业务适应性和合规性不足的问题。
...
Method: 采用"量化导向、专家驱动、多重验证"原则，建立涵盖5个核心维度、54个子指标和14,430个高质量问题的评估框架，包括保险理论知识、行业理解、安全合规、智能体应用和逻辑严谨性。

Result: 评估发现通用模型存在精算能力弱、合规适应性不足等共性瓶颈；高质量领域专用训练在保险垂直场景中具有显著优势但在业务适应性和合规性方面存在不足；准确识别了当前大模型在保险精算、核保理赔推理、合规营销文案等专业场景中的共性瓶颈。

Conclusion: CUFEInse的建立填补了保险领域专业评估基准的空白，其构建理念和方法论为垂直领域大模型评估范式提供了重要参考，为学术模型优化和工业模型选择提供了权威参考，展望了评估基准的未来迭代方向和保险大模型"领域适应+推理增强"的核心发展方向。
...
TL;DR: 本文提出了一种基于可训练多层图记忆框架的LLM智能体，通过将原始轨迹抽象为状态机中的结构化决策路径，并进一步提炼为高层战略元认知，提升LLM在自主任务解决中的推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体获取经验的方式存在局限性：隐式记忆训练存在灾难性遗忘和可解释性差的问题，显式记忆提示缺乏适应性。需要一种能够更好利用先验经验来指导当前决策的方法。

Method: 提出可训练的多层图记忆框架，将智能体轨迹抽象为状态机中的结构化决策路径，并提炼为高层战略元认知。采用基于强化学习的权重优化程序估计每个元认知的经验效用，并通过元认知提示动态集成到LLM智能体的训练循环中。

Result: 实验证明，可学习的图记忆框架提供了强大的泛化能力，提高了LLM智能体的战略推理性能，并在强化学习训练中带来持续收益。

Conclusion: 提出的可训练图记忆框架有效增强了LLM智能体利用参数化信息的能力，通过结构化的经验表示和动态集成机制，显著提升了自主任务解决中的推理性能。
...
Method: 提出AlignSurvey基准，定义四个与关键调查阶段对齐的任务，构建多层级数据集架构（包括社会基础语料库和完整流程调查数据集），并通过两阶段微调开源LLM获得SurveyLM系列模型。

Result: 开发了包含44K+访谈对话和400K+结构化调查记录的社会基础语料库，以及专家标注的AlignSurvey-Expert数据集和两个全国代表性调查用于跨文化评估，提供了任务特定的评估指标来评估对齐保真度、一致性和公平性。

Conclusion: AlignSurvey为透明和负责任的社会科学研究提供了完整的基准、数据集和工具，支持在个体和群体层面评估LLM在社会调查中的表现，特别关注人口多样性。
...
<details>
  <summary>Details</summary>
Motivation: 在印度等民主国家，公民表达诉求的自由可能导致未经批准的抗议活动，这些活动具有破坏性。提前预测这些事件有助于行政官员采取必要措施。

Method: 使用主题建模和word2vec过滤相关新闻文章，应用命名实体识别方法识别人物、组织、地点和日期等实体，并进行时间标准化处理。提出了相关实体提取方法。

Result: 开发了一个地理独立、通用的模型，能够识别过滤公民动荡事件的关键特征，并有效提取与事件相关的实体。

Conclusion: 该系统能够通过分析新闻公告中的相关实体，有效预测计划中的社会动荡事件，为行政管理提供决策支持。
...
TL;DR: MC^2F是一个两模块系统，直接在句子嵌入上操作，通过分层黎曼连续归一化流学习干净数据流形的密度，并通过测地线净化求解器将对抗点投影回学习到的流形，从而在保持干净数据性能的同时显著提升对抗鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 解决文本分类中增强模型对抗鲁棒性通常会降低干净数据性能的持久挑战，通过建模编码器嵌入流形中干净样本的分布来解决这一问题。
...
Conclusion: MC^2F方法成功解决了文本分类中对抗鲁棒性与干净数据性能之间的权衡问题，通过流形建模和校正实现了两方面的同时提升。
...
TL;DR: WorkBench是首个统一评估套件，涵盖六个工作相关任务，旨在评估通用嵌入模型在工作领域的性能。基于此基准，开发了UWE模型，通过多对多InfoNCE目标和软延迟交互实现零样本排序性能。


<details>
  <summary>Details</summary>
Motivation: 工作领域自然语言处理任务面临长尾分布、极端多标签和稀缺数据等挑战，需要评估通用嵌入模型在该领域的表现并开发专门解决方案。

Method: 构建WorkBench评估套件，基于跨任务转移洞察构建任务特定二分图，使用真实数据并通过接地增强。开发UWE双编码器，采用多对多InfoNCE目标和任务无关软延迟交互。

Result: UWE在未见目标空间上实现零样本排序性能，支持低延迟推理，在宏观平均MAP和RP@10指标上显著优于通用嵌入模型。

Conclusion: UWE模型通过利用训练数据结构，在工作领域任务上表现出色，为工作相关自然语言处理任务提供了有效的嵌入解决方案。
...
Method: 提出NOTAM-Evolve框架，将完整解析任务形式化为深度解析，结合动态知识基础（链接NOTAM到实时航空数据）和基于模式的推理（应用静态领域规则推导操作状态），通过知识图谱增强检索模块和闭环学习过程实现自主进化。

Result: 实验表明NOTAM-Evolve相比基础LLM实现了30.4%的绝对准确率提升，在结构化NOTAM解析任务上建立了新的最先进水平。

Conclusion: NOTAM-Evolve框架能够有效解决NOTAM深度解析的挑战，通过自进化学习显著提升了解析性能，为航空安全提供了更可靠的自动化解析方案。
...
TL;DR: 本文提出了一种自校正蒸馏（SCD）方法，通过错误提示机制（EPM）检测和纠正错误，以及两阶段蒸馏策略，将大语言模型的查询生成和错误校正能力迁移到小语言模型上，显著提升了小语言模型在结构化数据问答任务中的性能。


<details>
  <summary>Details</summary>
Motivation: 现有统一结构化问答框架（如TrustUQA）在小规模语言模型上表现不佳，因为小模型容易在生成结构化查询时出错。需要提升小模型在结构化数据问答中的能力。

Method: 提出自校正蒸馏（SCD）方法，包括：1）错误提示机制（EPM）用于推理时检测错误并提供定制化错误信息；2）两阶段蒸馏策略，将大语言模型的查询生成和错误校正能力迁移到小语言模型。

Result: 在5个基准测试和3种结构化数据类型上的实验表明，SCD在小规模语言模型（8B）上实现了最佳性能和优越的泛化能力，接近GPT4在某些数据集上的表现。配备EPM的大语言模型在大多数数据集上超越了最先进的结果。

Conclusion: SCD方法有效提升了小语言模型在结构化数据问答中的能力，通过错误检测和校正机制以及知识蒸馏，实现了接近大模型性能的表现。
...
TL;DR: 本文通过掩码语言建模测量科幻小说中人类、动物和机器概念之间的渗透性，发现科幻小说具有更高的概念流动性，特别是机器指称表现出显著的跨类别替代和分散，而人类术语保持语义连贯性。


<details>
  <summary>Details</summary>
Motivation: 研究动机是验证达科·苏文的陌生化理论，通过计算方式测量科幻小说如何破坏本体论类别，探索科幻小说特有的语义规范扰动。

Method: 使用RoBERTa生成掩码指称的词汇替代，通过Gemini进行分类，量化概念滑移的三个指标：保留率、替换率和熵，比较科幻小说和一般小说的语料库。

Result: 研究发现科幻小说表现出更高的概念渗透性，机器指称具有显著的跨类别替代和分散，而人类术语保持语义连贯并主导替代层次结构。

Conclusion: 科幻小说中的陌生化表现为语义规范的可控扰动，可通过概率建模检测，MLM可作为解释性工具揭示体裁条件化的本体论假设。
...
TL;DR: 该研究探索使用学习到的压缩令牌来压缩大语言模型的上下文，以减少处理长序列时的内存和计算需求。通过微调预训练LLM，可在2倍到8倍的压缩率下保持性能，并在30亿参数LLaMA模型上达到与替代压缩技术相当的结果。


<details>
  <summary>Details</summary>
Motivation: 减少大语言模型处理长序列时的内存和计算需求，提高模型效率。

Method: 通过微调预训练大语言模型，使其学习压缩令牌来压缩上下文。

Result: 在2倍到8倍压缩率下，模型在短上下文和长上下文基准测试中性能没有显著下降。在30亿参数LLaMA模型上，该方法达到与替代压缩技术相当的结果，同时获得更高的压缩比。

Conclusion: 学习到的上下文压缩令牌是减少大语言模型内存和计算需求的有效方法，可在保持性能的同时实现显著压缩。
...
TL;DR: 本文提出了一种新的Relation as a Prior (RelPrior)范式，用于解决大语言模型在文档级关系抽取中的性能差距问题。该方法通过使用关系作为先验来过滤不相关实体对并提取三元组，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型在文档级关系抽取中存在性能差距，主要原因是：(1)大量不相关实体对引入噪声干扰关系预测；(2)超出预定义关系集的关系标签被错误地视为预测错误。

Method: 提出RelPrior范式：使用二元关系作为先验来提取和确定实体是否相关，过滤不相关实体对；使用预定义关系作为先验来匹配实体进行三元组提取，避免严格预定义关系标签导致的误判。

Result: 在两个基准数据集上的广泛实验表明，RelPrior实现了最先进的性能，超越了现有的大语言模型方法。

Conclusion: RelPrior范式通过将关系作为先验，有效解决了大语言模型在文档级关系抽取中的关键挑战，显著提升了性能表现。
...
<details>
  <summary>Details</summary>
Motivation: 评估大语言模型在低资源、形态丰富的梵语中是否能够超越专门模型，特别是在具有挑战性的诗歌转散文任务上。
...
TL;DR: 分析BabyLM语料库和CHILDES中不同年龄组的句法特性，发现CHILDES在年龄上的句法区分不明显，但训练数据的句法知识有助于解释模型在语言任务上的表现。课程学习方面，探索了发展性和其他认知启发式课程方法，发现某些课程有助于阅读任务，但主要性能提升来自使用可句法分类的数据子集而非完整嘈杂语料库。


<details>
  <summary>Details</summary>
Motivation: 研究儿童语言习得语料库的句法特性，探索如何利用句法知识来理解和改进语言模型的性能表现，特别是在课程学习框架下。

Method: 分析BabyLM语料库和CHILDES的句法特性，比较不同年龄组的句法差异，探索发展性课程和其他认知启发式课程学习方法。

Result: CHILDES语料库在年龄上的句法区分不明显；句法知识有助于解释模型性能；某些课程学习方法对阅读任务有帮助；主要性能提升来自使用可句法分类的数据子集。

Conclusion: 训练数据的句法特性分析对理解模型表现有重要价值，在课程学习中，使用经过句法筛选的数据子集比使用完整嘈杂语料库能带来更好的性能提升。
...
TL;DR: 本文提出了一个基于嵌入的基准测试框架，用于检测大型语言模型在形成性反馈中的偏见。通过600篇真实学生作文构建性别反事实对比，发现所有模型对隐式性别线索都表现出不对称的语义响应，GPT和Llama模型对显式性别线索敏感。
...
Method: 使用600篇真实学生作文构建控制反事实，包括隐式线索（基于词典的性别术语替换）和显式线索（提示中的性别作者背景）。研究了6个代表性LLM，通过余弦和欧几里得距离量化响应差异，使用置换测试评估显著性，并通过降维可视化结构。

Result: 所有模型对隐式操作都表现出男性-女性反事实比女性-男性更大的语义偏移。只有GPT和Llama模型对显式性别线索敏感。定性分析显示一致的语用差异（男性线索下更多自主支持反馈，女性线索下更多控制性反馈）。

Conclusion: 即使是最先进的LLM也表现出对性别替换的不对称语义响应，表明其提供的反馈中存在持续的性别偏见。研究为教学GenAI的公平性审计提供了启示，提出了学习分析中反事实评估的报告标准，并为提示设计和部署提供了实用指导以保障公平反馈。
...
TL;DR: 本文提出了VocalBench-zh，一个面向中文语境的语音到语音评估套件，包含10个子集和超过1万个高质量实例，覆盖12个用户导向特性。通过对14个主流模型的评估实验，揭示了当前方法的共同挑战。


<details>
  <summary>Details</summary>
Motivation: 普通话作为全球使用最广泛的语言之一，虽然大多数模型都支持普通话，但缺乏全面的语音到语音基准测试阻碍了系统评估和公平模型比较。

Method: 开发了VocalBench-zh评估套件，包含10个精心设计的子集和超过1万个高质量实例，专门适应中文语境，涵盖12个用户导向特性。

Result: 对14个主流模型的评估实验显示，当前方法面临共同挑战，需要为下一代语音交互系统提供新的见解。

Conclusion: VocalBench-zh填补了中文语音到语音评估的空白，为开发者和用户提供了系统评估工具，揭示了当前模型的局限性，并指出了未来发展方向。
...
TL;DR: 本文提出了一种基于提示调优的NL-to-SQL错误纠正框架，结合生成式预训练LLMs和RAG技术，通过诊断错误类型、识别原因并提供修复指令来改进SQL查询准确性。
...
Method: 提出新颖框架，集成错误纠正机制，包括错误类型诊断、原因识别、修复指令生成和SQL查询修正。结合嵌入微调和RAG技术，利用外部知识库提升准确性和透明度。

Result: 通过全面实验验证，该框架相比现有基线实现了12%的准确率提升。

Conclusion: 该框架在当代数据驱动环境中具有革新数据访问和处理方式的潜力。
...
TL;DR: 提出了ParliaBench基准测试，用于评估议会演讲生成质量，包括语言质量、语义连贯性和政治真实性三个维度，并引入了新的基于嵌入的指标来量化意识形态定位。


<details>
  <summary>Details</summary>
Motivation: 议会演讲生成对大型语言模型提出了超出标准文本生成任务的特定挑战，需要政治真实性和意识形态一致性，而现有模型缺乏专门训练，评估方法也忽视了政治真实性。

Method: 构建了英国议会演讲数据集，提出了结合计算指标和LLM作为评判者的评估框架，引入了政治光谱对齐和党派对齐两个新的嵌入指标，对五个大型语言模型进行了微调并生成了28k个演讲。

Result: 微调在大多数指标上产生了统计显著的改进，新提出的指标在政治维度上表现出强大的区分能力。

Conclusion: ParliaBench基准测试为议会演讲生成提供了系统评估框架，微调能显著提升模型性能，新指标能有效衡量政治维度的生成质量。
...
TL;DR: 该研究探索视觉大语言模型理解科学论文中表格结构的能力，特别是推断复杂层次表格结构的能力，使用PubTables-1M数据集中的复杂层次表格作为基准。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索通用视觉大语言模型是否能够理解表格的层次结构，而无需额外的专门处理，为未来将结构化数据理解集成到通用VLLMs中提供指导。

Method: 采用多种提示工程策略，包括不同提示格式和写作风格，评估多个先进的开源VLLMs，包括现成版本和微调版本，并与人类表现进行对比。

Result: 实验结果表明，未专门设计用于理解表格结构的通用VLLMs能够完成此任务，揭示了VLLMs处理复杂表格的潜力和局限性。

Conclusion: 该研究证实了通用VLLMs理解表格结构的能力，为未来在通用VLLMs中集成结构化数据理解提供了重要见解和指导。
...
TL;DR: 本文提出了一种多智能体框架来改进LLM对话响应，通过分配不同角色处理事实性、个性化和连贯性三个关键方面，并采用动态通信策略自适应协调智能体，显著提升了对话质量。


<details>
  <summary>Details</summary>
Motivation: LLM在对话系统中生成类人响应时，在个性化或特定知识方面存在不足，且用户难以检测错误并要求重新生成，因此需要在返回给用户前改进响应质量。

Method: 使用多智能体框架，每个智能体负责审查和改进一个特定方面（事实性、个性化、连贯性），通过动态通信策略自适应选择和协调最相关的智能体。

Result: 在具有挑战性的对话数据集上验证，该方法在涉及知识或用户角色（或两者）的任务中显著优于相关基线方法。

Conclusion: 多智能体框架通过角色分工和动态协调，有效提升了LLM对话响应的整体质量，特别是在事实性和个性化方面表现突出。
...
Method: 提出DPRM模型，包含KG-PRM和CoT-PRM两个隐式过程奖励模型，通过偏好对学习知识图谱的结构约束，并引入一致性约束使两个PRM相互验证和协同优化推理路径。
...
TL;DR: 本文介绍了动态发音模型DYNARTmo的当前实现，该模型基于语音手势概念和相应的手势乐谱生成连续的发音器运动。
...
Method: 使用语音手势概念和手势乐谱，通过手势库结构和手势协调来生成连续的发音器轨迹，控制DYNARTmo声道模型。

Result: 成功实现了基于手势的连续发音器运动生成框架。
...
Result: 能够实现可验证的推理过程，支持黑盒设置下的链级验证，促进多LLM系统协作

Conclusion: PCRLLM框架结合了自然语言表达和形式严谨性，为LLM推理提供了可验证和协作的解决方案
...
TL;DR: 本文研究了社交机器人检测器在真实场景中的鲁棒性问题，特别关注了基于文本特征的捷径学习现象，并提出了基于大语言模型的反事实数据增强缓解策略。


<details>
  <summary>Details</summary>
Motivation: 现有社交机器人检测器在基准测试中表现良好，但在多样化真实场景中的鲁棒性有限，主要原因是真实标签不清晰和误导性线索多样。特别是捷径学习现象（模型依赖虚假相关性而非因果任务相关特征）的影响尚未得到充分关注。

Method: 通过构建用户标签与表面文本线索之间的虚假关联，设计了一系列捷径场景来评估模型鲁棒性。提出基于大语言模型的缓解策略，利用反事实数据增强，从数据和模型两个角度在三个层次上解决问题。

Result: 结果显示，无关特征分布的偏移显著降低了社交机器人检测器性能，基线模型的平均相对准确率下降了32%。提出的缓解策略在捷径场景下实现了56%的平均相对性能提升。

Conclusion: 社交机器人检测器容易受到捷径学习的影响，基于大语言模型的反事实数据增强策略能有效提升模型在捷径场景下的鲁棒性，为解决该问题提供了有效方法。
...
TL;DR: SPEAR-MM是一种选择性参数评估和恢复框架，通过模型合并解决LLMs在金融领域适应中的灾难性遗忘问题，在保持94%领域适应收益的同时实现91.2%通用能力保留。
...
TL;DR: S-RAG是一个专门为聚合查询设计的检索增强生成方法，通过在语料库构建结构化表示，将自然语言查询转换为形式化查询，显著优于传统RAG系统和长上下文LLM。


<details>
  <summary>Details</summary>
Motivation: 当前RAG方法主要关注少量相关文档的查询，无法有效处理需要从大量文档中聚合信息的查询任务。

Method: 在语料库摄入时构建结构化表示，在推理时将自然语言查询转换为对结构化表示的形式化查询。

Result: 在新建的HOTELS和WORLD CUP数据集以及公共基准测试中，S-RAG显著优于传统RAG系统和长上下文LLM。

Conclusion: S-RAG为聚合查询提供了有效的解决方案，填补了当前RAG方法的空白，推动了该领域的研究发展。
...
Method: 创建了包含1000个高质量人工标注句子-手势对的数据集，通过基于规则的检索增强生成(RAG)管道使用句法和形态规则生成了约3000个合成对，并微调了多个transformer模型进行句子到手势翻译。

Result: 基于BLEU评分评估了模型性能，并在数据集和RWTH-PHOENIX-2014T基准上比较了模型的手势翻译一致性。

Conclusion: 成功构建了孟加拉手语句子级翻译数据集，并验证了transformer模型在该任务上的有效性。
...
TL;DR: 本文系统研究了大型推理模型（LRMs）通过思维链（CoT）进行监控的可行性和挑战，重点关注模型在CoT中真实表达决策因素的程度以及基于CoT的监控可靠性。
...
Method: 从两个核心视角进行研究：(i) 语言化程度：LRMs在CoT中真实表达决策因素的程度；(ii) 监控可靠性：基于CoT的监控器可靠检测模型不当行为的程度。通过数学、科学和伦理领域的实证证据和相关分析，并研究不同CoT干预方法对监控效果的影响。

Result: 提出了MoME新范式，让LLMs通过其他模型的CoT监控其不当行为，并提供结构化判断和支持证据。

Conclusion: 系统研究了CoT监控性的挑战和潜力，为通过思维链分析提高AI安全监控效果提供了理论基础和实践方法。
...
TL;DR: 该研究分析了大型语言模型在角色扮演情境下的道德响应，通过道德基础问卷衡量道德易感性和道德稳健性，发现模型家族对道德稳健性影响最大，而模型大小对道德易感性影响更明显。
...
Method: 使用道德基础问卷（MFQ）创建基准，量化道德易感性和道德稳健性，通过在不同角色间和角色内的MFQ得分变异性来定义这两个属性。

Result: 克劳德家族在道德稳健性方面表现最佳，其次是Gemini和GPT-4模型；模型大小对道德稳健性无系统性影响，但对道德易感性有明显影响，较大模型更易受影响；稳健性和易感性呈正相关。

Conclusion: 角色扮演条件显著影响大型语言模型的道德行为，模型家族是决定道德稳健性的主要因素，而模型大小主要影响道德易感性。
...
TL;DR: TaH是一种动态潜在思考方法，通过轻量级决策器仅在困难token处进行深度迭代，使用LoRA模块调整LLM目标，并引入双因果注意力机制，在保持参数不变的情况下显著提升推理性能。


<details>
  <summary>Details</summary>
Motivation: 解决现有循环transformer中存在的潜在过度思考现象——简单token在第一次前向传播后已经正确预测，但在额外迭代中有时会被修改为错误。

Method: 提出Think-at-Hard方法：1) 轻量级神经决策器触发潜在迭代；2) LoRA模块将LLM目标从通用下一token预测转向专注困难token精炼；3) 双因果注意力机制扩展注意力到迭代深度维度。

Result: 在五个挑战性基准测试中提升LLM推理性能，相比对所有输出token迭代两次的基线方法，准确率提升8.1-11.3%，同时免除94%token的第二次迭代。与单次迭代Qwen3模型相比，准确率提升4.0-5.0%。

Conclusion: TaH方法有效解决了潜在过度思考问题，通过动态选择困难token进行深度迭代，在保持参数效率的同时显著提升了大型语言模型的推理能力。
...
TL;DR: 语言模型可以通过微调学习描述自身内部计算过程，包括特征编码信息、激活因果结构和输入令牌影响，且自我解释优于其他模型解释。


<details>
  <summary>Details</summary>
Motivation: 研究语言模型是否能够利用对其内部计算的特殊访问权限，开发新的解释技术来忠实描述自身行为。

Method: 使用现有可解释性技术作为真实标签，微调语言模型生成自然语言描述，包括特征信息、激活因果结构和输入令牌影响。

Result: 仅用数万个示例训练的解释器模型对新查询表现出非平凡泛化能力，自我解释效果优于其他模型解释。
...
### [134] [A Preliminary Study of RAG for Taiwanese Historical Archives](https://arxiv.org/abs/2511.07445)
*Claire Lin,Bo-Han Feng,Xuanjun Chen,Te-Lun Yang,Hung-yi Lee,Jyh-Shing Roger Jang*

Main category: cs.CL

TL;DR: 本文研究了检索增强生成(RAG)在台湾历史档案中的应用，分析了查询特征和元数据集成策略对检索质量、答案生成和系统性能的影响。
...
Method: 将RAG管道应用于两个繁体中文历史数据集（热兰遮城和台湾省议会公报）及其对应的开放式查询集，系统研究查询特征和元数据集成策略的影响。

Result: 早期元数据集成提高了检索和答案准确性，但也揭示了RAG系统的持续挑战，包括生成过程中的幻觉和处理时间或多跳历史查询的困难。

Conclusion: RAG在历史档案中具有应用价值，但需要解决幻觉和时间/多跳查询处理等挑战。
...
### [135] [REFLEX: Reference-Free Evaluation of Log Summarization via Large Language Model Judgment](https://arxiv.org/abs/2511.07458)
*Priyanka Mudgal*
...
Method: 使用大语言模型作为零样本评估器，在不依赖参考摘要的情况下评估摘要质量，重点关注相关性、信息量和连贯性等维度。
...
TL;DR: 本文比较了大型语言模型和小型判别模型在COVID-19胸部X光检测中的性能，发现小型模型虽然碳足迹较低但存在诊断偏差，而限制LLMs仅输出概率会导致性能下降。Covid-Net模型在实现95.5%最高准确率的同时，碳足迹比GPT-4.5-Preview减少99.9%。


<details>
  <summary>Details</summary>
Motivation: 由于LLMs的通用性和易用性，大型模型经常被用于医疗诊断任务，尽管小型定制模型可能更合适。本文旨在比较生成式模型和判别式模型在COVID-19检测中的表现，并评估其环境影响。

Method: 将LLMs和小型判别模型集成到Mendix应用中检测COVID-19胸部X光，使用判别模型为LLMs提供知识库以提高准确性，共测试了14种不同模型配置。

Result: 小型模型减少了碳足迹但输出偏向阳性诊断且置信度不足；限制LLMs仅输出概率导致准确率和碳足迹表现均差；Covid-Net模型实现95.5%最高准确率，碳足迹比GPT-4.5-Preview减少99.9%。

Conclusion: 生成式工具用于分类任务存在环境风险，小型判别模型在准确性和环境影响方面表现更优，不应将LLMs作为通用AI解决方案。
...
### [137] [Beyond Correctness: Confidence-Aware Reward Modeling for Enhancing Large Language Model Reasoning](https://arxiv.org/abs/2511.07483)
...
<details>
  <summary>Details</summary>
Motivation: 传统基于规则的奖励强化学习在推理链质量和答案一致性方面存在问题，特别是对于小规模基础模型，限制了资源有限组织进行直接强化学习训练。

Method: 设计了一种新颖的基于置信度的奖励模型，不仅惩罚错误答案，还惩罚低置信度的正确答案，并通过静态评估、Best-of-N推理测试和基于PPO的强化学习训练验证有效性。

Result: 该方法在多个STEM基准测试中优于几种最先进的开源奖励模型。

Conclusion: 提出的置信度奖励模型能够有效提升STEM推理能力，促进更稳健和逻辑一致的推理过程。
...
### [138] [Beyond Fact Retrieval: Episodic Memory for RAG with Generative Semantic Workspaces](https://arxiv.org/abs/2511.07587)
*Shreyas Rajesh,Pavan Holur,Chenda Duan,David Chong,Vwani Roychowdhury*

Main category: cs.AI

TL;DR: 提出了生成语义工作空间（GSW）框架，解决大语言模型在长上下文推理中的挑战，通过构建结构化、可解释的情境表示来跟踪实体在事件中的演变。


<details>
  <summary>Details</summary>
Motivation: 现有解决方案主要针对基于事实的检索，无法构建基于时空锚定的叙事表示来跟踪实体在事件中的演变过程。

Method: GSW框架包含操作器（将观察映射到中间语义结构）和协调器（将这些结构整合到持久工作空间中，强制时间、空间和逻辑一致性）。

Result: 在包含10万到100万token的Episodic Memory Benchmark上，GSW比现有RAG基线性能提升高达20%，查询时上下文token减少51%。

Conclusion: GSW为赋予大语言模型人类类似的情景记忆提供了具体蓝图，为能够进行长视野推理的更强大智能体铺平了道路。
...
TL;DR: 本文提出了一种AI增强的团队贡献评估框架，通过整合多维度数据和使用LLM分析来解决团队工作中不公平绩效评估的问题。


<details>
  <summary>Details</summary>
Motivation: 团队中个体贡献的公平评估存在挑战，工作量不均和冲突导致不公平绩效评估，需要手动干预但成本高且困难。

Method: 提出框架将异构数据组织为三个维度九个基准：贡献、互动和角色；客观指标归一化后按维度聚合，结合基尼系数识别冲突；使用LLM架构进行验证和情境分析生成可解释的建议判断。

Result: 设计了包含情感分析、任务保真度、字数/代码行数等实用分析工具，并考虑了偏见防护措施。

Conclusion: 论证了在当前法规和制度政策下的可行性，并概述了实际分析工具、偏见防护、局限性和实践挑战。
...
<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在可验证领域表现出色，但在高风险战略决策中可靠性不足，这源于人类和AI系统中相互强化的认知偏见，威胁到投资决策的可持续性。

Method: 通过对7个前沿级LLM和3个市场风险案例进行系统定性评估，开发了包含7阶段校准序列、4阶段初始化过程和5层保护架构的框架，包括偏见自我监控、人机对抗挑战、伙伴状态验证等功能。

Result: 研究发现：伙伴状态可通过有序校准实现但需要维护协议；当架构漂移和上下文耗尽同时发生时可靠性会下降；解散纪律可避免追求根本错误方向。不同LLM架构存在系统性性能差异。
...
### [141] [ResearchRubrics: A Benchmark of Prompts and Rubrics For Evaluating Deep Research Agents](https://arxiv.org/abs/2511.07685)
...
TL;DR: Deep Research (DR) 是一个新兴的智能体应用，利用大语言模型处理开放式查询。论文提出了ResearchRubrics基准，包含2,500+专家编写的细粒度评估标准，并开发了人类和模型评估协议，发现现有DR系统在事实基础、推理合理性和清晰度方面表现不足。


<details>
  <summary>Details</summary>
Motivation: 由于DR响应冗长多样、存在多种有效解决方案且依赖动态信息源，评估DR系统具有挑战性。需要建立标准化基准来评估事实基础、推理合理性和清晰度。

Method: 构建ResearchRubrics基准，包含2,800+小时人工标注的2,500+专家编写的细粒度评估标准，提出基于概念广度、逻辑嵌套和探索的三维复杂度框架，开发人类和模型评估协议。

Result: 评估多个最先进的DR系统，发现即使是Gemini和OpenAI的DR系统平均符合率也不到68%，主要问题是遗漏隐含上下文和对检索信息推理不足。
...
### [142] [Towards AI-Assisted Generation of Military Training Scenarios](https://arxiv.org/abs/2511.07690)
*Soham Hans,Volkan Ustun,Benjamin Nye,James Sterrett,Matthew Green*

Main category: cs.AI

TL;DR: 本文提出了一种基于大语言模型的多智能体、多模态推理框架，用于自动生成军事训练场景中的关键文档（如作战命令OPORDs），通过分层分解任务并使用专门的LLM智能体来解决复杂场景生成问题。


<details>
  <summary>Details</summary>
Motivation: 传统模拟训练中的复杂场景创建过程耗时耗力，而前LLM时代的AI工具难以生成足够复杂和适应性强的场景。本文旨在利用LLM技术解决这一挑战，提高军事训练场景生成的自动化水平。

Method: 采用多智能体、多模态推理框架，将场景生成分解为层次化的子问题，并为每个子问题定义AI工具的角色：生成选项供人类选择、生成候选产品供人类审批或修改、完全自动生成文本产物。使用专门的LLM智能体处理不同子问题，集成文本场景细节和视觉信息（如地图特征、单位位置）。

Result: 通过概念验证，成功生成了OPORD的机动方案和移动部分，同时估计了地图位置和移动，证明了该框架的可行性和准确性。结果表明LLM驱动的多智能体系统能够生成连贯、细致的文档，并动态适应变化条件。

Conclusion: LLM驱动的多智能体系统在生成连贯、细致的文档和动态适应变化条件方面具有巨大潜力，推动了军事训练场景生成的自动化进程。
...
### [143] [Alignment-Aware Quantization for LLM Safety](https://arxiv.org/abs/2511.07842)
*Sunghyun Wee,Suyoung Kim,Hyeonjin Kim,Kyomin Hwang,Nojun Kwak*

Main category: cs.AI

TL;DR: 本文提出对齐感知量化(AAQ)方法，通过在训练后量化中引入对齐保持对比损失，解决量化过程中模型安全对齐性能下降的问题，实现高效且安全的4位量化。


<details>
  <summary>Details</summary>
Motivation: 传统训练后量化方法仅追求低困惑度，但可能导致模型安全对齐性能显著下降，困惑度不能充分代表模型安全性。

Method: 提出对齐感知量化(AAQ)，在量化流程中集成对齐保持对比(APC)损失，使量化模型模仿安全指令调优模型，同时偏离未对齐的预训练模型。

Result: AAQ方法在LLaMA、Qwen和Mistral等模型上实现稳健的4位(W4A4)量化，在保持安全性的同时，其他方法失败的情况下仍能维持安全对齐。

Conclusion: AAQ方法解决了效率与安全性之间的关键权衡，为实现高效且可信赖的大语言模型铺平了道路。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于logit的水印方法由于随机分割词汇表可能排除最高概率标记而影响生成流畅性，同时欧盟AI法案等法规要求合成内容携带机器可验证的来源标记。

Method: 将词汇表按模型概率降序排列，按秩模k分区，对选定类别应用固定偏置。零比特设置中通过熵自适应门选择奇偶性，多比特设置中根据当前有效载荷数字选择颜色类别。

Result: 实验结果表明WaterMod在保持生成质量的同时实现强水印检测性能，在自然语言生成、数学推理和代码合成等任务中均表现稳健。

Conclusion: WaterMod通过模块化算术同时支持二进制归属和丰富有效载荷，在零比特和多比特设置下都能在保持生成质量的同时提供可靠的水印检测。
...
TL;DR: 本研究开发了一种基于EEG的通信支持系统，针对失语症患者的言语错误进行意图解码，通过软多任务学习框架提高了对错误发音试次的识别准确率。


<details>
  <summary>Details</summary>
Motivation: 失语症严重限制言语交流，但现有的脑机接口技术很少专门为失语症患者开发EEG通信支持系统。本研究旨在填补这一空白，帮助患者在发音错误时仍能有效沟通。

Method: 招募一名表达性失语症患者进行韩语自动言语任务，记录EEG信号并标记正确/错误试次。开发软多任务学习框架，使用最大均值差异正则化，重点关注delta特征，联合优化类别区分和EEG特征分布对齐。

Result: 频谱分析显示错误发音试次在广泛通道上表现出过度的delta功率，前额区域theta-alpha活动增加。所提模型对正确试次达到58.6%准确率，对错误发音试次达到45.5%准确率，比基线方法在后者上提高了45%以上。

Conclusion: 研究证明了基于EEG的辅助系统在失语症患者现实世界不完美言语条件下的可行性，能够有效解码意图，即使在发音错误时也能提供通信支持。
...
TL;DR: SparseRM是一种轻量级可解释的奖励模型，利用稀疏自编码器从LLM表示中提取偏好相关信息，仅需不到1%的可训练参数就能在三个偏好建模任务中超越主流奖励模型。
...
Method: 使用稀疏自编码器分解LLM表示为可解释方向，捕捉偏好相关特征，然后投影计算对齐分数，通过简单奖励头聚合预测偏好分数。

Result: 在三个偏好建模任务中，SparseRM使用不到1%的可训练参数实现了优于大多数主流奖励模型的性能。
...
TL;DR: 本文提出了一种通过语言解释数据的新方法，利用大语言模型生成文本描述，结合影响力估计和CLIP评分选择最有信息量的描述，并通过跨模态迁移分类任务验证效果。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型行为仍被视为黑盒，现有可解释AI方法主要关注模型预测解释，而本文希望从数据角度理解深度学习模型训练过程，让人类能够通过语言这一常见媒介理解数据。

Method: 提出一个流水线方法，利用大语言模型生成数据文本描述，结合外部知识库；引入影响力估计和CLIP评分来选择最有信息量的文本描述；提出跨模态迁移分类基准任务来验证文本描述的有效性。

Result: 在零样本设置实验中，本文的文本描述比其他基线描述更有效；在仅使用图像训练的模型上，在九个图像分类数据集上都成功提升了性能；GPT-4o评估进一步支持了这些结果。

Conclusion: 通过该方法可以深入了解模型决策过程的内在可解释性，证明了通过语言解释数据的有效性。
...
TL;DR: DANS-KGC是一种基于扩散的自适应负采样方法，用于知识图谱补全，通过难度评估、自适应负采样和动态训练机制生成不同难度的负样本，在多个基准数据集上取得最优结果。
...
Method: 提出DANS-KGC框架，包含三个核心组件：难度评估模块（整合语义和结构特征）、自适应负采样模块（使用条件扩散模型和难度感知噪声调度）、动态训练机制（动态调整负样本难度分布）。

Result: 在六个基准数据集上的广泛实验表明，DANS-KGC在UMLS和YAGO3-10数据集的所有三个评估指标上都达到了最先进的结果。

Conclusion: DANS-KGC通过自适应负采样和动态训练机制有效克服了现有负采样策略的局限性，展现出优异的性能和泛化能力。
...
TL;DR: 本研究通过脑电图和卡片分类任务探索人类适应性推理的神经机制，发现delta-theta-alpha振荡协调的动态特征，并与多模态大语言模型对比，揭示人类特有的层次规则抽象能力。


<details>
  <summary>Details</summary>
Motivation: 适应性推理使人类能够在环境规则变化时灵活调整推理策略，但其神经动力学机制尚不清楚，需要揭示人类与AI在适应性推理方面的根本差异。

Method: 使用卡片分类范式结合脑电图记录，分析刺激锁定和反馈锁定的神经活动，并与多模态大语言模型的表现进行对比。

Result: 发现协调的delta-theta-alpha动态：早期delta-theta活动反映探索性监测和规则推断，而枕叶alpha参与表明成功规则识别后的注意稳定化确认。大语言模型仅表现出短期反馈驱动调整，缺乏层次规则抽象。

Conclusion: 识别了人类适应性推理的神经特征，强调需要开发结合振荡反馈协调的脑启发人工智能，以实现真正的上下文敏感适应。
...
<details>
  <summary>Details</summary>
Motivation: 为严重表达性语言障碍患者开发临床沟通支持的想象语音脑机接口，解决现有方法在实时应用中的性能不稳定和环境适应性差的问题。

Method: 集成轻量级条件扩散编码器和卷积分类器，采用双准则早停策略、dropout正则化和分组时间卷积，处理两秒滑动窗口的连续EEG数据流。

Result: 在20次实时试验中达到65% top-1和70% top-2准确率，优于离线评估的50% top-1准确率，在临床约束下保持可靠性能。

Conclusion: 该框架证明了扩散模型在EEG解码中的临床应用可行性，推动了想象语音脑机接口向临床沟通支持的转化。
...
TL;DR: 本文提出了同构数学问题生成（IMPG）任务，通过计算蓝图同构双胞胎（CBIT）框架自动生成结构一致的数学问题变体，在商业教育平台上验证了其有效性和成本效益。


<details>
  <summary>Details</summary>
Motivation: 个性化数学教育快速发展，急需大量相似练习题，但现有研究主要关注数据增强而非直接教育部署，需要填补这一空白。

Method: 提出CBIT框架，采用元级生成和基于模板的选择性变体方法，通过连续精炼实现自动IMPG。
...
TL;DR: 本文介绍了一个实时无线想象语音脑电图解码系统，旨在实现灵活和日常使用的脑机接口技术。该系统通过用户识别模块提供个性化服务，使用实验室流层管理实时EEG信号流，在4类分类任务中达到62.00%（有线设备）和46.67%（无线头戴设备）的准确率。


<details>
  <summary>Details</summary>
Motivation: 当前脑机接口研究主要局限于静态和固定环境，限制了其实际应用。为了推动脑机接口走向实际应用，需要开发能够在日常环境中使用的灵活系统。

Method: 开发了一个实时无线想象语音EEG解码系统，使用实验室流层管理连续实时EEG信号流，包含用户识别模块提供个性化服务，支持从有线EEG设备扩展到便携无线硬件。

Result: 系统在4类想象语音EEG信号分类任务中，在有线设备上达到62.00%的准确率，在便携无线头戴设备上达到46.67%的准确率。
...
TL;DR: 提出了Thinker分层思维模型，通过多轮交互进行深度搜索，使推理过程可监督和可验证。它将复杂问题分解为可独立解决的子问题，支持知识库和网页搜索，并通过逻辑函数传递子问题间的依赖关系，增强逻辑连贯性。
...
Method: 分层思维模型，将复杂问题分解为子问题，每个子问题用自然语言和等效逻辑函数双重表示，执行知识边界确定以避免不必要的外部搜索。

Result: 仅需数百个训练样本，Thinker性能即可与现有基线竞争；使用完整训练集时，在各种数据集和模型规模上显著优于现有方法。

Conclusion: Thinker通过分层分解和逻辑函数表示，实现了可监督的推理过程，在少量训练数据下即表现出色，为LLMs的外部知识检索提供了有效解决方案。
...
TL;DR: 本文提出了TimeFlow，一种基于随机微分方程(SDE)的流匹配框架，用于高效生成高质量时间序列数据，解决了传统扩散模型计算效率低和普通流匹配方法无法有效建模随机性的问题。


<details>
  <summary>Details</summary>
Motivation: 时间序列数据生成对下游挖掘任务至关重要，但现有方法面临两大挑战：扩散模型计算效率低，而传统流匹配方法无法有效建模时间序列固有的随机性和局部变化。随机微分方程(SDE)天然适合建模随机性，因此提出结合SDE的流匹配框架。

Method: 提出TimeFlow框架，采用基于SDE的流匹配方法，设计组件分解的速度场来捕捉时间序列的多方面结构，并在标准流匹配优化中增加随机项以增强表达能力。采用编码器专用架构，支持无条件和条件生成任务。

Result: 在多个数据集上的广泛实验表明，TimeFlow在生成质量、多样性和效率方面持续优于强基线方法。

Conclusion: TimeFlow是一个灵活通用的时间序列生成框架，通过SDE-based流匹配有效平衡了生成质量与计算效率，为下游时间序列挖掘任务提供了高质量的数据支持。
...
TL;DR: VARS是一种基于图表示的心电图诊断方法，通过将ECG信号转换为图结构来统一处理不同配置的信号，提高风险信号检测能力，并在三个数据集上超越现有最佳模型。
...
Result: 在三个不同的ECG数据集上评估，VARS在所有数据集上持续超越现有最先进模型，并在识别风险信号方面表现出显著改进，同时提供可解释性。
...
TL;DR: 提出了一种细粒度反事实解释框架，通过对象级和部分级解释来增强模型误分类的可解释性，量化相似性和权重贡献，并引入基于Shapley值的显著性分割模块。
...
Method: 提出非生成式细粒度反事实解释框架，量化正确分类和误分类样本之间感兴趣区域的相似性和组件贡献权重，并引入基于Shapley值贡献的显著性分割模块来隔离具有区域特定相关性的特征。

Result: 大量实验证明该方法在捕捉更细粒度、直观有意义的区域方面优于现有细粒度方法。

Conclusion: 该框架能够有效识别导致模型误分类的细粒度特征，并定位影响反事实调整的主导局部特征，显著提升了视觉可解释性。
...
TL;DR: MSLR是中国首个基于真实司法决策的多步骤法律推理数据集，采用IRAC框架建模结构化专家推理，并通过人机协作标注流程生成细粒度推理注释。评估显示LLMs在法律推理上表现一般，但自主生成的思维链提示能改善推理质量。


<details>
  <summary>Details</summary>
Motivation: 现有法律基准常混淆事实回忆与真实推理，忽视推理过程和质量，需要开发更准确评估LLM法律推理能力的数据集。

Method: 基于IRAC框架构建多步骤法律推理数据集，设计可扩展的人机协作标注流程生成细粒度推理注释，评估多种LLM并测试自主生成思维链提示的效果。

Result: LLMs在MSLR上表现中等，显示适应复杂法律推理的挑战；自主生成的思维链提示优于人工设计提示，能提高推理连贯性和质量。

Conclusion: MSLR推进了LLM推理和思维链策略研究，为未来研究提供开放资源，数据集和代码已公开。
...
TL;DR: ST-SAM是一种新颖高效的时空自注意力模型，用于交通流量预测，通过联合捕获时空依赖关系，在准确性和效率上都显著优于现有方法。
...
Method: 使用区域嵌入层学习区域的时间特定嵌入，然后基于自注意力机制的时空依赖学习模块来捕获附近和远处区域的联合时空依赖关系。

Result: 在两个真实数据集上的实验表明，ST-SAM在RMSE上平均提升15%，MAPE上提升17%，训练时间提升32倍。

Conclusion: ST-SAM完全依赖自注意力来捕获局部和全局时空相关性，使其既有效又高效，显著优于现有最先进方法。
...
Result: 显著提高了STS及相邻ROI的大脑对齐度，并在与训练数据相关的社交认知任务（情景喜剧讽刺检测）上表现出性能提升。
...
TL;DR: 本文提出了一种验证本体语义陷阱的新方法VSPO，通过微调LLaMA-3.1-8B-Instruct模型生成能力问题来检测本体设计中的语义错误，相比GPT-4.1在精度和召回率上分别提升26%和28.2%。


<details>
  <summary>Details</summary>
Motivation: 传统基于相似度的方法无法有效检测语义陷阱（如"Misusing allValuesFrom"），而规则方法又不可靠，需要一种能自动验证本体语义一致性的方法。

Method: 使用LLMs生成类和属性的自然语言定义，通过移除公理或改变逻辑运算符引入定义与本体间的语义偏差，然后微调LLaMA-3.1-8B-Instruct模型生成验证这些语义差异的能力问题。

Result: 微调模型在生成验证陷阱的能力问题上表现优于基线，比GPT-4.1精度高26%，召回率高28.2%，能检测比现有公共数据集更广泛的建模错误。

Conclusion: 该方法首次使用LLMs针对语义陷阱验证生成能力问题，显著减少人工工作量，同时改善本体与专家知识间的语义对齐。
...
TL;DR: 本文提出PN-GNN方法，通过聚合推理路径上的节点邻居嵌入来增强GNN的逻辑表达能力，在知识图谱推理中表现出更强的表达能力和竞争力。
...
TL;DR: 本文提出了一种将多元时间序列转换为单变量时间序列的异常检测方法，结合FCM聚类、模糊积分和HMM模型来检测异常。
...
Method: 使用FCM聚类和模糊积分技术将多元时间序列转换为单变量时间序列，然后应用隐马尔可夫模型(HMM)进行异常检测。

Result: 构建了基于HMM的异常检测器，并比较了多种转换方法的效果。

Conclusion: 通过实验研究和比较分析验证了所提出的多元时间序列异常检测方法的有效性。
...
TL;DR: 提出新的扰动框架评估LLMs在复杂环境中的数学推理能力，通过注入语义无关的扰动句子并逐步增加扰动强度，发现LLMs对含数字的扰动更敏感，性能下降明显，且可能依赖记忆模板而非逻辑推理。


<details>
  <summary>Details</summary>
Motivation: 探讨LLMs是否真正具备数学理解能力，通过扰动测试来评估其在复杂环境中的推理能力。

Method: 提出扰动框架，注入语义无关的扰动句子并逐步增加扰动强度，同时使用核心提问指令缺失的扰动方法分析LLMs的解题机制。

Result: LLMs在面对不含数字的扰动时表现稳定但存在鲁棒性边界；面对含数字的扰动时性能显著下降，开源模型下降近10%以上，最大下降达51.55%；商业LLMs也有3%-10%的性能下降；在核心提问指令缺失时仍能保持20%-40%的准确率。

Conclusion: 当前LLMs在推理能力上存在缺陷和局限性，对含数字的扰动更敏感，可能依赖记忆模板而非逻辑推理，这对LLMs的进一步发展具有重要意义。
...
TL;DR: 提出了一种知识增强的长链推理框架，将大语言模型与基于知识图谱的多跳推理链相结合，用于解决生物分子推理中的逻辑不一致和领域知识缺乏问题。


<details>
  <summary>Details</summary>
Motivation: 现有的方法在处理生物分子问题时存在逻辑不一致、缺乏领域知识基础、难以捕捉长程机制依赖等问题，需要改进推理的可靠性和一致性。

Method: 通过知识图谱上的引导多跳遍历和剪枝构建机制链，然后将这些链整合到监督微调中以提高事实基础，并通过强化学习进一步优化推理可靠性和一致性。
...
TL;DR: 提出了一种基于心理学理论的Scaffold Reasoning框架用于代码调试，通过三个流（Scaffold Stream、Analytic Stream、Integration Stream）平衡推理复杂性和计算效率，在DebugBench上达到88.91%的通过率和5.36秒的平均推理时间。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs在推理算法方面已展现强大能力，但如何平衡推理步骤的复杂性和计算效率仍是未解问题。现有研究将LLM输出视为系统1和系统2，但对系统2推理的深入探索仍不足。
...
Conclusion: 该框架与人类认知过程一致，分析揭示了不同认知路径在不同问题难度和错误类型上的优势与局限。
...
<details>
  <summary>Details</summary>
Motivation: 由于大语言模型的快速发展和广泛应用导致计算资源需求激增，而测试时扩展进一步加剧了模型能力与资源消耗之间的矛盾，因此需要一种能够准确反映LLM效率的统一度量标准。

Method: 基于压缩与智能之间的相关性，引入信息容量作为模型效率的度量，通过文本压缩性能相对于计算复杂度的比值来评估模型效率。对主流开源模型进行实证评估。

Result: 实证评估表明，同一系列中不同规模的模型表现出一致的信息容量。该指标能够实现跨模型系列的公平效率比较和系列内性能的准确预测。信息容量还考虑了分词器效率的影响。

Conclusion: 信息容量是一个有效的模型效率度量标准，能够统一评估不同规模和架构的LLM，并考虑了分词器效率、预训练数据和专家混合架构等因素的影响。
...
<details>
  <summary>Details</summary>
Motivation: 多变量时间序列异常检测在科学和工程中有广泛应用，但由于需要同时考虑时间和变量关系而具有挑战性。

Method: 使用滑动窗口生成多变量子序列，应用扩展模糊聚类，采用重构准则重建子序列，构建置信指数量化异常水平，并使用粒子群优化进行优化。
...
TL;DR: 本文开发了一个评估再保险中大型语言模型可靠性的审慎框架，通过五支柱架构将监管期望转化为可衡量的生命周期控制，并在RAIRAB基准测试中验证了检索增强配置能显著提高准确性、减少幻觉并增强透明度。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型在再保险行业的应用增加，需要建立审慎框架来评估其可靠性，确保符合现有监管标准如Solvency II、SR 11-7等，降低风险转移和资本配置中的信息摩擦。

Method: 提出五支柱架构（治理、数据溯源、保证、韧性、监管对齐），开发RAIRAB基准来评估治理嵌入的LLM是否满足基础性、透明度和问责制的审慎标准，并在六个任务系列中进行测试。

Result: 检索增强配置实现了更高的基础准确性（0.90），幻觉和解释漂移减少约40%，透明度几乎翻倍，有效降低了风险转移和资本配置中的信息摩擦。

Conclusion: 现有审慎原则已经能够容纳可靠的AI系统，前提是治理明确、数据可追溯且保证可验证，这为再保险行业安全采用AI技术提供了可行路径。
...
TL;DR: 本文通过参数化复杂性理论重新审视Pearl因果层次(PCH)的可满足性问题，识别出首个可处理性的入口点，包括针对关键概率和反事实片段的固定参数和XP算法，以及匹配的硬度结果。
...
Method: 使用参数化复杂性方法，利用原始树宽和变量数量等参数，为关键概率和反事实片段开发固定参数和XP算法。技术上偏离了通常用于树宽算法的动态规划范式，转而利用结构良好的因果模型的结构特征。

Result: 获得了针对概率和反事实片段可满足性问题的固定参数和XP算法，同时提供了匹配的硬度结果，描绘了可处理性的界限。

Conclusion: 本文为因果推理提供了一个新的算法工具包，通过参数化复杂性理论识别了PCH可满足性问题的首个可处理性入口点，并建立了可处理性的界限。
...
TL;DR: 本研究探讨了使用XAI技术进行特征降维对注塑件质量分类的影响，通过SHAP、Grad-CAM和LIME分析LSTM模型中的特征重要性，将19个输入特征减少到9个和6个，评估了模型准确性、推理速度和可解释性之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 机器学习模型在工业质量控制中应用受限，主要由于模型复杂性和可解释性不足，以及工业设备传感器技术不完善导致数据采集不完整。可解释人工智能(XAI)能够提供模型决策洞察并识别分类中最相关的特征。
...
TL;DR: 研究探讨了在注塑成型过程中使用合成数据训练LSTM模型的可行性，通过模拟生产周期生成合成数据，并寻找合成数据与真实数据的最佳比例来提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 机器学习在工业过程中具有巨大潜力，但数据采集耗时且昂贵。合成数据可以增强不足的数据集并提高机器学习模型的鲁棒性。

Method: 通过模拟生产周期生成合成数据，并将其加入训练数据集，使用现有的LSTM架构进行训练，通过迭代实验寻找合成数据与真实数据的最佳比例。
...
TL;DR: SpeechCARE是一个多模态语音处理管道，利用预训练的多语言声学和语言转换器模型来检测与认知障碍相关的细微语音线索，通过动态融合架构整合声学、语言学和人口统计学输入，在阿尔茨海默病和相关痴呆症的早期检测中表现出色。
...
Method: SpeechCARE采用多模态语音处理管道，利用预训练的多语言声学和语言转换器模型，基于专家混合范式构建动态融合架构，整合声学、语言学和人口统计学输入，包含自动转录、LLM异常检测和任务识别等预处理步骤。

Result: SpeechCARE在分类认知健康、轻度认知障碍和阿尔茨海默病患者时达到AUC=0.88和F1=0.72，在MCI检测中达到AUC=0.90和F1=0.62。偏倚分析显示除80岁以上成人外差异最小。

Conclusion: SpeechCARE在认知障碍检测方面表现出色，未来工作包括在真实护理环境中部署和在纽约市为代表性不足人群提供EHR集成可解释性。
...
TL;DR: 日本开发的商业级开源图像生成模型'oboro:'，使用版权清理图像训练，旨在解决动漫产业劳动力短缺问题，首次在日本完全开发并开源。


<details>
  <summary>Details</summary>
Motivation: 解决日本动漫制作行业劳动力短缺等挑战，开发本土化的图像生成模型，促进国内AI开发生态系统。

Method: 从零开始开发'oboro:'图像生成模型，仅使用版权清理图像进行训练，采用特殊架构设计以在有限数据集上生成高质量图像。

Result: 成功开发出'oboro:'图像生成模型，公开发布基础模型权重和推理代码，成为日本首个完全开发的开源商业级图像生成AI。

Conclusion: 该项目标志着日本在图像生成AI领域的重大突破，通过保持开发过程透明度，旨在为日本AI研究社区做出贡献并促进国内AI生态系统发展。
...
<details>
  <summary>Details</summary>
Motivation: 随着机器学习模型对大规模数据的依赖，用户数据保护问题日益突出。现有不可学习样本方法缺乏理论保证且在混合干净数据时失效，需要更可靠的方法来保护用户数据隐私。

Method: 提出基于贝叶斯误差最大化的优化方法，使用投影梯度上升进行高效求解，系统性地构建不可学习样本。

Result: 实验结果表明，该方法能有效限制数据可学习性，在多种数据集和模型架构下均表现良好，且在混合干净数据时仍保持有效性。

Conclusion: 通过最大化贝叶斯误差构建的不可学习样本具有理论保证和实际有效性，为保护用户数据隐私提供了可靠的技术方案。
...
TL;DR: 本文介绍了EHRStruct基准测试，用于评估大语言模型在结构化电子健康记录数据上的性能，包含11个代表性任务和2200个评估样本。通过评估20个先进LLM，发现结构化EHR任务对模型理解和推理能力要求很高，并提出了EHRMaster方法实现最优性能。
...
Method: 开发了EHRStruct基准测试，包含11个代表性临床任务和2200个评估样本，评估了20个先进LLM，并分析了输入格式、少样本泛化和微调策略等关键因素。

Result: 研究发现结构化EHR任务对LLM的理解和推理能力要求很高，提出的EHRMaster方法实现了最先进的性能。

Conclusion: EHRStruct为结构化EHR数据上的LLM评估提供了标准化框架，EHRMaster方法在应对复杂临床任务方面表现出色。
...
Result: 在七个药物发现案例中评估显示MADD性能优于现有LLM解决方案，成功应用于五个生物靶点的AI优先药物设计，并发布了识别出的命中分子。
...
TL;DR: GAC是一种新的动作生成方法，通过方向向量和可学习浓度参数分解动作生成，在保持球面分布几何优势的同时简化计算，在连续控制任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 解决高斯策略在连续控制中因无界支持而需要临时压缩函数的问题，以及vMF分布计算复杂难以实用的问题。

Method: 将动作生成分解为方向向量和可学习浓度参数，实现确定性动作和均匀球面噪声之间的高效插值，参数数量从2d减少到d+1，复杂度从O(dk)降低到O(d)。

Result: 在6个MuJoCo基准测试中一致匹配或超越最先进方法，在Ant-v4上比SAC提升37.6%，在4/6任务中取得最佳结果。

Conclusion: 鲁棒高效的连续控制不需要复杂分布，而是需要对动作空间几何结构的合理尊重。
...
TL;DR: 本文提出了一种敏感性感知任务向量插入框架（STV），通过分析激活差异的结构模式来确定任务向量的插入位置和内容，解决了多模态模型在少样本学习中的上下文长度限制和推理成本问题。


<details>
  <summary>Details</summary>
Motivation: 大型多模态模型在上下文学习中表现出潜力，但由于上下文长度有限和推理成本高，扩展到多样本设置仍然困难。现有的任务向量方法要么忽略了插入位置的重要性，要么难以确定每个位置的合适值。

Method: 提出STV框架，基于查询-上下文对之间的激活差异结构模式识别敏感位置，为每个位置构建预聚类激活库，然后使用强化学习选择最合适的值进行插入。

Result: 在多种多模态模型（如Qwen-VL、Idefics-2）和任务（如VizWiz、OK-VQA）上评估STV，证明其有效性，相比之前的方法显示出持续改进和强泛化能力。

Conclusion: STV框架通过敏感性感知的任务向量插入，有效提升了多模态模型在多样本上下文学习中的性能，具有很好的泛化性。
...
TL;DR: 提出了DiagramIR，一个用于几何图形的自动可扩展评估管道，基于LaTeX TikZ代码的中间表示，比LLM-as-a-Judge等基线方法更符合人类评分者，并显著降低了推理成本。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型越来越多地被用作学习工具，但大多数工具仅限于文本，限制了在数学等需要可视化的领域的实用性。虽然LLM能够生成编译成教育图形的代码，但可扩展的图形评估仍是一个主要瓶颈。

Method: 提出DiagramIR评估管道，利用LaTeX TikZ代码的中间表示进行自动和可扩展的几何图形评估。

Result: DiagramIR方法比LLM-as-a-Judge等基线方法具有更高的人类评分者一致性，同时使GPT-4.1-Mini等较小模型在10倍降低推理成本的情况下表现与GPT-5等较大模型相当。
...
<details>
  <summary>Details</summary>
Motivation: 传统的人类中心软件开发知识共享环境正在被智能体中心实践所颠覆，AI编程智能体缺乏有价值的共享学习资源库。

Method: 引入Spark共享智能体记忆架构，使AI编程智能体能够贡献和获取持续演化的经验记忆，作为新知识的存储库。

Result: Spark的推荐提高了通用代码生成模型的代码质量，30亿参数的小型模型能够匹配更大规模最先进模型的代码质量；推荐的有用性在最高两个等级达到98.2%。

Conclusion: Spark架构能够有效提升AI编程智能体的代码生成质量，实现集体持续学习，弥补了传统知识共享环境与智能体中心实践之间的差距。
...
Result: 平台能实时处理10万行数据集，云平台可扩展支持多用户同时请求。使用两个数据集评估显示性能良好。

Conclusion: 云数据可视化应用显著减少数据分析过程的人工输入，同时保持高质量的可视化输出和用户体验。
...
TL;DR: 本文提出了一种利用自组织映射(SOMs)从语言模型中提取多个拒绝方向的新方法，证明SOMs能够泛化先前工作的均值差异技术，并通过实验验证多方向消融比单方向基线更有效地抑制模型的拒绝行为。
...
Method: 首先证明SOMs能够泛化均值差异技术，然后在有害提示表征上训练SOMs识别多个神经元，通过从每个神经元减去无害表征的质心来推导多个拒绝方向。

Result: 在广泛实验设置中验证了该方法，证明从模型内部消融多个方向不仅优于单方向基线，还优于专门的越狱算法，能有效抑制拒绝行为。
...
TL;DR: FaithEval用于评估多模态推理中的忠实性，FaithAct框架通过强制证据基础提高感知忠实性达26%而不降低任务准确性。
...
Method: 提出FaithEval评估框架量化步骤级和链级忠实性，并开发FaithAct框架在每一步推理中强制证据基础。

Result: 在多个推理基准测试中，FaithAct相比基于提示和工具增强的基线方法，将感知忠实性提高了26%，同时不降低任务准确性。
...
<details>
  <summary>Details</summary>
Motivation: 数据集完整性对AI系统的安全性和可靠性至关重要，特别是在自动驾驶领域。当前缺乏系统化的安全数据集开发框架，需要确保数据集符合安全标准并能够识别和缓解由数据集不足引起的风险。

Method: 提出结构化框架，引入AI数据飞轮和数据集生命周期概念，包含数据收集、标注、管理和维护流程。整合严格的安全分析来识别危险源，定义建立数据集安全要求的流程，并提出验证和验证策略。

Result: 开发了一个系统化的安全数据集开发框架，能够识别数据集相关的安全隐患并制定相应的风险缓解措施，确保数据集符合ISO/PAS 8800安全标准要求。

Conclusion: 通过整合数据集安全最佳实践和新兴趋势，该框架有助于推进自动驾驶应用中稳健、安全保证的AI系统发展，为未来研究方向提供重要见解。
...
TL;DR: 提出了一种轻量级模块化的LLM安全补丁方法，通过添加仅占0.003%参数的可学习前缀来修复安全漏洞，实现与新一代安全对齐模型相当的安全改进。


<details>
  <summary>Details</summary>
Motivation: 现有LLM版本更新成本高、频率低且难以定制，导致已发布模型存在已知安全漏洞，需要一种快速修复机制。

Method: 在现有模型前添加紧凑的可学习前缀作为"补丁"，仅引入极少额外参数，引导模型行为向更安全的参考模型靠拢。

Result: 在毒性缓解、偏见减少和有害拒绝三个关键领域，补丁方法实现了与新一代安全对齐模型相当的安全改进，同时保持流畅性。
...
TL;DR: 本文研究了AI系统（特别是LLMs）与人类在数学问题趣味性和难度判断上的一致性，通过两个实证研究比较了不同数学经验水平人群与LLMs的评估结果。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统越来越多地参与数学研究和教育，了解它们对人类趣味性判断的认知对齐程度变得重要，这关系到AI能否成为有效的数学思维伙伴。

Method: 通过两个实证研究，分别对众包平台参与者和国际数学奥林匹克竞赛选手进行调查，比较人类与多种LLMs对数学问题趣味性和难度的评估。

Result: 研究发现LLMs在总体上与人类的趣味性概念有广泛共识，但未能完全捕捉人类判断的分布模式；大多数LLMs与人类选择趣味性理由的关联性较弱。

Conclusion: 当前LLMs在捕捉人类数学趣味性判断方面既有潜力也有局限，这为数学AI思维伙伴的发展提供了重要启示。
...
TL;DR: 这篇论文系统性地概述了视频生成模型向世界模型发展的演变过程，将现代视频基础模型概念化为隐式世界模型和视频渲染器的组合，并追踪了四代视频生成技术的核心能力演进。


<details>
  <summary>Details</summary>
Motivation: 视频生成领域正在从生成视觉吸引力片段转向构建支持交互和保持物理合理性的虚拟环境，这指向了视频基础模型作为隐式世界模型的出现。

Method: 将现代视频基础模型分解为两个核心组件：隐式世界模型（编码物理规律、交互动态和智能体行为知识）和视频渲染器（将潜在模拟转换为真实视觉观察）。

Result: 论文追踪了四代视频生成技术的演进，每代核心能力逐步提升，最终形成具有内在物理合理性、实时多模态交互和多时空尺度规划能力的世界模型。
...
### [188] [From IDs to Semantics: A Generative Framework for Cross-Domain Recommendation with Adaptive Semantic Tokenization](https://arxiv.org/abs/2511.08006)
*Peiyu Hu,Wayne Lu,Jia Wang*

Main category: cs.IR

TL;DR: GenCDR是一个生成式跨域推荐框架，通过领域自适应分词、跨域自回归推荐和领域感知前缀树来解决传统方法依赖共享用户/物品ID的问题，显著提升了推荐准确性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 传统跨域推荐方法依赖共享用户/物品ID，这在现实场景中往往不可得。现有方法面临物品ID分词困境（导致词汇爆炸且无法捕捉高阶协同知识）和领域特定建模不足的问题。

Method: GenCDR包含三个核心模块：1) 领域自适应分词模块，通过动态路由在通用编码器和领域特定适配器之间生成解耦的语义ID；2) 跨域自回归推荐模块，融合通用和领域特定兴趣建模用户偏好；3) 领域感知前缀树，实现高效准确的生成。

Result: 在多个真实世界数据集上的广泛实验表明，GenCDR显著优于最先进的基线方法。

Conclusion: GenCDR通过创新的生成式框架有效解决了跨域推荐中的关键挑战，为实际应用提供了可行的解决方案。
...
### [189] [BiCA: Effective Biomedical Dense Retrieval with Citation-Aware Hard Negatives](https://arxiv.org/abs/2511.08029)
*Aarush Sinha,Pavan Kumar S,Roshan Balaji,Nirav Pravinbhai Bhatt*

Main category: cs.IR

TL;DR: 提出BiCA方法，利用PubMed文章的引用链接生成高质量负样本，改进生物医学领域的密集检索模型性能


<details>
  <summary>Details</summary>
Motivation: 生物医学和科学领域中，由于源文档和负样本文档难以区分，传统的负样本挖掘方法面临挑战。引用文档天然与源文档具有上下文相关性但不是重复文档，适合作为硬负样本。

Method: 提出BiCA方法，利用20,000篇PubMed文章的引用链接进行硬负样本挖掘，微调GTE_small和GTE_Base模型。

Result: 在BEIR数据集上的零样本密集检索任务中，使用nDCG@10指标在领域内和领域外任务上均获得持续改进，并在LoTTE的长尾主题上使用Success@5指标超越基线。
...
### [190] [DiffuGR: Generative Document Retrieval with Diffusion Language Models](https://arxiv.org/abs/2511.08150)
...
TL;DR: DiffuGR是一种基于扩散语言模型的生成式文档检索方法，将文档ID生成建模为离散扩散过程，通过可控的去噪步骤并行生成和优化文档ID，解决了传统自回归方法在检索效率和准确性之间的平衡问题。


<details>
  <summary>Details</summary>
Motivation: 解决现有基于自回归生成模型的生成式检索方法存在的两个主要问题：(1)文档ID生成与自然语言生成不匹配，早期错误会导致完全错误的检索；(2)无法动态平衡检索效率与准确性之间的权衡。
...
Conclusion: 非自回归扩散模型是生成式文档检索的一个实用且有效的替代方案，提供了显式的运行时控制来平衡质量与延迟的权衡。
...
### [191] [LLaDA-Rec: Discrete Diffusion for Parallel Semantic ID Generation in Generative Recommendation](https://arxiv.org/abs/2511.06254)
*Teng Shi,Chenglei Shen,Weijie Yu,Shen Nie,Chongxuan Li,Xiao Zhang,Ming He,Yan Han,Jun Xu*

Main category: cs.IR

TL;DR: LLaDA-Rec是一个基于离散扩散的生成式推荐框架，通过并行语义ID生成解决传统自回归模型的两个固有局限：单向约束和错误累积问题。


<details>
  <summary>Details</summary>
Motivation: 现有自回归模型存在两个内在限制：单向约束（因果注意力限制每个标记只能关注其前驱）阻碍全局语义建模，以及错误累积（固定从左到右生成顺序导致早期标记预测错误传播到后续标记）。

Method: 提出LLaDA-Rec离散扩散框架，包含三个关键设计：并行标记化方案生成语义ID用于双向建模；用户历史级和下一项目级两种掩码机制捕获项目间序列依赖和项目内语义关系；适配波束搜索策略实现自适应顺序离散扩散解码。

Result: 在三个真实世界数据集上的实验表明，LLaDA-Rec在一致优于基于ID的方法和最先进的生成式推荐器，确立了离散扩散作为生成式推荐的新范式。

Conclusion: 离散扩散框架通过结合双向注意力和自适应生成顺序，更有效地建模项目间和项目内依赖关系，并缓解错误累积问题，为生成式推荐提供了新的解决方案。
...
### [192] [MARC: Multimodal and Multi-Task Agentic Retrieval-Augmented Generation for Cold-Start Recommender System](https://arxiv.org/abs/2511.08181)
*Seung Hwan Cho,Yujin Yang,Danik Baeck,Minjoo Kim,Young-Min Kim,Heejung Lee,Sangjin Park*

Main category: cs.IR

TL;DR: MARC是一个基于Agentic RAG的多模态多任务鸡尾酒推荐系统，在冷启动条件下利用图数据库生成高质量、上下文相关的答案。
...
Method: 采用基于Agentic RAG的方法，构建图数据库处理鸡尾酒数据，通过任务识别路由器和反思过程生成答案。

Result: 使用200个手工制作的问题进行评估，通过LLM和人工评估显示，基于图数据库的答案质量优于简单向量数据库。

Conclusion: MARC系统在冷启动条件下能够生成高质量的鸡尾酒推荐，图数据库在推荐系统中具有显著优势。
...
### [193] [A Hybrid Multimodal Deep Learning Framework for Intelligent Fashion Recommendation](https://arxiv.org/abs/2511.07573)
*Kamand Kalashi,Babak Teimourpour*

Main category: cs.IR

TL;DR: 本文提出了一种用于时尚推荐的混合多模态深度学习框架，同时处理服装搭配兼容性预测和互补物品检索两个关键任务。模型利用CLIP架构的视觉和文本编码器获取时尚物品的联合潜在表示，通过transformer编码器处理。在Polyvore数据集上，兼容性预测AUC达到0.95，互补物品检索在FITB指标下准确率达69.24%。


<details>
  <summary>Details</summary>
Motivation: 随着在线时尚平台的快速发展，对能够理解视觉和文本线索的智能推荐系统的需求日益增长。需要开发能够同时处理多种推荐任务的多模态方法。

Method: 使用CLIP架构的视觉和文本编码器获取时尚物品的联合潜在表示，将这些表示整合为统一特征向量，通过transformer编码器处理。引入'搭配标记'来建模物品间的整体关系，以及'目标物品标记'来表示期望的物品描述进行检索。

Result: 在Polyvore数据集上，兼容性预测任务达到0.95的AUC值，互补物品检索任务在Fill-in-the-Blank指标下达到69.24%的准确率。
...
### [194] [Bid Farewell to Seesaw: Towards Accurate Long-tail Session-based Recommendation via Dual Constraints of Hybrid Intents](https://arxiv.org/abs/2511.08378)
*Xiao Wang,Ke Qin,Dongyang Zhang,Xiurui Xie,Shuang Liang*

Main category: cs.IR

TL;DR: HID框架通过混合意图学习和意图约束损失，解决了会话推荐中长尾项目与准确性之间的冲突，实现了从"跷跷板"到"双赢"的转变。


<details>
  <summary>Details</summary>
Motivation: 解决会话推荐中长尾项目导致的推荐多样性问题，现有方法在提升长尾项目时会造成准确性下降，存在"跷跷板"效应。

Method: 提出HID框架，包含混合意图学习（使用属性感知谱聚类重构项目到意图的映射）和意图约束损失（包含多样性和准确性约束）。

Result: 在多个SBR模型和数据集上的广泛实验表明，HID能够同时提升长尾性能和推荐准确性，在长尾推荐系统中建立了新的最先进性能。

Conclusion: HID框架通过引入混合意图的双重约束，有效解决了长尾项目与准确性之间的冲突，实现了推荐系统的双赢优化。
